...
export class FigmaClient {
  ...
  private extractNodeStyles(node: Figma.Node): ComponentStyles {
    ...
    if ('layoutMode' in node) {
      ...
      if ('paddingLeft' in node) styles.paddingLeft = node.paddingLeft as number
      if ('paddingRight' in node) styles.paddingRight = node.paddingRight as number
      if ('paddingTop' in node) styles.paddingTop = node.paddingTop as number
      if ('paddingBottom' in node) styles.paddingBottom = node.paddingBottom as number
      if ('itemSpacing' in node) styles.itemSpacing = node.itemSpacing as number
    }
