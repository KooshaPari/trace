...
export class DesignSync {
	...
	private async pullFromFigma(): Promise<number> {
		...
		const figmaComponents = await this.client.getComponents(
			this.config.figmaFileKey,
		);
	 ...
	 */
	private async updateComponentMetadata(
			...
			if (figmaComponent) {
				...
				try {
					...
					if (stats.mtime > lastSync) {
						conflicts.push(
							`${component.name}: Modified locally after last sync with Figma`,
						);
					}
				} catch {
					// File doesn't exist locally
					conflicts.push(
						`${component.name}: Exists in Figma but not found locally`,
					);
				}
			} else {
