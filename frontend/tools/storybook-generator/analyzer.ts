...
function extractComponentName(sourceFile: SourceFile): string | null {
	...
	for (const decl of exportAssignments) {
		...
		if (
			...
			const returnStatements = initializer.getDescendantsOfKind(
...
function extractCVAVariants(sourceFile: SourceFile): VariantInfo[] {
	...
	for (const call of callExpressions) {
		...
		if (
			defaultVariantsProp &&
			defaultVariantsProp.getKind() === SyntaxKind.PropertyAssignment
		) {
			...
			if (
				defaultValue &&
				defaultValue.getKind() === SyntaxKind.ObjectLiteralExpression
			) {
				const defaultObj = defaultValue.asKind(
					SyntaxKind.ObjectLiteralExpression,
				);
				if (defaultObj) {
