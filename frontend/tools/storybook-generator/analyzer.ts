...
function extractCVAVariants(sourceFile: SourceFile): VariantInfo[] {
  ...
  for (const call of callExpressions) {
    ...
    if (!variantsValue || variantsValue.getKind() !== SyntaxKind.ObjectLiteralExpression) continue
    ...
    for (const prop of variantsObj.getProperties()) {
      ...
      if (!variantValue || variantValue.getKind() !== SyntaxKind.ObjectLiteralExpression) continue

      const variantObj = variantValue.asKind(SyntaxKind.ObjectLiteralExpression)
      if (!variantObj) continue

      const options = variantObj.getProperties().map((p) => p.getName().replace(/['"]/g, ''))
