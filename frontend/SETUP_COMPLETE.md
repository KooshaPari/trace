...
- `.oxlintrc.json.strict` - Strict configuration (backup of proposed changes)
...
**Recommended**: Replace current config with `.oxlintrc.json.strict` which:
...
1. **Review strict linting config**
   ...
   diff .oxlintrc.json .oxlintrc.json.strict

   # Test strict config
   bunx oxlint --type-aware apps/web/src --config .oxlintrc.json.strict
   ```
...
| `.oxlintrc.json.strict` | Proposed strict config | ✅ Ready to apply |
