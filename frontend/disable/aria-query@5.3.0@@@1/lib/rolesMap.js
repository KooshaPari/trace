...
var _ariaGraphicsRoles = _interopRequireDefault(
	require("./etc/roles/ariaGraphicsRoles"),
);
var _iterationDecorator = _interopRequireDefault(
	require("./util/iterationDecorator"),
);
		...
		for (_iterator.s(); !(_step = _iterator.n()).done; ) {
			...
			try {
				var _loop = function _loop() {
					...
					var superClassRoleTuple = roles.find((_ref3) => {
					...
					if (superClassRoleTuple) {
						...
						for (
							...
							if (
								...
								!Object.hasOwn(roleDefinition.props, prop)
							) {
								Object.assign(
...
var rolesMap = {
	...
	get: function get(key) {
		var item = roles.find((tuple) => (tuple[0] === key ? true : false));
		return item && item[1];
	},
