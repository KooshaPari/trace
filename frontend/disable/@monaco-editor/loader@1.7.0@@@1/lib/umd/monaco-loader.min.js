!((global, factory) => {
	...
	function _objectSpread2$1(e) {
		for (var r = 1; r < arguments.length; r++) {
			...
			r % 2
				? ownKeys$1(Object(t), !0).forEach((r) => {
	...
	var errorHandler$1 = curry$1((errorMessages, type) => {
		...
		validators$1 = {
			changes: (initial, changes) => (
				isObject$1(changes) || errorHandler$1("changeType"),
			...
			selector: (selector) => {
				isFunction(selector) || errorHandler$1("selectorType");
			},
			handler: (handler) => {
				isFunction(handler) ||
					isObject$1(handler) ||
			...
			initial: (initial) => {
				var obj;
				initial || errorHandler$1("initialIsRequired"),
					isObject$1(initial) || errorHandler$1("initialType"),
					(obj = initial),
					Object.keys(obj).length || errorHandler$1("initialContent");
			},
		};
	...
	function updateState(state, changes) {
		return (
			(state.current = _objectSpread2(
				_objectSpread2({}, state.current),
				changes,
			)),
			changes
		);
	...
	var index = {
		create: function (initial) {
			...
			validators$1.initial(initial), validators$1.handler(handler);
			var state = { current: initial },
				didUpdate = curry$1(didStateUpdate)(state, handler),
				update = curry$1(updateState)(state),
				validate = curry$1(validators$1.changes)(initial),
				getChanges = curry$1(extractChanges)(state);
			return [
				function () {
					var selector =
						arguments.length > 0 && void 0 !== arguments[0]
							? arguments[0]
							: (state) => state;
					return validators$1.selector(selector), selector(state.current);
				},
				(causedChanges) => {
					!(function () {
						for (
	...
	var fn,
		...
		errorHandler = ((fn = (errorMessages, type) => {
			throw new Error(errorMessages[type] || errorMessages.default);
		}),
		function curried() {
			for (
	...
	function merge(target, source) {
		return (
			...
			_objectSpread2$1(_objectSpread2$1({}, target), source)
		);
	}
	...
	function makeCancelable(promise) {
		...
		return (wrappedPromise.cancel = () => (hasCanceled_ = !0)), wrappedPromise;
	}
	var r,
		e,
		_excluded = ["monaco"],
	...
	var wrapperPromise = new Promise((resolve, reject) =>
			setState({ resolve: resolve, reject: reject }),
		),
		loader = {
			config: (globalConfig) => {
			...
			init: () => {
				...
				if (!state.isInitialized) {
					if ((setState({ isInitialized: !0 }), state.monaco))
						return state.resolve(state.monaco), makeCancelable(wrapperPromise);
			...
			__getMonacoInstance: () => getState((_ref4) => _ref4.monaco),
		};
	return loader;
});
