...
exports.importAttributesOrAssertions =
	exports.importAttributes =
	exports.importAssertions =
		void 0;
...
const importAttributesOrAssertions = (exports.importAttributesOrAssertions =
	plugin({
		keyword: "with-assert",
	}));
function plugin(options) {
	return (Parser) => pluginImpl(options, Parser);
}
function pluginImpl(options, Parser) {
	...
	return class extends Parser {
		...
		_matchKeywordToken() {
			return (
				(isWithOrAssertKeyword &&
					(this.type === this.withToken || this.type === this.assertToken)) ||
		...
		readToken(code) {
			...
			if (isWithOrAssertKeyword) {
				if (
					this.input.slice(this.pos, this.pos + withKeyword.length) ===
					withKeyword
				) {
					keyword = withKeyword;
					token = this.withToken;
				} else if (
					this.input.slice(this.pos, this.pos + assertKeyword.length) ===
					assertKeyword
				) {
