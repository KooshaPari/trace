...
tape("multi bytes from single buffer (negative indexes)", (t) => {
	const bl = new BufferList();
...
tape("multiple bytes from crazy nested buffer lists", (t) => {
	const bl = new BufferList();
...
tape("append accepts arrays of Buffers", (t) => {
	const bl = new BufferList();
...
tape("append accepts arrays of Uint8Arrays", (t) => {
	const bl = new BufferList();
...
tape("append accepts arrays of BufferLists", (t) => {
	const bl = new BufferList();
...
tape("append chainable (test results)", (t) => {
...
tape("test readUIntLE / readUIntBE / readIntLE / readIntBE", (t) => {
	const buf1 = Buffer.alloc(1);
	const buf2 = Buffer.alloc(3);
	const buf3 = Buffer.alloc(3);
	const bl = new BufferList();
...
tape("shallow slice across buffer boundaries", (t) => {
...
tape("shallow slice within single buffer", (t) => {
	t.plan(2);
...
tape("shallow slice single buffer", (t) => {
	t.plan(3);
...
tape("shallow slice with negative or omitted indices", (t) => {
	t.plan(4);
...
tape("shallow slice does not make a copy", (t) => {
	t.plan(1);
...
tape("shallow slice with 0 length", (t) => {
	t.plan(1);
...
tape("shallow slice with 0 length from middle", (t) => {
	t.plan(1);
...
!process.browser &&
	tape("destroy with pipe before read end", (t) => {
		t.plan(2);
...
!process.browser &&
	tape("destroy with pipe before read end with race", (t) => {
		t.plan(2);
...
!process.browser &&
	tape("destroy with pipe after read end", (t) => {
		t.plan(2);
...
!process.browser &&
	tape("destroy with pipe while writing to a destination", (t) => {
		t.plan(4);
