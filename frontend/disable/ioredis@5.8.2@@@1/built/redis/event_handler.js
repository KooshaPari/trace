			...
			clientCommandPromises.push(
				self
					.client(
						...
						"LIB-NAME",
						(
							(_a = self.options) === null || _a === void 0
								? void 0
								: _a.clientInfoTag
						)
...
function abortError(command) {
	const err = new redis_errors_1.AbortError(
...
function abortIncompletePipelines(commandQueue) {
	...
	for (let i = 0; i < commandQueue.length; ) {
		const command =
			(_a = commandQueue.peekAt(i)) === null || _a === void 0
				? void 0
				: _a.command;
		...
		if (typeof maxRetriesPerRequest === "number") {
			if (maxRetriesPerRequest < 0) {
				...
				if (remainder === 0) {
					...
					self.flushQueue(
						new errors_1.MaxRetriesPerRequestError(maxRetriesPerRequest),
					);
				}
			}
		}
	...
	return () => {
		...
		if (self.options.monitor) {
			...
			self.sendCommand = (command) => {
				if (
					Command_1.default.checkFlag("VALID_IN_MONITOR_MODE", command.name)
				) {
