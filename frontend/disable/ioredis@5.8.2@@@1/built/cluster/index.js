...
class Cluster extends Commander_1.default {
	...
	constructor(startupNodes, options = {}) {
		...
		this.connectionPool = new ConnectionPool_1.default(
			this.options.redisOptions,
		);
		...
		this.subscriber = new ClusterSubscriber_1.default(
			this.connectionPool,
			this,
		);
	...
	quit(callback) {
		...
		if (status === "wait") {
			const ret = (0, standard_as_callback_1.default)(
				Promise.resolve("OK"),
				callback,
			);
	...
	refreshSlotsCache(callback) {
		...
		function tryNode(index) {
			...
			_this.getInfoFromNode(node, (err) => {
				switch (_this.status) {
					case "close":
					case "end":
