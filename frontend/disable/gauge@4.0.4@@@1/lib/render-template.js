...
function renderValueWithValues(values) {
	return (item) => renderValue(item, values);
}

var renderTemplate = (module.exports = (width, template, values) => {
...
function generatePreAndPost(baseItem, parentValues) {
	...
	return ($1, $2, length) => renderTemplate(length, template, values);
}
...
function prepareItems(width, template, values) {
	...
	var output = template.map(cloneAndObjectify).filter((item) => item != null);
