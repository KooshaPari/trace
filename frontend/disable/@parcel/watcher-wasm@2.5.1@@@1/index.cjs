...
var wasm_env = {
	__syscall_newfstatat(dirfd, path, buf, flags) {
		...
		const p = import_path.default.resolve(dir, env.getString(path));
		const nofollow = flags & 256;
		try {
			const stat = nofollow
				? import_fs.default.lstatSync(p, { bigint: true })
				: import_fs.default.statSync(p, { bigint: true });
	...
	__syscall_lstat64(path, buf) {
		...
		try {
			const stat = import_fs.default.lstatSync(p, { bigint: true });
			return writeStat(stat, buf);
		} catch (err) {
			env.i32[env.instance.exports.__errno_location >> 2] = err.errno;
			return -1;
	...
	__syscall_fstat64(fd, buf) {
		try {
			const stat = import_fs.default.fstatSync(fd, { bigint: true });
			return writeStat(stat, buf);
		} catch (err) {
			env.i32[env.instance.exports.__errno_location >> 2] = err.errno;
			return -1;
	...
	__syscall_stat64(path, buf) {
		const p = env.getString(path);
		try {
			const stat = import_fs.default.statSync(p, { bigint: true });
			return writeStat(stat, buf);
		} catch (err) {
			env.i32[env.instance.exports.__errno_location >> 2] = err.errno;
			return -1;
	...
	__syscall_openat(dirfd, path, flags, mode) {
		...
		const p = import_path.default.resolve(dir, env.getString(path));
		try {
			const fd = import_fs.default.openSync(p, f);
	...
	wasm_backend_add_watch(filename, backend) {
		...
		const watch = import_fs.default.watch(
			path,
			{ encoding: "buffer" },
			(eventType, filename2) => {
				if (filename2) {
					const type = eventType === "change" ? 1 : 2;
					const fptr = env.instance.exports.malloc(filename2.byteLength + 1);
...
var wasi = {
	...
	fd_write(fd, iov, iovcnt, pnum) {
		...
		const wrote = import_fs.default.writevSync(fd, buffers);
	...
	fd_read(fd, iov, iovcnt, pnum) {
		...
		const read = import_fs.default.readvSync(fd, buffers);
		env.u32[pnum >> 2] = read;
		return 0;
	},
	proc_exit() {},
