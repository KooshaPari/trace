...
async function streamTransformedFile(originalFilename, outStream, transformed) {
	return new Promise((resolve, reject) => {
		const stream = wrapped_fs_1.default.createReadStream(
			transformed ? transformed.path : originalFilename,
		);
...
const writeFileListToStream = async (
	...
	for (const file of files) {
		if (file.unpack) {
			...
			await streamTransformedFile(
				file.filename,
				out,
				metadata[file.filename].transformed,
			);
		}
	}
	...
	for (const file of unpackedSymlinks) {
		...
		await wrapped_fs_1.default.mkdirp(
			path.join(`${dest}.unpacked`, path.dirname(filename)),
		);
		// create symlink within unpacked dir
		await wrapped_fs_1.default
			.symlink(link, path.join(`${dest}.unpacked`, filename))
			.catch(async (error) => {
