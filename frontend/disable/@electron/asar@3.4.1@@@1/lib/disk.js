...
async function copyFile(dest, src, filename) {
	...
	return wrapped_fs_1.default.writeFile(targetFile, content, {
		mode: stats.mode,
	});
}
...
const writeFileListToStream = async (
	dest,
	filesystem,
	out,
	lists,
	...
	for (const file of files) {
		if (file.unpack) {
			...
			const stream = wrapped_fs_1.default.createReadStream(
				transformed ? transformed.path : file.filename,
			);
...
async function streamFilesystem(dest, filesystem, lists) {
	...
	try {
		for (
			var _d = true, files_1 = __asyncValues(files), files_1_1;
			(files_1_1 = await files_1.next()), (_a = files_1_1.done), !_a;
			_d = true
		) {
			_c = files_1_1.value;
			_d = false;
			...
			if (file.unpack) {
				...
				const writeStream = wrapped_fs_1.default.createWriteStream(targetFile, {
					mode: file.mode,
				});
...
function readFileSync(filesystem, filename, info) {
	...
	if (info.unpacked) {
		...
		buffer = wrapped_fs_1.default.readFileSync(
...
async function createSymlink(dest, filepath, link) {
	...
	await wrapped_fs_1.default.mkdirp(
		path.join(`${dest}.unpacked`, path.dirname(filepath)),
	);
	// create symlink within unpacked dir
	await wrapped_fs_1.default
		.symlink(link, path.join(`${dest}.unpacked`, filepath))
		.catch(async (error) => {
