...
function isUnpackedDir(dirPath, pattern, unpackDirs) {
	if (
		...
		(0, minimatch_1.default)(dirPath, pattern)
	) {
		...
		return unpackDirs.some(
			(unpackDir) =>
				dirPath.startsWith(unpackDir) &&
				!path.relative(unpackDir, dirPath).startsWith(".."),
		);
	}
}
...
async function createPackageWithOptions(src, dest, options) {
	...
	const [filenames, metadata] = await (0, crawlfs_1.crawl)(
		pattern,
		globOptions,
	);
...
async function createPackageFromFiles(
	...
	filenames = filenames.map((filename) => path.normalize(filename));
	...
	if (options.ordering) {
		const orderingFiles = (
			await wrapped_fs_1.default.readFile(options.ordering)
		)
			.toString()
			.split("\n")
			.map((line) => {
		...
		console.log(
			`Ordering file has ${((total - missing) / total) * 100}% coverage.`,
		);
	} else {
	...
	const handleFile = async (filename) => {
		...
		const shouldUnpackPath = (relativePath, unpack, unpackDir) => {
			let shouldUnpack = false;
			if (unpack) {
				shouldUnpack = (0, minimatch_1.default)(filename, unpack, {
					matchBase: true,
				});
			}
		...
		switch (file.type) {
			...
			case "file":
				...
				return filesystem.insertFile(
					filename,
					() => wrapped_fs_1.default.createReadStream(filename),
					shouldUnpack,
					file,
					options,
				);
...
function extractFile(archivePath, filename, followLinks = true) {
	...
	if ("link" in fileInfo || "files" in fileInfo) {
		throw new Error(
			"Expected to find file at: " +
				filename +
				" but found a directory or link",
		);
	}
