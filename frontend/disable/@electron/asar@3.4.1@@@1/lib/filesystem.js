...
class Filesystem {
	...
	async insertFile(p, streamGenerator, shouldUnpack, file, options = {}) {
		...
		if (shouldUnpack || dirNode.unpacked) {
			...
			node.unpacked = true;
			node.integrity = await (0, integrity_1.getFileIntegrity)(
				streamGenerator(),
			);
		...
		if (transformed) {
			const tmpdir = await wrapped_fs_1.default.mkdtemp(
				path.join(os.tmpdir(), "asar-"),
			);
	...
	insertLink(
		p,
		shouldUnpack,
		parentPath = wrapped_fs_1.default.realpathSync(path.dirname(p)),
		symlink = wrapped_fs_1.default.readlinkSync(p), // /var/tmp => /private/var
	...
	listFiles(options) {
		const files = [];
		const fillFilesFromMetadata = (basePath, metadata) => {
			...
			for (const [childPath, childMetadata] of Object.entries(metadata.files)) {
				...
				const packState =
					"unpacked" in childMetadata && childMetadata.unpacked
						? "unpack"
						: "pack  ";
				files.push(
					options && options.isPack ? `${packState} : ${fullPath}` : fullPath,
				);
