...
const argv = (0, yargs_1.default)(process.argv.slice(2))
	...
	.options({
		version: {
			alias: "v",
			type: "string",
			description: "The version of Electron to build against",
		},
		force: {
			alias: "f",
			type: "boolean",
			description:
				"Force rebuilding modules, even if we would skip it otherwise",
		},
		arch: {
			alias: "a",
			type: "string",
			description:
				"Override the target architecture to something other than your system's",
		},
		"module-dir": {
			alias: "m",
			type: "string",
		...
		"dist-url": {
			alias: "d",
			type: "string",
			description: "Custom header tarball URL",
		},
		// TODO: should be type: array
		types: {
			alias: "t",
		...
		parallel: {
			...
			type: "boolean",
			description:
				"Rebuild in parallel, this is enabled by default on macOS and Linux",
		},
		sequential: {
			alias: "s",
			type: "boolean",
			description:
				"Rebuild modules sequentially, this is enabled by default on Windows",
		},
		debug: {
			alias: "b",
			type: "boolean",
			description: "Build debug version of modules",
		},
		"prebuild-tag-prefix": {
			type: "string",
			description:
				'GitHub tag prefix passed to prebuild-install. Default is "v"',
		},
		"force-abi": {
			type: "number",
			description:
		...
		"disable-pre-gyp-copy": {
			type: "boolean",
			description: "Disables the pre-gyp copy step",
		},
		"build-from-source": {
			type: "boolean",
...
(async () => {
	const projectRootPath = await (0, search_module_1.getProjectRootPath)(
		process.cwd(),
	);
	const electronModulePath = argv.e
		? path.resolve(process.cwd(), argv.e)
		: await (0, electron_locator_1.locateElectronModule)(projectRootPath);
	...
	if (!rootDirectory) {
		...
		if (
			!(await fs.pathExists(rootDirectory)) ||
			!(await fs.pathExists(path.resolve(rootDirectory, "package.json")))
		) {
			// Then we try the CWD
