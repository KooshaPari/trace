...
exports.getProjectRootPath =
	exports.searchForNodeModules =
	exports.searchForModule =
		void 0;
...
async function traverseAncestorDirectories(
	...
	while (
		await shouldContinueSearch(traversedPath, rootPath, stopAtPackageJSON)
	) {
		...
		if (
			parentPath === traversedPath ||
			(maxItems && paths.length >= maxItems)
		) {
			break;
		}
...
async function searchForModule(cwd, moduleName, rootPath) {
	const pathGenerator = (traversedPath) =>
...
async function getProjectRootPath(cwd) {
	for (const lockFilename of [
		...
		const lockPaths = await traverseAncestorDirectories(
			cwd,
			pathGenerator,
			undefined,
			1,
