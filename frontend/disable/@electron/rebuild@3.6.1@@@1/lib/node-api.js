...
class NodeAPI {
	...
	getVersionForElectron() {
		const electronNapiVersion = (0, node_api_version_1.fromElectronVersion)(
	...
	getNapiVersion(moduleNapiVersions) {
		...
		const filteredVersions = moduleNapiVersions.filter(
			(v) => v <= electronNapiVersion,
		);
		if (filteredVersions.length === 0) {
			throw new Error(
				`Native module '${this.moduleName}' supports Node-API versions ${moduleNapiVersions} but Electron v${this.electronVersion} only supports Node-API v${electronNapiVersion}`,
			);
		}
