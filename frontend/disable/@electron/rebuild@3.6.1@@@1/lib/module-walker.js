...
class ModuleWalker {
	...
	get nodeModulesPaths() {
		return (0, search_module_1.searchForNodeModules)(
			this.buildPath,
			this.projectRootPath,
		);
	}
	async walkModules() {
		const rootPackageJson = await (0, read_package_json_1.readPackageJson)(
			this.buildPath,
		);
		const markWaiters = [];
		const depKeys = [];
		...
		for (const key of depKeys) {
			...
			const modulePaths = await (0, search_module_1.searchForModule)(
				this.buildPath,
				key,
				this.projectRootPath,
			);
	...
	async findModule(moduleName, fromDir, foundFn) {
		const testPaths = await (0, search_module_1.searchForModule)(
			fromDir,
			moduleName,
			this.projectRootPath,
		);
	...
	async markChildrenAsProdDeps(modulePath) {
		...
		try {
			childPackageJson = await (0, read_package_json_1.readPackageJson)(
				modulePath,
				true,
			);
		} catch (_err) {
	...
	async findAllModulesIn(nodeModulesPath, prefix = "") {
		...
		const realNodeModulesPath =
			await fs_extra_1.default.realpath(nodeModulesPath);
		...
		for (const modulePath of await fs_extra_1.default.readdir(
			realNodeModulesPath,
		)) {
			...
			const realPath = await fs_extra_1.default.realpath(
				path_1.default.resolve(nodeModulesPath, modulePath),
			);
			...
			if (
				...
				(!this.onlyModules || this.onlyModules.includes(modulePath))
			) {
			...
			if (
				await fs_extra_1.default.pathExists(
					path_1.default.resolve(nodeModulesPath, modulePath, "node_modules"),
				)
			) {
				await this.findAllModulesIn(
					path_1.default.resolve(realPath, "node_modules"),
				);
			}
		}
	}
