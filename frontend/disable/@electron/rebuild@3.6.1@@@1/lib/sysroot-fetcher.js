...
async function downloadLinuxSysroot(electronVersion, targetArch) {
	...
	const electronSysroots = JSON.parse(
		await (0, fetcher_1.fetch)(
			`https://raw.githubusercontent.com/electron/electron/v${electronVersion}/script/sysroots.json`,
			"text",
		),
	);
	const { Sha1Sum: sha, Tarball: fileName } =
		electronSysroots[`sid_${linuxArch}`] ||
		electronSysroots[`bullseye_${linuxArch}`];
	...
	const actualSha = crypto
		.createHash("SHA1")
		.update(sysrootBuffer)
		.digest("hex");
	d("expected sha:", sha);
	d("actual sha:", actualSha);
	if (sha !== actualSha)
		throw new Error(
			`Attempted to download the linux sysroot for ${electronVersion} but the SHA checksum did not match`,
		);
	d("writing sysroot to disk");
	const tmpTarFile = path.resolve(
		constants_1.ELECTRON_GYP_DIR,
		`${electronVersion}-${fileName}`,
	);
	...
	await (0, cross_spawn_promise_1.spawn)(
		"tar",
		["-xf", tmpTarFile, "-C", sysrootDir],
		{ stdio: "ignore" },
	);
	return sysrootDir;
}
