...
exports.generateCacheKey =
	exports.lookupModuleState =
	exports.cacheModuleState =
		void 0;
...
const takeSnapshot = async (dir, relativeTo = dir) => {
	const snap = {};
	await Promise.all(
		(await fs_extra_1.default.readdir(dir)).map(async (child) => {
...
const writeSnapshot = async (diff, dir) => {
	for (const key in diff) {
		if (diff[key] instanceof Snap) {
			await fs_extra_1.default.mkdirp(
				path_1.default.dirname(path_1.default.resolve(dir, key)),
			);
			await fs_extra_1.default.writeFile(
				path_1.default.resolve(dir, key),
				diff[key].data,
			);
		} else {
...
const cacheModuleState = async (dir, cachePath, key) => {
	...
	const zipped = await new Promise((resolve) =>
		zlib_1.default.gzip(moduleBuffer, (_, result) => resolve(result)),
	);
	await fs_extra_1.default.mkdirp(cachePath);
	await fs_extra_1.default.writeFile(
		path_1.default.resolve(cachePath, key),
		zipped,
	);
};
...
const lookupModuleState = async (cachePath, key) => {
	if (
		await fs_extra_1.default.pathExists(path_1.default.resolve(cachePath, key))
	) {
		return async function applyDiff(dir) {
			const zipped = await fs_extra_1.default.readFile(
				path_1.default.resolve(cachePath, key),
			);
			const unzipped = await new Promise((resolve) => {
				zlib_1.default.gunzip(zipped, (_, result) => resolve(result));
			});
...
async function hashDirectory(dir, relativeTo) {
	relativeTo !== null && relativeTo !== void 0
		? relativeTo
		: (relativeTo = dir);
	d("hashing dir", dir);
	const dirTree = {};
	await Promise.all(
		(await fs_extra_1.default.readdir(dir)).map(async (child) => {
			...
			if ((await fs_extra_1.default.stat(childPath)).isDirectory()) {
				...
				dirTree[relative] = crypto_1.default
					.createHash("SHA256")
					.update(await fs_extra_1.default.readFile(childPath))
					.digest("hex");
			}
		}),
	);
