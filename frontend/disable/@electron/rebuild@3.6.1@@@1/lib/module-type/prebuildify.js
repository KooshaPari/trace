...
exports.Prebuildify =
	exports.determineNativePrebuildExtension =
	exports.determineNativePrebuildArch =
		void 0;
...
class Prebuildify extends _1.NativeModule {
	async usesTool() {
		const devDependencies = await this.packageJSONFieldWithDefault(
			"devDependencies",
			{},
		);
		// eslint-disable-next-line no-prototype-builtins
		return Object.hasOwn(devDependencies, "prebuildify");
	}
	async findPrebuiltModule() {
		const nodeArch = (0, arch_1.getNodeArch)(
			this.rebuilder.arch,
		...
		const prebuiltModuleDir = path_1.default.join(
			prebuildsDir,
			`${this.rebuilder.platform}-${determineNativePrebuildArch(nodeArch)}`,
		);
		const nativeExt = determineNativePrebuildExtension(nodeArch);
		const electronNapiModuleFilename = path_1.default.join(
			prebuiltModuleDir,
			`electron.napi.${nativeExt}`,
		);
		const nodejsNapiModuleFilename = path_1.default.join(
			prebuiltModuleDir,
			`node.napi.${nativeExt}`,
		);
		const abiModuleFilename = path_1.default.join(
			prebuiltModuleDir,
			`electron.abi${this.rebuilder.ABI}.${nativeExt}`,
		);
		if (
			(await fs_extra_1.default.pathExists(electronNapiModuleFilename)) ||
			(await fs_extra_1.default.pathExists(nodejsNapiModuleFilename))
		) {
			...
			d(
				`Could not locate "${electronNapiModuleFilename}", "${nodejsNapiModuleFilename}", or "${abiModuleFilename}"`,
			);
			return false;
		}
		return true;
