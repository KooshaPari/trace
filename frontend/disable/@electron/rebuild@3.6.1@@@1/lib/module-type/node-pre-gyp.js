...
class NodePreGyp extends _1.NativeModule {
	async usesTool() {
		const dependencies = await this.packageJSONFieldWithDefault(
			"dependencies",
			{},
		);
		// eslint-disable-next-line no-prototype-builtins
		return Object.hasOwn(dependencies, "@mapbox/node-pre-gyp");
	}
	async locateBinary() {
		return (0, _1.locateBinary)(
			this.modulePath,
			...
			try {
				...
				if (
					(_a = err === null || err === void 0 ? void 0 : err.message) ===
						null || _a === void 0
						? void 0
						: _a.includes("requires Node-API but Electron")
				) {
					throw err;
				}
			}
	...
	async shouldUpdateBinary(nodePreGypPath) {
		...
		try {
			...
			for (const modulePath of modulePaths) {
				let moduleArch;
				try {
					moduleArch = await (0, read_binary_file_arch_1.readBinaryFileArch)(
						modulePath,
					);
				...
				if (moduleArch && moduleArch !== this.rebuilder.arch) {
					shouldUpdate = true;
					d(
						"module architecture differs:",
						`${moduleArch} !== ${this.rebuilder.arch}`,
					);
					break;
				}
			}
	...
	async getModulePaths(nodePreGypPath) {
		const results = await (0, cross_spawn_promise_1.spawn)(
			process.execPath,
			[
				nodePreGypPath,
				"reveal",
