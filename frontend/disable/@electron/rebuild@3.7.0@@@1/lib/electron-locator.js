...
async function locateElectronModule(
	projectRootPath = undefined,
	startDir = undefined,
) {
	startDir !== null && startDir !== void 0
		? startDir
		: (startDir = process.cwd());
	for (const moduleName of electronModuleNames) {
		const electronPaths = await (0, search_module_1.searchForModule)(
			startDir,
			moduleName,
			projectRootPath,
		);
		const electronPath = electronPaths.find(
			async (ePath) => await fs.pathExists(path.join(ePath, "package.json")),
		);
		if (electronPath) {
			return electronPath;
		}
