...
function getSDKRoot() {
	...
	const output = cp.execFileSync("xcrun", [
		"--sdk",
		"macosx",
		"--show-sdk-path",
	]);
...
async function getClangEnvironmentVars(electronVersion, targetArch) {
	...
	if (process.platform === "linux") {
		const sysrootPath = await (0, sysroot_fetcher_1.downloadLinuxSysroot)(
			electronVersion,
			targetArch,
		);
...
async function downloadClangVersion(electronVersion) {
	...
	const clangDirPath = path.resolve(
		constants_1.ELECTRON_GYP_DIR,
		`${electronVersion}-clang`,
	);
	...
	const electronDeps = await (0, fetcher_1.fetch)(
	...
	if (!chromiumRevisionMatch)
		throw new Error(
			"Failed to determine Chromium revision for given Electron version",
		);
	const chromiumRevision = chromiumRevisionMatch[1];
	d("fetching clang for Chromium:", chromiumRevision);
	const base64ClangUpdate = await (0, fetcher_1.fetch)(
		`https://chromium.googlesource.com/chromium/src.git/+/${chromiumRevision}/tools/clang/scripts/update.py?format=TEXT`,
		"text",
	);
	const clangUpdate = Buffer.from(base64ClangUpdate, "base64").toString("utf8");
	const clangVersionString =
		clangVersionFromRevision(clangUpdate) || clangVersionFromSVN(clangUpdate);
	...
	const clangDownloadURL = getClangDownloadURL(
		"clang",
		clangVersionString,
		process.platform,
		process.arch,
