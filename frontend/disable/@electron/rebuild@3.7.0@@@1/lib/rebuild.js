...
class Rebuilder {
	constructor(options) {
		...
		this.cachePath =
			options.cachePath ||
			path.resolve(os.homedir(), ".electron-rebuild-cache");
		...
		this.ABIVersion =
			(_a = options.forceABI) === null || _a === void 0
				? void 0
				: _a.toString();
		const onlyModules = options.onlyModules || null;
		const extraModules = (options.extraModules || []).reduce(
			(acc, x) => acc.add(x),
			new Set(),
		);
		const types = options.types || defaultTypes;
		this.moduleWalker = new module_walker_1.ModuleWalker(
			this.buildPath,
	...
	async rebuildModuleAt(modulePath) {
		...
		const moduleRebuilder = new module_rebuilder_1.ModuleRebuilder(
			this,
			modulePath,
		);
		...
		if (this.useCache) {
			...
			const applyDiffFn = await (0, cache_1.lookupModuleState)(
				this.cachePath,
				cacheKey,
			);
