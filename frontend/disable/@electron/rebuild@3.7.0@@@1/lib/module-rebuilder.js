...
class ModuleRebuilder {
	constructor(rebuilder, modulePath) {
		...
		this.prebuildInstall = new prebuild_install_1.PrebuildInstall(
			rebuilder,
			modulePath,
		);
	...
	async cacheModuleState(cacheKey) {
		if (this.rebuilder.useCache) {
			await (0, cache_1.cacheModuleState)(
				this.modulePath,
				this.rebuilder.cachePath,
				cacheKey,
			);
		}
	}
	...
	async findPrebuildInstallModule(cacheKey) {
		if (await this.prebuildInstall.usesTool()) {
			d(
				`assuming is prebuild-install powered: ${this.prebuildInstall.moduleName}`,
			);
	...
	async replaceExistingNativeModule() {
		const buildLocation = path.resolve(
			this.modulePath,
			"build",
			this.rebuilder.buildType,
		);
		...
		const nodeFile = buildLocationFiles.find(
			(file) => file !== ".node" && file.endsWith(".node"),
		);
		const nodePath = nodeFile
			? path.resolve(buildLocation, nodeFile)
			: undefined;
