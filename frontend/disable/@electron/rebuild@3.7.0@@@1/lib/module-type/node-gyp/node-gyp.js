...
class NodeGyp extends __1.NativeModule {
	async buildArgs(prefixedArgs) {
		...
		if (
			!semver_1.default.satisfies(
				this.rebuilder.electronVersion,
				"^14.2.0 || ^15.3.0",
			) &&
			semver_1.default.major(this.rebuilder.electronVersion) < 16
		) {
	...
	async buildArgsFromBinaryField() {
		...
		if (Array.isArray(binary.napi_versions)) {
			napiBuildVersion = this.nodeAPI.getNapiVersion(
				binary.napi_versions.map((str) => Number(str)),
			);
		}
		const flags = await Promise.all(
			Object.entries(binary).map(async ([binaryKey, binaryValue]) => {
				...
				value = value
					...
					.replace(
						"{node_abi}",
						`electron-v${this.rebuilder.electronVersion.split(".").slice(0, 2).join(".")}`,
					)
					...
					.replace(
						"{libc}",
						(await detect_libc_1.default.family()) || "unknown",
					);
				if (napiBuildVersion !== undefined) {
					value = value.replace(
						"{napi_build_version}",
						napiBuildVersion.toString(),
					);
				}
	...
	async rebuildModule() {
		...
		if (this.rebuilder.useElectronClang) {
			const { env: clangEnv, args: clangArgs } = await (0,
		...
		const forkedChild = (0, child_process_1.fork)(
		...
		const outputBuffers = [];
		(_a = forkedChild.stdout) === null || _a === void 0
			? void 0
			: _a.on("data", (chunk) => {
					outputBuffers.push(chunk);
				});
		(_b = forkedChild.stderr) === null || _b === void 0
			? void 0
			: _b.on("data", (chunk) => {
					outputBuffers.push(chunk);
				});
		forkedChild.send({
			...
			extraNodeGypArgs,
			devDir:
				this.rebuilder.mode === "sequential"
