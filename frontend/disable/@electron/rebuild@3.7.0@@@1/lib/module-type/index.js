...
class NativeModule {
	constructor(rebuilder, modulePath) {
		...
		this.nodeAPI = new node_api_1.NodeAPI(
			this.moduleName,
	...
	get moduleName() {
		if (!this._moduleName) {
			...
			const parentDir = path_1.default.basename(
				path_1.default.dirname(this.modulePath),
			);
	...
	async packageJSONField(key) {
		this.packageJSON ||
			(this.packageJSON = await (0, read_package_json_1.readPackageJson)(
				this.modulePath,
			));
	...
	async findPackageInDependencies(
		...
		if (Object.hasOwn(dependencies, packageName)) return packageName;
