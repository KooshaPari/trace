	...
	_parse(node, obj) {
		...
		if ((0, util_1.isFunction)(obj)) {
			...
			(0, util_1.forEachArray)(
				obj,
				(item) => (lastChild = this.parse(node, item)),
				this,
			);
			...
			(0, util_1.forEachObject)(
				...
				(key, val) => {
					...
					if (
						!options.ignoreConverters &&
						key.indexOf(options.convert.att) === 0
					) {
						// assign attributes
						if (key === options.convert.att) {
							...
							} /* if (isMap(val) || isObject(val)) */ else {
								(0, util_1.forEachObject)(val, (attrKey, attrVal) => {
									lastChild =
										this.attribute(
											node,
											undefined,
											this.sanitize(attrKey),
											this._decodeAttributeValue(this.sanitize(attrVal)),
										) || lastChild;
								});
							}
						} else {
							lastChild =
								this.attribute(
									...
									this._decodeAttributeValue(this.sanitize(val?.toString())),
								) || lastChild;
						}
					} else if (
						!options.ignoreConverters &&
						key.indexOf(options.convert.text) === 0
					) {
						// text node
						if ((0, util_1.isMap)(val) || (0, util_1.isObject)(val)) {
							...
							lastChild =
								this.text(
									node,
									this._decodeText(this.sanitize(val?.toString())),
								) || lastChild;
						}
					} else if (
						!options.ignoreConverters &&
						key.indexOf(options.convert.cdata) === 0
					) {
						// cdata node
						if ((0, util_1.isArray)(val) || (0, util_1.isSet)(val)) {
							(0, util_1.forEachArray)(
								...
								(item) =>
									(lastChild =
										this.cdata(node, this.sanitize(item)) || lastChild),
								this,
							);
						} else {
							lastChild =
								this.cdata(node, this.sanitize(val?.toString())) || lastChild;
						}
					} else if (
						!options.ignoreConverters &&
						key.indexOf(options.convert.comment) === 0
					) {
						// comment node
						if ((0, util_1.isArray)(val) || (0, util_1.isSet)(val)) {
							(0, util_1.forEachArray)(
								...
								(item) =>
									(lastChild =
										this.comment(node, this.sanitize(item)) || lastChild),
								this,
							);
						} else {
							lastChild =
								this.comment(node, this.sanitize(val?.toString())) || lastChild;
						}
					} else if (
						!options.ignoreConverters &&
						key.indexOf(options.convert.ins) === 0
					) {
						...
						if ((0, util_1.isString)(val)) {
							...
							(0, util_1.forEachArray)(
								...
								(item) => {
									...
									const insTarget =
										insIndex === -1 ? item : item.substr(0, insIndex);
									const insValue =
										insIndex === -1 ? "" : item.substr(insIndex + 1);
									lastChild =
										this.instruction(
											node,
											this.sanitize(insTarget),
						...
						} /* if (isMap(target) || isObject(target)) */ else {
							(0, util_1.forEachObject)(
								val,
								(insTarget, insValue) =>
									(lastChild =
						...
						((0, util_1.isArray)(val) || (0, util_1.isSet)(val)) &&
						(0, util_1.isEmpty)(val)
					) {
						// skip empty arrays
					} else if (
						((0, util_1.isMap)(val) || (0, util_1.isObject)(val)) &&
						(0, util_1.isEmpty)(val)
					) {
						// empty objects produce one node
						lastChild =
							this.element(node, undefined, this.sanitize(key)) || lastChild;
			...
			// text node
			lastChild =
				this.text(node, this._decodeText(this.sanitize(obj?.toString()))) ||
				lastChild;
		}
