...
class JSONWriter extends BaseWriter_1.BaseWriter {
	...
	serialize(node) {
		...
		const objectWriter = new ObjectWriter_1.ObjectWriter(
		...
		return (
			...
			this._convertObject(val, this._writerOptions)
		);
	}
	/**
	...
	_convertObject(obj, options, level = 0) {
		...
		if ((0, util_1.isArray)(obj)) {
			...
			if (isLeaf && options.prettyPrint) {
				...
				markup +=
					this._endLine(options, level) + this._beginLine(options, level);
			}
			markup += "}";
		} else {
	...
	_descendantCount(obj, count = 0) {
		if ((0, util_1.isArray)(obj)) {
			(0, util_1.forEachArray)(
				obj,
				(val) => (count += this._descendantCount(val, count)),
				this,
			);
		} else if ((0, util_1.isObject)(obj)) {
			(0, util_1.forEachObject)(
				obj,
				(key, val) => (count += this._descendantCount(val, count)),
				this,
			);
		} else {
			count++;
