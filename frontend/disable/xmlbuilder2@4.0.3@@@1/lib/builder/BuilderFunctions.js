...
function builder(p1, p2) {
	const options = formatBuilderOptions(
		isXMLBuilderCreateOptions(p1) ? p1 : interfaces_1.DefaultBuilderOptions,
	);
...
function create(p1, p2) {
	const options = formatBuilderOptions(
		p1 === undefined || isXMLBuilderCreateOptions(p1)
			? p1
...
function isXMLBuilderCreateOptions(obj) {
	if (!(0, util_1.isPlainObject)(obj)) return false;
...
function formatBuilderOptions(createOptions = {}) {
	const options = (0, util_1.applyDefaults)(
		createOptions,
		interfaces_1.DefaultBuilderOptions,
	);
	if (
		options.convert.att.length === 0 ||
