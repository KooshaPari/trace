...
class XMLBuilderImpl {
	...
	set(options) {
		this._options = (0, util_1.applyDefaults)(
			(0, util_1.applyDefaults)(this._options, options, true), // apply user settings
	...
	ele(p1, p2, p3) {
		...
		if ((p1 === null || (0, util_1.isString)(p1)) && (0, util_1.isString)(p2)) {
			...
			[namespace, name, attributes] = [
				undefined,
				p1,
				(0, util_1.isObject)(p2) ? p2 : undefined,
			];
		} else {
		...
		[namespace, name] = this._extractNamespace(
			(0, dom_1.sanitizeInput)(namespace, this._options.invalidCharReplacement),
			(0, dom_1.sanitizeInput)(name, this._options.invalidCharReplacement),
			true,
		);
	...
	att(p1, p2, p3) {
		if ((0, util_1.isMap)(p1) || (0, util_1.isObject)(p1)) {
			...
			// expand if object
			(0, util_1.forEachObject)(
				p1,
				(attName, attValue) => this.att(attName, attValue),
				this,
			);
			return this;
		}
		// get primitive values
		if (p1 !== undefined && p1 !== null) p1 = (0, util_1.getValue)(p1 + "");
		if (p2 !== undefined && p2 !== null) p2 = (0, util_1.getValue)(p2 + "");
		if (p3 !== undefined && p3 !== null) p3 = (0, util_1.getValue)(p3 + "");
		let namespace;
		let name;
		let value;
		if (
			(p1 === null || (0, util_1.isString)(p1)) &&
			(0, util_1.isString)(p2) &&
			(p3 === null || (0, util_1.isString)(p3))
		) {
			// att(namespace: string, name: string, value: string)
			[namespace, name, value] = [p1, p2, p3];
		} else if (
			(0, util_1.isString)(p1) &&
			(p2 == null || (0, util_1.isString)(p2))
		) {
		...
		namespace = (0, dom_1.sanitizeInput)(
			namespace,
			this._options.invalidCharReplacement,
		);
		value = (0, dom_1.sanitizeInput)(
			value,
			this._options.invalidCharReplacement,
		);
		const [prefix, localName] = (0, algorithm_1.namespace_extractQName)(name);
		const [elePrefix] = (0, algorithm_1.namespace_extractQName)(
	...
	txt(content) {
		...
		const child = this._doc.createTextNode(
			(0, dom_1.sanitizeInput)(content, this._options.invalidCharReplacement),
		);
	...
	ins(target, content = "") {
		...
		if ((0, util_1.isArray)(target) || (0, util_1.isSet)(target)) {
			...
			(0, util_1.forEachObject)(
				target,
				(insTarget, insValue) => this.ins(insTarget, insValue),
				this,
			);
		} else {
			const child = this._doc.createProcessingInstruction(
				(0, dom_1.sanitizeInput)(target, this._options.invalidCharReplacement),
				(0, dom_1.sanitizeInput)(content, this._options.invalidCharReplacement),
			);
			this.node.appendChild(child);
		}
	...
	/** @inheritdoc */
	dtd(options) {
		const name = (0, dom_1.sanitizeInput)(
		...
		const pubID = (0, dom_1.sanitizeInput)(
			(options && options.pubID) || "",
			this._options.invalidCharReplacement,
		);
		const sysID = (0, dom_1.sanitizeInput)(
	...
	import(node) {
		...
		const updateImportedNodeNs = (clone) => {
			...
			if (!clone._namespace) {
				const [prefix] = (0, algorithm_1.namespace_extractQName)(
	...
	each(callback, self = false, recursive = false, thisArg) {
		...
		while (result[0]) {
			const nextResult = this._getNextDescendantNode(
				this._domNode,
				result[0],
				recursive,
				result[1],
		...
		if (recursive) {
			...
			while (parent && parent !== root) {
				if (parent.nextSibling)
					return [
						parent.nextSibling,
						(0, algorithm_1.tree_index)(parent.nextSibling),
						level - 1,
					];
	...
	_updateNamespace(ns) {
		...
		if (
			...
			const [elePrefix, eleLocalName] = (0, algorithm_1.namespace_extractQName)(
				ele.prefix ? ele.prefix + ":" + ele.localName : ele.localName,
			);
			// re-create the element node if its namespace changed
			// we can't simply change the namespaceURI since its read-only
			const newEle = (0, algorithm_1.create_element)(
				this._doc,
				eleLocalName,
				ns,
				elePrefix,
			...
			for (const childNode of ele.childNodes) {
				...
				if (
					util_2.Guard.isElementNode(newChildNode) &&
					!newChildNode._namespace
				) {
					const [newChildNodePrefix] = (0, algorithm_1.namespace_extractQName)(
						newChildNode.prefix
