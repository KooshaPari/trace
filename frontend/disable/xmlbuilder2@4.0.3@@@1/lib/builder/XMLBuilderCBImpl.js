...
class XMLBuilderCBImpl extends events_1.EventEmitter {
	...
	com(content) {
		...
		if (
			...
			(!(0, algorithm_1.xml_isLegalChar)(node.data) ||
				node.data.indexOf("--") !== -1 ||
				node.data.endsWith("-"))
		) {
			this.emit(
	...
	dtd(options) {
		...
		if (
			this._options.wellFormed &&
			!(0, algorithm_1.xml_isPubidChar)(node.publicId)
		) {
			this.emit(
				"error",
				new Error(
	...
	_serializeOpenTag(hasChildren) {
		...
		let localDefaultNamespace = this._recordNamespaceInformation(
			node,
			map,
			localPrefixesMap,
		);
	...
	_serializeAttributes(
		...
		const localNameSet = requireWellFormed
			...
			if (attributeNamespace !== null) {
				...
				if (attributeNamespace === infra_1.namespace.XMLNS) {
					...
					if (requireWellFormed && attr.value === infra_1.namespace.XMLNS) {
						this.emit(
							"error",
							new Error("XMLNS namespace is reserved (well-formed required)."),
						);
						return;
					}
	...
	_serializeAttributeValue(value, requireWellFormed) {
		if (
			requireWellFormed &&
			value !== null &&
			!(0, algorithm_1.xml_isLegalChar)(value)
		) {
