...
module.exports = Any.extend({
	...
	validate(value, { schema, error, prefs }) {
		...
		if (prefs.convert) {
			...
			if (rule) {
				const precision = 10 ** rule.args.limit; // This is conceptually equivalent to using toFixed but it should be much faster
	...
	rules: {
		...
		multiple: {
			method(base) {
				const baseDecimalPlace =
					typeof base === "number" ? internals.decimalPlaces(base) : null;
				const pfactor = 10 ** baseDecimalPlace;
...
internals.normalizeDecimal = (str) => {
	str = str
