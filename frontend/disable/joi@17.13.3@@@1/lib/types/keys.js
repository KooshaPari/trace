...
module.exports = Any.extend({
	...
	validate(value, { schema, error, state, prefs }) {
		if (
			...
			return {
				value,
				errors: error("object.base", { type: schema.$_property("typeof") }),
			};
		}
				...
				if (!schema) {
					...
					obj.$_terms.keys = obj.$_terms.keys
						? obj.$_terms.keys.filter(
								(child) => !Object.hasOwn(schema, child.key),
							)
						: new internals.Keys();
...
internals.dependency = (schema, rel, key, peers, options) => {
...
internals.dependencies = {
	...
	nand(schema, dep, value, state, prefs) {
		...
		for (const peer of dep.peers) {
			if (
				isPresent(peer.resolve(value, state, prefs, null, { shadow: false }))
			) {
				present.push(peer.key);
			}
		}
...
internals.keysToLabels = (schema, keys) => {
...
internals.rename = (schema, value, state, prefs, errors) => {
	...
	for (const rename of schema.$_terms.renames) {
		...
		if (!pattern) {
			if (
				Object.hasOwn(value, rename.from) &&
...
internals.unknown = (schema, value, unprocessed, errors, state, prefs) => {
	...
	if (
		(prefs.stripUnknown && typeof schema._flags.unknown === "undefined") ||
		prefs.skipFunctions
	) {
