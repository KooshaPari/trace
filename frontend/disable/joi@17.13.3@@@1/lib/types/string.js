	...
	rules: {
		...
		case: {
			...
			validate(value, helpers, { direction }) {
				if (
					(direction === "lower" && value === value.toLocaleLowerCase()) ||
		...
		email: {
			method(options = {}) {
				...
				const regex = new RegExp(
					`\\s*[${options.separator ? EscapeRegex(options.separator) : ","}]\\s*`,
				);
		...
		guid: {
			...
			method(options = {}) {
				...
				const regex = new RegExp(
					`^([\\[{\\(]?)[0-9A-F]{8}(${separator})[0-9A-F]{4}\\2?[${versionNumbers || "0-9A-F"}][0-9A-F]{3}\\2?[${versionNumbers ? "89AB" : "0-9A-F"}][0-9A-F]{3}\\2?[0-9A-F]{12}([\\]}\\)]?)$`,
					"i",
				);
			...
			validate(value, helpers, args, { regex }) {
				...
				if (
					internals.guidBrackets[results[1]] !== results[results.length - 1]
				) {
					return helpers.error("string.guid");
				}
		...
		hex: {
			method(options = {}) {
				...
				Assert(
					typeof options.prefix === "boolean" || options.prefix === "optional",
		...
		length: {
			...
			validate(value, helpers, { limit, encoding }, { name, operator, args }) {
				const length = encoding
					...
					: value.length; // $lab:coverage:ignore$
...
internals.addressOptions = (options) => {
	if (!options) {
...
internals.validateTlds = (set, source) => {
...
internals.length = (schema, name, limit, operator, encoding) => {
	Assert(
