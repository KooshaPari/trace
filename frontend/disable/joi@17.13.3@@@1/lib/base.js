...
internals.Base = class {
	...
	when(condition, options) {
		...
		if (!["any", "link"].includes(obj.type)) {
			...
			for (const item of conditions) {
				Assert(
					!item.then || item.then.type === "any" || item.then.type === obj.type,
					"Cannot combine",
					obj.type,
					"with",
	...
	concat(source) {
		...
		Assert(
			this.type === "any" || source.type === "any" || source.type === this.type,
			"Cannot merge type",
			this.type,
			"with another type:",
			source.type,
	...
	$_match(value, state, prefs, overrides) {
		...
		state.snapshot();
		const result = !Validator.validate(value, this, state, prefs, overrides)
			.errors;
		state.restore();
