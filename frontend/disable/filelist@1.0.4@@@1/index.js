...
globSync = (pat, opts) => {
	...
	try {
		files = readdirR(dirname).map((file) => file.replace(/\\/g, "/"));
	} catch (e) {
...
FileList.prototype = new (function () {
	...
	var _addMatching = function (item) {
			...
			for (var i = 0, ii = pats.length; i < ii; i++) {
				...
				if (typeof pat == "string") {
					...
					if (/[*?]/.test(pat)) {
						...
						matches = matches.map((m) => escapeRegExpChars(m));
		...
		_resolveExclude = function () {
			...
			this.items = this.items.filter((name) => !this.shouldExclude(name));
		};
	...
	this.include = function () {
		...
		for (var i = 0, ilen = args.length; i < ilen; i++) {
			...
			if (typeof arg === "object" && !Array.isArray(arg)) {
				...
				includes.items = includes.items.concat(arg).filter((item) => !!item);
			}
		}

		var items = includes.items.map((item) => ({
			path: item,
	...
	this.shouldExclude = function (name) {
		...
		return excl.regex.test(name) || excl.funcs.some((f) => !!f(name));
	};
