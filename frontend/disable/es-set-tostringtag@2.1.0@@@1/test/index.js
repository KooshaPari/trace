...
test("setToStringTag", (t) => {
	...
	t["throws"](
		// @ts-expect-error
		() => {
			setToStringTag(obj, sentinel, { force: "yes" });
		},
		TypeError,
		"throws if options is not an object",
	);

	t.test("has Symbol.toStringTag", { skip: !hasToStringTag }, (st) => {
		...
		st.equal(
			String(tagged),
			"[object new tag]",
			"toStringTag is changed with force: true",
		);
	...
	t.test("does not have Symbol.toStringTag", { skip: hasToStringTag }, (st) => {
		var passed = true;
		for (var key in obj) {
