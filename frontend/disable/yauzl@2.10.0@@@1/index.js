				...
				(err) => {
					...
					if (this.decodeStrings) {
						...
						var errorMessage = validateFileName(
							entry.fileName,
							this.validateFileNameOptions,
						);
						if (errorMessage != null)
							return emitErrorAndAutoClose(this, new Error(errorMessage));
					}
	...
	readAndAssertNoEof(
		...
		(err) => {
			try {
				...
				var readStream = this.reader.createReadStream({
...
RandomAccessReader.prototype._readStreamForRange = (start, end) => {
...
RandomAccessReader.prototype.close = (callback) => {
...
if (typeof Buffer.allocUnsafe === "function") {
	newBuffer = (len) => Buffer.allocUnsafe(len);
} else {
	newBuffer = (len) => new Buffer(len);
}
