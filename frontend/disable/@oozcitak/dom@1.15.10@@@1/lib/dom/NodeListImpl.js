...
var NodeListImpl = /** @class */ (() => {
	...
	NodeListImpl.prototype.forEach = function (callback, thisArg) {
		...
		try {
			for (
				var _b = __values(this._root._children), _c = _b.next();
				!_c.done;
				_c = _b.next()
			) {
				var node = _c.value;
	 ...
	 */
	NodeListImpl.prototype.get = (target, key, receiver) => {
	 ...
	 */
	NodeListImpl.prototype.set = (target, key, value, receiver) => {
		if (!util_1.isString(key)) {
	 ...
	 * @param root - root node
	 */
	NodeListImpl._create = (root) => new NodeListImpl(root);
	return NodeListImpl;
})();
