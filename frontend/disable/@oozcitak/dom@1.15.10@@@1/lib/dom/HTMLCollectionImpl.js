...
var HTMLCollectionImpl = /** @class */ (() => {
	...
	Object.defineProperty(HTMLCollectionImpl.prototype, "length", {
		...
		get: function () {
			...
			var node = algorithm_1.tree_getFirstDescendantNode(
				...
				false,
				false,
				(e) => util_1.Guard.isElementNode(e) && this._filter(e),
			);
			while (node !== null) {
				count++;
	...
	HTMLCollectionImpl.prototype.namedItem = function (key) {
		 ...
		 */
		if (key === "") return null;
		var ele = algorithm_1.tree_getFirstDescendantNode(
			this._root,
			false,
			false,
			(e) => util_1.Guard.isElementNode(e) && this._filter(e),
		...
		while (ele != null) {
			if (ele._uniqueIdentifier === key) {
				...
				for (var i = 0; i < ele._attributeList.length; i++) {
					...
					if (
						attr._localName === "name" &&
						attr._namespace === null &&
			...
			ele = algorithm_1.tree_getNextDescendantNode(
				this._root,
				ele,
				false,
				false,
	...
	HTMLCollectionImpl.prototype[Symbol.iterator] = function () {
		...
		var currentNode = algorithm_1.tree_getFirstDescendantNode(
			root,
			false,
			false,
			(e) => util_1.Guard.isElementNode(e) && filter(e),
		);
		return {
			next: () => {
				if (currentNode === null) {
					...
					currentNode = algorithm_1.tree_getNextDescendantNode(
						root,
						currentNode,
						false,
						false,
	 ...
	 */
	HTMLCollectionImpl.prototype.get = (target, key, receiver) => {
		if (
			!util_2.isString(key) ||
			HTMLCollectionImpl.reservedNames.indexOf(key) !== -1
		) {
	 ...
	 */
	HTMLCollectionImpl.prototype.set = (target, key, value, receiver) => {
		if (
			!util_2.isString(key) ||
			HTMLCollectionImpl.reservedNames.indexOf(key) !== -1
		) {
		...
		var node = isNaN(index)
			? target.namedItem(key) || undefined
