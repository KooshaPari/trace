...
var DOMTokenListImpl = /** @class */ (() => {
	...
	function DOMTokenListImpl(element, attribute) {
		...
		if (DOMImpl_1.dom.features.steps) {
			algorithm_1.dom_runAttributeChangeSteps(
				element,
				localName,
				value,
				value,
	...
	DOMTokenListImpl.prototype.item = function (index) {
		...
		try {
			for (
				var _b = __values(this._tokenSet), _c = _b.next();
				!_c.done;
				_c = _b.next()
			) {
				var token = _c.value;
	...
	DOMTokenListImpl.prototype.add = function () {
		...
		try {
			 ...
			 */
			for (
				var tokens_1 = __values(tokens), tokens_1_1 = tokens_1.next();
				!tokens_1_1.done;
	...
	DOMTokenListImpl.prototype.remove = function () {
		...
		try {
			 ...
			 */
			for (
				var tokens_2 = __values(tokens), tokens_2_1 = tokens_2.next();
				!tokens_2_1.done;
	...
	DOMTokenListImpl.prototype.replace = function (token, newToken) {
		...
		if (token === "" || newToken === "") {
			...
			infra_1.codePoint.ASCIIWhiteSpace.test(token) ||
			infra_1.codePoint.ASCIIWhiteSpace.test(newToken)
		) {
			throw new DOMException_1.InvalidCharacterError(
				"Token cannot contain whitespace.",
			);
	...
	Object.defineProperty(DOMTokenListImpl.prototype, "value", {
		...
		set: function (value) {
			 ...
			 * value.
			 */
			algorithm_1.element_setAnAttributeValue(
				this._element,
				this._attribute._localName,
				value,
			);
		},
		enumerable: true,
	 ...
	 * @param attribute - associated attribute
	 */
	DOMTokenListImpl._create = (element, attribute) =>
		new DOMTokenListImpl(element, attribute);
