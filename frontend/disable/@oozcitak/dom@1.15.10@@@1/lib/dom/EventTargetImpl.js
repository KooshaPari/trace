...
var EventTargetImpl = /** @class */ (() => {
	...
	/** @inheritdoc */
	EventTargetImpl.prototype.addEventListener = function (
		type,
		callback,
		options,
	) {
		 ...
		 */
		var _a = __read(algorithm_1.eventTarget_flattenMore(options), 3),
			capture = _a[0],
			passive = _a[1],
			once = _a[2];
	...
	/** @inheritdoc */
	EventTargetImpl.prototype.removeEventListener = function (
		type,
		callback,
		options,
	) {
		...
		for (var i = 0; i < this._eventListenerList.length; i++) {
			...
			if (
				util_1.Guard.isEventListener(callback) &&
	...
	/** @inheritdoc */
	EventTargetImpl.prototype._getTheParent = (event) => null;
