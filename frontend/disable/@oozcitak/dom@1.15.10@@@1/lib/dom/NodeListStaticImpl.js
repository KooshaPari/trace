...
var NodeListStaticImpl = /** @class */ (() => {
	...
	NodeListStaticImpl.prototype.forEach = function (callback, thisArg) {
		...
		try {
			for (
				var _b = __values(this._items), _c = _b.next();
				!_c.done;
				_c = _b.next()
			) {
				var node = _c.value;
	 ...
	 */
	NodeListStaticImpl.prototype.get = (target, key, receiver) => {
	 ...
	 */
	NodeListStaticImpl.prototype.set = (target, key, value, receiver) => {
		if (!util_1.isString(key)) {
	 ...
	 */
	NodeListStaticImpl._create = (root, items) => {
