...
var ElementImpl = /** @class */ ((_super) => {
	...
	ElementImpl.prototype.getAttributeNames = function () {
		...
		try {
			for (
				var _b = __values(this._attributeList), _c = _b.next();
				!_c.done;
				_c = _b.next()
			) {
				var attr = _c.value;
	...
	ElementImpl.prototype.getAttributeNS = function (namespace, localName) {
		 ...
		 * 3. Return attr’s value.
		 */
		var attr = algorithm_1.element_getAnAttributeByNamespaceAndLocalName(
			namespace,
			localName,
			this,
		);
	...
	ElementImpl.prototype.setAttribute = function (qualifiedName, value) {
		 ...
		 */
		if (
			this._namespace === infra_1.namespace.HTML &&
			this._nodeDocument._type === "html"
		) {
	...
	/** @inheritdoc */
	ElementImpl.prototype.setAttributeNS = function (
		namespace,
		qualifiedName,
		value,
	) {
		 ...
		 */
		var _a = __read(
				algorithm_1.namespace_validateAndExtract(namespace, qualifiedName),
				3,
			),
			ns = _a[0],
			prefix = _a[1],
	...
	ElementImpl.prototype.removeAttributeNS = function (namespace, localName) {
		 ...
		 */
		algorithm_1.element_removeAnAttributeByNamespaceAndLocalName(
			namespace,
			localName,
			this,
		);
	...
	ElementImpl.prototype.getAttributeNodeNS = function (namespace, localName) {
		 ...
		 * the context object.
		 */
		return algorithm_1.element_getAnAttributeByNamespaceAndLocalName(
			namespace,
			localName,
			this,
		);
	...
	ElementImpl.prototype.attachShadow = function (init) {
		 ...
		 */
		if (
			!algorithm_1.customElement_isValidCustomElementName(this._localName) &&
		...
		if (
			...
			var definition = algorithm_1.customElement_lookUpACustomElementDefinition(
				this._nodeDocument,
				this._namespace,
				this._localName,
				this._is,
	...
	/** @inheritdoc */
	ElementImpl.prototype.closest = (selectors) => {
		/**
		 * TODO: Selectors
	...
	/** @inheritdoc */
	ElementImpl.prototype.matches = (selectors) => {
		/**
		 * TODO: Selectors
		 * 1. Let s be the result of parse a selector from selectors. [SELECTORS4]
		 * 2. If s is failure, throw a "SyntaxError" DOMException.
	...
	/** @inheritdoc */
	ElementImpl.prototype.getElementsByTagNameNS = function (
		namespace,
		localName,
	) {
		/**
		 ...
		 */
		return algorithm_1.node_listOfElementsWithNamespace(
			namespace,
			localName,
			this,
		);
	...
	/* istanbul ignore next */
	ElementImpl.prototype.querySelector = (selectors) => {
		throw new Error("Mixin: ParentNode not implemented.");
	};
	/* istanbul ignore next */
	ElementImpl.prototype.querySelectorAll = (selectors) => {
		throw new Error("Mixin: ParentNode not implemented.");
	};
	 ...
	 */
	ElementImpl._create = (document, localName, namespace, namespacePrefix) => {
		if (namespace === void 0) {
			namespace = null;
		}
