...
var ParentNodeImpl = /** @class */ (() => {
	...
	Object.defineProperty(ParentNodeImpl.prototype, "childElementCount", {
		...
		get: function () {
			...
			try {
				for (
					var _b = __values(util_1.Cast.asNode(this)._children), _c = _b.next();
					!_c.done;
					_c = _b.next()
				) {
	...
	ParentNodeImpl.prototype.prepend = function () {
		...
		var childNode = algorithm_1.parentNode_convertNodesIntoANode(
			nodes,
			node._nodeDocument,
		);
	...
	ParentNodeImpl.prototype.querySelector = function (selectors) {
		 ...
		 */
		var node = util_1.Cast.asNode(this);
		var result = algorithm_1.selectors_scopeMatchASelectorsString(
			selectors,
			node,
		);
