...
function text_descendantTextContent(node) {
	 ...
	 */
	var contents = "";
	var text = TreeAlgorithm_1.tree_getFirstDescendantNode(
		node,
		false,
		false,
	...
	while (text !== null) {
		contents += text._data;
		text = TreeAlgorithm_1.tree_getNextDescendantNode(
			node,
			text,
			false,
			false,
...
function text_split(node, offset) {
	...
	var newData = CharacterDataAlgorithm_1.characterData_substringData(
		node,
		offset,
		count,
	);
