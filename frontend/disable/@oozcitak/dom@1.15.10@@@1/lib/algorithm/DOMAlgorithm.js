...
function dom_runAttributeChangeSteps(
	...
	try {
		// run custom steps
		for (
			var _b = __values(element._attributeChangeSteps), _c = _b.next();
			!_c.done;
			_c = _b.next()
		) {
			var step = _c.value;
...
function removeNodeIterator(nodeIterator, toBeRemovedNode) {
	...
	if (nodeIterator._pointerBeforeReference) {
		 ...
		 */
		while (true) {
			var nextNode = TreeAlgorithm_1.tree_getFollowingNode(
				nodeIterator._root,
				toBeRemovedNode,
			);
			if (
				nextNode !== null &&
				TreeAlgorithm_1.tree_isDescendantOf(
					nodeIterator._root,
					nextNode,
					true,
				) &&
	...
	if (toBeRemovedNode._previousSibling === null) {
		...
		var childNode = TreeAlgorithm_1.tree_getFirstDescendantNode(
		...
		while (childNode !== null) {
			...
			childNode = TreeAlgorithm_1.tree_getNextDescendantNode(
				toBeRemovedNode._previousSibling,
				childNode,
				true,
				false,
...
function updateASlotsName(element, localName, oldValue, value, namespace) {
	...
	if (
		...
		ShadowTreeAlgorithm_1.shadowTree_assignSlotablesForATree(
			TreeAlgorithm_1.tree_rootNode(element),
		);
	}
}
/**
