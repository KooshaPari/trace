...
function document_internalCreateElementNS(
	 ...
	 */
	var _a = __read(
			NamespaceAlgorithm_1.namespace_validateAndExtract(
				namespace,
				qualifiedName,
			),
			3,
	...
	return ElementAlgorithm_1.element_createAnElement(
		document,
		localName,
		ns,
		prefix,
			...
			if (util_1.Guard.isElementNode(inclusiveDescendant)) {
				try {
					for (
						var _b =
								((e_1 = void 0),
								__values(inclusiveDescendant._attributeList._asArray())),
							_c = _b.next();
						!_c.done;
						_c = _b.next()
					) {
