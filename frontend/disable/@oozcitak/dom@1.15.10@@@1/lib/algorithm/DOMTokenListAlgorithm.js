...
function tokenList_validationSteps(tokenList, token) {
	...
	if (!DOMAlgorithm_1.dom_hasSupportedTokens(tokenList._attribute._localName)) {
		throw new TypeError(
			"There are no supported tokens defined for attribute name: '" +
				tokenList._attribute._localName +
				"'.",
		);
	}
	return DOMAlgorithm_1.dom_getSupportedTokens(
		tokenList._attribute._localName,
...
function tokenList_updateSteps(tokenList) {
	...
	ElementAlgorithm_1.element_setAnAttributeValue(
		tokenList._element,
		tokenList._attribute._localName,
		OrderedSetAlgorithm_1.orderedSet_serialize(tokenList._tokenSet),
	);
}
...
function tokenList_serializeSteps(tokenList) {
	 ...
	 */
	return ElementAlgorithm_1.element_getAnAttributeValue(
		tokenList._element,
		tokenList._attribute._localName,
	);
}
