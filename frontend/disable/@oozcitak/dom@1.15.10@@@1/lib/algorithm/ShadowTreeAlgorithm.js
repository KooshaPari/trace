...
function shadowTree_isConnected(element) {
	 ...
	 */
	return util_1.Guard.isDocumentNode(
		TreeAlgorithm_1.tree_rootNode(element, true),
	);
}
...
function shadowTree_findASlot(slotable, openFlag) {
	...
	var child = TreeAlgorithm_1.tree_getFirstDescendantNode(
		shadow,
		false,
		true,
	...
	while (child !== null) {
		...
		child = TreeAlgorithm_1.tree_getNextDescendantNode(
			shadow,
			child,
			false,
			true,
...
function shadowTree_findSlotables(slot) {
	...
	try {
		for (
			var _b = __values(host._children), _c = _b.next();
			!_c.done;
			_c = _b.next()
		) {
...
function shadowTree_findFlattenedSlotables(slot) {
	...
	if (util_2.isEmpty(slotables)) {
		try {
			for (
				var _c = __values(slot._children), _d = _c.next();
				!_d.done;
				_d = _c.next()
			) {
	...
	try {
		 ...
		 */
		for (
			var slotables_1 = __values(slotables), slotables_1_1 = slotables_1.next();
			!slotables_1_1.done;
			slotables_1_1 = slotables_1.next()
		) {
			 ...
			 */
			if (
				util_1.Guard.isSlot(node) &&
				util_1.Guard.isShadowRoot(TreeAlgorithm_1.tree_rootNode(node))
			) {
				/**
...
function shadowTree_assignSlotables(slot) {
	...
	try {
		for (
			var slotables_2 = __values(slotables), slotables_2_1 = slotables_2.next();
			!slotables_2_1.done;
			slotables_2_1 = slotables_2.next()
		) {
