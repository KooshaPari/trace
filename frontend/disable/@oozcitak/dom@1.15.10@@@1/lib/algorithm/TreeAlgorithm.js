...
function _getNextDescendantNode(root, node, shadow) {
	...
	if (
		...
		util_1.Guard.isShadowRoot(node.shadowRoot)
	) {
	...
	return (
		...
		(_a[Symbol.iterator] = () => {
			var currentNode = self
				? node
				: _getNextDescendantNode(node, node, shadow);
			return {
				next: () => {
...
function tree_getDescendantElements(node, self, shadow, filter) {
	...
	return (
		(_a = {}),
		(_a[Symbol.iterator] = () => {
			var it = tree_getDescendantNodes(node, self, shadow, (e) =>
				util_1.Guard.isElementNode(e),
			)[Symbol.iterator]();
			...
			return {
				next: () => {
					if (currentNode === null) {
						...
						var result = { done: false, value: currentNode };
						currentNode = tree_getNextAncestorNode(
							node,
							currentNode,
							self,
							filter,
...
function tree_isConstrained(node) {
	...
	switch (node._nodeType) {
		case interfaces_1.NodeType.Document: {
			...
			try {
				for (
					var _d = __values(node._children), _e = _d.next();
					!_e.done;
					_e = _d.next()
				) {
		...
		case interfaces_1.NodeType.Element:
			try {
				for (
					var _f = __values(node._children), _g = _f.next();
					!_g.done;
					_g = _f.next()
				) {
	...
	try {
		for (
			var _h = __values(node._children), _j = _h.next();
			!_j.done;
			_j = _h.next()
		) {
...
function tree_isAncestorOf(node, other, self, shadow) {
	...
	var ancestor = self
		? node
		: shadow && util_1.Guard.isShadowRoot(node)
			? node._host
			: node._parent;
	while (ancestor !== null) {
		if (ancestor === other) return true;
		ancestor =
			shadow && util_1.Guard.isShadowRoot(ancestor)
				? ancestor._host
				: ancestor._parent;
	}
	return false;
...
function tree_isHostIncludingAncestorOf(node, other, self) {
	...
	if (
		util_1.Guard.isDocumentFragmentNode(root) &&
		root._host !== null &&
...
function tree_retarget(a, b) {
	...
	while (true) {
		...
		if (
			b &&
			util_1.Guard.isNode(b) &&
			tree_isAncestorOf(rootOfA, b, true, true)
		) {
			return a;
		}
		a = rootOfA.host;
