...
function element_change(attribute, element, value) {
	...
	if (DOMImpl_1.dom.features.mutationObservers) {
		MutationObserverAlgorithm_1.observer_queueAttributeMutationRecord(
			element,
	...
	if (DOMImpl_1.dom.features.steps) {
		DOMAlgorithm_1.dom_runAttributeChangeSteps(
			element,
			attribute._localName,
			attribute._value,
			value,
...
function element_getAnAttributeValue(element, localName, namespace) {
	 ...
	 */
	var attr = element_getAnAttributeByNamespaceAndLocalName(
		namespace,
		localName,
		element,
	);
...
function element_setAnAttribute(attr, element) {
	...
	if (attr._element !== null && attr._element !== element)
		throw new DOMException_1.InUseAttributeError(
			"This attribute already exists in the document: " +
				attr._qualifiedName +
				" as a child of " +
				attr._element._qualifiedName +
				".",
		);
	var oldAttr = element_getAnAttributeByNamespaceAndLocalName(
		attr._namespace || "",
		attr._localName,
		element,
	);
...
function element_setAnAttributeValue(
	 ...
	 */
	var attribute = element_getAnAttributeByNamespaceAndLocalName(
		namespace || "",
		localName,
		element,
	);
	if (attribute === null) {
		var newAttr = CreateAlgorithm_1.create_attr(
...
function element_createAnElement(
	...
	var result = null;
	if (!DOMImpl_1.dom.features.customElements) {
		result = CreateAlgorithm_1.create_element(
			document,
			localName,
			namespace,
			prefix,
	...
	if (definition !== null && definition.name !== definition.localName) {
		 ...
		 * and definition.
		 */
		var elemenInterface = DocumentAlgorithm_1.document_elementInterface(
			localName,
		...
		if (synchronousCustomElementsFlag) {
			...
			try {
				...
				result = CreateAlgorithm_1.create_htmlUnknownElement(
					document,
					localName,
					infra_1.namespace.HTML,
					prefix,
			 ...
			 * definition.
			 */
			result = CreateAlgorithm_1.create_htmlElement(
				document,
				localName,
				infra_1.namespace.HTML,
				prefix,
		 ...
		 */
		var elementInterface = DocumentAlgorithm_1.document_elementInterface(
			localName,
			namespace,
		);
		...
		if (
			...
			(is !== null ||
				CustomElementAlgorithm_1.customElement_isValidCustomElementName(
					localName,
				))
		) {
			result._customElementState = "undefined";
...
function element_insertAdjacent(element, where, node) {
	...
	switch (where.toLowerCase()) {
		case "beforebegin":
			...
			return MutationAlgorithm_1.mutation_preInsert(
				node,
				element._parent,
				element,
			);
