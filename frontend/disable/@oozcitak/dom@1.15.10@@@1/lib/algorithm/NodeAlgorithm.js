...
function node_clone(node, document, cloneChildrenFlag) {
	...
	if (util_1.Guard.isElementNode(node)) {
		 ...
		 */
		copy = ElementAlgorithm_1.element_createAnElement(
			document,
			node._localName,
			node._namespace,
		...
		try {
			for (
				var _c = __values(node._attributeList), _d = _c.next();
				!_d.done;
				_d = _c.next()
			) {
		...
		if (util_1.Guard.isDocumentNode(node)) {
			...
			var doctype = CreateAlgorithm_1.create_documentType(
				document,
				node._name,
				node._publicId,
				node._systemId,
			...
			copy = CreateAlgorithm_1.create_processingInstruction(
				document,
				node._target,
				node._data,
			);
...
function node_equals(a, b) {
	 ...
	 */
	if (
		util_1.Guard.isDocumentTypeNode(a) &&
		util_1.Guard.isDocumentTypeNode(b)
	) {
		if (
			a._name !== b._name ||
		...
		if (
			a._namespace !== b._namespace ||
			a._namespacePrefix !== b._namespacePrefix ||
		...
		util_1.Guard.isProcessingInstructionNode(a) &&
		util_1.Guard.isProcessingInstructionNode(b)
	) {
		if (a._target !== b._target || a._data !== b._data) return false;
	} else if (
		util_1.Guard.isCharacterDataNode(a) &&
		util_1.Guard.isCharacterDataNode(b)
	) {
	...
	if (util_1.Guard.isElementNode(a) && util_1.Guard.isElementNode(b)) {
		...
		try {
			for (
				var _c = __values(a._attributeList), _d = _c.next();
				!_d.done;
				_d = _c.next()
			) {
				var attrA = _d.value;
		...
		try {
			for (
				var _e = __values(b._attributeList), _f = _e.next();
				!_f.done;
				_f = _e.next()
			) {
				var attrB = _f.value;
...
function node_listOfElementsWithQualifiedName(qualifiedName, root) {
	...
	if (qualifiedName === "*") {
		...
		return CreateAlgorithm_1.create_htmlCollection(root, (ele) => {
			if (
				ele._namespace === infra_1.namespace.HTML &&
		...
		return CreateAlgorithm_1.create_htmlCollection(
			root,
			(ele) => ele._qualifiedName === qualifiedName,
		);
	}
}
exports.node_listOfElementsWithQualifiedName =
...
function node_listOfElementsWithNamespace(namespace, localName, root) {
	...
	if (namespace === "*" && localName === "*") {
		...
		return CreateAlgorithm_1.create_htmlCollection(
			root,
			(ele) => ele._localName === localName,
		);
	} else if (localName === "*") {
		return CreateAlgorithm_1.create_htmlCollection(
			root,
			(ele) => ele._namespace === namespace,
		);
	} else {
		return CreateAlgorithm_1.create_htmlCollection(
			root,
			(ele) => ele._localName === localName && ele._namespace === namespace,
		);
	}
}
...
function node_listOfElementsWithClassNames(classNames, root) {
	...
	return CreateAlgorithm_1.create_htmlCollection(root, (ele) => {
		...
		return OrderedSetAlgorithm_1.orderedSet_contains(
			eleClasses._tokenSet,
			classes,
			caseSensitive,
		);
	});
...
function node_locateANamespace(node, prefix) {
	if (util_1.Guard.isElementNode(node)) {
		...
		for (var i = 0; i < node._attributeList.length; i++) {
			...
			if (
				attr._namespace === infra_1.namespace.XMLNS &&
		...
		util_1.Guard.isDocumentTypeNode(node) ||
		util_1.Guard.isDocumentFragmentNode(node)
	) {
		return null;
	} else if (util_1.Guard.isAttrNode(node)) {
		/**
