...
function eventTarget_addEventListener(eventTarget, listener) {
	...
	for (var i = 0; i < eventTarget._eventListenerList.length; i++) {
		...
		if (
			entry.type === listener.type &&
			entry.callback.handleEvent === listener.callback.handleEvent &&
...
function eventTarget_removeAllEventListeners(eventTarget) {
	...
	try {
		for (
			var _b = __values(eventTarget._eventListenerList), _c = _b.next();
			!_c.done;
			_c = _b.next()
		) {
			var e = _c.value;
...
exports.eventTarget_removeAllEventListeners =
	eventTarget_removeAllEventListeners;
