...
function event_createAnEvent(eventInterface, realm) {
	...
	var dictionary = {};
	var event = event_innerEventCreationSteps(
		eventInterface,
		realm,
		new Date(),
		dictionary,
 ...
 *
 */
function event_innerEventCreationSteps(
	eventInterface,
	realm,
	time,
	dictionary,
...
function event_dispatch(
	...
	var relatedTarget = TreeAlgorithm_1.tree_retarget(
		event._relatedTarget,
		target,
	);
	if (target !== relatedTarget || target === event._relatedTarget) {
		...
		try {
			for (
				var _c = __values(event._touchTargetList), _d = _c.next();
				!_d.done;
				_d = _c.next()
			) {
		...
		var isActivationEvent =
			util_1.Guard.isMouseEvent(event) && event._type === "click";
		if (isActivationEvent && target._activationBehavior !== undefined) {
			activationTarget = target;
		}
		var slotable =
			util_1.Guard.isSlotable(target) &&
			ShadowTreeAlgorithm_1.shadowTree_isAssigned(target)
				? target
				: null;
		...
		while (parent !== null && util_1.Guard.isNode(parent)) {
			 ...
			 */
			if (
				util_1.Guard.isSlotable(parent) &&
				ShadowTreeAlgorithm_1.shadowTree_isAssigned(parent)
			) {
				slotable = parent;
			}
			relatedTarget = TreeAlgorithm_1.tree_retarget(
				event._relatedTarget,
				parent,
			);
			touchTargets = [];
			try {
				for (
					var _e = ((e_2 = void 0), __values(event._touchTargetList)),
						_f = _e.next();
					!_f.done;
					_f = _e.next()
				) {
			 ...
			 */
			if (
				util_1.Guard.isWindow(parent) ||
				(util_1.Guard.isNode(parent) &&
					util_1.Guard.isNode(target) &&
					TreeAlgorithm_1.tree_isAncestorOf(
						TreeAlgorithm_1.tree_rootNode(target, true),
						parent,
						true,
						true,
					))
		...
		if (clearTargetsStruct !== null) {
			if (
				util_1.Guard.isNode(clearTargetsStruct.shadowAdjustedTarget) &&
				util_1.Guard.isShadowRoot(
					TreeAlgorithm_1.tree_rootNode(
				...
				util_1.Guard.isNode(clearTargetsStruct.relatedTarget) &&
				util_1.Guard.isShadowRoot(
					TreeAlgorithm_1.tree_rootNode(clearTargetsStruct.relatedTarget, true),
				)
			) {
				clearTargets = true;
			} else {
				for (var j = 0; j < clearTargetsStruct.touchTargetList.length; j++) {
					...
					if (
						util_1.Guard.isNode(struct) &&
						util_1.Guard.isShadowRoot(
							TreeAlgorithm_1.tree_rootNode(struct, true),
						)
					) {
						clearTargets = true;
						break;
	...
	if (activationTarget !== null) {
		if (
			...
			activationTarget._legacyCanceledActivationBehavior !== undefined
		) {
 ...
 */
function event_appendToAnEventPath(
	event,
	invocationTarget,
	...
	if (
		util_1.Guard.isNode(invocationTarget) &&
	...
	if (
		util_1.Guard.isShadowRoot(invocationTarget) &&
...
function event_invoke(struct, event, phase, legacyOutputDidListenersThrowFlag) {
	...
	var listeners = new (Array.bind.apply(
		Array,
		__spread([void 0], targetListeners),
	))();
	/**
...
function event_createLegacyEvent(eventInterface) {
	...
	if (constructor === null) {
		throw new DOMException_1.NotSupportedError(
			"Event constructor not found for interface " + eventInterface + ".",
		);
	}
	/**
...
exports.event_getterEventHandlerIDLAttribute =
	event_getterEventHandlerIDLAttribute;
/**
...
exports.event_determineTheTargetOfAnEventHandler =
...
exports.event_getTheCurrentValueOfAnEventHandler =
	event_getTheCurrentValueOfAnEventHandler;
/**
