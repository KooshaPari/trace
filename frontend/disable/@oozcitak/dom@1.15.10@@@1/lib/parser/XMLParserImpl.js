...
var XMLParserImpl = /** @class */ (() => {
			...
			switch (token.type) {
				...
				case interfaces_1.TokenType.DocType: {
					...
					if (!algorithm_1.xml_isPubidChar(doctype.pubId)) {
						throw new Error(
							"DocType public identifier does not match PubidChar construct.",
						);
					}
					if (
						!algorithm_1.xml_isLegalChar(doctype.sysId) ||
						(doctype.sysId.indexOf('"') !== -1 &&
							doctype.sysId.indexOf("'") !== -1)
					) {
						throw new Error(
							"DocType system identifier contains invalid characters.",
						);
					}
					context.appendChild(
				...
				case interfaces_1.TokenType.CDATA: {
					var cdata = token;
					if (
						!algorithm_1.xml_isLegalChar(cdata.data) ||
				...
				case interfaces_1.TokenType.Comment: {
					var comment = token;
					if (
						!algorithm_1.xml_isLegalChar(comment.data) ||
						comment.data.indexOf("--") !== -1 ||
				...
				case interfaces_1.TokenType.PI: {
					...
					if (pi.target.indexOf(":") !== -1 || /^xml$/i.test(pi.target)) {
						throw new Error(
							"Processing instruction target contains invalid characters.",
						);
					}
					if (
						!algorithm_1.xml_isLegalChar(pi.data) ||
						pi.data.indexOf("?>") !== -1
					) {
						throw new Error(
							"Processing instruction data contains invalid characters.",
						);
					}
					context.appendChild(
						doc.createProcessingInstruction(pi.target, pi.data),
					);
					break;
				}
				...
				case interfaces_1.TokenType.Element: {
					...
					var _c = __read(algorithm_1.namespace_extractQName(element.name), 2),
						prefix = _c[0],
						localName = _c[1];
					if (
						localName.indexOf(":") !== -1 ||
						!algorithm_1.xml_isName(localName)
					) {
					...
					try {
						for (
							var _d = ((e_1 = void 0), __values(element.attributes)),
								_e = _d.next();
							!_e.done;
							_e = _d.next()
						) {
							...
							if (attName === "xmlns") {
								...
								var _g = __read(algorithm_1.namespace_extractQName(attName), 2),
									attPrefix = _g[0],
									attLocalName = _g[1];
								if (attPrefix === "xmlns") {
					...
					try {
						for (
							var _h = ((e_2 = void 0), __values(element.attributes)),
								_j = _h.next();
							!_j.done;
							_j = _h.next()
						) {
							var _k = __read(_j.value, 2),
								attName = _k[0],
								attValue = _k[1];
							var _l = __read(algorithm_1.namespace_extractQName(attName), 2),
								attPrefix = _l[0],
								attLocalName = _l[1];
							var attNamespace = null;
							if (
								attPrefix === "xmlns" ||
								(attPrefix === null && attLocalName === "xmlns")
							) {
								...
								if (
									attNamespace !== null &&
									elementNode.isDefaultNamespace(attNamespace)
								) {
									attNamespace = null;
							...
							if (
								attLocalName.indexOf(":") !== -1 ||
								!algorithm_1.xml_isName(attLocalName)
							) {
								throw new Error(
									"Attribute local name contains invalid characters.",
								);
							}
							...
							if (attNamespace !== null)
								elementNode.setAttributeNS(
									attNamespace,
									attName,
									this._decodeAttributeValue(attValue),
								);
							else
	 ...
	 */
	XMLParserImpl.prototype._decodeText = (text) =>
		text == null
			? text
	 ...
	 */
	XMLParserImpl.prototype._decodeAttributeValue = (text) =>
		text == null
			? text
			: text.replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&amp;/g, "&");
