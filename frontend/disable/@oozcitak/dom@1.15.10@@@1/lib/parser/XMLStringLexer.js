...
var XMLStringLexer = /** @class */ (() => {
	...
	function XMLStringLexer(str, options) {
		...
		if (options) {
			this._options.skipWhitespaceOnlyText =
				options.skipWhitespaceOnlyText || false;
		}
	}
	/**
	...
	XMLStringLexer.prototype.nextToken = function () {
		...
		if (this._options.skipWhitespaceOnlyText) {
			if (
				token.type === interfaces_1.TokenType.Text &&
	...
	XMLStringLexer.prototype.declaration = function () {
		...
		while (!this.eof()) {
			...
			if (this.skipIfStartsWith("?>")) {
				...
				// attribute name
				var _a = __read(this.attribute(), 2),
					attName = _a[0],
					attValue = _a[1];
	...
	XMLStringLexer.prototype.takeUntil2 = function (char1, char2, space) {
		...
		while (this._index < this._length) {
			...
			if (
				c !== char1 &&
				c !== char2 &&
				(!space || !XMLStringLexer.isSpace(c))
			) {
				this._index++;
			} else {
				break;
	 ...
	 * @param char - the character to check
	 */
	XMLStringLexer.isQuote = (char) => char === '"' || char === "'";
	/**
