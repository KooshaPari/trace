		...
		while (token.type !== interfaces_1.TokenType.EOF) {
			switch (token.type) {
				...
				case interfaces_1.TokenType.DocType: {
					...
					if (
						!(0, algorithm_1.xml_isLegalChar)(doctype.sysId) ||
				...
				case interfaces_1.TokenType.CDATA: {
					const cdata = token;
					if (
						!(0, algorithm_1.xml_isLegalChar)(cdata.data) ||
				...
				case interfaces_1.TokenType.Comment: {
					const comment = token;
					if (
						!(0, algorithm_1.xml_isLegalChar)(comment.data) ||
				...
				case interfaces_1.TokenType.PI: {
					...
					if (
						!(0, algorithm_1.xml_isLegalChar)(pi.data) ||
						pi.data.indexOf("?>") !== -1
					) {
						throw new Error(
				...
				case interfaces_1.TokenType.Element: {
					...
					const [prefix, localName] = (0, algorithm_1.namespace_extractQName)(
						element.name,
					);
					if (
						localName.indexOf(":") !== -1 ||
						!(0, algorithm_1.xml_isName)(localName)
					) {
					...
					for (const [attName, attValue] of element.attributes) {
						if (attName === "xmlns") {
							...
							const [attPrefix, attLocalName] = (0,
							algorithm_1.namespace_extractQName)(attName);
							if (attPrefix === "xmlns") {
					...
					for (const [attName, attValue] of element.attributes) {
						...
						if (
							...
							!(0, algorithm_1.xml_isName)(attLocalName)
						) {
							throw new Error(
