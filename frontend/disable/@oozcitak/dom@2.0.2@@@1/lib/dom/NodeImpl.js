...
class NodeImpl extends EventTargetImpl_1.EventTargetImpl {
	...
	get _childNodes() {
		return (
			this.__childNodes ||
			(this.__childNodes = (0, algorithm_1.create_nodeList)(this))
		);
	}
	_nodeDocumentOverride;
	get _nodeDocument() {
		return (
			this._nodeDocumentOverride || DOMImpl_1.dom.window._associatedDocument
		);
	}
	...
	get _registeredObserverList() {
		return (
			this.__registeredObserverList || (this.__registeredObserverList = [])
		);
	}
	_parent = null;
	...
	get isConnected() {
		 ...
		 */
		return (
			util_1.Guard.isElementNode(this) &&
			(0, algorithm_1.shadowTree_isConnected)(this)
		);
	}
	/**
	...
	set nodeValue(value) {
		...
		if (util_1.Guard.isAttrNode(this)) {
			...
			(0, algorithm_1.characterData_replaceData)(
				this,
				0,
				this._data.length,
				value,
	...
	get textContent() {
		if (
			util_1.Guard.isDocumentFragmentNode(this) ||
			util_1.Guard.isElementNode(this)
		) {
	...
	set textContent(value) {
		...
		if (
			...
			(0, algorithm_1.characterData_replaceData)(
				this,
				0,
				(0, algorithm_1.tree_nodeLength)(this),
				value,
			);
		}
	}
	...
	normalize() {
		...
		let node = (0, algorithm_1.tree_getFirstDescendantNode)(
			this,
			false,
			false,
			(e) => util_1.Guard.isExclusiveTextNode(e),
		);
		while (node !== null) {
			descendantNodes.push(node);
			node = (0, algorithm_1.tree_getNextDescendantNode)(
				this,
				node,
				false,
				false,
		...
		for (let i = 0; i < descendantNodes.length; i++) {
			...
			const textSiblings = [];
			let data = "";
			for (const sibling of (0, algorithm_1.text_contiguousExclusiveTextNodes)(
				node,
			)) {
			...
			if (DOMImpl_1.dom.rangeList.size !== 0) {
				...
				while (
					currentNode !== null &&
					util_1.Guard.isExclusiveTextNode(currentNode)
				) {
					/**
	...
	compareDocumentPosition(other) {
		 ...
		 * attr1’s element.
		 */
		if (other === this) return interfaces_1.Position.SameNode;
		let node1 = other;
		let node2 = this;
		let attr1 = null;
		let attr2 = null;
			...
			if (attr1 && node1 && node1 === node2) {
				...
				for (let i = 0; i < node2._attributeList.length; i++) {
					...
					if ((0, algorithm_1.node_equals)(attr, attr1)) {
						return (
							interfaces_1.Position.ImplementationSpecific |
							interfaces_1.Position.Preceding
						);
					} else if ((0, algorithm_1.node_equals)(attr, attr2)) {
						return (
							interfaces_1.Position.ImplementationSpecific |
							interfaces_1.Position.Following
						);
					}
				}
			}
		...
		if (
			node1 === null ||
			node2 === null ||
			(0, algorithm_1.tree_rootNode)(node1) !==
				(0, algorithm_1.tree_rootNode)(node2)
		) {
			// nodes are disconnected
			// return a random result but cache the value for consistency
			return (
				interfaces_1.Position.Disconnected |
				interfaces_1.Position.ImplementationSpecific |
				(DOMImpl_1.dom.compareCache.check(this, other)
					? interfaces_1.Position.Preceding
					: interfaces_1.Position.Following)
			);
		}
		 ...
		 */
		if (
			(!attr1 && (0, algorithm_1.tree_isAncestorOf)(node2, node1)) ||
			(attr2 && node1 === node2)
		) {
		 ...
		 */
		if (
			(!attr2 && (0, algorithm_1.tree_isDescendantOf)(node2, node1)) ||
			(attr1 && node1 === node2)
		) {
			return (
				interfaces_1.Position.ContainedBy | interfaces_1.Position.Following
			);
		}
		/**
	...
	lookupPrefix(namespace) {
		...
		if (util_1.Guard.isElementNode(this)) {
			...
			if (this.documentElement === null) {
				...
				return (0, algorithm_1.node_locateANamespacePrefix)(
					this.documentElement,
					namespace,
				);
			}
		} else if (
			util_1.Guard.isDocumentTypeNode(this) ||
			util_1.Guard.isDocumentFragmentNode(this)
		) {
			return null;
		} else if (util_1.Guard.isAttrNode(this)) {
	...
	_getTheParent(event) {
		 ...
		 */
		if (
			util_1.Guard.isSlotable(this) &&
			(0, algorithm_1.shadowTree_isAssigned)(this)
		) {
...
(0, WebIDLAlgorithm_1.idl_defineConst)(
	NodeImpl.prototype,
	"CDATA_SECTION_NODE",
	4,
);
