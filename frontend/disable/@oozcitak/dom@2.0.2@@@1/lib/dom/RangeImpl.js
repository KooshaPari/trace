...
class RangeImpl extends AbstractRangeImpl_1.AbstractRangeImpl {
	...
	setStartBefore(node) {
		...
		(0, algorithm_1.range_setTheStart)(
			this,
			parent,
			(0, algorithm_1.tree_index)(node),
		);
	}
	/** @inheritdoc */
	setStartAfter(node) {
		...
		(0, algorithm_1.range_setTheStart)(
			this,
			parent,
			(0, algorithm_1.tree_index)(node) + 1,
		);
	}
	/** @inheritdoc */
	setEndBefore(node) {
	...
	compareBoundaryPoints(how, sourceRange) {
		 ...
		 */
		if (
			(0, algorithm_1.range_root)(this) !==
			(0, algorithm_1.range_root)(sourceRange)
		)
		 ...
		 * Return 1.
		 */
		const position = (0, algorithm_1.boundaryPoint_position)(
			thisPoint,
			otherPoint,
		);
	...
	deleteContents() {
		 ...
		 */
		if ((0, algorithm_1.range_collapsed)(this)) return;
		...
		let newOffset;
		if (
			(0, algorithm_1.tree_isAncestorOf)(
				originalEndNode,
				originalStartNode,
				true,
			)
		...
		if (util_1.Guard.isCharacterDataNode(originalStartNode)) {
			(0, algorithm_1.characterData_replaceData)(
				originalStartNode,
				originalStartOffset,
				(0, algorithm_1.tree_nodeLength)(originalStartNode) -
					originalStartOffset,
				"",
			);
		}
	...
	surroundContents(newParent) {
		 ...
		 */
		for (const node of (0, algorithm_1.range_getPartiallyContainedNodes)(
			this,
		)) {
	...
	isPointInRange(node, offset) {
		 ...
		 */
		if (
			(0, algorithm_1.tree_rootNode)(node) !== (0, algorithm_1.range_root)(this)
		) {
			return false;
		}
		/**
		...
		if (
			(0, algorithm_1.boundaryPoint_position)(bp, this._start) ===
				interfaces_1.BoundaryPosition.Before ||
			(0, algorithm_1.boundaryPoint_position)(bp, this._end) ===
	...
	intersectsNode(node) {
		 ...
		 */
		if (
			(0, algorithm_1.boundaryPoint_position)([parent, offset], this._end) ===
				interfaces_1.BoundaryPosition.Before &&
			(0, algorithm_1.boundaryPoint_position)(
				[parent, offset + 1],
				this._start,
			) === interfaces_1.BoundaryPosition.After
		) {
