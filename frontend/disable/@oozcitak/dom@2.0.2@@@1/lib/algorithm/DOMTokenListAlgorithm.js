...
function tokenList_validationSteps(tokenList, token) {
	 ...
	 */
	if (
		!(0, DOMAlgorithm_1.dom_hasSupportedTokens)(tokenList._attribute._localName)
	) {
		throw new TypeError(
			`There are no supported tokens defined for attribute name: '${tokenList._attribute._localName}'.`,
		);
	}
	return (0, DOMAlgorithm_1.dom_getSupportedTokens)(
		tokenList._attribute._localName,
...
function tokenList_updateSteps(tokenList) {
	 ...
	 */
	if (
		!tokenList._element.hasAttribute(tokenList._attribute._localName) &&
		tokenList._tokenSet.size === 0
	) {
		return;
	}
	(0, ElementAlgorithm_1.element_setAnAttributeValue)(
		tokenList._element,
		tokenList._attribute._localName,
		(0, OrderedSetAlgorithm_1.orderedSet_serialize)(tokenList._tokenSet),
	);
}
/**
...
function tokenList_serializeSteps(tokenList) {
	 ...
	 */
	return (0, ElementAlgorithm_1.element_getAnAttributeValue)(
		tokenList._element,
		tokenList._attribute._localName,
	);
}
