...
function observer_notifyMutationObservers() {
	...
	if (DOMImpl_1.dom.features.slots) {
		for (const slot of signalSet) {
			(0, EventAlgorithm_1.event_fireAnEvent)("slotchange", slot, undefined, {
				bubbles: true,
			});
		}
	}
...
function observer_queueMutationRecord(
	...
	for (const [observer, mappedOldValue] of interestedObservers) {
		 ...
		 */
		const record = (0, CreateAlgorithm_1.create_mutationRecord)(
			type,
			target,
			(0, CreateAlgorithm_1.create_nodeListStatic)(target, addedNodes),
			(0, CreateAlgorithm_1.create_nodeListStatic)(target, removedNodes),
			previousSibling,
			nextSibling,
			name,
			namespace,
