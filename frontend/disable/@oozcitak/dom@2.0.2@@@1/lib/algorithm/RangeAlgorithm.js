...
function range_isContained(node, range) {
	 ...
	 */
	return (
		(0, TreeAlgorithm_1.tree_rootNode)(node) === range_root(range) &&
		(0, BoundaryPointAlgorithm_1.boundaryPoint_position)(
			[node, 0],
			range._start,
		) === interfaces_1.BoundaryPosition.After &&
		(0, BoundaryPointAlgorithm_1.boundaryPoint_position)(
			[node, (0, TreeAlgorithm_1.tree_nodeLength)(node)],
			range._end,
...
function range_isPartiallyContained(node, range) {
	 ...
	 * or vice versa.
	 */
	const startCheck = (0, TreeAlgorithm_1.tree_isAncestorOf)(
		range._startNode,
		node,
		true,
	);
	const endCheck = (0, TreeAlgorithm_1.tree_isAncestorOf)(
		range._endNode,
		node,
		true,
	);
...
function range_setTheStart(range, node, offset) {
	...
	if (
		range_root(range) !== (0, TreeAlgorithm_1.tree_rootNode)(node) ||
		(0, BoundaryPointAlgorithm_1.boundaryPoint_position)(bp, range._end) ===
			interfaces_1.BoundaryPosition.After
	) {
		range._end = bp;
	}
...
function range_setTheEnd(range, node, offset) {
	...
	if (
		range_root(range) !== (0, TreeAlgorithm_1.tree_rootNode)(node) ||
		(0, BoundaryPointAlgorithm_1.boundaryPoint_position)(bp, range._start) ===
			interfaces_1.BoundaryPosition.Before
	) {
		range._start = bp;
	}
...
function range_extract(range) {
	 ...
	 */
	const fragment = (0, CreateAlgorithm_1.create_documentFragment)(
	...
	if (
		...
		clone._data = (0, CharacterDataAlgorithm_1.characterData_substringData)(
			originalStartNode,
			originalStartOffset,
			originalEndOffset - originalStartOffset,
		);
		(0, MutationAlgorithm_1.mutation_append)(clone, fragment);
		(0, CharacterDataAlgorithm_1.characterData_replaceData)(
			originalStartNode,
			originalStartOffset,
			originalEndOffset - originalStartOffset,
			"",
	...
	if (util_1.Guard.isCharacterDataNode(firstPartiallyContainedChild)) {
		...
		clone._data = (0, CharacterDataAlgorithm_1.characterData_substringData)(
			originalStartNode,
			originalStartOffset,
			(0, TreeAlgorithm_1.tree_nodeLength)(originalStartNode) -
				originalStartOffset,
		);
		(0, MutationAlgorithm_1.mutation_append)(clone, fragment);
		(0, CharacterDataAlgorithm_1.characterData_replaceData)(
		...
		(0, MutationAlgorithm_1.mutation_append)(clone, fragment);
		const subrange = (0, CreateAlgorithm_1.create_range)(
			[originalStartNode, originalStartOffset],
			[
				firstPartiallyContainedChild,
				(0, TreeAlgorithm_1.tree_nodeLength)(firstPartiallyContainedChild),
			],
		);
...
function range_insert(node, range) {
	 ...
	 */
	(0, MutationAlgorithm_1.mutation_ensurePreInsertionValidity)(
		node,
		parent,
		referenceNode,
	);
	 ...
	 */
	if (util_1.Guard.isTextNode(range._startNode)) {
		referenceNode = (0, TextAlgorithm_1.text_split)(
			range._startNode,
			range._startOffset,
		);
	}
	...
	let newOffset =
		referenceNode === null
			? (0, TreeAlgorithm_1.tree_nodeLength)(parent)
			: (0, TreeAlgorithm_1.tree_index)(referenceNode);
	/**
...
function range_getContainedNodes(range) {
	return {
		[Symbol.iterator]: () => {
			...
			let currentNode = (0, TreeAlgorithm_1.tree_getFirstDescendantNode)(
				container,
			);
			return {
				next: () => {
					while (currentNode && !range_isContained(currentNode, range)) {
						currentNode = (0, TreeAlgorithm_1.tree_getNextDescendantNode)(
							container,
							currentNode,
						);
					}
