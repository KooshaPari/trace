...
function mutation_ensurePreInsertionValidity(node, parent, child) {
	...
	if ((0, TreeAlgorithm_1.tree_isHostIncludingAncestorOf)(parent, node, true))
		throw new DOMException_1.HierarchyRequestError(
			`The node to be inserted cannot be an inclusive ancestor of parent node. Node is ${node.nodeName}, parent node is ${parent.nodeName}.`,
		);
	/**
	...
	if (child !== null && child._parent !== parent)
		throw new DOMException_1.NotFoundError(
			`The reference child node cannot be found under parent node. Child node is ${child.nodeName}, parent node is ${parent.nodeName}.`,
		);
	/**
	...
	if (
		...
		throw new DOMException_1.HierarchyRequestError(
			`Only document fragment, document type, element, text, processing instruction, cdata section or comment nodes can be inserted. Node is ${node.nodeName}.`,
		);
	/**
	...
	if (
		...
		throw new DOMException_1.HierarchyRequestError(
			`Cannot insert a text node as a child of a document node. Node is ${node.nodeName}.`,
		);
	if (
		nodeNodeType === interfaces_1.NodeType.DocumentType &&
		parentNodeType !== interfaces_1.NodeType.Document
		...
		throw new DOMException_1.HierarchyRequestError(
			`A document type node can only be inserted under a document node. Parent node is ${parent.nodeName}.`,
		);
	/**
		...
		if (nodeNodeType === interfaces_1.NodeType.DocumentFragment) {
			...
			for (const childNode of node._children) {
				if (childNode._nodeType === interfaces_1.NodeType.Element) eleCount++;
					...
					throw new DOMException_1.HierarchyRequestError(
						`Cannot insert text a node as a child of a document node. Node is ${childNode.nodeName}.`,
					);
			}
			if (eleCount > 1) {
				throw new DOMException_1.HierarchyRequestError(
					`A document node can only have one document element node. Document fragment to be inserted has ${eleCount} element nodes.`,
				);
			} else if (eleCount === 1) {
			...
			for (const ele of parent._children) {
				if (ele._nodeType === interfaces_1.NodeType.Element)
					throw new DOMException_1.HierarchyRequestError(
						`Document already has a document element node. Node is ${node.nodeName}.`,
					);
			}
			if (child) {
				if (childNodeType === interfaces_1.NodeType.DocumentType)
					throw new DOMException_1.HierarchyRequestError(
						`Cannot insert an element node before a document type node. Node is ${node.nodeName}.`,
					);
				let doctypeChild = child._nextSibling;
				while (doctypeChild) {
					if (doctypeChild._nodeType === interfaces_1.NodeType.DocumentType)
			...
			for (const ele of parent._children) {
				if (ele._nodeType === interfaces_1.NodeType.DocumentType)
					throw new DOMException_1.HierarchyRequestError(
						`Document already has a document type node. Node is ${node.nodeName}.`,
					);
			}
			if (child) {
				...
				while (elementChild) {
					if (elementChild._nodeType === interfaces_1.NodeType.Element)
						throw new DOMException_1.HierarchyRequestError(
							`Cannot insert a document type node before an element node. Node is ${node.nodeName}.`,
						);
...
function mutation_insert(node, parent, child, suppressObservers) {
	...
	if (DOMImpl_1.dom.features.mutationObservers) {
		if (node._nodeType === interfaces_1.NodeType.DocumentFragment) {
			(0, MutationObserverAlgorithm_1.observer_queueTreeMutationRecord)(
				node,
				[],
				nodes,
				null,
	...
	for (let i = 0; i < nodes.length; i++) {
		...
		if (DOMImpl_1.dom.features.slots) {
			if (
				util_1.Guard.isShadowRoot((0, TreeAlgorithm_1.tree_rootNode)(parent)) &&
				util_1.Guard.isSlot(parent) &&
				(0, util_2.isEmpty)(parent._assignedNodes)
			) {
		 ...
		 */
		if (DOMImpl_1.dom.features.slots) {
			(0, ShadowTreeAlgorithm_1.shadowTree_assignSlotablesForATree)(
				(0, TreeAlgorithm_1.tree_rootNode)(node),
			);
		}
		/**
		 ...
		 * order:
		 */
		let inclusiveDescendant = (0, TreeAlgorithm_1.tree_getFirstDescendantNode)(
			node,
			true,
			true,
		);
			...
			if (DOMImpl_1.dom.features.customElements) {
				...
				if (
					...
					if (util_1.Guard.isCustomElementNode(inclusiveDescendant)) {
						 ...
						 */
						(0, CustomElementAlgorithm_1.customElement_tryToUpgrade)(
							inclusiveDescendant,
						);
					}
				}
			}
			inclusiveDescendant = (0, TreeAlgorithm_1.tree_getNextDescendantNode)(
				node,
				inclusiveDescendant,
				true,
				true,
...
function mutation_replace(child, node, parent) {
	...
	if ((0, TreeAlgorithm_1.tree_isHostIncludingAncestorOf)(parent, node, true))
		throw new DOMException_1.HierarchyRequestError(
			`The node to be inserted cannot be an ancestor of parent node. Node is ${node.nodeName}, parent node is ${parent.nodeName}.`,
		);
	/**
	...
	if (referenceChild === node) referenceChild = node._nextSibling;
	const previousSibling = child._previousSibling;
	/**
...
function mutation_preRemove(child, parent) {
	...
	if (child._parent !== parent)
		throw new DOMException_1.NotFoundError(
			`The child node cannot be found under parent node. Child node is ${child.nodeName}, parent node is ${parent.nodeName}.`,
		);
...
function mutation_remove(node, parent, suppressObservers) {
	if (DOMImpl_1.dom.rangeList.size !== 0) {
		...
		for (const range of DOMImpl_1.dom.rangeList) {
			if (
				(0, TreeAlgorithm_1.tree_isDescendantOf)(node, range._start[0], true)
			) {
	...
	if (DOMImpl_1.dom.features.steps) {
		...
		NodeIteratorAlgorithm_1.nodeIterator_iteratorList)()) {
	...
	if (DOMImpl_1.dom.features.slots) {
		if (
			...
			(0, ShadowTreeAlgorithm_1.shadowTree_isAssigned)(node)
		) {
	 ...
	 */
	if (DOMImpl_1.dom.features.slots) {
		const descendant = (0, TreeAlgorithm_1.tree_getFirstDescendantNode)(
			node,
			true,
			false,
		...
		if (descendant !== null) {
			(0, ShadowTreeAlgorithm_1.shadowTree_assignSlotablesForATree)(
				(0, TreeAlgorithm_1.tree_rootNode)(parent),
			);
	 ...
	 */
	let descendant = (0, TreeAlgorithm_1.tree_getFirstDescendantNode)(
		node,
		false,
		true,
	);
	while (descendant !== null) {
		...
		descendant = (0, TreeAlgorithm_1.tree_getNextDescendantNode)(
			node,
			descendant,
			false,
			true,
	 ...
	 */
	if (DOMImpl_1.dom.features.mutationObservers) {
		let inclusiveAncestor = (0, TreeAlgorithm_1.tree_getFirstAncestorNode)(
			parent,
			true,
		);
		while (inclusiveAncestor !== null) {
			...
			inclusiveAncestor = (0, TreeAlgorithm_1.tree_getNextAncestorNode)(
				parent,
				inclusiveAncestor,
				true,
			);
