...
function node_clone(node, document = null, cloneChildrenFlag = false) {
	...
	if (util_1.Guard.isElementNode(node)) {
		 ...
		 */
		copy = (0, ElementAlgorithm_1.element_createAnElement)(
			document,
			node._localName,
			node._namespace,
		...
		if (util_1.Guard.isDocumentNode(node)) {
			...
			const doctype = (0, CreateAlgorithm_1.create_documentType)(
				document,
				node._name,
				node._publicId,
				node._systemId,
			...
			copy = (0, CreateAlgorithm_1.create_processingInstruction)(
				document,
				node._target,
				node._data,
			);
	...
	if (DOMImpl_1.dom.features.steps) {
		(0, DOMAlgorithm_1.dom_runCloningSteps)(
			copy,
			node,
			document,
			cloneChildrenFlag,
...
function node_listOfElementsWithQualifiedName(qualifiedName, root) {
	...
	if (qualifiedName === "*") {
		...
		return (0, CreateAlgorithm_1.create_htmlCollection)(root, (ele) => {
			if (
		...
		return (0, CreateAlgorithm_1.create_htmlCollection)(
			root,
...
function node_listOfElementsWithClassNames(classNames, root) {
	...
	return (0, CreateAlgorithm_1.create_htmlCollection)(root, (ele) => {
		...
		return (0, OrderedSetAlgorithm_1.orderedSet_contains)(
