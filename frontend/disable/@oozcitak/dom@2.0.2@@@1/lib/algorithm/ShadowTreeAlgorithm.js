...
function shadowTree_isConnected(element) {
	...
	return util_1.Guard.isDocumentNode(
		(0, TreeAlgorithm_1.tree_rootNode)(element, true),
	);
}
/**
...
function shadowTree_findASlot(slotable, openFlag = false) {
	...
	let child = (0, TreeAlgorithm_1.tree_getFirstDescendantNode)(
		shadow,
		false,
		true,
	...
	while (child !== null) {
		...
		child = (0, TreeAlgorithm_1.tree_getNextDescendantNode)(
			shadow,
			child,
			false,
			true,
...
function shadowTree_findFlattenedSlotables(slot) {
	...
	for (const node of slotables) {
		...
		if (
			...
			util_1.Guard.isShadowRoot((0, TreeAlgorithm_1.tree_rootNode)(node))
		) {
			/**
