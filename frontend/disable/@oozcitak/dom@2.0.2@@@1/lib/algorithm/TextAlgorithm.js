...
function text_contiguousTextNodes(node, self = false) {
	...
	return {
		[Symbol.iterator]() {
			...
			while (
				currentNode &&
				util_1.Guard.isTextNode(currentNode._previousSibling)
			) {
...
function text_contiguousExclusiveTextNodes(node, self = false) {
	...
	return {
		[Symbol.iterator]() {
			...
			while (
				currentNode &&
				util_1.Guard.isExclusiveTextNode(currentNode._previousSibling)
			) {
				currentNode = currentNode._previousSibling;
			}
			return {
...
function text_descendantTextContent(node) {
	 ...
	 */
	let contents = "";
	let text = (0, TreeAlgorithm_1.tree_getFirstDescendantNode)(
		node,
		false,
		false,
		(e) => util_1.Guard.isTextNode(e),
	);
	while (text !== null) {
		contents += text._data;
		text = (0, TreeAlgorithm_1.tree_getNextDescendantNode)(
			node,
			text,
			false,
			false,
...
function text_split(node, offset) {
	...
	const newData = (0, CharacterDataAlgorithm_1.characterData_substringData)(
		node,
		offset,
		count,
	);
	const newNode = (0, CreateAlgorithm_1.create_text)(
		node._nodeDocument,
		newData,
	);
	...
	if (parent !== null) {
		 ...
		 */
		(0, MutationAlgorithm_1.mutation_insert)(
			newNode,
			parent,
			node._nextSibling,
		);
