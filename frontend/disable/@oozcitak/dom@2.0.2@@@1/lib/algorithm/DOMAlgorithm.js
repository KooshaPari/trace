...
exports.dom_runNodeIteratorPreRemovingSteps =
	dom_runNodeIteratorPreRemovingSteps;
 ...
 */
function dom_runAttributeChangeSteps(
	element,
	localName,
	oldValue,
	value,
...
function removeNodeIterator(nodeIterator, toBeRemovedNode) {
	 ...
	 */
	if (
		toBeRemovedNode === nodeIterator._root ||
		!(0, TreeAlgorithm_1.tree_isAncestorOf)(
			nodeIterator._reference,
			toBeRemovedNode,
			true,
		)
	) {
	...
	if (nodeIterator._pointerBeforeReference) {
		 ...
		 */
		while (true) {
			const nextNode = (0, TreeAlgorithm_1.tree_getFollowingNode)(
				nodeIterator._root,
				toBeRemovedNode,
			);
			if (
				nextNode !== null &&
				(0, TreeAlgorithm_1.tree_isDescendantOf)(
					nodeIterator._root,
					nextNode,
					true,
				) &&
	...
	if (toBeRemovedNode._previousSibling === null) {
		...
		let childNode = (0, TreeAlgorithm_1.tree_getFirstDescendantNode)(
			toBeRemovedNode._previousSibling,
			true,
			false,
		);
		while (childNode !== null) {
			...
			childNode = (0, TreeAlgorithm_1.tree_getNextDescendantNode)(
				toBeRemovedNode._previousSibling,
				childNode,
				true,
				false,
...
function updateASlotsName(element, localName, oldValue, value, namespace) {
	...
	if (
		...
		(0, ShadowTreeAlgorithm_1.shadowTree_assignSlotablesForATree)(
			(0, TreeAlgorithm_1.tree_rootNode)(element),
		);
	}
}
/**
...
function updateASlotablesName(element, localName, oldValue, value, namespace) {
	 ...
	 */
	if (
		util_1.Guard.isSlotable(element) &&
		localName === "slot" &&
		namespace === null
	) {
		if (value === oldValue) return;
		...
		if ((0, ShadowTreeAlgorithm_1.shadowTree_isAssigned)(element)) {
			(0, ShadowTreeAlgorithm_1.shadowTree_assignSlotables)(
				element._assignedSlot,
			);
		}
