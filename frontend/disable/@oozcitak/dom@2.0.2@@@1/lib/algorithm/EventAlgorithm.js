...
function event_createAnEvent(eventInterface, realm = undefined) {
	...
	const dictionary = {};
	const event = event_innerEventCreationSteps(
		eventInterface,
		realm,
		new Date(),
		dictionary,
...
function event_dispatch(
	...
	let relatedTarget = (0, TreeAlgorithm_1.tree_retarget)(
	...
	if (target !== relatedTarget || target === event._relatedTarget) {
		...
		for (const touchTarget of event._touchTargetList) {
			touchTargets.push(
				(0, TreeAlgorithm_1.tree_retarget)(touchTarget, target),
			);
		}
		...
		let slotable =
			...
			(0, ShadowTreeAlgorithm_1.shadowTree_isAssigned)(target)
				? target
				: null;
		...
		while (parent !== null && util_1.Guard.isNode(parent)) {
			...
			if (
				...
				(0, ShadowTreeAlgorithm_1.shadowTree_isAssigned)(parent)
			) {
				slotable = parent;
			}
			relatedTarget = (0, TreeAlgorithm_1.tree_retarget)(
				event._relatedTarget,
				parent,
			);
			touchTargets = [];
			for (const touchTarget of event._touchTargetList) {
				touchTargets.push(
					(0, TreeAlgorithm_1.tree_retarget)(touchTarget, parent),
				);
			}
			/**
			...
			if (
				...
				(util_1.Guard.isNode(parent) &&
					...
					(0, TreeAlgorithm_1.tree_isAncestorOf)(
						(0, TreeAlgorithm_1.tree_rootNode)(target, true),
						parent,
						true,
						true,
					))
		...
		if (clearTargetsStruct !== null) {
			if (
				...
				util_1.Guard.isShadowRoot(
					(0, TreeAlgorithm_1.tree_rootNode)(
				...
				for (let j = 0; j < clearTargetsStruct.touchTargetList.length; j++) {
					...
					if (
						...
						util_1.Guard.isShadowRoot(
							(0, TreeAlgorithm_1.tree_rootNode)(struct, true),
						)
					) {
						clearTargets = true;
						break;
...
function event_appendToAnEventPath(
	...
	if (
		...
		util_1.Guard.isShadowRoot(
			(0, TreeAlgorithm_1.tree_rootNode)(invocationTarget),
		)
	) {
