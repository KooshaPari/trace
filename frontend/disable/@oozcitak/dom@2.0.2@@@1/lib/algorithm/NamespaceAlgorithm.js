...
function namespace_validate(qualifiedName) {
	...
	if (!(0, XMLAlgorithm_1.xml_isName)(qualifiedName))
		throw new DOMException_1.InvalidCharacterError(
			`Invalid XML name: ${qualifiedName}`,
		);
	if (!(0, XMLAlgorithm_1.xml_isQName)(qualifiedName))
		throw new DOMException_1.InvalidCharacterError(
			`Invalid XML qualified name: ${qualifiedName}.`,
		);
}
/**
...
function namespace_validateAndExtract(namespace, qualifiedName) {
	...
	if (prefix && namespace === null)
		throw new DOMException_1.NamespaceError(
			"Qualified name includes a prefix but the namespace is null.",
		);
	if (prefix === "xml" && namespace !== infra_1.namespace.XML)
		throw new DOMException_1.NamespaceError(
			`Qualified name includes the "xml" prefix but the namespace is not the XML namespace.`,
		);
	if (
		...
		throw new DOMException_1.NamespaceError(
			`Qualified name includes the "xmlns" prefix but the namespace is not the XMLNS namespace.`,
		);
	if (
		namespace === infra_1.namespace.XMLNS &&
		prefix !== "xmlns" &&
		...
		throw new DOMException_1.NamespaceError(
			`Qualified name does not include the "xmlns" prefix but the namespace is the XMLNS namespace.`,
		);
