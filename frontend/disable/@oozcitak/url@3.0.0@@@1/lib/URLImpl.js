...
class URLImpl {
	...
	set href(value) {
		...
		if (query !== null) {
			this._queryObject._list = (0, URLAlgorithm_1.urlEncodedStringParser)(
				query,
			);
		}
	}
	...
	get origin() {
		 ...
		 */
		return (0, URLAlgorithm_1.asciiSerializationOfAnOrigin)(
			(0, URLAlgorithm_1.origin)(this._url),
		);
	}
	/** @inheritdoc */
	get protocol() {
	...
	set protocol(val) {
		 ...
		 */
		(0, URLAlgorithm_1.basicURLParser)(
			val + ":",
			undefined,
			undefined,
			this._url,
	...
	set username(val) {
		 ...
		 */
		if ((0, URLAlgorithm_1.cannotHaveAUsernamePasswordPort)(this._url)) return;
	...
	get host() {
		...
		if (this._url.host === null) {
			...
			return (
				(0, URLAlgorithm_1.hostSerializer)(this._url.host) +
				":" +
				this._url.port.toString()
			);
		}
	}
	set host(val) {
	...
	set hostname(val) {
		...
		(0, URLAlgorithm_1.basicURLParser)(
			...
			undefined,
			this._url,
			interfaces_1.ParserState.Hostname,
		);
	}
	/** @inheritdoc */
	get port() {
	...
	set pathname(val) {
		...
		(0, URLAlgorithm_1.basicURLParser)(
			...
			undefined,
			this._url,
			interfaces_1.ParserState.PathStart,
		);
	}
	/** @inheritdoc */
	get search() {
	...
	set search(val) {
		...
		(0, URLAlgorithm_1.basicURLParser)(
			...
			undefined,
			url,
			interfaces_1.ParserState.Query,
		);
	...
	set hash(val) {
		...
		(0, URLAlgorithm_1.basicURLParser)(
			...
			undefined,
			this._url,
			interfaces_1.ParserState.Fragment,
		);
	}
	/** @inheritdoc */
	toJSON() {
