 ...
 */
const _userInfoPercentEncodeSet =
	/[ "<>`#?{}/:;=@[\]\\^|]|[\0-\x1F\x7F-\uD7FF\uE000-\uFFFF]|[\uD800-\uDBFF][\uDC00-\uDFFF]|[\uD800-\uDBFF](?![\uDC00-\uDFFF])|(?:[^\uD800-\uDBFF]|^)[\uDC00-\uDFFF]/;
/**
			...
			case interfaces_1.ParserState.Authority:
				...
				if (walker.c() === "@") {
					...
					for (const codePoint of buffer) {
						...
						const encodedCodePoints = utf8PercentEncode(
							codePoint,
...
function hostParser(input, isNotSpecial = false) {
	...
	if (ipv4Host === null || (0, util_1.isNumber)(ipv4Host)) return ipv4Host;
	return asciiDomain;
}
/**
	...
	for (let i = 0; i < input.length; i++) {
		...
		if (byte !== 0x25) {
			...
			const bytePoint = parseInt(
				(0, util_1.utf8Decode)(Uint8Array.of(input[i + 1], input[i + 2])),
				16,
			);
			output[n] = bytePoint;
			n++;
