...
function urlSerializer(url, excludeFragmentFlag) {
	...
	if (url._cannotBeABaseURLFlag) {
		...
		try {
			for (
				var _b = __values(url.path), _c = _b.next();
				!_c.done;
				_c = _b.next()
			) {
				var str = _c.value;
...
function basicURLParser(input, baseURL, encodingOverride, url, stateOverride) {
	...
	if (url === undefined) {
		...
		if (
			leadingControlOrSpace.test(input) ||
			trailingControlOrSpace.test(input)
		) {
			validationError(
				"Input string contains leading or trailing control characters or space.",
			);
		}
	...
	var state =
		stateOverride === undefined
			? interfaces_1.ParserState.SchemeStart
			: stateOverride;
	if (baseURL === undefined) baseURL = null;
	var encoding =
		encodingOverride === undefined ||
		encodingOverride === "replacement" ||
		encodingOverride === "UTF-16BE" ||
		encodingOverride === "UTF-16LE"
			? "UTF-8"
			: encodingOverride;
	/**
	...
	while (true) {
		switch (state) {
			...
			case interfaces_1.ParserState.Scheme:
				 ...
				 */
				if (
					infra_1.codePoint.ASCIIAlphanumeric.test(walker.c()) ||
					walker.c() === "+" ||
					walker.c() === "-" ||
					walker.c() === "."
				) {
			...
			case interfaces_1.ParserState.NoScheme:
				...
				if (
					baseURL === null ||
					(baseURL._cannotBeABaseURLFlag && walker.c() !== "#")
				) {
					validationError("Invalid input string.");
					return null;
			...
			case interfaces_1.ParserState.Authority:
				...
				if (walker.c() === "@") {
					...
					try {
						for (
							var buffer_1 = ((e_2 = void 0), __values(buffer)),
							...
							var encodedCodePoints = utf8PercentEncode(
								codePoint,
					...
					switch (walker.c()) {
						...
						default:
							...
							if (!startsWithAWindowsDriveLetter(walker.substring())) {
								...
								validationError(
									"Unexpected windows drive letter in input string.",
								);
							}
							state = interfaces_1.ParserState.Path;
							walker.pointer--;
				...
				if (
					...
					if (isDoubleDotPathSegment(buffer)) {
						...
						if (
							...
							buffer =
								bufferCodePoints.slice(0, 1) + ":" + bufferCodePoints.slice(2);
						}
						/**
					...
					if (
						url.scheme === "file" &&
						(walker.c() === EOF || walker.c() === "?" || walker.c() === "#")
					) {
					...
					if (!_urlCodePoints.test(walker.c()) && walker.c() !== "%") {
						validationError(
							"Character is not a URL code point or a percent encoded character.",
						);
					}
					if (
						walker.c() === "%" &&
						!/^[0-9a-fA-F][0-9a-fA-F]/.test(walker.remaining())
					) {
						validationError(
							"Percent encoded character must be followed by two hex digits.",
						);
					}
			...
			case interfaces_1.ParserState.CannotBeABaseURLPath:
				...
				if (walker.c() === "?") {
					...
					if (walker.c() !== EOF) {
						url.path[0] += utf8PercentEncode(
							walker.c(),
							_c0ControlPercentEncodeSet,
						);
					}
				}
				break;
			case interfaces_1.ParserState.Query:
				...
				if (
					encoding !== "UTF-8" &&
					(!isSpecial(url) || url.scheme === "ws" || url.scheme === "wss")
				) {
					encoding = "UTF-8";
				}
				if (stateOverride === undefined && walker.c() === "#") {
					...
					if (
						...
						url.query +=
							"%26%23" + infra_1.byteSequence.isomorphicDecode(bytes) + "%3B";
					} else {
						try {
							/**
							 ...
							 */
							for (
								var bytes_1 = ((e_3 = void 0), __values(bytes)),
									bytes_1_1 = bytes_1.next();
								!bytes_1_1.done;
								bytes_1_1 = bytes_1.next()
							) {
			...
			case interfaces_1.ParserState.Fragment:
				...
				if (walker.c() === EOF) {
					...
					if (
						walker.c() === "%" &&
						!/^[A-Za-z0-9][A-Za-z0-9]/.test(walker.remaining())
					) {
						validationError("Unexpected character in fragment string.");
					}
					url.fragment += utf8PercentEncode(
						walker.c(),
						_fragmentPercentEncodeSet,
					);
				}
				break;
		}
...
function setTheUsername(url, username) {
	...
	try {
		for (
			var username_1 = __values(username), username_1_1 = username_1.next();
			!username_1_1.done;
			username_1_1 = username_1.next()
		) {
...
function setThePassword(url, password) {
	...
	try {
		for (
			var password_1 = __values(password), password_1_1 = password_1.next();
			!password_1_1.done;
			password_1_1 = password_1.next()
		) {
...
function isNormalizedWindowsDriveLetter(str) {
	...
	return (
		str.length >= 2 &&
		infra_1.codePoint.ASCIIAlpha.test(str[0]) &&
		str[1] === ":"
	);
}
...
function iPv4Parser(input) {
	...
	try {
		for (
			var numbers_1 = __values(numbers), numbers_1_1 = numbers_1.next();
			!numbers_1_1.done;
			numbers_1_1 = numbers_1.next()
		) {
...
function iPv6Parser(input) {
	...
	if (compress !== null) {
		...
		while (pieceIndex !== 0 && swaps > 0) {
			(_a = __read([address[compress + swaps - 1], address[pieceIndex]], 2)),
				(address[pieceIndex] = _a[0]),
				(address[compress + swaps - 1] = _a[1]);
			pieceIndex--;
			swaps--;
		}
...
function opaqueHostParser(input) {
	...
	try {
		for (
			var input_1 = __values(input), input_1_1 = input_1.next();
			!input_1_1.done;
...
function percentDecode(input) {
	...
	for (var i = 0; i < input.length; i++) {
		...
		if (byte !== 0x25) {
			...
			var bytePoint = parseInt(
				util_1.utf8Decode(Uint8Array.of(input[i + 1], input[i + 2])),
				16,
			);
			output[n] = bytePoint;
			n++;
...
function utf8PercentEncode(codePoint, percentEncodeSet) {
	...
	try {
		for (
			var bytes_2 = __values(bytes), bytes_2_1 = bytes_2.next();
			!bytes_2_1.done;
			bytes_2_1 = bytes_2.next()
		) {
...
function urlEquals(urlA, urlB, excludeFragmentsFlag) {
	 ...
	 */
	return (
		urlSerializer(urlA, excludeFragmentsFlag) ===
...
function urlEncodedParser(input) {
	...
	try {
		for (
			var input_2 = __values(input), input_2_1 = input_2.next();
			!input_2_1.done;
			input_2_1 = input_2.next()
		) {
	...
	try {
		 ...
		 */
		for (
			var sequences_1 = __values(sequences), sequences_1_1 = sequences_1.next();
			!sequences_1_1.done;
			sequences_1_1 = sequences_1.next()
		) {
			...
			for (var i = 0; i < name.length; i++)
				if (name[i] === 0x2b) name[i] = 0x20;
			for (var i = 0; i < value.length; i++)
				if (value[i] === 0x2b) value[i] = 0x20;
			/**
...
function urlEncodedByteSerializer(input) {
	...
	try {
		for (
			var input_3 = __values(input), input_3_1 = input_3.next();
			!input_3_1.done;
...
function urlEncodedSerializer(tuples, encodingOverride) {
	...
	try {
		 ...
		 */
		for (
			var tuples_1 = __values(tuples), tuples_1_1 = tuples_1.next();
			!tuples_1_1.done;
			tuples_1_1 = tuples_1.next()
		) {
