...
function convertAJSONDerivedJavaScriptValueToAnInfraValue(jsValue) {
	...
	if (util_1.isArray(jsValue)) {
		...
		try {
			for (
				var jsValue_1 = __values(jsValue), jsValue_1_1 = jsValue_1.next();
				!jsValue_1_1.done;
				jsValue_1_1 = jsValue_1.next()
			) {
				var jsValueAtIndex = jsValue_1_1.value;
				result.push(
					convertAJSONDerivedJavaScriptValueToAnInfraValue(jsValueAtIndex),
				);
			}
		} catch (e_1_1) {
			e_1 = { error: e_1_1 };
		...
		for (var key in jsValue) {
			...
			if (Object.hasOwn(jsValue, key)) {
				var jsValueAtKey = jsValue[key];
				result.set(
					key,
					convertAJSONDerivedJavaScriptValueToAnInfraValue(jsValueAtKey),
				);
			}
		}
		return result;
...
exports.convertAJSONDerivedJavaScriptValueToAnInfraValue =
	convertAJSONDerivedJavaScriptValueToAnInfraValue;
