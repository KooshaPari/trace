...
function isomorphicEncode(str) {
	...
	try {
		for (
			...
			console.assert(
				byte !== undefined && byte <= 0x00ff,
				"isomorphicEncode requires string bytes to be less than or equal to 0x00FF.",
			);
...
function asciiLowercase(str) {
	...
	try {
		for (
			var str_2 = __values(str), str_2_1 = str_2.next();
			!str_2_1.done;
			str_2_1 = str_2.next()
		) {
			var c = str_2_1.value;
...
function asciiUppercase(str) {
	...
	try {
		for (
			var str_3 = __values(str), str_3_1 = str_3.next();
			!str_3_1.done;
			str_3_1 = str_3.next()
		) {
			var c = str_3_1.value;
...
function asciiDecode(bytes) {
	...
	try {
		 ...
		 */
		for (
			var bytes_1 = __values(bytes), bytes_1_1 = bytes_1.next();
			!bytes_1_1.done;
			bytes_1_1 = bytes_1.next()
		) {
			var byte = bytes_1_1.value;
			console.assert(
				Byte_1.isASCIIByte(byte),
				"asciiDecode requires an ASCII byte sequence.",
			);
		}
	} catch (e_4_1) {
		e_4 = { error: e_4_1 };
...
exports.stripLeadingAndTrailingASCIIWhitespace =
...
function collectASequenceOfCodePoints(condition, input, options) {
	...
	var result = "";
	while (
		options.position < input.length &&
		!!condition.call(null, input[options.position])
	) {
...
function skipASCIIWhitespace(input, options) {
	...
	collectASequenceOfCodePoints(
		(str) => CodePoints_1.ASCIIWhiteSpace.test(str),
		input,
		options,
	);
}
...
function strictlySplit(input, delimiter) {
	...
	var tokens = [];
	var token = collectASequenceOfCodePoints(
		(str) => delimiter !== str,
		input,
		options,
	);
	...
	while (options.position < input.length) {
		console.assert(
			input[options.position] === delimiter,
			"strictlySplit found no delimiter in input string.",
		);
		options.position++;
		token = collectASequenceOfCodePoints(
			(str) => delimiter !== str,
			input,
			options,
		);
	...
	while (options.position < input.length) {
		...
		if (options.position < input.length) {
			console.assert(
				...
				"splitAStringOnCommas found no delimiter in input string.",
			);
			options.position++;
		}
	}
