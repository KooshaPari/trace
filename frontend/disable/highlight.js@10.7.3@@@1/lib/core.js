...
function compileKeywords(
	...
	if (typeof rawKeywords === "string") {
		...
		Object.keys(rawKeywords).forEach((className) => {
  ...
  */
	...
	function compileMode(mode, parent) {
		...
		mode.contains = [].concat(
			...mode.contains.map((c) => expandOrCloneMode(c === "self" ? mode : c)),
		);
...
function expandOrCloneMode(mode) {
	if (mode.variants && !mode.cachedVariants) {
		mode.cachedVariants = mode.variants.map((variant) =>
			inherit(mode, { variants: null }, variant),
		);
	}
	...
	function _highlight(
		...
		function keywordData(mode, match) {
			const matchText = language.case_insensitive
				...
				: match[0];
			return (
				Object.hasOwn(mode.keywords, matchText) && mode.keywords[matchText]
			);
		}
