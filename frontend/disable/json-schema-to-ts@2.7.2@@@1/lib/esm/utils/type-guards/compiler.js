 ...
 */
export var wrapCompilerAsTypeGuard = function wrapCompilerAsTypeGuard(
	compiler,
) {
	return function (schema) {
		for (
			var _len = arguments.length,
				compilingOptions = new Array(_len > 1 ? _len - 1 : 0),
				_key = 1;
			_key < _len;
			_key++
		) {
		...
		return function (data) {
			for (
				var _len2 = arguments.length,
					validationOptions = new Array(_len2 > 1 ? _len2 - 1 : 0),
					_key2 = 1;
				_key2 < _len2;
				_key2++
			) {
