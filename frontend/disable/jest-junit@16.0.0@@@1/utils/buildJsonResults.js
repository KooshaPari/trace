...
const toTemplateTag = (varName) => "{" + varName + "}";
...
const replaceVars = (strOrFunc, variables) => {
...
const executionTime = (startTime, endTime) => (endTime - startTime) / 1000;
...
const generateTestCase = (
	...
	if (tc.status === testFailureStatus || tc.status === testErrorStatus) {
		const failureMessages =
			junitOptions.noStackTrace === "true" && tc.failureDetails
				? tc.failureDetails.map((detail) => detail.message)
				: tc.failureMessages;

		failureMessages.forEach((failure) => {
			const tagName =
				tc.status === testFailureStatus ? "failure" : testErrorStatus;
	...
	if (getGetCaseProperties !== null) {
		const junitCaseProperties = getGetCaseProperties(tc);

		// Add any test suite properties
		const testCasePropertyMain = {
			properties: [],
		};
...
const addErrorTestResult = (suite) => {
...
const strip = (str) => stripAnsi(str).replace(/\u001b/g, "");

module.exports = (report, appDirectory, options, rootDir = null) => {
	...
	const getTestCaseProperties = fs.existsSync(testCasePropertiesPath)
		? require(testCasePropertiesPath)
		: null;
	...
	report.testResults.forEach((suite) => {
		...
		const filepath = path.join(
			suiteOptions.filePathPrefix,
			path.relative(appDirectory, suite.testFilePath),
		);
			...
			const testSuiteConsole = {
				"system-out": {
					_cdata: JSON.stringify(
						suite.console.map((item) => item.message),
						null,
						2,
					),
				},
