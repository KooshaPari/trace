...
import { isAbsoluteUrl, isRef } from "../../../ref-utils.js";
...
export function buildSubjectVisitor(assertId, assertion) {
	return (node, ctx) => {
		...
		const defaultMessage =
			`${colorize.blue(assertId)} failed because the ${colorize.blue(assertion.subject.type)} ${colorize.blue(properties.join(", "))} didn't meet the assertions: ${assertionMessageTemplates.problems}`.replace(
				/ +/g,
				" ",
			);
