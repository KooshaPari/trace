		...
		Schema: {
			...
			enter(schema, { location, report, resolve }) {
				...
				const elevateProperties = (
					schema,
					from,
					includeFirstLevelExclusiveSchemas = true,
				) => {
				...
				const recursivelyGetParentProperties = (
					...
					const isMemberOfComposedType =
						...
						!!/(allOf|oneOf|anyOf)/.exec(
							splitLocation[splitLocation.length - locationLookupIndex],
						);
				...
				const parentProperties = recursivelyGetParentProperties(
					location.pointer.split("/"),
				);
				for (const [i, requiredProperty] of schema.required.entries()) {
					if (
						(!allProperties ||
							getOwn(allProperties, requiredProperty) === undefined) &&
						(!parentProperties ||
							getOwn(parentProperties, requiredProperty) === undefined)
					) {
						report({
