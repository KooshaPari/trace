...
import { getTypes, specVersions } from "../oas-types.js";
...
export function createConfigTypes(extraSchemas, config) {
	const nodeNames = specVersions.flatMap((version) => {
		const types = config
			? config.extendTypes(getTypes(version), version)
			: getTypes(version);
...
// @ts-expect-error FIXME: remove this once we remove `theme` from the schema
