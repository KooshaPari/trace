...
// @ts-expect-error this works but some types are not working
...
class Logger {
	...
	warn(str) {
		return isBrowser
			? console.warn(str)
			: process.stderr.write(colorize.yellow(str));
	}
	error(str) {
		return isBrowser
			? console.error(str)
			: process.stderr.write(colorize.red(str));
	}
	output(str) {
	...
	printSeparator(separator) {
		...
		const separatorLine = separator
			...
			.slice(0, windowWidth);
		const coloredSeparatorLine = isBrowser
			? separatorLine
			: colorize.gray(separatorLine);
		return isBrowser
