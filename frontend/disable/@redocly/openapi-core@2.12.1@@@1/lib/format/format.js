...
import { colorize, colorOptions, logger } from "../logger.js";
...
export function formatProblems(problems, opts) {
	...
	switch (format) {
		...
		case "stylish": {
			...
			for (const [
				...
				logger.output(
					`${colorize.blue(isAbsoluteUrl(file) ? file : path.relative(cwd, file))}:\n`,
				);
		...
		case "markdown": {
			...
			for (const [file, { fileProblems }] of Object.entries(groupedByFile)) {
				logger.output(
					`## Lint: ${isAbsoluteUrl(file) ? file : path.relative(cwd, file)}\n\n`,
				);
	...
	if (totalProblems - ignoredProblems > maxProblems) {
		logger.info(
			`< ... ${totalProblems - maxProblems} more problems hidden > ${colorize.gray("increase with `--max-problems N`")}\n`,
		);
	}
...
const groupByFiles = (problems) => {
	...
	for (const problem of problems) {
		...
		const mappedProblem = {
			...problem,
			location: problem.location.map(getLineColLocation),
		};
