...
export async function resolveConfig({
	...
	const config =
		rawConfigDocument === undefined ? DEFAULT_CONFIG : rawConfigDocument.parsed;
	...
	if (isPlainObject(config) && config?.extends?.some(isNotString)) {
		throw new Error(
			`Configuration format not detected in extends: values must be strings.`,
		);
	}
...
export const preResolvePluginPath = (plugin, base, rootConfigDir) => {
	...
	return fs.existsSync(maybeAbsolutePluginPath)
		? {
				absolutePath: maybeAbsolutePluginPath,
				rawPath: plugin,
				isModule: false,
			}
		: {
				absolutePath: module
					.createRequire(import.meta.url ?? __dirname)
					.resolve(plugin, {
						paths: [
	...
	const requireFunc = async (plugin) => {
		...
		try {
			...
			if (!pluginsCache.has(absolutePluginPath)) {
				...
				// @ts-expect-error FIXME: investigate if we still need this (2.0)
				if (typeof __webpack_require__ === "function") {
					...
					const mod = await import(
						url.pathToFileURL(absolutePluginPath).pathname
					);
				...
				for (const requiredPluginInstance of requiredPluginInstances) {
					if (
						requiredPluginInstance?.id &&
						isDeprecatedPluginFormat(requiredPluginInstance)
					) {
						logger.info(
							`Deprecated plugin format detected: ${requiredPluginInstance.id}\n`,
						);
					}
				}
				...
				const pluginInstances = Array.isArray(pluginModule)
					? pluginModule
					: [pluginModule];
				if (pluginModule) {
					pluginsCache.set(
					...
					pluginInstances.map(async (pluginInstance) => {
						...
						if (existingPluginPath) {
							if (pluginPath !== existingPluginPath) {
								throw new Error(
									colorize.red(
										`Plugin "id" must be unique. Plugin ${colorize.blue(pluginPath)} uses id "${colorize.blue(id)}" already seen in ${colorize.blue(pluginPath)}`,
									),
								);
							}
							return undefined;
						...
						const plugin = {
							id,
							...(pluginInstance.configs
								? { configs: pluginInstance.configs }
								: {}),
						...
						if (pluginInstance.rules) {
							...
							if (pluginInstance.rules.async2) {
								plugin.rules.async2 = prefixRules(
									pluginInstance.rules.async2,
									id,
								);
							}
							if (pluginInstance.rules.async3) {
								plugin.rules.async3 = prefixRules(
									pluginInstance.rules.async3,
									id,
								);
							}
							if (pluginInstance.rules.arazzo1) {
								plugin.rules.arazzo1 = prefixRules(
									pluginInstance.rules.arazzo1,
									id,
								);
							}
							if (pluginInstance.rules.overlay1) {
								plugin.rules.overlay1 = prefixRules(
									pluginInstance.rules.overlay1,
									id,
								);
							}
