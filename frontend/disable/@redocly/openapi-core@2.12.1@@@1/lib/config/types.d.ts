...
export type RuleConfig =
	| RuleSeverity
	| (Partial<RuleSettings> & Record<string, any>);
...
export type RawGovernanceConfig<T extends "built-in" | undefined = undefined> =
	{
		...
		oas3_2Rules?: RuleMap<
			Exclude<
				BuiltInOAS3RuleId,
				"nullable-type-sibling" | "no-example-value-and-externalValue"
			>,
			RuleConfig,
			T
		>;
...
export type TypeExtensionsConfig = Partial<
	Record<SpecMajorVersion, TypesExtensionFn>
>;
...
export type AssertionContext = Partial<UserContext> &
	SkipFunctionContext & {
		node: any;
	};
...
export type RawUniversalConfig = Omit<
	Partial<RedoclyConfig>,
	"apis" | "plugins"
> &
	RawGovernanceConfig & {
		plugins?: (string | Plugin)[];
