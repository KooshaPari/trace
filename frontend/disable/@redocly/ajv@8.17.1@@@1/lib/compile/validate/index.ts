...
function iterateKeywords(it: SchemaObjCxt, group: RuleGroup): void {
	...
	gen.block(() => {
		for (const rule of group.rules) {
			if (
				shouldUseRule(schema, rule) ||
				shouldForceUnevaluatedProperties(schema, rule)
			) {
	...
	function shouldForceUnevaluatedProperties(
		schemaObj: AnySchemaObject,
		rule: Rule,
	): boolean {
		return !!(
