...
const def: CodeKeywordDefinition = {
	...
	code(cxt: KeywordCxt) {
		...
		const keyword = parentSchema.oneOf
			? "oneOf"
			: parentSchema.anyOf
				? "anyOf"
				: undefined;
		...
		if (!keyword)
			throw new Error(
				"discriminator: requires oneOf or anyOf composite keyword",
			);
		...
		function getMapping(): { [T in string]?: number } {
			...
			for (let i = 0; i < parentSchemaVariants.length; i++) {
				...
				if (schRef && schema.mapping) {
					...
					const matchedKeys = Object.keys(mapping).filter(
						(key) => mapping[key] === sch.$ref,
					);
				...
				if (schRef && !schemaHasRulesButRef(sch, it.self.RULES)) {
					...
					if (sch === undefined)
						throw new MissingRefError(it.opts.uriResolver, it.baseId, schRef);
				}
			...
			function hasRequired(sch: AnySchemaObject): boolean {
				...
				if (sch.allOf && Array.isArray(sch.allOf)) {
					for (const subschema of sch.allOf) {
						...
						if (
							Array.isArray(subSch.required) &&
							subSch.required.includes(tagName)
						) {
							return true;
						}
					}
