...
class RedoclyClient {
	constructor(region) {
		...
		this.loadTokens();
		this.domain = region
			? domains_1.DOMAINS[region]
			: env_1.env.REDOCLY_DOMAIN || domains_1.DOMAINS[domains_1.DEFAULT_REGION];
		(0, domains_1.setRedoclyDomain)(this.domain);
		this.registryApi = new registry_api_1.RegistryApi(
			this.accessTokens,
			this.region,
		);
	}
	loadRegion(region) {
		...
		if ((0, domains_1.getRedoclyDomain)()) {
			return (
				domains_1.AVAILABLE_REGIONS.find(
					(region) =>
						domains_1.DOMAINS[region] === (0, domains_1.getRedoclyDomain)(),
				) || domains_1.DEFAULT_REGION
			);
		}
	...
	loadTokens() {
		const credentialsPath = (0, path_1.resolve)(
			(0, os_1.homedir)(),
			exports.TOKEN_FILENAME,
		);
	...
	async isAuthorizedWithRedocly() {
		return (
			this.hasTokens() &&
			(0, utils_1.isNotEmptyArray)(await this.getValidTokens())
		);
	}
	readCredentialsFile(credentialsPath) {
		return (0, fs_1.existsSync)(credentialsPath)
			? JSON.parse((0, fs_1.readFileSync)(credentialsPath, "utf-8"))
			: {};
	}
