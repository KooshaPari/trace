...
function detectSpec(root) {
	...
	if (root.openapi && typeof root.openapi !== "string") {
		throw new Error(
			`Invalid OpenAPI version: should be a string but got "${typeof root.openapi}"`,
		);
	}
	...
	if (root.openapi || root.swagger) {
		throw new Error(
			`Unsupported OpenAPI version: ${root.openapi || root.swagger}`,
		);
	}
	...
	if (
		typeof root.arazzo === "string" &&
		arazzo_2.VERSION_PATTERN.test(root.arazzo)
	) {
		return SpecVersion.Arazzo1;
	}
	if (
		typeof root.overlay === "string" &&
		arazzo_2.VERSION_PATTERN.test(root.overlay)
	) {
