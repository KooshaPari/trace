...
const RemoveXInternal = ({
	...
	function removeInternal(node, ctx, originalMapping) {
		...
		if (Array.isArray(node)) {
			...
			for (const key of Object.keys(node)) {
				if ((0, ref_utils_1.isRef)(node[key])) {
					...
					if (
						(0, utils_1.isPlainObject)(resolved.node) &&
						resolved.node?.[internalFlagProperty]
					) {
						delete node[key];
						didDelete = true;
					}
				...
				if (
					(0, utils_1.isPlainObject)(node[key]) &&
					node[key]?.[internalFlagProperty]
				) {
					delete node[key];
					didDelete = true;
				}
		...
		if (
			didDelete &&
			((0, utils_1.isEmptyObject)(node) || (0, utils_1.isEmptyArray)(node))
		) {
			delete parent[key];
		}
	}
