			...
			enter(operation, ctx) {
				...
				if (properties.responses && operation.responses) {
					for (const responseCode of Object.keys(properties.responses)) {
						const resolvedResponse = checkAndResolveRef(
						...
						resolvedResponse.content = resolvedResponse.content
							? resolvedResponse.content
							: {};
						Object.keys(properties.responses[responseCode]).forEach(
							(mimeType) => {
								resolvedResponse.content[mimeType] = {
									...resolvedResponse.content[mimeType],
									examples: (0, utils_1.yamlAndJsonSyncReader)(
										properties.responses[responseCode][mimeType],
									),
								};
							},
						);
				...
				if (properties.request && operation.requestBody) {
					const resolvedRequest = checkAndResolveRef(
					...
					resolvedRequest.content = resolvedRequest.content
