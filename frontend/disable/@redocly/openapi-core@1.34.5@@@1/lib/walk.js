			...
			for (const {
				...
				if (context.isSkippedLevel) {
					...
					if (
						...
						context.activatedOn = (0, utils_1.pushStack)(
							context.activatedOn,
							activatedOn,
						);
						let ctx = context.parent;
						while (ctx) {
							ctx.activatedOn.value.nextLevelTypeActivated = (0,
							utils_1.pushStack)(
			...
			if (visitedBySome || !isNodeSeen) {
				...
				if (Array.isArray(resolvedNode)) {
					...
					if (type.additionalProperties) {
						...
						props.push(
							...Object.keys(resolvedNode).filter((k) =>
								k.startsWith(type.extensionsPrefix),
							),
						);
					}
					if ((0, ref_utils_1.isRef)(node)) {
					...
					for (const propName of props) {
						...
						if (
							!(0, types_1.isNamedType)(propType) &&
							propType?.directResolveAs
						) {
						...
						if (
							!(0, types_1.isNamedType)(propType) ||
							(propType.name === "scalar" && !(0, ref_utils_1.isRef)(value))
						) {
							continue;
						}
						walkNode(
			...
			for (const context of activatedContexts.reverse()) {
				if (context.isSkippedLevel) {
					...
					if (context.parent) {
						...
						while (ctx) {
							ctx.activatedOn.value.nextLevelTypeActivated = (0,
							utils_1.popStack)(ctx.activatedOn.value.nextLevelTypeActivated);
							ctx = ctx.parent;
						}
					}
				}
