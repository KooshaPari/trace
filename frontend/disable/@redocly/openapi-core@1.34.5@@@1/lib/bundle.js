...
async function bundle(opts) {
	...
	const document =
		doc === undefined
			? await externalRefResolver.resolveDocument(base, ref, true)
			: doc;
...
async function bundleFromString(opts) {
	...
	const document = (0, resolve_1.makeDocumentFromString)(
		source,
		absoluteRef || "/",
	);
	return bundleDocument({
...
async function bundleDocument(opts) {
	...
	const types = (0, types_1.normalizeTypes)(
		config.extendTypes(
			customTypes ?? (0, oas_types_1.getTypes)(specVersion),
			specVersion,
		),
		config,
	);
	const preprocessors = (0, rules_1.initRules)(
		rules,
		config,
		"preprocessors",
		specVersion,
	);
	const decorators = (0, rules_1.initRules)(
		rules,
		config,
		"decorators",
		specVersion,
	...
	if (removeUnusedComponents) {
		decorators.push({
			...
			visitor:
				specMajorVersion === oas_types_1.SpecMajorVersion.OAS2
	...
	if (preprocessors.length > 0) {
		...
		(0, walk_1.walkDocument)({
			document,
			rootType: types.Root,
			normalizedVisitors: (0, visitors_1.normalizeVisitors)(
				preprocessors,
				types,
			),
			resolvedRefMap,
...
function makeBundleVisitor(
	...
	const visitor = {
		ref: {
			leave(node, ctx, resolved) {
				if (!resolved.location || resolved.node === undefined) {
					(0, no_unresolved_refs_1.reportUnresolvedRef)(
						resolved,
						ctx.report,
						ctx.location,
					);
				...
				if (
					skipRedoclyRegistryRefs &&
					(0, domains_1.isRedoclyRegistryURL)(node.$ref)
				) {
					return;
				}
		...
		Example: {
			leave(node, ctx) {
				if (
					(0, ref_utils_1.isExternalValue)(node) &&
					...
					if (
						keepUrlRefs &&
						(0, ref_utils_1.isAbsoluteUrl)(node.externalValue)
					) {
						return;
					}
	...
	function resolveBundledComponent(node, resolved, ctx) {
		const newRefId = (0, resolve_1.makeRefId)(
	...
	function saveComponent(componentType, target, ctx) {
		...
		if (
			version === oas_types_1.SpecMajorVersion.OAS3 ||
	...
	function isEqualOrEqualRef(node, target, ctx) {
		if (
			(0, ref_utils_1.isRef)(node) &&
			ctx.resolve(node, rootLocation.absolutePointer).location
				?.absolutePointer === target.location.absolutePointer
		) {
			return true;
		}
	...
	function getComponentName(target, componentType, ctx) {
		const [fileRef, pointer] = [
			target.location.source.absoluteRef,
