...
function formatProblems(problems, opts) {
	const {
		maxProblems = 100,
		cwd = env_1.isBrowser ? "" : process.cwd(),
		format = "codeframe",
		color = logger_1.colorOptions.enabled,
	...
	switch (format) {
		...
		case "stylish": {
			...
			for (const [
				file,
				{ ruleIdPad, locationPad: positionPad, fileProblems },
			] of Object.entries(groupedByFile)) {
				logger_1.logger.info(
					`${logger_1.colorize.blue((0, ref_utils_1.isAbsoluteUrl)(file) ? file : path.relative(cwd, file))}:\n`,
				);
				for (let i = 0; i < fileProblems.length; i++) {
					const problem = fileProblems[i];
					logger_1.logger.info(
						`${formatStylish(problem, positionPad, ruleIdPad)}\n`,
					);
				}
				logger_1.logger.info("\n");
			}
		...
		case "markdown": {
			...
			for (const [file, { fileProblems }] of Object.entries(groupedByFile)) {
				output_1.output.write(
					`## Lint: ${((0, ref_utils_1.isAbsoluteUrl))(file) ? file : path.relative(cwd, file)}\n\n`,
				);
		...
		case "checkstyle": {
			...
			for (const [file, { fileProblems }] of Object.entries(groupedByFile)) {
				output_1.output.write(
					`<file name="${xmlEscape((0, ref_utils_1.isAbsoluteUrl)(file) ? file : path.relative(cwd, file))}">\n`,
				);
	...
	if (totalProblems - ignoredProblems > maxProblems) {
		logger_1.logger.info(
			`< ... ${totalProblems - maxProblems} more problems hidden > ${logger_1.colorize.gray("increase with `--max-problems N`")}\n`,
		);
	}
				...
				const problem = {
					...
					from: p.from
						? {
								...p.from,
								source: {
									ref: (0, ref_utils_1.isAbsoluteUrl)(
	...
	function formatCodeframe(problem, idx) {
		...
		const relativePath = (0, ref_utils_1.isAbsoluteUrl)(
			location.source.absoluteRef,
		)
			? location.source.absoluteRef
			: path.relative(cwd, location.source.absoluteRef);
	...
	function formatStylish(problem, locationPad, ruleIdPad) {
		...
		const severityName = color(
			SEVERITY_NAMES[problem.severity].toLowerCase().padEnd(7),
		);
...
function formatSummary(problems) {
	...
	for (const problem of problems) {
		counts[problem.ruleId] = counts[problem.ruleId] || {
			count: 0,
	...
	for (const [ruleId, info] of sorted) {
		...
		const severityName = color(
			SEVERITY_NAMES[info.severity].toLowerCase().padEnd(7),
		);
...
const groupByFiles = (problems) => {
	...
	for (const problem of problems) {
		...
		fileGroups[absoluteRef].locationPad = Math.max(
			Math.max(
				...mappedProblem.location.map(
					(loc) => `${loc.start.line}:${loc.start.col}`.length,
				),
			),
...
function outputForGithubActions(problems, cwd) {
	for (const problem of problems) {
		for (const location of problem.location.map(
			codeframes_1.getLineColLocation,
		)) {
			let command;
			...
			output_1.output.write(
				`::${command} ${formatProperties(properties)}::${escapeMessage(message)}\n`,
			);
		}
	}
