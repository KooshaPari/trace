...
function getCodeframe(location, color) {
	...
	let skipLines = Math.max(
		endLineNum - startLineNum - MAX_CODEFRAME_LINES + 1,
		0,
	);
	...
	for (let i = startLineNum; i <= endLineNum; i++) {
		...
		prefixedLines.push([
			`${i}`,
			markLine(line, startIdx, endIdx, logger_1.colorize.red),
		]);
	...
	if (skipLines > 0) {
		...
		if (!color)
			prefixedLines.push([
				"",
				underlineLine(lines[endLineNum - 1], -1, end.col - 1),
			]);
	}
	...
	function markLine(
		...
		startIdx = -1,
		endIdx = +Infinity,
		variant = logger_1.colorize.gray,
	) {
		...
		return (
			line.substr(0, startIdx) +
			variant(line.substring(startIdx, endIdx)) +
			line.substr(endIdx)
		);
	}
}
function printPrefixedLines(lines) {
	...
	const dedentLen = Math.min(
		...existingLines.map(([_, line]) =>
			line === "" ? Infinity : padSize(line),
		),
	);
	return existingLines
		.map(
			([prefix, line]) =>
				logger_1.colorize.gray(leftPad(padLen, prefix) + " |") +
...
function positionsToLoc(source, startPos, endPos) {
	...
	const end =
		startPos === endPos
			? { ...start }
			: { line: currentLine, col: currentCol + 1 };
	return { start, end };
}
