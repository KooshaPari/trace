...
class Source {
	...
	getAst(safeLoad) {
		if (this._ast === undefined) {
			this._ast =
				safeLoad(this.body, { filename: this.absoluteRef }) ?? undefined;
...
class BaseResolver {
	...
	async loadExternalRef(absoluteRef) {
		try {
			if ((0, ref_utils_1.isAbsoluteUrl)(absoluteRef)) {
				const { body, mimeType } = await (0, utils_1.readFileFromUrl)(
					absoluteRef,
					this.config.http,
				);
	...
	parseDocument(source, isRoot = false) {
		...
		try {
			return {
				source,
				parsed: (0, utils_1.parseYaml)(source.body, {
...
async function resolveDocument(opts) {
	...
	function resolveRefsInParallel(
		...
		function walk(node, type, nodeAbsoluteRef) {
			...
			const [_, anchor] =
				Object.entries(node).find(([key]) => key === "$anchor") || [];
			if (anchor) {
			...
			if (Array.isArray(node)) {
				...
				for (let i = 0; i < node.length; i++) {
					const itemType = isTypeAFunction
						? itemsType(
								node[i],
								(0, ref_utils_1.joinPointer)(nodeAbsoluteRef, i),
							)
						: itemsType;
					// we continue resolving unknown types, but stop early on known scalars
					if (
					...
					walk(
						node[i],
						(0, types_1.isNamedType)(itemType) ? itemType : unknownType,
						(0, ref_utils_1.joinPointer)(nodeAbsoluteRef, i),
					);
				}
				return;
			...
			for (const propName of Object.keys(node)) {
				...
				walk(
					propValue,
					propType,
					(0, ref_utils_1.joinPointer)(
						nodeAbsoluteRef,
						(0, ref_utils_1.escapePointer)(propName),
					),
				);
			}
		...
		async function followRef(document, ref, refStack) {
			...
			for (const segment of segments) {
				if (typeof target !== "object") {
					...
					resolvedRef.nodePointer = (0, ref_utils_1.joinPointer)(
						resolvedRef.nodePointer,
						(0, ref_utils_1.escapePointer)(segment),
					);
				} else if ((0, ref_utils_1.isRef)(target)) {
					resolvedRef = await followRef(
						targetDoc,
						target,
						pushRef(refStack, target),
					);
