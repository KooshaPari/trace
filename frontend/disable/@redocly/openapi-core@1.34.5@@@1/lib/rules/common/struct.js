...
const Struct = () => {
	return {
		any(
			...
			const required =
				typeof type.required === "function"
					? type.required(node, key)
					: type.required;
			...
			for (const propName of Object.keys(node)) {
				...
				if (
					propSchema.resolvable !== false &&
					(0, ref_utils_1.isRef)(propValue)
				) {
				...
				if (propSchema.enum) {
					...
					propSchema.type &&
					!(0, utils_1.matchesJsonSchemaType)(propValue, propSchema.type, false)
				) {
					report({
