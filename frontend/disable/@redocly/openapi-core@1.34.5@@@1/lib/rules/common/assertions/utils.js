...
function getPredicatesFromLocators(locators) {
	...
	const matchKeysPredicate =
		keyMatcher && ((key) => keyMatcher.test(key.toString()));
	...
	return [matchKeysPredicate, filterInPredicate, filterOutPredicate].filter(
		utils_1.isTruthy,
	);
}
function getAssertsToApply(assertion) {
	...
	const shouldRunOnKeys = assertsToApply.find(
		(assert) => assert.runsOnKeys && !assert.runsOnValues,
	);
	const shouldRunOnValues = assertsToApply.find(
		(assert) => assert.runsOnValues && !assert.runsOnKeys,
	);
...
function getAssertionProperties({ subject }) {
	return (
		Array.isArray(subject.property) ? subject.property : [subject?.property]
	).filter(Boolean);
}
...
function buildVisitorObject(assertion, subjectVisitor) {
	const targetVisitorLocatorPredicates = getPredicatesFromLocators(
		assertion.subject,
	);
	...
	for (let index = 0; index < context.length; index++) {
		...
		const nodeVisitor = {
			...((locatorPredicates.length || assertsToApply.length) && {
				skip: skipFunction,
			}),
		};
		if (
				...
				...((nodeVisitor.skip && { skip: nodeVisitor.skip }) ||
					(targetVisitorSkipFunction && {
						skip: (
							...
							!!(
								nodeVisitor.skip?.(node, key, ctx) ||
								targetVisitorSkipFunction?.(node, key)
							),
					})),
			};
		} else {
			currentVisitorLevel = currentVisitorLevel[
				assertionDefinitionNode.subject?.type
			] = nodeVisitor;
		}
	}
	currentVisitorLevel[assertion.subject.type] =
		targetVisitor[assertion.subject.type];
	return visitor;
}
function buildSubjectVisitor(assertId, assertion) {
	return (node, ctx) => {
		...
		const defaultMessage =
			`${logger_1.colorize.blue(assertId)} failed because the ${logger_1.colorize.blue(assertion.subject.type)} ${logger_1.colorize.blue(properties.join(", "))} didn't meet the assertions: ${assertionMessageTemplates.problems}`.replace(
				/ +/g,
				" ",
			);
...
function isOrdered(value, options) {
	...
	for (let i = 1; i < value.length; i++) {
		...
		const result =
			direction === "asc" ? currValue >= prevVal : currValue <= prevVal;
		if (!result) {
			return false;
		}
	}
...
function runAssertion({ assert, ctx, assertionProperty }) {
	const currentLocation =
		assert.name === "ref" ? ctx.rawLocation : ctx.location;
