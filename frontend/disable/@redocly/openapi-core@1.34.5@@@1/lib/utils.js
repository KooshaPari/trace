...
Object.defineProperty(exports, "parseYaml", {
	enumerable: true,
	get: () => js_yaml_2.parseYaml,
});
Object.defineProperty(exports, "stringifyYaml", {
	enumerable: true,
	get: () => js_yaml_2.stringifyYaml,
});
...
function validateMimeType(
	...
	if (!allowedValues)
		throw new Error(
			`Parameter "allowedValues" is not provided for "${ruleType}-mime-type" rule`,
		);
...
function doesYamlFileExist(filePath) {
	return (
		((0, path_1.extname)(filePath) === ".yaml" ||
			(0, path_1.extname)(filePath) === ".yml") &&
		fs?.hasOwnProperty?.("existsSync") &&
		fs.existsSync(filePath)
	);
}
function showWarningForDeprecatedField(
	deprecatedField,
	updatedField,
	updatedObject,
	link,
	...
	logger_1.logger.warn(
		`The '${logger_1.colorize.red(deprecatedField)}' field is deprecated. ${
			updatedField
...
function showErrorForDeprecatedField(
	deprecatedField,
	updatedField,
	updatedObject,
) {
	throw new Error(
		`Do not use '${deprecatedField}' field. ${updatedField ? `Use '${getUpdatedFieldName(updatedField, updatedObject)}' instead. ` : ""}\n`,
	);
}
...
function dequal(foo, bar) {
	...
	if (foo && bar && (ctor = foo.constructor) === bar.constructor) {
		...
		if (!ctor || typeof foo === "object") {
			len = 0;
			for (ctor in foo) {
				if (Object.hasOwn(foo, ctor) && ++len && !Object.hasOwn(bar, ctor))
					return false;
