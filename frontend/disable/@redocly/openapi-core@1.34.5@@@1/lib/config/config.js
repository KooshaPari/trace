...
function getIgnoreFilePath(configFile) {
	if (configFile) {
		...
		return env_1.isBrowser
			? undefined
			: path.join(process.cwd(), exports.IGNORE_FILE);
	}
}
class StyleguideConfig {
	constructor(rawConfig, configFile) {
		...
		this.rules = {
			...
			[oas_types_1.SpecVersion.OAS3_0]: {
				...rawConfig.rules,
				...rawConfig.oas3_0Rules,
			},
			[oas_types_1.SpecVersion.OAS3_1]: {
				...rawConfig.rules,
				...rawConfig.oas3_1Rules,
			},
			[oas_types_1.SpecVersion.Async2]: {
				...rawConfig.rules,
				...rawConfig.async2Rules,
			},
			[oas_types_1.SpecVersion.Async3]: {
				...rawConfig.rules,
				...rawConfig.async3Rules,
			},
			[oas_types_1.SpecVersion.Arazzo1]: {
				...rawConfig.rules,
				...rawConfig.arazzo1Rules,
			},
			[oas_types_1.SpecVersion.Overlay1]: {
				...rawConfig.rules,
		...
		this.preprocessors = {
			...
			[oas_types_1.SpecVersion.Async2]: {
				...rawConfig.preprocessors,
				...rawConfig.async2Preprocessors,
			},
			[oas_types_1.SpecVersion.Async3]: {
				...rawConfig.preprocessors,
				...rawConfig.async3Preprocessors,
			},
		...
		this.decorators = {
			...
			[oas_types_1.SpecVersion.Async2]: {
				...rawConfig.decorators,
				...rawConfig.async2Decorators,
			},
			[oas_types_1.SpecVersion.Async3]: {
				...rawConfig.decorators,
				...rawConfig.async3Decorators,
			},
	...
	resolveIgnore(ignoreFile) {
		if (!ignoreFile || !(0, utils_1.doesYamlFileExist)(ignoreFile)) return;
		this.ignore =
		...
		for (const fileName of Object.keys(this.ignore)) {
			this.ignore[
				(0, ref_utils_1.isAbsoluteUrl)(fileName)
					? fileName
					: path.resolve(path.dirname(ignoreFile), fileName)
	...
	saveIgnore() {
		...
		for (const absFileName of Object.keys(this.ignore)) {
			...
			const ignoredRules = (mapped[mappedDefinitionName] =
		...
		fs.writeFileSync(
			ignoreFile,
			IGNORE_BANNER + (0, js_yaml_1.stringifyYaml)(mapped),
		);
	}
	addIgnore(problem) {
		...
		const fileIgnore = (ignore[loc.source.absoluteRef] =
			ignore[loc.source.absoluteRef] || {});
		const ruleIgnore = (fileIgnore[problem.ruleId] =
			fileIgnore[problem.ruleId] || new Set());
			...
			if (plugin.typeExtension !== undefined) {
				switch (version) {
					...
					case oas_types_1.SpecVersion.Arazzo1:
						...
						extendedTypes = plugin.typeExtension.arazzo1(
							extendedTypes,
							version,
						);
						break;
					case oas_types_1.SpecVersion.Overlay1:
						...
						extendedTypes = plugin.typeExtension.overlay1(
							extendedTypes,
							version,
						);
						break;
	...
	getUnusedRules() {
		...
		for (const usedVersion of Array.from(this._usedVersions)) {
			rules.push(
				...Object.keys(this.rules[usedVersion]).filter(
					(name) => !this._usedRules.has(name),
				),
			);
			decorators.push(
				...Object.keys(this.decorators[usedVersion]).filter(
					(name) => !this._usedRules.has(name),
				),
			);
			preprocessors.push(
		...
		switch (version) {
			case oas_types_1.SpecMajorVersion.OAS3: {
				...
				this.plugins.forEach(
					(p) => p.preprocessors?.oas3 && oas3Rules.push(p.preprocessors.oas3),
				);
				this.plugins.forEach(
					(p) => p.rules?.oas3 && oas3Rules.push(p.rules.oas3),
				);
				this.plugins.forEach(
					(p) => p.decorators?.oas3 && oas3Rules.push(p.decorators.oas3),
				);
				return oas3Rules;
			}
			case oas_types_1.SpecMajorVersion.OAS2: {
				...
				const oas2Rules = [];
				this.plugins.forEach(
					(p) => p.preprocessors?.oas2 && oas2Rules.push(p.preprocessors.oas2),
				);
				this.plugins.forEach(
					(p) => p.rules?.oas2 && oas2Rules.push(p.rules.oas2),
				);
				this.plugins.forEach(
					(p) => p.decorators?.oas2 && oas2Rules.push(p.decorators.oas2),
				);
				return oas2Rules;
			}
			case oas_types_1.SpecMajorVersion.Async2: {
				...
				this.plugins.forEach(
					(p) =>
						p.preprocessors?.async2 &&
						asyncApi2Rules.push(p.preprocessors.async2),
				);
				this.plugins.forEach(
					(p) => p.rules?.async2 && asyncApi2Rules.push(p.rules.async2),
				);
				this.plugins.forEach(
					(p) =>
						p.decorators?.async2 && asyncApi2Rules.push(p.decorators.async2),
				);
			...
			case oas_types_1.SpecMajorVersion.Async3: {
				...
				this.plugins.forEach(
					(p) =>
						p.preprocessors?.async3 &&
						asyncApi3Rules.push(p.preprocessors.async3),
				);
				this.plugins.forEach(
					(p) => p.rules?.async3 && asyncApi3Rules.push(p.rules.async3),
				);
				this.plugins.forEach(
					(p) =>
						p.decorators?.async3 && asyncApi3Rules.push(p.decorators.async3),
				);
			...
			case oas_types_1.SpecMajorVersion.Arazzo1: {
				...
				this.plugins.forEach(
					(p) =>
						p.preprocessors?.arazzo1 &&
						arazzo1Rules.push(p.preprocessors.arazzo1),
				);
				this.plugins.forEach(
					(p) => p.rules?.arazzo1 && arazzo1Rules.push(p.rules.arazzo1),
				);
				this.plugins.forEach(
					(p) =>
						p.decorators?.arazzo1 && arazzo1Rules.push(p.decorators.arazzo1),
				);
				return arazzo1Rules;
			}
			case oas_types_1.SpecMajorVersion.Overlay1: {
				...
				this.plugins.forEach(
					(p) =>
						p.preprocessors?.overlay1 &&
						overlay1Rules.push(p.preprocessors.overlay1),
				);
				this.plugins.forEach(
					(p) => p.rules?.overlay1 && overlay1Rules.push(p.rules.overlay1),
				);
				this.plugins.forEach(
					(p) =>
						p.decorators?.overlay1 && overlay1Rules.push(p.decorators.overlay1),
				);
...
function replaceSpecWithStruct(ruleGroups, config) {
	for (const ruleGroup of ruleGroups) {
		if (
			config[ruleGroup] &&
			(0, utils_1.isPlainObject)(config[ruleGroup]) &&
			"spec" in config[ruleGroup]
		) {
...
class Config {
	constructor(rawConfig, configFile) {
		...
		this.styleguide = new StyleguideConfig(
