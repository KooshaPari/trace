...
const DEFAULT_PROJECT_PLUGIN_PATHS = [
	"@theme/plugin.js",
	"@theme/plugin.cjs",
	"@theme/plugin.mjs",
];
...
async function resolveConfigFileAndRefs({
	configPath,
...
async function resolveConfig({ rawConfig, configPath, externalRefResolver }) {
	...
	const resolver =
		externalRefResolver ??
		new resolve_1.BaseResolver(
			(0, utils_2.getResolveConfig)(rawConfig.resolve),
		);
			...
			try {
				...
				if (!pluginsCache.has(absolutePluginPath)) {
					...
					if (typeof __webpack_require__ === "function") {
						...
						const mod = await _importDynamic(
							(0, url_1.pathToFileURL)(absolutePluginPath).href,
						);
						requiredPlugin = mod.default || mod;
					}
					const pluginCreatorOptions = { contentDir: configDir };
					const pluginModule = (0, utils_2.isDeprecatedPluginFormat)(
						requiredPlugin,
					)
						? requiredPlugin
					...
					if (
						pluginModule?.id &&
						(0, utils_2.isDeprecatedPluginFormat)(requiredPlugin)
					) {
						logger_1.logger.info(
							`Deprecated plugin format detected: ${pluginModule.id}\n`,
						);
					}
					if (pluginModule) {
				...
				throw new Error(
					`Failed to load plugin "${plugin}": ${e.message}\n\n${e.stack}`,
				);
			}
		}
		return plugin;
		...
		plugins.map(async (p) => {
			if ((0, utils_1.isString)(p)) {
				if ((0, ref_utils_1.isAbsoluteUrl)(p)) {
					throw new Error(
						logger_1.colorize.red(`We don't support remote plugins yet.`),
					);
				}
			...
			if (typeof id !== "string") {
				throw new Error(
					logger_1.colorize.red(
						`Plugin must define \`id\` property in ${logger_1.colorize.blue(p.toString())}.`,
					),
				);
			}
			if (seenPluginIds.has(id)) {
				...
				throw new Error(
					logger_1.colorize.red(
						`Plugin "id" must be unique. Plugin ${logger_1.colorize.blue(p.toString())} uses id "${logger_1.colorize.blue(id)}" already seen in ${logger_1.colorize.blue(pluginPath)}`,
					),
				);
			}
			...
			if (pluginModule.rules) {
				...
				if (pluginModule.rules.oas3) {
					plugin.rules.oas3 = (0, utils_2.prefixRules)(
						pluginModule.rules.oas3,
						id,
					);
				}
				if (pluginModule.rules.oas2) {
					plugin.rules.oas2 = (0, utils_2.prefixRules)(
						pluginModule.rules.oas2,
						id,
					);
				}
				if (pluginModule.rules.async2) {
					plugin.rules.async2 = (0, utils_2.prefixRules)(
						pluginModule.rules.async2,
						id,
					);
				}
				if (pluginModule.rules.async3) {
					plugin.rules.async3 = (0, utils_2.prefixRules)(
						pluginModule.rules.async3,
						id,
					);
				}
				if (pluginModule.rules.arazzo1) {
					plugin.rules.arazzo1 = (0, utils_2.prefixRules)(
						pluginModule.rules.arazzo1,
						id,
					);
				}
				if (pluginModule.rules.overlay1) {
					plugin.rules.overlay1 = (0, utils_2.prefixRules)(
						pluginModule.rules.overlay1,
						id,
					);
				}
			...
			if (pluginModule.preprocessors) {
				...
				if (pluginModule.preprocessors.oas3) {
					plugin.preprocessors.oas3 = (0, utils_2.prefixRules)(
						pluginModule.preprocessors.oas3,
						id,
					);
				}
				if (pluginModule.preprocessors.oas2) {
					plugin.preprocessors.oas2 = (0, utils_2.prefixRules)(
						pluginModule.preprocessors.oas2,
						id,
					);
				}
				if (pluginModule.preprocessors.async2) {
					plugin.preprocessors.async2 = (0, utils_2.prefixRules)(
						pluginModule.preprocessors.async2,
						id,
					);
				}
				if (pluginModule.preprocessors.async3) {
					plugin.preprocessors.async3 = (0, utils_2.prefixRules)(
						pluginModule.preprocessors.async3,
						id,
					);
				}
				if (pluginModule.preprocessors.arazzo1) {
					plugin.preprocessors.arazzo1 = (0, utils_2.prefixRules)(
						pluginModule.preprocessors.arazzo1,
						id,
					);
				}
				if (pluginModule.preprocessors.overlay1) {
					plugin.preprocessors.overlay1 = (0, utils_2.prefixRules)(
						pluginModule.preprocessors.overlay1,
						id,
					);
				}
			...
			if (pluginModule.decorators) {
				...
				if (pluginModule.decorators.oas3) {
					plugin.decorators.oas3 = (0, utils_2.prefixRules)(
						pluginModule.decorators.oas3,
						id,
					);
				}
				if (pluginModule.decorators.oas2) {
					plugin.decorators.oas2 = (0, utils_2.prefixRules)(
						pluginModule.decorators.oas2,
						id,
					);
				}
				if (pluginModule.decorators.async2) {
					plugin.decorators.async2 = (0, utils_2.prefixRules)(
						pluginModule.decorators.async2,
						id,
					);
				}
				if (pluginModule.decorators.async3) {
					plugin.decorators.async3 = (0, utils_2.prefixRules)(
						pluginModule.decorators.async3,
						id,
					);
				}
				if (pluginModule.decorators.arazzo1) {
					plugin.decorators.arazzo1 = (0, utils_2.prefixRules)(
						pluginModule.decorators.arazzo1,
						id,
					);
				}
				if (pluginModule.decorators.overlay1) {
					plugin.decorators.overlay1 = (0, utils_2.prefixRules)(
						pluginModule.decorators.overlay1,
						id,
					);
				}
...
async function resolveAndMergeNestedStyleguideConfig({
	...
	const plugins = env_1.isBrowser
		...
		: (0, utils_2.getUniquePlugins)(
				await resolvePlugins(
					[...(styleguideConfig?.plugins || []), builtIn_1.defaultPlugin],
	...
	const extendConfigs = await Promise.all(
		styleguideConfig?.extends?.map(async (presetItem) => {
			if (
				!(0, ref_utils_1.isAbsoluteUrl)(presetItem) &&
				!path.extname(presetItem)
			) {
			...
			const extendedStyleguideConfig = await loadExtendStyleguideConfig(
				pathItem,
				resolver,
			);
	...
	const { plugins: mergedPlugins = [], ...styleguide } = (0,
	utils_2.mergeExtends)([
		...extendConfigs,
		{
			...styleguideConfig,
			plugins,
	...
	return {
		...styleguide,
		extendPaths: styleguide.extendPaths?.filter(
			(path) => path && !(0, ref_utils_1.isAbsoluteUrl)(path),
		),
...
async function resolveStyleguideConfig(opts) {
	const resolvedStyleguideConfig =
		await resolveAndMergeNestedStyleguideConfig(opts);
	return {
		...resolvedStyleguideConfig,
		rules:
			resolvedStyleguideConfig.rules &&
			groupStyleguideAssertionRules(resolvedStyleguideConfig),
	};
}
function resolvePreset(presetName, plugins) {
	...
	if (!plugin) {
		throw new Error(
			`Invalid config ${logger_1.colorize.red(presetName)}: plugin ${pluginId} is not included.`,
		);
	}
...
async function loadExtendStyleguideConfig(filePath, resolver) {
	try {
		...
		if (!rawConfig.styleguide) {
			throw new Error(
				`Styleguide configuration format not detected: "${filePath}"`,
			);
		}
...
function getMergedRawStyleguideConfig(
	...
	const resultLint = {
		...
		oas2Rules: {
			...rootStyleguideConfig?.oas2Rules,
			...apiStyleguideConfig?.oas2Rules,
		},
		oas3_0Rules: {
			...rootStyleguideConfig?.oas3_0Rules,
			...apiStyleguideConfig?.oas3_0Rules,
		},
		oas3_1Rules: {
			...rootStyleguideConfig?.oas3_1Rules,
			...apiStyleguideConfig?.oas3_1Rules,
		},
		async2Rules: {
			...rootStyleguideConfig?.async2Rules,
			...apiStyleguideConfig?.async2Rules,
		},
		async3Rules: {
			...rootStyleguideConfig?.async3Rules,
			...apiStyleguideConfig?.async3Rules,
		},
		arazzo1Rules: {
			...rootStyleguideConfig?.arazzo1Rules,
			...apiStyleguideConfig?.arazzo1Rules,
		},
		overlay1Rules: {
		...
		decorators: {
			...rootStyleguideConfig?.decorators,
			...apiStyleguideConfig?.decorators,
		},
		oas2Decorators: {
...
function registerCustomAssertions(plugins, assertion) {
	for (const field of (0, utils_1.keysOf)(assertion.assertions)) {
		...
		if (!plugin) {
			throw Error(
				logger_1.colorize.red(
					`Plugin ${logger_1.colorize.blue(pluginId)} isn't found.`,
				),
			);
		}
		if (!plugin.assertions || !plugin.assertions[fn]) {
			throw Error(
				`Plugin ${logger_1.colorize.red(pluginId)} doesn't export assertions function with name ${logger_1.colorize.red(fn)}.`,
			);
		}
		asserts_1.asserts[field] = (0, asserts_1.buildAssertCustomFunction)(
