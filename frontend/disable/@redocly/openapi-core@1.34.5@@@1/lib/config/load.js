...
async function loadConfig(options = {}) {
	...
	const redoclyClient = env_1.isBrowser
		? undefined
		: new redocly_1.RedoclyClient();
	const tokens =
		redoclyClient && redoclyClient.hasTokens()
			? redoclyClient.getAllTokens()
			: [];
	...
	if (
		...
		parsed &&
		resolvedRefMap &&
		typeof processRawConfig === "function"
	) {
		try {
			...
			throw new Error(
				`Error parsing config file at '${configPath}': ${e.message}`,
			);
		}
	}
	return config;
...
function findConfig(dir) {
	...
	const existingConfigFiles = exports.CONFIG_FILE_NAMES.map((name) =>
...
async function getConfig(options = {}) {
	...
	try {
		const { document, resolvedRefMap } = await (0,
		config_resolvers_1.resolveConfigFileAndRefs)({
			configPath,
			externalRefResolver,
		});
		const bundledRefMap = (0, utils_2.deepCloneMapWithJSON)(resolvedRefMap);
		const parsed = await (0, bundle_1.bundleConfig)(
...
async function createConfig(config, options) {
	return addConfigMetadata({
		rawConfig: (0, utils_2.transformConfig)(
			typeof config === "string" ? (0, js_yaml_1.parseYaml)(config) : config,
		),
		...options,
	});
}
