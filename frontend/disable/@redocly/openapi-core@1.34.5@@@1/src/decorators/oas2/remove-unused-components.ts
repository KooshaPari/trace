	...
	return {
		ref: {
			leave(ref, { location, type, resolve, key }) {
				if (
					["Schema", "Parameter", "Response", "SecurityScheme"].includes(
						type.name,
					)
				) {
					...
					const [fileLocation, localPointer] =
						resolvedRef.location.absolutePointer.split("#", 2);
					const componentLevelLocalPointer = localPointer
						.split("/")
						.slice(0, 3)
						.join("/");
