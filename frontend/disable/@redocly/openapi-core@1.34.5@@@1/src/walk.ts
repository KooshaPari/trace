...
import type { Document, ResolvedRefMap, ResolveError, Source } from "./resolve";
import { makeRefId, YamlParseError } from "./resolve";
import type { NormalizedNodeType } from "./types";
import { isNamedType, SpecExtension } from "./types";
import type { Referenced } from "./typings/openapi";
import { popStack, pushStack } from "./utils";
import type {
	BaseVisitor,
...
export function walkDocument<T extends BaseVisitor>(opts: {
	...
	function walkNode(
		...
		const resolve: ResolveFn = (
			ref,
			from = currentLocation.source.absoluteRef,
		) => {
		...
		if (
			...
			const currentEnterVisitors = anyEnterVisitors.concat(
				(normalizedVisitors[type.name]?.enter as NormalizeVisitor<
					VisitorNode<unknown>[]
				>) || [],
			);

			const activatedContexts: Array<
				VisitorSkippedLevelContext | VisitorLevelContext
			> = [];
			...
			for (const {
				...
				if (
					ignoredNodes.has(
						`${currentLocation.absolutePointer}${currentLocation.pointer}`,
					)
				)
					break;
				...
				if (context.isSkippedLevel) {
					...
					if (
						(context.parent && // if nested
							context.parent.activatedOn &&
							context.activatedOn?.value.withParentNode !==
							...
							context.parent.activatedOn.value.nextLevelTypeActivated?.value !==
								type) ||
						...
						context.activatedOn = pushStack<any>(
							context.activatedOn,
							activatedOn,
						);
			...
			if (visitedBySome || !isNodeSeen) {
				...
				if (Array.isArray(resolvedNode)) {
					...
					if (itemsType !== undefined) {
						...
						for (let i = 0; i < resolvedNode.length; i++) {
							const itemType = isTypeAFunction
								? itemsType(
										resolvedNode[i],
										resolvedLocation.child([i]).absolutePointer,
									)
								: itemsType;
					...
					if (type.additionalProperties) {
						props.push(
							...Object.keys(resolvedNode).filter((k) => !props.includes(k)),
						);
					...
					for (const propName of props) {
						...
						if (
							...
							propType.resolvable !== false
						) {
							propType = { name: "scalar", properties: {} };
						}

						if (
							!isNamedType(propType) ||
							(propType.name === "scalar" && !isRef(value))
						) {
							continue;
						}
