...
import type { ProblemSeverity, ResolveResult, UserContext } from "./walk";
...
type Oas3FlatVisitor = {
	...
	ClientCredentials?: VisitFunctionOrObject<
		Oas3SecurityScheme["flows"]["clientCredentials"]
	>;
	AuthorizationCode?: VisitFunctionOrObject<
		Oas3SecurityScheme["flows"]["authorizationCode"]
	>;
...
export function normalizeVisitors<T extends BaseVisitor>(
	...
	function addWeakNodes(
		...
		if (
			...
			typeof from.additionalProperties !== "function"
		) {
		...
		function addWeakFromStack(
			ruleConf: RuleInstanceConfig,
			stack: NormalizedNodeType[],
		) {
			for (const interType of stack.slice(1)) {
				(normalizedVisitors as any)[interType.name] = normalizedVisitors[
					interType.name
				] || {
					enter: [],
					leave: [],
