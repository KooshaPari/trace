...
export function parsePresetName(presetName: string): {
	pluginId: string;
	configName: string;
} {
...
function extractFlatConfig<
	T extends Partial<
		(Api & DeprecatedInApi & FlatApi) &
			(DeprecatedInRawConfig & RawConfig & FlatRawConfig)
	>,
>({
	plugins,
	extends: _extends,
...
export function prefixRules<T extends Record<string, any>>(
	rules: T,
	prefix: string,
) {
