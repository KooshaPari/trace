...
describe.skip("resolving a plugin", () => {
	...
	it("should prefix rule names with the plugin id", async () => {
		...
		expect(plugin.rules?.oas3).toHaveProperty(
			"test-plugin/openid-connect-url-well-known",
		);
	});
	...
	it("should prefix preprocessor names with the plugin id", async () => {
		...
		expect(plugin.preprocessors?.oas2).toHaveProperty(
			"test-plugin/description-preprocessor",
		);
	});
	...
	it("should prefix decorator names with the plugin id", async () => {
		...
		expect(plugin.decorators?.oas3).toHaveProperty(
			"test-plugin/inject-x-stats",
		);
	});
});
