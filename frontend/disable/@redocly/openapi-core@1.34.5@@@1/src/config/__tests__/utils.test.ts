import type { DeprecatedInRawConfig, FlatRawConfig, RawConfig } from "../types";
...
describe("transformConfig", () => {
	...
	it("should be equal for both `lint` and `styleguide` names", () => {
		expect(
			utils.transformConfig(makeTestRawConfig("lint", "styleguide")),
		).toEqual(utils.transformConfig(makeTestRawConfig("styleguide", "lint")));
	});
	...
	it("should fail when there is a mixed config", () => {
		expect(() =>
			utils.transformConfig({ ...rawTestConfig, extends: ["recommended"] }),
		).toThrow(
