...
import { createConfig, findConfig, getConfig, loadConfig } from "../load";
import type { FlatRawConfig, RuleConfig } from "./../types";
...
describe("loadConfig", () => {
	it("should resolve config http header by US region", async () => {
		...
		jest
			.spyOn(RedoclyClient.prototype, "hasTokens")
	...
	it("should resolve config and call processRawConfig", async () => {
		...
		await loadConfig({
			configPath: path.join(
				__dirname,
				"./fixtures/resolve-refs-in-config/config-with-refs.yaml",
			),
			processRawConfig: async ({
				document,
				parsed,
				resolvedRefMap,
				config,
...
describe("findConfig", () => {
	it("should find redocly.yaml", async () => {
		jest
			.spyOn(fs, "existsSync")
			.mockImplementation((name) => name === "redocly.yaml");
	...
	it("should find a nested config ", async () => {
		jest
			.spyOn(fs, "existsSync")
			.mockImplementation((name) => name === "dir/redocly.yaml");
		jest
			.spyOn(path, "resolve")
			.mockImplementationOnce((dir, name) => `${dir}/${name}`);
...
function verifyExtendedConfig(
	...
	const defaultPlugin = config.styleguide.plugins.find(
		(plugin) => plugin.id === "",
	);
