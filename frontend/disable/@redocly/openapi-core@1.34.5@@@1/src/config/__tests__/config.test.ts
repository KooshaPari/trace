...
describe("StyleguideConfig.extendTypes", () => {
	...
	it("should throw error if for oas version different from 2 and 3", () => {
		const styleguideConfig = new StyleguideConfig(testRawConfigStyleguide);
		expect(() =>
			styleguideConfig.extendTypes({}, "something else" as SpecVersion),
...
describe("generation ignore object", () => {
	it("should generate config with absoluteUri for ignore", () => {
		...
		(parseYaml as jest.Mock<any, any>).mockImplementationOnce(
			() => ignoredFileStub,
		);
		(doesYamlFileExist as jest.Mock<any, any>).mockImplementationOnce(
			() => true,
		);

		jest
			.spyOn(path, "resolve")
			.mockImplementationOnce((_, filename) => `some-path/${filename}`);
