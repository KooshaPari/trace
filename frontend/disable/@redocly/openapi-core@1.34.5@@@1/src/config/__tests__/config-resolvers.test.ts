...
import { type Asserts, asserts } from "../../rules/common/assertions/asserts";
import {
	resolveApis,
	resolveConfig,
...
const removeAbsolutePath = (item: string) =>
	item.match(
		/^.*\/packages\/core\/src\/config\/__tests__\/fixtures\/(.*)$/,
	)![1];
...
describe("resolveStyleguideConfig", () => {
	it("should return the config with no recommended", async () => {
		const styleguide = await resolveStyleguideConfig({
			styleguideConfig: baseStyleguideConfig,
		});
	...
	it("should resolve extends with local file config which contains path to nested config", async () => {
		...
		expect(styleguide?.rules?.["no-invalid-media-type-examples"]).toEqual(
			"warn",
		);
	...
	it("should resolve custom assertion from plugin", async () => {
		...
		expect(plugins?.length).toBe(2);
		expect(
			asserts["test-plugin/checkWordsCount" as keyof Asserts],
		).toBeDefined();
	});
	...
	it("should resolve `recommended-strict` ruleset correctly", async () => {
		...
		for (const section of Object.values(expectedStrict)) {
			for (const ruleName in section as any) {
				// @ts-expect-error
...
describe("resolveApis", () => {
	it("should resolve apis styleguideConfig and merge minimal extends", async () => {
		...
		expect(apisResult["petstore"].styleguide).toEqual(
	...
	it("should not merge recommended extends by default by every level", async () => {
		...
		expect(
			apisResult["petstore"].styleguide.extendPaths!.map(removeAbsolutePath),
		).toEqual(["resolve-config/redocly.yaml"]);
		expect(
			apisResult["petstore"].styleguide.pluginPaths!.map(removeAbsolutePath),
		).toEqual([]);
	...
	it("should resolve apis styleguideConfig when it contains file", async () => {
		...
		expect(
			apisResult["petstore"].styleguide.rules?.["operation-2xx-response"],
		).toEqual("warn"); // think about prioritize in merge ???
		expect(
			apisResult["petstore"].styleguide.rules?.["operation-4xx-response"],
		).toEqual("error");
		expect(
			apisResult["petstore"].styleguide.rules?.["local/operation-id-not-test"],
		).toEqual("error");
		//@ts-expect-error
		...
		expect(
			apisResult["petstore"].styleguide.pluginPaths!.map(removeAbsolutePath),
		).toEqual(["resolve-config/plugin.js"]);
	});
});
...
describe("resolveConfig", () => {
	it("should NOT add recommended to top level by default IF there is a config file", async () => {
		...
		expect(
			apis["petstore"].styleguide.extendPaths!.map(removeAbsolutePath),
		).toEqual(["resolve-config/redocly.yaml"]);
		expect(
			apis["petstore"].styleguide.pluginPaths!.map(removeAbsolutePath),
		).toEqual([]);
	...
	it("should not add recommended to top level by default when apis have extends file", async () => {
		...
		expect(Object.keys(apis["petstore"].styleguide.rules || {}).length).toEqual(
			7,
		);
		expect(
			apis["petstore"].styleguide.rules?.["operation-2xx-response"],
		).toEqual("warn");
		expect(
			apis["petstore"].styleguide.rules?.["operation-4xx-response"],
		).toEqual("error");
		expect(
			apis["petstore"].styleguide.rules?.["operation-description"],
		).toEqual("error"); // from extends file config
		//@ts-expect-error
	...
	it("should default to the extends from the main config if no extends defined", async () => {
		...
		expect(
			apis["petstore"].styleguide.rules?.["operation-2xx-response"],
		).toEqual("warn"); // from minimal ruleset
	});
});
