...
import type {
	...
	AssertionDefinition,
	RawAssertion,
} from "../rules/common/assertions";
import type { AssertionFn, Asserts } from "../rules/common/assertions/asserts";
import {
	asserts,
...
export async function resolveConfig({
	...
	const resolver =
		externalRefResolver ??
		new BaseResolver(getResolveConfig(rawConfig.resolve));
				...
				if (!pluginsCache.has(absolutePluginPath)) {
					...
					if (typeof __webpack_require__ === "function") {
						...
						const mod = await _importDynamic(
							pathToFileURL(absolutePluginPath).href,
						);
		...
		plugins.map(async (p) => {
			...
			if (pluginModule.preprocessors) {
				...
				if (pluginModule.preprocessors.oas3) {
					plugin.preprocessors.oas3 = prefixRules(
						pluginModule.preprocessors.oas3,
						id,
					);
				}
				if (pluginModule.preprocessors.oas2) {
					plugin.preprocessors.oas2 = prefixRules(
						pluginModule.preprocessors.oas2,
						id,
					);
				}
				if (pluginModule.preprocessors.async2) {
					plugin.preprocessors.async2 = prefixRules(
						pluginModule.preprocessors.async2,
						id,
					);
				}
				if (pluginModule.preprocessors.async3) {
					plugin.preprocessors.async3 = prefixRules(
						pluginModule.preprocessors.async3,
						id,
					);
				}
			...
			if (pluginModule.decorators) {
				...
				if (pluginModule.decorators.oas3) {
					plugin.decorators.oas3 = prefixRules(
						pluginModule.decorators.oas3,
						id,
					);
				}
				if (pluginModule.decorators.oas2) {
					plugin.decorators.oas2 = prefixRules(
						pluginModule.decorators.oas2,
						id,
					);
				}
				if (pluginModule.decorators.async2) {
					plugin.decorators.async2 = prefixRules(
						pluginModule.decorators.async2,
						id,
					);
				}
				if (pluginModule.decorators.async3) {
					plugin.decorators.async3 = prefixRules(
						pluginModule.decorators.async3,
						id,
					);
				}
				if (pluginModule.decorators.arazzo1) {
					plugin.decorators.arazzo1 = prefixRules(
						pluginModule.decorators.arazzo1,
						id,
					);
				}
				if (pluginModule.decorators.overlay1) {
					plugin.decorators.overlay1 = prefixRules(
						pluginModule.decorators.overlay1,
						id,
					);
				}
...
async function loadExtendStyleguideConfig(
	...
	try {
		const { parsed } = (await resolver.resolveDocument(
			null,
			filePath,
		)) as Document;
		const rawConfig = transformConfig(
			parsed as RawConfig & DeprecatedInRawConfig,
		);
...
function registerCustomAssertions(
	...
	for (const field of keysOf(assertion.assertions)) {
		...
		if (!plugin) {
			throw Error(
				colorize.red(`Plugin ${colorize.blue(pluginId)} isn't found.`),
			);
		}
		...
		(asserts as Asserts & { [name: string]: AssertionFn })[field] =
			buildAssertCustomFunction(plugin.assertions[fn]);
	}
}
