...
export const ComponentNameUnique: Oas3Rule | Oas2Rule = (options) => {
			...
			leave(root: Oas3Definition | Oas3_1Definition, ctx: UserContext) {
				components.forEach((value, key, _) => {
					if (value.absolutePointers.size > 1) {
						...
						const optionComponentName = getOptionComponentNameForTypeName(
							component.typeName,
						);
						const componentSeverity = optionComponentName
							? options[optionComponentName]
							: null;
	...
	function getComponentNameFromAbsoluteLocation(
	...
	function addComponentFromAbsoluteLocation(
		typeName: string,
		location: Location,
	): void {
...
function getComponentFromKey(key: string): {
	typeName: string;
	componentName: string;
} {
