...
import type { Oas3_1Schema, Oas3Schema, Referenced } from "../typings/openapi";
import {
	isPlainObject,
 ...
 * @returns boolean
 */
export function matchesJsonSchemaType(
	value: unknown,
	type: string,
	nullable: boolean,
): boolean {
...
export function missingRequiredOneOfFields(
	type: string,
	fields: string[],
): string {
...
export function validateOneOfDefinedAndNonEmpty(
	...
	if (!fieldNames.some((fieldName) => Object.hasOwn(value, fieldName))) {
		ctx.report({
	...
	for (const fieldName of fieldNames) {
		if (Object.hasOwn(value, fieldName) && !value[fieldName]) {
			ctx.report({
				message: fieldNonEmpty(ctx.type.name, fieldName),
				location: ctx.location.child([fieldName]).key(),
			});
...
export function validateExample(
	...
	try {
		...
		if (
			e.message === "discriminator: requires oneOf or anyOf composite keyword"
		) {
			return;
		}
...
export function getAdditionalPropertiesOption(
