...
import { isString, isTruthy, keysOf } from "../../../utils";
import type {
	Oas2Visitor,
	Oas3Visitor,
	SkipFunctionContext,
...
export function getAssertsToApply(
...
export function buildSubjectVisitor(
	...
	return (node: any, ctx: UserContext) => {
		...
		const defaultMessage =
			`${colorize.blue(assertId)} failed because the ${colorize.blue(
				assertion.subject.type,
...
export function isOrdered(
	value: any[],
	options: OrderOptions | OrderDirection,
): boolean {
	const direction =
		(options as OrderOptions).direction || (options as OrderDirection);
