    ...
    `;
		...
		const results = await lintDocument({
			...
			config: await makeConfig({
				rules: { "no-schema-type-mismatch": "warn" },
			}),
		});
		...
		expect(replaceSourceWithRef(results)).toEqual([
			{
				location: [
					{
						pointer:
							"#/paths/~1test/get/responses/200/content/application~1json/schema/items",
						reportOnKey: false,
						source: "test.yaml",
					},
				],
		...
		expect(replaceSourceWithRef(results)).toEqual([
			{
				location: [
					{
						pointer:
							"#/paths/~1test/get/responses/200/content/application~1json/schema/properties",
						reportOnKey: false,
						source: "test.yaml",
					},
				],
