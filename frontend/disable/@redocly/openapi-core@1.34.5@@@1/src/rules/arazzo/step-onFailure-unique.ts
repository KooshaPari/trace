			...
			enter(onFailureActionList, { report, location }: UserContext) {
				...
				for (const onFailureAction of onFailureActionList) {
					if (seenFailureActions.has(onFailureAction?.name)) {
						report({
							message:
								"The action `name` must be unique amongst listed `onFailure` actions.",
							location: location.child([
								onFailureActionList.indexOf(onFailureAction),
							]),
						});
					}
					...
					if (seenFailureActions.has(onFailureAction?.reference)) {
						report({
							message:
								"The action `reference` must be unique amongst listed `onFailure` actions.",
							location: location.child([
								onFailureActionList.indexOf(onFailureAction),
							]),
						});
