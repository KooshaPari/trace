...
export function pickObjectProps<T extends Record<string, unknown>>(
	...
	return Object.fromEntries(
		keys
			.filter((key: string) => key in object)
...
export function assignOnlyExistingConfig<
	T extends string | { severity?: string },
>(target: Record<string, T>, obj?: Record<string, T>) {
	if (!obj) return;
	for (const k of Object.keys(obj)) {
...
export function showErrorForDeprecatedField(
	...
	throw new Error(
		`Do not use '${deprecatedField}' field. ${
			updatedField
				? `Use '${getUpdatedFieldName(updatedField, updatedObject)}' instead. `
				: ""
		}\n`,
	);
}
