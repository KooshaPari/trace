...
describe("utils", () => {
	...
	describe("pickObjectProps", () => {
		it("returns correct object result", () => {
			expect(pickObjectProps(testObject, ["a", "b"])).toStrictEqual({
				a: "value a",
				b: "value b",
			});
		});
		...
		it("returns correct object if passed non existing key", () => {
			expect(
				pickObjectProps(testObject, ["a", "b", "nonExisting"]),
			).toStrictEqual({
				a: "value a",
				b: "value b",
			});
	...
	describe("omitObjectProps", () => {
		...
		it("returns correct object if passed non existing key", () => {
			expect(
				omitObjectProps(testObject, ["a", "b", "c", "nonExisting"]),
			).toStrictEqual({
				d: "value d",
				e: "value e",
			});
	...
	describe("pickDefined", () => {
		...
		it("picks only defined values", () => {
			expect(pickDefined({ a: 1, b: undefined, c: 3 })).toStrictEqual({
				a: 1,
				c: 3,
			});
		});
