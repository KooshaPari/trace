...
import { getTypes, SpecVersion } from "../oas-types";
...
const createConfigApisProperties = (
...
const Rules: NodeType = {
	...
	additionalProperties: (value: unknown, key: string) => {
		if (key.startsWith("rule/")) {
			...
			builtInRules.includes(key as BuiltInRuleId) ||
			isCustomRuleId(key)
		) {
			if (typeof value === "string") {
				return { enum: ["error", "warn", "off"] };
...
const BadgeFontConfig: NodeType = {
	properties: pickObjectProps(FontConfig.properties, [
		"fontSize",
		"lineHeight",
	]),
};
...
export function createConfigTypes(extraSchemas: JSONSchema, config?: Config) {
	...
	const nodeTypes = getNodeTypesFromJSONSchema(
