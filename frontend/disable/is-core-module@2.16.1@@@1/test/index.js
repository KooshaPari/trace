...
test("core modules", (t) => {
	...
	t.test("core via repl module", { skip: !data.repl }, (st) => {
	...
	t.test("core via builtinModules list", { skip: !data.module }, (st) => {
		...
		if (!libs) {
			...
			for (var i = 0; i < libs.length; ++i) {
				...
				if (excludeList.indexOf(mod) === -1) {
					...
					if (Module.isBuiltin) {
						st.ok(
							Module.isBuiltin(mod),
							"module.isBuiltin(" + mod + ") is true",
						);
					}
	...
	t.test("Object.prototype pollution", (st) => {
		var nonKey = "not a core module";
		st.teardown(mockProperty(Object.prototype, "fs", { value: false }));
		st.teardown(
			mockProperty(Object.prototype, "path", { value: ">= 999999999" }),
		);
