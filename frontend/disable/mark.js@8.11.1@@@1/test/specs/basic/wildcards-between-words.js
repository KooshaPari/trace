...
describe("basic mark with wildcards between words", function () {
	...
	it("should match wildcard with zero to one non-whitespace in the keyword", function () {
		expect($ctx1.find("mark")).toHaveLength(4);
	});
	it("should match wildcard with zero or more non-whitespace in the keyword", function () {
		expect($ctx2.find("mark")).toHaveLength(5);
	});
	it("should match wildcard with zero to one character in the keyword", function () {
		expect($ctx3.find("mark")).toHaveLength(6);
	});
	it("should match wildcard with zero or more characters in the keyword", function () {
