"use strict";
describe("mark with acrossElements, regular expression and ignoreGroups", function () {
	var $ctx1,
		$ctx2,
		prefix = "across-elements-regexp";
	beforeEach(function (done) {
		...
		new Mark($ctx1[0]).markRegExp(/(Lor)([^]?m[\s]*)(ipsum)/gim, {
			acrossElements: true,
			done: function () {
				new Mark($ctx2[0]).markRegExp(/(Lor)([^]?m[\s]*)(ipsum)/gim, {
					acrossElements: true,
					ignoreGroups: 2,
					done: done,
				});
