...
function pushCommonFlags(flags, options, keys) {
	...
	const drop = getFlag(options, keys, "drop", mustBeArray);
	const dropLabels = getFlag(options, keys, "dropLabels", mustBeArray);
...
function flagsForBuildOptions(
	...
	if (resolveExtensions) {
		const values = [];
		for (const value of resolveExtensions) {
	...
	const handleIncomingPacket = (bytes) => {
		if (isFirstPacket) {
			...
			if (binaryVersion !== "0.20.2") {
				throw new Error(
					`Cannot start service: Host version "${"0.20.2"}" does not match binary version ${quote(binaryVersion)}`,
				);
			}
			return;
		}
		...
		for (const id2 of request.ids) {
			try {
				...
				if (result != null) {
					...
					const errors = getFlag(result, keys, "errors", mustBeArray);
					const warnings = getFlag(result, keys, "warnings", mustBeArray);
					const watchFiles = getFlag(result, keys, "watchFiles", mustBeArray);
					const watchDirs = getFlag(result, keys, "watchDirs", mustBeArray);
					checkForInvalidFlags(
						result,
						keys,
...
var ensureServiceIsRunning = () => {
	...
	const child = child_process.spawn(
		command,
		args.concat(`--service=${"0.20.2"}`, "--ping"),
		{
			windowsHide: true,
