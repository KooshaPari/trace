...
const makeContentText = (text, { padding, width, textAlignment, height }) => {
	...
	if (textWidth > max) {
		...
		for (const line of lines) {
			...
			const longestLength = Math.max(
				...alignedLinesArray.map((s) => stringWidth(s)),
			);
	...
	if (padding.top > 0) {
		lines = [
			...Array.from({ length: padding.top }).fill(PAD.repeat(width)),
			...lines,
		];
	}
	...
	if (padding.bottom > 0) {
		lines = [
			...lines,
			...Array.from({ length: padding.bottom }).fill(PAD.repeat(width)),
		];
	}
	...
	if (height && lines.length > height) {
		...
		lines = [
			...lines,
			...Array.from({ length: height - lines.length }).fill(PAD.repeat(width)),
		];
	}
	...
	if (options.float === "center") {
		...
		const marginWidth = Math.max(
			columns -
				...
				getBorderWidth(options.borderStyle),
			0,
		);
		marginLeft = PAD.repeat(marginWidth);
	}
...
const sanitizeOptions = (options) => {
	...
	if (options.width) {
		options.width = Math.max(
			1,
			options.width - getBorderWidth(options.borderStyle),
		);
	}
	...
	if (options.height) {
		options.height = Math.max(
			1,
			options.height - getBorderWidth(options.borderStyle),
		);
	}
...
const formatTitle = (title, borderStyle) =>
...
const determineDimensions = (text, options) => {
	...
	const maxWidth =
		columns - options.margin.left - options.margin.right - borderWidth;

	const widest =
		widestLine(
			wrapAnsi(text, columns - borderWidth, { hard: true, trim: false }),
		) +
	...
	if (!widthOverride) {
		if (
			...
			const multiplier =
				spaceForMargins / (options.margin.left + options.margin.right);
			// Here: multiplier = 4/8 = 0.5
			options.margin.left = Math.max(
				0,
				Math.floor(options.margin.left * multiplier),
			);
			options.margin.right = Math.max(
				0,
				Math.floor(options.margin.right * multiplier),
			);
			// Left: 3 * 0.5 = 1.5 -> 1
			// Right: 6 * 0.5 = 3
		}
	...
	if (
		options.height &&
		options.height - (options.padding.top + options.padding.bottom) <= 0
	) {
