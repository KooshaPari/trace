		...
		static init(moduleOptions) {
			...
			return (initPromise = new Promise((resolveInitPromise) => {
				...
				function getBinaryPromise(binaryFile) {
					...
					if (!wasmBinary) {
						...
						return readAsync(binaryFile).then(
							(response) =>
								new Uint8Array(/** @type{!ArrayBuffer} */ (response)), // Fall back to getBinarySync if readAsync fails
				...
				var _ts_language_symbol_for_name = (Module[
					...
					(_ts_language_symbol_for_name = Module[
						"_ts_language_symbol_for_name"
					] =
				...
				var _ts_lookahead_iterator_reset = (Module[
					...
					(_ts_lookahead_iterator_reset = Module[
						"_ts_lookahead_iterator_reset"
					] =
						wasmExports["ts_lookahead_iterator_reset"])(a0, a1, a2));
				var _ts_lookahead_iterator_next = (Module[
				...
				var _ts_query_new = (Module["_ts_query_new"] = (a0, a1, a2, a3, a4) =>
					(_ts_query_new = Module["_ts_query_new"] =
						wasmExports["ts_query_new"])(a0, a1, a2, a3, a4));
				...
				var _ts_node_grammar_symbol_wasm = (Module[
					...
					(_ts_node_grammar_symbol_wasm = Module[
						"_ts_node_grammar_symbol_wasm"
					] =
				...
				var _ts_node_named_children_wasm = (Module[
					...
					(_ts_node_named_children_wasm = Module[
						"_ts_node_named_children_wasm"
					] =
				...
				var _setThrew = (a0, a1) =>
					(_setThrew = wasmExports["setThrew"])(a0, a1);
				...
				class Node {
					...
					fieldNameForChild(index) {
						marshalNode(this);
						const address = C._ts_node_field_name_for_child_wasm(
							this.tree[0],
							index,
						);
						if (!address) {
				...
				class TreeCursor {
					...
					get currentDescendantIndex() {
						marshalTreeCursor(this);
						return C._ts_tree_cursor_current_descendant_index_wasm(
							this.tree[0],
						);
					}
					gotoFirstChild() {
					...
					gotoPreviousSibling() {
						marshalTreeCursor(this);
						const result = C._ts_tree_cursor_goto_previous_sibling_wasm(
							this.tree[0],
						);
						unmarshalTreeCursor(this);
						return result === 1;
					...
					gotoDescendant(goalDescendantindex) {
						marshalTreeCursor(this);
						C._ts_tree_cursor_goto_descendant_wasm(
							this.tree[0],
							goalDescendantindex,
						);
						unmarshalTreeCursor(this);
									...
									switch (operator) {
										...
										case "match?":
											...
											textPredicates[i].push((captures) => {
												...
												const test = (text, positive) =>
													positive ? regex.test(text) : !regex.test(text);
										...
										case "any-of?":
											...
											textPredicates[i].push((captures) => {
												...
												if (nodes.length === 0) return !isPositive;
												return (
													nodes.every((text) => values.includes(text)) ===
													isPositive
												);
											});
											break;
				...
				class Query {
					...
					matches(
						node,
						{
							startPosition: startPosition = ZERO_POINT,
							endPosition: endPosition = ZERO_POINT,
							startIndex: startIndex = 0,
							endIndex: endIndex = 0,
							matchLimit: matchLimit = 4294967295,
							maxStartDepth: maxStartDepth = 4294967295,
							timeoutMicros: timeoutMicros = 0,
						} = {},
					) {
