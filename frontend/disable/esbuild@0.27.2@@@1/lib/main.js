	...
	const handleIncomingPacket = (bytes) => {
		if (isFirstPacket) {
			...
			if (binaryVersion !== "0.27.2") {
				throw new Error(
					`Cannot start service: Host version "${"0.27.2"}" does not match binary version ${quote(binaryVersion)}`,
				);
			}
			return;
		}
...
var ensureServiceIsRunning = () => {
	...
	const child = child_process.spawn(
		command,
		args.concat(`--service=${"0.27.2"}`, "--ping"),
		{
			windowsHide: true,
