...
EventTargetMock.prototype.addEventListener = function addEventListener(
	name,
	listener,
	options,
) {
...
EventTargetMock.prototype.removeEventListener = function removeEventListener(
	name,
	callback,
) {
	if (!(name in this.events)) {
...
function onceAnEvent() {
	...
	return once(ee, "myevent").then((args) => {
		var value = args[0];
...
function onceAnEventWithTwoArgs() {
	...
	return once(ee, "myevent").then((value) => {
...
function stopListeningAfterCatchingError() {
	...
	return once(ee, "myevent").then(common.mustNotCall, (err) => {
...
function onceWithEventTarget() {
	...
	return once(et, "myevent").then((args) => {
		var value = args[0];
...
function onceWithEventTargetError() {
	...
	return once(et, "error").then((args) => {
		var err = args[0];
...
if (hasBrowserEventTarget) {
	...
	var onceWithBrowserEventTargetError =
