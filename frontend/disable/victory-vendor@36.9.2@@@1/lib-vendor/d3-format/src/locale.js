...
function _default(locale) {
	var group =
		...
		numerals =
			locale.numerals === undefined
				? _identity.default
				: (0, _formatNumerals.default)(map.call(locale.numerals, String)),
	...
	function newFormat(specifier) {
		...
		if (type === "n")
			(comma = true), (type = "g"); // The "" type, and any invalid type, is an alias for ".12~g".
		else if (!_formatTypes.default[type])
			precision === undefined && (precision = 12), (trim = true), (type = "g"); // If zero fill is specified, padding goes after sign and before digits.

		if (zero || (fill === "0" && align === "="))
			(zero = true), (fill = "0"), (align = "="); // Compute the prefix and suffix.
		...
		var prefix =
				symbol === "$"
					...
					: symbol === "#" && /[boxX]/.test(type)
						...
						: "",
			suffix =
				symbol === "$" ? currencySuffix : /[%p]/.test(type) ? percent : ""; // What format function should we use?
		...
		function format(value) {
			...
			if (type === "c") {
				...
				if (valueNegative && +value === 0 && sign !== "+")
					valueNegative = false; // Compute the prefix and suffix.
				...
				valueSuffix =
					(type === "s"
						? prefixes[8 + _formatPrefixAuto.prefixExponent / 3]
						: "") +
					valueSuffix +
					(valueNegative && sign === "(" ? ")" : ""); // Break the formatted value into the integer “value” part that can be
			...
			var length = valuePrefix.length + value.length + valueSuffix.length,
				padding =
					length < width ? new Array(width - length + 1).join(fill) : ""; // If the fill character is "0", grouping is applied after padding.
			...
			if (comma && zero)
				(value = group(
					...
					(padding = ""); // Reconstruct the final output based on the desired alignment.
