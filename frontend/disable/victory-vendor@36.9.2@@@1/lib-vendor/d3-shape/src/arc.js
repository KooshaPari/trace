...
function cornerTangents(x0, y0, x1, y1, r1, rc, cw) {
	var x01 = x0 - x1,
		...
		d =
			(dy < 0 ? -1 : 1) *
			(0, _math.sqrt)((0, _math.max)(0, r * r * d2 - D * D)),
		cx0 = (D * dy - dx * d) / d2,
		cy0 = (-D * dx - dy * d) / d2,
		cx1 = (D * dy + dx * d) / d2,
		cy1 = (-D * dx + dy * d) / d2,
...
function _default() {
	...
	function arc() {
		...
		if (!(r1 > _math.epsilon))
			...
			if (rp > _math.epsilon) {
				...
				if ((da0 -= p0 * 2) > _math.epsilon)
					(p0 *= cw ? 1 : -1), (a00 += p0), (a10 -= p0);
				else (da0 = 0), (a00 = a10 = (a0 + a1) / 2);
				if ((da1 -= p1 * 2) > _math.epsilon)
					...
					var ax = x01 - oc[0],
						...
						kc =
							...
							(0, _math.sin)(
								(0, _math.acos)(
									(ax * bx + ay * by) /
										((0, _math.sqrt)(ax * ax + ay * ay) *
											(0, _math.sqrt)(bx * bx + by * by)),
								) / 2,
							),
			...
			if (!(da1 > _math.epsilon))
				...
				if (rc1 < rc)
					context.arc(
						...
						t0.cy,
						rc1,
						(0, _math.atan2)(t0.y01, t0.x01),
						(0, _math.atan2)(t1.y01, t1.x01),
						!cw,
					...
					context.arc(
						...
						0,
						r1,
						(0, _math.atan2)(t0.cy + t0.y11, t0.cx + t0.x11),
						(0, _math.atan2)(t1.cy + t1.y11, t1.cx + t1.x11),
						!cw,
					);
					context.arc(
						t1.cx,
			...
			if (!(r0 > _math.epsilon) || !(da0 > _math.epsilon))
	...
	arc.innerRadius = function (_) {
		return arguments.length
			? ((innerRadius =
					typeof _ === "function" ? _ : (0, _constant.default)(+_)),
				arc)
			: innerRadius;
	};
	...
	arc.endAngle = function (_) {
		return arguments.length
			? ((endAngle = typeof _ === "function" ? _ : (0, _constant.default)(+_)),
				arc)
			: endAngle;
	};
	...
	arc.padAngle = function (_) {
		return arguments.length
			? ((padAngle = typeof _ === "function" ? _ : (0, _constant.default)(+_)),
				arc)
			: padAngle;
	};
