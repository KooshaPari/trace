		...
		return function (string) {
			...
			if ("V" in d) {
				...
				if ("Z" in d) {
					...
					week =
						day > 4 || day === 0
							? _index.utcMonday.ceil(week)
							: (0, _index.utcMonday)(week);
					...
					week =
						day > 4 || day === 0
							? _index.timeMonday.ceil(week)
							: (0, _index.timeMonday)(week);
...
function dISO(d) {
	var day = d.getDay();
	return day >= 4 || day === 0
		? (0, _index.timeThursday)(d)
		: _index.timeThursday.ceil(d);
}

function formatWeekNumberISO(d, p) {
	d = dISO(d);
	return pad(
		_index.timeThursday.count((0, _index.timeYear)(d), d) +
			((0, _index.timeYear)(d).getDay() === 4),
		p,
		2,
	);
}
...
function UTCdISO(d) {
	var day = d.getUTCDay();
	return day >= 4 || day === 0
		? (0, _index.utcThursday)(d)
		: _index.utcThursday.ceil(d);
}

function formatUTCWeekNumberISO(d, p) {
	d = UTCdISO(d);
	return pad(
		_index.utcThursday.count((0, _index.utcYear)(d), d) +
			((0, _index.utcYear)(d).getUTCDay() === 4),
		p,
		2,
	);
}
