...
class NoDefaultLabelsError extends Error {
	constructor(lang, message) {
		super(
			message ||
				`There are no default labels for ${lang.englishName}. You must provide your own labels for this language.`,
		);
		this.lang = lang;
	}
}
...
((Labels) => {
	...
	function forEach(labels, fun, { onNoLanguageName } = {}) {
		for (const name of Labels.names) {
			...
			if (label === undefined && name === "languageName") {
				if (onNoLanguageName) onNoLanguageName();
			} else fun(label, name, labels);
		}
	}
	...
	function prepare(labels, lang) {
		...
		function writeStr(string, description, isDefaultLanguageName = false) {
			...
			try {
				...
				errors.add(
					isDefaultLanguageName &&
						e instanceof iconv_corefoundation_1.NotRepresentableError
						? new NoDefaultLabelsError(
								lang,
								`The default languageName label for ${lang.englishName}, “${lang.localizedName}”, is not representable in ${lang.charset}, the native character set for that language. Please provide a languageName label for this language that is representable in that character set.`,
							)
			...
			if (length > 255) {
				const e = new LabelEncodingError(
					description,
					lang,
					"the label is too large to write into a STR# resource. The maximum size is 255 bytes, but it is %d bytes.",
					length,
				);
				e.text = data;
				errors.add(e);
	...
	function prepareDefault(lang) {
		...
		if (!labels) throw new NoDefaultLabelsError(lang);
