...
((BodySpec) => {
	async function prepare(spec, lang, contextOrOptions = {}) {
		...
		function encodeBodyText(text) {
			try {
				if (typeof text === "string") return lang.charset.encode(text);
				else
					return iconv_corefoundation_1.transcode(
						text,
						spec.charset || "UTF-8",
						lang.charset,
					);
			} catch (e) {
				throw new format_verror_1.PrettyVError(
					e,
					"Cannot encode %s license text",
					lang.englishName,
				);
			}
		}
		...
		if (fpath) {
			...
			try {
				...
				throw new format_verror_1.PrettyVError(
					e,
					"Cannot read %s license text from “%s”",
					lang.englishName,
					fpath,
				);
			}
			data = encodeBodyText(ftext);
		} else data = encodeBodyText(spec.text);
		let type;
		if (spec.type === "rtf" || (fpath && fpath.endsWith(".rtf"))) type = "RTF ";
		else type = "TEXT";
		return { data, type };
	}
