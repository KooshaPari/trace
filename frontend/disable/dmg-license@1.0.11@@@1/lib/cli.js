...
function showHelp(onStderr = true) {
	...
	(onStderr
		? stderr
		: stdout
	).write(`Add a license agreement to a Mac disk image.
...
async function main() {
	...
	try {
		...
		if (args.help) {
			showHelp(false);
			if (args.version) showVersion(false);
			return;
		}
		if (args.version) {
			if (args._.length) showVersion();
			else {
				showVersion(false);
				return;
			}
				...
				if (language) {
					if (language.langTags.length) {
						for (const languageTag of language.langTags)
							stdout.write(
								`${language.languageID}\t${languageTag}\t${language.charset}\t${language.englishName}\n`,
							);
					} else
						stdout.write(
							`${language.languageID}\t\t${language.charset}\t${language.englishName}\n`,
						);
				}
			return;
		}
		...
		const jsonText = await fs_1.promises
			.readFile(args._[0], { encoding: "utf8" })
			.catch((e) => {
