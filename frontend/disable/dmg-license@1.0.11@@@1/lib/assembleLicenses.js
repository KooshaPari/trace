...
function warnAboutLanguageIDCollisions(kind, errors, context) {
	return (languageIDs) => {
		...
		context.warning(
			new Error(
				`More than one ${kind} was assigned to the language${plural ? "s" : ""} ${Array.from(
					languageIDs,
				)
					.map((languageID) => {
...
function hashAssembledLicense(content) {
	return [content.body.type, content.body.data, content.labels].reduce(
		(hash, next) => crc_1.crc32(next, hash),
		0,
	);
}
...
async function assembleLoadedLicenses(bodies, labelSets, errors, context) {
	...
	for (const license of assembled.values()) {
		...
		if (!withThisHash) hashes.set(hash, [license]);
		else {
			...
			if (hashCollision) withThisHash.push(license);
		}
	}
	return assembled;
}
function chooseDefaultLanguageID(spec, outputs, context) {
	// Use the configured default language, if available.
	{
		...
		switch (typeof configuredDefaultLanguage) {
			...
			case "string": {
				const lookup = Language_1.default.bySpec(
					configuredDefaultLanguage,
					context,
				)[0];
				if (lookup) return lookup.languageID;
			}
		}
	}
	...
	for (const output of outputs)
		for (const lang of output.languageIDs) return lang;
}
function labelCache(spec, errors, context) {
	const labelSpecs = Language_1.indexByLanguage(
		(function* () {
			const { labels, rawLabels } = spec;
			if (labels) for (const label of labels) yield label;
			if (rawLabels) for (const label of rawLabels) yield label;
		})(),
		{
	...
	return (lang) => {
		...
		if (!result) {
			result = Labels_1.default.prepareSpec(
				labelSpecs.get(languageID),
				lang,
				context,
			);
...
async function assembleLicenses(spec, optionsOrContext) {
	const context =
		optionsOrContext instanceof Context_1.default
			? optionsOrContext
			: new Context_1.default(optionsOrContext);
	...
	const bodies = Language_1.indexByLanguage(spec.body, {
		...
		onCollisions: warnAboutLanguageIDCollisions(
			"license body",
			errors,
			context,
		),
	});
	if (!bodies.size) errors.throw(new Error("No license bodies were provided."));
	let result;
	try {
		const assembled = await assembleLoadedLicenses(
			bodies,
			labelSets,
			errors,
			context,
