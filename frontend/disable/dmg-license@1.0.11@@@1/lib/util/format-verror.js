...
class PrettyVError extends VError {
	static errorFromList(errors) {
		switch (errors.length) {
			...
			case 1:
				return errors[0];
			default:
				return new PrettyMultiError(errors);
		}
	}
	constructor(...params) {
		super(...params);
	...
	[util_1.inspect.custom](
		depth = util_1.inspect.defaultOptions.depth || 2,
...
class PrettyMultiError extends verror_1.MultiError {
	...
	[util_1.inspect.custom](
		...
		if (depth < 0) return options.stylize(this.toString(), "special");
		const errors = this.errors();
		switch (errors.length) {
			case 0:
				return `${this.toString()} (empty)`;
			case 1:
				return util_1.inspect(errors[0]);
			default:
				return `${errors.length} errors:\n${errors
					.map((error, errorIndex) => {
						const isLastError = errorIndex + 1 === errors.length;
						const formattedError = util_1.inspect(
							error,
