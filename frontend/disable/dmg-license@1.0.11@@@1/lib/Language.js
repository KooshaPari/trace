...
class NoSuchLanguageError extends Error {
	constructor(lang) {
		super(
			`No known languages found for specification ${Array.isArray(lang) ? `[${lang.join(", ")}]` : lang}.`,
		);
		this.lang = lang;
	}
}
...
class Language {
	static add(lang) {
		...
		for (const tag of lang.langTags) Language.byTag[tag.toLowerCase()] = lang;
	}
	static bySpec(lang, context) {
		...
		else throw new NoSuchLanguageError(lang);
	}
	toString() {
...
exports.default = Language;
{
	const langJSON = JSON.parse(
		FS.readFileSync(Path.resolve(__dirname, "..", "language-info.json"), {
			encoding: "utf8",
		}),
	);
	const labelsByName = {};
		...
		const entry = new (class extends Language {
			constructor() {
				...
				this.charset = (() => {
					...
					if (!charset) {
						charset = iconv_corefoundation_1.StringEncoding.byIANACharSetName(
							rawLang.charset,
						);
		...
		if (!filter || filter(object))
			for (const lang of Language.bySpec(object.lang)) {
				...
				if (seen.has(languageID)) {
					if (collisions) collisions.add(languageID);
				} else {
					seen.add(languageID);
					const mapped = map ? map(object, lang) : object;
					if (mapped !== undefined) result.set(lang.languageID, mapped);
				}
			}
	if (collisions && collisions.size) onCollisions(collisions);
	return result;
}
