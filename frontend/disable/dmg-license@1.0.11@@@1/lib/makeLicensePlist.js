...
function makeLicensePlist(licenses, context) {
	...
	// Remove empty keys.
	for (const prop of ["RTF ", "TEXT"]) if (!ret[prop].length) delete ret[prop];
	// Generate LPic.
	{
		const buf = smart_buffer_1.SmartBuffer.fromSize(
			4 + 6 * licenses.inOrder.length,
		);
		// LPic header
		...
		for (const [languageID, item] of licenses.byLanguageID.entries()) {
			...
			buf.writeInt16BE(
				Language_1.default.byID[languageID].doubleByteCharset ? 1 : 0,
			);
		}
		ret.LPic.push({
			Attributes: "0x0000",
