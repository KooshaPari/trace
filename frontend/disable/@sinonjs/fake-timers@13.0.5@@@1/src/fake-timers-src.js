...
function withGlobal(_global) {
	const maxTimeout = 2 ** 31 - 1; //see https://heycam.github.io/webidl/#abstract-opdef-converttoint
	...
	isPresent.queueMicrotask = Object.hasOwn(_global, "queueMicrotask");
	...
	function createIntl() {
		...
		ClockIntl.DateTimeFormat = (...args) => {
			...
			["format", "formatToParts"].forEach((method) => {
				formatter[method] = (date) =>
					realFormatter[method](date || ClockIntl.clock.now);
			});
	...
	function addTimer(clock, timer) {
		...
		if (Object.hasOwn(timer, "interval")) {
			timer.type = "Interval";
			timer.interval = timer.interval > maxTimeout ? 1 : timer.interval;
		}

		if (Object.hasOwn(timer, "animation")) {
			timer.type = "AnimationFrame";
			timer.animation = true;
		}

		if (Object.hasOwn(timer, "idleCallback")) {
	...
	function clearTimer(clock, timerId, ttype) {
		...
		if (Object.hasOwn(clock.timers, id)) {
	...
	function hijackMethod(target, method, clock) {
		clock[method].hadOwnProperty = Object.hasOwn(target, method);
	...
	function install(config) {
		...
		clock.uninstall = () => uninstall(clock, config);
