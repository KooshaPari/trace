...
const userHasOptedIn = (rootPackage) => {
	return (
		(rootPackage &&
			rootPackage.scarfSettings &&
			rootPackage.scarfSettings.enabled) ||
...
function allowTopLevel(rootPackage) {
	return (
		rootPackage &&
		rootPackage.scarfSettings &&
		rootPackage.scarfSettings.allowTopLevel
	);
}
...
function skipTraversal(rootPackage) {
	return (
		rootPackage &&
		rootPackage.scarfSettings &&
		rootPackage.scarfSettings.skipTraversal
	);
}
...
function parentIsRoot(dependencyToReport) {
	...
	return (
		parent &&
		rootPackage &&
		parent.name === rootPackage.name &&
		parent.version === rootPackage.version
	);
}
...
function redactSensitivePackageInfo(dependencyInfo) {
	if (dependencyInfo.grandparent && dependencyInfo.grandparent.name) {
		dependencyInfo.grandparent.nameHash = hashWithDefault(
			dependencyInfo.grandparent.name,
			privatePackageRewrite,
		);
		dependencyInfo.grandparent.versionHash = hashWithDefault(
			dependencyInfo.grandparent.version,
			privateVersionRewrite,
		);
	}

	if (dependencyInfo.rootPackage && dependencyInfo.rootPackage.name) {
		dependencyInfo.rootPackage.nameHash = hashWithDefault(
			dependencyInfo.rootPackage.name,
			privatePackageRewrite,
		);
		dependencyInfo.rootPackage.versionHash = hashWithDefault(
			dependencyInfo.rootPackage.version,
			privateVersionRewrite,
		);
	}
...
function isYarn() {
	...
	return ["yarn", "yarn.js", "yarnpkg", "yarn.cmd", "yarnpkg.cmd"].some(
		(packageManBinName) => execPath.endsWith(packageManBinName),
	);
}
...
function processDependencyTreeOutput(resolve, reject) {
	return (error, stdout, stderr) => {
		if (error && !stdout) {
			return reject(
				new Error(`Scarf received an error from npm -ls: ${error} | ${stderr}`),
			);
		}
		...
		try {
			...
			const depsToScarf = findScarfInFullDependencyTree(output).filter(
				(depChain) => depChain.length >= 2,
			);
			...
			dependencyInfo.forEach((d) => {
				d.parent.scarfSettings = Object.assign(
					makeDefaultSettings(),
					d.parent.scarfSettings || {},
				);
			});

			// Here, we find the dependency chain that corresponds to the scarf package we're currently in
			const dependencyToReport =
				dependencyInfo.find(
					(dep) => dep.scarf.path === module.exports.dirName(),
				) || dependencyInfo[0];
			if (!dependencyToReport) {
				return reject(
					new Error(
						`Couldn't find dependency info for path ${module.exports.dirName()}`,
					),
				);
			}
			...
			if (
				dependencyToReport.anyInChainDisabled &&
				!userHasOptedIn(dependencyToReport.rootPackage)
			) {
				return reject(
					new Error(
						"Scarf has been disabled via a package.json in the dependency chain.",
					),
				);
			}

			if (
				isTopLevel(dependencyToReport) &&
				!isGlobal(dependencyToReport) &&
				!allowTopLevel(rootPackageDetails)
			) {
				return reject(
					new Error(
						"The package depending on Scarf is the root package being installed, but Scarf is not configured to run in this case. To enable it, set `scarfSettings.allowTopLevel = true` in your package.json",
					),
				);
			}
...
function processGitRevParseOutput(resolve, reject) {
	return (error, stdout, stderr) => {
		if (error && !stdout) {
			return reject(
				new Error(
					`Scarf received an error from git rev-parse: ${error} | ${stderr}`,
				),
			);
		}
		...
		if (output.length > 0) {
			...
			return reject(
				new Error("Scarf did not receive usable output from git rev-parse"),
			);
		}
	};
}
...
async function getDependencyInfo(packageJSONOverride) {
	try {
		const rootPackageJSON = require(
			packageJSONOverride || path.join(rootPath, "package.json"),
		);
	...
	return new Promise((resolve, reject) => {
		exec(
			`cd ${rootPath} && npm ls @scarf/scarf --json --long`,
			{ timeout: execTimeout, maxBuffer: 1024 * 1024 * 1024 },
			processDependencyTreeOutput(resolve, reject),
		);
	});
}
...
async function getGitShaFromRootPath() {
	const promise = new Promise((resolve, reject) => {
		exec(
			`cd ${rootPath} && git rev-parse HEAD`,
			{ timeout: execTimeout, maxBuffer: 1024 * 1024 * 1024 },
			processGitRevParseOutput(resolve, reject),
		);
	});
	try {
...
async function reportPostInstall() {
	...
	if (
		parentIsRoot(dependencyInfo) &&
		allowTopLevel(dependencyInfo.rootPackage)
	) {
	...
	if (!userHasOptedIn(rootPackage) && isYarn()) {
		return Promise.reject(
			new Error(
				"Package manager is yarn. scarf-js is unable to inform user of analytics. Aborting.",
			),
		);
	}
  ...
  `,
			...
			if (!userHasOptedIn(rootPackage)) {
				if (!userHasOptedOut(rootPackage)) {
					...
					if (
						hasHitRateLimit(optedOutLogRateLimitKey, getRateLimitedLogHistory())
					) {
						return reject(
							new Error(
								"Analytics are opt-out by default, but rate limit already hit for prompting opt-in.",
							),
						);
					}
					rateLimitedUserLog(
						optedOutLogRateLimitKey,
						`
  ...
  `,
					...
					process.stdout.write(
						`Would you like to support ${dependencyInfo.parent.name} by sending analytics for this install? (y/N): `,
					);
					...
					stdin.on("data", async (data) => {
						...
						const afterUserInput = (enabled, saved) => {
							...
							if (!saved) {
								console.log(
									"To prevent this message in the future, you can also set the `SCARF_ANALYTICS=true|false` environment variable",
								);
							}
						...
						process.stdout.write(
							"Save this preference to your project's package.json file? (y/N): ",
						);
						...
						stdin.on("data", async (data) => {
							try {
								...
								if (savePreference) {
									await savePreferencesToRootPackage(
										dependencyInfo.rootPackage.packageJsonPath,
										enabled,
									);
								}
...
function findScarfInSubDepTree(pathToDep, deps) {
	...
	if (scarfFound) {
		output.push(
			pathToDep.concat([
				{
					name: scarfLibName,
					version: deps[scarfLibName].version,
					path: deps[scarfLibName].path,
				},
			]),
		);
	}
	for (let i = 0; i < depNames.length; i++) {
		...
		const results = findScarfInSubDepTree(
			newPathToDep,
			deps[depName].dependencies,
		);
		if (results) {
...
function findScarfInFullDependencyTree(tree) {
	if (tree.name === scarfLibName) {
		...
		return findScarfInSubDepTree(
			[packageDetailsFromDepInfo(tree)],
			tree.dependencies,
		);
	}
}
