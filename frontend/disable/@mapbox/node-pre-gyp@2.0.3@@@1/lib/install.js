...
function place_binary_authenticated(opts, targetDir, callback) {
	...
	if (!process.env.AWS_ACCESS_KEY_ID || !process.env.AWS_SECRET_ACCESS_KEY) {
		const err = new Error(
			"Binary is private but AWS credentials not found. Please configure AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY environment variables, or use --fallback-to-build to compile from source.",
		);
		err.statusCode = 403;
		return callback(err);
	}
	...
	try {
		...
		if (e.code === "MODULE_NOT_FOUND" && e.message.includes("aws-sdk")) {
			const err = new Error(
				"Binary is private and requires aws-sdk for authenticated download. Please run: npm install aws-sdk",
			);
			err.statusCode = 403;
			return callback(err);
		}
		...
		.then((res) => {
			if (!res.ok) {
				...
				if (res.status === 403) {
					log.info(
						"install",
						"Received 403 Forbidden - attempting authenticated download",
					);
