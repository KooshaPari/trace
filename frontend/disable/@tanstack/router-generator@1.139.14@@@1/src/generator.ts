...
type GeneratorRouteNodeCache = Map<
	/** filePath **/ string,
	RouteNodeCacheEntry
>;
...
// Additionally, you should also exclude this file from your linter and/or formatter to prevent it from being checked or modified.`,
	...
	private static handleNode(
		...
		resetRegex(Generator.routeGroupPatternRegex);
		...
		node.isNonPath =
			...
			split.every((part) => Generator.routeGroupPatternRegex.test(part));
		...
		if (!node.isVirtual && node.isVirtualParentRequired) {
			...
			if (!anchorRoute) {
				...
				Generator.handleNode(parentNode, acc, config);
			} else {
