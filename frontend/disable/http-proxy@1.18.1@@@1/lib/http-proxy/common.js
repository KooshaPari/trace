...
common.setupOutgoing = (outgoing, options, req, forward) => {
	outgoing.port =
...
common.hasEncryptedConnection = (req) =>
	Boolean(req.connection.encrypted || req.connection.pair);
...
common.rewriteCookieProperty = function rewriteCookieProperty(
	...
	if (Array.isArray(header)) {
		return header.map((headerElement) =>
			rewriteCookieProperty(headerElement, config, property),
		);
	}
