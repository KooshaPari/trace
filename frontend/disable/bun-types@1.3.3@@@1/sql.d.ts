...
declare module "bun" {
	...
	namespace SQL {
		...
		type ContextCallbackResult<T> =
			T extends Array<PromiseLike<any>> ? AwaitPromisesArray<T> : Awaited<T>;
	...
	interface SQL extends AsyncDisposable {
		...
		<T extends { [Key in PropertyKey]: unknown }>(
			obj: T | T[] | readonly T[],
		): SQL.Helper<T>; // Contributor note: This is the same as the signature below with the exception of the columns and the Pick<T, Keys>
