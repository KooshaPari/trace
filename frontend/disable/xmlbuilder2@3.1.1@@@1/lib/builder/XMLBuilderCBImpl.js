...
var XMLBuilderCBImpl = /** @class */ (function (_super) {
	...
	function XMLBuilderCBImpl(options, fragment) {
		...
		_this._options = util_1.applyDefaults(
			options || {},
			interfaces_1.DefaultXMLBuilderCBOptions,
		);
	...
	XMLBuilderCBImpl.prototype.ele = function (p1, p2, p3) {
		...
		if (
			...
			try {
				for (
					var _b = __values(frag.node.childNodes), _c = _b.next();
					!_c.done;
					_c = _b.next()
				) {
					var node = _c.value;
		...
		if (
			!this._fragment &&
			this._hasDocumentElement &&
			this._writer.level === 0
		) {
			this.emit(
				"error",
				new Error("Document cannot have multiple document element nodes."),
			);
			return this;
		}
		try {
		...
		if (
			...
			this.emit(
				"error",
				new Error(
					"Document element name does not match DocType declaration name.",
				),
			);
			return this;
		}
	...
	XMLBuilderCBImpl.prototype.att = function (p1, p2, p3) {
		if (this._currentElement === undefined) {
			this.emit(
				"error",
				new Error(
					"Cannot insert an attribute node as child of a document node.",
				),
			);
			return this;
		}
	...
	XMLBuilderCBImpl.prototype.txt = function (content) {
		if (!this._fragment && this._currentElement === undefined) {
			this.emit(
				"error",
				new Error("Cannot insert a text node as child of a document node."),
			);
			return this;
		}
		this._serializeOpenTag(true);
	...
	XMLBuilderCBImpl.prototype.dec = function (options) {
		...
		if (this._fragment) {
			this.emit(
				"error",
				Error("Cannot insert an XML declaration into a document fragment."),
			);
			return this;
		}
	...
	XMLBuilderCBImpl.prototype.dtd = function (options) {
		if (this._fragment) {
			this.emit(
				"error",
				Error("Cannot insert a DocType declaration into a document fragment."),
			);
			return this;
		}
		...
		if (this._hasDocumentElement) {
			this.emit(
				"error",
				new Error("Cannot insert DocType declaration after document element."),
			);
			return this;
		}
		var node;
		...
		if (
			this._options.wellFormed &&
			!algorithm_1.xml_isPubidChar(node.publicId)
		) {
			this.emit(
				"error",
				new Error(
		...
		this._push(
			this._writer.docType(options.name, node.publicId, node.systemId),
		);
		return this;
	};
	/** @inheritdoc */
	...
	XMLBuilderCBImpl.prototype._serializeOpenTag = function (hasChildren) {
		...
		var inheritedNS =
			this._openTags.length === 0
				? null
				: this._openTags[this._openTags.length - 1][1];
	...
	XMLBuilderCBImpl.prototype._fromNode = function (node) {
		...
		if (util_2.Guard.isElementNode(node)) {
			...
			try {
				for (
					var _c = __values(node.attributes), _d = _c.next();
					!_d.done;
					_d = _c.next()
				) {
					var attr = _d.value;
					var name_1 = attr.prefix
			...
			try {
				for (
					var _e = __values(node.childNodes), _f = _e.next();
					!_f.done;
					_f = _e.next()
				) {
					var child = _f.value;
	 ...
	 */
	XMLBuilderCBImpl.prototype._serializeAttributes = function (
		node,
		map,
		prefixIndex,
		localPrefixesMap,
		...
		try {
			for (
				...
				this._push(
					this._writer.attribute(
						(candidatePrefix !== null ? candidatePrefix + ":" : "") +
							attr.localName,
	 ...
	 * @param requireWellFormed - whether to check conformance
	 */
	XMLBuilderCBImpl.prototype._serializeAttributeValue = function (
		value,
		requireWellFormed,
	) {
		if (
	 ...
	 * @param localPrefixesMap - local prefixes map
	 */
	XMLBuilderCBImpl.prototype._recordNamespaceInformation = function (
		node,
		map,
		localPrefixesMap,
	) {
	 ...
	 * @param prefixIndex - generated namespace prefix index
	 */
	XMLBuilderCBImpl.prototype._generatePrefix = function (
		newNamespace,
		prefixMap,
		prefixIndex,
	) {
	 ...
	 */
	XMLBuilderCBImpl.prototype._isPrefixMapModified = function (
		originalMap,
		newMap,
	) {
