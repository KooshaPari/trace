...
var XMLBuilderImpl = /** @class */ (function () {
	...
	XMLBuilderImpl.prototype.set = function (options) {
		this._options = util_1.applyDefaults(
			util_1.applyDefaults(this._options, options, true), // apply user settings
	...
	XMLBuilderImpl.prototype.ele = function (p1, p2, p3) {
		...
		if ((p1 === null || util_1.isString(p1)) && util_1.isString(p2)) {
			...
			(_a = __read([p1, p2, p3], 3)),
				(namespace = _a[0]),
				(name = _a[1]),
				(attributes = _a[2]);
		} else if (p1 !== null) {
			// ele(name: string, attributes?: AttributesObject)
			(_b = __read([undefined, p1, util_1.isObject(p2) ? p2 : undefined], 3)),
				(namespace = _b[0]),
				(name = _b[1]),
				(attributes = _b[2]);
		} else {
		...
		(_c = __read(
			this._extractNamespace(
				dom_1.sanitizeInput(namespace, this._options.invalidCharReplacement),
				dom_1.sanitizeInput(name, this._options.invalidCharReplacement),
				true,
			),
			2,
		)),
		...
		if (namespace === undefined) {
			var _d = __read(algorithm_1.namespace_extractQName(name), 1),
				prefix = _d[0];
		...
		if (childNode === this._doc.documentElement && oldDocType !== null) {
			var docType = this._doc.implementation.createDocumentType(
	...
	XMLBuilderImpl.prototype.att = function (p1, p2, p3) {
		...
		var value;
		if (
			(p1 === null || util_1.isString(p1)) &&
			util_1.isString(p2) &&
			(p3 === null || util_1.isString(p3))
		) {
			...
			(_b = __read([undefined, p1, p2], 3)),
				(namespace = _b[0]),
				(name = _b[1]),
				(value = _b[2]);
		} else {
			throw new Error(
				"Attribute name and value not specified. " + this._debugInfo(),
			);
		}
		...
		var ele = this.node;
		(_c = __read(this._extractNamespace(namespace, name, false), 2)),
			(namespace = _c[0]),
			(name = _c[1]);
		name = dom_1.sanitizeInput(name, this._options.invalidCharReplacement);
		namespace = dom_1.sanitizeInput(
			namespace,
			this._options.invalidCharReplacement,
		);
		value = dom_1.sanitizeInput(value, this._options.invalidCharReplacement);
		var _d = __read(algorithm_1.namespace_extractQName(name), 2),
			prefix = _d[0],
			localName = _d[1];
		var _e = __read(
				algorithm_1.namespace_extractQName(
					ele.prefix ? ele.prefix + ":" + ele.localName : ele.localName,
				),
				1,
			),
			elePrefix = _e[0];
	...
	XMLBuilderImpl.prototype.removeAtt = function (p1, p2) {
		...
		if (p1 !== null && p2 === undefined) {
			...
			throw new Error(
				"Attribute namespace must be a string. " + this._debugInfo(),
			);
		}
			...
			util_1.forEachArray(
				...
				function (attName) {
					return namespace === undefined
						? _this.removeAtt(attName)
						: _this.removeAtt(namespace, attName);
				},
				this,
			);
	...
	XMLBuilderImpl.prototype.txt = function (content) {
		...
		var child = this._doc.createTextNode(
			dom_1.sanitizeInput(content, this._options.invalidCharReplacement),
		);
	...
	XMLBuilderImpl.prototype.com = function (content) {
		...
		var child = this._doc.createComment(
			dom_1.sanitizeInput(content, this._options.invalidCharReplacement),
		);
		this.node.appendChild(child);
		return this;
	...
	XMLBuilderImpl.prototype.dat = function (content) {
		...
		var child = this._doc.createCDATASection(
			dom_1.sanitizeInput(content, this._options.invalidCharReplacement),
		);
		this.node.appendChild(child);
		return this;
	...
	XMLBuilderImpl.prototype.ins = function (target, content) {
		...
		if (util_1.isArray(target) || util_1.isSet(target)) {
			...
			util_1.forEachObject(
				target,
				function (insTarget, insValue) {
					return _this.ins(insTarget, insValue);
				},
				this,
			);
		} else {
			var child = this._doc.createProcessingInstruction(
				dom_1.sanitizeInput(target, this._options.invalidCharReplacement),
				dom_1.sanitizeInput(content, this._options.invalidCharReplacement),
			);
			this.node.appendChild(child);
		}
	...
	XMLBuilderImpl.prototype.dtd = function (options) {
		...
		var pubID = dom_1.sanitizeInput(
			(options && options.pubID) || "",
			this._options.invalidCharReplacement,
		);
		var sysID = dom_1.sanitizeInput(
			(options && options.sysID) || "",
			this._options.invalidCharReplacement,
		);
		// name must match document element
		if (
			this._doc.documentElement !== null &&
			name !== this._doc.documentElement.tagName
		) {
	...
	XMLBuilderImpl.prototype.import = function (node) {
		...
		if (util_2.Guard.isDocumentNode(importedNode)) {
			...
			if (elementNode === null) {
				throw new Error(
					"Imported document has no document element node. " +
						this._debugInfo(),
				);
			}
			...
			try {
				// import child nodes
				for (
					var _c = __values(importedNode.childNodes), _d = _c.next();
					!_d.done;
					_d = _c.next()
				) {
	...
	XMLBuilderImpl.prototype.doc = function () {
		if (this._doc._isFragment) {
			...
			if (node === null) {
				throw new Error(
					"Node has no parent node while searching for document fragment ancestor. " +
						this._debugInfo(),
				);
			}
	...
	/** @inheritdoc */
	XMLBuilderImpl.prototype.each = function (
		callback,
		self,
		recursive,
		thisArg,
		...
		while (result[0]) {
			var nextResult = this._getNextDescendantNode(
				this._domNode,
				result[0],
				recursive,
				result[1],
	...
	/** @inheritdoc */
	XMLBuilderImpl.prototype.reduce = function (
		callback,
		initialValue,
		self,
		recursive,
	...
	/** @inheritdoc */
	XMLBuilderImpl.prototype.find = function (
		predicate,
		self,
		recursive,
		thisArg,
		...
		while (result[0]) {
			...
			result = this._getNextDescendantNode(
				this._domNode,
				result[0],
				recursive,
				result[1],
	...
	/** @inheritdoc */
	XMLBuilderImpl.prototype.filter = function (
		predicate,
		self,
		recursive,
		thisArg,
	...
	/** @inheritdoc */
	XMLBuilderImpl.prototype.some = function (
		predicate,
		self,
		recursive,
		thisArg,
	 ...
	 */
	XMLBuilderImpl.prototype._getFirstDescendantNode = function (
		root,
		self,
		recursive,
	) {
		if (self) return [this._domNode, 0, 0];
		else if (recursive)
			return this._getNextDescendantNode(root, root, recursive, 0, 0);
		else return [this._domNode.firstChild, 0, 1];
	};
	/**
	 * Gets the next descendant of the given node of the tree rooted at `root`
	 * in depth-first pre-order. Returns a three-tuple with
	 ...
	 */
	XMLBuilderImpl.prototype._getNextDescendantNode = function (
		root,
		node,
		recursive,
		index,
		...
		if (recursive) {
			...
			if (root === node) return [node.firstChild, 0, level + 1];
			else return [node.nextSibling, index + 1, level];
		}
		return [null, -1, -1];
	};
	/**
	...
	XMLBuilderImpl.prototype._extractNamespace = function (namespace, name, ele) {
		...
		if (namespace === undefined) {
			...
			namespace = ele
				? this._options.defaultNamespace.ele
				: this._options.defaultNamespace.att;
	...
	XMLBuilderImpl.prototype._updateNamespace = function (ns) {
		...
		if (
			util_2.Guard.isElementNode(ele) &&
			ns !== null &&
			ele.namespaceURI !== ns
		) {
			var _c = __read(
			...
			// re-create the element node if its namespace changed
			// we can't simply change the namespaceURI since its read-only
			var newEle = algorithm_1.create_element(
				this._doc,
				eleLocalName,
				ns,
				elePrefix,
			...
			try {
				for (
					var _d = __values(ele.attributes), _e = _d.next();
					!_e.done;
					_e = _d.next()
				) {
					var attr = _e.value;
					var attrQName = attr.prefix
						? attr.prefix + ":" + attr.localName
						: attr.localName;
					var _f = __read(algorithm_1.namespace_extractQName(attrQName), 1),
						attrPrefix = _f[0];
			...
			try {
				// check child nodes
				for (
					var _g = __values(ele.childNodes), _h = _g.next();
					!_h.done;
					_h = _g.next()
				) {
					...
					if (util_2.Guard.isElementNode(newChildNode)) {
						var _j = __read(
								algorithm_1.namespace_extractQName(
									newChildNode.prefix
										? newChildNode.prefix + ":" + newChildNode.localName
										: newChildNode.localName,
								),
								1,
							),
							newChildNodePrefix = _j[0];
