...
var BaseReader = /** @class */ (function () {
	...
	BaseReader.prototype._element = function (parent, namespace, name) {
		return namespace === undefined
			? parent.ele(name)
			: parent.ele(namespace, name);
	};
	BaseReader.prototype._attribute = function (parent, namespace, name, value) {
		return namespace === undefined
			? parent.att(name, value)
			: parent.att(namespace, name, value);
	};
	BaseReader.prototype._sanitize = function (str) {
		return dom_1.sanitizeInput(
			str,
			this._builderOptions.invalidCharReplacement,
		);
	};
	/**
	...
	BaseReader.prototype._decodeText = function (text) {
		...
		return text
			.replace(/&(quot|amp|apos|lt|gt);/g, function (_match, tag) {
				return BaseReader._entityTable[tag];
			})
			.replace(
				/&#(?:x([a-fA-F0-9]+)|([0-9]+));/g,
				function (_match, hexStr, numStr) {
					return String.fromCodePoint(
						parseInt(hexStr || numStr, hexStr ? 16 : 10),
					);
				},
			);
	};
