			...
			switch (token.type) {
				case interfaces_1.TokenType.Declaration:
					...
					if (declaration.standalone) {
						builderOptions.standalone =
							this.sanitize(declaration.standalone) === "yes";
					}
				...
				case interfaces_1.TokenType.DocType:
					...
					context =
						this.docType(
							context,
							this.sanitize(doctype.name),
							this.sanitize(doctype.pubId),
							this.sanitize(doctype.sysId),
						) || context;
					break;
				...
				case interfaces_1.TokenType.Comment:
					var comment = token;
					context =
						this.comment(context, this.sanitize(comment.data)) || context;
					break;
				case interfaces_1.TokenType.PI:
					...
					context =
						this.instruction(
							context,
							this.sanitize(pi.target),
							this.sanitize(pi.data),
						) || context;
					break;
				case interfaces_1.TokenType.Text:
					...
					var text = token;
					context =
						this.text(context, this._decodeText(this.sanitize(text.data))) ||
						context;
					break;
				case interfaces_1.TokenType.Element:
					...
					var _c = __read(algorithm_1.namespace_extractQName(elementName), 1),
						prefix = _c[0];
					...
					try {
						...
						for (
							...
							if (
								...
								if (
									attNamespace !== null &&
									elementNode.node.isDefaultNamespace(attNamespace)
								) {
									attNamespace = null;
