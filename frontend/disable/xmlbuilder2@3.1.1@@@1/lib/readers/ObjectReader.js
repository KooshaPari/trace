...
var ObjectReader = /** @class */ (function (_super) {
	...
	ObjectReader.prototype._parse = function (node, obj) {
		...
		if (util_1.isFunction(obj)) {
				...
				function (key, val) {
					...
					if (
						...
						if (key === options.convert.att) {
								...
								util_1.forEachObject(val, function (attrKey, attrVal) {
									lastChild =
										_this.attribute(
											node,
											undefined,
											_this.sanitize(attrKey),
											_this._decodeAttributeValue(_this.sanitize(attrVal)),
										) || lastChild;
								});
							}
						} else {
							lastChild =
								_this.attribute(
									node,
									undefined,
									_this.sanitize(key.substr(options.convert.att.length)),
									_this._decodeAttributeValue(_this.sanitize(val)),
								) || lastChild;
						}
					} else if (
						!options.ignoreConverters &&
						...
						if (util_1.isMap(val) || util_1.isObject(val)) {
							...
							lastChild =
								_this.text(node, _this._decodeText(_this.sanitize(val))) ||
								lastChild;
						}
					} else if (
						!options.ignoreConverters &&
						...
						if (util_1.isArray(val) || util_1.isSet(val)) {
							util_1.forEachArray(
								...
								function (item) {
									return (lastChild =
										_this.cdata(node, _this.sanitize(item)) || lastChild);
								},
								_this,
							);
						} else {
						...
						if (util_1.isArray(val) || util_1.isSet(val)) {
							util_1.forEachArray(
								...
								function (item) {
									return (lastChild =
										_this.comment(node, _this.sanitize(item)) || lastChild);
								},
								_this,
							);
						} else {
						...
						if (util_1.isString(val)) {
							...
							lastChild =
								_this.instruction(
									node,
									_this.sanitize(insTarget),
									_this.sanitize(insValue),
								) || lastChild;
						...
						(util_1.isArray(val) || util_1.isSet(val)) &&
						util_1.isEmpty(val)
					) {
						// skip empty arrays
					} else if (
						(util_1.isMap(val) || util_1.isObject(val)) &&
						util_1.isEmpty(val)
					) {
						// empty objects produce one node
						lastChild =
							_this.element(node, undefined, _this.sanitize(key)) || lastChild;
			...
			// text node
			lastChild =
				this.text(node, this._decodeText(this.sanitize(obj))) || lastChild;
		}
