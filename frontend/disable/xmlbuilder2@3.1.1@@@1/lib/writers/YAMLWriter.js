...
var YAMLWriter = /** @class */ (function (_super) {
	...
	function YAMLWriter(builderOptions, writerOptions) {
		...
		if (_this._writerOptions.indent.length < 2) {
			throw new Error(
				"YAML indententation string must be at least two characters long.",
			);
		}
	...
	YAMLWriter.prototype.serialize = function (node) {
		...
		var objectWriter = new ObjectWriter_1.ObjectWriter(
			this._builderOptions,
			objectWriterOptions,
		);
		var val = objectWriter.serialize(node);
		var markup =
			this._beginLine(this._writerOptions, 0) +
			"---" +
			this._endLine(this._writerOptions) +
			this._convertObject(val, this._writerOptions, 0);
		// remove trailing newline
		/* istanbul ignore else */
		if (
			markup.slice(-this._writerOptions.newline.length) ===
	 ...
	 */
	YAMLWriter.prototype._convertObject = function (
		obj,
		options,
		level,
		suppressIndent,
		...
		if (util_1.isArray(obj)) {
			try {
				for (
					var obj_1 = __values(obj), obj_1_1 = obj_1.next();
					!obj_1_1.done;
					obj_1_1 = obj_1.next()
				) {
