...
var ObjectWriter = /** @class */ (function (_super) {
	...
	ObjectWriter.prototype._process = function (items, options) {
		...
		var defInstructionKey = this._getNodeKey(
			interfaces_1.NodeType.ProcessingInstruction,
		);
		var defCdataKey = this._getNodeKey(interfaces_1.NodeType.CData);
		if (
			textCount === 1 &&
			items.length === 1 &&
			util_1.isString(items[0]["#"])
		) {
					...
					default:
						...
						if (ele[key].length !== 0 && util_1.isArray(ele[key][0])) {
							...
							if (options.verbose) {
								result.push(
									((_f = {}),
									(_f[key] = [this._process(ele[key], options)]),
									_f),
								);
							} else {
								result.push(
									((_g = {}), (_g[key] = this._process(ele[key], options)), _g),
								);
							}
						}
						break;
			...
			for (var i = 0; i < items.length; i++) {
				...
				switch (key) {
					...
					case "#":
						...
						break;
					case "!":
						commentId = this._processSpecItem(
							item["!"],
							obj,
							options.group,
							defCommentKey,
						...
						break;
					case "?":
						instructionId = this._processSpecItem(
							item["?"],
							obj,
							options.group,
							defInstructionKey,
	...
	ObjectWriter.prototype._processSpecItem = function (
		item,
		obj,
		group,
		defKey,
		...
		if (!group && util_1.isArray(item) && count + item.length > 2) {
			try {
				for (
					var item_1 = __values(item), item_1_1 = item_1.next();
					!item_1_1.done;
				...
				try {
					if (item_1_1 && !item_1_1.done && (_a = item_1.return))
						_a.call(item_1);
				} finally {
