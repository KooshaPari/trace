...
describe("FdSlicer", () => {
	...
	it("writes 4 chunks simultaneously", (done) => {
		fs.open(testOutBlobFile, "w", (err, fd) => {
			if (err) return done(err);
			var slicer = fdSlicer.createFromFd(fd);
			var actualPart1 = slicer.createWriteStream({
				start: (testBlobFileSize * 0) / 4,
			});
			var actualPart2 = slicer.createWriteStream({
				start: (testBlobFileSize * 1) / 4,
			});
			var actualPart3 = slicer.createWriteStream({
				start: (testBlobFileSize * 2) / 4,
			});
			var actualPart4 = slicer.createWriteStream({
				start: (testBlobFileSize * 3) / 4,
			});
