...
function getOptionsWithDefaults(options, manifest) {
	const defaults = {
		...
		"auto-install-runtime":
			typeof options["runtime-flatpakref"] !== "undefined",
...
async function flatpakBuilderVersion() {
	if (!_flatpakBuilderVersion) {
		const versionString = _.last(
			(await exec("flatpak-builder --version")).toString().split(" "),
		);
		_flatpakBuilderVersion = versionString
			.split(".")
...
async function ensureWorkingDir(options) {
	if (!options["working-dir"]) {
		const dir = await tmp.dir({
			prefix: "flatpak-bundler",
			unsafeCleanup: options["clean-tmpdirs"],
		});
...
async function copyFiles(options, manifest) {
	...
	return Promise.all(
		options.files.map(async (sourceDest) => {
...
async function createSymlinks(options, manifest) {
	...
	return Promise.all(
		options.symlinks.map(async ([targetPath, location]) => {
...
async function copyExports(options, manifest) {
	...
	return Promise.all(
		options["extra-exports"].map(async (source) => {
...
exports.bundle = async (manifest, options) => {
