...
Object.defineProperty(CSSOM.CSSImportRule.prototype, "cssText", {
	get: function () {
		...
		return (
			'@import url("' +
			this.href.replace(/\\/g, "\\\\").replace(/"/g, '\\"') +
			'")' +
			(this.layerName !== null
				? " layer" + (this.layerName && "(" + this.layerName + ")")
				: "") +
			(this.supportsText ? " supports(" + this.supportsText + ")" : "") +
			(mediaText ? " " + mediaText : "") +
			";"
		);
	},
					...
					if (state === "media") {
						if (buffer) {
							...
							if (supportsResult) {
								...
								bufferTrimmed =
									bufferTrimmed.slice(0, 0) +
									bufferTrimmed.slice(supportsResult.endIndex + 1);
								bufferTrimmed = bufferTrimmed
									.replace(doubleOrMoreSpacesRegExp, " ")
									.trim();
							}
