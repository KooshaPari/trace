...
CSSOM.CSSStyleSheet.prototype.insertRule = function (rule, index) {
	...
	var parseErrors = [];
	var parsedSheet = CSSOM.parse(ruleToParse, undefined, (err) => {
	...
	if (cssRule.constructor.name === "CSSImportRule") {
		...
		var firstLayerIndex = findFirstNonConstructorIndex(this.cssRules, [
			"CSSImportRule",
		]);
		if (
			firstLayerIndex < this.cssRules.length &&
			this.cssRules[firstLayerIndex].constructor.name ===
		...
		var firstNonImportIndex = findFirstNonConstructorIndex(this.cssRules, [
			"CSSImportRule",
		]);
		if (
			index > firstNonImportIndex &&
			firstNonImportIndex < this.cssRules.length &&
			this.cssRules[firstNonImportIndex].constructor.name !==
				"CSSLayerStatementRule"
		) {
			errorUtils.throwError(
				this,
		...
		var firstNonImportNamespaceIndex = findFirstNonConstructorIndex(
			this.cssRules,
			["CSSLayerStatementRule", "CSSImportRule", "CSSNamespaceRule"],
		);
		...
		if (parseErrors.filter((error) => !error.isNested).length !== 0) {
			errorUtils.throwParseError(
				this,
				"insertRule",
				this.constructor.name,
...
CSSOM.CSSStyleSheet.prototype.deleteRule = function (index) {
	...
	if (this.cssRules[index]) {
		if (this.cssRules[index].constructor.name == "CSSNamespaceRule") {
			var shouldContinue = this.cssRules.every(
				(rule) =>
					[
						"CSSImportRule",
						"CSSLayerStatementRule",
			...
			if (!shouldContinue) {
				errorUtils.throwError(
					this,
					"DOMException",
					"Failed to execute 'deleteRule' on '" +
						this.constructor.name +
...
CSSOM.CSSStyleSheet.prototype.replace = function (text) {
	...
	return new _Promise((resolve, reject) => {
		...
		if (!this.__constructed || this.__disallowModification) {
			reject(
				errorUtils.createError(
					this,
					"DOMException",
					"Failed to execute 'replaceSync' on '" +
						this.constructor.name +
						"': Not allowed.",
					"NotAllowedError",
				),
		...
		this.__disallowModification = true;
		...
		setTimeout(() => {
			...
			CSSOM.parse(text, { styleSheet: this, cssRules: rules });
			...
			delete this.__disallowModification;
