...
Object.defineProperty(CSSOM.CSSKeyframesRule.prototype, "cssText", {
	get: function () {
		...
		var processedName = cssWideKeywords.includes(this.name)
			? '"' + this.name + '"'
			: this.name;
		return (
			"@" + (this._vendorPrefix || "") + "keyframes " + processedName + values
		);
	},
});
...
CSSOM.CSSKeyframesRule.prototype.appendRule = function appendRule(rule) {
	...
	try {
		...
		if (
			tempStyleSheet.cssRules.length > 0 &&
			tempStyleSheet.cssRules[0].cssRules.length > 0
		) {
 ...
 */
CSSOM.CSSKeyframesRule.prototype._normalizeKeyText = function _normalizeKeyText(
	keyText,
) {
	if (!keyText) return "";
