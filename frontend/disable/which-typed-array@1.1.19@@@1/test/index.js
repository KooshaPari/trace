...
test("not arrays", function (t) {
	...
	t.equal(
		false,
		whichTypedArray(new RegExp("a", "g")),
	...
	t.test("numbers", function (st) {
		...
		st.equal(
			false,
			whichTypedArray(Object(42)),
			"number object is not typed array",
		);
	...
	t.test("strings", function (st) {
		st.equal(
			false,
			whichTypedArray("foo"),
			"string primitive is not typed array",
		);
		st.equal(
			false,
			whichTypedArray(Object("foo")),
			"string object is not typed array",
		);
		st.end();
	});
...
test("Generators", { skip: generators.length === 0 }, function (t) {
	forEach(generators, function (genFn) {
		t.equal(
			false,
			whichTypedArray(genFn),
			"generator function " + genFn + " is not typed array",
		);
	});
	t.end();
});
...
test("Arrow functions", { skip: arrows.length === 0 }, function (t) {
	forEach(arrows, function (arrowFn) {
		t.equal(
			false,
			whichTypedArray(arrowFn),
			"arrow function " + arrowFn + " is not typed array",
		);
	});
	t.end();
});
...
test("@@toStringTag", { skip: !hasToStringTag }, function (t) {
	forEach(typedArrayNames, function (typedArray) {
		...
		if (typeof global[typedArray] === "function") {
			...
			t.equal(
				false,
				whichTypedArray(fakeTypedArray),
...
test("Typed Arrays", function (t) {
	forEach(typedArrayNames, function (typedArray) {
		// @ts-expect-error TODO: fix
		/** @type {import('../').TypedArrayConstructor} */ var TypedArray =
			global[typedArray];
		if (isCallable(TypedArray)) {
			...
			t.equal(
				whichTypedArray(arr),
				typedArray,
				"new " + typedArray + "(10) is typed array of type " + typedArray,
			);
		} else {
			t.comment("# SKIP " + typedArray + " is not supported");
		}
