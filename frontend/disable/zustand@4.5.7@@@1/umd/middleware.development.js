((global, factory) => {
	typeof exports === "object" && typeof module !== "undefined"
		...
		: typeof define === "function" && define.amd
			...
			: ((global =
				...
				factory((global.zustandMiddleware = {})));
})(this, (exports) => {
	function _extends() {
		return (
	...
	var getTrackedConnectionState = function getTrackedConnectionState(name) {
		...
		return Object.fromEntries(
			Object.entries(api.stores).map((_ref) => {
				var key = _ref[0],
					api = _ref[1];
	...
	var extractConnectionInformation = function extractConnectionInformation(
		store,
		extensionConnector,
		options,
	) {
	...
	var devtoolsImpl = function devtoolsImpl(fn, devtoolsOptions) {
		...
		return (set, get, api) => {
			...
			try {
				extensionConnector =
					(enabled != null ? enabled : "development" !== "production") &&
			...
			var _extractConnectionInf = extractConnectionInformation(
					store,
					extensionConnector,
					options,
				),
				connection = _extractConnectionInf.connection,
				connectionInformation = _objectWithoutPropertiesLoose(
			...
			if (connectionInformation.type === "untracked") {
				...
				connection == null ||
					connection.init(
						Object.fromEntries(
							Object.entries(connectionInformation.stores).map((_ref2) => {
								var key = _ref2[0],
									store = _ref2[1];
								return [
									key,
					...
					case "DISPATCH":
						switch (message.payload.type) {
							...
							case "IMPORT_STATE": {
								...
								var lastComputedState =
									(_nextLiftedState$comp =
										nextLiftedState.computedStates.slice(-1)[0]) == null
										? void 0
										: _nextLiftedState$comp.state;
	...
	var oldImpl = function oldImpl(config, baseOptions) {
		return (set, get, api) => {
			...
			var hydrate = function hydrate() {
				...
				var postRehydrationCallback =
					(options.onRehydrateStorage == null
						? void 0
						: options.onRehydrateStorage(get())) || undefined;
				return _toThenable(storage.getItem.bind(storage))(options.name)
					.then((storageValue) => {
						if (storageValue) {
					...
					.then((migratedState) => {
						...
						stateFromStorage = options.merge(
							migratedState,
							(_get = get()) != null ? _get : configResult,
						);
				...
				var postRehydrationCallback =
					(options.onRehydrateStorage == null
						...
						: options.onRehydrateStorage(
								(_get3 = get()) != null ? _get3 : configResult,
							)) || undefined;
				return _toThenable(storage.getItem.bind(storage))(options.name)
					.then((deserializedStorageValue) => {
						if (deserializedStorageValue) {
					...
					.then((migrationResult) => {
						...
						stateFromStorage = options.merge(
							migratedState,
							(_get4 = get()) != null ? _get4 : configResult,
						);
						set(stateFromStorage, true);
						if (migrated) {
							return setItem();
