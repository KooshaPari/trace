...
const toHaveText = async function (...args) {
	try {
		const [elementHandle, [expectedValue]] =
			await utils_1.getElementHandle(args);
		/* istanbul ignore next */
		const actualTextContent = await elementHandle.evaluate(
			(el) => el.textContent,
		);
		return {
			pass: !!(actualTextContent === null || actualTextContent === void 0
				? void 0
				: actualTextContent.includes(expectedValue)),
			message: () =>
				utils_1.getMessage(
					this,
					"toHaveText",
