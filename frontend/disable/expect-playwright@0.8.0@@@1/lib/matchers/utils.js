...
const getElementHandle = async (args, valueArgCount = 1) => {
	...
	const options =
		args.length > 1 && isObject(args[args.length - 1]) ? args.pop() : {};
	...
	handle =
		(_a = await exports.getFrame(handle)) !== null && _a !== void 0
			? _a
			: handle;
	if (isLocator(handle)) {
...
const compareText = (expectedValue, actualValue) => {
	return typeof expectedValue === "string"
		...
		: expectedValue.test(
				actualValue !== null && actualValue !== void 0 ? actualValue : "",
			);
};
