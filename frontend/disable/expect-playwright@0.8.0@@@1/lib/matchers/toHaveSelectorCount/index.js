...
const toHaveSelectorCount = async function (
	arg,
	selector,
	expectedValue,
	options = {},
	...
	try {
		const [elementHandle] = await utils_1.getElementHandle(
			[arg, selector, options],
			1,
		);
		...
		return {
			pass: false,
			message: () =>
				`${utils_1.quote(selector)} could not be found on the page.`,
		};
	}
};
