	...
	options.parents.forEach((parent) => {
		...
		if (typeof parent._defaults !== "undefined") {
			for (var defaultKey in parent._defaults) {
				if (Object.hasOwn(parent._defaults, defaultKey)) {
...
ArgumentParser.prototype._getOptionalActions = function () {
	return this._actions.filter((action) => action.isOptional());
};

ArgumentParser.prototype._getPositionalActions = function () {
	return this._actions.filter((action) => action.isPositional());
};
...
ArgumentParser.prototype._parseKnownArgs = function (argStrings, namespace) {
	...
	this._mutuallyExclusiveGroups.forEach((mutexGroup) => {
		mutexGroup._groupActions.forEach((mutexAction, i, groupActions) => {
	...
	function takeAction(action, argumentStrings, optionString) {
		...
		if (argumentValues !== action.defaultValue) {
			...
			if (actionConflicts[actionHash(action)]) {
				actionConflicts[actionHash(action)].forEach((actionConflict) => {
	...
	Object.keys(optionStringIndices).forEach((position) => {
	...
	while (startIndex <= maxOptionStringIndex) {
		...
		for (position in optionStringIndices) {
			if (!Object.hasOwn(optionStringIndices, position)) {
				continue;
			}
	...
	self._mutuallyExclusiveGroups.forEach((group) => {
		if (group.required) {
			actionUsed = group._groupActions.some(
				(action) => seenNonDefaultActions.indexOf(action) !== -1,
			);

			// if no actions were used, report the error
			if (!actionUsed) {
				var names = [];
				group._groupActions.forEach((action) => {
		...
		if (this.fromfilePrefixChars.indexOf(argString[0]) < 0) {
			...
			try {
				...
				content.forEach((argLine) => {
					this.convertArgLineToArgs(argLine).forEach((arg) => {
						argstrs.push(arg);
					});
...
ArgumentParser.prototype.convertArgLineToArgs = (argLine) => [argLine];
...
ArgumentParser.prototype._parseOptional = function (argString) {
	...
	if (optionTuples.length > 1) {
		var optionStrings = optionTuples.map((optionTuple) => optionTuple[1]);
		this.error(
			format(
...
ArgumentParser.prototype._getNargsPattern = (action) => {
...
ArgumentParser.prototype._getValues = function (action, argStrings) {
	...
	if (action.nargs !== c.PARSER && action.nargs !== c.REMAINDER) {
		argStrings = argStrings.filter((arrayElement) => arrayElement !== "--");
	}
	...
	if (argStrings.length === 0 && action.nargs === c.OPTIONAL) {
		...
		value = argStrings.map((v) => this._getValue(action, v));
...
ArgumentParser.prototype._checkValue = (action, value) => {
...
ArgumentParser.prototype.formatHelp = function () {
	...
	this._actionGroups.forEach((actionGroup) => {
...
ArgumentParser.prototype._printMessage = (message, stream) => {
	if (!stream) {
