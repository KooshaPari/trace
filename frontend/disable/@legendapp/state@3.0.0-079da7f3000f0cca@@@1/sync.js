...
function removeNullUndefined(a, recursive) {
	...
	Object.keys(a).forEach((key) => {
		if (a[key] !== null && a[key] !== void 0) {
			out[key] =
				recursive && state.isObject(a[key])
					? removeNullUndefined(a[key])
					: a[key];
		}
	});
	return out;
...
function deepEqual(a, b, ignoreFields, nullVsUndefined) {
	if (a === b) return true;
	if (state.isNullOrUndefined(a) !== state.isNullOrUndefined(b)) return false;
	if (!state.isObject(a) || !state.isObject(b)) return a === b;
	if (nullVsUndefined) {
	...
	const keysA = Object.keys(a).filter(
		(key) => !(ignoreFields == null ? void 0 : ignoreFields.includes(key)),
	);
...
function calculateRetryDelay(retryOptions, retryNum) {
	...
	if (infinite || retryNum < times) {
		const delayTime = Math.min(
			delay * (backoff === "constant" ? 1 : 2 ** retryNum),
			maxDelay,
		);
		return delayTime;
	}
				...
				const run = () => {
					value
						...
						.catch((error) => {
							...
							const timeout = createRetryTimeout(
								retryOptions,
								state.retryNum,
								() => {
									value = fn(state);
...
async function waitForSet(waitForSet2, changes, value, params = {}) {
	const waitFn = state.isFunction(waitForSet2)
		? waitForSet2({ changes, value, ...params })
		: waitForSet2;
	if (waitFn) {
...
async function updateMetadataImmediate(
	value$,
	localState,
	syncState$,
	syncOptions,
...
function mergeQueuedChanges(allChanges) {
	...
	for (let i = 0; i < allChanges.length; i++) {
		...
		const changesMap = inRemoteChange
			? changesByOptionsRemote
			: changesByOptionsLocal;
...
async function processQueuedChanges() {
	...
	const preppedChangesLocal = await Promise.all(
		queuedChanges.map(prepChangeLocal),
	);
	...
	for (const options of pendingSyncOptions) {
		const timeout =
			(_b = options.debounceSet) != null
				? _b
				: (_a = observableSyncConfiguration) == null
					? void 0
					: _a.debounceSet;
...
async function processQueuedRemoteChanges(syncOptions) {
	...
	if (arr == null ? void 0 : arr.length) {
		...
		const preppedChangesRemote = await Promise.all(
			queuedRemoteChanges.map(prepChangeRemote),
		);
...
async function prepChangeLocal(queuedChange) {
	...
	const saveLocal =
		(persist == null ? void 0 : persist.name) &&
		!configLocal.readonly &&
		!isApplyingPending &&
		syncState2.isPersistEnabled.peek();
	const saveRemote = !!(
		!inRemoteChange &&
		(syncOptions == null ? void 0 : syncOptions.set) &&
		syncState2.isSyncEnabled.peek()
	);
		...
		for (let i = changes.length - 1; i >= 0; i--) {
			...
			if (changesPaths.size > 0) {
				for (let u = 0; u < path.length; u++) {
					if (
						changesPaths.has(
							(u === path.length - 1 ? path : path.slice(0, u + 1)).join("/"),
						)
					) {
						found = true;
						break;
			...
			if (!found) {
				...
				if (saveLocal) {
					...
					promisesTransform.push(
						doInOrder(
							promiseTransformLocal,
							({ value: valueTransformed, path: pathTransformed }) => {
								changesLocal.push({
					...
					promisesTransform.push(
						doInOrder(
							...
							({ value: valueTransformed, path: pathTransformed }) => {
								...
								for (
									let i2 = 0;
									!found2 && i2 < pathTransformed.length - 1;
									i2++
								) {
									const pathParent = pathTransformed.slice(0, i2 + 1).join("/");
									if (
										(_a = localState.pendingChanges[pathParent]) == null
											? void 0
											: _a.v
									) {
										found2 = true;
								...
								if (!found2) {
									...
									if (!localState.pendingChanges[pathStr]) {
										localState.pendingChanges[pathStr] = {
											p: prevAtPath != null ? prevAtPath : null,
											t: pathTypes,
										};
									}
...
async function doChangeRemote(changeInfo) {
	...
	if (changesRemote.length > 0) {
		...
		const transformSave =
			(_a = syncOptions == null ? void 0 : syncOptions.transform) == null
				? void 0
				: _a.save;
		if (transformSave) {
		...
		if (!beforeSetParams.cancel) {
			...
			const onSetError = (error, params, noThrow) => {
				...
				if (lastErrorHandled !== error) {
					...
					(_a2 = syncOptions.onError) == null
						? void 0
						: _a2.call(syncOptions, error, params);
			...
			if (!didError || (updateResult == null ? void 0 : updateResult.changes)) {
				const { value: updateValue, changes: updateChanges = changesRemote } =
					updateResult || {};
				...
				if (pathStrs.length > 0) {
					...
					if (saveLocal) {
						const pendingMetadata =
							(_b = pluginPersist.getMetadata(table, configLocal)) == null
								? void 0
								: _b.pending;
						const pending = localState.pendingChanges;
					...
					if (transformedChanges !== void 0) {
						...
						onChangeRemote2(() =>
							state.mergeIntoObservable(obs$, transformedChanges),
						);
					}
					if (saveLocal) {
...
function onObsChange(
	...
	localState,
	syncOptions,
	{ changes, isFromPersist, isFromSync, getPrevious },
) {
	if (!isFromPersist) {
...
async function loadLocal(value$, syncOptions, syncState$, localState) {
	...
	if (persist == null ? void 0 : persist.name) {
		const PersistPlugin =
			persist.plugin ||
			((_a = observableSyncConfiguration.persist) == null ? void 0 : _a.plugin);
		const { table, config } = parseLocalConfig(persist);
		syncStateValue.numPendingLocalLoads =
			(syncStateValue.numPendingLocalLoads || 0) + 1;
		if (!PersistPlugin) {
			throw new Error("Local persist is not configured");
		}
		if (!mapSyncPlugins.has(PersistPlugin)) {
			const persistPlugin2 = state.isFunction(PersistPlugin)
				? new PersistPlugin()
				: PersistPlugin;
			const mapValue = {
				plugin: persistPlugin2,
				initialized: state.observable(false),
			};
			mapSyncPlugins.set(PersistPlugin, mapValue);
			if (persistPlugin2.initialize) {
				const initializePromise =
					(_c = persistPlugin2.initialize) == null
						? void 0
						: _c.call(
								persistPlugin2,
								((_b = observableSyncConfiguration) == null
									? void 0
									: _b.persist) || {},
							);
		...
		const { plugin, initialized: initialized$ } =
			mapSyncPlugins.get(PersistPlugin);
		...
		nodeValue.resetPersistence = () =>
			prevResetPersistence == null ? void 0 : prevResetPersistence();
	}
...
function syncObservable(obs$, syncOptionsOrSynced) {
	...
	if (
		...
		throw new Error(
			"[legend-state] syncObservable called with undefined observable",
		);
	}
	...
	const onGetError = (error, params, noThrow) => {
		...
		if (lastErrorHandled !== error) {
			...
			(_a = syncOptions.onError) == null
				? void 0
				: _a.call(syncOptions, error, params);
			lastErrorHandled = error;
			if (!noThrow) {
				throw error;
			}
	...
	if (isWaitingForLoad) {
		syncStateValue.numPendingRemoteLoads =
			(syncStateValue.numPendingRemoteLoads || 0) + 1;
	}
	...
	if (get || subscribe) {
		...
		sync = async (options) => {
			if (
				isSynced &&
				(!getNodeValue(getNode(syncState$)).isSyncEnabled ||
					state.shouldIgnoreUnobserved(node, sync))
			) {
				if (unsubscribe) {
					isSubscribed = false;
					unsubscribe();
			...
			const runGet = () => {
				...
				const onChange = async ({ value, mode, lastSync: lastSync2 }) => {
					...
					if (value !== void 0) {
						...
						if (options == null ? void 0 : options.resetLastSync) {
							setNodeValue(
								node,
								(_a2 = syncOptions.initial) != null ? _a2 : void 0,
							);
						}
						onChangeRemote2(() => {
							...
							if (mode === "assign") {
								...
								if (
									...
									console.error(
										"[legend-state] mode:append expects the value to be an array",
									);
								}
								obs$.push(...value);
								...
								if (
									...
									console.error(
										"[legend-state] mode:prepend expects the value to be an array",
									);
								}
				...
				if (get) {
					const getParams = {
						...
						value$: obs$,
						value:
							state.isFunction(existingValue) ||
							(existingValue == null ? void 0 : existingValue[symbolLinked])
								? void 0
								: existingValue,
					...
					const beforeGetParams = {
						...
						lastSync,
						pendingChanges:
							pending && !state.isEmpty(pending) ? pending : void 0,
						clearPendingChanges: async () => {
							localState.pendingChanges = {};
							await updateMetadataImmediate(
								obs$,
								localState,
								syncState$,
								syncOptions,
						...
						resetCache: () => {
							...
							getParams.mode = "set";
							return (_a2 = syncStateValue.resetPersistence) == null
								? void 0
								: _a2.call(syncStateValue);
						},
						cancel: false,
					};
					(_a = syncOptions.onBeforeGet) == null
						? void 0
						: _a.call(syncOptions, beforeGetParams);
	...
	const onAllPersistLoaded = () => {
		...
		while (parentNode) {
			if (
				((_b = (_a = parentNode.state) == null ? void 0 : _a.isPersistLoaded) ==
				null
					? void 0
					: _b.get()) === false
			) {
	...
	state.when(onAllPersistLoaded, function () {
		if (
			(syncOptions.get || syncOptions.subscribe) &&
			syncOptions.syncMode === "auto"
		) {
			sync();
		}
		if (
			(syncOptions == null ? void 0 : syncOptions.set) ||
			(syncOptions == null ? void 0 : syncOptions.persist)
		) {
			obs$.onChange(
...
var {
	...
	setNodeValue: setNodeValue2,
	getNodeValue: getNodeValue2,
} = state.internal;
function enableActivateSyncedNode() {
	globalState2.activateSyncedNode = function activateSyncedNode(
		node,
		newValue,
	) {
		const obs$ = getProxy(node);
		if (node.activationState) {
			const { get: getOrig, initial, set, onChange } = node.activationState;
			...
			return {
				update: onChange,
				value: promiseReturn != null ? promiseReturn : newValue,
			};
		} else {
			let update = void 0;
