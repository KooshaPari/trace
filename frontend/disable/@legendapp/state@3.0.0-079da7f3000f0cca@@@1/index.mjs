...
function isObject(obj) {
	return (
		!!obj && typeof obj === "object" && !(obj instanceof Date) && !isArray(obj)
	);
}
...
function isPrimitive(arg) {
	...
	return (
		arg !== void 0 &&
		(isDate(arg) || (type !== "object" && type !== "function"))
	);
}
function isDate(obj) {
...
function setNodeValue(node, newValue) {
	...
	newValue =
		!parentNode.isAssigning && isFunc && !isFunction(prevValue)
			? newValue(prevValue)
			: newValue;
...
function findIDKey(obj, node) {
	...
	if (!idKey && node.parent) {
		...
		const keyExtractor =
			(_b = (_a = node.functions) == null ? void 0 : _a.get(k)) != null
				? _b
				: getNodeValue(node.parent)[node.key + "_keyExtractor"];
...
function checkPlain(node, value) {
	...
	if (
		!node.isPlain &&
		(((_a = node.parent) == null ? void 0 : _a.isPlain) || isHintPlain(value))
	) {
		node.isPlain = true;
	}
}
...
function opaqueObject2(value) {
	if (process.env.NODE_ENV === "development") {
		console.warn(
			"[legend-state]: In version 3.0 opaqueObject is moved to ObservableHint.opaque",
		);
	}
	if (value) {
...
function mergeIntoObservable(target, ...sources) {
	if (
		...
		if (!isObservable(target)) {
			console.error(
				"[legend-state] should only use mergeIntoObservable with observables",
			);
		}
	}
	beginBatch();
...
function _mergeIntoObservable(target, source, levelsDeep) {
	...
	if (isSourceSet && isSet(targetValue)) {
		...
		(isTargetObj && isObject(source)) ||
		(isTargetArr && targetValue.length > 0)
	) {
		const keys =
			isSourceMap || isSourceSet
	...
	if (immediate ? node.listenersImmediate : node.listeners) {
		...
		if (changeInBatch && path.length > 0) {
			...
			if (!isArraySubset(changes[0].path, change.path)) {
				...
				changeInBatch.whenOptimizedOnlyIf ||
...
function batchNotifyChanges(changesInBatch, immediate) {
	...
	changesInBatch.forEach(
		(
			{ changes, level, value, isFromPersist, isFromSync, whenOptimizedOnlyIf },
			node,
		) => {
...
function runBatch() {
	...
	map.forEach(
		(
			{ value, prev, level, isFromPersist, isFromSync, whenOptimizedOnlyIf },
			node,
		) => {
			computeChangesRecursive(
				changesInBatch,
...
function processQueuedEvents() {
	...
	for (let i = 0; i < queueSize; i++) {
		...
		if (type === "listener-added") {
			isValid =
				!!(nodeListeners == null ? void 0 : nodeListeners.has(listener)) ||
				!!(nodeListenersImmediate == null
					? void 0
			...
			isValid =
				!(nodeListeners == null ? void 0 : nodeListeners.size) &&
				!(nodeListenersImmediate == null
					? void 0
					: nodeListenersImmediate.size);
		}
		if (isValid) {
			eventObj.type = type;
			eventObj.node = node;
...
function onChange(node, callback, options = {}, fromLinks) {
	...
	function addLinkedNodeListeners(childNode, cb = callback, from) {
		if (!(fromLinks == null ? void 0 : fromLinks.has(childNode))) {
			...
			extraDisposes = [
				...(extraDisposes || []),
				onChange(childNode, cb, childOptions, fromLinks),
			];
		}
	}
	...
	(_a = node.linkedFromNodes) == null
		? void 0
		: _a.forEach((linkedFromNode) => addLinkedNodeListeners(linkedFromNode));
...
function createCb(linkedFromNode, path, callback) {
	let prevAtPath = deconstructObjectWithPath(
		path,
		[],
		getNodeValue(linkedFromNode),
	);
	return ({ value: valueA, isFromPersist, isFromSync }) => {
...
function setupTracking(nodes, update, noArgs, immediate) {
	...
	nodes == null
		...
		: nodes.forEach((tracked) => {
				...
				listeners.push(
					onChange(node, update, { trackingType: track, immediate, noArgs }),
				);
			});
	return () => {
		if (listeners) {
...
function trackSelector(
	...
	const value = selector
		? computeSelector(
				...
				getOptions,
				observeEvent,
				observeOptions == null ? void 0 : observeOptions.fromComputed,
			)
		: selector;
	const tracker = tracking.current;
	...
	if (!(observeEvent == null ? void 0 : observeEvent.cancel)) {
		dispose = setupTracking(
			...
			updateFn,
			false,
			observeOptions == null ? void 0 : observeOptions.immediate,
		);
		if (
			process.env.NODE_ENV === "development" ||
			process.env.NODE_ENV === "test"
...
function observe(selectorOrRun, reactionOrOptions, options) {
	...
	const update = () => {
		...
		if (
			reaction &&
			((options == null ? void 0 : options.fromComputed) ||
...
function _when(predicate, effect, checkReady) {
	...
	let effectValue;
	function run(e) {
		const ret = isPredicateArray
			? predicate.map((p) => computeSelector(p))
			: computeSelector(predicate);
		if (isPromise(ret)) {
			value = ret;
			return void 0;
		} else {
		...
		const promise = new Promise((resolve) => {
			if (effect) {
				...
				effect = (value2) => {
					...
					resolve(
						isPromise(effectValue2)
							? effectValue2.then((value3) => value3)
							: effectValue2,
					);
				};
			} else {
...
function updateNodes(parent, obj, prevValue) {
	...
	if (
		(isObject(obj) && isHintOpaque(obj)) ||
		(isObject(prevValue) && isHintOpaque(prevValue))
	) {
	...
	const lengthPrev =
		((_b = keysPrev || prevValue) == null ? void 0 : _b.length) || 0;
	let idField;
			...
			if (isDiff) {
				const id =
					idField && value
						...
						: void 0;
				const existingChild =
					(_c = parent.children) == null ? void 0 : _c.get(key);
				if (isObservable(value)) {
					const valueNode = getNode(value);
					if (
						(existingChild == null ? void 0 : existingChild.linkedToNode) ===
						valueNode
					) {
				...
				if (isArr && id !== void 0) {
					const prevChild =
						id !== void 0
							? prevChildrenById == null
								? void 0
								: prevChildrenById.get(id)
							: void 0;
					if (!prevChild) {
						hasADiff = true;
...
function getProxy(node, p, asFunction) {
	if (p !== void 0) node = getChildNode(node, p, asFunction);
...
var proxyHandler = {
	get(node, p, receiver) {
		...
		const targetNode =
			node.linkedToNode || (value == null ? void 0 : value[symbolGetNode]);
		...
		if (
			...
			vProp === void 0 &&
			(ArrayModifiers.has(p) || ArrayLoopers.has(p))
		) {
			value = [];
	...
	ownKeys(node) {
		...
		if (isFunction(node)) {
			...
			["caller", "arguments", "prototype"].forEach(
				(key) => reflectedKeys.includes(key) && keys.push(key),
			);
		}
		return keys;
	},
	...
	set(node, prop, value) {
		...
		if (process.env.NODE_ENV === "development") {
			console.warn(
				"[legend-state]: Error: Cannot set a value directly:",
				prop,
				value,
			);
		}
	...
	deleteProperty(node, prop) {
		if (node.isSetting) {
			...
			if (process.env.NODE_ENV === "development") {
				console.warn(
					"[legend-state]: Error: Cannot delete a value directly:",
					prop,
				);
			}
			return false;
...
function setKey(node, key, newValue, level) {
	if (process.env.NODE_ENV === "development") {
		if (typeof HTMLElement !== "undefined" && newValue instanceof HTMLElement) {
			console.warn(
				`[legend-state] Set an HTMLElement into state. You probably don't want to do that.`,
			);
		}
	}
	...
	if (isObservable(newValue)) {
		...
		const forceNotify =
			!notify2 && childNode.isComputing && !isPrim && !childNode.isPlain;
		if (
			(notify2 || forceNotify) &&
			!(
				childNode.isComputing &&
				childNode.isPlain &&
				node.numListenersRecursive === 0
			)
		) {
	...
	if (p === "size") {
		...
		return function (a, b, c) {
			...
			if (p === "get") {
				...
				const prev = isSet(valueMapOrSet)
					? new Set(valueMapOrSet)
					: new Map(valueMapOrSet);
...
function updateNodesAndNotify(
	...
	if (valueAsArr) {
		whenOptimizedOnlyIf =
			(valueAsArr == null ? void 0 : valueAsArr.length) !==
			(prevValue == null ? void 0 : prevValue.length);
	} else if (valueAsMap) {
		whenOptimizedOnlyIf =
			(valueAsMap == null ? void 0 : valueAsMap.size) !==
			(prevValue == null ? void 0 : prevValue.size);
	}
	if (
		isPrim || !newValue || (isEmpty(newValue) && !isEmpty(prevValue))
			? newValue !== prevValue
			: hasADiff
	) {
		notify(
...
function peekInternal(node, activateRecursive) {
	...
	if (
		activateRecursive &&
		((_a = node.dirtyChildren) == null ? void 0 : _a.size)
	) {
...
function isObserved(node) {
	...
	while (parent && !hasListeners) {
		if (
			!!((_a = parent.listeners) == null ? void 0 : _a.size) ||
			!!((_b = parent.listenersImmediate) == null ? void 0 : _b.size)
		) {
			hasListeners = true;
		}
		parent = parent.parent;
	...
	const observeDispose = observe(
		() => {
			...
			if (!node.activated) {
				...
				if (
					(_b = (_a = node.state) == null ? void 0 : _a.peek()) == null
						? void 0
						: _b.sync
				) {
					node.state.sync();
					...
					value =
						(_d =
							(_c = activated2.get) == null ? void 0 : _c.call(activated2)) !=
						null
							? _d
							: activated2.initial;
				}
					...
					if (changes.length > 1 || !isFunction(changes[0].prevAtPath)) {
						...
						if (allChanges.length > 0) {
							changes = changes.filter(
								(change) => !isArraySubset(allChanges[0].path, change.path),
							);
						}
...
function setToObservable(node, value) {
	...
	if (
		linkedNode !== node &&
		(linkedNode == null ? void 0 : linkedNode.linkedToNode) !== node
	) {
		node.linkedToNode = linkedNode;
		(_a = node.linkedToNodeDispose) == null ? void 0 : _a.call(node);
		if (linkedNode) {
			linkedNode.linkedFromNodes ||
		...
		for (const key in obj) {
			if (hasOwnProperty.call(obj, key)) {
				...
				if (isObservable(value)) {
					...
					const linkedOptions =
						isFunction(value) &&
						((_a = value.prototype) == null ? void 0 : _a[symbolLinked]);
					if (linkedOptions) {
