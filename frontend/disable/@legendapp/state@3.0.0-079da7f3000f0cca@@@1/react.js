...
function usePauseProvider() {
	...
	return {
		PauseProvider: ({ children }) =>
			React.createElement(getPauseContext().Provider, { value }, children),
		isPaused$: value,
	};
}
	...
	const _update = ({ value }) => {
		if (isPaused$ == null ? void 0 : isPaused$.peek()) {
			...
			if (!changed) {
				...
				if (
					newValue !== prev ||
					(!state.isPrimitive(newValue) && newValue === value)
				) {
					changed = true;
				}
			}
...
function useSelector(selector, options) {
	...
	let value;
	if (
		reactGlobals.inObserver &&
		state.isObservable(selector) &&
		!(options == null ? void 0 : options.suspense)
	) {
	...
	try {
		...
		const selectorFn = React.useMemo(
			() => createSelectorFunctions(options, isPaused$),
			[],
		);
...
function Computed({ children }) {
	return useSelector(
		() => state.computeSelector(state.computeSelector(children)),
		{ skipCheck: true },
	);
}
...
function createReactiveComponent(
	...
	if (ReactForwardRefSymbol && render["$$typeof"] === ReactForwardRefSymbol) {
		...
		if (
			...
			throw new Error(
				`[legend-state] \`render\` property of ForwardRef was not a function`,
			);
		}
	}
		...
		apply(target, thisArg, argArray) {
			if (reactive2) {
				...
				for (let i = 0; i < keys.length; i++) {
					...
					if (
						key === "children" &&
						(state.isFunction(p) || state.isObservable(p))
					) {
						...
						propsOut[k] =
							shouldBind && (bind == null ? void 0 : bind.selector)
								? bind.selector(propsOut, p)
								: useSelector(p);
						if (shouldBind) {
...
function For({
	...
	if (!item && children) {
		...
		item = React.useMemo(
			() => observer(({ item$, id }) => refChildren.current(item$, id)),
			[],
		);
	} else {
	...
	if (isArr) {
		...
		const idField =
			length > 0
				? (node && findIDKey(v0, node)) ||
					(v0.id !== void 0 ? "id" : v0.key !== void 0 ? "key" : void 0)
				: void 0;
		...
		for (let i = 0; i < length; i++) {
			if (value[i]) {
				const val = value[i];
				const key =
					(_a = isIdFieldFunction ? idField(val) : val[idField]) != null
						? _a
						: i;
				const item$ = each[i];
				const props = {
				...
				out.push(
					React.createElement(
						item,
						itemProps ? Object.assign(props, itemProps) : props,
					),
				);
			}
		}
		...
		if (sortValues) {
			keys.sort((A, B) =>
				sortValues(
					asMap ? value.get(A) : value[A],
					asMap ? value.get(B) : value[B],
					A,
					B,
				),
			);
...
var Memo = React.memo(Computed, (prev, next) =>
...
function Switch({ value, children }) {
	...
	return (_b = child
		? child()
		: (_a = children["default"]) == null
			? void 0
			: _a.call(children)) != null
		? _b
		: null;
}
...
function useObservable(initialValue, deps) {
	...
	if (!((_a = ref.current) == null ? void 0 : _a.obs$)) {
		const value = depsObs$
			? state.isFunction(initialValue) && initialValue.length === 1
				? (p) => {
						depsObs$.get();
...
function useObserve(selector, reactionOrOptionsOrDeps, options, deps) {
	...
	if (!ref.current.dispose) {
		ref.current.dispose = state.observe(
			...
			(e) => {
				var _a, _b;
				return (_b = (_a = ref.current).reaction) == null
					? void 0
					: _b.call(_a, e);
			},
			options,
	...
	useUnmountOnce(() => {
		var _a, _b;
		(_b = (_a = ref.current) == null ? void 0 : _a.dispose) == null
			? void 0
			: _b.call(_a);
	});
