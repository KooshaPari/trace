...
function removeNullUndefined(a, recursive) {
	...
	Object.keys(a).forEach((key) => {
		if (a[key] !== null && a[key] !== void 0) {
			out[key] =
				recursive && isObject(a[key]) ? removeNullUndefined(a[key]) : a[key];
		}
	});
	return out;
}
...
async function waitForSet(waitForSet2, changes, value, params = {}) {
	const waitFn = isFunction(waitForSet2)
			...
			if (!didError || (updateResult == null ? void 0 : updateResult.changes)) {
				...
				if (pathStrs.length > 0) {
					...
					if (transformedChanges !== void 0) {
						...
						onChangeRemote2(() =>
							mergeIntoObservable(obs$, transformedChanges),
						);
					}
					if (saveLocal) {
...
async function loadLocal(value$, syncOptions, syncState$, localState) {
	...
	if (persist == null ? void 0 : persist.name) {
		...
		if (!mapSyncPlugins.has(PersistPlugin)) {
			const persistPlugin2 = isFunction(PersistPlugin)
				? new PersistPlugin()
				: PersistPlugin;
			const mapValue = {
		...
		sync = async (options) => {
			if (
				...
				(!getNodeValue(getNode(syncState$)).isSyncEnabled ||
					shouldIgnoreUnobserved(node, sync))
			) {
				if (unsubscribe) {
					isSubscribed = false;
					unsubscribe();
...
export {
	...
	diffObjects,
	getAllSyncStates,
	internal5 as internal,
	mapSyncPlugins,
	onChangeRemote2 as onChangeRemote,
	removeNullUndefined,
	syncObservable,
	synced,
