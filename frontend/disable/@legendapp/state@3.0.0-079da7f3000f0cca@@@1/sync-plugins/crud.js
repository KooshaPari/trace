...
function computeLastSync(data, fieldUpdatedAt, fieldCreatedAt) {
	...
	for (let i = 0; i < data.length; i++) {
		const updated =
			(fieldUpdatedAt ? data[i][fieldUpdatedAt] : 0) ||
			(fieldCreatedAt ? data[i][fieldCreatedAt] : 0);
		if (updated) {
...
function syncedCrud(props) {
	...
	const resultsToOutType = (results) => {
		...
		const out =
			asType === "array" ? [] : asMap ? /* @__PURE__ */ new Map() : {};
		for (let i = 0; i < results.length; i++) {
			let result = results[i];
			const value = result;
			if (value) {
				result =
					(fieldDeleted && result[fieldDeleted]) ||
					(fieldDeletedList && result[fieldDeletedList]) ||
					result[state.symbolDelete]
						? state.internal.symbolDelete
						: result;
				if (asArray) {
					out.push(result);
				} else if (asMap) {
					...
					return runWithRetry(getParams, retry, getFn || listFn, () => {
						...
						if (listFn) {
							...
							const processResults = (data) => {
								data || (data = []);
								if (fieldUpdatedAt) {
									const newLastSync = computeLastSync(
										data,
										fieldUpdatedAt,
										fieldCreatedAt,
									);
								...
								return state.isPromise(transformed)
									? transformed.then(toOut)
									: toOut(transformed);
							};
							return state.isPromise(listPromise)
							...
							const processData = (data) => {
								...
								if (data) {
									const newLastSync =
										data[fieldUpdatedAt] || data[fieldCreatedAt];
									if (newLastSync && newLastSync !== lastSync) {
										updateLastSync(newLastSync);
									}
									if (transform == null ? void 0 : transform.load) {
							...
							return state.isPromise(dataPromise)
								? dataPromise.then(processData)
								: processData(dataPromise);
						}
					});
				}
			: void 0;
		...
		createFn || updateFn || deleteFn
			? async (params) => {
					...
					const getUpdateValue = (itemValue, prev) => {
						return updatePartial
							? Object.assign(
									...
									!state.isNullOrUndefined(itemValue[fieldId])
					...
					changes.forEach((change) => {
						...
						if (asType === "value") {
							if (value) {
								let isCreate = fieldCreatedAt
									? !value[fieldCreatedAt]
									: !prevAtPath;
								...
								if (!state.isNullOrUndefined(id)) {
									...
									console.error(
										"[legend-state]: added synced item without an id",
									);
								}
							} else if (path.length === 0) {
								deletes.add(prevAtPath);
							...
							if (path.length === 0) {
								...
								const valueAsObject = asArray
									? arrayToRecord(valueAtPath, fieldId)
									: valueAtPath;
								const prevAsObject = asArray
									? arrayToRecord(prevAtPath, fieldId)
									: prevAtPath;
								const keys = getKeys(valueAsObject, false, asMap, false);
								const keysPrev = getKeys(prevAsObject, false, asMap, false);
								const keysSet = new Set(keys);
								const length =
									((_a = keys || valueAsObject) == null ? void 0 : _a.length) ||
									0;
								const lengthPrev =
									((_b = keysPrev || prevAsObject) == null
										? void 0
										: _b.length) || 0;
								...
								for (let i = 0; i < length; i++) {
									...
									const prev = prevAsObject
										...
										: void 0;
									if (
										state.isNullOrUndefined(value2) &&
										...
										const isDiff =
											!prevAsObject || !sync.deepEqual(value2, prev);
										if (isDiff) {
											itemsChanged.push([
												getUpdateValue(value2, prev),
												prev,
												value2,
											]);
										}
								...
								if (!itemValue) {
									...
									itemsChanged = [
										[getUpdateValue(itemValue, previous), previous, itemValue],
									];
								}
							}
							itemsChanged == null
								...
								: itemsChanged.forEach(([item, prev, fullValue]) => {
										const isCreate =
											...
											(fieldCreatedAt
												? !item[fieldCreatedAt] &&
													!(prev == null ? void 0 : prev[fieldCreatedAt])
												: fieldUpdatedAt
													? !item[fieldUpdatedAt] &&
														!(prev == null ? void 0 : prev[fieldCreatedAt])
													: state.isNullOrUndefined(prev));
										if (isCreate) {
											if (!item[fieldId]) {
												console.error(
													"[legend-state]: added item without an id",
												);
											}
											if (createFn) {
											...
											if (updateFn) {
												...
												updates.set(
													id,
													updates.has(id)
														? Object.assign(updates.get(id), item)
														: item,
												);
					...
					const saveResult = async (itemKey, input, data, isCreate, change) => {
						var _a;
						if (data) {
							let saved = (transform == null ? void 0 : transform.load)
								? await transform.load(data, "set")
								: data;
							const isChild = itemKey !== "undefined" && asType !== "value";
							const currentPeeked = state.getNodeValue(node);
							const currentValue = isChild
								? (_a =
										asType === "array" && state.isArray(currentPeeked)
											? currentPeeked.find((v) => v[fieldId] === itemKey)
											: void 0) != null
									? _a
									: asType === "Map"
										? currentPeeked.get(itemKey)
										: currentPeeked[itemKey]
								: currentPeeked;
							if (saved && !state.isNullOrUndefined(currentValue)) {
								...
								if (asType === "array") {
									...
									if (index < 0) {
										console.warn(
											"[legend-state] Item saved that does not exist in array",
											saved,
										);
									} else {
									...
									value2 =
										itemKey !== "undefined" && asType !== "value"
											? { [itemKey]: saved }
											: saved;
								}
								if (value2 !== void 0) {
					...
					return Promise.all([
						...
						...Array.from(creates).map(async ([itemKey, itemValue]) => {
							if (waitForSetParam) {
								await waitForSet(waitForSetParam, changes, itemValue, {
									type: "create",
								});
							}
							const createObj = await transformOut(
								itemValue,
						...
						...Array.from(updates).map(async ([itemKey, itemValue]) => {
							...
							if (waitForSetParam) {
								await waitForSet(waitForSetParam, changes, fullValue, {
									type: "update",
								});
							}
							const changed = await transformOut(
								itemValue,
								transform == null ? void 0 : transform.save,
							);
						...
						// Handle deletes
						...Array.from(deletes)
							.filter((val) => val !== state.symbolDelete)
							.map(async (valuePrevious) => {
								if (waitForSetParam) {
									await waitForSet(waitForSetParam, changes, valuePrevious, {
										type: "delete",
									});
								}
					...
					update: async (paramsUpdate) => {
						...
						if (
							...
							if (!state.isArray(rows)) {
								console.error(
									"[legend-state] subscribe:update expects an array of changed items",
								);
							}
						}
						const newLastSync = computeLastSync(
