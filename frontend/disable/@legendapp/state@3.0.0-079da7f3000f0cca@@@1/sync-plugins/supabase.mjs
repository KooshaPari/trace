...
function syncedSupabase(props) {
	...
	if (process.env.NODE_ENV === "development" && changesSince === "last-sync") {
		if (!fieldCreatedAt) {
			console.warn(
				"[legend-state] fieldCreatedAt is required when using last-sync mode",
			);
		}
		if (!fieldUpdatedAt) {
			console.warn(
				"[legend-state] fieldUpdatedAt is required when using last-sync mode",
			);
		}
		if (!fieldDeleted) {
			console.warn(
				"[legend-state] fieldDeleted is required when using last-sync mode",
			);
		}
	}
	const list =
	...
	const subscribe = realtime
		? ({ node, value$, update: update2 }) => {
				const { filter: filter2, schema: schema2 } = isObject(realtime)
					? realtime
					: {};
					...
					.on(
						...
						(payload) => {
							...
							if (eventType === "INSERT" || eventType === "UPDATE") {
								const cur =
									(_a = value$.peek()) == null ? void 0 : _a[value[fieldId]];
	...
	if (stringifyDates) {
		...
		transform = transform
			? combineTransforms(stringifier, transform)
			: stringifier;
	}
	return syncedCrud({
		...rest,
		...
		generateId,
		waitFor: () =>
			isEnabled$.get() && (waitFor ? computeSelector(waitFor) : true),
		waitForSet: (params) =>
			isEnabled$.get() &&
			(waitForSet
