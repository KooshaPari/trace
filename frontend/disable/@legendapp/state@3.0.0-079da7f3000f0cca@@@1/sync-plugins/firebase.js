...
function transformObjectFields(dataIn, map) {
	...
	let ret = dataIn;
	if (dataIn) {
		if (dataIn === state.symbolDelete) return dataIn;
...
var fns = {
	...
	orderByChild: (ref, child, start) =>
		database.query(ref, database.orderByChild(child), database.startAt(start)),
...
function syncedFirebase(props) {
	...
	const list = async (getParams) => {
		...
		return new Promise((resolve) => {
			fns.once(
				...
				(error) =>
					onError(error, { source: "list", type: "get", retry: getParams }),
			);
		});
	};
	const subscribe = isRealtime
		? ({ lastSync, update: update2, onError }) => {
				const ref = computeRef(lastSync);
				let unsubscribes;
				if (asType === "value") {
					const onValue2 = (snap) => {
					...
					const onChildDelete = (snap) => {
						...
						const valueWithId = ensureFieldId(
							key,
							state.isNullOrUndefined(valueRaw) ? {} : valueRaw,
						);
	...
	const upsert = (input, params) => {
		...
		const pendingKey =
			fieldId && asType !== "value" ? String(id != null ? id : "") : "";
		...
		if (!isRealtime) {
			updatePromise
				.then(() => {
					const onceRef =
						fieldId && asType !== "value" ? ref : fns.ref(basePath);
					fns.once(
						onceRef,
						(snap) => {
							...
							const value =
								fieldId && asType !== "value"
									? ensureFieldId(
											pendingKey,
											state.isNullOrUndefined(rawValue) ? {} : rawValue,
										)
									: rawValue;
	...
	if (fieldTransforms) {
		...
		transform = {
			load(value, method) {
				...
				return (transformProp == null ? void 0 : transformProp.load)
					? transformProp.load(fieldTransformed, method)
					: fieldTransformed;
			},
			save(value) {
				const transformed = (
	...
	return crud.syncedCrud({
		...
		waitFor: () =>
			...
			(waitFor ? state.computeSelector(waitFor) : true),
		waitForSet: (params) =>
			isEnabled$.get() &&
			(isAuthedIfRequired$ ? isAuthedIfRequired$.get() : true) &&
			(waitForSet
				? state.isFunction(waitForSet)
					? waitForSet(params)
					: waitForSet
				: true),
