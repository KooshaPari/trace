...
async function handleApiError(props, error) {
	...
	if (error.type === "unauthorized" || error.type === "forbidden") {
		...
		((_a = error.error) == null ? void 0 : _a.message) === "Failed to fetch"
	) {
		throw error.error;
	}
	return false;
...
async function getAllPages(props, listFn, params, listParams, onError) {
	...
	do {
		const first = firstParam
			? Math.min(firstParam - allData.length, NumPerPage)
			: NumPerPage;
		if (first < 1) {
			break;
		}
...
function syncedKeel(props) {
	...
	const handleSetError = async (error, params, input, fn, from) => {
		...
		if (
			from === "create" &&
			((_a = error.message) == null ? void 0 : _a.includes("for the unique")) &&
			((_b = error.message) == null ? void 0 : _b.includes("must be unique"))
		) {
			if (__DEV__) {
	...
	const subscribe = realtime
		? (params) => {
				...
				const unsubscribeParam =
					subscribeParam == null ? void 0 : subscribeParam(params);
				return () => {
