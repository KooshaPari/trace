...
function syncedSupabase(props) {
	...
	const subscribe = realtime
		? ({ node, value$, update: update2 }) => {
				const { filter: filter2, schema: schema2 } = state.isObject(realtime)
					? realtime
					: {};
	...
	if (stringifyDates) {
		...
		transform = transform
			? sync.combineTransforms(stringifier, transform)
			: stringifier;
	}
	return crud.syncedCrud({
		...
		generateId,
		waitFor: () =>
			isEnabled$.get() && (waitFor ? state.computeSelector(waitFor) : true),
		waitForSet: (params) =>
			isEnabled$.get() &&
			(waitForSet
