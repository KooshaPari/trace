import { computeSelector, getNodeValue, isString } from "@legendapp/state";
	...
	if (setParam) {
		set = async ({ value, node, update }) => {
			...
			const response = await fetch(
				url,
				Object.assign({ method: "POST" }, setInit, {
			...
			if (onSaved) {
				const responseValue =
					await response[onSavedValueType || valueType || "json"]();
				const transformed = (transform == null ? void 0 : transform.load)
					? await transform.load(responseValue, "set")
					: responseValue;
				const currentValue = getNodeValue(node);
				const valueSave = onSaved({
					input: value,
					saved: transformed,
					currentValue,
					props,
