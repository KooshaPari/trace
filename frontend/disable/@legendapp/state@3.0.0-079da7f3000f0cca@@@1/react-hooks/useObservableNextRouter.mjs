import { isEmpty, observable, setSilently } from "@legendapp/state";
...
routes$.onChange(({ value, getPrevious }) => {
	...
	if (!setter) {
		if (value.pathname) {
			...
			console.error(
				"[legend-state]: Must provide a set method to useObservableNextRouter",
			);
		}
	}
	...
	transitionOptions =
		transitionOptions ||
		(routeParams == null ? void 0 : routeParams.transitionOptions);
