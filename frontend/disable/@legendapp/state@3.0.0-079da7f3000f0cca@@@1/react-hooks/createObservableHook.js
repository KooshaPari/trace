...
function overrideHooks(refObs) {
	React__default.default.useState = function useState(initialState) {
		var _a;
		const obs =
			(_a = refObs.current) != null
				? _a
				: (refObs.current = state.observable(
						state.isFunction(initialState) ? initialState() : initialState,
					));
		return [obs.get(), obs.set];
	};
	React__default.default.useReducer = function useReducer(
		reducer,
		initializerArg,
		initializer,
	) {
		...
		const obs =
			(_a = refObs.current) != null
				? _a
				: (refObs.current = state.observable(
						initializerArg !== void 0 && state.isFunction(initializerArg)
