...
type BuiltIns =
	...
	| Function
	| Promise<any>;
type IsUserDefinedObject<T> = T extends Function | BuiltIns | any[]
	? false
	: T extends object
		? true
		: false;
type RemoveObservables<T> =
...
interface ObservableObjectFunctions<T = Record<string, any>>
	extends ObservablePrimitive<T>,
		ObservableObjectFns<T> {}
type MapKey<T extends Map<any, any> | WeakMap<any, any>> = Parameters<
	T["has"]
>[0];
type MapValue<T extends Map<any, any> | WeakMap<any, any>> = ReturnType<
	T["get"]
>;
type ObservableMap<T extends Map<any, any> | WeakMap<any, any>> = Omit<
	T,
	"get" | "size" | "set"
> &
	...
	Record<MapKey<T>, Observable<MapValue<T>>> & {
		...
		assign(
			value:
				| Record<MapKey<T>, MapValue<T>>
				| Map<MapKey<T>, MapValue<T>>
				| WeakMap<MapKey<T>, MapValue<T>>,
		): Observable<T>;
	};
type SetValue<T extends Set<any> | WeakSet<any>> = Parameters<T["has"]>[0];
type ObservableSet<T extends Set<any> | WeakSet<any>> = Omit<
	T,
	"size" | "add"
> &
	Omit<ObservablePrimitive<T>, "size"> & {
		size: number;
...
interface ObservablePrimitive<T>
	extends ImmutableObservableBase<T>,
		MutableObservableBase<T> {}
type ObservableAny = Partial<ObservableObjectFns<any>> &
...
type ObservableProps<T> =
	NonObservableKeys<NonNullable<T>> extends never
		...
		: RestoreNullability<
				T,
				Omit<NonNullable<T>, NonObservableKeys<NonNullable<T>>>
			>;
type NonObservableProps<T> = RestoreNullability<
	T,
	NullablePropsIf<
		Pick<NonNullable<T>, NonObservableKeys<NonNullable<T>>>,
		IsNullable<T>
	>
>;
...
type RestoreNullability<Source, Target> =
	IsNullable<Source> extends true
...
type ObservableObject<T> = ObservableObjectFunctions<
	ObservableProps<T> & NonObservableProps<T>
> &
...
type IsLookupFunction<T> = T extends (...args: infer P) => any
	? P extends {
		...
		: false
	: false;
type ObservableNode<T, NT = NonNullable<T>> = [NT] extends [never]
...
type ObservableParam<T = any> = ImmutableObservableSimple<T> &
	MutableObservableSimple;
...
declare function getChildNode(
	node: NodeInfo,
	key: string,
	asFunction?: Function,
): NodeInfo;
declare function ensureNodeValue(node: NodeInfo): any;
declare function findIDKey(
...
declare function shouldIgnoreUnobserved(
	node: NodeInfo,
	refreshFn: () => void,
): true | undefined;

declare function createPreviousHandler(
	value: any,
	changes: Change[],
): () => any;
...
declare function computed<T, T2 = T>(
...
declare function computeSelector<T>(
	selector: Selector<T>,
...
declare function setAtPath<T extends object>(
	obj: T,
	path: string[],
	pathTypes: TypeAtPath[],
	value: any,
...
declare function mergeIntoObservable<T extends ObservableParam<any>>(
	target: T,
	...sources: any[]
): T;
declare function constructObjectWithPath(
	path: string[],
	pathTypes: TypeAtPath[],
	value: any,
): object;
declare function deconstructObjectWithPath(
	path: string[],
	pathTypes: TypeAtPath[],
	value: any,
): object;
...
declare function applyChange<T extends object>(
	value: T,
	change: Change,
	applyPrevious?: boolean,
): T;
declare function applyChanges<T extends object>(
	value: T,
	changes: Change[],
	applyPrevious?: boolean,
): T;
...
declare function observable<T>(
	value:
		| Promise<RecursiveValueOrFunction<T>>
		| (() => RecursiveValueOrFunction<T>)
		| RecursiveValueOrFunction<T>,
): Observable<T>;
declare function observable<T>(value: T): Observable<T>;
declare function observablePrimitive<T>(
	value: Promise<T>,
): ObservablePrimitive<T>;
...
declare function proxy<T, T2 = T>(
	get: (key: string) => T,
	set: (key: string, value: T2) => void,
): Observable<Record<string, T>>;
declare function proxy<T extends Record<string, any>>(
	get: <K extends keyof T>(key: K) => ObservableParam<T[K]>,
): Observable<T>;
declare function proxy<T>(
...
declare function syncState(
	obs: ObservableParam,
): Observable<ObservableSyncState>;

declare function trackSelector<T>(
	selector: Selector<T>,
...
declare function when<T>(predicate: Selector<T>): Promise<T>;
declare function whenReady<T, T2>(
	predicate: Promise<T>,
	effect: (value: T) => T2,
): Promise<T2>;
declare function whenReady<T, T2>(
...
declare function ObservablePrimitiveClass<T>(
	this: ObservablePrimitive<T> & ObservablePrimitiveState,
	node: NodeInfo,
): void;
...
export {
	...
	type ObservableObjectFns,
	type ObservableParam,
	type ObservablePrimitive,
	type ObservableRoot,
	type ObservableState,
	type ObservableSyncState,
	type ObservableSyncStateBase,
	type ObservableSyncStateOptions,
	...
	type Primitive,
	type RecordValue,
	type RecursiveValueOrFunction,
	type RemoveObservables,
	type RetryOptions,
	type RootNodeInfo,
	type Selector,
	type SetParams,
	...
	type UpdateFnParams,
	type UpdateSetFn,
	type UpdateSetFnParams,
	type WaitForSet,
	type WaitForSetFnParams,
	applyChange,
	applyChanges,
	batch,
	beginBatch,
