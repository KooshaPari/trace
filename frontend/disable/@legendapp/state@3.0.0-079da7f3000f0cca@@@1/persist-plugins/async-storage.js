			...
			try {
				if (preload === true) {
					...
					const metadataTables = preload.map((table) =>
						table.endsWith(MetadataSuffix) ? void 0 : table + MetadataSuffix,
					);
				...
				console.error(
					"[legend-state] ObservablePersistAsyncStorage failed to initialize",
					e,
				);
			}
		} else {
	...
	loadTable(table) {
		if (this.data[table] === void 0) {
			return AsyncStorage.multiGet([table, table + MetadataSuffix])
				...
				.catch((err) => {
					if (
						(err == null ? void 0 : err.message) !== "window is not defined"
					) {
						console.error(
							"[legend-state] AsyncStorage.multiGet failed",
							table,
							err,
						);
					}
