...
function extractTarget(target) {
	...
	const protocol = target.toLowerCase().startsWith("http://")
		? "http"
		: undefined;
	...
	if (split.length > 2)
		throw new TargetError(
			`'${target}' is an invalid target, it has more than two ':' symbols`,
		);
	...
	if (!/^[0-9]+$/.test(portString))
		throw new TargetError(
			`'${target}' is an invalid target, '${portString}' is not a valid port number - try something like 'host:port'`,
		);
