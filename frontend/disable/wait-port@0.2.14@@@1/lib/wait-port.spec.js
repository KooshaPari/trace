...
describe("wait-port", () => {
	it("should wait until a port is open", () => {
		...
		//  fail.
		return waitPort({ host: "127.0.0.1", port: 9021, output: "silent" }).then(
			(open) => {
				server.close();
	...
	it("should timeout after the specified time", () => {
		...
		return waitPort({
			...
			assert(
				timeout - delta < elapsed && elapsed < timeout + delta,
		...
		return waitPort({
			...
			.catch((err) => {
				assert.strictEqual(
					err.name,
					"ConnectionError",
					"A ConnectionFailed error should be thrown",
				);
				assert(
					/.*address.*ireallyhopethatthisdomainnamedoesnotexist.com/.test(
						err.message,
					),
				);
			});
