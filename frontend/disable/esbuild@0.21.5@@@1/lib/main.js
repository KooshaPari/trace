	...
	const handleIncomingPacket = (bytes) => {
		if (isFirstPacket) {
			...
			if (binaryVersion !== "0.21.5") {
				throw new Error(
					`Cannot start service: Host version "${"0.21.5"}" does not match binary version ${quote(binaryVersion)}`,
				);
			}
			return;
		}
...
var ensureServiceIsRunning = () => {
	...
	const child = child_process.spawn(
		command,
		args.concat(`--service=${"0.21.5"}`, "--ping"),
		{
			windowsHide: true,
