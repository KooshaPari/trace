import { generate, parse, walk } from "../definition-syntax/index.js";
import * as names from "../utils/names.js";
import { SyntaxMatchError, SyntaxReferenceError } from "./error.js";
...
export class Lexer {
	constructor(config, syntax, structure) {
		...
		this.cssWideKeywordsSyntax = buildMatchGraph(
			this.cssWideKeywords.join(" |  "),
		);
	}
	...
	validate() {
				...
				walk(
					...
					(node) => {
						...
						if (!hasOwnProperty.call(map, node.name)) {
							errors.push(
								`${syntaxRef(name, broken === brokenTypes)} used missed syntax definition ${syntaxRef(node.name, node.type === "Type")}`,
							);
							broken.set(name, true);
						} else if (validate(syntax, node.name, brokenMap, map[node.name])) {
							errors.push(
								`${syntaxRef(name, broken === brokenTypes)} used broken syntax definition ${syntaxRef(node.name, node.type === "Type")}`,
							);
							broken.set(name, true);
						}
					},
		...
		const errors = [];
		const brokenTypes = new Map();
		const brokenProperties = new Map();
		...
		const brokenTypesArray = [...brokenTypes.keys()].filter((name) =>
			brokenTypes.get(name),
		);
		const brokenPropertiesArray = [...brokenProperties.keys()].filter((name) =>
