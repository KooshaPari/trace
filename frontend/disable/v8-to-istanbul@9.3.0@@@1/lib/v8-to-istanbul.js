...
module.exports = class V8ToIstanbul {
	constructor(scriptPath, wrapperLength, sources, excludePath) {
		...
		this.wrapperLength =
			wrapperLength === undefined ? cjsWrapperLength : wrapperLength;
	...
	async load() {
		const rawSource =
			this.sources.source || (await readFile(this.path, "utf8"));
		this.rawSourceMap =
			...
			convertSourceMap.fromMapFileSource(
				rawSource,
				this._readFileFromDir.bind(this),
			);
			...
			if (this.rawSourceMap.sourcemap.sources.length > 1) {
				...
				if (!this.sourceMap.sourcesContent) {
					this.sourceMap.sourcesContent =
						await this.sourcesContentFromSources();
				}
				...
				if (
					this.sources.sourceMap &&
					this.sources.sourceMap.sourcemap &&
					...
					this.sourceMap.sourcesContent &&
					this.sourceMap.sourcesContent[0]
				) {
	...
	applyCoverage(blocks) {
		blocks.forEach((block) => {
			block.ranges.forEach((range, i) => {
				...
				const { startCol, endCol, path, covSource } =
					this._maybeRemapStartColEndCol(range, isEmptyCoverage);
	...
	_maybeRemapStartColEndCol(range, isEmptyCoverage) {
		...
		const covSourceWrapperLength = isEmptyCoverage
			? 0
			: covSource.wrapperLength;
		let startCol = Math.max(0, range.startOffset - covSourceWrapperLength);
		let endCol = Math.min(
			covSource.eof,
			range.endOffset - covSourceWrapperLength,
		);
		let path = this.path;

		if (this.sourceMap) {
			const sourceTranspiledWrapperLength = isEmptyCoverage
				? 0
				: this.sourceTranspiled.wrapperLength;
			startCol = Math.max(0, range.startOffset - sourceTranspiledWrapperLength);
			endCol = Math.min(
				this.sourceTranspiled.eof,
				range.endOffset - sourceTranspiledWrapperLength,
			);

			const { startLine, relStartCol, endLine, relEndCol, source } =
				this.sourceTranspiled.offsetToOriginalRelative(
					this.sourceMap,
					startCol,
					endCol,
				);

			const matchingSource = this.covSources.find(
				(covSource) => covSource.path === source,
			);
