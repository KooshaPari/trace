...
module.exports = class CovSource {
	...
	_parseIgnore(lineStr) {
		const testIgnoreNextLines = lineStr.match(
			/^\W*\/\* (?:[cv]8|node:coverage) ignore next (?<count>[0-9]+)/,
		);
		...
		const testNodeIgnoreStartStop = lineStr.match(
			/\/\* node:coverage (?<mode>enable|disable)/,
		);
		if (testNodeIgnoreStartStop) {
			if (testNodeIgnoreStartStop.groups.mode === "disable")
				return { start: true };
			if (testNodeIgnoreStartStop.groups.mode === "enable")
				return { stop: true };
		}
	}
