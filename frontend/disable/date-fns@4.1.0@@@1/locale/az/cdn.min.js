(() => {
	...
	var S = Object.defineProperty,
		...
		R = function G(H, J, X) {
			...
			if (X !== null && X !== void 0 && X.addSuffix)
				if (X.comparison && X.comparison > 0) return Y + " sonra";
				else return Y + " \u0259vv\u0259l";
			return Y;
		};
	function K(G) {
	...
	var x = {
		...
		j = {
			date: K({ formats: x, defaultWidth: "full" }),
	...
	var v = {
		...
		b = function G(H) {
			if (H === 0) return H + "-\u0131nc\u0131";
			var J = H % 10,
				X = (H % 100) - J,
				Y = H >= 100 ? 100 : null;
			if (O[J]) return O[J];
			else if (O[X]) return O[X];
			else if (Y !== null) return O[Y];
			return "";
		},
		y = function G(H, J) {
			var X = Number(H),
	...
	function Q(G) {
		return function (H) {
			...
			var C = Z[0],
				...
				$ = Array.isArray(T) ? p(T, (A) => A.test(C)) : m(T, (A) => A.test(C)),
				U;
			(U = G.valueCallback ? G.valueCallback($) : $),
				(U = J.valueCallback ? J.valueCallback(U) : U);
			var XG = H.slice(C.length);
			return { value: U, rest: XG };
		};
	}
	function m(G, H) {
	...
	var g = /^(\d+)(-?(ci|inci|nci|uncu|üncü|ncı))?/i,
		l = /\d+/i,
		u = {
			narrow: /^(b|a)$/i,
			abbreviated:
