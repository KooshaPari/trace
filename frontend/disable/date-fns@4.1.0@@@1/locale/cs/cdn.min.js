(() => {
	...
	var S = Object.defineProperty,
		...
		D = function B(C, G, H) {
			var J,
				X = x[C];
			if (X.type === "other") J = X.other;
			else if (G === 1) J = X.one;
			else if (G > 1 && G < 5) J = X.few;
			else J = X.many;
			var Y = (H === null || H === void 0 ? void 0 : H.addSuffix) === !0,
				Z = H === null || H === void 0 ? void 0 : H.comparison,
				T;
			if (Y && Z === -1) T = J.past;
			else if (Y && Z === 1) T = J.future;
			else T = J.regular;
			return T.replace("{{count}}", String(G));
		};
	function $(B) {
		return function () {
			var C =
				...
				H = B.formats[G] || B.formats[B.defaultWidth];
			return H;
		};
	}
	var M = {
		...
		j = {
			...
			time: $({ formats: L, defaultWidth: "full" }),
			dateTime: $({ formats: V, defaultWidth: "full" }),
		},
		w = [
			"ned\u011Bli",
			"pond\u011Bl\xED",
		...
		_ = function B(C, G) {
			var H = R[C];
			if (typeof H === "function") return H(G);
			return H;
		};
	function O(B) {
		return (C, G) => {
			...
			if (H === "formatting" && B.formattingValues) {
				var X = B.defaultFormattingWidth || B.defaultWidth,
					Y = G !== null && G !== void 0 && G.width ? String(G.width) : X;
				...
				var Z = B.defaultWidth,
					T =
						G !== null && G !== void 0 && G.width
							? String(G.width)
							: B.defaultWidth;
				J = B.values[T] || B.values[Z];
			}
			var U = B.argumentCallback ? B.argumentCallback(C) : C;
			return J[U];
		};
	}
	var F = {
		...
		m = {
			ordinalNumber: y,
			era: O({ values: F, defaultWidth: "wide" }),
			quarter: O({
				values: P,
				defaultWidth: "wide",
			...
			day: O({ values: k, defaultWidth: "wide" }),
			dayPeriod: O({
				values: b,
				defaultWidth: "wide",
				formattingValues: h,
	...
	function Q(B) {
		return function (C) {
			...
			var Y = X[0],
				...
				T = Array.isArray(Z) ? g(Z, (K) => K.test(Y)) : c(Z, (K) => K.test(Y)),
				U;
			(U = B.valueCallback ? B.valueCallback(T) : T),
				(U = G.valueCallback ? G.valueCallback(U) : U);
			var HB = C.slice(Y.length);
			return { value: U, rest: HB };
		};
	}
	function c(B, C) {
