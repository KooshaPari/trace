(() => {
	...
	var W = Object.defineProperty,
		...
		D = function G(H, J, X) {
			...
			else if (J === 11 && Z.eleven) Y = Z.eleven;
			else Y = Z.other.replace("{{count}}", String(J));
			if (X !== null && X !== void 0 && X.addSuffix)
				if (X.comparison && X.comparison > 0) return "en " + Y;
				else return "fa " + Y;
			return Y;
		};
	function K(G) {
		...
		return (H, J) => {
			...
			if (X === "formatting" && G.formattingValues) {
				var Z = G.defaultFormattingWidth || G.defaultWidth,
					T = J !== null && J !== void 0 && J.width ? String(J.width) : Z;
				Y = G.formattingValues[T] || G.formattingValues[Z];
			} else {
	...
	function O(G) {
		return function (H) {
			...
			var T = Z[0],
				...
				q = Array.isArray(U) ? c(U, (x) => x.test(T)) : m(U, (x) => x.test(T)),
				B;
			(B = G.valueCallback ? G.valueCallback(q) : q),
				(B = J.valueCallback ? J.valueCallback(B) : B);
			var JG = H.slice(T.length);
			return { value: B, rest: JG };
		};
	}
	function m(G, H) {
	...
	function p(G) {
		return function (H) {
			...
			if (!Z) return null;
			var T = G.valueCallback ? G.valueCallback(Z[0]) : Z[0];
