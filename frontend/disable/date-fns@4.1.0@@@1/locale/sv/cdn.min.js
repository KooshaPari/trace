(() => {
	...
	var W = Object.defineProperty,
		...
		M = function C(G, H, J) {
			...
			else X = Y.other.replace("{{count}}", H < 13 ? S[H] : String(H));
	...
	function O(C) {
		return function (G) {
			...
			var Z = Y[0],
				...
				q = Array.isArray(B) ? c(B, (x) => x.test(Z)) : y(B, (x) => x.test(Z)),
				T;
			(T = C.valueCallback ? C.valueCallback(q) : q),
				(T = H.valueCallback ? H.valueCallback(T) : T);
			var HC = G.slice(Z.length);
			return { value: T, rest: HC };
		};
	}
	function y(C, G) {
	...
	var g = /^(\d+)(:a|:e)?/i,
		...
		a = { any: [/^s/i, /^m/i, /^ti/i, /^o/i, /^to/i, /^f/i, /^l/i] },
		e = {
