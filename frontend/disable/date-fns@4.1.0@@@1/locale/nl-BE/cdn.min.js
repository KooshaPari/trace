	...
	var W = Object.defineProperty,
		...
		S = function G(H, J, X) {
			...
			if (X !== null && X !== void 0 && X.addSuffix)
				if (X.comparison && X.comparison > 0) return "over " + Y;
				else return Y + " geleden";
			return Y;
		};
	function K(G) {
		return function () {
	...
	function O(G) {
		return function (H) {
			...
			var B = Z[0],
				...
				q = Array.isArray(C) ? m(C, (x) => x.test(B)) : h(C, (x) => x.test(B)),
				T;
			(T = G.valueCallback ? G.valueCallback(q) : q),
				(T = J.valueCallback ? J.valueCallback(T) : T);
			var GG = H.slice(B.length);
			return { value: T, rest: GG };
		};
	}
	function h(G, H) {
