(() => {
	...
	var M = {
		...
		L = {
			...
			dateTime: $({ formats: V, defaultWidth: "any" }),
		},
		j = {
	...
	var f = {
		...
		b = {
			...
			month: I({ values: P, defaultWidth: "wide" }),
			day: I({ values: v, defaultWidth: "wide" }),
			dayPeriod: I({ values: F, defaultWidth: "wide" }),
		};
	function h(G) {
		return function (H) {
			var J =
	...
	function O(G) {
		return function (H) {
			...
			var C = Z[0],
				...
				q = Array.isArray(T) ? y(T, (E) => E.test(C)) : m(T, (E) => E.test(C)),
				U;
			(U = G.valueCallback ? G.valueCallback(q) : q),
				(U = J.valueCallback ? J.valueCallback(U) : U);
			var GG = H.slice(C.length);
			return { value: U, rest: GG };
		};
	}
	function m(G, H) {
