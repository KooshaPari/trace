(() => {
	...
	function N(C, G) {
		if (G !== null && G !== void 0 && G.addSuffix)
			if (G.comparison && G.comparison > 0) return C.future;
			else return C.past;
		return C.default;
	}
	var M = {
		...
		T = function C(G, H, J) {
			var X,
				Y = M[G];
			if (Y.one && H === 1) X = N(Y.one, J);
			else X = N(Y.other, J);
			return X.replace("{{count}}", String(H));
		};
	function K(C) {
		return function () {
			var G =
	...
	var V = {
		...
		j = {
			date: K({ formats: V, defaultWidth: "full" }),
	...
	function Q(C) {
		return function (G) {
			...
			var Z = Y[0],
				...
				A = Array.isArray(U) ? y(U, (x) => x.test(Z)) : c(U, (x) => x.test(Z)),
				B;
			(B = C.valueCallback ? C.valueCallback(A) : A),
				(B = H.valueCallback ? H.valueCallback(B) : B);
			var HC = G.slice(Z.length);
			return { value: B, rest: HC };
		};
	}
	function c(C, G) {
