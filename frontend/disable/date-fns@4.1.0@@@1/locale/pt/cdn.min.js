(() => {
	...
	var W = Object.defineProperty,
		...
		S = function C(G, H, J) {
			...
			else Y = X.other.replace("{{count}}", String(H));
			if (J !== null && J !== void 0 && J.addSuffix)
				if (J.comparison && J.comparison > 0) return "daqui a " + Y;
				else return "h\xE1 " + Y;
			return Y;
		};
	function K(C) {
	...
	var M = {
		...
		j = {
			lastWeek: function C(G) {
				var H = G.getDay(),
					J = H === 0 || H === 6 ? "\xFAltimo" : "\xFAltima";
		...
		w = function C(G, H, J, Y) {
			var X = j[G];
			if (typeof X === "function") return X(H);
			return X;
		};
	function I(C) {
		return (G, H) => {
			...
			if (J === "formatting" && C.formattingValues) {
				var X = C.defaultFormattingWidth || C.defaultWidth,
					Z = H !== null && H !== void 0 && H.width ? String(H.width) : X;
