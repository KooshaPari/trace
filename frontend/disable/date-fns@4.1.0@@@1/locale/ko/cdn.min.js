(() => {
	...
	var W = Object.defineProperty,
		...
		S = function G(H, J, X) {
			...
			if (X !== null && X !== void 0 && X.addSuffix)
				if (X.comparison && X.comparison > 0) return Y + " \uD6C4";
				else return Y + " \uC804";
			return Y;
		};
	function A(G) {
	...
	var _ = {
		...
		h = function G(H, J) {
			var X = Number(H),
				Y = String(J === null || J === void 0 ? void 0 : J.unit);
			switch (Y) {
				case "minute":
				case "second":
					return String(X);
	...
	function O(G) {
		return function (H) {
			...
			var C = Z[0],
				...
				q = Array.isArray(T) ? c(T, (E) => E.test(C)) : m(T, (E) => E.test(C)),
				U;
	...
	var d = /^(\d+)(일|번째)?/i,
		...
		r = { any: [/^일/, /^월/, /^화/, /^수/, /^목/, /^금/, /^토/] },
		a = { any: /^(am|pm|오전|오후|자정|정오|아침|저녁|밤)/i },
		e = {
			any: {
				am: /^(am|오전)/i,
				pm: /^(pm|오후)/i,
