(() => {
	...
	var W = Object.defineProperty,
		...
		S = function G(H, J, X) {
			...
			else if (J === 2 && !!Z.two) Y = Z.two;
			else Y = Z.other.replace("{{count}}", String(J));
			if (X !== null && X !== void 0 && X.addSuffix)
				if (X.comparison && X.comparison > 0) return "mewn " + Y;
				else return Y + " yn \xF4l";
			return Y;
		};
	function $(G) {
		return function () {
	...
	function O(G) {
		return function (H) {
			...
			var C = Z[0],
				...
				q = Array.isArray(T) ? c(T, (K) => K.test(C)) : y(T, (K) => K.test(C)),
				U;
	...
	var p = /^(\d+)(af|ail|ydd|ed|fed|eg|ain)?/i,
		d = /\d+/i,
		g = {
			narrow: /^(c|o)/i,
		...
		l = { wide: [/^c/i, /^(ar ôl crist|ar ol crist)/i], any: [/^c/i, /^o/i] },
		u = {
			narrow: /^[1234]/i,
