...
const insertAreas = require("./hacks/grid-utils").insertAreas;
...
function hasRowsAndColumns(decl) {
	const hasRows = decl.parent.some((i) => i.prop === "grid-template-rows");
	const hasColumns = decl.parent.some(
...
class Processor {
	...
	add(css, result) {
		...
		function insideGrid(decl) {
			return decl.parent.nodes.some((node) => {
				...
				const gridTemplate = node.prop.startsWith("grid-template");
				const gridGap = /^grid-([A-z]+-)?gap/.test(node.prop);
			...
			if (prop === "color-adjust") {
				...
				if (gridPrefixes && this.gridStatus(decl, result)) {
					...
					if (/^(align|justify|place)-items$/.test(prop) && insideGrid(decl)) {
						const fixed = prop.replace("-items", "-self");
						result.warn(
						...
						const status = this.gridStatus(decl, result);
						if (
							status === "autoplace" &&
						...
						const hasRows = parent.some((i) => i.prop === "grid-template-rows");
						const hasCols = parent.some(
							(i) => i.prop === "grid-template-columns",
						);
				...
				if (value.includes("radial-gradient")) {
					if (OLD_RADIAL.test(decl.value)) {
						...
						for (const i of ast.nodes) {
							if (i.type === "function" && i.value === "radial-gradient") {
								for (const word of i.nodes) {
									if (word.type === "word") {
										if (word.value === "cover") {
											result.warn(
												"Gradient has outdated direction syntax. " +
		...
		return css.walkDecls((decl) => {
			...
			const list = this.prefixes.values("add", unprefixed);
	...
	gridStatus(node, result) {
		...
		if (node.nodes) {
			...
			node.each((i) => {
				...
				if (GRID_REGEX.test(i.text)) {
					const hasAutoplace = /:\s*autoplace/i.test(i.text);
					const noAutoplace = /no-autoplace/i.test(i.text);
					if (typeof status !== "undefined") {
						result.warn(
		...
		if (!node.nodes || value === null) {
			if (node.parent) {
				const isParentGrid = this.gridStatus(node.parent, result);
				if (node.parent._autoprefixerSelfDisabled === true) {
					value = false;
				} else {
					value = isParentGrid;
		...
		return css.walkDecls((decl, i) => {
			...
			if (
				...
				if (decl.prop === "-webkit-box-orient") {
					const hacks = { "flex-direction": true, "flex-flow": true };
			...
			for (const checker of this.prefixes.values("remove", unprefixed)) {
				...
				const notHack = this.prefixes.group(decl).down((other) => {
