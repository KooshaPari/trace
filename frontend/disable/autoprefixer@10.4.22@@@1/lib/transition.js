...
class Transition {
	...
	add(decl, result) {
		let prefix, prop;
		const add = this.prefixes.add[decl.prop];
		const vendorPrefixes = this.ruleVendorPrefixes(decl);
		const declPrefixes = vendorPrefixes || (add && add.prefixes) || [];
	...
	checkForWarning(result, decl) {
		...
		decl.parent.each((i) => {
			...
			const values = list.comma(i.value);
			// check if current Rule's transition-property comma separated value list needs prefixes
			if (i.prop === "transition-property") {
				values.forEach((value) => {
					const lookup = this.prefixes.add[value];
	...
	findProp(param) {
		...
		if (/^\d/.test(prop)) {
			for (const [i, token] of param.entries()) {
	...
	ruleVendorPrefixes(decl) {
		...
		const selectors = Browsers.prefixes().filter((s) =>
