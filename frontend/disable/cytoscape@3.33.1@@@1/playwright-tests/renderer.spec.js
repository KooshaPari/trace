...
test.describe("Renderer", () => {
	...
	test.describe("node style", () => {
		...
		test("node bounding box extends beyond width and height for bordered triangle", async ({
			page,
		}) => {
	...
	test.describe("straight edges", () => {
		...
		test("initial bounding box is zero", async ({ page }) => {
			const bb1 = await page.evaluate(() => cy.$("#ab1").boundingBox());
			const bb2 = await page.evaluate(() => cy.$("#ab2").boundingBox());
	...
	test.describe("bundled beziers", () => {
		...
		test("move when adding to the bundle", async ({ page }) => {
			...
			const d_1_01 = dist(ctrlpts1[0], ctrlpts1[1]);
			...
			const d_2_01 = dist(ctrlpts2[0], ctrlpts2[1]);
			const d_2_12 = dist(ctrlpts2[1], ctrlpts2[2]);
			const d_2_23 = dist(ctrlpts2[2], ctrlpts2[3]);

			expect(d_2_01).toBeCloseTo(stepSize);
			expect(d_2_12).toBeCloseTo(stepSize);
			expect(d_2_23).toBeCloseTo(stepSize);

			const pt_ab1_2 = await page.evaluate(() => {
				return window.cy.$("#ab1").controlPoints()[0];
			});

			const pt_ab2_2 = await page.evaluate(() => {
				return window.cy.$("#ab2").controlPoints()[0];
			});
		...
		test("move when removing from the bundle", async ({ page }) => {
			...
			const pt_ab1_2 = await page.evaluate(
				() => cy.$("#ab1").controlPoints()[0],
			);
			const pt_ab2_2 = await page.evaluate(
				() => cy.$("#ab2").controlPoints()[0],
			);

			const ctrlpts3 = await page.evaluate(() => {
				const cy = window.cy;
			...
			const pt_ab1_3 = await page.evaluate(
				() => cy.$("#ab1").controlPoints()[0],
			);
			const pt_ab2_3 = await page.evaluate(
				() => cy.$("#ab2").controlPoints()[0],
			);
			...
			const d_3_01 = dist(ctrlpts3[0], ctrlpts3[1]);
			const d_3_12 = dist(ctrlpts3[1], ctrlpts3[2]);
			const d_3_23 = dist(ctrlpts3[2], ctrlpts3[3]);
		...
		test("do not move when setting one edge to visibility:hidden", async ({
			page,
		}) => {
			await page.evaluate(() => {
		...
		test("move when setting one edge to display:none", async ({ page }) => {
			...
			const pt_ab1_2 = await page.evaluate(() => cy.$("#ab1").controlPoints());
			const pt_ab2_2 = await page.evaluate(() => cy.$("#ab2").controlPoints());
		...
		test("move when setting one edge to display:none (bigger bundle)", async ({
			page,
		}) => {
			await page.evaluate(() => {
				window.cy.add([
		...
		test("move when setting one edge to curve-style:straight", async ({
			page,
		}) => {
			await page.evaluate(() => {
				window.cy.add([
			...
			const pt_ab1_2 = await page.evaluate(() =>
				window.cy.$("#ab1").controlPoints(),
			);
			const pt_ab2_2 = await page.evaluate(
				() => window.cy.$("#ab2").controlPoints()[0],
			);
		...
		test("diagonal bottom left to top right", async ({ page }) => {
			const controlPoints = await page.evaluate(() => {
				...
				const pt_ab1_1 = cy.$("#ab1").controlPoints()[0];
				const pt_ab2_1 = cy.$("#ab2").controlPoints()[0];
	...
	test.describe("rounded-edges", () => {
		...
		test("collinear mid point correctly defined", async ({ page }) => {
			...
			const points = await page.evaluate(() =>
				midCollinearEdge.segmentPoints(),
			);
		...
		test("mid point correctly defined", async ({ page }) => {
			...
			expect(
				isInsideTriangle(midpoint, { a, b, c: controlPoint }),
			).toBeTruthy();
		});
	}); // Rounded edges
