...
CRp.colorFillStyle = (context, r, g, b, a) => {
...
CRp.colorStrokeStyle = (context, r, g, b, a) => {
...
CRp.matchCanvasSize = function (container) {
	var data = this.data;
	var bb = this.findContainerClientCoords();
	var width = bb[2];
	var height = bb[3];
	...
	if (
		container === this.data.bufferCanvases[this.MOTIONBLUR_BUFFER_NODE] ||
		container === this.data.bufferCanvases[this.MOTIONBLUR_BUFFER_DRAG]
	) {
	...
	for (var i = 0; i < this.CANVAS_LAYERS; i++) {
	...
	this.textureMult = 1;
	if (pixelRatio <= 1) {
		canvas = data.bufferCanvases[this.TEXTURE_BUFFER];
...
CRp.render = function (options) {
	...
	if (motionBlur && mbPxRatio !== 1) {
		...
		var drawMotionBlur = (cxt, txt, needClear) => {
			cxt.setTransform(1, 0, 0, 1, 0, 0);
	...
	if (motionBlur) {
		r.motionBlurTimeout = setTimeout(() => {
...
CRp.drawSelectionRectangle = function (options, setContextTransform) {
	...
	if (
		this.showFps ||
		(!drawOnlyNodeLayer && needDraw[this.SELECT_BOX] && !drawAllLayers)
	) {
		var context = forcedContext || data.contexts[this.SELECT_BOX];

		setContextTransform(context);

		if (
			this.selection[4] == 1 &&
			(this.hoverData.selecting || this.touchData.selecting)
		) {
		...
		if (data.bgActivePosistion && !this.hoverData.selecting) {
			var zoom = this.cy.zoom();
		...
		var timeToRender = this.lastRedrawTime;
		if (this.showFps && timeToRender) {
		...
		if (!drawAllLayers) {
			needDraw[this.SELECT_BOX] = false;
		}
	}
};
