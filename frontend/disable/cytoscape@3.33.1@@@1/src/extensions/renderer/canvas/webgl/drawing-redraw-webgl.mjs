...
import { debounce, getPrefixedProperty } from "../../../../util/index.mjs";
...
CRp.initWebgl = function (opts, fns) {
	const gl = this.data.contexts[this.WEBGL];

	// Set defaults and limits for configuration options.
	opts.bgColor = getBGColor(this);
	...
	this.webglDebug = opts.webglDebug;
	this.webglDebugShowAtlases = opts.webglDebugShowAtlases;
	...
	this.pickingFrameBuffer = util.createPickingFrameBuffer(gl);
	this.pickingFrameBuffer.needsDraw = true;
	...
	this.drawing = new ElementDrawingWebGL(this, gl, opts);

	// Some functions that are used to configure ElementDrawingWebGL
	const getLabelRotation = (prop) => (ele) => this.getTextAngle(ele, prop);
	...
	this.drawing.addAtlasCollection("node", {
		texRows: opts.webglTexRowsNodes,
	});
	this.drawing.addAtlasCollection("label", {
	...
	this.drawing.addTextureAtlasRenderType("node-body", {
		collection: "node",
	...
	this.drawing.addSimpleShapeRenderType("node-body", {
	...
	this.drawing.addSimpleShapeRenderType("node-overlay", {
		getBoundingBox: getBBForSimpleShape,
	...
	this.drawing.addSimpleShapeRenderType("node-underlay", {
		getBoundingBox: getBBForSimpleShape,
	...
	this.drawing.addTextureAtlasRenderType("label", {
	...
	this.drawing.addTextureAtlasRenderType("edge-source-label", {
		collection: "label",
		getTexPickingMode,
	...
	this.drawing.addTextureAtlasRenderType("edge-target-label", {
		collection: "label",
		getTexPickingMode,
	...
	this.onUpdateEleCalcs((willDraw, eles) => {
		let gcNeeded = false;
		if (eles && eles.length > 0) {
			gcNeeded |= this.drawing.invalidate(eles);
		}
		if (gcNeeded) {
			setGCFlag();
		}
	...
	overrideCanvasRendererFunctions(this);
};
...
function overrideCanvasRendererFunctions(r) {
	...
	r.findNearestElements = (x, y, interactiveElementsOnly, isTouch) => {
