...
class MiscUpscaler {
	...
	init(inputCanvas, outputCanvas) {
		...
		this.outputCanvas.height =
			this.inputCanvas.height * this.options.scaleFactor;
	...
	initBuffers() {
		...
		const texCoords = [
			0,
			1, // top-left (flipped from 0,0)
			1,
			1, // top-right (flipped from 1,0)
			0,
			0, // bottom-left (flipped from 0,1)
			0,
			0, // bottom-left (flipped from 0,1)
			1,
			1, // top-right (flipped from 1,0)
			1,
			0, // bottom-right (flipped from 1,1)
		];
		this.gl.bufferData(
			this.gl.ARRAY_BUFFER,
	...
	resize() {
		if (this.inputCanvas && this.outputCanvas) {
			this.outputCanvas.width =
				this.inputCanvas.width * this.options.scaleFactor;
			this.outputCanvas.height =
				this.inputCanvas.height * this.options.scaleFactor;
		}
	}
