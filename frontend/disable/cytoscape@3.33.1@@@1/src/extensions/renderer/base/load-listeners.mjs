...
BRp.nodeIsDraggable = (node) =>
	node && node.isNode() && !node.locked() && node.grabbable();
...
BRp.load = function () {
	...
	var triggerEvents = (target, names, e, position) => {
		if (target == null) {
			target = r.cy;
		}
	...
	var isMultSelKeyDown = (e) => {
		return e.shiftKey || e.metaKey || e.ctrlKey; // maybe e.altKey
	};

	var allowPanningPassthrough = (down, downs) => {
	...
	var setInDragLayer = (ele) => {
		ele[0]._private.rscratch.inDragLayer = true;
	};

	var setOutDragLayer = (ele) => {
		ele[0]._private.rscratch.inDragLayer = false;
	};

	var setGrabTarget = (ele) => {
		ele[0]._private.rscratch.isGrabTarget = true;
	};

	var removeGrabTarget = (ele) => {
		ele[0]._private.rscratch.isGrabTarget = false;
	};

	var addToDragList = (ele, opts) => {
	...
	var addDescendantsToDrag = (node, opts) => {
	...
	var addNodesToDrag = (nodes, opts) => {
		...
		if (opts.inDragLayer) {
			...
			nodes
				.neighborhood()
				.stdFilter((ele) => !hasCompoundNodes || ele.isEdge())
	...
	var freeDraggedElements = (grabbedEles) => {
		if (!grabbedEles) {
			return;
		}

		// just go over all elements rather than doing a bunch of (possibly expensive) traversals
		r.getCachedZSortedEles().forEach((ele) => {
			setFreed(ele);
			setOutDragLayer(ele);
	...
	var updateAncestorsInDragLayer = (node, opts) => {
		if (opts.inDragLayer == null && opts.addToList == null) {
			return;
		} // nothing to do
	...
	if (haveMutationsApi) {
		r.removeObserver = new MutationObserver((mutns) => {
		...
		r.registerBinding(r.container, "DOMNodeRemoved", (e) => {
			// eslint-disable-line no-unused-vars
			r.destroy();
		});
	}

	var onResize = util.debounce(() => {
		r.cy.resize();
	}, 100);
	...
	var invalidateCoords = () => {
		r.invalidateContainerClientCoordsCache();
	};

	forEachUp(r.container, (domEle) => {
	...
	r.registerBinding(r.container, "contextmenu", (e) => {
		e.preventDefault();
	});

	var inBoxSelection = () => r.selection[4] !== 0;

	var eventInContainer = (e) => {
	...
	r.registerBinding(
		...
		function mousedownHandler(e) {
			...
			var checkForTaphold = () => {
				...
				r.hoverData.tapholdTimeout = setTimeout(() => {
		...
		function mouseupHandler(e) {
			...
			if (r.hoverData.which === 3) {
				...
				if (r.hoverData.selecting) {
					...
					var eleWouldBeSelected = (ele) => ele.selectable() && !ele.selected();
	...
	var allAreDivisibleBy = (list, factor) => {
	...
	r.registerBinding(
		...
		function scrollHandler(e) {
			...
			r.scrollingPageTimeout = setTimeout(() => {
	...
	r.registerBinding(
		...
		(touchstartHandler = (e) => {
			...
			if (e.touches[2]) {
				...
				if (near != null) {
					...
					if (r.nodeIsGrabbable(near)) {
						...
						if (near.selected()) {
							...
							selectedNodes = cy.$(
								(ele) => ele.selected() && r.nodeIsGrabbable(ele),
							);
				...
				r.touchData.tapholdTimeout = setTimeout(() => {
					if (
						r.touchData.singleTouchMoved === false &&
	...
	if (typeof TouchEvent === "undefined") {
		...
		var updatePointer = (e) => {
			var p = pointers.filter((p) => p.event.pointerId === e.pointerId)[0];

			p.event = e;
			p.touch = makeTouch(e);
		};

		var addTouchesToEvent = (e) => {
			e.touches = pointers.map((p) => p.touch);
		};

		var pointerIsMouse = (e) =>
			e.pointerType === "mouse" || e.pointerType === 4;

		r.registerBinding(r.container, "pointerdown", (e) => {
			if (pointerIsMouse(e)) {
				return;
		...
		r.registerBinding(r.container, "pointerup", (e) => {
			if (pointerIsMouse(e)) {
				return;
			} // mouse already handled
		...
		r.registerBinding(r.container, "pointercancel", (e) => {
			if (pointerIsMouse(e)) {
				return;
			} // mouse already handled
		...
		r.registerBinding(r.container, "pointermove", (e) => {
			if (pointerIsMouse(e)) {
				return;
			} // mouse already handled
