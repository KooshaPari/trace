...
BRp.generateEllipse = function () {
	return (this.nodeShapes["ellipse"] = {
		...
		intersectLine: (nodeX, nodeY, width, height, x, y, padding, cornerRadius) =>
			math.intersectLineEllipse(
				x,
				y,
				nodeX,
				nodeY,
		...
		checkPoint: (
			...
			centerY,
			cornerRadius,
		) => math.checkInEllipse(x, y, width, height, centerX, centerY, padding),
	});
};
	...
	return (this.nodeShapes["round-rectangle"] = this.nodeShapes[
	...
	] =
		{
			...
			intersectLine: (
				...
				math.roundRectangleIntersectLine(
					x,
					y,
					nodeX,
					nodeY,
...
BRp.generateBarrel = function () {
	return (this.nodeShapes["barrel"] = {
		...
		generateBarrelBezierPts: (width, height, centerX, centerY) => {
			var hh = height / 2;
			var hw = width / 2;
			var xBegin = centerX - hw;
			var xEnd = centerX + hw;
		...
		checkPoint: function (
			...
			var getCurveT = (x, y, curvePts) => {
				...
				if (xMin <= x && x <= xMax && yMin <= y && y <= yMax) {
					...
					var validRoots = roots.filter((r) => 0 <= r && r <= 1);
