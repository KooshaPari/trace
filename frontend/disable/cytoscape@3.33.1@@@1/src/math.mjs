...
export const shiftBoundingBox = (bb, dx, dy) => ({
	x1: bb.x1 + dx,
	x2: bb.x2 + dx,
	y1: bb.y1 + dy,
	y2: bb.y2 + dy,
...
export const solveCubic = (a, b, c, d, result) => {
	...
	if (discriminant > 0) {
		...
		s = s < 0 ? -((-s) ** (1.0 / 3.0)) : s ** (1.0 / 3.0);
		t = r - Math.sqrt(discriminant);
		t = t < 0 ? -((-t) ** (1.0 / 3.0)) : t ** (1.0 / 3.0);
		result[0] = -term1 + s + t;
		term1 += (s + t) / 2.0;
...
export const pointInsideRoundPolygon = (
	...
	for (let i = 0; i < corners.length; i++) {
		...
		const squaredDistance = (corner.cx - x) ** 2 + (corner.cy - y) ** 2;
		if (squaredDistance <= corner.radius ** 2) {
			return true;
		}
	}
...
export const roundPolygonIntersectLine = (
	...
	if (intersections.length > 2) {
		...
		let lowestSquaredDistance =
			(lowestIntersection[0] - x) ** 2 + (lowestIntersection[1] - y) ** 2;
		for (let i = 1; i < intersections.length / 2; i++) {
			const squaredDistance =
				(intersections[i * 2] - x) ** 2 + (intersections[i * 2 + 1] - y) ** 2;
