...
const updateBounds = (b, x1, y1, x2, y2) => {
...
const updateBoundsFromBox = (b, b2) => {
	if (b2 == null) {
		return b;
	}
...
const prefixedProperty = (obj, field, prefix) =>
	getPrefixedProperty(obj, field, prefix);

const updateBoundsFromArrow = (bounds, ele, prefix) => {
	if (ele.cy().headless()) {
		return;
	}
...
const updateBoundsFromLabel = (bounds, ele, prefix) => {
	if (ele.cy().headless()) {
		return;
	}
...
const updateBoundsFromOutline = (bounds, ele) => {
	if (ele.cy().headless()) {
		return;
	}
...
const updateBoundsFromMiterBorder = (bounds, ele) => {
	if (ele.cy().headless()) {
		return;
	}
...
const boundingBoxImpl = (ele, options) => {
...
const cachedBoundingBoxImpl = (ele, opts) => {
	const _p = ele._private;
	let bb;
	const isEdge = ele.isEdge();
