...
const getDist = (type, node, centroid, attributes, mode) => {
...
const randomCentroids = (nodes, k, attributes) => {
...
const classify = (node, centroids, distance, attributes, type) => {
	let min = Infinity;
	let index = 0;
...
const buildCluster = (centroid, nodes, assignment) => {
	const cluster = [];
	let node = null;
...
const haveValuesConverged = (v1, v2, sensitivityThreshold) =>
	Math.abs(v2 - v1) <= sensitivityThreshold;

const haveMatricesConverged = (v1, v2, sensitivityThreshold) => {
...
const seenBefore = (node, medoids, n) => {
	for (let i = 0; i < n; i++) {
...
const findCost = (potentialNewMedoid, cluster, attributes) => {
	let cost = 0;
...
const updateCentroids = (centroids, nodes, U, weight, opts) => {
	let numerator, denominator;

	for (let n = 0; n < nodes.length; n++) {
		for (let c = 0; c < centroids.length; c++) {
			weight[n][c] = U[n][c] ** opts.m;
		}
	}
...
const updateMembership = (U, _U, centroids, nodes, opts) => {
	// Save previous step
