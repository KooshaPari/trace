...
const elesfn = {
		...
		if (filter === undefined) {
			...
			for (let i = 0; i < this.length; i++) {
				...
				const include = thisArg
					? filter.apply(thisArg, [ele, i, this])
					: filter(ele, i, this);
	...
	intersect: function (other) {
		...
		const colS = col1Smaller ? this : col2;
		const colL = col1Smaller ? col2 : this;
	...
	map: function (mapFn, thisArg) {
		...
		for (let i = 0; i < this.length; i++) {
			const ele = this[i];
			const ret = thisArg
				? mapFn.apply(thisArg, [ele, i, this])
				: mapFn(ele, i, this);
	...
	max: function (valFn, thisArg) {
		...
		for (let i = 0; i < this.length; i++) {
			const ele = this[i];
			const val = thisArg
				? valFn.apply(thisArg, [ele, i, this])
				: valFn(ele, i, this);
