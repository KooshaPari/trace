...
styfn.parse = function (name, value, propIsBypass, propIsFlat) {
	...
	if (is.fn(value)) {
		return this.parseImplWarn(name, value, propIsBypass, propIsFlat);
	}
	...
	const propCache = (this.propCache = this.propCache || []);
	let ret;

	if (!(ret = propCache[argHash])) {
		ret = propCache[argHash] = this.parseImplWarn(
			name,
			value,
			propIsBypass,
			propIsFlat,
...
styfn.parseImpl = function (name, value, propIsBypass, propIsFlat) {
	...
	if (type.multiple && propIsFlat !== "multiple") {
		...
		for (let i = 0; i < vals.length; i++) {
			const p = this.parse(name, vals[i], propIsBypass, "multiple");
