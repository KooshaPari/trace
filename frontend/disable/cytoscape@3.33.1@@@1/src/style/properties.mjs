...
(() => {
	...
	const data = (prefix) => "^" + prefix + "\\s*\\(\\s*([\\w\\.]+)\\s*\\)$";
	...
	const diff = {
		...
		any: (val1, val2) => val1 != val2,
		emptyNonEmpty: (str1, str2) => {
	...
	[
		...
		arrowPrefixes.forEach((prefix) => {
			const name = prefix + "-" + prop.name;
			const { type, triggersBounds } = prop;
	...
	styfn.propertyNames = props.map((p) => p.name);
...
styfn.getDefaultProperties = function () {
	...
	const rawProps = util.extend(
		...
		[
			...
			styfn.arrowPrefixes.forEach((prefix) => {
				const name = prefix + "-" + prop.name;
				const val = prop.value;
