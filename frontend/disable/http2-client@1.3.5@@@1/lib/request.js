...
function addFunctions(container, obj) {
	...
	Object.keys(proto).forEach((name) => {
		if (container.indexOf(name) != -1) return;
...
function ClientRequest() {
	...
	for (var i = 0; i < STUBBED_METHODS_NAME.length; i++) {
		const name = STUBBED_METHODS_NAME[i];
	...
	if (isInternal) {
		...
		if (enforceOptions.method) isInternalMethod = enforceOptions.method;
		if (enforceOptions.protocol) isInternalProtocol = enforceOptions.protocol;
	}
	if (isInternal) {
	...
	} else if (!requestOptions.method) requestOptions.method = "GET";

	requestOptions.method = requestOptions.method.toUpperCase();

	const requestManager =
		requestOptions.requestManager || this.getGlobalManager(requestOptions);
	requestManager.handleClientRequest(this, requestOptions, cb);
}
ClientRequest.prototype = {
	getGlobalManager(options) {
		if (options.agent)
			return options.agent.protocol == "https:"
				? HttpsRequest.globalManager
				: HttpRequest.globalManager;
		else return HttpRequestManager.globalManager;
	},
	genericStubber(method, args) {
		if (this[$stubs]) {
			this[$stubs].push([method, args]);
			return true;
		} else return this[method](...arguments);
	},
	on(eventName, cb) {
		...
		if (this._on) {
			this._on(...arguments);
		} else this.genericStubber("on", arguments);
	},
	once(eventName, cb) {
		if (eventName == "response") {
			if (!cb.http2Safe) {
		...
		} else this.genericStubber("once", arguments);
	},
	emitError(error) {
		if (this[$stubs]) {
		...
		} else return this.emit("error", error);
	},
	take(stream) {
	...
	proxyProps(http2Stream) {
		...
		const notToProxy = ["on", "_on", "_once", "once", "http2Mimic"].concat(
			STUBBED_METHODS_NAME,
		);
		...
		for (var i = 0; i < keysToProxy.length; i++) {
			const name = keysToProxy[i];
			const propConfig = properties[name];
			let shouldCopyValue;
			if (!propConfig) shouldCopyValue = true;
			if (propConfig && (propConfig.writable || propConfig))
				shouldCopyValue = true;

			if (shouldCopyValue) http2Stream[name] = this[name];
...
class HttpRequestManager extends EventEmitter {
	...
	init(options) {
		...
		this.setModules();
		this.http2Debouncer = new DebounceTimers(
			function stopConnection(key) {
		...
		this.keepH1IdentificationCacheFor =
			options.keepH1IdentificationCacheFor || 30000;
	...
	handleClientRequest(clientRequest, requestOptions, cb) {
		...
		if (requestManager.hasCachedConnection(clientKey)) {
			...
			requestManager.holdConnectionToIdentification(
				clientKey,
				requestOptions,
				function onIdentification(error, connectionOptions) {
					if (error) {
	...
	setHttp2Client(clientKey, client) {
		...
		if (prevClient) httpManager.removeHttp2Client(clientKey, prevClient);
	...
	hasCachedConnection(clientKey) {
		...
		return (
			this.cachedHTTP1Result[clientKey] + this.keepH1IdentificationCacheFor <
			Date.now()
		);
	}
	makeRequest(inStream, clientKey, requestOptions, cb, connectionOptions) {
		...
		if (http2Client) {
			return this.makeHttp2Request(
				...
				inStream,
				http2Client,
				Object.assign(connectionOptions || {}, requestOptions),
				cb,
			);
		}
		...
		if (!requestOptions.agent) {
			...
			else requestOptions.agent = this.httpAgent;
		}
	...
	holdConnectionToIdentification(clientKey, requestOptions, cb) {
		...
		if (this._events[topic])
			...
			const socket = this.identifyConnection(
				requestOptions,
				function onIdentify(error, type) {
					if (error) {
						return cb(error);
					}
	...
	makeHttpRequest(clientKey, inStream, options, cb, connectionOptions) {
		if (options instanceof URL) options = urlToOptions(options);
	...
	makeHttp2Request(clientKey, inStream, http2Client, requestOptions, cb) {
		...
		if (requestOptions.method) headers[":method"] = requestOptions.method;
		if (requestOptions.path) headers[":path"] = requestOptions.path;
		...
		var req = http2Client.request(headers);
		...
		function onResponse(headers) {
			...
			if (maxContentLength < 0) this.http2Debouncer.unpauseAndTime(clientKey);
	...
	connect(authority, options, listener) {
		...
		var protocol =
			authority.protocol ||
			options.protocol ||
			(this.enforceProtocol != "detect" ? this.enforceProtocol : null) ||
			"http:";
		var port =
			"" +
			(authority.port !== ""
		...
		if (typeof options.createConnection === "function") {
			...
			switch (protocol) {
				case "http:":
					...
					break;
				case "https:":
					socket = this.tls.connect(
						port,
						host,
						initializeTLSOptions.call(this, options, host),
						listener,
					);
					break;
				default:
