...
test("defineDataProperty", (t) => {
	t.test("argument validation", (st) => {
		forEach(v.primitives, (nonObject) => {
			st["throws"](
				// @ts-expect-error
				() => {
	...
	t.test("normal data property", (st) => {
		...
		st.ok(
			hasOwn(obj, "explicit1"),
			"has expected own property (explicit enumerable)",
		);
		st.equal(
			obj.explicit1,
			"new value",
			"has new expected value (explicit enumerable)",
		);

		defineDataProperty(obj, "explicit2", "new value", false, false);
		st.ok(
			hasOwn(obj, "explicit2"),
			"has expected own property (explicit writable)",
		);
		st.equal(
			obj.explicit2,
			"new value",
			"has new expected value (explicit writable)",
		);

		defineDataProperty(obj, "explicit3", "new value", false, false, false);
		st.ok(
			hasOwn(obj, "explicit3"),
			"has expected own property (explicit configurable)",
		);
		st.equal(
			obj.explicit3,
			"new value",
			"has new expected value (explicit configurable)",
		);

		st.end();
	});

	t.test("loose mode", { skip: !hasPropertyDescriptors }, (st) => {
		var obj = { existing: "existing property" };
	...
	t.test(
		"new non-normal data property, ES5+",
		{ skip: !hasPropertyDescriptors },
		(st) => {
			/** @type {Record<PropertyKey, string>} */
			var obj = { existing: "existing property" };
	...
	t.test(
		"existing non-normal data property, ES5+",
		{ skip: !hasPropertyDescriptors },
		(st) => {
	...
	t.test("frozen object, ES5+", { skip: !hasPropertyDescriptors }, (st) => {
	...
	t.test("sealed object, ES5+", { skip: !hasPropertyDescriptors }, (st) => {
