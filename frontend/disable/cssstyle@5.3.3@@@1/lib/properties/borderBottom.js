			...
			switch (type) {
				...
				case "Identifier": {
					if (parsers.isValidPropertyValue("border-bottom-width", name)) {
						...
						parsers.isValidPropertyValue("border-bottom-style", name)
					) {
						...
						parsers.isValidPropertyValue("border-bottom-color", name)
					) {
						if (parsedValues.has("border-bottom-color")) {
							return;
						}
		...
		for (const key of keys) {
			if (parsedValues.has(key)) {
				...
				if (parsedValue !== module.exports.initialValues.get(key)) {
					...
					if (
						obj["border-bottom-width"] &&
						obj["border-bottom-width"] === "medium"
					) {
