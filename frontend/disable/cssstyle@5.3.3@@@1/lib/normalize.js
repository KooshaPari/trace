...
const matchesBorderShorthandValue = (
	property,
	value,
	shorthandValue,
	opt = {},
...
const replaceBorderShorthandValue = (value, shorthandValue, opt = {}) => {
	...
	const borderFirstInitialValue = border.initialValues.get(
		borderFirstInitialKey,
	);
...
exports.prepareBorderProperties = (
	property,
	value,
	priority,
	properties,
	...
	if (typeof value === "string") {
		...
		if (prop3) {
			...
			if (propertyValue === "") {
				...
				if (lineItem.value) {
					lineItem.value = replacePositionValue(
						propertyValue,
			...
			if (propertyValue === "") {
				...
				if (
					lineWidthItem.value &&
					isValidPropertyValue(lineWidthProperty, propertyValue)
				) {
					lineWidthItem.value = propertyValue;
				}
				if (
					lineStyleItem.value &&
					isValidPropertyValue(lineStyleProperty, propertyValue)
				) {
					lineStyleItem.value = propertyValue;
				}
				if (
					lineColorItem.value &&
					isValidPropertyValue(lineColorProperty, propertyValue)
				) {
		...
		if (value.length === 1) {
			...
			if (nameItem.value && propertyValue) {
				nameItem.value = replaceBorderShorthandValue(
					propertyValue,
					nameItem.value,
					{
						globalObject,
...
const prepareBorderShorthands = (properties) => {
	...
	if (lineWidthItems.size === 4) {
		const [property, item] =
			generateBorderLineShorthand(lineWidthItems, "border-width") ?? [];
		if (property && item) {
			properties.set(property, item);
		}
	} else if (lineWidthPriorItems.size === 4) {
		const [property, item] =
			generateBorderLineShorthand(
				lineWidthPriorItems,
				"border-width",
				"important",
			) ?? [];
	...
	if (lineStyleItems.size === 4) {
		const [property, item] =
			generateBorderLineShorthand(lineStyleItems, "border-style") ?? [];
		if (property && item) {
			properties.set(property, item);
		}
	...
	if (lineColorItems.size === 4) {
		const [property, item] =
			generateBorderLineShorthand(lineColorItems, "border-color") ?? [];
		if (property && item) {
			properties.set(property, item);
		}
	...
	if (positionTopItems.size === 3) {
		const [property, item] =
			generateBorderPositionShorthand(positionTopItems, "border-top") ?? [];
		if (property && item) {
			properties.set(property, item);
		...
		const [property, item] =
			generateBorderPositionShorthand(
				positionTopPriorItems,
				"border-top",
				"important",
			) ?? [];
		if (property && item) {
	...
	if (nameItems.length === 4) {
		const [property, item] =
			generateBorderNameShorthand(nameItems, "border") ?? [];
		if (property && item) {
			properties.set(property, item);
	...
	if (mixedPriorities) {
		...
		const { priority: firstPropertyPriority } =
			properties.get(firstPropertyKey);
...
exports.prepareProperties = (properties, opt = {}) => {
	...
	for (const [property, item] of properties) {
		...
		const { logicalPropertyGroup: shorthandProperty } =
			implementedProperties.get(property) ?? {};
		if (exports.borderProperties.has(property)) {
			...
			if (longhandItems.size) {
				...
				const { priority: firstPropertyPriority } =
					longhandItems.get(firstPropertyKey);
				...
				if (typeof parsedValue === "string") {
					for (const [
						...
						if (!priority && properties.has(longhandProperty)) {
							const { priority: longhandPriority } =
								properties.get(longhandProperty);
							if (longhandPriority) {
						...
						const longhandValue = exports.getPositionValue(
							[parsedValue],
							position,
						);
		...
		for (const [property, item] of prepareShorthands) {
			...
			if (item.size === shorthandItem.shorthandFor.size) {
				if (shorthandItem.position) {
					...
					const value = exports.getPositionValue(
						positionValues,
						shorthandItem.position,
					);
					...
					for (const lineProperty of Object.keys(lineItems)) {
						...
						for (const position of positions) {
							...
							if (longhandProperties.has(longhandProperty)) {
								const { priority: longhandPriority } =
									longhandProperties.get(longhandProperty);
								if (!longhandPriority) {
