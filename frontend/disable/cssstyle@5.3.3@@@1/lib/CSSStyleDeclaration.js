	...
	constructor(onChangeCallback, opt = {}) {
		...
		if (context) {
			if (typeof context.getComputedStyle === "function") {
				...
				const window =
					context?.parentStyleSheet?.ownerNode?.ownerDocument?.defaultView;
				if (window) {
					this._global = window;
				}
			}
...
Object.defineProperties(CSSStyleDeclaration.prototype, {
	...
	_borderSetter: {
		...
		value(prop, val, prior) {
			...
			const parsedProperties = prepareBorderProperties(
				prop,
				val,
				prior,
				properties,
			...
			if (val && !hasVarFunc(val)) {
				...
				for (const [longhandProperty] of shorthandItem.shorthandFor) {
					if (longhandProperty === prop) {
						...
						const longhandPriority =
							this._priorities.get(longhandProperty) ?? "";
	...
	_positionShorthandSetter: {
		...
		value(prop, val, prior) {
			...
			for (const [longhandProperty, longhandItem] of shorthandFor) {
				...
				const longhandValue = getPositionValue(
					shorthandValues,
					longhandPosition,
				);
				if (priority) {
		...
		value(prop, val, prior, shorthandProperty) {
			...
			if (val && !hasVarFunc(val)) {
				...
				if (longhandValues.length === shorthandFor.size) {
					const replacedValue = getPositionValue(
						longhandValues,
						shorthandPosition,
					);
