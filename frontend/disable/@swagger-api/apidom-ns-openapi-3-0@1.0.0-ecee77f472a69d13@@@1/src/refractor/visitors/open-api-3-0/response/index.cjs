...
var _Response = _interopRequireDefault(
	require("../../../../elements/Response.cjs"),
);
 ...
 * @public
 */
class ResponseVisitor extends (0, _tsMixer.Mixin)(
	...
	ObjectElement(objectElement) {
		...
		if ((0, _apidomCore.isObjectElement)(this.element.contentProp)) {
			this.element.contentProp
				.filter(_predicates.isMediaTypeElement)
				// @ts-ignore
				.forEach((mediaTypeElement, key) => {
					mediaTypeElement.setMetaProperty(
						"media-type",
		...
		if ((0, _apidomCore.isObjectElement)(this.element.headers)) {
			this.element.headers
				.filter(_predicates.isHeaderElement)
				// @ts-ignore
				.forEach((headerElement, key) => {
					headerElement.setMetaProperty(
						"header-name",
						(0, _apidomCore.toValue)(key),
					);
				});
