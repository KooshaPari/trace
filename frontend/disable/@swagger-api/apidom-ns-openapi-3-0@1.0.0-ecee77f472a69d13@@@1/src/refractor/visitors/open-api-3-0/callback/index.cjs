 ...
 * @public
 */
class CallbackVisitor extends (0, _tsMixer.Mixin)(
	...
	constructor(options) {
		...
		this.fieldPatternPredicate = (value) =>
			/{(?<expression>[^}]{1,2083})}/.test(String(value)); // 2,083 characters is the maximum length of a URL in Chrome
	}
	ObjectElement(objectElement) {
		const result = _MapVisitor.default.prototype.ObjectElement.call(
			this,
			objectElement,
		);
		...
		this.element
			.filter(_predicates.isPathItemElement)
			// @ts-ignore
			.forEach((pathItemElement, key) => {
				pathItemElement.setMetaProperty(
					"runtime-expression",
					(0, _apidomCore.toValue)(key),
				);
			});
		return result;
	}
