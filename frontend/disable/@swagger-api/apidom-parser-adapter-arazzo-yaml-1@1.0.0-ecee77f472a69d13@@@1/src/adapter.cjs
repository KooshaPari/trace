...
var _apidomNsArazzo = _interopRequireWildcard(
	require("@swagger-api/apidom-ns-arazzo-1"),
);
...
const detect = async (source) =>
	...
	(await (0, _apidomParserAdapterYaml.detect)(source));
...
const parse = async (source, options = {}) => {
	...
	const parseResultElement = await (0, _apidomParserAdapterYaml.parse)(
		source,
		parserOpts,
	);
	const { result } = parseResultElement;
	if ((0, _ramdaAdjunct.isNotUndefined)(result)) {
		const arazzoSpecificationElement =
			_apidomNsArazzo.ArazzoSpecification1Element.refract(
				result,
				refractorOpts,
			);
