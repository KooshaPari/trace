...
var _FixedFieldsVisitor = _interopRequireDefault(
	require("../generics/FixedFieldsVisitor.cjs"),
);
var _ParentSchemaAwareVisitor = _interopRequireDefault(
	require("./ParentSchemaAwareVisitor.cjs"),
);
var _FallbackVisitor = _interopRequireDefault(
	require("../FallbackVisitor.cjs"),
);
var _JSONSchema = _interopRequireDefault(
	require("../../../elements/JSONSchema.cjs"),
);
 ...
 * @public
 */
class JSONSchemaVisitor extends (0, _tsMixer.Mixin)(
	_FixedFieldsVisitor.default,
	_ParentSchemaAwareVisitor.default,
	...
	ObjectElement(objectElement) {
		...
		return _FixedFieldsVisitor.default.prototype.ObjectElement.call(
			this,
			objectElement,
		);
	}
	handleDialectIdentifier(objectElement) {
		...
		if (
			(0, _ramdaAdjunct.isUndefined)(this.parent) &&
			!(0, _apidomCore.isStringElement)(objectElement.get("$schema"))
		) {
			// no parent available and no $schema is defined, set default $schema
			this.element.setMetaProperty(
				"inheritedDialectIdentifier",
				this.defaultDialectIdentifier,
			);
		} else if (
			(0, _predicates.isJSONSchemaElement)(this.parent) &&
			!(0, _apidomCore.isStringElement)(objectElement.get("$schema"))
		) {
			// parent is available and no $schema is defined, set parent $schema
			const inheritedDialectIdentifier = (0, _ramda.defaultTo)(
				(0, _apidomCore.toValue)(
					this.parent.meta.get("inheritedDialectIdentifier"),
				),
				(0, _apidomCore.toValue)(this.parent.$schema),
			);
			this.element.setMetaProperty(
				"inheritedDialectIdentifier",
				inheritedDialectIdentifier,
		...
		const ancestorsSchemaIdentifiers =
			this.parent !== undefined
				? (0, _apidomCore.cloneDeep)(
						this.parent.getMetaProperty("ancestorsSchemaIdentifiers", []),
					)
				: new _apidomCore.ArrayElement();
		// get current schema identifier
		const schemaIdentifier = (0, _apidomCore.toValue)(
			objectElement.get(identifierKeyword),
		);
