 ...
 * @public
 */
class ItemsVisitor extends (0, _tsMixer.Mixin)(
	_SpecificationVisitor.default,
	_ParentSchemaAwareVisitor.default,
	_FallbackVisitor.default,
) {
	ObjectElement(objectElement) {
		const specPath = (0, _predicates.isJSONReferenceLikeElement)(objectElement)
	...
	ArrayElement(arrayElement) {
		...
		arrayElement.forEach((item) => {
			const specPath = (0, _predicates.isJSONReferenceLikeElement)(item)
				? ["document", "objects", "JSONReference"]
				: ["document", "objects", "JSONSchema"];
