...
class FixedFieldsVisitor extends _SpecificationVisitor.default {
	specPath;
	ignoredFields;
	constructor({ specPath, ignoredFields, ...rest }) {
		super({
			...rest,
		});
	...
	ObjectElement(objectElement) {
		...
		objectElement.forEach((value, key, memberElement) => {
			if (
				(0, _apidomCore.isStringElement)(key) &&
				fields.includes((0, _apidomCore.toValue)(key)) &&
				!this.ignoredFields.includes((0, _apidomCore.toValue)(key))
			) {
				const fixedFieldElement = this.toRefractedElement(
					[...specPath, "fixedFields", (0, _apidomCore.toValue)(key)],
					value,
				);
