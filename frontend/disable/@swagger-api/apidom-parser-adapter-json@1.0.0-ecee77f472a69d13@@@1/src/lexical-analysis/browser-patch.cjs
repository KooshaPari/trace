...
var _treeSitter = _interopRequireDefault(
	require("web-tree-sitter/tree-sitter.wasm"),
);
// @ts-ignore
...
if ((0, _ramdaAdjunct.isFunction)(realFetch)) {
	globalThis.fetch = (...args) => {
		// @ts-ignore
		if (
			(0, _ramdaAdjunct.isString)(args[0]) &&
			args[0].endsWith("tree-sitter.wasm")
		) {
			// @ts-ignore
			return realFetch.apply(globalThis, [
				_treeSitter.default,
				(0, _ramda.tail)(args),
			]);
		}
