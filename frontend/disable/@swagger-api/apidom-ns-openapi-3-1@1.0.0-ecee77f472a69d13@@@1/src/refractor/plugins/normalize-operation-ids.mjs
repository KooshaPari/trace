...
import { defaultTo, groupBy, last } from "ramda";
				...
				OpenApi3_1Element: {
					...
					leave() {
						...
						links.forEach((linkElement) => {
							...
							linkElement.operationId = cloneDeep.safe(
				...
				PathItemElement: {
					enter(pathItemElement) {
						...
						const pathTemplate = defaultTo(
							"path",
							toValue(pathItemElement.meta.get("path")),
						);
				...
				OperationElement: {
					enter(operationElement, key, parent, path, ancestors) {
						...
						const originalOperationId = String(
							toValue(operationElement.operationId),
						);
