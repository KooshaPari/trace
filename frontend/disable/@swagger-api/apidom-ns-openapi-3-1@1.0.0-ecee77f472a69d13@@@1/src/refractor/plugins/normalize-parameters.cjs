...
const plugin =
	...
	(toolbox) => {
		...
		const parameterEquals = (parameter1, parameter2) => {
			...
			return (
				(0, _apidomCore.toValue)(parameter1.name) ===
					(0, _apidomCore.toValue)(parameter2.name) &&
				(0, _apidomCore.toValue)(parameter1.in) ===
					(0, _apidomCore.toValue)(parameter2.in)
			);
		};
			...
			visitor: {
				...
				OperationElement: {
					leave(operationElement, key, parent, path, ancestors) {
						const parentPathItemParameters = (0, _ramda.last)(
							pathItemParameters,
						);

						// no Path Item Object parameters to inherit from
						if (
							!Array.isArray(parentPathItemParameters) ||
							parentPathItemParameters.length === 0
						) {
							return;
						}
						...
						const operationParameters = (0, _ramda.pathOr)(
							[],
							["parameters", "content"],
							operationElement,
						);
						...
						const mergedParameters = (0, _ramda.uniqWith)(parameterEquals, [
							...operationParameters,
							...parentPathItemParameters,
						]);
						operationElement.parameters =
							new _apidomNsOpenapi.OperationParametersElement(mergedParameters);
