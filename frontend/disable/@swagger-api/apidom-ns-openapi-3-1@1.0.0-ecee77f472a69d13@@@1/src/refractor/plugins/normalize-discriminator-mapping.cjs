...
var _NormalizeStorage = _interopRequireDefault(
	require("./normalize-header-examples/NormalizeStorage.cjs"),
);
 ...
 */
const plugin =
	({ storageField = "x-normalized", baseURI = "" } = {}) =>
	(toolbox) => {
			...
			visitor: {
				OpenApi3_1Element: {
					enter(element) {
						...
						storage = new _NormalizeStorage.default(
							element,
							storageField,
							"discriminator-mapping",
						);
						allOfDiscriminatorMapping =
							(_element$getMetaPrope = element.getMetaProperty(
								"allOfDiscriminatorMapping",
							)) != null
								? _element$getMetaPrope
								: new _apidomCore.ObjectElement();
					},
					leave() {
						storage = undefined;
					},
				...
				SchemaElement: {
					leave(schemaElement, key, parent, path, ancestors) {
						...
						if (
							!(0, _apidomNsOpenapi.isDiscriminatorElement)(
								schemaElement.discriminator,
							)
						) {
							return;
						}
						...
						if (
							(0, _apidomCore.isArrayElement)(schemaElement.oneOf) &&
							(0, _apidomCore.isArrayElement)(schemaElement.anyOf)
						) {
							return;
						}
						const parentElement = ancestors[ancestors.length - 1];
						const schemaName = schemaElement.getMetaProperty("schemaName");
						const allOfMapping = allOfDiscriminatorMapping.getMember(
						...
						const mapping =
							(_schemaElement$discri =
								schemaElement.discriminator.get("mapping")) != null
								? _schemaElement$discri
								: new _apidomCore.ObjectElement();
						const normalizedMapping = new _apidomCore.ObjectElement();
						let isNormalized = true;
						const items = (0, _apidomCore.isArrayElement)(schemaElement.oneOf)
							? schemaElement.oneOf
							: (0, _apidomCore.isArrayElement)(schemaElement.anyOf)
								? schemaElement.anyOf
								: allOfMapping.value;
						items.forEach((item) => {
							...
							const metaRefFields = (0, _apidomCore.toValue)(
								item.getMetaProperty("ref-fields"),
							);
							const metaRefOrigin = (0, _apidomCore.toValue)(
								item.getMetaProperty("ref-origin"),
							);
							const metaSchemaName = (0, _apidomCore.toValue)(
								item.getMetaProperty("schemaName"),
							);
							...
							if (
								!hasAllOfMapping &&
								(metaRefOrigin !== baseURI ||
									(!metaSchemaName && metaRefFields))
							) {
								let hasMatchingMapping = false;
								mapping.forEach((mappingValue, mappingKey) => {
									var _mappingValueSchema$g;
									const mappingValueSchema =
										mappingValue.getMetaProperty("ref-schema");
									const mappingValueSchemaRefBaseURI =
										mappingValueSchema == null ||
										(_mappingValueSchema$g =
											mappingValueSchema.getMetaProperty("ref-fields")) == null
											? void 0
											: _mappingValueSchema$g.get("$refBaseURI");
									if (
										mappingValueSchemaRefBaseURI != null &&
										mappingValueSchemaRefBaseURI.equals(
											metaRefFields == null
												? void 0
												: metaRefFields.$refBaseURI,
										)
									) {
										normalizedMapping.set(
											(0, _apidomCore.toValue)(mappingKey),
											(0, _apidomCore.cloneShallow)(item),
										);
							...
							if (metaSchemaName) {
								...
								mapping.forEach((mappingValue, mappingKey) => {
									...
									const mappingValueSchema =
										mappingValue.getMetaProperty("ref-schema");
									const mappingValueSchemaName =
										mappingValueSchema == null
											? void 0
											: mappingValueSchema.getMetaProperty("schemaName");
									const mappingValueSchemaRefBaseURI =
										mappingValueSchema == null ||
										(_mappingValueSchema$g2 =
											mappingValueSchema.getMetaProperty("ref-fields")) == null
											? void 0
											: _mappingValueSchema$g2.get("$refBaseURI");
									if (
										mappingValueSchemaName != null &&
						...
						isNormalized =
							isNormalized &&
							normalizedMapping.filter(
								(mappingValue) =>
									!(0, _predicates.isSchemaElement)(mappingValue),
							).length === 0 &&
							mappingKeys.every((mappingKey) =>
								normalizedMappingKeys.includes(mappingKey),
							);
						if (isNormalized) {
							schemaElement.discriminator.set(
							...
							(0, _apidomCore.visit)(
								...
								{},
								{
									...
									detectCyclesCallback: (node, nodeKey, nodeParent) => {
										if (
											!nodeParent ||
											!(0, _apidomCore.isMemberElement)(node) ||
											!(0, _apidomCore.isStringElement)(node.key) ||
											!node.key.equals("discriminator") ||
											!(0, _apidomNsOpenapi.isDiscriminatorElement)(node.value)
										) {
											return;
										}
										const discriminator = (0, _apidomCore.cloneShallow)(
											node.value,
										);
										...
										if (discriminator.get("propertyName")) {
											discriminatorCopy.propertyName =
												discriminator.get("propertyName");
										}

										// eslint-disable-next-line no-param-reassign
										nodeParent[nodeKey] = new _apidomCore.MemberElement(
											new _apidomCore.StringElement("discriminator"),
											discriminatorCopy,
										);
									},
								},
