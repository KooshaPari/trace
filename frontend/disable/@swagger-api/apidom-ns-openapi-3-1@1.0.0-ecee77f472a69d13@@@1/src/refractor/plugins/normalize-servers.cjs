			...
			visitor: {
				OpenApi3_1Element: {
					enter(openapiElement) {
						const isServersUndefined =
							typeof openapiElement.servers === "undefined";
						const isServersArrayElement = predicates.isArrayElement(
							openapiElement.servers,
						);
						const isServersEmpty =
							isServersArrayElement && openapiElement.servers.length === 0;
						// @ts-ignore
						const defaultServer = namespace.elements.Server.refract({
							url: "/",
						});
						if (isServersUndefined || !isServersArrayElement) {
							openapiElement.servers = new _apidomNsOpenapi.ServersElement([
								defaultServer,
							]);
				...
				PathItemElement(pathItemElement, key, parent, path, ancestors) {
					...
					const parentOpenapiElement = ancestors.find(
						predicates.isOpenApi3_1Element,
					);
					const isServersUndefined =
						typeof pathItemElement.servers === "undefined";
					const isServersArrayElement = predicates.isArrayElement(
						pathItemElement.servers,
					);
					const isServersEmpty =
						isServersArrayElement && pathItemElement.servers.length === 0;
					...
					if (predicates.isOpenApi3_1Element(parentOpenapiElement)) {
						...
						const openapiServersContent =
							(_parentOpenapiElement = parentOpenapiElement.servers) == null
								? void 0
								: _parentOpenapiElement.content;
						const openapiServers =
							openapiServersContent != null ? openapiServersContent : [];
						if (isServersUndefined || !isServersArrayElement) {
							pathItemElement.servers =
								new _apidomNsOpenapi.PathItemServersElement(openapiServers);
						} else if (isServersArrayElement && isServersEmpty) {
				...
				OperationElement(operationElement, key, parent, path, ancestors) {
					...
					const parentPathItemElement = [...ancestors]
						.reverse()
						.find(predicates.isPathItemElement);
					const isServersUndefined =
						typeof operationElement.servers === "undefined";
					const isServersArrayElement = predicates.isArrayElement(
						operationElement.servers,
					);
					const isServersEmpty =
						isServersArrayElement && operationElement.servers.length === 0;
					if (predicates.isPathItemElement(parentPathItemElement)) {
						var _parentPathItemElemen;
						const pathItemServersContent =
							(_parentPathItemElemen = parentPathItemElement.servers) == null
								? void 0
								: _parentPathItemElemen.content;
						const pathItemServers =
							pathItemServersContent != null ? pathItemServersContent : [];
						if (isServersUndefined || !isServersArrayElement) {
							// duplicate parent PathItem.servers into this Operation object
							operationElement.servers =
								new _apidomNsOpenapi.OperationServersElement(pathItemServers);
						} else if (isServersArrayElement && isServersEmpty) {
