...
const createNormalizedOperationId = (path, method) => {
	const normalizedMethod = replaceSpecialCharsWithUnderscore(
		removeSpaces(method.toLowerCase()),
	);
			...
			visitor: {
				OpenApi3_1Element: {
					...
					leave() {
						...
						const normalizedOperationGroups = (0, _ramda.groupBy)(
							(operationElement) => {
						...
						links.forEach((linkElement) => {
							...
							const linkOperationId = String(
								(0, _apidomCore.toValue)(linkElement.operationId),
							);
							const operationElement = normalizedOperations.find(
								(normalizedOperationElement) => {
									const originalOperationId = (0, _apidomCore.toValue)(
							...
							linkElement.operationId = _apidomCore.cloneDeep.safe(
								operationElement.operationId,
							);
				...
				PathItemElement: {
					enter(pathItemElement) {
						...
						const pathTemplate = (0, _ramda.defaultTo)(
							"path",
							(0, _apidomCore.toValue)(pathItemElement.meta.get("path")),
						);
				...
				OperationElement: {
					enter(operationElement, key, parent, path, ancestors) {
						...
						const originalOperationId = String(
							(0, _apidomCore.toValue)(operationElement.operationId),
						);
						...
						const method = (0, _ramda.defaultTo)(
							"method",
							(0, _apidomCore.toValue)(
								operationElement.meta.get("http-method"),
							),
						);
