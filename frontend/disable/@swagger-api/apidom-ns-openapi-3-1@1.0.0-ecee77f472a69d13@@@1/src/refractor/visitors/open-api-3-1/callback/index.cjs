 ...
 * @public
 */
const BaseCallbackVisitor = (exports.BaseCallbackVisitor =
	_apidomNsOpenapi.specificationObj.visitors.document.objects.Callback.$visitor);
/**
 * @public
 */
class CallbackVisitor extends BaseCallbackVisitor {
	constructor(options) {
		...
		this.specPath = (element) => {
			// @ts-ignore
			return (0, _apidomNsOpenapi.isReferenceLikeElement)(element)
				? ["document", "objects", "Reference"]
				: ["document", "objects", "PathItem"];
		};
	}
	ObjectElement(objectElement) {
		const result = BaseCallbackVisitor.prototype.ObjectElement.call(
			this,
			objectElement,
		);
