...
var _TreeCursorSyntaxNode = _interopRequireDefault(
	require("../../TreeCursorSyntaxNode.cjs"),
);
...
class CstVisitor {
	...
	static kindNodeToYamlTag(node) {
		...
		const explicitName =
			(tagNode == null ? void 0 : tagNode.text) ||
			(node.type === "plain_scalar" ? "?" : "!");
		const kind = node.type.endsWith("mapping")
			? _apidomAst.YamlNodeKind.Mapping
			: node.type.endsWith("sequence")
				? _apidomAst.YamlNodeKind.Sequence
				: _apidomAst.YamlNodeKind.Scalar;
	...
	yaml_directive = {
		enter: (node) => {
			...
			const version =
				node == null || (_node$firstNamedChild = node.firstNamedChild) == null
					? void 0
	...
	block_scalar = {
		enter: (node) => {
			...
			const style = node.text.startsWith("|")
				? _apidomAst.YamlStyle.Literal
				: node.text.startsWith(">")
					? _apidomAst.YamlStyle.Folded
					: _apidomAst.YamlStyle.Plain;
			const scalarNode = new _apidomAst.YamlScalar(
				(0, _apidomCore.assignSourceMap)(
					{
						content: node.text,
