...
class TreeCursorIterator {
	...
	block_node() {
		return new _TreeCursorSyntaxNode.default(this.cursor).setFieldName(
			this.cursor,
		);
	}
	flow_node() {
	...
	ERROR() {
		return new _TreeCursorSyntaxNode.default(this.cursor).setHasError(
			this.cursor,
		);
	}
	*[Symbol.iterator]() {
		...
		if (this.cursor.gotoFirstChild()) {
			...
			while (this.cursor.gotoNextSibling()) {
				const firstChildSiblings = Array.from(
					new TreeCursorIterator(this.cursor),
				);
