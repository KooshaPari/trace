...
var _treeSitterYaml = _interopRequireDefault(
	require("../../wasm/tree-sitter-yaml.wasm"),
);
// @ts-ignore
...
const analyze = async (source) => {
	if (parser === null && parserInitLock === null) {
		// acquire lock
		parserInitLock = _webTreeSitter.default
			.init()
			.then(() => _webTreeSitter.default.Language.load(_treeSitterYaml.default))
			.then((jsonLanguage) => {
