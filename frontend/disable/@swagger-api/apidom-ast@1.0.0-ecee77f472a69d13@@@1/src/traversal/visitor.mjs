		...
		enter(node, key, parent, path, ancestors, link) {
			...
			for (let i = 0; i < visitors.length; i += 1) {
				if (skipping[i] === skipSymbol) {
					const visitFn = visitFnGetter(
						visitors[i],
						nodeTypeGetter(currentNode),
						false,
					);
					if (typeof visitFn === "function") {
						...
						if (
							typeof (result === null || result === void 0
								? void 0
...
export const visit = (
	...
	do {
		...
		if (isLeaving) {
			...
			if (isEdited) {
				if (inArray) {
					// @ts-expect-error; creating clone
					node = node.slice();
					let editOffset = 0;
		...
		if (!Array.isArray(node)) {
			...
			// check if the visitor is async
			if (
				typeof ((_result = result) === null || _result === void 0
					? void 0
		...
		if (!isLeaving) {
			...
			keys = inArray
				? node
				: (_visitorKeys$nodeType = visitorKeys[nodeTypeGetter(node)]) !==
							null && _visitorKeys$nodeType !== void 0
					? _visitorKeys$nodeType
					: [];
			index = -1;
			edits = [];
...
visit[Symbol.for("nodejs.util.promisify.custom")] = async (
	...
	do {
		...
		if (!isLeaving) {
			...
			keys = inArray
				? node
				: (_visitorKeys$nodeType2 = visitorKeys[nodeTypeGetter(node)]) !==
							null && _visitorKeys$nodeType2 !== void 0
					? _visitorKeys$nodeType2
					: [];
			index = -1;
			edits = [];
			if (parent !== deleteNodeSymbol && parent !== undefined) {
