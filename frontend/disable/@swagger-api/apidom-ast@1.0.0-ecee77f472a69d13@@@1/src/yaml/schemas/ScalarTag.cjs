...
class ScalarTag {
	static test(node) {
		return (
			node.tag.kind === _YamlTag.YamlNodeKind.Scalar &&
	...
	static canonicalFormat(node) {
		...
		if (node.style === _YamlStyle.YamlStyle.Plain) {
			...
			canonicalForm = (0, _canonicalFormat.formatFlowSingleQuoted)(
				node.content,
			);
		} else if (node.style === _YamlStyle.YamlStyle.DoubleQuoted) {
			canonicalForm = (0, _canonicalFormat.formatFlowDoubleQuoted)(
				node.content,
			);
