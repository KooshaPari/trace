...
var _YamlTagError = _interopRequireDefault(
	require("../../errors/YamlTagError.cjs"),
);
...
class FailsafeSchema {
	...
	toSpecificTagName(node) {
		...
		if (node.tag.explicitName === "!") {
			...
			// verbatim form
			specificTagName = node.tag.explicitName
				.replace(/^!</, "")
				.replace(/>$/, "");
		} else if (node.tag.explicitName.startsWith("!!")) {
			// shorthand notation
	...
	resolve(node) {
		...
		const tag = this.tags.find(
			(itag) => (itag == null ? void 0 : itag.tag) === specificTagName,
		);

		// mechanism for resolving node (tag implementation) not found
		if (typeof tag === "undefined") {
			throw new _YamlTagError.default(
				`Tag "${specificTagName}" was not recognized.`,
				{
					specificTagName,
		...
		if (!tag.test(canonicalNode)) {
			throw new _YamlTagError.default(
				`Node couldn't be resolved against the tag "${specificTagName}"`,
				{
					specificTagName,
					explicitTagName: node.tag.explicitName,
					tagKind: node.tag.kind,
