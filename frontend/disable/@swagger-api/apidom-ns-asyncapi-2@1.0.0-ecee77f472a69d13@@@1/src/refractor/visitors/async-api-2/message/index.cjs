...
var _mediaTypes = _interopRequireDefault(
	require("../../../../media-types.cjs"),
);
var _Message = _interopRequireDefault(
	require("../../../../elements/Message.cjs"),
);
 ...
 * @public
 */
class MessageVisitor extends (0, _tsMixer.Mixin)(
	...
	ObjectElement(objectElement) {
		const result = _FixedFieldsVisitor.default.prototype.ObjectElement.call(
			this,
			objectElement,
		);
		...
		const schemaFormat = (0, _ramda.defaultTo)(
			...
			(0, _apidomCore.toValue)(objectElement.get("schemaFormat")),
		);
		if (
			_mediaTypes.default.includes(schemaFormat) &&
			(0, _predicates.isReferenceLikeElement)(payload)
		) {
			...
			_mediaTypes.default.includes(schemaFormat) &&
			(0, _apidomCore.isObjectElement)(this.element.payload)
		) {
