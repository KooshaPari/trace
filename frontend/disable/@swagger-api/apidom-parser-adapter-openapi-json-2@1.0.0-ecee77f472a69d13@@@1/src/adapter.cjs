...
const detectionRegExp = (exports.detectionRegExp =
	/"swagger"\s*:\s*"(?<version_json>2\.0)"/);

/**
 * @public
 */
const detect = async (source) =>
	detectionRegExp.test(source) &&
	(await (0, _apidomParserAdapterJson.detect)(source));
...
const parse = async (source, options = {}) => {
	...
	const parseResultElement = await (0, _apidomParserAdapterJson.parse)(
		source,
		parserOpts,
	);
	const { result } = parseResultElement;
	if ((0, _ramdaAdjunct.isNotUndefined)(result)) {
		const swaggerElement = _apidomNsOpenapi.SwaggerElement.refract(
			result,
			refractorOpts,
		);
