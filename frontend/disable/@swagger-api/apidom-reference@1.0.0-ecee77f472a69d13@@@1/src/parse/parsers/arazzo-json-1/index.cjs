...
var _apidomParserAdapterArazzoJson = require("@swagger-api/apidom-parser-adapter-arazzo-json-1");
var _ParserError = _interopRequireDefault(
	require("../../../errors/ParserError.cjs"),
);
...
class ArazzoJSON1Parser extends _Parser.default {
	...
	async canParse(file) {
		const hasSupportedFileExtension =
			this.fileExtensions.length === 0
				? true
				: this.fileExtensions.includes(file.extension);
	...
	async parse(file) {
		...
		try {
			const parserOpts = (0, _ramda.pick)(
				["sourceMap", "syntacticAnalysis", "refractorOpts"],
				this,
			);
			return await (0, _apidomParserAdapterArazzoJson.parse)(
				source,
				parserOpts,
			);
		} catch (error) {
