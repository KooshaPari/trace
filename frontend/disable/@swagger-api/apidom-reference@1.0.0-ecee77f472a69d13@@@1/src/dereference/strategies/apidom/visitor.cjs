...
class ApiDOMDereferenceVisitor {
	...
	async RefElement(refElement, key, parent, path, ancestors) {
		...
		let referencedElement = (0, _elementId.evaluate)(
			elementID,
			reference.value.result,
		);
		if (!(0, _apidomCore.isElement)(referencedElement)) {
			throw new _apidomError.ApiDOMError(
				`Referenced element with id="${elementID}" was not found`,
			);
		}
		...
		if ((0, _apidomCore.isRefElement)(referencedElement)) {
			throw new _apidomError.ApiDOMError(
				"RefElement cannot reference another RefElement",
			);
		}
		...
		if (
			referencedElementPath !== "element" &&
			(0, _apidomCore.isElement)(referencedElement)
		) {
			referencedElement = (0, _apidomCore.refract)(
				referencedElement[referencedElementPath],
			);
		}
		...
		if (
			...
			(0, _apidomCore.isObjectElement)(ancestors[ancestors.length - 1]) &&
			...
			(0, _apidomCore.isArrayElement)(referencedElement) &&
			Array.isArray(parent) &&
			typeof key === "number"
		) {
			/**
