...
var _MaximumDereferenceDepthError = _interopRequireDefault(
	require("../../../errors/MaximumDereferenceDepthError.cjs"),
);
var _MaximumResolveDepthError = _interopRequireDefault(
	require("../../../errors/MaximumResolveDepthError.cjs"),
);
...
var _EvaluationJsonSchemaUriError = _interopRequireDefault(
	require("../../../errors/EvaluationJsonSchemaUriError.cjs"),
);
// @ts-ignore
const visitAsync =
	_apidomCore.visit[Symbol.for("nodejs.util.promisify.custom")];
...
class OpenAPI3_1DereferenceVisitor {
	...
	async toReference(uri) {
		...
		if (this.reference.depth >= this.options.resolve.maxDepth) {
			throw new _MaximumResolveDepthError.default(
	...
	toAncestorLineage(ancestors) {
		...
		const ancestorsLineage = new _util2.AncestorLineage(
			...this.ancestors,
			directAncestors,
		);
	...
	OpenApi3_1Element = {
		leave: (openApi3_1Element, key, parent, path, ancestors, link) => {
			...
			if (
				!(
					(_this$options$derefer =
						this.options.dereference.strategyOpts["openapi-3-1"]) != null &&
					_this$options$derefer.dereferenceDiscriminatorMapping
				)
			) {
				return undefined;
			}
			const openApi3_1ElementCopy = (0, _apidomCore.cloneShallow)(
				openApi3_1Element,
			);
	...
	async ReferenceElement(
		...
		const retrievalURI = this.toBaseURI(
			(0, _apidomCore.toValue)(referencingElement.$ref),
		);
		...
		const $refBaseURI = url.resolve(
			retrievalURI,
			(0, _apidomCore.toValue)(referencingElement.$ref),
		);
		this.indirections.push(referencingElement);
		const jsonPointer =
			_modern.URIFragmentIdentifier.fromURIReference($refBaseURI);

		// possibly non-semantic fragment
		let referencedElement = (0, _modern.evaluate)(
		...
		if ((0, _apidomCore.isPrimitiveElement)(referencedElement)) {
			const referencedElementType = (0, _apidomCore.toValue)(
			...
			if (this.refractCache.has(cacheKey)) {
				...
				(0, _apidomNsOpenapi.isReferenceLikeElement)(referencedElement)
			) {
				// handling indirect references
				referencedElement =
					_apidomNsOpenapi.ReferenceElement.refract(referencedElement);
				referencedElement.setMetaProperty(
					"referenced-element",
				...
				const ElementClass = this.namespace.getElementClass(
					referencedElementType,
				);
		...
		if (this.indirections.length > this.options.dereference.maxDepth) {
			throw new _MaximumDereferenceDepthError.default(
		...
		if (ancestorsLineage.includes(referencedElement)) {
			...
			if (this.options.dereference.circular === "error") {
				...
				const replacer =
					(_this$options$derefer2 =
						(_this$options$derefer3 =
							this.options.dereference.strategyOpts["openapi-3-1"]) == null
							? void 0
							: _this$options$derefer3.circularReplacer) != null
						? _this$options$derefer2
		...
		const isNonRootDocument =
			url.stripHash(reference.refSet.rootRef.uri) !== reference.uri;
		...
		if (
			(isExternalReference ||
				isNonRootDocument ||
				(0, _apidomNsOpenapi.isReferenceElement)(referencedElement) ||
		...
		mergedElement.setMetaProperty(
			...
			(0, _apidomCore.cloneDeep)(identityManager.identify(referencingElement)),
		);

		// override description and summary (outer has higher priority then inner)
		if (
			(0, _apidomCore.isObjectElement)(referencedElement) &&
			(0, _apidomCore.isObjectElement)(mergedElement)
		) {
			if (
				referencingElement.hasKey("description") &&
			...
			if (
				referencingElement.hasKey("summary") &&
				"summary" in referencedElement
			) {
	...
	async PathItemElement(
		...
		if ((0, _apidomCore.isPrimitiveElement)(referencedElement)) {
			...
			if (this.refractCache.has(cacheKey)) {
				...
				referencedElement =
					_apidomNsOpenapi.PathItemElement.refract(referencedElement);
				this.refractCache.set(cacheKey, referencedElement);
			}
		}
		...
		// detect direct or indirect reference
		if (referencingElement === referencedElement) {
			throw new _apidomError.ApiDOMError(
		...
		if (ancestorsLineage.includes(referencedElement)) {
			...
			if (this.options.dereference.circular === "error") {
				...
				const replacer =
					(_this$options$derefer4 =
						(_this$options$derefer5 =
							this.options.dereference.strategyOpts["openapi-3-1"]) == null
							? void 0
							: _this$options$derefer5.circularReplacer) != null
						? _this$options$derefer4
						: this.options.dereference.circularReplacer;
				const replacement = replacer(refElement);
				link.replaceWith(replacement, mutationReplacer);
		...
		if (
			(isExternalReference ||
				isNonRootDocument ||
				((0, _apidomNsOpenapi.isPathItemElement)(referencedElement) &&
					(0, _apidomCore.isStringElement)(referencedElement.$ref)) ||
				shouldDetectCircular) &&
			!ancestorsLineage.includesCycle(referencedElement)
		) {
			// append referencing reference to ancestors lineage
		...
		if ((0, _apidomNsOpenapi.isPathItemElement)(referencedElement)) {
			const mergedElement = new _apidomNsOpenapi.PathItemElement(
				[...referencedElement.content],
				(0, _apidomCore.cloneDeep)(referencedElement.meta),
				(0, _apidomCore.cloneDeep)(referencedElement.attributes),
			);
			// assign unique id to merged element
			mergedElement.setMetaProperty("id", identityManager.generateId());
	...
	async LinkElement(linkElement, key, parent, path, ancestors, link) {
		...
		if (
			!(0, _apidomCore.isStringElement)(linkElement.operationRef) &&
			!(0, _apidomCore.isStringElement)(linkElement.operationId)
		) {
			return undefined;
		}
		...
		if ((0, _apidomCore.isStringElement)(linkElement.operationRef)) {
			...
			// possibly non-semantic referenced element
			const jsonPointer = _modern.URIFragmentIdentifier.fromURIReference(
				(0, _apidomCore.toValue)(linkElement.operationRef),
			);
			const retrievalURI = this.toBaseURI(
				(0, _apidomCore.toValue)(linkElement.operationRef),
			);
			...
			operationElement = (0, _modern.evaluate)(
				reference.value.result,
				jsonPointer,
			);
			...
			if ((0, _apidomCore.isPrimitiveElement)(operationElement)) {
				...
				if (this.refractCache.has(cacheKey)) {
					...
					operationElement =
						_apidomNsOpenapi.OperationElement.refract(operationElement);
			...
			(_linkElementCopy$oper = linkElementCopy.operationRef) == null ||
				_linkElementCopy$oper.meta.set("operation", operationElement);
		...
		if ((0, _apidomCore.isStringElement)(linkElement.operationId)) {
			...
			const reference = await this.toReference(
				url.unsanitize(this.reference.uri),
			);
			operationElement = (0, _apidomCore.find)(
				(e) =>
					(0, _apidomNsOpenapi.isOperationElement)(e) &&
					(0, _apidomCore.isElement)(e.operationId) &&
					e.operationId.equals(operationId),
				reference.value.result,
			);
			...
			const linkElementCopy = (0, _apidomCore.cloneShallow)(linkElement);
			(_linkElementCopy$oper2 = linkElementCopy.operationId) == null ||
				_linkElementCopy$oper2.meta.set("operation", operationElement);
	...
	async ExampleElement(exampleElement, key, parent, path, ancestors, link) {
		...
		if (
			exampleElement.hasKey("value") &&
			(0, _apidomCore.isStringElement)(exampleElement.externalValue)
		) {
			throw new _apidomError.ApiDOMError(
		...
		const retrievalURI = this.toBaseURI(
			(0, _apidomCore.toValue)(exampleElement.externalValue),
		);
		const isInternalReference =
			url.stripHash(this.reference.uri) === retrievalURI;
		const isExternalReference = !isInternalReference;
	...
	async MemberElement(memberElement, key, parent, path, ancestors, link) {
		...
		if (
			!(0, _apidomCore.isObjectElement)(parentElement) ||
		...
		if (
			!(
				(_this$options$derefer6 =
					this.options.dereference.strategyOpts["openapi-3-1"]) != null &&
				_this$options$derefer6.dereferenceDiscriminatorMapping
			)
		) {
			return false;
		}
		...
		if (
			!(0, _apidomCore.isStringElement)(memberElement.key) ||
			!(0, _apidomCore.isStringElement)(memberElement.value)
		) {
			return false;
		}
		...
		const parentSchemaElement = [...directAncestors].findLast(
			_apidomNsOpenapi.isSchemaElement,
		);
		const ancestorsSchemaIdentifiers = (0, _apidomCore.cloneDeep)(
	...
	async SchemaElement(referencingElement, key, parent, path, ancestors, link) {
		...
		const $refBaseURI = (0, _util.resolveSchema$refField)(
			retrievalURI,
			referencingElement,
		);
		...
		const isUnknownURI = (0, _ramda.none)(
			(r) => r.canRead(file),
			this.options.resolve.resolvers,
		);
		...
		try {
			...
			if (isURL && error instanceof _EvaluationJsonSchemaUriError.default) {
				if ((0, _$anchor.isAnchor)((0, _$anchor.uriToAnchor)($refBaseURI))) {
					...
					referencedElement = (0, _$anchor.evaluate)(
						selector,
						referenceAsSchema,
					);
					referencedElement = (0, _util.maybeRefractToSchemaElement)(
					...
					referencedElement = (0, _modern.evaluate)(
						referenceAsSchema,
						selector,
					);
					referencedElement = (0, _util.maybeRefractToSchemaElement)(
		...
		if (ancestorsLineage.includes(referencedElement)) {
			...
			if (this.options.dereference.circular === "error") {
				...
				const replacer =
					(_this$options$derefer7 =
						(_this$options$derefer8 =
							this.options.dereference.strategyOpts["openapi-3-1"]) == null
							? void 0
							: _this$options$derefer8.circularReplacer) != null
						? _this$options$derefer7
						: this.options.dereference.circularReplacer;
				const replacement = replacer(refElement);
				link.replaceWith(replacement, mutationReplacer);
		...
		if (
			(isExternalReference ||
				isNonRootDocument ||
				((0, _apidomNsOpenapi.isSchemaElement)(referencedElement) &&
					(0, _apidomCore.isStringElement)(referencedElement.$ref)) ||
				shouldDetectCircular) &&
			!ancestorsLineage.includesCycle(referencedElement)
		) {
		...
		if ((0, _apidomNsOpenapi.isBooleanJsonSchemaElement)(referencedElement)) {
			const booleanJsonSchemaElement = (0, _apidomCore.cloneDeep)(
				referencedElement,
			);
			// assign unique id to merged element
			booleanJsonSchemaElement.setMetaProperty(
				"id",
				identityManager.generateId(),
			);
			// annotate referenced element with info about original referencing element
		...
		if ((0, _apidomNsOpenapi.isSchemaElement)(referencedElement)) {
			...
			const mergedElement = new _apidomNsOpenapi.SchemaElement(
				[...referencedElement.content],
				(0, _apidomCore.cloneDeep)(referencedElement.meta),
				(0, _apidomCore.cloneDeep)(referencedElement.attributes),
			);
			...
			if (
				(_this$options$derefer9 =
					this.options.dereference.strategyOpts["openapi-3-1"]) != null &&
				_this$options$derefer9.dereferenceDiscriminatorMapping
			) {
				...
				const parentSchemaElementName =
					parentSchemaElement == null
						? void 0
						: parentSchemaElement.getMetaProperty("schemaName");
				const mergedElementName = (0, _apidomCore.toValue)(
					mergedElement.getMetaProperty("schemaName"),
				);
				if (
					mergedElementName &&
					parentSchemaElementName && // @ts-ignore
					parentElement != null &&
					(_parentElement$classe = parentElement.classes) != null &&
					_parentElement$classe.contains("json-schema-allOf")
				) {
					var _this$allOfDiscrimina;
					const currentMapping =
						(_this$allOfDiscrimina =
							this.allOfDiscriminatorMapping.get(mergedElementName)) != null
							? _this$allOfDiscrimina
							: [];
