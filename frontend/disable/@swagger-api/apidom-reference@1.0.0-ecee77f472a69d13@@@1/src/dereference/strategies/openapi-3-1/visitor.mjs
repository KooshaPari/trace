...
class OpenAPI3_1DereferenceVisitor {
	...
	toAncestorLineage(ancestors) {
		...
		const ancestorsLineage = new AncestorLineage(
			...this.ancestors,
			directAncestors,
		);
	...
	async ReferenceElement(
		...
		const $refBaseURI = url.resolve(
			retrievalURI,
			toValue(referencingElement.$ref),
		);
				...
				const replacer =
					(_this$options$derefer2 =
						(_this$options$derefer3 =
							this.options.dereference.strategyOpts["openapi-3-1"]) === null ||
						_this$options$derefer3 === void 0
							? void 0
							: _this$options$derefer3.circularReplacer) !== null &&
					_this$options$derefer2 !== void 0
						? _this$options$derefer2
	...
	async PathItemElement(
				...
				const replacer =
					(_this$options$derefer4 =
						(_this$options$derefer5 =
							this.options.dereference.strategyOpts["openapi-3-1"]) === null ||
						_this$options$derefer5 === void 0
							? void 0
							: _this$options$derefer5.circularReplacer) !== null &&
					_this$options$derefer4 !== void 0
						? _this$options$derefer4
						: this.options.dereference.circularReplacer;
				const replacement = replacer(refElement);
				link.replaceWith(replacement, mutationReplacer);
		...
		if (
			(isExternalReference ||
				isNonRootDocument ||
				(isPathItemElement(referencedElement) &&
	...
	async LinkElement(linkElement, key, parent, path, ancestors, link) {
		...
		if (
			!isStringElement(linkElement.operationRef) &&
			!isStringElement(linkElement.operationId)
		) {
			return undefined;
		}
		...
		if (isStringElement(linkElement.operationRef)) {
			...
			const jsonPointer = URIFragmentIdentifier.fromURIReference(
				toValue(linkElement.operationRef),
			);
			...
			operationElement = jsonPointerEvaluate(
				reference.value.result,
				jsonPointer,
			);
		...
		if (isStringElement(linkElement.operationId)) {
			...
			operationElement = find(
				(e) =>
					isOperationElement(e) &&
					isElement(e.operationId) &&
	...
	async ExampleElement(exampleElement, key, parent, path, ancestors, link) {
		...
		if (
			...
			isStringElement(exampleElement.externalValue)
		) {
			throw new ApiDOMError(
		...
		const reference = await this.toReference(
			toValue(exampleElement.externalValue),
		);
	...
	async MemberElement(memberElement, key, parent, path, ancestors, link) {
		...
		if (
			!isStringElement(memberElement.key) ||
			!isStringElement(memberElement.value)
		) {
			return false;
		}
	...
	async SchemaElement(referencingElement, key, parent, path, ancestors, link) {
		...
		const $refBaseURI = resolveSchema$refField(
			retrievalURI,
			referencingElement,
		);
				...
				const replacer =
					(_this$options$derefer7 =
						(_this$options$derefer8 =
							this.options.dereference.strategyOpts["openapi-3-1"]) === null ||
						_this$options$derefer8 === void 0
							? void 0
							: _this$options$derefer8.circularReplacer) !== null &&
					_this$options$derefer7 !== void 0
						? _this$options$derefer7
						: this.options.dereference.circularReplacer;
				const replacement = replacer(refElement);
				link.replaceWith(replacement, mutationReplacer);
		...
		if (isSchemaElement(referencedElement)) {
			...
			if (
				...
				const parentSchemaElementName =
					parentSchemaElement === null || parentSchemaElement === void 0
						? void 0
						: parentSchemaElement.getMetaProperty("schemaName");
				const mergedElementName = toValue(
