...
const evaluate = (uri, element) => {
	...
	const isSchemaElementWith$id = (e) =>
		(0, _apidomNsOpenapi.isSchemaElement)(e) && typeof e.$id !== "undefined";

	// warm the cache
	if (!cache.has(element)) {
		const schemaObjectElements = (0, _apidomCore.filter)(
	...
	if ((0, _ramdaAdjunct.isUndefined)(result)) {
		throw new _EvaluationJsonSchemaUriError.default(
	...
	return (0, _modern.evaluate)(
		result,
		_modern.URIFragmentIdentifier.fromURIReference(uri),
	);
};
