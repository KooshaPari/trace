...
const evaluate = (anchor, element) => {
	...
	const result = (0, _apidomCore.find)(
		(e) =>
			(0, _apidomNsOpenapi.isSchemaElement)(e) &&
			(0, _apidomCore.toValue)(e.$anchor) === token,
		element,
	);
	if ((0, _ramdaAdjunct.isUndefined)(result)) {
		throw new _EvaluationJsonSchema$anchorError.default(
			`Evaluation failed on token: "${token}"`,
		);
	}
