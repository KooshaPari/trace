	...
	async ReferenceElement(
		...
		if ((0, _apidomCore.isPrimitiveElement)(referencedElement)) {
			...
			if (this.refractCache.has(cacheKey)) {
				...
				(0, _apidomNsAsyncapi.isReferenceLikeElement)(referencedElement)
			) {
				// handling indirect references
				referencedElement =
					_apidomNsAsyncapi.ReferenceElement.refract(referencedElement);
				...
				const replacer =
					(_this$options$derefer =
						(_this$options$derefer2 =
							this.options.dereference.strategyOpts["asyncapi-2"]) == null
							? void 0
		...
		if (
			(isExternalReference ||
				isNonRootDocument ||
				(0, _apidomNsAsyncapi.isReferenceElement)(referencedElement) ||
	...
	async ChannelItemElement(
		...
		if ((0, _apidomCore.isPrimitiveElement)(referencedElement)) {
			...
			if (this.refractCache.has(cacheKey)) {
				...
				referencedElement =
					_apidomNsAsyncapi.ChannelItemElement.refract(referencedElement);
				this.refractCache.set(cacheKey, referencedElement);
			}
		}
		...
		if (referencingElement === referencedElement) {
			throw new _apidomError.ApiDOMError(
				"Recursive Channel Item Object reference detected",
			);
		}
		...
		if (
			(isExternalReference ||
				isNonRootDocument ||
				((0, _apidomNsAsyncapi.isChannelItemElement)(referencedElement) &&
		...
		if ((0, _apidomNsAsyncapi.isChannelItemElement)(referencedElement)) {
			const mergedElement = new _apidomNsAsyncapi.ChannelItemElement(
