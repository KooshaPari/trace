...
class HTTPResolverAxios extends _HTTPResolver.default {
	...
	previousAxiosConfig;
	constructor(options) {
		const { axiosConfig = {}, ...rest } = options != null ? options : {};
		super({
			...rest,
			name: "http-axios",
		});
	...
	getHttpClient() {
		if (
			...
			if (Array.isArray(interceptors == null ? void 0 : interceptors.request)) {
				interceptors.request.forEach((requestInterceptor) => {
					this.axiosInstance.interceptors.request.use(
						...(0, _ramdaAdjunct.ensureArray)(requestInterceptor),
					);
				});
			}
			...
			if (
				Array.isArray(interceptors == null ? void 0 : interceptors.response)
			) {
				interceptors.response.forEach((responseInterceptor) => {
					this.axiosInstance.interceptors.response.use(
						...(0, _ramdaAdjunct.ensureArray)(responseInterceptor),
					);
				});
			}
			this.previousAxiosConfig = this.axiosConfig;
