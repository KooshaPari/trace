...
var _DeepCloneError = _interopRequireDefault(
	require("./errors/DeepCloneError.cjs"),
);
var _ShallowCloneError = _interopRequireDefault(
	require("./errors/ShallowCloneError.cjs"),
);
...
const cloneDeep = (value, options = {}) => {
	...
	if (value instanceof _minim.KeyValuePair) {
		...
		const keyCopy = (0, _index.isElement)(key)
			? cloneDeep(key, passThroughOptions)
			: key;
		const valueCopy = (0, _index.isElement)(val)
			? cloneDeep(val, passThroughOptions)
			: val;
	...
	throw new _DeepCloneError.default(
...
const cloneShallow = (value) => {
	...
	throw new _ShallowCloneError.default(
