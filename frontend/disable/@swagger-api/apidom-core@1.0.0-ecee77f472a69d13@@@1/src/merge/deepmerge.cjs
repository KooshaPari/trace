...
const emptyElement = (element) => {
	const meta =
		element.meta.length > 0 ? (0, _index2.cloneDeep)(element.meta) : undefined;
	const attributes =
		element.attributes.length > 0
			? (0, _index2.cloneDeep)(element.attributes)
			: undefined;
...
const cloneUnlessOtherwiseSpecified = (element, options) =>
	options.clone && options.isMergeableElement(element)
		? deepmerge(emptyElement(element), element, options)
		: element;
...
const mergeArrayElement = (targetElement, sourceElement, options) =>
	targetElement
		...
		["fantasy-land/map"]((item) =>
			cloneUnlessOtherwiseSpecified(item, options),
		);
const mergeObjectElement = (targetElement, sourceElement, options) => {
	const destination = (0, _index.isObjectElement)(targetElement)
	...
	sourceElement.forEach((value, key, member) => {
		...
		let clonedMember;
		if (
			(0, _index.isObjectElement)(targetElement) &&
...
const defaultOptions = (exports.defaultOptions = {
	clone: true,
	isMergeableElement: (element) =>
		(0, _index.isObjectElement)(element) || (0, _index.isArrayElement)(element),
...
const deepmerge = (targetElement, sourceElement, options) => {
	...
	mergedOptions.isMergeableElement =
		(_mergedOptions$isMerg = mergedOptions.isMergeableElement) != null
			? _mergedOptions$isMerg
			: defaultOptions.isMergeableElement;
	mergedOptions.arrayElementMerge =
		(_mergedOptions$arrayE = mergedOptions.arrayElementMerge) != null
			? _mergedOptions$arrayE
			: defaultOptions.arrayElementMerge;
	mergedOptions.objectElementMerge =
		(_mergedOptions$object = mergedOptions.objectElementMerge) != null
			? _mergedOptions$object
			: defaultOptions.objectElementMerge;
	...
	const sourceAndTargetTypesMatch =
		sourceIsArrayElement === targetIsArrayElement;
	...
	const mergedElement =
		sourceIsArrayElement &&
		typeof mergedOptions.arrayElementMerge === "function"
			? mergedOptions.arrayElementMerge(
					targetElement,
					sourceElement,
					mergedOptions,
