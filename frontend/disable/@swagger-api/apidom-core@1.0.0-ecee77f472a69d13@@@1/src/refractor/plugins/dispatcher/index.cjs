...
const dispatchPluginsSync = (element, plugins, options = {}) => {
	...
	const mergedOptions = (0, _ramda.mergeDeepRight)(
	...
	const mergedPluginsVisitor = (0, _visitor.mergeAllVisitors)(
		pluginsSpecs.map((0, _ramda.propOr)({}, "visitor")),
		{
			...visitorOptions,
		},
	);
	pluginsSpecs.forEach((0, _ramdaAdjunct.invokeArgs)(["pre"], []));
	const newElement = (0, _visitor.visit)(
		element,
		mergedPluginsVisitor,
		visitorOptions,
	);
...
const dispatchPluginsAsync = async (element, plugins, options = {}) => {
	...
	const toolbox = toolboxCreator();
	const pluginsSpecs = plugins.map((plugin) => plugin(toolbox));
	const mergeAllVisitorsAsync =
		_visitor.mergeAllVisitors[Symbol.for("nodejs.util.promisify.custom")];
	const visitAsync = _visitor.visit[Symbol.for("nodejs.util.promisify.custom")];
	const mergedPluginsVisitor = mergeAllVisitorsAsync(
		pluginsSpecs.map((0, _ramda.propOr)({}, "visitor")),
		{
			...visitorOptions,
		},
	...
	await Promise.allSettled(
		pluginsSpecs.map((0, _ramdaAdjunct.invokeArgs)(["pre"], [])),
	);
	const newElement = await visitAsync(
		element,
		mergedPluginsVisitor,
		visitorOptions,
	);
	await Promise.allSettled(
		pluginsSpecs.map((0, _ramdaAdjunct.invokeArgs)(["post"], [])),
	);
	return newElement;
};
