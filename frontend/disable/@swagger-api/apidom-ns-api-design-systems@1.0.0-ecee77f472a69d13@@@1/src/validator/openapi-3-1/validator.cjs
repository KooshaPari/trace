...
const makeMessage = (value, requirementLevel, standardIdentifier) => {
	...
	throw new _apidomError.NotImplementedError(
		'[Requirement levels] other than "may" are not yet implemented.',
	);
};
...
const makeAnnotation = (message, value, level, standardIdentifier) => {
	...
	annotation.attributes.set(
		"standardIdentifier",
		(0, _apidomCore.cloneDeep)(standardIdentifier),
	);
	return annotation;
};
const validateValue = (value, requirement) => {
	...
	if ((0, _apidomCore.toValue)(requirement.level) === "may") {
		const isValid = (0, _requirementLevel.may)(
			(0, _apidomCore.toValue)(value),
			(0, _apidomCore.toValue)(requirement.values),
		);
		if (!isValid) {
...
const validate = (mainElement, openApiElement) => {
	...
	if (
		...
		!(0, _apidomCore.isArrayElement)(scenarios)
	)
		return [];
