		...
		visitor: {
				...
				enter(element, ...rest) {
					...
					operationIdentifiers.push(
						...
						[
							...
							"request",
							"url",
							toValue(parentPathItem.meta.get("path")),
						],
						["http", "request", "method"],
						[
							"http",
							"request",
							"method",
							toValue(element.meta.get("http-method")).toLowerCase(),
						],
					);
			...
			RequestBodyElement(element) {
				...
				if (
					...
					isObjectElement(element.contentProp)
				) {
				...
				enter(element) {
					...
					if (element.meta.hasKey("http-status-code")) {
						const statusCode = String(
							toValue(element.meta.get("http-status-code")),
						);
					...
					if (
						...
						isObjectElement(element.headers)
					) {
