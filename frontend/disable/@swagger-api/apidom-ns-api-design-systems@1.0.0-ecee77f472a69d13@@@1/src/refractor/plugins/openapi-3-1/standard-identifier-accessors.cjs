	...
	return {
		visitor: {
			OperationElement(element, ...rest) {
				...
				const httpMethod = (0, _apidomCore.cloneDeep)(
				...
				if (
					...
					(0, _apidomNsOpenapi.isArrayElement)(parentPathItem.parameters)
				) {
					// @ts-ignore
			...
			RequestBodyElement(element) {
				...
				element.contentProp.forEach((mediaType, key) => {
					standardIdentifiers.push(
						{
							subject: ["http", "request", "header"],
							value: new _apidomCore.StringElement(
								"Content-Type",
								(0, _apidomCore.cloneDeep)(key.meta),
							),
						},
						{
							subject: ["http", "message", "header"],
				...
				const standardIdentifiers = [
					{
						...
						value: (0, _apidomCore.cloneDeep)(
							element.meta.get("http-status-code"),
						),
					},
				];
				element.setMetaProperty(
