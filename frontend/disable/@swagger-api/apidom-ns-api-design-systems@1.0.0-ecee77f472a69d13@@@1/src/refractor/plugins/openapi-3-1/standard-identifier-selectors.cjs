	...
	return {
		visitor: {
				...
				enter(element, ...rest) {
					...
					operationIdentifiers.push(
						...
						[
							...
							"request",
							"url",
							(0, _apidomCore.toValue)(parentPathItem.meta.get("path")),
						],
						["http", "request", "method"],
						[
							"http",
					...
					// @ts-ignore
					parentPathItem == null ||
						(_parentPathItem$param = parentPathItem.parameters) == null ||
						_parentPathItem$param.forEach((parameter) => {
							if (
								(0, _apidomNsOpenapi.isStringElement)(parameter.in) &&
								(0, _apidomNsOpenapi.isStringElement)(parameter.name) &&
								(0, _apidomCore.toValue)(parameter.in) === "header"
							) {
								operationIdentifiers.push(
									["http", "request", "header"],
									[
										...
										"request",
										"header",
										(0, _apidomCore.toValue)(parameter.name),
									],
									["http", "message", "header"],
									[
										"http",
			...
			ParameterElement(element) {
				if (
					(0, _apidomNsOpenapi.isStringElement)(element.in) &&
					(0, _apidomNsOpenapi.isStringElement)(element.name) &&
					(0, _apidomCore.toValue)(element.in) === "header"
				) {
					operationIdentifiers.push(
						["http", "request", "header"],
						[
							...
							"request",
							"header",
							(0, _apidomCore.toValue)(element.name),
						],
						["http", "message", "header"],
						[
							"http",
			...
			RequestBodyElement(element) {
				...
				if (
					typeof element.contentProp !== "undefined" &&
					(0, _apidomNsOpenapi.isObjectElement)(element.contentProp)
				) {
				...
				enter(element) {
					...
					if (element.meta.hasKey("http-status-code")) {
						const statusCode = String(
							(0, _apidomCore.toValue)(element.meta.get("http-status-code")),
						);
					...
					if (
						typeof element.headers !== "undefined" &&
						(0, _apidomNsOpenapi.isObjectElement)(element.headers)
					) {
