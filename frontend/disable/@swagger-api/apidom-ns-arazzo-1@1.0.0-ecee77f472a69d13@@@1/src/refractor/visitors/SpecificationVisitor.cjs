...
class SpecificationVisitor extends _Visitor.default {
	...
	retrieveFixedFields(specPath) {
		const fixedFields = (0, _ramda.path)(
			["visitors", ...specPath, "fixedFields"],
			this.specObj,
		);
	...
	retrieveVisitor(specPath) {
		...
		return (0, _ramda.path)(
			["visitors", ...specPath, "$visitor"],
			this.specObj,
		);
	}
	...
	toRefractedElement(specPath, element, options = {}) {
		...
		if (
			visitor instanceof _FallbackVisitor.default &&
			(visitor == null ? void 0 : visitor.constructor) ===
