...
suite("Loading a single document", () => {
	test("should work with document-end delimiters", () => {
		const input = `---
whatever: true
...`;
		const doc = YAML.safeLoad(input);
		const expected_structure = YAML.newMap([
			YAML.newMapping(YAML.newScalar("whatever"), YAML.newScalar("true")),
		]);
	...
	test("Document end position should be equal to input length", () => {
		const input = `
outer:
inner:
    `;
...
...`,
		...
		const expected_structure = [
			...
			YAML.newMap([
				YAML.newMapping(YAML.newScalar("whatever"), YAML.newScalar("false")),
			]),
		];

		assert.deepEqual(
			docs.map((d) => structure(d)),
			expected_structure,
		);
	...
	test("Last document end position should be equal to input length", () => {
		const input = `
outer1:
inner1:
...
