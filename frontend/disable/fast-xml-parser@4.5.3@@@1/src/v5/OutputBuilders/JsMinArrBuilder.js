const {
	buildOptions,
	registerCommonValueParsers,
} = require("./ParserOptionsBuilder");
...
class OutputBuilder {
	...
	registerValueParser(name, parserInstance) {
		//existing name will override the parser without warning
		this.registeredParsers[name] = parserInstance;
	}

	getInstance(parserOptions) {
		return new JsMinArrBuilder(
			parserOptions,
			this.options,
...
class JsMinArrBuilder extends BaseOutputBuilder {
	...
	closeTag() {
		...
		if (this.options.onClose !== undefined) {
			//TODO TagPathMatcher
			const resultTag = this.options.onClose(
				node,
	...
	addValue(text) {
		this.currentNode[this.currentNodeTagName].push({
			[this.options.nameFor.text]: this.parseValue(
				text,
				this.options.tags.valueParsers,
			),
		});
	}
