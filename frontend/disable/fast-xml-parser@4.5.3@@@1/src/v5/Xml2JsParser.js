...
class Xml2JsParser {
	...
	parseXml() {
		...
		if (
			...
			throw new Error("Unexpected data in the end of document");
	}
	...
	validateClosingTag(tagName) {
		...
		if (this.isUnpaired(tagName) || this.isStopNode(tagName))
	...
	readSpecialTag(startCh) {
		if (startCh == "!") {
			...
			if (nextChar === null || nextChar === undefined)
				throw new Error("Unexpected ending of the source");
			...
			throw new Error(
				`Invalid tag '<${startCh}' at ${this.source.line}:${this.source.col}`,
			);
		}
	}
