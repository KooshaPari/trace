...
function composeObjectVisitors(res, visitors, key, apply, wrapKey) {
	const [values, hasFunction, allKeys] = extractObjectsOrFunctions(
		visitors,
		key,
	);
	if (values.length === 0) {
	...
	const f = createArrayVisitor(visitors, (visitor, item) =>
...
function composeTokenVisitors(res, visitors, key, type, isExit) {
	...
	const f = createTokenVisitor(visitors, type, isExit);
	if (hasFunction) {
		res[key] = f;
	} else {
		const v = {};
...
function composeSimpleVisitors(res, visitors, key) {
	const functions = extractFunctions(visitors, key);
...
function createArrayVisitor(visitors, apply) {
	const seen = new Bitset(visitors.length);
	return (arg) => {
		const arr = [arg];
		let mutated = false;
		seen.clear();
