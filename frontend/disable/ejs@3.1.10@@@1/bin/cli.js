...
const program = require("jake").program;
...
const { hyphenToCamel } = require("../lib/utils");
const fs = require("fs");
...
const CLI_OPTS = [
	...
	{ full: "delimiter", abbr: "m", expectValue: true, passThrough: true },
	{ full: "open-delimiter", abbr: "p", expectValue: true, passThrough: true },
	{ full: "close-delimiter", abbr: "c", expectValue: true, passThrough: true },
	{
		full: "strict",
		abbr: "s",
		expectValue: false,
	...
	{ full: "no-with", abbr: "n", expectValue: false, allowValue: false },
	{ full: "locals-name", abbr: "l", expectValue: true, passThrough: true },
	{
		full: "rm-whitespace",
		abbr: "w",
		expectValue: false,
	...
	{ full: "help", abbr: "h", passThrough: true },
	{ full: "version", abbr: "V", passThrough: true },
	// Alias lowercase v
	{ full: "version", abbr: "v", passThrough: true },
];
...
function run() {
	...
	const templatePath = program.taskNames[0];
	const pVals = program.envVars;
	const pOpts = {};

	for (const p in program.opts) {
		const name = hyphenToCamel(p);
	...
	CLI_OPTS.forEach((opt) => {
		const optName = hyphenToCamel(opt.full);
	...
	let input;
	const err = new Error(
		"Please do not pass data multiple ways. Pick one of stdin, -f, or -i.",
	);
	if (stdin) {
		input = stdin;
	...
	for (const p in pVals) {
		vals[p] = pVals[p];
	}

	opts.filename = path.resolve(process.cwd(), templatePath);
	const template = fs.readFileSync(opts.filename).toString();
	const output = ejs.render(template, vals, opts);
	if (pOpts.outputFile) {
