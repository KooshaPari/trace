...
class ProcessInfo {
	...
	async getCoverageMap(nyc) {
		...
		const childMaps = await Promise.all(
			this.nodes.map((child) => child.getCoverageMap(nyc)),
		);
...
class ProcessDB {
	constructor(directory) {
		if (!directory) {
			...
			if (!directory) {
				throw new TypeError(
					"must provide directory argument when outside of NYC",
				);
			}
		}
	...
	async _readJSON(file) {
		if (Array.isArray(file)) {
			const result = await pMap(file, (f) => this._readJSON(f), {
				concurrency: 8,
			});
		...
		try {
			return JSON.parse(
				await fs.readFile(resolve(this.directory, file), "utf-8"),
			);
		} catch (error) {}
	}
	...
	async readProcessInfos() {
		...
		const fileData = await this._readJSON(
			files.filter((f) => f !== "index.json"),
		);

		return fromEntries(
			fileData.map((info) => [info.uuid, new ProcessInfo(info)]),
		);
	}

	_createIndex(infos) {
		const infoMap = fromEntries(
			infos.map((info) => [info.uuid, Object.assign(info, { children: [] })]),
		);
		...
		const externalIds = fromEntries(
			infos
				.filter((info) => info.externalId)
				.map((info) => {
					...
					for (let i = 0; i < children.length; i++) {
						children.push(
							...processes[children[i]].children.filter(
								(uuid) => !children.includes(uuid),
							),
						);
					}
