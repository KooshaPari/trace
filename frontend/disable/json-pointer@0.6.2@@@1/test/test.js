...
describe("json-api", () => {
	...
	describe("#get", () => {
		...
		each(Object.keys(rfcParsed), (p) => {
			var tokens = immutable(rfcParsed[p].tokens);
			it("should work for " + JSON.stringify(tokens), () => {
	...
	describe("#remove", () => {
		...
		each(Object.keys(rfcParsed), (p) => {
			...
			it("should work for " + JSON.stringify(rfcParsed[p].tokens), () => {
	...
	describe("#walk", () => {
		it("should iterate over an object", () => {
			pointer.walk({ bla: { test: "expected" } }, (value, pointer) => {
	...
	describe("#compile", () => {
		it("should build a json pointer from an array of reference tokens", () => {
			pointer
				.compile(["hello~bla", "test/bla"])
				.should.equal("/hello~0bla/test~1bla");
		});
	});
...
describe("convenience api wrapper", () => {
	it("should call #get when passed 2 args", () => {
		var obj = {
			existing: "expected",
		};
	...
	it("should call #get when passed 2 args with tokens", () => {
		var obj = {
			existing: "expected",
		};
	...
	it("should call #set when passed 3 args", () => {
		var obj = {
			existing: "bla",
		};
	...
	it("should call #set when passed 3 args with tokens", () => {
		var obj = {
			existing: "bla",
		};
	...
	it("should return a partially applied function when passed 1 arg", () => {
		var obj = {
				existing: "bla",
			},
	...
	it("should support chainable oo-style", () => {
		var obj = {
				existing: "bla",
			},
			objPointer = pointer(obj);
	...
	it("should support chainable oo-style with tokens", () => {
		var obj = {
				existing: "bla",
			},
			objPointer = pointer(obj);
	...
	it("should not set __proto__ (array)", () => {
		var obj = {},
			objPointer = pointer(obj);
		expect(obj.polluted).to.be.undefined();
		objPointer.set([["__proto__"], "polluted"], true);
