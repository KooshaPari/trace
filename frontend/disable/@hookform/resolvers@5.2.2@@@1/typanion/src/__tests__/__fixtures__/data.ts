import type { Field, InternalFieldName } from "react-hook-form";
...
export const schema = t.isObject({
	...
	password: t.cascade(t.isString(), [
		t.matchesRegExp(/.*[A-Z].*/), // one uppercase character
		t.matchesRegExp(/.*[a-z].*/), // one lowercase character
		t.matchesRegExp(/.*\d.*/), // one number
		t.matchesRegExp(/.*[`~<>?,./!@#$%^&*()\-_+="'|{}[\];:\\].*/), // one special character
