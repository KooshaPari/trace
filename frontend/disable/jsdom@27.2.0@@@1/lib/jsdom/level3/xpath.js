...
module.exports = (core) => {
   ...
   */
	...
	var sortUniqDocumentOrder = (xpath.sortUniqDocumentOrder = (nodes) => {
		var a = [];
	...
	var fn = {
		...
		translate: function translate(string, from, to) {
			...
			return sString.replace(re, (c) => map[c]);
		},
		/// Boolean functions
		not: function not(x) {
			var bx = fn["boolean"](x);
	 ...
	 ***************************************************************************/
	var more = {
		UnaryMinus: (x) => -fn.number(x),
		"+": (x, y) => fn.number(x) + fn.number(y),
		"-": (x, y) => fn.number(x) - fn.number(y),
		"*": (x, y) => fn.number(x) * fn.number(y),
		div: (x, y) => fn.number(x) / fn.number(y),
		mod: (x, y) => fn.number(x) % fn.number(y),
		"<": (x, y) =>
			comparisonHelper((x, y) => fn.number(x) < fn.number(y), x, y, true),
		"<=": (x, y) =>
			comparisonHelper((x, y) => fn.number(x) <= fn.number(y), x, y, true),
		">": (x, y) =>
			comparisonHelper((x, y) => fn.number(x) > fn.number(y), x, y, true),
		">=": (x, y) =>
			comparisonHelper((x, y) => fn.number(x) >= fn.number(y), x, y, true),
		and: (x, y) => fn["boolean"](x) && fn["boolean"](y),
		or: (x, y) => fn["boolean"](x) || fn["boolean"](y),
		"|": (x, y) => ({ nodes: mergeNodeLists(x.nodes, y.nodes) }),
		"=": (x, y) => {
			...
			if ("object" === typeof x && "object" === typeof y) {
				...
				return comparisonHelper((x, y) => x === y, x, y);
			}
		},
		"!=": (x, y) => {
			// optimization for two node-sets case: avoid n^2 comparisons.
	...
	XPathEvaluator.prototype = {
		createExpression: (expression, resolver) =>
			new XPathExpression(expression, resolver),
