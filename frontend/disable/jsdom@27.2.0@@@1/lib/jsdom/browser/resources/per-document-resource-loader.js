...
module.exports = class PerDocumentResourceLoader {
	constructor(document) {
		...
		this._resourceLoader = document._defaultView
			? document._defaultView._resourceLoader
			: null;
	...
	fetch(url, { element, onLoad, onError }) {
		...
		const onErrorWrapped = (cause) => {
			...
			const jsomError = new Error(
				`Could not load ${element.localName}: "${url}"`,
				{ cause },
			);
		...
		if (
			...
			element.hasAttributeNS(null, "async")
		) {
			this._asyncQueue.push(
				request,
				onLoadWrapped,
				onErrorWrapped,
				this._queue.getLastScript(),
			);
		} else if (
			element.localName === "script" &&
