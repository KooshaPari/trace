...
class SelectionImpl {
	...
	extend(node, offset) {
		...
		this._direction =
			compareBoundaryPointsPosition(newFocus, oldAnchor) === -1
	...
	setBaseAndExtent(anchorNode, anchorOffset, focusNode, focusOffset) {
		...
		this._direction =
			compareBoundaryPointsPosition(focus, anchor) === -1
				? SELECTION_DIRECTION.BACKWARDS
				: SELECTION_DIRECTION.FORWARDS;
	}
	...
	containsNode(node, allowPartialContainment) {
		if (
			this._isEmpty() ||
			nodeRoot(node) !== implForWrapper(this._globalObject._document)
		) {
			return false;
		}
		...
		const startIsBeforeNode =
			compareBoundaryPointsPosition(_start, { node, offset: 0 }) === -1;
		const endIsAfterNode =
			compareBoundaryPointsPosition(_end, {
				node,
	...
	_associateRange(newRange) {
		const didSelectionChange =
			...
			(newRange === null ||
				...
				compareBoundaryPointsPosition(newRange._start, this._range._start) !==
					0 ||
				compareBoundaryPointsPosition(newRange._end, this._range._end) !== 0);
		this._range = newRange;
		this._direction =
			newRange === null
				? SELECTION_DIRECTION.DIRECTIONLESS
				: SELECTION_DIRECTION.FORWARDS;
