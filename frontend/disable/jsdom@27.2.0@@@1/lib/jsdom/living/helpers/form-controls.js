...
exports.isDisabled = (formControl) => {
	...
	while (e) {
		if (e.localName === "fieldset" && e.hasAttributeNS(null, "disabled")) {
			...
			if (
				!firstLegendElementChild ||
				!firstLegendElementChild.contains(formControl)
			) {
				return true;
			}
		}
...
exports.isListed = (formControl) =>
	listedElements.has(formControl._localName) &&
...
exports.isSubmittable = (formControl) => {
	return (
		submittableElements.has(formControl._localName) &&
		formControl.namespaceURI === HTML_NS
	);
};
...
exports.isSubmitButton = (formControl) => {
	return (
		((formControl._localName === "input" &&
			submitButtonInputTypes.has(formControl.type)) ||
...
exports.isButton = (formControl) => {
	return (
		((formControl._localName === "input" &&
			buttonInputTypes.has(formControl.type)) ||
...
exports.isValidEmailAddress = (emailAddress, multiple = false) => {
	...
	if (multiple) {
		return splitOnCommas(emailAddress).every((value) =>
			emailAddressRegExp.test(value),
		);
	}
...
exports.sanitizeValueByType = (input, val) => {
	switch (input.type.toLowerCase()) {
		...
		case "datetime-local": {
			...
			val =
				dateAndTime !== null ? serializeNormalizedDateAndTime(dateAndTime) : "";
			break;
		}
		...
		case "email":
			...
			if (input.hasAttributeNS(null, "multiple")) {
				val = val
					.split(",")
					.map((token) => stripLeadingAndTrailingASCIIWhitespace(token))
					.join(",");
			} else {
				val = stripNewlines(val);
		...
		case "number":
			...
			// Possible spec bug.
			if (
				!isValidFloatingPointNumber(val) ||
				parseFloatingPointNumber(val) === null
			) {
				val = "";
			}
			break;
		...
		case "range":
			...
			if (
				...
				const defaultValue =
					maximum < minimum ? minimum : (minimum + maximum) / 2;
				val = `${defaultValue}`;
