...
function getValidTagNames(namespace, name) {
	if (
		INTERFACE_TAG_MAPPING[namespace] &&
		INTERFACE_TAG_MAPPING[namespace][name]
	) {
	...
	if (definition !== null && definition.name !== localName) {
		...
		if (synchronousCE) {
			try {
				...
				if (
					!result._ceState ||
					!result._ceDefinition ||
					result._namespaceURI !== HTML_NS
				) {
				...
				const interfaceWrapper =
					interfaces.getInterfaceWrapper("HTMLUnknownElement");
		...
		if (
			namespace === HTML_NS &&
			(isValidCustomElementName(localName) || isValue !== null)
		) {
...
function internalCreateElementNSSteps(
	document,
	namespace,
	qualifiedName,
	options,
