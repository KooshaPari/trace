...
class RangeImpl extends AbstractRangeImpl {
	constructor(globalObject, args, privateData) {
		...
		const { start = defaultBoundaryPoint, end = defaultBoundaryPoint } =
			privateData;
	...
	deleteContents() {
		...
		if (
			...
			originalStart.node.replaceData(
		...
		if (
			...
			originalStart.node.replaceData(
				originalStart.offset,
				nodeLength(originalStart.node) - originalStart.offset,
				"",
			);
		}
	...
	surroundContents(newParent) {
		...
		while (node !== endNode) {
			if (
				...
				isPartiallyContained(node, this)
			) {
				throw DOMException.create(this._globalObject, [
	...
	intersectsNode(node) {
		...
		return (
			compareBoundaryPointsPosition({ node: parent, offset }, this._end) ===
				-1 &&
	...
	toString() {
		...
		if (
			_start.node === _end.node &&
			_start.node.nodeType === NODE_TYPE.TEXT_NODE
		) {
		...
		while (currentNode && currentNode !== endNode) {
			if (
				currentNode.nodeType === NODE_TYPE.TEXT_NODE &&
				isContained(currentNode, this)
			) {
				s += currentNode.data;
			}
...
function isContained(node, range) {
	...
	return (
		...
		compareBoundaryPointsPosition({ node, offset: nodeLength(node) }, _end) ===
			-1
	);
}
...
function isPartiallyContained(node, range) {
	const { _start, _end } = range;
	return (
		(isInclusiveAncestor(node, _start.node) &&
			!isInclusiveAncestor(node, _end.node)) ||
		(!isInclusiveAncestor(node, _start.node) &&
			isInclusiveAncestor(node, _end.node))
	);
}
...
function insertNodeInRange(node, range) {
	...
	if (
		...
		(startNode.nodeType === NODE_TYPE.TEXT_NODE &&
			!domSymbolTree.parent(startNode)) ||
		node === startNode
	) {
		throw DOMException.create(node._globalObject, [
			"Invalid start node.",
	...
	let newOffset = !referenceNode
		? nodeLength(parent)
		: domSymbolTree.index(referenceNode);
	newOffset +=
		node.nodeType === NODE_TYPE.DOCUMENT_FRAGMENT_NODE ? nodeLength(node) : 1;
...
function cloneRange(range) {
	...
	if (
		...
		cloned._data = cloned.substringData(
			originalStart.offset,
			originalEnd.offset - originalStart.offset,
		);
	...
	const containedChildren = domSymbolTree
		.childrenToArray(commonAncestor)
		.filter((node) => isContained(node, range));

	const hasDoctypeChildren = containedChildren.some(
		(node) => node.nodeType === NODE_TYPE.DOCUMENT_TYPE_NODE,
	);
	...
	if (
		...
		(firstPartialContainedChild.nodeType === NODE_TYPE.TEXT_NODE ||
			firstPartialContainedChild.nodeType ===
	...
	if (
		...
		(lastPartiallyContainedChild.nodeType === NODE_TYPE.TEXT_NODE ||
			lastPartiallyContainedChild.nodeType ===
				NODE_TYPE.PROCESSING_INSTRUCTION_NODE ||
