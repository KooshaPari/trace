...
class CustomElementRegistryImpl {
	...
	define(name, constructor, options) {
		...
		const nameAlreadyRegistered = this._customElementDefinitions.some(
		...
		const ctorAlreadyRegistered = this._customElementDefinitions.some(
			(entry) => entry.objectReference === ctor,
		);
		...
		try {
			...
			for (const callbackName of LIFECYCLE_CALLBACKS) {
				...
				if (callbackValue !== undefined) {
					lifecycleCallbacks[callbackName] = IDLFunction.convert(
						_globalObject,
						callbackValue,
						{
			...
			if (lifecycleCallbacks.attributeChangedCallback !== null) {
				...
				if (observedAttributesIterable !== undefined) {
					observedAttributes = convertToSequenceDOMString(
		...
		for (const candidate of shadowIncludingInclusiveDescendantsIterator(
			document,
		)) {
			if (
	...
	get(name) {
		const definition = this._customElementDefinitions.find(
			(entry) => entry.name === name,
		);
	...
	getName(constructor) {
		const found = this._customElementDefinitions.find(
			(entry) => entry.objectReference === constructor.objectReference,
		);
	...
	whenDefined(name) {
		if (!isValidCustomElementName(name)) {
			return Promise.reject(
				DOMException.create(this._globalObject, [
					"Name argument is not a valid custom element name.",
					"SyntaxError",
				]),
			);
		...
		const alreadyRegistered = this._customElementDefinitions.find(
			(entry) => entry.name === name,
		);
