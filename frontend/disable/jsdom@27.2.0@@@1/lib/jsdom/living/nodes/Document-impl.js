...
class DocumentImpl extends NodeImpl {
	constructor(globalObject, args, privateData) {
		...
		this._domSelector = new DOMSelector(
			this._globalObject,
			this._ownerDocument,
			{
				domSymbolTree,
	...
	_fallbackBaseURL() {
		if (
			...
			const parentDocument = idlUtils.implForWrapper(
				this._defaultView._parent._document,
			);
	...
	_adoptNode(node) {
		...
		if (oldDocument !== this) {
			for (const inclusiveDescendant of shadowIncludingInclusiveDescendantsIterator(
				...
				inclusiveDescendant._ownerDocument = this;
			}
...
DocumentImpl.prototype.getElementsByTagName = memoizeQuery(
	function (qualifiedName) {
		return listOfElementsWithQualifiedName(qualifiedName, this);
	},
);

DocumentImpl.prototype.getElementsByTagNameNS = memoizeQuery(
