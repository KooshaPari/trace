...
class ElementImpl extends NodeImpl {
	...
	setAttributeNS(namespace, name, value) {
		const extracted = validateNames.validateAndExtract(
			this._globalObject,
			namespace,
			name,
		);
	...
	attachShadow(init) {
		...
		if (
			!isValidHostElementName(_localName) &&
			!isValidCustomElementName(_localName)
		) {
			const message =
...
ElementImpl.prototype.getElementsByTagName = memoizeQuery(
	function (qualifiedName) {
		return listOfElementsWithQualifiedName(qualifiedName, this);
	},
);

ElementImpl.prototype.getElementsByTagNameNS = memoizeQuery(
	function (namespace, localName) {
		return listOfElementsWithNamespaceAndLocalName(namespace, localName, this);
	},
);

ElementImpl.prototype.getElementsByClassName = memoizeQuery(
	function (classNames) {
