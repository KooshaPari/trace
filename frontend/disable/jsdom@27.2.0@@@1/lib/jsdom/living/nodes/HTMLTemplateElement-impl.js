...
class HTMLTemplateElementImpl extends HTMLElementImpl {
	constructor(globalObject, args, privateData) {
		...
		const doc = this._appropriateTemplateContentsOwnerDocument(
			this._ownerDocument,
		);
	...
	[cloningSteps](copy, node, document, cloneChildren) {
		...
		for (const child of domSymbolTree.childrenIterator(
			node._templateContents,
		)) {
			const childCopy = clone(
				child,
				copy._templateContents._ownerDocument,
				true,
			);
