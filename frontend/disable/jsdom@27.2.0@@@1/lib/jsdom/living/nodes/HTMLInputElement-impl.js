...
class HTMLInputElementImpl extends HTMLElementImpl {
	...
	_stepAlign(v, roundUp) {
		...
		return new Decimal(v)
			...
			.toNearest(
				allowedValueStep,
				roundUp ? Decimal.ROUND_UP : Decimal.ROUND_DOWN,
			)
			.add(stepBase);
	}
	...
	_legacyPreActivationBehavior() {
		if (this.type === "checkbox") {
			...
			this._legacyActivationBehaviorPreviousIndeterminateState =
				this.indeterminate;
			this.indeterminate = false;
	...
	_activationBehavior(event) {
		...
		if (
			this.type === "checkbox" ||
			(this.type === "radio" && !this._preCheckedRadioState)
		) {
			if (this.isConnected) {
		...
		if (name === "type") {
			...
			if (prevType !== curType) {
				...
				if (
					...
					prevValueMode !== "filename" &&
					curValueMode === "filename"
				) {
					this._value = "";
				}
				...
				const previouslySelectable = this._idlMemberApplies(
					"setRangeText",
					prevType,
				);
	...
	_someInRadioGroup(name) {
		...
		return this._otherRadioGroupElements.some(
			(radioGroupElement) => radioGroupElement[name],
		);
	}
	...
	set valueAsDate(v) {
		...
		if (v !== null && !isDate(v)) {
			throw new TypeError(
				"Failed to set the 'valueAsDate' property on 'HTMLInputElement': The provided value is " +
					"not a Date.",
			);
		}
	...
	_stepUpdate(n, isUp) {
		...
		if (min !== null && max !== null) {
			...
			const candidateStepValue = this._stepAlign(
				Decimal.add(min, allowedValueStep),
				/* roundUp = */ false,
			);
	...
	get _stepBase() {
		...
		if (this.hasAttributeNS(null, "value")) {
			const value = this._convertStringToNumber(
				this.getAttributeNS(null, "value"),
			);
			if (value !== null) {
				return value;
			}
	...
	_hasAttributeAndApplies(attribute) {
		return (
			this._contentAttributeApplies(attribute) &&
			this.hasAttributeNS(null, attribute)
		);
	}
	...
	get validity() {
		if (!this._validity) {
			...
			const reversedRangeSufferingOverUnderflow = () => {
				...
				return (
					parsedValue !== null &&
					parsedValue > this._maximum &&
					parsedValue < this._minimum
				);
			};
			...
			const state = {
				...
				valueMissing: () => {
					...
					if (
						this._required &&
						valueAttributeMode(this.type) === "value" &&
						this._mutable &&
						this._value === ""
					) {
						return true;
					...
					switch (this.type) {
						...
						case "radio":
							if (
								this._someInRadioGroup("_required") &&
								!this._someInRadioGroup("checked")
							) {
								return true;
							}
							break;
				...
				patternMismatch: () => {
					...
					if (this._hasAttributeAndApplies("multiple")) {
						return !splitOnCommas(this._value).every((value) =>
							regExp.test(value),
						);
					}
...
mixin(
	HTMLInputElementImpl.prototype,
