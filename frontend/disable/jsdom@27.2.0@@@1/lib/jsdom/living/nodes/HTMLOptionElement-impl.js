...
class HTMLOptionElementImpl extends HTMLElementImpl {
	...
	get text() {
		return stripAndCollapseASCIIWhitespace(
			childTextContentExcludingDescendantsOfScript(this),
		);
	}
	set text(value) {
...
function childTextContentExcludingDescendantsOfScript(root) {
	...
	for (const child of domSymbolTree.childrenIterator(root)) {
		if (
			child._localName === "script" &&
			(child._namespaceURI === HTML_NS || child._namespaceURI === SVG_NS)
		) {
			continue;
		}
