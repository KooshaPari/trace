...
class NodeImpl extends EventTargetImpl {
	...
	normalize() {
		...
		for (const node of domSymbolTree.treeToArray(this)) {
			...
			const data = continuousExclusiveTextNodes.reduce(
				(d, n) => d + n._data,
				"",
			);
			...
			while (currentNode && currentNode.nodeType === NODE_TYPE.TEXT_NODE) {
				...
				for (const range of parentNode._referencedRanges) {
					...
					if (
						_start.node === parentNode &&
						_start.offset === currentNodeIndex
					) {
			...
			switch (nodeType) {
				case NODE_TYPE.DOCUMENT_FRAGMENT_NODE: {
					const nodeChildrenElements = nodeChildren.filter(
						(child) => child.nodeType === NODE_TYPE.ELEMENT_NODE,
					);
					...
					const hasNodeTextChildren = nodeChildren.some(
						(child) => child.nodeType === NODE_TYPE.TEXT_NODE,
					);
					...
					if (
						...
						(parentChildren.some(
							...
							(childImpl &&
								childImpl.nodeType === NODE_TYPE.DOCUMENT_TYPE_NODE) ||
							(childImpl &&
								domSymbolTree.nextSibling(childImpl) &&
								domSymbolTree.nextSibling(childImpl).nodeType ===
				...
				case NODE_TYPE.DOCUMENT_TYPE_NODE:
					if (
						parentChildren.some(
							(child) => child.nodeType === NODE_TYPE.DOCUMENT_TYPE_NODE,
						) ||
						(childImpl &&
							domSymbolTree.previousSibling(childImpl) &&
							domSymbolTree.previousSibling(childImpl).nodeType ===
	...
	_insert(nodeImpl, childImpl, suppressObservers) {
		...
		for (const node of nodesImpl) {
			...
			if (
				isSlot(this) &&
				this._assignedNodes.length === 0 &&
				isShadowRoot(nodeRoot(this))
			) {
		...
		if (parentType === NODE_TYPE.DOCUMENT_NODE) {
			...
			switch (nodeType) {
				case NODE_TYPE.DOCUMENT_FRAGMENT_NODE: {
					...
					const parentChildElements = parentChildren.filter(
						(child) => child.nodeType === NODE_TYPE.ELEMENT_NODE,
					);
					if (
						nodeChildrenElements.length === 1 &&
						((parentChildElements.length === 1 &&
							parentChildElements[0] !== childImpl) ||
							(childImpl &&
								domSymbolTree.nextSibling(childImpl) &&
								domSymbolTree.nextSibling(childImpl).nodeType ===
									NODE_TYPE.DOCUMENT_TYPE_NODE))
	...
	_remove(nodeImpl, suppressObservers) {
		...
		if (this.isConnected) {
			...
			for (const descendantImpl of shadowIncludingDescendantsIterator(
				nodeImpl,
			)) {
