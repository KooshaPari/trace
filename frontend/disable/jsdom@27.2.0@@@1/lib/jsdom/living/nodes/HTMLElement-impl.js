...
class HTMLElementImpl extends ElementImpl {
	...
	get translate() {
		...
		if (
			translateAttrString === "yes" ||
			(translateAttr && translateAttrString === "")
		) {
			return true;
	...
	get draggable() {
		const attributeValue = asciiLowercase(
			this.getAttributeNS(null, "draggable") || "",
		);
		...
		return (
			this._localName === "img" ||
			(this._localName === "a" && this.hasAttributeNS(null, "href"))
		);
	}
	set draggable(value) {
	...
	attachInternals() {
		...
		this._attachedInternals = ElementInternals.createImpl(
			this._globalObject,
			[],
			{ targetElement: this },
		);
