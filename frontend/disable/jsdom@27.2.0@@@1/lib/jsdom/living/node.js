				...
				filter(node) {
					...
					if (isQuirksMode) {
						for (const className of classes) {
							if (
								!classList.tokenSet.some((cur) =>
									asciiCaseInsensitiveMatch(cur, className),
								)
							) {
								return false;
							}
		...
		return HTMLCollection.createImpl(root._globalObject, [], {
			...
			query: () =>
				domSymbolTree.treeToArray(root, {
					filter: (node) =>
						node.nodeType === NODE_TYPE.ELEMENT_NODE && node !== root,
				}),
		});
	}
...
exports.listOfElementsWithNamespaceAndLocalName = (
	namespace,
	localName,
	root,
) => {
		...
		query: () =>
			domSymbolTree.treeToArray(root, {
				filter(node) {
					...
					return (
						node._localName === localName && node._namespaceURI === namespace
					);
				},
			}),
	});
	...
	for (let i = 0; i < nodes.length; i++) {
		fragment._append(
			typeof nodes[i] === "string"
				? document.createTextNode(nodes[i])
				: nodes[i],
		);
	}
	return fragment;
