	...
	class AbortSignal extends globalObject.EventTarget {
		...
		set onabort(V) {
			...
			if (!utils.isObject(V)) {
				...
				V = EventHandlerNonNull.convert(globalObject, V, {
					context:
						"Failed to set the 'onabort' property on 'AbortSignal': The provided value",
				});
			}
		...
		static abort() {
			...
			return utils.tryWrapperForImpl(
				Impl.implementation.abort(globalObject, ...args),
			);
		}
		...
		static timeout(milliseconds) {
			...
			return utils.tryWrapperForImpl(
				Impl.implementation.timeout(globalObject, ...args),
			);
		}
		...
		static any(signals) {
			...
			const args = [];
			{
				...
				if (!utils.isObject(curArg)) {
					throw new globalObject.TypeError(
						"Failed to execute 'any' on 'AbortSignal': parameter 1" +
			...
			return utils.tryWrapperForImpl(
				Impl.implementation.any(globalObject, ...args),
			);
		}
	}
