...
exports.convert = (
	...
	throw new globalObject.TypeError(
		`${context} is not of type 'HTMLFormElement'.`,
	);
};
...
exports.install = (globalObject, globalNames) => {
	...
	class HTMLFormElement extends globalObject.HTMLElement {
			...
			const args = [];
			{
				...
				if (curArg !== undefined) {
					curArg = HTMLElement.convert(globalObject, curArg, {
						context:
							"Failed to execute 'requestSubmit' on 'HTMLFormElement': parameter 1",
					});
				}
				args.push(curArg);
			}
		...
		get acceptCharset() {
			...
			try {
				const value =
					esValue[implSymbol]._reflectGetTheContentAttribute("accept-charset");
		...
		get noValidate() {
			...
			try {
				return (
					esValue[implSymbol]._reflectGetTheContentAttribute("novalidate") !==
					null
				);
			} finally {
				ceReactionsPostSteps_helpers_custom_elements(globalObject);
