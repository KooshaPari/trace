...
exports.install = (globalObject, globalNames) => {
	...
	class DocumentType extends globalObject.Node {
		...
		before() {
			...
			if (!exports.is(esValue)) {
				throw new globalObject.TypeError(
					"'before' called on an object that is not a valid instance of DocumentType.",
				);
			}
			const args = [];
			for (let i = 0; i < arguments.length; i++) {
				...
				if (Node.is(curArg)) {
					...
					curArg = conversions["DOMString"](curArg, {
						context:
							"Failed to execute 'before' on 'DocumentType': parameter " +
							(i + 1),
		...
		after() {
			...
			if (!exports.is(esValue)) {
				throw new globalObject.TypeError(
					"'after' called on an object that is not a valid instance of DocumentType.",
				);
			}
			const args = [];
			for (let i = 0; i < arguments.length; i++) {
				...
				if (Node.is(curArg)) {
					...
					curArg = conversions["DOMString"](curArg, {
						context:
							"Failed to execute 'after' on 'DocumentType': parameter " +
							(i + 1),
						globals: globalObject,
					});
				}
		...
		remove() {
			...
			if (!exports.is(esValue)) {
				throw new globalObject.TypeError(
					"'remove' called on an object that is not a valid instance of DocumentType.",
				);
			}
