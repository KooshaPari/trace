...
class EventTargetImpl {
	...
	addEventListener(
		type,
		callback,
		options = { __proto__: null, capture: false, once: false },
	) {
		let { capture, once, passive, signal } =
			flattenMoreEventListenerOptions(options);
	...
	removeEventListener(
		type,
		callback,
		options = { __proto__: null, capture: false },
	) {
	...
	_dispatch(
		eventImpl,
		legacyTargetOverrideFlag /* , legacyOutputDidListenersThrowFlag */,
	) {
		let targetImpl = this;
		...
		if (
			...
			const isActivationEvent =
				MouseEvent.isImpl(eventImpl) && eventImpl.type === "click";
			...
			while (parent !== null) {
				if (slotable !== null) {
					if (parent.localName !== "slot") {
						throw new Error(
							`JSDOM Internal Error: Expected parent to be a Slot`,
						);
					}
