...
exports.implementation = class FormDataImpl {
	...
	get(name) {
		...
		return foundEntry !== undefined
			? idlUtils.tryWrapperForImpl(foundEntry.value)
			: null;
	}
	...
	getAll(name) {
		return this._entries
			...
			.map((entry) => idlUtils.tryWrapperForImpl(entry.value));
	}
...
function constructTheEntryList(form, submitter) {
	...
	for (const field of controls) {
		...
		if (field.localName === "select") {
			...
			const value = field.hasAttributeNS(null, "value")
				? field.getAttributeNS(null, "value")
				: "on";
