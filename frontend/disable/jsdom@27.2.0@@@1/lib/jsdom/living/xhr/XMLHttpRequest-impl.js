...
const {
	serializeEntryList,
	chunksToBuffer,
} = require("./multipart-form-data.js");
			...
			if (body !== null) {
				...
				if (Document.isImpl(body)) {
					...
					mimeType =
						...
						";charset=UTF-8";
					flag.body = Buffer.from(
						fragmentSerialization(body, { requireWellFormed: false }),
					);
				} else {
...
function extractBody(bodyInit) {
	...
	if (Blob.isImpl(bodyInit)) {
		...
		return {
			...
			contentType:
				"multipart/form-data; boundary=" + utf8Decoder.decode(boundary),
		};
	}
