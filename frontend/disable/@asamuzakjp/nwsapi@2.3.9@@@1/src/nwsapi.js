...
(function Export(global, factory) {
	...
	const GROUPS = {
		...
		logicalsel:
			"(is|where|matches|not|has)(?:\\x28\\s?(" +
			createMatchingParensRegex(3) +
			")\\s?\\x29)",
		treestruct:
		...
		inputstate:
			"(enabled|disabled|read-(?:only|write)|placeholder-shown|default)\\b",
		...
		pseudoDbl:
			":(after|before|first-letter|first-line|selection|part|placeholder|slotted|-webkit-[-a-z0-9]{2,})\\b",
	};
	...
	const concatCall = (nodes, callback) => {
		let i = 0;
	...
	const hasMixedCaseTagNames = (context) => {
		...
		const ns =
			context.documentElement && context.documentElement.namespaceURI
	...
	const stringFromCodePoint = (codePoint) => {
		// out of range, use replacement character
		if (
			codePoint < 1 ||
			codePoint > 0x10ffff ||
	...
	const convertEscapes = (str) =>
		REX.hasEscapes.test(str)
	...
	const compat = {
		...
		".": (c, n) => {
			REX.hasEscapes.test(n) && (n = unescapeIdentifier(n));
			return (e, f) => byClass(n, c);
		},
	};

	// namespace aware hasAttribute
	// helper for XML/XHTML documents
	const hasAttributeNS = (e, name) => {
		let i;
		let l;
	...
	const switchContext = (context, force) => {
		const oldDoc = doc;
	...
	const compileSelector = (expression, source, mode, callback) => {
						...
						switch (match[1]) {
							...
							case "only-of-type":
								source =
									...
									"n=e;while((n=n.previousElementSibling)&&n.localName!=o);}if(" +
									D +
									"n){" +
									source +
									"}";
						...
						switch (match[1]) {
							case "any-link":
								source =
									"if(" +
									N +
									'(/^a|area$/i.test(e.localName)&&e.hasAttribute("href")||e.visited)){' +
									source +
									"}";
								break;
							case "link":
	...
	const compile = (selector, mode, callback) => {
		let head = "";
		let loop = "";
		let macro = "";
		let source = "";
	...
	const collect = (selectors, context, callback) => {
		let i;
		let l;
		const seen = {};
		let token = ["", "*", "*"];
	...
	const makeref = (selectors, element) => {
		// DOCUMENT_NODE (9)
	...
	const matchAssert = (f, element, callback) => {
		let r = false;
	...
	const matchCollect = (selectors, callback) => {
		const f = [];
