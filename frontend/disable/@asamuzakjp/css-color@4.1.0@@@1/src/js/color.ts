...
export const transformRgbToLinearRgb = (
	...
	if (r > COND_POW) {
		r = ((r + LINEAR_OFFSET) / (1 + LINEAR_OFFSET)) ** POW_LINEAR;
	} else {
		r /= LINEAR_COEF;
	}
	if (g > COND_POW) {
		g = ((g + LINEAR_OFFSET) / (1 + LINEAR_OFFSET)) ** POW_LINEAR;
	} else {
		g /= LINEAR_COEF;
	}
	if (b > COND_POW) {
		b = ((b + LINEAR_OFFSET) / (1 + LINEAR_OFFSET)) ** POW_LINEAR;
	} else {
		b /= LINEAR_COEF;
	}
	return [r, g, b];
...
export const transformLinearRgbToRgb = (
	...
	if (r > COND_POW) {
		r = r ** (1 / POW_LINEAR) * (1 + LINEAR_OFFSET) - LINEAR_OFFSET;
	} else {
		r *= LINEAR_COEF;
	}
	r *= MAX_RGB;
	if (g > COND_POW) {
		g = g ** (1 / POW_LINEAR) * (1 + LINEAR_OFFSET) - LINEAR_OFFSET;
	} else {
		g *= LINEAR_COEF;
	}
	g *= MAX_RGB;
	if (b > COND_POW) {
		b = b ** (1 / POW_LINEAR) * (1 + LINEAR_OFFSET) - LINEAR_OFFSET;
	} else {
		b *= LINEAR_COEF;
	}
	b *= MAX_RGB;
...
export const transformXyzToOklch = (
	...
	if (lPct === 0 || lPct === MAX_PCT) {
		...
		c = Math.max(Math.sqrt(a ** POW_SQR + b ** POW_SQR), 0);
...
export const parseLab = (
	...
	const fa = a / LAB_A + fl;
	const fb = fl - b / LAB_B;
	const powFl = fl ** POW_CUBE;
	const powFa = fa ** POW_CUBE;
	const powFb = fb ** POW_CUBE;
	const xyz = [
...
export const parseOklab = (
	...
	const xyzLms = lms.map((c) => c ** POW_CUBE) as TriColorChannels;
...
export const parseOklch = (
	...
	const b = c * Math.sin((h * Math.PI) / DEG_HALF);
	const lms = transformMatrix(MATRIX_OKLAB_TO_LMS, [l, a, b]);
	const xyzLms = lms.map((cc) => cc ** POW_CUBE) as TriColorChannels;
	const [x, y, z] = transformMatrix(MATRIX_LMS_TO_XYZ, xyzLms, true);
	return [
		"xyz-d65",
		roundToPrecision(x, HEX),
