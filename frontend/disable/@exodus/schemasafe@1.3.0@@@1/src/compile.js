...
const schemaTypes = new Map(
	Object.entries({
		...
		array: (arg) =>
			Array.isArray(arg) && Object.getPrototypeOf(arg) === Array.prototype,
...
const propimm = (parent, keyval, checked = false) =>
	Object.freeze({ parent, keyval, checked }); // property by immediate value
...
const generateMeta = (root, $schema, enforce, requireSchema) => {
	if ($schema) {
		...
		enforce(
			schemaVersions.includes(version),
...
const compileSchema = (schema, root, opts, scope, basePathRoot = "") => {
	const {
		...
		$schemaDefault = null,
		formatAssertion = opts.mode !== "spec" ||
			schemaIsUnkownOrOlder(root.$schema, "draft/2019-09"),
		formats: optFormats = {},
	...
	if (!["strong", "lax", "default", "spec"].includes(mode))
	...
	if (mode === "strong") {
		...
		for (const [k, v] of Object.entries(strong))
			if (!v) throw new Error(`Strong mode demands ${k}`);
		for (const [k, v] of Object.entries(weak))
			if (v) throw new Error(`Strong mode forbids ${k}`);
	}
	...
	const buildPath = (prop) => {
		...
		for (const { keyname, keyval, number } of path) {
			if (keyname) {
				...
				res = res
					? format("%s+%j+%s", res, str, value)
					: format("%j+%s", str, value);
			} else if (keyval) stringParts.push(keyval);
		}
		return stringParts.length > 0
			? format("%s+%j", res, `/${stringJoined()}`)
			: res;
	};
	...
	let dynamicAnchorsNext = opts[optDynAnchors]
		? format(", dynAnchors")
		: format("");
	if (hasDynAnchors) {
	...
	const helpers = jsHelpers(
		fun,
		scope,
		propvar,
		{ unmodifiedPrototypes, isJSON },
		noopRegExps,
	);
	...
	const visit = (
		...
		const isSub =
			history.length > 0 && history[history.length - 1].prop === current;
		const queryCurrent = () => history.filter((h) => h.prop === current);
		const definitelyPresent =
			!current.parent ||
			current.checked ||
			(current.inKeys && isJSON) ||
			queryCurrent().length > 0;
		...
		const error = ({ path = [], prop = current, source, suberr }) => {
			...
			if (includeErrors === true && errors && source) {
				...
				if (allErrors) {
					...
					fun.write(
						"validate.errors.push(...%s.map(e => errorMerge(e, %j, %s)))",
						...args,
					);
				} else
					fun.write("validate.errors = [errorMerge(%s[0], %j, %s)]", ...args);
			} else if (includeErrors === true && errors) {
				const errorJS = format(
					"{ keywordLocation: %j, instanceLocation: %s }",
					schemaP,
					dataP,
				);
				if (allErrors) {
		...
		const errorIf = (condition, errorArgs) =>
			fun.if(condition, () => error(errorArgs));
		...
		const fail = (msg, value) => {
			...
			if (lintOnly)
				return scope.lintErrors.push({ message, keywordLocation, schema }); // don't fail if we are just collecting all errors
		...
		const laxMode = (ok, ...args) =>
			enforce(mode === "lax" || mode === "spec" || ok, ...args);
		const enforceMinMax = (a, b) =>
			laxMode(!(node[b] < node[a]), `Invalid ${a} / ${b} combination`);
		...
		const uncertain = (msg) =>
			enforce(
				!removeAdditional && !useDefaults,
				`[removeAdditional/useDefaults] uncertain: ${msg}`,
			);
		const complex = (msg, arg) =>
			enforce(!complexityChecks, `[complexityChecks] ${msg}`, arg);
		const saveMeta = ($sch) =>
			generateMeta(root, $sch || $schemaDefault, enforce, requireSchema);
		...
		if (typeof node === "boolean") {
			...
			errorIf(
				definitelyPresent || current.inKeys ? true : present(current),
				{},
			); // node === false
		...
		for (const key of Object.keys(node))
			enforce(
				knownKeywords.includes(key) || allowUnusedKeywords,
		...
		const consume = (prop, ...ruleTypes) => {
			enforce(
				multiConsumable.has(prop) || unused.has(prop),
				"Unexpected double consumption:",
				prop,
			);
			enforce(functions.hasOwn(node, prop), "Is not an own property:", prop);
			enforce(
				ruleTypes.every((t) => schemaTypes.has(t)),
				"Invalid type used in consume",
			);
			enforce(
				ruleTypes.some((t) => schemaTypes.get(t)(node[prop])),
				"Unexpected type for",
				prop,
			);
			unused.delete(prop);
		...
		if (node === root) {
			...
			handle("$vocabulary", ["object"], ($vocabulary) => {
				for (const [vocab, flag] of Object.entries($vocabulary)) {
					...
					enforce(
						flag === true && knownVocabularies.includes(vocab),
						"Unknown vocabulary:",
						vocab,
					);
				}
		...
		for (const ignore of ["title", "description", "$comment"])
			handle(ignore, ["string"], null); // unused, meta-only strings
		for (const ignore of ["deprecated", "readOnly", "writeOnly"])
			handle(ignore, ["boolean"], null); // unused, meta-only flags

		handle("$defs", ["object"], null) ||
			handle("definitions", ["object"], null); // defs are allowed, those are validated on usage

		const compileSub = (sub, subR, path) =>
			sub === schema
				? safe("validate")
				: getref(sub) || compileSchema(sub, subR, opts, scope, path);
		const basePath = () =>
			basePathStack.length > 0 ? basePathStack[basePathStack.length - 1] : "";
		...
		const isDynScope =
			hasDynAnchors && (node === schema || node.id || node.$id);
		if (isDynScope) {
			...
			for (const [key, subcheck] of allDynamic) {
				...
				enforce(
					sub === subcheck,
					`Unexpected $dynamicAnchor resolution: ${key}`,
				);
		...
		const needUnevaluated = (rule) =>
			opts[optDynamic] &&
			(node[rule] || node[rule] === false || node === schema);
		const local = Object.freeze({
			item: needUnevaluated("unevaluatedItems")
				? gensym("evaluatedItem")
				: null,
			items: needUnevaluated("unevaluatedItems")
				? gensym("evaluatedItems")
				: null,
			props: needUnevaluated("unevaluatedProperties")
				? gensym("evaluatedProps")
				: null,
		});
		...
		const canSkipDynamic = () =>
			...
			(!dyn.props || stat.properties.includes(true));
		const evaluateDeltaDynamic = (delta) => {
			...
			if (dyn.items && delta.items > stat.items)
				fun.write("%s.push(%d)", dyn.items, delta.items);
			if (
				dyn.props &&
				(delta.properties || []).includes(true) &&
				!stat.properties.includes(true)
			) {
				fun.write("%s[0].push(true)", dyn.props);
			} else if (dyn.props) {
				const inStat = (properties, patterns) =>
					inProperties(stat, { properties, patterns });
				const properties = (delta.properties || []).filter(
					(x) => !inStat([x], []),
				);
		...
		const applyRef = (n, errorArgs) => {
			...
			const delta = (scope[n] && scope[n][evaluatedStatic]) || {
				unknown: true,
			}; // assume unknown if ref is cyclic
			evaluateDelta(delta);
			const call = format(
				"%s(%s%s%s)",
				n,
			...
			fun.if(res, () => {
				const item = isDynamic(delta).items
					? format("%s.evaluatedDynamic[0]", n)
					: null;
				const items = isDynamic(delta).items
					? format("%s.evaluatedDynamic[1]", n)
					: null;
				const props = isDynamic(delta).properties
					? format("%s.evaluatedDynamic[2]", n)
					: null;
		...
		const someIn = (arr, possible) =>
			possible.some((x) => arr === null || arr.includes(x)); // all possible are in arrs

		const parentCheckedType = (...valid) =>
			queryCurrent().some((h) => allIn(h.stat.type, valid));
		const definitelyType = (...valid) =>
			allIn(stat.type, valid) || parentCheckedType(...valid);
		const typeApplicable = (...possible) =>
			someIn(stat.type, possible) &&
			queryCurrent().every((h) => someIn(h.stat.type, possible));
		...
		const haveComplex =
			node.uniqueItems || havePattern || node.patternProperties || node.format;
		const prev = allErrors && haveComplex ? gensym("prev") : null;
		const prevWrap = (shouldWrap, writeBody) =>
			fun.if(
				shouldWrap && prev !== null ? format("errorCount === %s", prev) : true,
				writeBody,
			);
		...
		const lintRequired = (properties, patterns) => {
			...
			const known = (key) =>
				properties.includes(key) || regexps.some((r) => r.test(key));
			for (const key of stat.required)
		...
		const checkNumbers = () => {
			const minMax = (value, operator) =>
				format("!(%d %c %s)", value, operator, name); // don't remove negation, accounts for NaN

			if (Number.isFinite(node.exclusiveMinimum)) {
				handle("exclusiveMinimum", ["finite"], (min) => minMax(min, "<"));
			} else {
				handle("minimum", ["finite"], (min) =>
					minMax(min, node.exclusiveMinimum ? "<" : "<="),
				);
			...
			if (Number.isFinite(node.exclusiveMaximum)) {
				...
				handle("maximum", ["finite"], (max) =>
					minMax(max, node.exclusiveMaximum ? ">" : ">="),
				);
			...
			const multipleOf =
				node.multipleOf === undefined ? "divisibleBy" : "multipleOf"; // draft3 support
			handle(multipleOf, ["finite"], (value) => {
				...
				if (Number.isInteger(value * 2 ** e))
					return format("%s %% %d !== 0", name, value); // exact
				scope.isMultipleOf = functions.isMultipleOf;
				const args = [name, value, e, Math.round(value * 10 ** e)]; // precompute for performance
		...
		const checkStrings = () => {
			...
			prevWrap(true, () => {
				const checkFormat = (fmtname, target, formatsObj = fmts) => {
					const known =
						typeof fmtname === "string" &&
						functions.hasOwn(formatsObj, fmtname);
					enforce(known, "Unrecognized format used:", fmtname);
					const formatImpl = formatsObj[fmtname];
					const valid =
						formatImpl instanceof RegExp || typeof formatImpl === "function";
				...
				handle("pattern", ["string"], (pattern) => {
					...
					evaluateDelta({ fullstring: true });
					return noopRegExps.has(pattern)
						? null
						: safenot(patternTestSafe(pattern, name));
				});

				enforce(
					node.contentSchema !== false,
					"contentSchema cannot be set to false",
				);
				const cV =
					contentValidation === undefined
						? getMeta().contentValidation
						: contentValidation;
				const haveContent =
					node.contentEncoding || node.contentMediaType || node.contentSchema;
				const contentErr =
				...
				if (haveContent && cV) {
					...
					if (node.contentEncoding === "base64") {
						errorIf(checkFormat("base64", name, formats.extra), {
					...
					if (node.contentSchema) {
						enforce(
							json,
							"contentSchema requires contentMediaType application/json",
						);
		...
		const checkArrays = () => {
			handle("maxItems", ["natural"], (max) => {
				const prefixItemsName = getMeta().newItemsSyntax
					? "prefixItems"
					: "items";
				if (
					Array.isArray(node[prefixItemsName]) &&
					node[prefixItemsName].length > max
				)
					fail(
						`Invalid maxItems: ${max} is less than ${prefixItemsName} array length`,
					);
				return format("%s.length > %d", name, max);
			});
			handle("minItems", ["natural"], (min) =>
				format("%s.length < %d", name, min),
			...
			const itemsSimple = (ischema) => {
				...
				if (ischema.type) {
					const itemTypes = Array.isArray(ischema.type)
						? ischema.type
						: [ischema.type];
				...
				if (ischema.$ref) {
					const [sub] =
						resolveReference(root, schemas, ischema.$ref, basePath())[0] || [];
					if (itemsSimple(sub)) return true;
				}
				return false;
			};
			const itemsSimpleOrFalse = (ischema) =>
				ischema === false || itemsSimple(ischema);
			const uniqueSimple = () => {
				if (node.maxItems !== undefined || itemsSimpleOrFalse(node.items))
					return true;
				// In old format, .additionalItems requires .items to have effect
				if (
					Array.isArray(node.items) &&
					itemsSimpleOrFalse(node.additionalItems)
				)
					return true;
				return false;
			};
			prevWrap(true, () => {
				handle("uniqueItems", ["boolean"], (uniqueItems) => {
					...
					if (!uniqueSimple())
						complex(
							"maxItems should be specified for non-primitive uniqueItems",
						);
					Object.assign(scope, {
		...
		const checked = (p) =>
			!allErrors &&
			(stat.required.includes(p) ||
				queryCurrent().some((h) => h.stat.required.includes(p)));

		const checkObjects = () => {
			const propertiesCount = format("Object.keys(%s).length", name);
			handle("maxProperties", ["natural"], (max) =>
				format("%s > %d", propertiesCount, max),
			);
			handle("minProperties", ["natural"], (min) =>
				format("%s < %d", propertiesCount, min),
			);
			...
			handle("propertyNames", ["object", "boolean"], (s) => {
				forObjectKeys(current, (sub, key) => {
					...
					const nameSchema =
						typeof s === "object" && !s.$ref ? { type: "string", ...s } : s;
					const nameprop = Object.freeze({
						name: key,
						errorParent: sub,
						type: "string",
					});
				...
				for (const [key, variants] of Object.entries(propertyDependencies)) {
					...
					fun.if(item.checked ? true : present(item), () => {
						for (const [val, deps] of Object.entries(variants)) {
							enforce(
								isSchemaish(deps),
								"propertyDependencies must contain schemas",
							);
							fun.if(compare(buildName(item), val), () => {
								...
								const delta = rule(
									current,
									deps,
									subPath("propertyDependencies", key, val),
									dyn,
								);
								evaluateDelta(orDelta({}, delta));
								evaluateDeltaDynamic(delta);
			...
			handle("properties", ["object"], (properties) => {
				for (const p of Object.keys(properties)) {
					...
					rule(
						currPropImm(p, checked(p)),
						properties[p],
						subPath("properties", p),
					);
				}
			...
			prevWrap(node.patternProperties, () => {
				...
				if (node.additionalProperties || node.additionalProperties === false) {
					...
					const condition = (key) =>
						additionalCondition(key, properties, patternProperties);
		...
		const checkConst = () => {
			const handledConst = handle("const", ["jsonval"], (val) =>
				safenot(compare(name, val)),
			);
			...
			const handledEnum = handle("enum", ["array"], (vals) => {
				const objects = vals.filter(
					(value) => value && typeof value === "object",
				);
				const primitive = vals.filter(
					(value) => !(value && typeof value === "object"),
				);
				return safenotor(
					...[...primitive, ...objects].map((value) => compare(name, value)),
				);
			});
		...
		const checkContains = (iterate) => {
			...
			handle("contains", ["object", "boolean"], () => {
				...
				if (
					getMeta().objectContains &&
					typeApplicable("array") &&
					typeApplicable("object")
				) {
					enforceValidation(
						"possible type confusion in 'contains',",
						"forbid 'object' or 'array'",
					);
				}
				...
				iterate((prop, evaluate) => {
					...
					fun.if(sub, () => {
						...
						if (getMeta().containsEvaluates) {
							enforce(
								!removeAdditional,
								'Can\'t use removeAdditional with draft2020+ "contains"',
							);
							evaluate();
						}
					});
				...
				if (
					!handle(
						"minContains",
						["natural"],
						(mn) => format("%s < %d", passes, mn),
						{ suberr },
					)
				)
					errorIf(format("%s < 1", passes), { path: ["contains"], suberr });

				handle("maxContains", ["natural"], (max) =>
					format("%s > %d", passes, max),
				);
		...
		const checkGeneric = () => {
			handle(
				"not",
				["object", "boolean"],
				(not) => subrule(null, current, not, subPath("not")).sub,
			);
			if (node.not) uncertain("not");

			const thenOrElse =
				node.then || node.then === false || node.else || node.else === false;
			...
			if (thenOrElse || allowUnusedKeywords)
				handle("if", ["object", "boolean"], (ifS) => {
					...
					const { sub, delta: deltaIf } = subrule(
						null,
						current,
						ifS,
						subPath("if"),
					...
					evaluateDelta(
						orDelta(deltaElse || {}, andDelta(deltaIf, deltaThen || {})),
					);
					return null;
				});
			...
			handle("discriminator", ["object"], (discriminator) => {
				const seen = new Set();
				const fix = (check, message, arg) =>
					enforce(check, `[discriminator]: ${message}`, arg);
				const { propertyName: pname, mapping: map, ...e0 } = discriminator;
				const prop = currPropImm(pname);
				fix(
					pname && !node.oneOf !== !node.anyOf,
					"need propertyName, oneOf OR anyOf",
				);
				fix(
					Object.keys(e0).length === 0,
					'only "propertyName" and "mapping" are supported',
				);
				const keylen = (obj) =>
					isPlainObject(obj) ? Object.keys(obj).length : null;
				handleDiscriminator = (branches, ruleName) => {
					const runDiscriminator = () => {
						...
						for (const [i, branch] of Object.entries(branches)) {
							const {
								...
								enum: myenum,
								...e1
							} = (branch.properties || {})[pname] || {};
							let vals = myval !== undefined ? [myval] : myenum;
							if (!vals && branch.$ref) {
								const [sub] =
									resolveReference(root, schemas, branch.$ref, basePath())[0] ||
									[];
								enforce(
									isPlainObject(sub),
							...
							fix(
								ok1,
								"branches should have unique string const or enum values for [propertyName]",
							);
							const ok2 = Object.keys(e1).length === 0 && (!myval || !myenum);
							fix(
								ok2,
								"only const OR enum rules are allowed on [propertyName] in branches",
							);
							for (const val of vals) {
								const okMapping =
									!map ||
									(functions.hasOwn(map, val) && map[val] === branch.$ref);
								fix(okMapping, "mismatching mapping for", val);
								const valok = typeof val === "string" && !seen.has(val);
								fix(
									valok,
									"const/enum values for [propertyName] should be unique strings",
								);
								seen.add(val);
								fun.write("case %j:", val);
							}
							const subd = rule(current, branch, subPath(ruleName, i), dyn, {
								constProp: pname,
							});
						...
						fix(
							map === undefined || keylen(map) === seen.size,
							"mismatching mapping size",
						);
						evaluateDelta(delta);
						fun.write("default:");
						error({ path: [ruleName] });
					...
					const propCheck = () => {
						if (!checked(pname)) {
							...
							fun.if(present(prop), runDiscriminator, () =>
								error({ path: errorPath, prop }),
							);
						} else runDiscriminator();
					};
					if (allErrors || !functions.deepEqual(stat.type, ["object"])) {
						fun.if(types.get("object")(name), propCheck, () =>
							error({ path: ["discriminator"] }),
						);
					} else propCheck();
					...
					fix(
						functions.deepEqual(stat.type, ["object"]),
						"has to be checked for type:",
						"object",
					);
					fix(
						stat.required.includes(pname),
						"propertyName should be placed in required:",
						pname,
					);
					return null;
				};
			...
			const uncertainBranchTypes = (key, arr) => {
				...
				const btypes = arr.map(
					(x) => x.type || (Array.isArray(x.const) ? "array" : typeof x.const),
				); // typeof can be 'undefined', but we don't care
				const maybeObj = btypes.filter(
					(x) => !primitiveTypes.includes(x) && x !== "array",
				).length;
				const maybeArr = btypes.filter(
					(x) => !primitiveTypes.includes(x) && x !== "object",
				).length;
			...
			handle("anyOf", ["array"], (anyOf) => {
				...
				if (!canSkipDynamic()) {
					...
					evaluateDelta(
						entries.map((x) => x.delta).reduce((acc, cur) => orDelta(acc, cur)),
					);
					errorIf(safenotor(...entries.map(({ sub }) => sub)), {
						path: ["anyOf"],
						suberr,
					});
					for (const { delta, sub } of entries)
						fun.if(sub, () => evaluateDeltaDynamic(delta));
					return null;
				}
				...
				if (!getMeta().exclusiveRefs) {
					...
					delta = entries
						.map((x) => x.delta)
						.reduce((acc, cur) => orDelta(acc, cur));
					errorIf(safenotor(...entries.map(({ sub }) => sub)), {
						path: ["anyOf"],
						suberr,
					});
					...
					for (const [key, sch] of Object.entries(blocks).reverse()) {
						const oldBody = body;
						body = () => {
							const { sub, delta: deltaVar } = subrule(
								suberr,
								current,
								sch,
								subPath("anyOf", key),
		...
		const typeWrap = (checkBlock, validTypes, queryType) => {
			...
			if (funSize !== fun.size() || unusedSize !== unused.size)
				enforce(
					typeApplicable(...validTypes),
					`Unexpected rules in type`,
					node.type,
				);
		};
			...
			if (stat.items === Infinity) {
				...
				if (isDynamic(stat).items) {
					...
					additionalItems(
						"unevaluatedItems",
						limit,
						getMeta().containsEvaluates ? extra : null,
					);
				} else {
			...
			prevWrap(
				...
				() => {
					if (stat.properties.includes(true)) {
						...
						const notStatic = (key) =>
							additionalCondition(key, stat.properties, stat.patterns);
						if (isDynamic(stat).properties) {
							if (!opts[optDynamic])
								throw new Error(
									"[opt] Dynamic unevaluated tracing not enabled",
								);
							scope.propertyIn = functions.propertyIn;
							const notDynamic = (key) =>
								format("!propertyIn(%s, %s)", key, dyn.props);
							const condition = (key) =>
								safeand(notStatic(key), notDynamic(key));
							additionalProperties("unevaluatedProperties", condition);
						} else {
							if (node.unevaluatedProperties === false)
								lintRequired(stat.properties, stat.patterns);
		...
		const performValidation = () => {
			...
			if (checkConst()) {
				...
				evaluateDelta({
					...
					type: typeKeys,
					fullstring: true,
				}); // everything is evaluated for const
		...
		const writeMain = () => {
			...
			if (getMeta().exclusiveRefs) {
				enforce(
					!opts[optDynamic],
					"unevaluated* is supported only on draft2019-09 and above",
				);
			...
			handle("$recursiveRef", ["string"], ($recursiveRef) => {
				...
				enforce(
					$recursiveRef === "#",
					'Behavior of $recursiveRef is defined only for "#"',
				);
			...
			handle("$dynamicRef", ["string"], ($dynamicRef) => {
				...
				laxMode(
					/^[^#]*#[a-zA-Z0-9_-]+$/.test($dynamicRef),
					"Unsupported $dynamicRef format",
				);
				...
				if (!resolved[0] && !getMeta().bookending) {
					...
					laxMode(
						false,
						"$dynamicRef bookending resolution failed (even though not required)",
					);
					scope.dynamicResolve = functions.dynamicResolve;
					const nrec = format(
						"dynamicResolve(dynAnchors || [], %j)",
						dynamicTail,
					);
				...
				enforce(
					resolved[0],
					"$dynamicRef bookending resolution failed",
					$dynamicRef,
				);
				const [sub, subRoot, path] = resolved[0];
				const ok =
					sub.$dynamicAnchor && `#${sub.$dynamicAnchor}` === dynamicTail;
				laxMode(ok, "$dynamicRef without $dynamicAnchor in the same scope");
				const n = compileSub(sub, subRoot, path);
				scope.dynamicResolve = functions.dynamicResolve;
				const nrec = ok
					? format(
							"(dynamicResolve(dynAnchors || [], %j) || %s)",
							dynamicTail,
							n,
						)
					: n;
			...
			handle("type", ["string", "array"], (type) => {
				...
				if (current.type) {
					enforce(
						functions.deepEqual(typearr, [current.type]),
						"One type allowed:",
						current.type,
					);
			...
			if (stat.items < Infinity && node.maxItems <= stat.items)
				evaluateDelta({ items: Infinity });
		};
		...
		if (node.default !== undefined && useDefaults) {
			...
			fun.if(present(current), writeMain, () =>
				fun.write("%s = %j", name, defvalue),
			);
		} else {
		...
		if (isSub) {
			const logicalOp = ["not", "if", "then", "else"].includes(
				schemaPath[schemaPath.length - 1],
			);
			const branchOp = ["oneOf", "anyOf", "allOf"].includes(
				schemaPath[schemaPath.length - 2],
			);
			const depOp = ["dependencies", "dependentSchemas"].includes(
				schemaPath[schemaPath.length - 2],
			);
			const propDepOp = ["propertyDependencies"].includes(
				schemaPath[schemaPath.length - 3],
			);
			// Coherence check, unreachable, double-check that we came from expected path
			enforce(
				logicalOp || branchOp || depOp || propDepOp,
			...
			if (!isRefTop || refsNeedFullValidation.has(funname)) {
				...
				if (typeApplicable("array") && stat.items !== Infinity)
					enforceValidation(
						node.items ? "additionalItems or unevaluatedItems" : "items rule",
					);
				...
				if (!stat.fullstring && requireStringValidation) {
					const stringWarning =
						"pattern, format or contentSchema should be specified for strings";
					fail(
						`[requireStringValidation] ${stringWarning}, use pattern: ^[\\s\\S]*$ to opt-out`,
					);
				}
			}
		...
		enforce(unused.size === 0 || allowUnusedKeywords, "Unprocessed keywords:", [
			...unused,
		]);
...
const compile = (schemas, opts) => {
	...
	try {
		...
		const refs = schemas.map(
			(s) => getref(s) || compileSchema(s, s, opts, scope),
		);
		if (refsNeedFullValidation.size !== 0)
		...
		if (
			!opts[optDynamic] &&
			e.message === "[opt] Dynamic unevaluated tracing not enabled"
		)
			return compile(schemas, { ...opts, [optDynamic]: true });
		// Also enable dynamic and recursive refs only if needed
		if (
			!opts[optDynAnchors] &&
			e.message === "[opt] Dynamic anchors are not enabled"
		)
			return compile(schemas, { ...opts, [optDynAnchors]: true });
		if (
			!opts[optRecAnchors] &&
			e.message === "[opt] Recursive anchors are not enabled"
		)
