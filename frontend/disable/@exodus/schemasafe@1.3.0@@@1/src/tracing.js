...
const wrapArgs =
	(f) =>
	(...args) =>
		f(...args.map(normalize));
const wrapFull =
	(f) =>
	(...args) =>
		normalize(f(...args.map(normalize)));
...
const normalize = ({ type = null, dyn: d = {}, ...A }) => ({
	...
	properties: typeIsNot(type, "object")
		? [true]
		: [...(A.properties || [])].sort(),
	...
	dyn: {
		...
		properties: typeIsNot(type, "object")
			? []
			: merge(A.properties || [], d.properties || []),
		patterns: typeIsNot(type, "object")
			? []
			: merge(A.patterns || [], d.patterns || []),
	},
	unknown:
		(A.unknown && !(typeIsNot(type, "object") && typeIsNot(type, "array"))) ||
		false,
});
...
const regtest = (pattern, value) =>
	value !== true && new RegExp(pattern, "u").test(value);

const intersectProps = (
	{ properties: a, patterns: rega },
	{ properties: b, patterns: regb },
) => {
	// properties
	const af = a.filter(
		(x) => b.includes(x) || b.includes(true) || regb.some((p) => regtest(p, x)),
	);
	const bf = b.filter(
		(x) => a.includes(x) || a.includes(true) || rega.some((p) => regtest(p, x)),
	);
	// patterns
...
const isDynamic = wrapArgs(({ unknown, items, dyn, ...stat }) => ({
	...
	properties:
		!stat.properties.includes(true) && (unknown || !inProperties(stat, dyn)),
}));
