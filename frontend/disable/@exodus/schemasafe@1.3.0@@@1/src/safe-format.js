...
const jsval = (val) => {
	if ([Infinity, -Infinity, NaN, undefined, null].includes(val))
		return `${val}`;
	...
	if (!primitive) {
		...
		const ok =
			(proto === Array.prototype && Array.isArray(val)) ||
...
const safepriority = (arg) =>
	...
	/^[a-z][a-z0-9_().]*$/i.test(arg) || /^\([^()]+\)$/i.test(arg)
		? arg
		: format("(%s)", arg);
const safeor = safewrap((...args) =>
	args.some((arg) => `${arg}` === "true")
		? "true"
		: args.join(" || ") || "false",
);
const safeand = safewrap((...args) =>
	args.some((arg) => `${arg}` === "false")
		? "false"
		: args.join(" && ") || "true",
);
const safenot = (arg) => {
