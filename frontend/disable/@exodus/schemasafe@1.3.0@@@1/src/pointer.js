...
function get(obj, pointer, objpath) {
	...
	for (const part of parts) {
		...
		if (!Object.hasOwn(curr, prop)) return undefined;
		curr = curr[prop];
	}
	return curr;
}
...
function traverse(schema, work) {
	const visit = (sub, specialChilds = false) => {
		...
		for (const k of Object.keys(sub)) {
			...
			const kres = visit(
				sub[k],
				!specialChilds && withSpecialChilds.includes(k),
			);
...
function resolveReference(root, schemas, ref, base = "") {
	...
	if (schemas.has(main) && schemas.get(main) !== root) {
		const additional = resolveReference(
			schemas.get(main),
			schemas,
			`#${hash}`,
			main,
		...
		results.push(
			...additional.map(([res, rRoot, rPath]) => [
				res,
				rRoot,
				joinPath(main, rPath),
			]),
...
const hasKeywords = (schema, keywords) =>
	traverse(
		schema,
		(s) => Object.keys(s).some((k) => keywords.includes(k)) || undefined,
	) || false;
...
const addSchemasArrayToMap = (schemas, input, optional = false) => {
	...
	for (const schema of input) {
		traverse(schema, (sub) => {
			...
			const id =
				idRaw && typeof idRaw === "string" ? idRaw.replace(/#$/, "") : null; // # is allowed only as the last symbol here
