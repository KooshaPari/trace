...
const jsonCheckWithErrors = (validate) =>
	function validateIsJSON(data) {
		if (!deepEqual(data, JSON.parse(JSON.stringify(data)))) {
			validateIsJSON.errors = [
				{ instanceLocation: "#", error: "not JSON compatible" },
			];
			return false;
		}
...
const validator = (
	...
	if (parse && (jsonCheck || isJSON))
		throw new Error(
			"jsonCheck and isJSON options are not applicable in parser mode",
		);
	...
	const options = {
		mode,
		...opts,
		schemas: buildSchemas(schemas, arg),
		isJSON: willJSON,
	};
	...
	if (parse) {
		...
		scope.jsonCheckWrap = opts.includeErrors
			? jsonCheckWithErrors
			: jsonCheckWithoutErrors;
	}
	if (multi) {
		fun.write("[");
	...
	validate.toModule = ({ semi = true } = {}) =>
		fun.makeModule(scope) + (semi ? ";" : "");
