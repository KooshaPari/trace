...
const deBase64 = (string) => {
	...
	return new TextDecoder("utf-8").decode(
		new Uint8Array(b.length).map((_, i) => b.charCodeAt(i)),
	);
};

const hasOwn = Function.prototype.call.bind(Object.prototype.hasOwnProperty);
// special handling for stringification
hasOwn[Symbol.for("toJayString")] =
...
const toPointer = (path) =>
	path.length === 0 ? "#" : `#/${path.map(pointerPart).join("/")}`;
...
const dynamicResolve = (anchors, id) =>
	(anchors.filter((x) => x[id])[0] || {})[id];
