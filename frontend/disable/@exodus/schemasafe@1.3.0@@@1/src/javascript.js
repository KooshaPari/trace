...
const types = new Map(
	Object.entries({
		...
		object: (n) =>
			format('typeof %s === "object" && %s && !Array.isArray(%s)', n, n, n),
...
const jsonProtoKeys = new Set(
	[].concat(
		...[Object, Array, String, Number, Boolean].map((c) =>
...
const jsHelpers = (
	...
	const forObjectKeys = (obj, writeBody) => {
		...
		fun.block(
			format("for (const %s of Object.keys(%s))", key, buildName(obj)),
			() => {
	...
	const forArray = (obj, start, writeBody) => {
		...
		fun.block(
			format("for (let %s = %s; %s < %s.length; %s++)", i, start, i, name, i),
			() => {
	...
	const patternTest = (pat, key) => {
		...
		if ([`${r}*`, `${r}?`].includes(pat))
			return subr.length === 0
				? format("true")
				: format("%s.includes(%j)", key, subr); // abc*, abc? -> includes(ab)
		if ([`^${r}*`, `^${r}?`].includes(pat))
			return subr.length === 0
				? format("true")
				: format("%s.startsWith(%j)", key, subr); // ^abc*, ^abc? -> startsWith(ab)
	...
	const compare = (name, val) => {
		...
		const shouldInline = (arr) =>
			arr.length <= 3 && arr.every((x) => !x || typeof x !== "object");
...
function jaystring(item) {
	if (typeof item === "function") {
		...
		if (Object.getPrototypeOf(item) !== Function.prototype)
			throw new Error(
				"Can not stringify: a function with unexpected prototype",
			);
		...
		throw new Error(
			"Can not stringify: only either normal or arrow functions are supported",
		);
