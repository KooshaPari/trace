...
var isTypedArrayIndex = (target, key) =>
	isTypedArray(target) &&
	!isSymbol(key) &&
	key in target &&
...
if (DESCRIPTORS) {
	...
	module.exports = (TYPE, wrapper, CLAMPED) => {
		...
		if (!NATIVE_ARRAY_BUFFER_VIEWS) {
			TypedArrayConstructor = wrapper((that, data, offset, $length) => {
			...
			TypedArrayConstructor = wrapper(
				(dummy, data, typedArrayOffset, $length) => {
