...
var IteratorProxy = createIteratorProxy(
	function () {
		...
		while (true) {
			...
			if (allowPartial && done && buffer.length && buffer.length < windowSize)
				return createIterResultObject(buffer, false);
...
module.exports = (O, windowSize, undersized) => {
	anObject(O);
	if (
		...
		return iteratorClose(
			O,
			"throw",
			new $RangeError("`windowSize` must be integer in [1, 2^32-1]"),
		);
	}
	if (
		undersized !== undefined &&
		undersized !== "only-full" &&
		undersized !== ALLOW_PARTIAL
	) {
		return iteratorClose(
			O,
			"throw",
			new $TypeError("Incorrect `undersized` argument"),
		);
	}
