...
class Logger {
	...
	set level(level) {
		this.#level =
			(this.#levels.get(level) && this.#levels.get(level).id) || null;
	}
	...
	isVisible(level) {
		return (
			(this.level &&
				this.level.index <=
					(this.#levels.get(level) && this.#levels.get(level).index)) ||
			-1
		);
	}
	...
	_log(levelId, msgPrefix, ...args) {
		if (
			!this.isVisible(levelId) ||
			!this.#stream ||
			typeof this.#stream.write !== "function"
		) {
			return;
		}
