...
describe("download", () => {
	...
	it("download over https with custom ca", async () => {
	...
	it("download over http with proxy", async () => {
	...
	it("download over http with noproxy", async () => {
		const server = http.createServer((req, res) => {
			assert.strictEqual(
				req.headers["user-agent"],
				`node-gyp v42 (node ${process.version})`,
	...
	it("download with missing cafile", async () => {
		const gyp = {
	...
	it("check certificate splitting", async () => {
