...
describe("configure-nodedir", () => {
	it("configure nodedir with node-gyp command line", (done) => {
		const prog = gyp();
		prog.parseArgv([
			"dummy_prog",
			"dummy_script",
			"--nodedir=" + path.sep + "usr",
		]);

		prog.spawn = (program, args) => {
	...
	if (process.config.variables.use_prefix_to_find_headers) {
		it("use-prefix-to-find-headers build time option - match", (done) => {
			const prog = gyp();
		...
		it("use-prefix-to-find-headers build time option - no match", (done) => {
			const prog = gyp();
			prog.parseArgv(["dummy_prog", "dummy_script"]);
		...
		it("use-prefix-to-find-headers build time option, target specified", (done) => {
			const prog = gyp();
