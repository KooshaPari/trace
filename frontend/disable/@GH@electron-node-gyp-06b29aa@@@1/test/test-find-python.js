...
describe("find-python", () => {
			...
			switch (err.code) {
				case "EPERM":
					return assert.fail(
						err,
						null,
						"Please try to run console as an administrator",
					);
				default:
	...
	it("find python - python", async () => {
		const f = new TestPythonFinder("python");
		f.execFile = async (program, args, opts) => {
			f.execFile = async (program, args, opts) => {
				poison(f, "execFile");
	...
	it("find python - python too old", async () => {
	...
	it("find python - no python", async () => {
		const f = new TestPythonFinder(null);
		f.execFile = async (program, args, opts) => {
			if (/sys\.executable/.test(args[args.length - 1])) {
	...
	it("find python - no python2, no python, unix", async () => {
		const f = new TestPythonFinder(null);
	...
	it("find python - no python, use python launcher", async () => {
		const f = new TestPythonFinder(null);
		f.win = true;
	...
	it("find python - no python, no python launcher, good guess", async () => {
		const f = new TestPythonFinder(null);
		f.win = true;
	...
	it("find python - no python, no python launcher, bad guess", async () => {
		const f = new TestPythonFinder(null);
		f.win = true;
