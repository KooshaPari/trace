...
describe("find-visualstudio", function () {
	...
	it("VS2013 should not be found on new node versions", async () => {
		...
		finder.findVisualStudio2019OrNewer = async () => {
			...
			const parsedData = finder.parseData(null, data, "", {
				checkIsArray: true,
			});
	...
	it("error from PowerShell", async () => {
		...
		assert.ok(
			/error msg/i.test(finder.errorLog[0]),
			`expect error, output: ${finder.errorLog[0]}`,
		);
		assert.equal(vsInfo, null);
	});

	it("empty output from PowerShell", async () => {
		const finder = new TestVisualStudioFinder(semverV1, null, null);
	...
	it("output from PowerShell not JSON", async () => {
		const finder = new TestVisualStudioFinder(semverV1, null, null);

		const vsInfo = finder.parseData(null, "AAAABBBB", "", {
			checkIsArray: true,
		});
		assert.ok(
			/use PowerShell/i.test(finder.errorLog[0]),
	...
	it("wrong JSON from PowerShell", async () => {
		const finder = new TestVisualStudioFinder(semverV1, null, null);
	...
	it("empty JSON from PowerShell", async () => {
		const finder = new TestVisualStudioFinder(semverV1, null, null);
	...
	it("single unusable VS2017", async () => {
		const finder = new TestVisualStudioFinder(semverV1, null, null);
	...
	it("minimal VS2017 Build Tools", async () => {
		const finder = new TestVisualStudioFinder(semverV1, null);
	...
	it("VS2017 Community with C++ workload", async () => {
		const finder = new TestVisualStudioFinder(semverV1, null);
	...
	it("VS2019 Preview with C++ workload", async () => {
		const finder = new TestVisualStudioFinder(semverV1, null);
	...
	it("minimal VS2019 Build Tools", async () => {
		const finder = new TestVisualStudioFinder(semverV1, null);
	...
	it("VS2019 Community with C++ workload", async () => {
		const finder = new TestVisualStudioFinder(semverV1, null);
	...
	it("VS2022 Preview with C++ workload", async () => {
		const msBuildPath =
			process.arch === "arm64"
	...
	it("VSSetup: VS2022 with C++ workload without SDK", async () => {
		const finder = new TestVisualStudioFinder(semverV1, null);
		finder.msBuildPathExists = (path) => {
			return true;
		};
	...
	it("VSSetup: VS2019 with C++ workload", async () => {
	...
	it("VSSetup: VS2022 with C++ workload", async () => {
		await verifyVSSetupData(
	...
	it("VSSetup: VS2022 and VS2019 with C++ workload", async () => {
		await verifyVSSetupData(
	...
	it("VSSetup: VS2022 with multiple installations", async () => {
		await verifyVSSetupData(
	...
	function setupExecFixture(finder, fixtureName) {
		finder.execFile = async (exec, args) => {
			if (args.length > 2 && args[2].includes("Get-Module")) {
				...
				args.length > 2 &&
				args.at(-1).includes("Get-VSSetupInstance")
			) {
	...
	it("fail when looking for invalid path", async () => {
	...
	it("look for VS2013 by version number", async () => {
	...
	it("look for VS2013 by installation path", async () => {
	...
	it("look for VS2015 by version number", async () => {
	...
	it("look for VS2015 by installation path", async () => {
	...
	it("look for VS2017 by version number", async () => {
	...
	it("look for VS2017 by installation path", async () => {
		const finder = new TestVisualStudioFinder(
			semverV1,
	...
	it("look for VS2019 by version number", async () => {
	...
	it("look for VS2019 by installation path", async () => {
		const finder = new TestVisualStudioFinder(
			semverV1,
	...
	it("look for VS2022 by version number", async () => {
	...
	it("msvs_version match should be case insensitive", async () => {
		const finder = new TestVisualStudioFinder(
			semverV1,
			"c:\\program files (x86)\\microsoft visual studio\\2019\\BUILDTOOLS",
		);
	...
	it("latest version should be found by default", async () => {
		const finder = new TestVisualStudioFinder(semverV1, null);
	...
	it("run on a usable VS Command Prompt", async () => {
	...
	it("VCINSTALLDIR match should be case insensitive", async () => {
	...
	it("run on a unusable VS Command Prompt", async () => {
		process.env.VCINSTALLDIR =
	...
	it("run on a VS Command Prompt with matching msvs_version", async () => {
		process.env.VCINSTALLDIR = "C:\\VS2015\\VC";
	...
	it("run on a VS Command Prompt with mismatched msvs_version", async () => {
		process.env.VCINSTALLDIR =
			"C:\\Program Files (x86)\\Microsoft Visual Studio\\2019\\BuildTools\\VC";
	...
	it("run on a portable VS Command Prompt with sufficient environs", async () => {
		process.env.VCINSTALLDIR =
