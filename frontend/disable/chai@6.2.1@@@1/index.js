...
var getPromiseValue = /* @__PURE__ */ __name(
	() => "Promise{\u2026}",
	"getPromiseValue",
);
...
var baseTypesMap = {
	...
	null: /* @__PURE__ */ __name(
		(value, options) => options.stylize("null", "null"),
		"null",
	),
...
var inspectCustom = /* @__PURE__ */ __name(
	(value, options, type3, inspectFn) => {
...
var _Assertion = class _Assertion {
	...
	static addProperty(name, fn) {
		addProperty(_Assertion.prototype, name, fn);
	}
	/**
	 * @param {string} name
	 * @param {Function} fn
	...
	static addMethod(name, fn) {
		addMethod(_Assertion.prototype, name, fn);
	}
	/**
	 * @param {string} name
	 * @param {Function} fn
	...
	static addChainableMethod(name, fn, chainingBehavior) {
		addChainableMethod(_Assertion.prototype, name, fn, chainingBehavior);
	}
	/**
	 * @param {string} name
	 * @param {Function} fn
	...
	static overwriteProperty(name, fn) {
		overwriteProperty(_Assertion.prototype, name, fn);
	}
	/**
	 * @param {string} name
	 * @param {Function} fn
	...
	static overwriteMethod(name, fn) {
		overwriteMethod(_Assertion.prototype, name, fn);
	}
	/**
	 * @param {string} name
	 * @param {Function} fn
	...
	static overwriteChainableMethod(name, fn, chainingBehavior) {
		overwriteChainableMethod(_Assertion.prototype, name, fn, chainingBehavior);
	}
	/**
...
var fnLengthDesc = Object.getOwnPropertyDescriptor(() => {}, "length");
...
function addMethod(ctx, name, method) {
	const methodWrapper = /* @__PURE__ */ __name(function () {
...
function overwriteMethod(ctx, name, method) {
	let _method = ctx[name],
		_super = /* @__PURE__ */ __name(() => {
			throw new Error(name + " is not a function");
		}, "_super");
	if (_method && "function" === typeof _method) _super = _method;
	const overwritingMethodWrapper = /* @__PURE__ */ __name(function () {
		if (!flag(this, "lockSsfi")) {
...
var testFn = /* @__PURE__ */ __name(() => {}, "testFn");
var excludeNames = Object.getOwnPropertyNames(testFn).filter((name) => {
...
function addChainableMethod(ctx, name, method, chainingBehavior) {
	if (typeof chainingBehavior !== "function") {
		chainingBehavior = /* @__PURE__ */ __name(() => {}, "chainingBehavior");
	}
	...
	Object.defineProperty(ctx, name, {
		get: /* @__PURE__ */ __name(function chainableMethodGetter() {
			...
			const chainableMethodWrapper = /* @__PURE__ */ __name(function () {
				if (!flag(this, "lockSsfi")) {
...
function overwriteChainableMethod(ctx, name, method, chainingBehavior) {
	...
	chainableBehavior.method = /* @__PURE__ */ __name(
...
function an(type3, msg) {
	...
	const obj = flag2(this, "object"),
		article = ~["a", "e", "i", "o", "u"].indexOf(type3.charAt(0))
			? "an "
			: "a ";
...
function include(val, msg) {
	...
	switch (objType) {
		...
		case "array":
			if (isDeep) {
				included = obj.some((item) => isEql(item, val));
			} else {
				included = obj.indexOf(val) !== -1;
			}
			break;
...
function assertEqual(val, msg) {
	...
	const obj = flag2(this, "object");
	if (flag2(this, "deep")) {
		const prevLockSsfi = flag2(this, "lockSsfi");
...
Assertion.addMethod("within", function (start, finish, msg) {
	...
	if (
		...
		(!isNumeric(start) || !isNumeric(finish)) &&
		(doLength || isNumeric(obj))
	) {
...
function assertInstanceOf(constructor, msg) {
	if (msg) flag2(this, "message", msg);
	const target = flag2(this, "object");
	const ssfi = flag2(this, "ssfi");
...
function assertProperty(name, val, msg) {
	...
	descriptor += "property ";
	let hasProperty2;
	if (isOwn) hasProperty2 = Object.hasOwn(obj, name);
...
function assertKeys(keys) {
	...
	if (any) {
		ok = expected.some((expectedKey) =>
			actual.some((actualKey) => isEql(expectedKey, actualKey)),
		);
	}
	if (all) {
	...
	if (len > 1) {
		keys = keys.map((key) => inspect2(key));
		const last = keys.pop();
		if (all) {
...
function assertThrows(errorLike, errMsgMatcher, msg) {
	...
	const everyArgIsUndefined = errorLike === void 0 && errMsgMatcher === void 0;
	...
	if (errorLike && caughtErr) {
		if (errorLike instanceof Error) {
			const isCompatibleInstance = check_error_exports.compatibleInstance(
				caughtErr,
				errorLike,
			);
		...
		const isCompatibleConstructor = check_error_exports.compatibleConstructor(
			caughtErr,
			errorLike,
		);
	...
	if (caughtErr && errMsgMatcher !== void 0 && errMsgMatcher !== null) {
		...
		const isCompatibleMessage = check_error_exports.compatibleMessage(
			caughtErr,
			errMsgMatcher,
		);
...
Assertion.addMethod("members", function (subset, msg) {
	...
	const contains = flag2(this, "contains");
	const ordered = flag2(this, "ordered");
	...
	const cmp = flag2(this, "deep") ? flag2(this, "eql") : void 0;
	this.assert(
...
function oneOf(list, msg) {
	if (msg) flag2(this, "message", msg);
	const expected = flag2(this, "object"),
		flagMsg = flag2(this, "message"),
		ssfi = flag2(this, "ssfi"),
	...
	if (contains) {
		...
		if (isDeep) {
			this.assert(
				list.some((possibility) => eql(expected, possibility)),
...
function assertChanges(subject, prop, msg) {
	...
	fn();
	const final = prop === void 0 || prop === null ? subject() : subject[prop];
	const msgObj = prop === void 0 || prop === null ? initial : "." + prop;
...
function assertDelta(delta, msg) {
	if (msg) flag2(this, "message", msg);
	const msgObj = flag2(this, "deltaMsgObj");
	const initial = flag2(this, "initialDeltaValue");
	const final = flag2(this, "finalDeltaValue");
	const behavior = flag2(this, "deltaBehavior");
	const realDelta = flag2(this, "realDelta");
	let expression;
	if (behavior === "change") {
...
function compareSubset(expected, actual) {
	...
	if (Array.isArray(expected)) {
		...
		return expected.every((exp) =>
			actual.some((act) => compareSubset(exp, act)),
		);
	}
...
function loadShould() {
	...
	should2.equal = (actual, expected, message) => {
		new Assertion(actual, message).to.equal(expected);
	};
	should2.Throw = (fn, errt, errs, msg) => {
		new Assertion(fn, msg).to.Throw(errt, errs);
	};
	should2.exist = (val, msg) => {
		new Assertion(val, msg).to.exist;
	};
	should2.not = {};
	should2.not.equal = (actual, expected, msg) => {
		new Assertion(actual, msg).to.not.equal(expected);
	};
	should2.not.Throw = (fn, errt, errs, msg) => {
		new Assertion(fn, msg).to.not.Throw(errt, errs);
	};
	should2.not.exist = (val, msg) => {
...
function assert(express, errmsg) {
	const test2 = new Assertion(null, null, assert, true);
...
assert.equal = (act, exp, msg) => {
	const test2 = new Assertion(act, msg, assert.equal, true);
	test2.assert(
...
assert.notEqual = (act, exp, msg) => {
	const test2 = new Assertion(act, msg, assert.notEqual, true);
	test2.assert(
		exp != flag(test2, "object"),
		"expected #{this} to not equal #{exp}",
...
assert.isCallable = (value, message) => {
	new Assertion(value, message, assert.isCallable, true).is.callable;
};
assert.isNotCallable = (value, message) => {
...
assert.isNumeric = (val, msg) => {
	new Assertion(val, msg, assert.isNumeric, true).is.numeric;
};
assert.isNotNumeric = (val, msg) => {
...
assert.typeOf = (val, type3, msg) => {
	new Assertion(val, msg, assert.typeOf, true).to.be.a(type3);
};
assert.notTypeOf = (value, type3, message) => {
	new Assertion(value, message, assert.notTypeOf, true).to.not.be.a(type3);
};
assert.instanceOf = (val, type3, msg) => {
	new Assertion(val, msg, assert.instanceOf, true).to.be.instanceOf(type3);
};
assert.notInstanceOf = (val, type3, msg) => {
...
assert.nestedInclude = (exp, inc, msg) => {
	new Assertion(exp, msg, assert.nestedInclude, true).nested.include(inc);
};
assert.notNestedInclude = (exp, inc, msg) => {
	new Assertion(exp, msg, assert.notNestedInclude, true).not.nested.include(
		inc,
	);
};
assert.deepNestedInclude = (exp, inc, msg) => {
	new Assertion(exp, msg, assert.deepNestedInclude, true).deep.nested.include(
		inc,
	);
};
assert.notDeepNestedInclude = (exp, inc, msg) => {
	new Assertion(
		exp,
		msg,
...
assert.ownInclude = (exp, inc, msg) => {
	new Assertion(exp, msg, assert.ownInclude, true).own.include(inc);
};
assert.notOwnInclude = (exp, inc, msg) => {
	new Assertion(exp, msg, assert.notOwnInclude, true).not.own.include(inc);
};
assert.deepOwnInclude = (exp, inc, msg) => {
	new Assertion(exp, msg, assert.deepOwnInclude, true).deep.own.include(inc);
};
assert.notDeepOwnInclude = (exp, inc, msg) => {
...
assert.throws = (fn, errorLike, errMsgMatcher, msg) => {
	...
	const assertErr = new Assertion(fn, msg, assert.throws, true).to.throw(
		errorLike,
		errMsgMatcher,
	);
...
assert.operator = (val, operator, val2, msg) => {
	...
	const test2 = new Assertion(ok, msg, assert.operator, true);
	test2.assert(
...
assert.containsSubset = (val, exp, msg) => {
	new Assertion(val, msg).to.containSubset(exp);
};
assert.doesNotContainSubset = (val, exp, msg) => {
