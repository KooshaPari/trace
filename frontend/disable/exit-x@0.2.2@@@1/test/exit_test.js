...
exports["exit"] = {
	...
	grep: (test) => {
		...
		test.ok(
			grep,
			'A suitable "grep" or "find" program was not found in the PATH.',
		);
		test.done();
	},
...
function buildTests() {
	...
	counts.forEach((count) => {
		outputs.forEach((output) => {
			pipes.forEach((pipe) => {
				...
				exports["exit"]["output (" + command + ")"] = (test) => {
					test.expect(2);
					run(command, (code, actual) => {
						...
						test.ok(
							actual.indexOf("fail") === -1,
							"should not output after exit is called.",
						);
						test.done();
					});
				};
	...
	codes.forEach((code) => {
		...
		exports["exit"]["exit code (" + command + ")"] = (test) => {
			test.expect(1);
			run(command, (actual) => {
