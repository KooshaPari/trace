...
suite("rule", function () {
	...
	test.skip("rule w pattern w folder w namespace", () => {
		...
		assert.equal(
			"src/src_1src/src_2src/src_3 pattern folder namespace",
			data.toString(),
		);
		cleanUpAndNext();
	});
	...
	["precedence", "regexPattern", "sourceFunction"].forEach((key) => {
		...
		test("rule with source file now created (" + key + ")", () => {
			...
			const out = exec(`${JAKE_CMD} -q  ` + key + ":test")
				.toString()
				.trim();
			// Should run prereq and test task
			const output = ["created html", "ran test"];
			assert.equal(output.join("\n"), out);
		});
