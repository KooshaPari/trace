...
namespace("foo", () => {
	...
	task("bar", function () {
		if (arguments.length) {
			console.log(
				"foo:bar[" + Array.prototype.join.call(arguments, ",") + "] task",
			);
		} else {
	...
	desc(
		"The foo:frang task,`invokes` foo:bar with passed args as a prerequisite.",
	);
	task("frang", function () {
	...
	desc(
		"The foo:zerb task, `executes` foo:bar with passed args as a prerequisite.",
	);
	task("zerb", function () {
		const t = jake.Task["foo:bar"];
		// Do args pass-through
...
namespace("bar", () => {
	desc(
		"The bar:foo task, has no prerequisites, is async, returns Promise which resolves.",
	);
	task(
		"foo",
		async () =>
	...
	desc(
		"The bar:promise task has no prerequisites, is async, returns Q-based promise.",
	);
	task("promise", () =>
		Q().then(() => {
	...
	task("dependOnpromise", ["promise"], () => {
		console.log(
			"bar:dependOnpromise task saw value",
			jake.Task["bar:promise"].value,
		);
	});
...
namespace("fileTest", () => {
	...
	file(
		"foo/concat.txt",
		["fileTest:foo", "fileTest:foo/src1.txt", "fileTest:foo/src2.txt"],
		() => {
	...
	desc(
		"File task, has a preexisting file (with no associated task) as a prereq",
	);
