...
class Task extends EventEmitter {
	constructor(name, prereqs, action, options) {
		...
		if (name.indexOf(":") > -1) {
			throw new Error(
				"Task name cannot include a colon. It is used internally as namespace delimiter.",
			);
		}
		const opts = options || {};
		...
		if (Array.isArray(this.prereqs) && this.prereqs.indexOf(this.name) !== -1) {
			throw new Error(
				"Cannot use prereq " + this.name + " as a dependency of itself",
			);
		}
	}
	...
	_initInvocationChain() {
		...
		if (!this._invocationChain) {
			...
			if (jake.currentRunningTask) {
				jake.currentRunningTask._waitForChains =
					jake.currentRunningTask._waitForChains || [];
   ...
   */
		...
		if (this.prereqs && this.prereqs.length) {
			if (this.concurrency > 1) {
				async.eachLimit(
					...
					(name, cb) => {
						const parsed = parsePrereqName(name);

						const prereq =
							this.namespace.resolveTask(parsed.name) ||
	...
	nextPrereq() {
		const index = this._currentPrereqIndex;
		const name = this.prereqs[index];
		let prereq;
		let parsed;
   ...
   */
	...
	run() {
		let val, previous;
		const hasAction = typeof this.action == "function";
				...
				if (!(this.reenabled && previous == this)) {
					if (previous.taskStatus != Task.runStatuses.DONE) {
						...
						if (now - this.startTime > jake._taskTimeout) {
							return jake.fail(
								`Timed out waiting for task: ${previous.name} with status of ${previous.taskStatus}`,
							);
						}
	...
	complete(val) {
		if (Array.isArray(this._waitForChains)) {
			const stillWaiting = this._waitForChains.some((chain) => {
				return !(
					chain.chainStatus == Task.runStatuses.DONE ||
			...
			if (stillWaiting) {
				...
				if (elapsed > jake._taskTimeout) {
					return jake.fail(
						`Timed out waiting for task: ${this.name} with status of ${this.taskStatus}. Elapsed: ${elapsed}`,
					);
				}
				setTimeout(() => {
					this.complete(val);
		...
		if (!this._skipped) {
			...
			const taskTime = this.endTime - this.startTime;
			...
			if (!(this._internal || jake.program.opts.quiet)) {
				...
				console.log(
					"Finished '" +
						pc.green(this.fullName) +
						"' after " +
						pc.magenta(taskTime + " ms"),
				);
			}
		}
	}
	...
	_getParams() {
		...
		const params = (new RegExp(
			"(?:" + this.action.name + "\\s*|^)\\s*\\((.*?)\\)",
		).exec(this.action.toString().replace(/\n/g, "")) || [""])[1]
			.replace(/\/\*.*?\*\//g, "")
			.replace(/ /g, "");
		return params;
	}
