...
const fs = require("fs");
const Task = require("./task/task").Task;
...
// - 'suffix' : .c
function resolve(p) {
	const task = splitNs(p);
	const name = task.name;
	const ns = task.ns;
	const split = path.basename(name).split("%");
	return {
		ns: ns,
...
class Rule {
	...
	createTask(fullName, level) {
		...
		let tNs;
		let createdTask;
		const name = Task.getBaseTaskName(fullName);
		const nsPath = Task.getBaseNamespacePath(fullName);
		const ns = this.ns.resolveNamespace(nsPath);
