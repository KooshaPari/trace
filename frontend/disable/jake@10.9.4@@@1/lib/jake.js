...
if (!global.jake) {
	...
	const Task = taskNs.Task;
	const FileTask = taskNs.FileTask;
	const DirectoryTask = taskNs.DirectoryTask;
	const Rule = require("./rule").Rule;
	const Namespace = require("./namespace").Namespace;
	const RootNamespace = require("./namespace").RootNamespace;
	...
	Object.assign(
		...
		new (function () {
			...
			this.parseAllTasks = () => {
				const _parseNs = (ns) => {
					const nsTasks = ns.tasks;
					const nsNamespaces = ns.childNamespaces;
					for (const q in nsTasks) {
						const nsTask = nsTasks[q];
						jake.Task[nsTask.fullName] = nsTask;
					}
					for (const p in nsNamespaces) {
						const nsNamespace = nsNamespaces[p];
			 ...
			 * a Jakefile
			 */
			this.showAllTaskDescriptions = (f) => {
				let p;
				...
				for (p in jake.Task) {
					...
					if (task.description) {
						...
						maxTaskNameLength =
							len > maxTaskNameLength ? len : maxTaskNameLength;
					}
				}
				...
				for (p in jake.Task) {
					...
					if (descr) {
						...
						padding = " ".repeat(
							maxTaskNameLength - p.length - taskParams.length + 4,
						);
			...
			this.attemptRule = (name, ns, level) => {
				let prereqRule;
				let prereq;
			...
			this.createPlaceholderFileTask = (name, namespace) => {
				const parsed = name.split(":");
				const filePath = parsed.pop(); // Strip any namespace
				let task;
			...
			this.run = function () {
				...
				if (preempt) {
					...
					if (!jakefileLoaded && !jakelibdirLoaded && !opts.autocomplete) {
						fail(
							"No Jakefile. Specify a valid path with -f/--jakefile, " +
