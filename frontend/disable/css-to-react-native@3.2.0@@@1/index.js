...
var noopToken = function noopToken(predicate) {
	return (node) => (predicate(node) ? "<token>" : null);
};

var valueForTypeToken = function valueForTypeToken(type) {
	return (node) => (node.type === type ? node.value : null);
};
...
var SPACE = noopToken((node) => node.type === "space");
var SLASH = noopToken((node) => node.type === "div" && node.value === "/");
var COMMA = noopToken((node) => node.type === "div" && node.value === ",");
...
var ANGLE = regExpToken(angleRe, (angle) => angle.toLowerCase());
...
var xyTransformFactory = function xyTransformFactory(tokenType) {
	return (key, valueIfOmitted) => (functionStream) => {
		var _ref3, _ref4;
...
var index = function index(rules, shorthandBlacklist) {
	...
	return rules.reduce((accum, rule) => {
