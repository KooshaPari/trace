...
const areaCode = (sys, chars, phraseIndex, data) => {
	if (sys.state === id.MATCH) {
		...
		data["area-code"] = utils.charsToString(
			chars,
			phraseIndex,
			sys.phraseLength,
		);
...
test("test the stats display for rules and UDTs", () => {
	...
	expect(/RNM \| {6}10 \| {7}0 \| {7}0 \| {6}10 \|/.test(out)).toBe(true);
	expect(/UDT \| {7}1 \| {7}0 \| {7}0 \| {7}1 \|/.test(out)).toBe(true);
	out = parser.stats.displayHits();
	expect(/\| {7}5 \| {7}0 \| {7}0 \| {7}5 \| digit/.test(out)).toBe(true);
});
