...
function typesPlugin(_fork) {
	...
	var DefImpl = /** @class */ ((_super) => {
		...
		DefImpl.prototype.build = function () {
			...
			this.field("type", String, () => _this.typeName);
			...
			var addParam = (built, param, arg, isArgAvailable) => {
				...
				if (isArgAvailable) {
					...
					var message =
						...
						_this.typeName +
						"(" +
						_this.buildParams.map((name) => all[name]).join(", ") +
						")";
			...
			var builder = function () {
				...
				if (!_this.finalized) {
					throw new Error(
						"attempting to instantiate unfinalized type " + _this.typeName,
					);
				}
				var built = Object.create(nodePrototype);
				_this.buildParams.forEach((param, i) => {
					if (i < argc) {
				...
				Object.keys(_this.allFields).forEach((param) => {
