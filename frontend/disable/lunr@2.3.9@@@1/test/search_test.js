			...
			suite("one match", function () {
				var assertions = function () {
					...
					test("matching term", function () {
						assert.sameMembers(
							["scarlett"],
							Object.keys(this.results[0].matchData.metadata),
						);
					});
				};
		...
		suite("multiple terms", function () {
			suite("all terms match", function () {
				...
				test("matched terms returned", function () {
					...
					assert.sameMembers(
						["body"],
						Object.keys(this.results[0].matchData.metadata["fellow"]),
					);
					assert.sameMembers(
						["body"],
						Object.keys(this.results[0].matchData.metadata["candlestick"]),
					);
				});
			});
			...
			suite("documents with all terms score higher", function () {
				...
				test("matching terms are returned", function () {
					...
					assert.sameMembers(
						["green"],
						Object.keys(this.results[1].matchData.metadata),
					);
					assert.sameMembers(
						["green"],
						Object.keys(this.results[2].matchData.metadata),
					);
				});
			});
