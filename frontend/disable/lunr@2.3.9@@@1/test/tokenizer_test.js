suite("lunr.tokenizer", function () {
	...
	test("undefined is converted to empty string", function () {
		var tokens = lunr.tokenizer(["foo", undefined, "baz"]).map(toString);
	...
	test("converting a date to tokens", function () {
		...
		assert.sameMembers(
			["tue", "jan", "01", "2013"],
			lunr.tokenizer(date).slice(0, 4).map(toString),
		);
	});
	...
	test("splits strings with hyphens and spaces", function () {
		assert.sameMembers(lunr.tokenizer("foo - bar").map(toString), [
			"foo",
			"bar",
		]);
	});
