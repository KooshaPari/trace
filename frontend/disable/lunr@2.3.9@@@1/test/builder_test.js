suite("lunr.Builder", function () {
	suite("#add", function () {
		...
		test("field contains terms that clash with object prototype", function () {
			...
			assert.equal(
				this.builder.fieldTermFrequencies["title/id"].constructor,
				1,
			);
		});
		...
		test("field name clashes with object prototype", function () {
			...
			assert.deepProperty(
				this.builder.invertedIndex,
				"constructor.constructor.id",
			);
			assert.deepEqual(
				this.builder.invertedIndex.constructor.constructor.id,
				{},
			);
		});
		...
		test("token metadata clashes with object prototype", function () {
			...
			assert.deepEqual(this.builder.invertedIndex.word.title.id.constructor, [
				"foo",
			]);
		});
