...
var _cookieValue = _interopRequireDefault(
	require("../../../cookie/test/cookie-value.cjs"),
);
...
const cookieValueLenientPercentEncoder = (cookieValue) => {
	...
	for (const char of valueToEncode) {
		result += parser.parse(grammar, startRule, char).success
			? char
			: (0, _utils.percentEncodeChar)(char);
	}

	// return quoted if input was quoted, unquoted otherwise
	return isQuoted ? (0, _utils.quote)(result) : result;
};
var _default = (exports.default = cookieValueLenientPercentEncoder);
