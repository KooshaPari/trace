...
const evaluate = (
	...
	const tracer =
		typeof trace === "object" && trace !== null
			? new _TraceBuilder.default(trace, {
					jsonPointer,
					referenceTokens,
					strictArrays,
					strictObjects,
	...
	try {
		...
		if (!parseResult.success) {
			...
			message += parseTrace
				? `, expected ${parseTrace.inferExpectations()}`
				: "";
		...
		return referenceTokens.reduce(
			(current, referenceToken, referenceTokenPosition) => {
				if (realm.isArray(current)) {
					if ((0, _arrayDash.default)(referenceToken)) {
						if (strictArrays) {
							throw new _JSONPointerIndexError.default(
				...
				if (realm.isObject(current)) {
					if (!realm.has(current, referenceToken) && strictObjects) {
						throw new _JSONPointerKeyError.default(
				...
				throw new _JSONPointerTypeError.default(
		...
		throw new _JSONPointerEvaluateError.default(
			"Unexpected error during JSON Pointer evaluation",
			{
				cause: error,
				jsonPointer,
				referenceTokens,
