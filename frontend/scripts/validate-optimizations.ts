...
async function validatePackageOptimizations(): Promise<void> {
	...
	addCheck(
		...
		"Bun package manager",
		hasBunManager,
		pkg.packageManager || "Not specified",
	);
	...
	addCheck(
		...
		"Lock file exists",
		hasLockFile,
		hasLockFile ? "bun.lock found" : "Missing",
	);
...
async function validateBuildOptimizations(): Promise<void> {
	...
	addCheck(
		...
		hasCache,
		hasCache ? "Cache enabled" : "No cache",
	);
	...
	if (existsSync(viteConfigPath)) {
		...
		const hasSwc =
			viteConfig.includes("react-swc") ||
			viteConfig.includes("@vitejs/plugin-react-swc");
		addCheck(
			"Build",
			"SWC compiler enabled",
			hasSwc,
			hasSwc ? "Using React SWC" : "Using babel",
		);
	}
}
...
async function validateTestSetup(): Promise<void> {
	...
	const hasPlaywrightConfig =
		existsSync("apps/web/playwright.config.ts") ||
