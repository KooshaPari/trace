...
runPerformanceAudit()
  .then((metrics) => {
    ...
    // Success criteria
    const checks = [
      { name: 'Build time', pass: metrics.buildTime < 60_000, value: `${(metrics.buildTime / 1000).toFixed(2)}s` },
      { name: 'Bundle size', pass: metrics.bundleSizes.total * 0.3 < 204_800, value: formatBytes(metrics.bundleSizes.total * 0.3) }
    ];
