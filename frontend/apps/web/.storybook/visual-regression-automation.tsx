...
function getComponentConfig(
  ...
  return Object.hasOwn(COMPONENT_VISUAL_CONFIGS, componentName)
    ? COMPONENT_VISUAL_CONFIGS[componentName as ConfigKey]
    : undefined;
}
...
export function generateVisualTestParameters(
  ...
  return {
    chromatic: {
      ...
      modes: mergedConfig.themes.reduce<Record<string, { query: string }>>((acc, theme) => {
        const themeConfig = Object.hasOwn(THEMES, theme) ? THEMES[theme] : undefined;
        if (themeConfig != null) {
...
export function validateComponentVisualTests(
  ...
  const missingViewports = requiredViewports.filter((v) => !viewportList.includes(v));
