...
const button = page.locator('button').filter({ hasText: /delete/i }).first();
...
await page.locator('tbody tr').filter({ hasText: /Target Item/i }).first().click();
...
// Generic card/div
const card = page.locator('div').filter({ hasText: /Target Item/i }).first();
await card.click();
```
...
await page.locator('button').filter({ hasText: /new|create/i }).click();
...
const exists = await page.locator('[data-testid="item"]').isVisible().catch(() => false);
