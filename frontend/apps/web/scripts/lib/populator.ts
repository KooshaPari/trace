...
export async function populateProject(
	...
	const totalExpected = Object.values(projectConfig.itemCounts).reduce(
		(a, b) => a + b,
		0,
	);
	...
	if (existingCount > totalExpected * 0.8) {
		console.log(
			`  ⏭️  Skipping - project already has ${existingCount}/${totalExpected} items`,
		);
		return { itemCount: 0, linkCount: 0 };
	}

	console.log(
		`  ℹ️  Found ${existingCount}/${totalExpected} existing items, will add more...`,
	);
	...
	const totalItems = Object.values(allItemsByType).reduce(
		(a, b) => a + b.length,
		0,
	);
