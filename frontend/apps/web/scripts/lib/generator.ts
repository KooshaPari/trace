		...
		switch (itemType) {
			...
			case "feature":
				items = await createItemsBatch(
					...
					(i) =>
						`Feature implementation ${i} for ${projectName}. Enhances platform capabilities.`,
					(i) =>
						allItemsByType.requirement[i % allItemsByType.requirement.length]
							?.id,
				);
				break;
			...
			case "code":
				items = await createItemsBatch(
					...
					(i) => {
						const lang =
							projectDomain === "data" || projectDomain === "ai"
								? "python"
								: "typescript";
						const snippet = randomChoice(
							codeSnippets[lang as keyof typeof codeSnippets],
						);
...
export async function createTraceabilityLinks(
	...
	if (
		allItemsByType.requirement.length > 0 &&
		allItemsByType.feature.length > 0
	) {
		...
		for (let i = 0; i < linkCount; i++) {
			const req =
				allItemsByType.requirement[i % allItemsByType.requirement.length];
			const feat =
				allItemsByType.feature[
					Math.floor(Math.random() * allItemsByType.feature.length)
				];
	...
	if (allItemsByType.feature.length > 0 && allItemsByType.code.length > 0) {
		...
		for (let i = 0; i < linkCount; i++) {
			...
			const code =
				allItemsByType.code[
					Math.floor(Math.random() * allItemsByType.code.length)
				];
	...
	if (allItemsByType.code.length > 0 && allItemsByType.test.length > 0) {
		const linkCount = Math.min(
			allItemsByType.code.length,
			allItemsByType.test.length,
		);
		for (let i = 0; i < linkCount; i++) {
	...
	if (allItemsByType.code.length > 0 && allItemsByType.database.length > 0) {
		const linkCount = Math.min(
			allItemsByType.code.length / 2,
			allItemsByType.database.length * 3,
		);
		for (let i = 0; i < linkCount; i++) {
			const code =
				allItemsByType.code[
			...
			const db =
				allItemsByType.database[
					Math.floor(Math.random() * allItemsByType.database.length)
				];
	...
	if (
		allItemsByType.feature.length > 0 &&
		allItemsByType.wireframe.length > 0
	) {
		const linkCount = Math.min(
		...
		for (let i = 0; i < linkCount; i++) {
			...
			const wire =
				allItemsByType.wireframe[
					Math.floor(Math.random() * allItemsByType.wireframe.length)
				];
