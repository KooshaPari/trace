...
class VerificationTester {
	...
	async runAllTests(): Promise<VerificationReport> {
		...
		if (!testData) {
			throw new Error(
				"Failed to load test data. Run generate-test-graph.ts first.",
			);
		}
	...
	private async testMemoryUsage(testData: any) {
		...
		if (!passed) {
			this.recommendations.push(
				"Consider using memory pooling or data compression",
			);
		}
	}
	...
	private async testMaximumNodeCount() {
		...
		for (const size of testSizes) {
			...
			results.push(
				`${size}: ${Math.round(estimatedFPS)} FPS (${usable ? "usable" : "degraded"})`,
			);
		}
	...
	private async testZoomPerformance(testData: any) {
		...
		if (!passed) {
			this.recommendations.push(
				"Add animation frame throttling for zoom operations",
			);
		}
	}
