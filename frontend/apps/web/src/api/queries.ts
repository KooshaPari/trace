...
import type { Agent, Item, Link, Mutation, PaginatedResponse, Project } from '@tracertm/types'
...
export function useCreateProject(
  options?: UseMutationOptions<Project, Error, { name: string; description?: string }>
) {
  const queryClient = useQueryClient()

  return useMutation({
    mutationFn: (data) => handleApiResponse(apiClient.POST('/api/v1/projects', { body: data })),
    onSuccess: () => {
...
export function useUpdateProject(
  options?: UseMutationOptions<Project, Error, { projectId: string; data: Partial<Project> }>
) {
  const queryClient = useQueryClient()
...
export function useCreateItem(
  options?: UseMutationOptions<Item, Error, { projectId: string; data: Partial<Item> }>
) {
  const queryClient = useQueryClient()
  ...
  return useMutation({
    ...
    onSuccess: (_, { projectId }) => {
      queryClient.invalidateQueries({ queryKey: queryKeys.projectItems(projectId) })
    },
    ...options,
  })
}
...
export function useUpdateItem(
  options?: UseMutationOptions<Item, Error, { itemId: string; data: Partial<Item> }>
) {
  const queryClient = useQueryClient()
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: queryKeys.projectItems(data.projectId) })
    },
    ...options,
  })
}
...
export function useDeleteItem(
  options?: UseMutationOptions<void, Error, { itemId: string; projectId: string }>
) {
  const queryClient = useQueryClient()
...
// Links
export function useProjectLinks(projectId: string, options?: UseQueryOptions<Link[]>) {
  return useQuery({
...
export function useCreateLink(
  options?: UseMutationOptions<Link, Error, { projectId: string; data: Partial<Link> }>
) {
  const queryClient = useQueryClient()
...
export function useDeleteLink(
  options?: UseMutationOptions<void, Error, { linkId: string; projectId: string }>
) {
  const queryClient = useQueryClient()
...
export function useCreateMutation(
  ...
  return useMutation({
    mutationFn: (data) =>
      handleApiResponse(apiClient.POST('/api/v1/mutations', { body: data as any })),
    onSuccess: () => {
