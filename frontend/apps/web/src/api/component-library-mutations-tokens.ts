...
> => {
	...
	> = {
		mutationFn: async (
			input: CreateDesignTokenInput,
		): Promise<TracerTypes.DesignToken> =>
			QueryClient.handleApiResponse(
				QueryClient.api.post<TracerTypes.DesignToken>(
...
> => {
	...
	> = {
		mutationFn: async (input: {
			...
			QueryClient.handleApiResponse(
				QueryClient.api.put<TracerTypes.DesignToken>(
...
> => {
	...
	> = {
		...
		onSuccess: async (
			...
			await queryClient.invalidateQueries({
				queryKey: QueryKeys.componentLibraryQueryKeys.tokens(
					variables.libraryId,
				),
			});
		},
