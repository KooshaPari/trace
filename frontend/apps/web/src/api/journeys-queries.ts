...
options?: ReactQuery.UseQueryOptions<JourneyTypes.Journey[]>,
	...
	const baseOptions = {
		...
		queryFn: (): Promise<JourneyTypes.Journey[]> =>
			QueryClient.handleApiResponse(
				QueryClient.api.get<JourneyTypes.Journey[]>(
...
const useJourneySteps = (
	...
	const baseOptions = {
		enabled: Boolean(journeyId),
		queryFn: (): Promise<JourneyTypes.JourneyStep[]> =>
			QueryClient.handleApiResponse(
				QueryClient.api.get<JourneyTypes.JourneyStep[]>(
