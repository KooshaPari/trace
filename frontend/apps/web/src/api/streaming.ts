...
const shouldUpdateProgress = (count: number): boolean =>
	count % PROGRESS_UPDATE_INTERVAL === ZERO;
...
const streamItems = async function* streamItems(
	...
	yield* parseNDJSONWithProgress<Item>(
		response,
		options.onProgress,
		options.onMetadata,
	);
...
const streamGraph = async function* streamGraph(
	...
	yield* parseNDJSONWithProgress<GraphStreamItem>(
		response,
		options.onProgress,
		options.onMetadata,
	);
...
const createCancellableExportStream = (
	options: StreamExportOptions,
): CancellableNDJSONStream<unknown> =>
	createCancellableNDJSONStream(buildExportStreamUrl(options));
