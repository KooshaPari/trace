...
export function ImpactAnalysisView() {
  ...
  const analyzeImpact = (itemId: string) => {
    if (!links) return { direct: [], indirect: [] }
            ...
            {items?.map((item) => (
              <div
                ...
                className={`p-3 border rounded-lg cursor-pointer hover:border-blue-500 ${
                  selectedItemId === item.id ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : ''
                }`}
              >
        ...
        <Card className="p-6">
          ...
          {selectedItem && impact ? (
            <div className="space-y-6">
              ...
              <div>
                <h3 className="font-semibold mb-2">Direct Impact ({impact.direct.length})</h3>
              ...
              <div>
                <h3 className="font-semibold mb-2">Indirect Impact ({impact.indirect.length})</h3>
                {impact.indirect.length > 0 ? (
                  ...
                  <p className="text-gray-500 text-sm">No indirect dependencies</p>
                )}
              </div>

              <div className="pt-4 border-t">
                <div className="text-sm text-gray-600">
                  Total Impact: {impact.direct.length + impact.indirect.length} items
                </div>
              </div>
            </div>
          ) : (
