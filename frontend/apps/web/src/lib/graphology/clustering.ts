...
export class GraphClustering {
	...
	createClusterGraph(
		...
		const reductionRatio = this.calculateReductionRatio(
			graph.size,
			clusterEdges.length,
		);
	...
	expandCluster(
		...
		const clusterNumber = Number.parseInt(
			clusterId.replace("cluster-", ""),
		);
	...
	private calculateReductionRatio(
		originalEdgeCount: number,
		clusterEdgeCount: number,
	): number {
		return (
			(1 - clusterEdgeCount / originalEdgeCount) * PERCENT_SCALE
		);
	}
}
