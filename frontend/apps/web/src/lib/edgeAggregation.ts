...
export interface AggregatedEdge extends EdgeBase {
	...
	_aggregatedTypes: Record<LinkType, number>;
...
export interface EdgeCluster {
	...
	edges: EdgeBase[];
	totalCount: number;
	typeDistribution: Record<LinkType, number>;
...
export function aggregateParallelEdges(
	...
	for (const [key, groupEdges] of edgeGroups) {
		if (groupEdges.length < minEdgesForAggregation) {
			...
			// Find dominant type
			let dominantType = groupEdges[0].type;
			let maxCount = 0;
			...
			const aggregatedEdge: AggregatedEdge = {
				...groupEdges[0], // Use first edge as base
				id: `agg_${key}`,
				type: dominantType,
				_isAggregated: true,
...
export function sampleEdgesByImportance(
	...
	return [...priorityEdges.slice(0, priorityCount), ...sampledNormal];
}
