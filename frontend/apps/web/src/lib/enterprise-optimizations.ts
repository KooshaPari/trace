 ...
 */
export const useGridLayout = (items: any[], minItemWidth = 200, gap = 16) => {
  const containerRef = useRef<HTMLDivElement>(null)
  const [columns, setColumns] = useRef(1)
  ...
  return {
    ...
    itemWidth: `calc((100% - ${(columns.current - 1) * gap}px) / ${columns.current}`,
  }
}
