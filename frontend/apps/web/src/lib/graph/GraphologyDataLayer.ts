...
export class GraphologyDataLayer {
	...
	addNode(node: Node): void {
		...
		try {
			if (this.graph.hasNode(node["id"])) {
				this.graph.setNodeAttributes(node["id"], attributes);
			} else {
				this.graph.addNode(node["id"], attributes);
			}
		} catch (error) {
			logger.warn(
				`[GraphologyDataLayer] Failed to add node ${node["id"]}:`,
				error,
			);
		}
	}
	...
	addEdge(edge: Edge): void {
		...
		if (!this.graph.hasNode(edge.source) || !this.graph.hasNode(edge.target)) {
			logger.warn(
				`[GraphologyDataLayer] Skipping edge ${edge["id"]}: ` +
		...
		const attributes: GraphologyEdgeData = {
			...
			color:
				typeof edge.style?.stroke === "string" ? edge.style.stroke : "#94a3b8",
			...edge.data,
		};
	...
	removeEdge(edgeId: string): void {
		try {
			...
			logger.warn(
				`[GraphologyDataLayer] Failed to remove edge ${edgeId}:`,
				error,
			);
		}
	}
	...
	updateEdge(
		source: string,
		target: string,
		updates: Partial<GraphologyEdgeData>,
	): void {
	...
	toReactFlow(): { nodes: Node[]; edges: Edge[] } {
		...
		this.graph.forEachEdge((edgeId, attributes, source, target) => {
			...
			edges.push({
				...
				label: attrs["label"],
				data: attrs,
				style: attrs.color ? { stroke: attrs.color } : undefined,
			});
		});
	...
	async computeLayout(options: LayoutOptions = {}): Promise<void> {
		...
		try {
			...
			logger["error"](
				"[GraphologyDataLayer] Layout computation failed:",
				error,
			);
			throw error;
		}
	...
	async detectCommunities(): Promise<Map<string, number>> {
		...
		try {
			...
			logger["error"](
				"[GraphologyDataLayer] Community detection failed:",
				error,
			);
			throw error;
		}
	...
	getStats(): GraphStats {
		...
		try {
			...
			logger.warn(
				"[GraphologyDataLayer] Failed to calculate some metrics:",
				error,
			);
		}
