...
function generateCacheKey(items: Item[], links: Link[], options: ClusteringOptions): string {
  ...
  const linkIds = links.map(l => `${l.source_id}-${l.target_id}`).sort().join(',');
...
function calculateStats(
  ...
  const avgCommunitySize = sizes.length > 0
...
function assignColors(communityIds: Set<string>): Map<string, string> {
  ...
  const colorPalette = communityIds.size <= COMMUNITY_COLORS.length
    ? COMMUNITY_COLORS
