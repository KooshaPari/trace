...
function buildGraph(items: Item[], links: Link[]): Graph {
	...
	for (const link of links) {
		...
		if (!graph.hasNode(link.source_id) || !graph.hasNode(link.target_id)) {
			continue;
		}

		// Create normalized edge key (alphabetically sorted for undirected)
		const [nodeA, nodeB] = [link.source_id, link.target_id].sort();
		...
		try {
			graph.addEdge(link.source_id, link.target_id, {
				id: edgeKey,
				type: link.link_type,
			});
...
function assignColors(communityIds: Set<string>): Map<string, string> {
	...
	const colorPalette =
		communityIds.size <= COMMUNITY_COLORS.length
			? COMMUNITY_COLORS
			: EXTENDED_COMMUNITY_COLORS;
...
export async function detectCommunities(
	...
	if (options.minCommunitySize && options.minCommunitySize > 1) {
		...
		for (const communityId of communities.values()) {
			communitySizes.set(
				communityId,
				(communitySizes.get(communityId) || 0) + 1,
			);
		}
...
export function exportCommunitiesJSON(result: CommunityResult): string {
	const data = {
		communities: Array.from(result.communities.entries()).map(
			([nodeId, communityId]) => ({
				nodeId,
				communityId,
		...
		stats: {
			...result.stats,
			communitySizes: Array.from(result.stats.communitySizes.entries()).map(
				([id, size]) => ({
					communityId: id,
					size,
