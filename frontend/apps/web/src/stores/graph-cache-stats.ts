...
const calculateGroupingStats = (
	...
	const avgGroupingAge = calculateAverageAge(
		totalGroupingAge,
		state.groupings.size,
	);
	return createStatsBlock(avgGroupingAge, state.groupings.size);
};

const createStatsActions = (
	set: StoreSetter,
	get: StoreGetter,
): StatsActions => ({
	getStats: (): CacheStatistics => {
	...
	recordInvalidation: (pattern: string, description: string): void => {
		set((draft) => {
			...
			if (
				draft.invalidationPatterns.length >
				graphCacheConstants.invalidationPatternLimit
			) {
