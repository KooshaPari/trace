...
const removeGroupingsByGraphId = (
	groupings: Map<string, GroupingData>,
	graphId: string,
): void => {
...
const invalidateItem = (
	...
	set((draft) => {
		...
		if (cascading && deps) {
			const relatedItemIds = collectRelatedItemIds(
				itemId,
				deps,
				state.getItemDependencies,
			);
...
const createInvalidationActions = (
	set: StoreSetter,
	get: StoreGetter,
): InvalidationActions => ({
