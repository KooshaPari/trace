...
function layoutDagre(
  ...
  for (const [nodeLevel, nodeIds] of levelGroups) {
    ...
    const maxLevelWidth = Math.max(...Array.from(levelGroups.values()).map(ids => {
      const lvlNodes = ids.map(id => nodeMap.get(id)!);
      return lvlNodes.reduce((sum, n) => sum + n.width, 0) + (ids.length - 1) * nodeSep;
    }));
  ...
  return {
    positions,
    size: { width: maxWidth || 800, height: maxHeight || 600 },
  };
}
