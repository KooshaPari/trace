...
const createUploadError = function (
	message: string,
...
const resolveCompressOptions = function (
...
const calculateScaledDimensions = function (
	width: number,
	height: number,
	maxWidth: number,
	maxHeight: number,
...
const toCompressedDataUrl = function (
...
const getThumbnailDimensions = function (
	size: ThumbnailSize,
): ThumbnailDimensions {
	return THUMBNAIL_SIZES[size];
};

const drawThumbnail = function (
	image: HTMLImageElement,
 ...
 */
const captureComponentScreenshot = async function (
	element: HTMLElement,
	width = CAPTURE_WIDTH,
 ...
 */
const dataUrlToFile = function (dataUrl: string, filename: string): File {
 ...
 */
const getPresignedUploadUrl = async function (
	filename: string,
	contentType: string,
...
const mapUploadProgress = function (percent: number): number {
	return PROGRESS_UPLOAD_BASE + (percent * PROGRESS_UPLOAD_RANGE) / PERCENT_BASE;
};

const validateScreenshotInput = function (screenshotData: string): void {
	if (!screenshotData) {
		throw createUploadError("Screenshot data is empty", "INVALID_FILE");
	}
};

const validateFileSize = function (file: File): void {
 ...
 */
const uploadScreenshot = async function ({
	componentId,
	options = {},
	screenshotData,
	version = "v1",
 ...
 */
const deleteScreenshot = async function (key: string): Promise<void> {
	try {
 ...
 */
const cacheScreenshot = function (metadata: ScreenshotMetadata): void {
 ...
 */
const getScreenshotFromCache = function (
	id: string,
 ...
 */
const getComponentScreenshots = function (
	componentId: string,
): ScreenshotMetadata[] {
 ...
 */
const generateContentHash = function (data: string): string {
	let hash = ZERO;
...
const buildMetadata = function (
	...
	return {
		componentId,
		contentHash: screenshotData
			? generateContentHash(screenshotData)
			: undefined,
		createdAt: now,
 ...
 */
const createScreenshot = async function ({
	componentId,
	element,
	options = {},
	version = "1.0.0",
 ...
 */
const updateItemWithScreenshot = function (
	item: Item,
 ...
 */
const screenshotNeedsUpdate = function (
 ...
 */
const batchCaptureScreenshots = async function (
	elements: {
		componentId: string;
		element: HTMLElement;
		version: string;
		...
		try {
			const elementProgress = (percent: number) => {
				...
				const elementPercent =
					(percent / PERCENT_BASE) * (PERCENT_BASE / totalElements);
 ...
 */
const clearScreenshotCache = function (): void {
 ...
 */
const getScreenshotCacheStats = function (): {
	screenshots: number;
	thumbnails: number;
