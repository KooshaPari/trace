...
const ValidationTabs: FC<ValidationTabsProps> = ({ validation }) => (
  <Tabs defaultValue='summary' className='w-full'>
    <TabsList>
      ...
      <OptionalTabsTrigger
        show={Boolean(validation?.errors?.length > 0)}
        value='errors'
        className='text-red-600'
        label={`Errors (${validation?.errors.length ?? 0})`}
      />
      <OptionalTabsTrigger
        show={Boolean(validation?.warnings?.length > 0)}
        value='warnings'
    ...
    <OptionalTabsContent show={Boolean(validation?.errors?.length > 0)} value='errors'>
      <ValidationList items={validation?.errors ?? EMPTY_ERRORS} variant='errors' />
    </OptionalTabsContent>
    <OptionalTabsContent show={Boolean(validation?.warnings?.length > 0)} value='warnings'>
