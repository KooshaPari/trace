...
function EquivalenceExportComponent({
	...
	const filteredLinks = equivalenceLinks.filter(
		(link) =>
			link.confidence >= filters.minConfidence &&
			filters.selectedStatuses.has(link.status),
	);

	const filteredConcepts = canonicalConcepts.filter(
		(concept) =>
			filters.selectedDomains.size === 0 ||
			filters.selectedDomains.has(concept.domain),
	);
	...
	const createExportPackage = useCallback(() => {
		...
		const data = {
			...
			canonicalProjections: options.includeProjections
				? filteredProjections
				: [],
		};
		...
		<Dialog open={isOpen} onOpenChange={setIsOpen}>
			...
			<DialogContent className="max-w-2xl max-h-[80vh]">
				<DialogHeader>
					...
					<DialogDescription>
						Export equivalence mappings and canonical concepts with filtering
						options
					</DialogDescription>
				</DialogHeader>
					...
					<div className="space-y-6 pr-4">
						...
						<Card>
							<CardHeader className="pb-3">
								<Button
									variant="ghost"
									className="w-full justify-between px-0 h-auto py-0"
									onClick={() =>
										setFilters({
											...filters,
				...
				<div className="flex items-center justify-end gap-2 border-t pt-4">
					...
					<Button
						...
						className="gap-2"
						onClick={
							options.format === "json" ? handleExportJSON : handleExportCSV
						}
					>
						<Download className="h-4 w-4" />
