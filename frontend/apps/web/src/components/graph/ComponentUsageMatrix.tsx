...
export interface ComponentUsageMatrixProps {
	...
	onCategoryChange?:
		| ((category: ComponentCategory | "all") => void)
		| undefined;
	...
	usageData.forEach((usage) => {
		if (usage.componentId === component.id) {
			if (usage.usedInFilePath) {
				stats.pageUsage[usage.usedInFilePath] =
					(stats.pageUsage[usage.usedInFilePath] || 0) + 1;
			}
			if (usage.variantUsed) {
				stats.variantUsage[usage.variantUsed] =
					(stats.variantUsage[usage.variantUsed] || 0) + 1;
			}
		}
	});
...
const MatrixRow = memo(function MatrixRow({
	...
	return (
		<div>
			<div
				className={`flex items-center gap-3 px-4 py-3 border-b transition-colors cursor-pointer hover:bg-slate-50
					${isSelected ? "bg-blue-50" : ""}
					${isUnused && highlightUnused ? "bg-orange-50" : ""}
					${isDeprecated && highlightUnused ? "bg-red-50" : ""}
				`}
				onClick={onSelect}
			>
				{/* Expand button */}
				{(showVariants && component.variants?.length) ||
				(showProps && component.props?.length) ? (
					<button
						onClick={(e) => {
							e.stopPropagation();
				...
				<div className="flex-1 min-w-0">
					<div className="flex items-center gap-2">
						<CategoryIcon className="w-4 h-4 text-slate-600 flex-shrink-0" />
						...
						{isDeprecated && highlightUnused && (
							<Badge
								variant="outline"
								className="bg-red-50 text-red-700 border-red-200 text-xs flex-shrink-0"
							>
								Deprecated
							</Badge>
						)}
					...
					<p className="text-xs text-slate-500 truncate">{component.name}</p>
				</div>
				...
				<div className="flex-shrink-0">
					<Badge
						variant="outline"
						className={`text-xs ${STATUS_COLORS[component.status]}`}
					>
				...
				<div className="flex-shrink-0 text-right min-w-[60px]">
					<p className="font-semibold text-sm">{stats.totalUsage}</p>
			...
			{/* Expanded details */}
			{expanded && (
				<div className="bg-slate-50 px-4 py-3 border-b text-sm">
					{/* Props section */}
					{showProps && component.props && component.props.length > 0 && (
						<div className="mb-4">
							<p className="font-semibold text-xs uppercase text-slate-700 mb-2">
								Props
							</p>
...
const CategorySection = memo(function CategorySection({
	...
	const totalUsage = components.reduce(
		(sum, comp) => sum + (stats[comp.id]?.totalUsage || 0),
		0,
	);
	const unusedCount = components.filter(
		(comp) => (stats[comp.id]?.totalUsage || 0) === 0,
	).length;
	...
	return (
		<Collapsible
			...
			className="border-b"
		>
			<CollapsibleTrigger className="flex items-center gap-3 px-4 py-3 hover:bg-slate-50 w-full">
				...
				<div className="ml-auto flex gap-3 text-xs text-slate-500">
					...
					{unusedCount > 0 && (
						<span className="text-orange-600">{unusedCount} unused</span>
					)}
				</div>
			</CollapsibleTrigger>
...
export const ComponentUsageMatrix = memo(function ComponentUsageMatrix({
	...
	const stats = useMemo(() => {
		...
		const deprecated = filteredComponents.filter(
	...
	return (
		<Card className="w-full">
					...
					<div className="flex flex-col gap-3 mt-4">
						<div className="flex gap-3">
							...
							<Select
								...
								onValueChange={(value) =>
									onCategoryChange?.(
										value === "all" ? "all" : (value as ComponentCategory),
									)
								}
							>
							...
							<div className="flex items-center gap-2 text-xs">
								...
								{selectedCategory !== "all" && (
									<Badge variant="secondary">
										{CATEGORY_LABELS[selectedCategory as ComponentCategory]}
									</Badge>
								)}
								{searchQuery && (
			...
			{/* Stats footer */}
			{filteredComponents.length > 0 && (
				<div className="border-t px-4 py-3 bg-slate-50 text-xs text-slate-600 flex items-center justify-between">
					<div>
