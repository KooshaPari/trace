...
function filterByDimensions(
  ...
  const filteredItems = applyDimensionFilters(
    items as { dimensions?: Record<string, unknown> }[],
    filters,
  ) as Item[];
  const itemIds = new Set(filteredItems.map((i) => i.id));
  const filteredLinks = links.filter(
              ...
              {pivotTargets.slice(0, 5).map((target) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => onPivot(target.perspectiveId, target.item.id)}
                  >
        ...
        {displayMode === 'split' && (
          <div className='flex items-center gap-2'>
            <Select
              ...
              onValueChange={(v) =>
                setSplitPerspectives([v as GraphPerspective, splitPerspectives[1]])
              }
            >
            ...
            <Select
              ...
              onValueChange={(v) =>
                setSplitPerspectives([splitPerspectives[0], v as GraphPerspective])
              }
            >
              <SelectTrigger className='h-8 w-[140px]'>
                <SelectValue />
