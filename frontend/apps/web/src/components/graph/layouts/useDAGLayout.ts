...
function useDagLayout<NodeData extends Record<string, unknown>>(
  ...
  const applySyncLayout = useCallback(
    (inputNodes: Node<NodeData>[], inputEdges: Edge[]) =>
      resolveSyncLayout({
        centerX,
        centerY,
        inputEdges,
  ...
  const computeElkLayout = useCallback(
    async (
      ...
      computeElkLayoutInternal(inputNodes, inputEdges, resolvedElkOptions),
    [],
  );
...
export { LAYOUT_CONFIGS, useDagLayout };
