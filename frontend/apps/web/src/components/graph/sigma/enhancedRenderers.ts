...
import type { NodeDisplayData, EdgeDisplayData, PartialButFor } from 'sigma/types';
import { _NodeProgram } from 'sigma/rendering/webgl/programs/node';
import { _EdgeProgram } from 'sigma/rendering/webgl/programs/edge';
...
export function enhancedNodeRenderer(
  ...
  data: PartialButFor<NodeDisplayData, 'key' | 'x' | 'y' | 'size' | 'color'>,
  ...
  // Base node color
  const nodeColor = color || ENHANCED_TYPE_COLORS[type] || ENHANCED_TYPE_COLORS["default"];
  ...
  if (lod === 'far') {
    ...
    context.fillStyle = nodeColor + Math.round(opacity * 255).toString(16).padStart(2, '0');
    context.fill();
    return;
  }
  ...
  if (status) {
    ...
    context.fillStyle = statusColors[status as keyof typeof statusColors] || '#64748b';
    context.fill();
  ...
  if (zoomRatio >= 1.5 && label) {
    ...
    const maxLength = 25;
    const displayLabel = String(label).length > maxLength
      ? String(label).substring(0, maxLength) + '...'
      : String(label);
...
export function enhancedEdgeRenderer(
  context: CanvasRenderingContext2D,
  data: PartialButFor<EdgeDisplayData, 'key' | 'size' | 'color'>,
  sourceData: PartialButFor<NodeDisplayData, 'key' | 'x' | 'y' | 'size' | 'color'>,
  targetData: PartialButFor<NodeDisplayData, 'key' | 'x' | 'y' | 'size' | 'color'>,
  settings: Record<string, unknown>
): void {
