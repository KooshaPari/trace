...
export function HybridGraphViewExample() {
  ...
  const edges: Edge[] = Array.from({ length: Math.min(nodeCount * 2, 20000) }, (_, i) => ({
    id: `edge-${i}`,
    ...
    <div className="h-screen w-screen flex flex-col">
      ...
      <div className="p-4 border-b flex items-center gap-4 bg-card">
        <div className="text-sm font-medium">
          Test Hybrid Graph Switching:
        </div>
        <Button
          size="sm"
          variant={nodeCount === 5000 ? 'default' : 'outline'}
          onClick={() => setNodeCount(5000)}
        >
          5k nodes (ReactFlow)
        </Button>
        <Button
          ...
          onClick={() => setNodeCount(9500)}
        >
          9.5k nodes (Near threshold)
        </Button>
        <Button
          size="sm"
          variant={nodeCount === 15000 ? 'default' : 'outline'}
          onClick={() => setNodeCount(15000)}
        >
          15k nodes (WebGL)
        </Button>
        <Button
          size="sm"
          variant={nodeCount === 50000 ? 'default' : 'outline'}
          onClick={() => setNodeCount(50000)}
        >
          50k nodes (WebGL)
        </Button>
      </div>
      ...
      <div className="flex-1">
        <HybridGraphView
          nodes={nodes}
          edges={edges}
          onNodeClick={(nodeId) => logger.info('Node clicked:', nodeId)}
          onNodeExpand={(nodeId) => logger.info('Node expand:', nodeId)}
          onNodeNavigate={(nodeId) => logger.info('Node navigate:', nodeId)}
        />
      </div>
    </div>
  );
...
export function ForceOverrideExample() {
  ...
  const nodeCount = 12000; // Above threshold
  ...
  return (
    <div className="h-screen w-screen flex flex-col">
      <div className="p-4 border-b flex items-center gap-4 bg-card">
        ...
        <Button
          size="sm"
          variant={threshold === 10000 ? 'default' : 'outline'}
        ...
        <Button
          size="sm"
          variant={threshold === 20000 ? 'default' : 'outline'}
          onClick={() => setThreshold(20000)}
        >
      ...
      <div className="flex-1">
        <HybridGraphView
          nodes={nodes}
          edges={[]}
          config={{ nodeThreshold: threshold }}
        />
      </div>
    </div>
  );
