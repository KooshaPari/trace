...
export const SigmaGraphViewEnhanced = memo(function SigmaGraphViewEnhanced(
  ...
  const autoPerformanceMode = useMemo(() => {
    if (performanceMode !== 'balanced') return performanceMode;

    if (nodeCount > 50_000) return 'performance';
    if (nodeCount < 1_000) return 'quality';
    return 'balanced';
  ...
  return (
    <SigmaContainer
      ...
      settings={{
        ...
        defaultNodeType: autoPerformanceMode === 'performance' ? 'fast' : 'circle',
