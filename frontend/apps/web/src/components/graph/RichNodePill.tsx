...
function RichNodePillComponent({
	...
	const hasPreview =
		Boolean(data.uiPreview?.screenshotUrl) ||
		Boolean(data.uiPreview?.thumbnailUrl);
				...
				>
					...
					(showWidget && hasWidget) ? (
						<RichNodePillPreviewBlock
							previewUrl={
								hasPreview && (isExpanded || data.showPreview)
									? data.uiPreview?.thumbnailUrl ||
										data.uiPreview?.screenshotUrl
									: undefined
							}
						...
						<div className="flex items-center gap-2 mb-2">
							...
							<Tooltip>
								<TooltipTrigger asChild>
									...
									>
										{data.type.replaceAll(/_/g, " ")}
									</Badge>
								</TooltipTrigger>
								<TooltipContent>
									<p>{data.type.replaceAll(/_/g, " ")}</p>
								</TooltipContent>
							</Tooltip>
