					...
					<div className="flex flex-wrap gap-1.5 items-start">
						{result.equivalences.map((equiv) => (
							<div
								...
								className="flex items-center gap-1 rounded-full bg-muted/50 px-2 py-1 text-[10px]"
							>
								<LinkIcon className="w-3 h-3 text-primary" />
								<span className="text-muted-foreground">
									{equiv.equivalentPerspective}
								</span>
...
const PerspectiveGroup = memo(function PerspectiveGroupComponent({
	...
	return (
		<div className="border-t first:border-t-0">
			{/* Group header */}
			<div
				className="px-4 py-2 bg-muted/30 sticky top-0 z-10 flex items-center justify-between"
				style={{ borderLeftColor: perspectiveColor, borderLeftWidth: "4px" }}
			>
				<div className="flex items-center gap-2">
					<Network className="w-4 h-4" style={{ color: perspectiveColor }} />
			...
			{displayResults.map((result, idx) => {
				...
				return (
					<ResultItem
						...
						onMouseEnter={() => onSelectResult(globalIndex)}
					/>
				);
			})}
...
function CrossPerspectiveSearchComponent({
				...
				case "Enter": {
					...
					for (const group of results) {
						...
						if (selectedResultIndex < currentIndex + groupSize) {
							...
							onSelect(group.results[resultIdx].item.id);
							break;
						}
		...
		return (
			<div className={cn("relative", className)}>
				...
				{query && results.length > 0 && (
					<div className="absolute top-full left-0 right-0 mt-2 bg-card border rounded-lg shadow-lg z-50 max-h-72 overflow-y-auto">
						{results.map((group) => {
							...
							return (
								<PerspectiveGroup
									...
									maxResults={Math.ceil(
										maxResultsPerPerspective / results.length,
									)}
	...
	return (
		<Card className={cn("overflow-hidden", className)}>
				...
				<div className="p-8 text-center text-muted-foreground space-y-4">
					...
					<div>
						...
						<p className="text-xs mt-1">
							Search across all {results.length || items.length} items in
			...
			{query && results.length > 0 && (
				<div className="border-t bg-muted/30 px-4 py-2 text-[10px] text-muted-foreground space-y-1">
					<div className="flex items-center justify-between">
						<span>
							<kbd className="rounded border bg-background px-1 text-[9px]">
								↑↓
							</kbd>
