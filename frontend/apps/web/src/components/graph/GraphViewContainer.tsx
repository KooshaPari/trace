...
interface GraphViewContainerProps {
	...
	canLoadMore?: boolean | undefined; // ✅ NEW: For progressive loading
	visibleEdges?: number | undefined; // ✅ NEW: Current visible edge count
	totalEdges?: number | undefined; // ✅ NEW: Total edge count
	onLoadMore?: (() => void) | undefined; // ✅ NEW: Load more handler
	children: (props: {
			...
			<div className="flex-1 flex flex-col overflow-hidden">
				...
				<div className="border-b p-3 flex items-center justify-between gap-4 bg-background">
					<div className="flex items-center gap-3">
						...
						<Select
							value={viewMode}
							onValueChange={(v) => handleViewChange(v as GraphViewMode)}
						>
							<SelectTrigger className="w-[220px] h-9">
								<div className="flex items-center gap-2">
									{currentConfig && (
										<currentConfig.icon className="h-4 w-4 text-primary" />
									)}
									<SelectValue />
								</div>
							</SelectTrigger>
				...
				<div className="flex-1 overflow-hidden relative">
					...
					{/* ✅ NEW: Load More button for progressive loading */}
					{canLoadMore && onLoadMore && (
						<div className="absolute bottom-4 left-4 z-10">
							...
							>
								...
								Load More Links ({visibleEdges} of {totalEdges})
							</Button>
						</div>
					)}
				</div>
