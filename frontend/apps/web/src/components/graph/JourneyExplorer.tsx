...
interface JourneyExplorerProps {
	...
	onJourneyUpdate?: (
		journeyId: string,
		updates: Partial<DerivedJourney>,
	) => void;
...
function calculateJourneyMetrics(
	...
	const coverage =
		allNodeIds.length > 0 ? (nodeCount / allNodeIds.length) * 100 : 0;
		...
		<div className="flex flex-col gap-2">
			<div className="flex items-center gap-2 overflow-x-auto pb-2">
				{visibleNodes.map((nodeId, idx) => (
					<div key={nodeId} className="flex items-center gap-2 shrink-0">
						<div className="flex items-center px-2 py-1 bg-muted rounded-full text-xs font-medium whitespace-nowrap">
				...
				<div className="flex items-center gap-3 mt-3 text-xs text-muted-foreground">
					<TooltipProvider>
						<Tooltip>
							...
							<TooltipContent>
								Journey contains {metrics.nodeCount} items
							</TooltipContent>
						</Tooltip>
					</TooltipProvider>
					<span>•</span>
					<TooltipProvider>
						<Tooltip>
							...
							<TooltipContent>
								{metrics.linkCount} connections between nodes
							</TooltipContent>
						</Tooltip>
					</TooltipProvider>
				</div>
					...
					<Button
						...
						onClick={(e) => {
							...
							onUpdate?.({
								color:
									JOURNEY_COLORS[
										Math.floor(Math.random() * JOURNEY_COLORS.length)
									] ?? "#9333ea",
							});
						}}
					>
			...
			<DialogContent className="max-w-md">
				...
				<div className="space-y-4">
					...
					<div className="space-y-2">
						...
						<Select
							value={type}
							onValueChange={(v) => setType(v as DerivedJourney["type"])}
						>
							<SelectTrigger>
								<SelectValue />
							</SelectTrigger>
					...
					<div className="bg-muted/50 rounded-lg p-3 text-xs text-muted-foreground">
						<p>
							After creating the journey, you can edit it to add nodes and links
							in the journey editor.
						</p>
					</div>
				</div>
			...
			<div className="space-y-4">
				...
				<div className="flex items-center justify-between gap-2">
					<div className="flex items-center gap-2">
						...
						<h3 className="font-semibold text-base">Journey Explorer</h3>
							...
							<div className="space-y-2">
								<div className="flex items-center justify-between text-sm">
									...
									<span className="font-semibold">
										{coverageMetrics.nodesInJourneys} /{" "}
										{coverageMetrics.totalNodes} items
									</span>
								</div>
								<Progress
									value={Math.min(coverageMetrics.coverage, 100)}
									className="h-2"
								/>
				...
				{filteredJourneys.length > 0 ? (
					<ScrollArea
						className={cn("rounded-lg border", compact ? "h-96" : "h-[500px]")}
					>
