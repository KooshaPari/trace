...
function getEquivalences(
	...
	for (const link of links) {
		if (
			...
			(link.type === "same_as" ||
				link.type === "represents" ||
				link.type === "manifests_as")
		) {
			const equivalentId =
				link.sourceId === item.id ? link.targetId : link.sourceId;
			const equivalentItem = allItems.get(equivalentId);
...
export function performCrossPerspectiveSearch(
	...
	const groupedByPerspective = new Map<
		string,
		CrossPerspectiveSearchResult[]
	>();
...
export function useCrossPerspectiveSearch() {
	...
	const exportResults = useCallback(
		(results: GroupedSearchResults[], filename?: string) => {
	...
	const exportResultsCSV = useCallback(
		(results: GroupedSearchResults[], filename?: string) => {
			const rows: string[] = [
				"ID,Title,Perspective,Type,Status,Match Type,Score",
			];
