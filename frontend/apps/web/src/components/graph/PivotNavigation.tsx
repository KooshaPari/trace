								...
								<Tooltip key={config.id} delayDuration={200}>
									<TooltipTrigger asChild>
										<Button
											...
											disabled={!hasItems}
											onClick={() => onPivot(config.id, target.item.id)}
										>
							...
							return (
								<Tooltip key={config.id} delayDuration={200}>
									...
									<TooltipContent side="bottom">
										No equivalents in {config.label}
									</TooltipContent>
								</Tooltip>
							);
						}
									...
									<div key={config.id} className="space-y-1">
										...
										{items.map((target) => (
											<Button
												...
												variant="ghost"
												size="sm"
												className="w-full justify-between gap-2 h-auto py-2 pl-6"
												onClick={() => onPivot(config.id, target.item.id)}
											>
												<div className="flex-1 text-left min-w-0">
...
function getPerspectiveForItem(item: Item): GraphPerspective {
	if (item.perspective) {
		return item.perspective as GraphPerspective;
	}
