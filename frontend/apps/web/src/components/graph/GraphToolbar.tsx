...
export function GraphToolbar({
	...
	return (
		<TooltipProvider>
			<div
				className={`flex flex-wrap items-center justify-between gap-2 sm:gap-3 p-2 sm:p-3 bg-card border rounded-lg ${className || ""}`}
				role="toolbar"
				aria-label="Graph view controls"
			>
				...
				>
					...
					<LayoutSelector
						...
						onChange={onLayoutChange}
						variant="select"
						className="w-full min-w-0 max-w-[160px] sm:max-w-[200px]"
					/>
					...
					{totalNodes > 0 && (
						<div className="hidden md:flex items-center gap-1.5 px-2 py-1 rounded-md bg-muted/50 text-xs">
