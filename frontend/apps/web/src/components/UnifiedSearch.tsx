...
import { CrossPerspectiveSearch, GraphSearch } from "./graph";
...
export function UnifiedSearch({
	...
	const [searchMode, setSearchMode] = useState<"basic" | "cross-perspective">(
		"basic",
	);
	...
	return (
		<div className={className}>
			<Tabs
				value={searchMode}
				onValueChange={(mode) => setSearchMode(mode as typeof searchMode)}
			>
				...
				<TabsContent value="basic" className="space-y-4">
					<div className="text-sm text-muted-foreground">
						Search items by name, description, or type within current
						perspective
					</div>
					<GraphSearch
						items={items}
				...
				<TabsContent value="cross-perspective" className="space-y-4">
					<div className="text-sm text-muted-foreground">
						Search across perspectives to find equivalent concepts and related
						items
					</div>
