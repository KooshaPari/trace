...
export function StateMachineViewer({
	...
	const positionMap = Object.fromEntries(
		positions.map((p) => [p.state, { x: p.x, y: p.y }]),
	);
	...
	if (states.length === 0) {
		return (
			<Card className={`p-6 text-center text-muted-foreground ${className}`}>
				...
				<p className="text-xs">
					Define states and transitions to visualize the state machine.
				</p>
			</Card>
		);
	}
	...
	const transitionsBySource: Record<string, ContractTransition[]> =
		Object.fromEntries(
			states.map((state) => [
				state,
				transitions.filter((t) => t.fromState === state),
			]),
		);
		...
		<div className={`space-y-4 ${className}`}>
							...
							return (
								<g key={`transition-${transition.id}`}>
									...
									<text
										x={fromPos.x + 60}
										y={fromPos.y - 60}
										className="text-xs fill-muted-foreground font-mono pointer-events-none"
										textAnchor="middle"
									>
										{transition.trigger}
									</text>
						...
						return (
							<g key={`transition-${transition.id}`}>
								<path
									...
									strokeWidth="2"
									markerEnd="url(#arrowhead)"
									className="opacity-60 hover:opacity-100 hover:stroke-primary transition-opacity"
			...
			{currentState && (
				<div className="grid gap-2">
					<h4 className="font-semibold text-sm">Current State Details</h4>
					<Card className="p-3 space-y-2">
						...
						{currentState &&
							(transitionsBySource[currentState]?.length ?? 0) > 0 && (
								<div>
									<p className="text-xs text-muted-foreground mb-2">
										Available Transitions
									</p>
									<div className="space-y-1">
										{(transitionsBySource[currentState] ?? []).map(
											(transition) => (
												<motion.div
													key={transition.id}
												...
												>
													...
													{isExecutable && (
														<motion.button
															...
															className="p-1.5 rounded hover:bg-primary/10 transition-colors disabled:opacity-50"
															whileHover={{ scale: 1.1 }}
															whileTap={{ scale: 0.95 }}
														>
			...
			{initialState && (
				<div className="text-xs text-muted-foreground flex items-center gap-2">
					...
					<span>
						Initial State:{" "}
						<span className="font-mono font-semibold">{initialState}</span>
					</span>
				</div>
			)}
		</div>
