...
export const VerificationSummaryCards: FC<VerificationSummaryCardsProps> = ({
	contracts,
}) => {
	const { passRate, total, verified, violated } = useMemo(() => {
		const totalVal = contracts.length;
		const verifiedVal = contracts.filter((c) => c.status === "verified").length;
		const violatedVal = contracts.filter((c) => c.status === "violated").length;
		const passRateVal =
			totalVal > 0 ? ((verifiedVal / totalVal) * 100).toFixed(1) : "0";
		return {
