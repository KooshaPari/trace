	...
	return (
		<Card className="w-full max-w-6xl mx-auto">
			<form onSubmit={handleSubmit}>
				<CardHeader>
					<CardTitle>
						{initialData?.id ? "Edit Contract" : "Create New Contract"}
					</CardTitle>
					<CardDescription>
						Define a design-by-contract specification with preconditions,
				...
				<CardContent className="space-y-6">
					...
					<Tabs defaultValue="preconditions" className="w-full">
						...
						{["preconditions", "postconditions", "invariants"].map(
							(conditionType) => (
								<TabsContent
									key={conditionType}
									value={conditionType}
								...
								>
									...
									<div className="space-y-2">
										...
										<AnimatePresence mode="popLayout">
											{(
												formData[
													conditionType as keyof Contract
												] as ContractCondition[]
											)?.map((condition) => (
												<motion.div
													key={condition.id}
					...
					<Tabs defaultValue="states" className="w-full">
						<TabsList className="grid w-full grid-cols-2">
							<TabsTrigger value="states">
								States ({formData.states?.length || 0})
							</TabsTrigger>
					...
					<div className="space-y-2">
						...
						<Select
							...
							onValueChange={(value) =>
								handleChange(
									"specLanguage",
									value as "typescript" | "python" | "gherkin",
								)
							}
						>
						...
						<Textarea
							...
							onChange={(e) => handleChange("executableSpec", e.target.value)}
						/>
						<p className="text-xs text-muted-foreground">
							Code that can be executed to verify contract compliance during
							testing.
						</p>
					</div>
				</CardContent>
