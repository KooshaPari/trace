...
export function ItemSpecsOverview({
  ...
  const reqSpecs = ((reqData as any)?.specs as RequirementSpec[]) ?? [];
  const testSpecs = ((testData as any)?.specs as TestSpec[]) ?? [];
  const epicSpecs = ((epicData as any)?.specs as EpicSpec[]) ?? [];
  const storySpecs = ((storyData as any)?.specs as UserStorySpec[]) ?? [];
  const taskSpecs = ((taskData as any)?.specs as TaskSpec[]) ?? [];
  const defectSpecs = ((defectData as any)?.specs as DefectSpec[]) ?? [];
  const unverifiedReqs = ((unverifiedData as any)?.specs as RequirementSpec[]) ?? [];
  const highRiskReqs = ((highRiskData as any)?.specs as RequirementSpec[]) ?? [];
