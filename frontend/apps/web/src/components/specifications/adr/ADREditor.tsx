...
export function ADREditor({
	...
	const removeDriver = (index: number) => {
		setFormData((prev) => ({
			...prev,
			decisionDrivers: (prev.decisionDrivers || []).filter(
				(_, i) => i !== index,
			),
		}));
	};
	...
	const removeRequirement = (req: string) => {
		setFormData((prev) => ({
			...prev,
			relatedRequirements: (prev.relatedRequirements || []).filter(
				(r) => r !== req,
			),
		}));
	};
	...
	return (
		<Card className="w-full max-w-4xl mx-auto">
			<form onSubmit={handleSubmit}>
				<CardHeader>
					<CardTitle>
						{initialData?.id ? "Edit ADR" : "Create New ADR"}
					</CardTitle>
					<CardDescription>
						...
						<div className="space-y-2 bg-muted/20 p-4 rounded-lg border border-border">
							...
							<div className="space-y-2">
								{(formData.decisionDrivers || []).map((driver, idx) => (
									<div
										key={idx}
										className="flex items-center justify-between gap-2 bg-background p-2 rounded border border-border/50"
									>
