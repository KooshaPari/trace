		...
		for (const adr of adrs) {
			if (adr.supersedes) {
				edges.push({
					source: adr.id,
					target:
						adrs.find((a) => a.adrNumber === adr.supersedes)?.id || adr.id,
					type: "supersedes",
				});
			}
	...
	return (
		<Card className={className}>
			...
			<CardContent className="space-y-4">
				{/* Graph Canvas */}
				<div className="overflow-auto bg-muted/20 rounded-lg border border-border relative">
					<svg
