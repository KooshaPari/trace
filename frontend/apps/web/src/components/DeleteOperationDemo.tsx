	...
	return (
		<div className="p-6 space-y-4">
			<div className="flex gap-2">
				<Button
					variant="outline"
					onClick={() => handleDeleteClick("item-123", "Project Roadmap")}
				>
	...
	return (
		<div className="p-6 space-y-4">
			...
			<BulkConfirmationDialog
				...
				isLoading={isDeleting}
				onConfirm={() => executeDelete(performBulkDelete)}
			/>
		</div>
	);
}
...
export function EmptyStateExample() {
				...
				actions={[
					{
						...
						onClick: () => logger.info("Create clicked"),
					},
					{
	...
	if (items.length === 0 && searchQuery !== "") {
		...
		} = require("@/components/ui/empty-state");
		return (
			<NoSearchResultsEmptyState
				query={searchQuery}
				actions={[
	...
	if (items.length === 0 && filters.length > 0) {
		const { FilteredEmptyState } = require("@/components/ui/empty-state");
		return (
			<FilteredEmptyState
				filters={filters}
				actions={[
...
export function InlineDeleteExample() {
	...
	return (
		<Button
			...
			size="sm"
			disabled={isDeleting}
			onClick={() => handleQuickDelete("item-456")}
		>
			<Trash2 className="h-4 w-4" />
		</Button>
	);
...
export function CompleteListViewExample() {
	...
	if (items.length === 0) {
		return (
			<EmptyState
				icon={require("lucide-react").InboxIcon}
				title="No items yet"
				description="Create your first item to get started"
				actions={[
					{
						label: "Create Item",
						onClick: () => setItems([{ id: "1", name: "Sample Item" }]),
					},
				]}
			/>
		);
	...
	return (
		<div className="p-6 space-y-4">
			...
			{selectedItems.length > 0 && (
				<div className="flex items-center gap-2 p-3 bg-blue-50 dark:bg-blue-950/20 rounded-lg border border-blue-200 dark:border-blue-900/50">
