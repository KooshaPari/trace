  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 8, padding: 16 }}>
        <button onClick={() => handleLoadMore('north')} disabled={!hasMore.north || isLoading}>
          Load North ⬆️
        </button>
        <button onClick={() => handleLoadMore('south')} disabled={!hasMore.south || isLoading}>
          Load South ⬇️
        </button>
        <button onClick={() => handleLoadMore('east')} disabled={!hasMore.east || isLoading}>
          Load East ➡️
        </button>
        <button onClick={() => handleLoadMore('west')} disabled={!hasMore.west || isLoading}>
          Load West ⬅️
        </button>
      </div>
      <div style={{ height: '600px', width: '100%' }}>
