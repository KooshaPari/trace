...
export function useExecutionArtifacts(
  ...
  return useQuery({
    enabled: Boolean(projectId) && Boolean(executionId),
    queryFn: () => executionsApi.listArtifacts(projectId, executionId, artifactType),
...
export function useCreateExecution(projectId: string) {
  ...
  return useMutation({
    mutationFn: (data: ExecutionCreate) => executionsApi.create(projectId, data),
    onSuccess: async () => {
...
export function useStartExecution(projectId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (executionId: string) => executionsApi.start(projectId, executionId),
...
export function useCompleteExecution(projectId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ executionId, data }: { executionId: string; data: ExecutionComplete }) =>
