	...
	const transformedItems = itemsArray.map((item: Record<string, unknown>) => {
		...
		if (item["type"] === "bug" || item["type"] === "defect") {
			return {
				...
				stepsToReproduce:
					item["steps_to_reproduce"] || item["stepsToReproduce"],
...
async function createItemWithSpec(
	...
	if (
		...
		return {
			...
			lastExecutionResult:
				responseData["last_execution_result"] ||
				responseData["lastExecutionResult"],
		} as TypedItem;
	}
	...
	if (responseData["type"] === "epic") {
		return {
			...baseItem,
			acceptanceCriteria:
				responseData["acceptance_criteria"] ||
				responseData["acceptanceCriteria"],
			businessValue:
				responseData["business_value"] || responseData["businessValue"],
			targetRelease:
				responseData["target_release"] || responseData["targetRelease"],
		} as TypedItem;
	}

	if (
		responseData["type"] === "user_story" ||
