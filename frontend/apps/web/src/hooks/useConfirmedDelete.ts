	...
	const executeDelete = useCallback(
		async (deleteFunction: () => Promise<void> | void) => {
			...
			try {
				...
				if (showErrorToast) {
					toast.error(
						pendingDelete.errorMessage ||
							`Failed to delete ${pendingDelete.name}: ${errorMsg}`,
					);
				}
...
export function useConfirmedBulkDelete(
	options: UseConfirmedDeleteOptions = {},
) {
	const {
		showSuccessToast = true,
	...
	const [dialogOpen, setDialogOpen] = useState(false);
	const [pendingDelete, setPendingDelete] = useState<BulkDeleteConfig | null>(
		null,
	);
	const [isDeleting, setIsDeleting] = useState(false);
	...
	const executeDelete = useCallback(
		async (deleteFunction: () => Promise<void> | void) => {
			...
			try {
				...
				if (showErrorToast) {
					toast.error(
						pendingDelete.errorMessage || `Failed to delete items: ${errorMsg}`,
					);
				}
