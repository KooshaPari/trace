...
const useGitHubRepos = (
  ...
  search?: string,
  page?: number,
): UseQueryResult<GitHubReposResponse, Error> =>
  useQuery({
    enabled: Boolean(credentialId),
    queryFn: () => fetchGitHubRepos(credentialId, search, page),
...
const useGitHubIssues = (
  ...
  state?: string,
  page?: number,
): UseQueryResult<GitHubIssuesResponse, Error> =>
  useQuery({
    enabled: Boolean(credentialId) && Boolean(owner) && Boolean(repo),
    queryFn: () => fetchGitHubIssues(credentialId, owner, repo, state, page),
...
const useGitHubProjects = (
  ...
  owner: string,
  isOrg?: boolean,
): UseQueryResult<GitHubProjectsResponse, Error> =>
  useQuery({
    enabled: Boolean(credentialId) && Boolean(owner),
    queryFn: () => fetchGitHubProjects(credentialId, owner, isOrg),
    queryKey: ['integrations', 'github', 'projects', credentialId, owner, isOrg],
  });

const useLinearTeams = (credentialId: string): UseQueryResult<LinearTeamsResponse, Error> =>
  useQuery({
    enabled: Boolean(credentialId),
    queryFn: () => fetchLinearTeams(credentialId),
...
const useLinearIssues = (
  ...
  teamId: string,
  first?: number,
): UseQueryResult<LinearIssuesResponse, Error> =>
  useQuery({
    enabled: Boolean(credentialId) && Boolean(teamId),
    queryFn: () => fetchLinearIssues(credentialId, teamId, first),
...
const useLinearProjects = (
  credentialId: string,
  first?: number,
): UseQueryResult<LinearProjectsResponse, Error> =>
  useQuery({
    enabled: Boolean(credentialId),
    queryFn: () => fetchLinearProjects(credentialId, first),
