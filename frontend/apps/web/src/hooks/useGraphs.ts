 ...
 */
type GraphNode = Record<string, string | number | boolean | object | null | undefined>;

/**
 * Graph link data structure
 */
type GraphLink = Record<string, string | number | boolean | object | null | undefined>;
  ...
  return {
    graph: data['graph']
      ? {
          ...
          id: data['graph'].id,
          metadata:
            data['graph'].metadata || data['graph'].graph_metadata || data['graph'].graphMetadata,
...
const useGraphs = (projectId?: string): ReturnType<typeof useQuery> =>
  useQuery({
