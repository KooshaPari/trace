 ...
 */
export function useNodeExpansion(items: Array<{ id: string; parentId?: string }>) {
  ...
  // State tracking
  const expansionStateRef = useRef<Map<string, NodeExpansionState>>(
    new Map()
  );
    ...
    (nodeId: string): string | null => {
      ...
      if (parent) {
        navigationHistoryRef.current.previous.push(
          navigationHistoryRef.current.current ?? nodeId
        );
...
export function useExpansionStateSelector(
  ...
  return useMemo(
    () => expansionState.get(nodeId) ?? "collapsed",
    [expansionState, nodeId]
  );
}
