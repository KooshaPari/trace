...
describe('Security Headers Tests', () => {
  ...
  describe('Cookie Security Attributes', () => {
    ...
    it('should set Path and Domain appropriately', () => {
      const cookie = 'sessionId=abc123; Secure; HttpOnly; SameSite=Strict; Path=/; Domain=.tracertm.com';
  ...
  describe('Request Validation Headers', () => {
    it('should require Content-Type for API requests', () => {
      ...
      expect(validateContentType(validHeaders)).toBe(true);
      expect(validateContentType(invalidHeaders)).toBe(false);
    });
    ...
    it('should require Authorization header for protected endpoints', () => {
      ...
      expect(requiresAuth(validHeaders)).toBe(true);
      expect(requiresAuth(invalidHeaders)).toBe(false);
    });
  });
  ...
  describe('WebSocket Security Headers', () => {
    it('should validate WebSocket origin', () => {
      ...
      expect(validateWebSocketOrigin('https://evil.com')).toBe(false);
    });
  ...
  describe('HTTP Method Security', () => {
    ...
    it('should disable TRACE method', () => {
      ...
      expect(isMethodDisabled('GET')).toBe(false);
    });
  });
});
