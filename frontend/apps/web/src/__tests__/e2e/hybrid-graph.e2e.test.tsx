...
describe('Hybrid Graph E2E Tests', () => {
  describe('Threshold Switching', () => {
    ...
    it('should use WebGL for ≥10k nodes', async () => {
      const { nodes, edges } = generateSyntheticGraph(15000, 22500, {
        distribution: 'clustered',
        seed: 12345,
      });
    ...
    it('should show threshold warning at 8k-9.9k nodes', async () => {
      const { nodes, edges } = generateSyntheticGraph(9000, 13500, {
        distribution: 'random',
        seed: 54321,
      });
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText(/Approaching 10k node threshold/i)
        ).toBeInTheDocument();
      });
    });
    ...
    it('should not show warning for <8k nodes', async () => {
      ...
      expect(
        screen.queryByText(/Approaching 10k node threshold/i)
    ...
    it('should switch modes when node count changes', async () => {
      ...
      const largeGraph = generateSyntheticGraph(15000, 22500);

      rerender(
        <HybridGraphView nodes={largeGraph.nodes} edges={largeGraph.edges} />
      );
  ...
  describe('Performance - WebGL Mode', () => {
    ...
    it('should handle 50k nodes (stress test)', async () => {
      ...
      expect(renderTime).toBeLessThan(30000); // 30 seconds for stress test
    });
  });
  ...
  describe('Error Handling', () => {
    ...
    it('should handle rapid prop changes', async () => {
      ...
      const { rerender } = render(
        <HybridGraphView nodes={graph1.nodes} edges={graph1.edges} />
      );
