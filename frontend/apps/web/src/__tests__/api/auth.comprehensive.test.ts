...
describe("Authentication API", () => {
	...
	describe("login", () => {
		...
		it("should throw AuthError on CSRF token missing", async () => {
			...
			vi.mocked(csrfLib.fetchCSRFToken).mockRejectedValueOnce(
	...
	describe("updateProfile", () => {
		...
		it("should handle validation errors", async () => {
			...
			await expect(authApi.updateProfile({ name: "" })).rejects.toThrow(
				AuthError,
			);
		});
	});
	...
	describe("CSRF Integration", () => {
		it("should fetch CSRF token if not available", async () => {
			...
			vi.mocked(csrfLib.fetchCSRFToken).mockResolvedValueOnce(
				"fetched-csrf-token",
			);
			vi.mocked(authClient.handleApiResponse).mockResolvedValueOnce(
				mockAuthResponse,
