	...
	describe("agentsApi", () => {
		describe("list", () => {
			it("should list agents", async () => {
				const mockAgents = [{ id: "agent-1", name: "Agent 1", type: "worker" }];
				vi.mocked(apiClient.GET).mockResolvedValue({
					data: mockAgents,
					error: undefined,
					response: new Response(),
		...
		describe("get", () => {
			it("should get an agent by id", async () => {
				const mockAgent = { id: "agent-1", name: "Agent 1", type: "worker" };
				vi.mocked(apiClient.GET).mockResolvedValue({
					data: mockAgent,
					error: undefined,
					response: new Response(),
		...
		describe("create", () => {
			it("should create an agent", async () => {
				...
				const created = { id: "agent-1", ...newAgent };
		...
		describe("update", () => {
			it("should update an agent", async () => {
				...
				const result = await agentsApi.update("agent-1", updates);
				expect(result).toEqual(updated);
			});
		});
		...
		describe("register", () => {
			it("should register an agent", async () => {
				const agentData = { name: "New Agent", type: "worker" };
				const registered = { id: "agent-1", ...agentData };
				...
				const result = await agentsApi.register(agentData);
				expect(result).toEqual(registered);
				expect(apiClient.POST).toHaveBeenCalledWith("/api/v1/agents/register", {
					body: agentData,
				});
			});
		});
		...
		describe("heartbeat", () => {
			it("should send heartbeat", async () => {
				...
				await expect(agentsApi.heartbeat("agent-1")).resolves.toBeUndefined();
		...
		describe("getNextTask", () => {
			it("should get next task for agent", async () => {
				const mockTask = { id: "task-1", agent_id: "agent-1", type: "process" };
				vi.mocked(apiClient.GET).mockResolvedValue({
					data: mockTask,
					error: undefined,
					response: new Response(),
				...
				const result = await agentsApi.getNextTask("agent-1");
				expect(result).toEqual(mockTask);
				expect(apiClient.GET).toHaveBeenCalledWith("/api/v1/agents/{id}/task", {
					params: { path: { id: "agent-1" } },
				});
			});

			it("should return null when no task available", async () => {
				vi.mocked(apiClient.GET).mockResolvedValue({
					data: null,
					error: undefined,
					response: new Response(),
		...
		describe("submitTaskResult", () => {
			it("should submit task result", async () => {
				const taskResult = { task_id: "task-1", result: "success", data: {} };
				vi.mocked(apiClient.POST).mockResolvedValue({
					data: undefined,
					error: undefined,
					response: new Response(),
				...
				await expect(
					agentsApi.submitTaskResult(taskResult),
				).resolves.toBeUndefined();
				expect(apiClient.POST).toHaveBeenCalledWith(
					"/api/v1/agents/task/result",
					{
						body: taskResult,
					},
				);
		...
		describe("submitTaskError", () => {
			it("should submit task error", async () => {
				const taskError = { task_id: "task-1", error: "Failed" };
				vi.mocked(apiClient.POST).mockResolvedValue({
					data: undefined,
					error: undefined,
					response: new Response(),
				...
				await expect(
					agentsApi.submitTaskError(taskError),
				).resolves.toBeUndefined();
				expect(apiClient.POST).toHaveBeenCalledWith(
					"/api/v1/agents/task/error",
					{
						body: taskError,
					},
				);
		...
		describe("assignTask", () => {
			it("should assign task to agent", async () => {
				const task = { type: "process", data: {} };
				const assigned = { id: "task-1", agent_id: "agent-1", ...task };
				...
				const result = await agentsApi.assignTask("agent-1", task);
				expect(result).toEqual(assigned);
				expect(apiClient.POST).toHaveBeenCalledWith(
					"/api/v1/agents/task/assign",
					{
						body: { agent_id: "agent-1", ...task },
					},
				);
			});
		});
		...
		describe("listRegistered", () => {
			it("should list registered agents", async () => {
				...
				const result = await agentsApi.listRegistered();
				expect(result).toEqual(mockAgents);
		...
		describe("getStatus", () => {
			it("should get agent status", async () => {
				...
				const result = await agentsApi.getStatus("agent-1");
				expect(result).toEqual(mockAgent);
				expect(apiClient.GET).toHaveBeenCalledWith(
		...
		describe("runTask (alias)", () => {
			it("should call assignTask", async () => {
				const task = { type: "process" };
				const assigned = { id: "task-1", agent_id: "agent-1", ...task };
				...
				// Mock assignTask to be callable
				vi.spyOn(agentsApi, "assignTask").mockResolvedValue(assigned);

				const result = await agentsApi.runTask("agent-1", task);
				expect(result).toEqual(assigned);
				expect(agentsApi.assignTask).toHaveBeenCalledWith("agent-1", task);
			});
		});

		describe("getTask (alias)", () => {
			it("should call getNextTask", async () => {
				const mockTask = { id: "task-1", agent_id: "agent-1" };
				vi.mocked(apiClient.GET).mockResolvedValue({
					data: mockTask,
					error: undefined,
					response: new Response(),
				...
				// Mock getNextTask to be callable
				vi.spyOn(agentsApi, "getNextTask").mockResolvedValue(mockTask);

				const result = await agentsApi.getTask("agent-1");
				expect(result).toEqual(mockTask);
				expect(agentsApi.getNextTask).toHaveBeenCalledWith("agent-1");
			});
		});

		describe("cancelTask (alias)", () => {
			it("should call submitTaskError with cancel reason", async () => {
				vi.mocked(apiClient.POST).mockResolvedValue({
					data: undefined,
					error: undefined,
					response: new Response(),
				...
				// Mock submitTaskError to be callable
				vi.spyOn(agentsApi, "submitTaskError").mockResolvedValue(undefined);

				await expect(
					agentsApi.cancelTask("agent-1", "task-1", "User cancelled"),
				).resolves.toBeUndefined();
				expect(agentsApi.submitTaskError).toHaveBeenCalledWith({
					task_id: "task-1",
			...
			it("should use default reason when not provided", async () => {
				...
				await expect(
					agentsApi.cancelTask("agent-1", "task-1"),
				).resolves.toBeUndefined();
				expect(agentsApi.submitTaskError).toHaveBeenCalledWith({
					task_id: "task-1",
	...
	describe("searchApi", () => {
		...
		describe("batchIndex", () => {
			it("should batch index items", async () => {
				...
				await expect(
					searchApi.batchIndex(["item-1", "item-2"]),
				).resolves.toBeUndefined();
				expect(apiClient.POST).toHaveBeenCalledWith(
					"/api/v1/search/batch-index",
					{
	...
	describe("exportImportApi", () => {
		...
		describe("import", () => {
			...
			it("should handle import errors", async () => {
				...
				await expect(
					exportImportApi.import("proj-1", "json", "invalid"),
				).rejects.toThrow();
			});
		});
		...
		describe("importProject (alias)", () => {
			it("should call import", async () => {
				...
				const result = await exportImportApi.importProject(
					"proj-1",
					"json",
					'{"data": "test"}',
				);
				expect(result).toEqual(mockResult);
				expect(exportImportApi.import).toHaveBeenCalledWith(
					"proj-1",
					"json",
					'{"data": "test"}',
				);
