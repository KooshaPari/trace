...
describe.skip('API Endpoints', () => {
  beforeEach(() => {
    ...
    const fetchImpl = async (url: string | Request, options?: any) => {
      ...
      if (urlNormalized === '/api/v1/projects' && method === 'POST') {
        return createMockApiResponse({ id: 'new-proj', name: 'New Project', created_at: new Date().toISOString() }, 201)
      }

      // Projects - PUT/PATCH update
      if (urlNormalized === '/api/v1/projects/proj-1' && (method === 'PUT' || method === 'PATCH')) {
        return createMockApiResponse({ ...mockProjects[0], name: 'Updated Project', created_at: new Date().toISOString() })
      }
      ...
      if (urlNormalized === '/api/v1/items/item-1' && (method === 'PUT' || method === 'PATCH')) {
        return createMockApiResponse({ ...mockItems[0], title: 'Updated Item' })
      }
  ...
  describe('itemsApi', () => {
    ...
    it('should filter items by project', async () => {
      ...
      expect(items.every((item) => item.project_id === 'proj-1')).toBe(true)
    })
