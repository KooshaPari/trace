...
describe.skip("API Endpoints", () => {
	beforeEach(() => {
		...
		const fetchImpl = async (url: string | Request, options?: any) => {
			...
			if (
				urlNormalized === "/api/v1/projects/proj-1" &&
			...
			if (urlNormalized === "/api/v1/items" && method === "POST") {
				return createMockApiResponse(
					{ id: "new-item", ...mockItems[0], title: "New Item" },
					201,
				);
			}
			...
			if (
				urlNormalized === "/api/v1/items/item-1" &&
				(method === "PUT" || method === "PATCH")
			) {
				return createMockApiResponse({
					...mockItems[0],
			...
			if (
				urlNormalized === "/api/v1/graph/dependencies/item-1" &&
				method === "GET"
			) {
