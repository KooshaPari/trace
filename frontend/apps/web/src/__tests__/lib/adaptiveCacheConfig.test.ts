...
describe("Adaptive Cache Configuration", () => {
	describe("getDataSizeCategory", () => {
		...
		it("should categorize huge data correctly", () => {
			...
			expect(getDataSizeCategory(100 * 1024 * 1024)).toBe(
				DataSizeCategory.HUGE,
			);
		});
	});
	...
	describe("getUpdateFrequencyCategory", () => {
		it("should categorize rare updates correctly", () => {
			expect(getUpdateFrequencyCategory(0)).toBe(
				UpdateFrequencyCategory.RARELY,
			);
			expect(getUpdateFrequencyCategory(0.5)).toBe(
				UpdateFrequencyCategory.RARELY,
			);
		});

		it("should categorize infrequent updates correctly", () => {
			expect(getUpdateFrequencyCategory(2)).toBe(
				UpdateFrequencyCategory.INFREQUENT,
			);
			expect(getUpdateFrequencyCategory(4)).toBe(
				UpdateFrequencyCategory.INFREQUENT,
			);
		});

		it("should categorize moderate updates correctly", () => {
			expect(getUpdateFrequencyCategory(10)).toBe(
				UpdateFrequencyCategory.MODERATE,
			);
			expect(getUpdateFrequencyCategory(15)).toBe(
				UpdateFrequencyCategory.MODERATE,
			);
		});

		it("should categorize frequent updates correctly", () => {
			expect(getUpdateFrequencyCategory(50)).toBe(
				UpdateFrequencyCategory.FREQUENT,
			);
			expect(getUpdateFrequencyCategory(80)).toBe(
				UpdateFrequencyCategory.FREQUENT,
			);
		});

		it("should categorize very frequent updates correctly", () => {
			expect(getUpdateFrequencyCategory(150)).toBe(
				UpdateFrequencyCategory.VERY_FREQUENT,
			);
			expect(getUpdateFrequencyCategory(500)).toBe(
				UpdateFrequencyCategory.VERY_FREQUENT,
			);
		});
	});
	...
	describe("getAccessFrequencyCategory", () => {
		it("should categorize cold access correctly", () => {
			...
			expect(getAccessFrequencyCategory(0.5)).toBe(
				AccessFrequencyCategory.COLD,
			);
		});
		...
		it("should categorize very hot access correctly", () => {
			expect(getAccessFrequencyCategory(100)).toBe(
				AccessFrequencyCategory.VERY_HOT,
			);
			expect(getAccessFrequencyCategory(500)).toBe(
				AccessFrequencyCategory.VERY_HOT,
			);
		});
	});
	...
	describe("calculateAdaptiveTTL", () => {
		...
		it("should respect TTL bounds", () => {
			...
			expect(result.staleTime).toBeGreaterThanOrEqual(
