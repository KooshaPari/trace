  ...
  describe('Task Execution', () => {
    ...
    it('should respect task priority', async () => {
      ...
      const lowPriorityTasks = Array.from({ length: 3 }, (_, i) =>
        pool.executeTask('test', { priority: 'low', index: i }, {
          priority: TaskPriority.LOW,
        }).then(() => results.push(i))
      );

      // Queue high priority task
      const highPriorityTask = pool.executeTask('test', { priority: 'high' }, {
    ...
    it('should handle task timeout', async () => {
      ...
      await expect(
        slowPool.executeTask('test', {}, { timeout: 100 })
  ...
  describe('Worker Management', () => {
    ...
    it('should handle worker errors gracefully', async () => {
      const errorPool = new WorkerPool({
        ...
        workerFactory: () => {
          const worker = new MockWorker() as any;
          worker.postMessage = function(_message: any) {
            setTimeout(() => {
              if (this.onerror) {
