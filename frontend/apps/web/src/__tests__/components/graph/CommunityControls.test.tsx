...
describe(CommunityControls, () => {
	...
	it("should render the component", () => {
		...
		expect(
			screen.getByLabelText("Toggle community detection"),
		).toBeInTheDocument();
	});
	...
	it("should display community results when enabled", () => {
		...
		render(
			<CommunityControls enabled onToggle={onToggle} result={mockResult} />,
		);
	...
	it("should show export buttons when enabled with results", () => {
		...
		expect(
			screen.getByLabelText("Export communities as JSON"),
		).toBeInTheDocument();
		expect(
			screen.getByLabelText("Export communities as CSV"),
		).toBeInTheDocument();
	});
	...
	it("should limit legend display to top 12 communities", () => {
		...
		render(
			<CommunityControls enabled onToggle={onToggle} result={largeResult} />,
		);
