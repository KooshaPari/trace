...
describe("Item CRUD Integration", () => {
	...
	it("should complete full CRUD cycle", async () => {
		...
		await waitFor(
			() => {
				expect(createResult.current.isPending === false).toBe(true);
			},
			{ timeout: 3000 },
		);
		...
		if (createdItemId) {
			...
			await waitFor(
				() => {
					expect(updateResult.current.isPending === false).toBe(true);
				},
				{ timeout: 3000 },
			);
		}
		...
		if (createdItemId) {
			...
			await waitFor(
				() => {
					expect(deleteResult.current.isPending === false).toBe(true);
				},
				{ timeout: 3000 },
			);
		}
