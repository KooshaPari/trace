...
function MockProjectWorkflow({
	...
	return (
		<div className="p-4 border rounded">
			<h2 className="text-xl font-bold mb-4">Project Setup Workflow</h2>

			{step === 1 && (
				<div>
					<h3 className="font-semibold mb-3">Step 1: Create Project</h3>
					<form
						onSubmit={(e) => {
							e.preventDefault();
			...
			{step === 2 && project && (
				<div>
					<h3 className="font-semibold mb-3">Step 2: Add Items</h3>
					...
					<div className="mt-4">
						...
						{items.map((item) => (
							<div key={item.id} className="p-2 bg-gray-100 rounded mb-2">
								{item.name}
							</div>
						))}
					</div>
		...
		<div className="p-4 border rounded">
			...
			{results.length > 0 && (
				<div>
					...
					{results.map((result) => (
						<div key={result.id} className="p-3 bg-blue-50 rounded mb-2">
							{result.name}
						</div>
					))}
				</div>
...
function MockLinkCreationWorkflow({
	...
	return (
		<div className="p-4 border rounded">
			<h2 className="text-xl font-bold mb-4">Create Traceability Links</h2>
			...
			{links.length > 0 && (
				<div>
					...
					{links.map((link) => (
						<div key={link.id} className="p-2 bg-green-50 rounded mb-2">
		...
		return (
			<div className="p-4">
				...
				{error && (
					<div
						role="alert"
						className="mt-4 p-3 bg-red-50 border-l-4 border-red-600 text-red-800"
					>
						Error: {error}
						<button
