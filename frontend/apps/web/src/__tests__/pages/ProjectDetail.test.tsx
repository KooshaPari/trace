...
describe("Project Detail Page", () => {
	...
	describe("Page Rendering", () => {
		it("renders project detail page with header", async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByRole("heading", { name: /E-Commerce Platform/i }),
				).toBeInTheDocument();
			});

			expect(
				screen.getByText(/Complete online shopping solution/i),
			).toBeInTheDocument();
	...
	describe("Loading States", () => {
		it("shows loading skeleton while fetching project", async () => {
			...
			vi.mocked(fetchProject).mockImplementation(
				() =>
					new Promise((resolve) =>
						setTimeout(() => resolve({ id: "proj-123", name: "Test" }), 100),
					),
			);
			vi.mocked(fetchProjectItems).mockResolvedValue({ data: [], total: 0 });
			vi.mocked(fetchProjectLinks).mockResolvedValue({ data: [], total: 0 });
	...
	describe("Error Handling", () => {
		...
		it("shows error when items fail to load", async () => {
			...
			vi.mocked(fetchProjectItems).mockRejectedValue(
	...
	describe("Links Management", () => {
		...
		it("creates new link between items", async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByRole("dialog", { name: /create link/i }),
				).toBeInTheDocument();
			});
			...
			await waitFor(() => {
				expect(
					screen.getByText(/link created successfully/i),
				).toBeInTheDocument();
			});
		});
	...
	describe("Project Actions", () => {
		it("edits project details", async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByRole("dialog", { name: /edit project/i }),
				).toBeInTheDocument();
			});
	...
	describe("Accessibility", () => {
		it("has proper ARIA labels", async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByRole("navigation", { name: /tabs/i }),
				).toBeInTheDocument();
			});
