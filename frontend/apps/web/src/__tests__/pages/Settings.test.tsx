...
describe("Settings Page", () => {
	...
	describe("Notification Settings", () => {
		...
		it("manages notification filters", async () => {
			...
			await userEvent.click(
				screen.getByRole("option", { name: /documentation/i }),
			);
	...
	describe("Integration Settings", () => {
		...
		it("shows connected integration status", async () => {
			...
			await waitFor(() => {
				...
				expect(
					within(githubSection).getByText(/connected/i),
				).toBeInTheDocument();
			});
		...
		it("connects new integration", async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByRole("button", { name: /connect jira/i }),
				).toBeInTheDocument();
			});
			...
			await waitFor(() => {
				expect(
					screen.getByRole("dialog", { name: /connect jira/i }),
				).toBeInTheDocument();
			});
		...
		it("disconnects integration", async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByRole("button", { name: /disconnect slack/i }),
				).toBeInTheDocument();
			});
		...
		it("configures integration settings", async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByRole("button", { name: /configure slack/i }),
				).toBeInTheDocument();
			});
	...
	describe("Security Settings", () => {
		it("displays security options", async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByLabelText(/two-factor authentication/i),
				).toBeInTheDocument();
			});
