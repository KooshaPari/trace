...
describe("Projects List Page", () => {
	...
	describe("Error Handling", () => {
		...
		it("handles partial data load gracefully", async () => {
			...
			expect(
				screen.getByText(/some items could not be loaded/i),
			).toBeInTheDocument();
		});
	});
	...
	describe("Sorting", () => {
		it("sorts projects by name ascending", async () => {
			...
			await userEvent.click(
				screen.getByRole("option", { name: /name \(a-z\)/i }),
			);
		...
		it("sorts by item count", async () => {
			...
			await userEvent.click(sortSelect);
			await userEvent.click(
				screen.getByRole("option", { name: /most items/i }),
			);
	...
	describe("Bulk Operations", () => {
		...
		it("bulk deletes selected projects", async () => {
			...
			// Select projects
			await userEvent.click(
				screen.getByRole("checkbox", { name: /select project 1/i }),
			);
			await userEvent.click(
				screen.getByRole("checkbox", { name: /select project 2/i }),
			);
