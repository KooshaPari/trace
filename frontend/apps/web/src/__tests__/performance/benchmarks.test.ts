...
describe("Performance Benchmarks", () => {
	describe("FPS Benchmarks", () => {
		...
		scenarios.forEach(({ nodes, edges, expectedFps, mode }) => {
			it(`should achieve ${expectedFps}+ FPS with ${nodes.toLocaleString()} nodes (${mode})`, async () => {
				const { nodes: testNodes, edges: testEdges } = generateSyntheticGraph(
					nodes,
					edges,
					{
	...
	describe("Memory Usage", () => {
		it("should stay under 600MB for 10k nodes", async () => {
			...
			// @ts-expect-error - Performance memory API may not be available in all environments
			const memBefore = performance.memory?.usedJSHeapSize || 0;

			const { result } = renderHook(() => useHybridGraph(nodes, edges));
			await waitFor(() => expect(result.current.nodeCount).toBe(10_000));
		...
		it("should stay under 1GB for 100k nodes", async () => {
			...
			const { result } = renderHook(() => useHybridGraph(nodes, edges));
			await waitFor(() => expect(result.current.nodeCount).toBe(100_000));
	...
	describe("Interaction Latency", () => {
		it("should respond to click in <50ms (ReactFlow)", async () => {
			...
			logger.info(
				`✓ Click latency (ReactFlow, 5k nodes): ${latency.toFixed(2)}ms`,
			);
	...
	describe("Summary Report", () => {
		it("should generate comprehensive benchmark report", () => {
			...
			logger.info("\n📊 FPS Benchmarks:");
			logger.info("─".repeat(80));
			benchmarkResults.fps.forEach(
				({ nodes, edges: _edges, fps, mode, renderTime }) => {
					const status = fps >= 50 ? "✓" : "✗";
					logger.info(
			...
			logger.info("─".repeat(80));
			benchmarkResults.spatialIndex.forEach(
				({ nodes, queryTime, speedup, visibleNodes }) => {
			...
			logger.info("\n🔗 Clustering (Louvain):");
			logger.info("─".repeat(80));
			benchmarkResults.clustering.forEach(
				({ nodes, edges: _edges, time, edgeReduction }) => {
			...
			const allPassed =
				...
				benchmarkResults.spatialIndex.every(
					(r) => r.queryTime < (r.nodes === 10_000 ? 5 : 10),
				) &&
