...
describe("Phase 2: Legend Filter O(1) Performance", () => {
	...
	it("should have O(1) lookup performance with Set", () => {
		const largeTypeSet = new Set(
			Array.from({ length: 1000 }, (_, i) => `type-${i}`),
		);
		const largeTypeArray = [...largeTypeSet];
...
describe("Phase 4: Selected Node O(1) Lookup", () => {
	...
	it("should have O(1) selection lookup performance", () => {
		const largeSelection = new Set(
			Array.from({ length: 10_000 }, (_, i) => `node-${i}`),
		);
		const largeSelectionArray = [...largeSelection];
...
describe("Integration: Combined Optimizations", () => {
	it("should apply all optimizations together", () => {
		...
		const processGraph = (
			...
			const filteredNodes = nodes.filter((n) =>
				visibleTypes.has(n.data?.itemType),
			);
	...
	it("should maintain performance with large graphs", () => {
		...
		const largeSelectedNodes = new Set(
		...
		const filtered = nodes.filter((n) =>
			largeVisibleTypes.has(n.data?.itemType),
		);
