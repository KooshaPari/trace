...
describe('Phase 1: Deterministic Edge Culling', () => {
  ...
  it('should not use Math.random() for edge culling', () => {
    ...
    const mockEdges: Edge[] = [
      { id: '1', source: 'a', target: 'b', data: { tier: 1 } },
      { id: '2', source: 'a', target: 'c', data: { tier: 2 } },
      { id: '3', source: 'b', target: 'c', data: { tier: 3 } },
    ];
  ...
  it('should have stable culling results across re-renders', () => {
    const edges: Edge[] = Array.from({ length: 1000 }, (_, i) => ({
      ...
      source: `node-${i}`,
      target: `node-${i + 1}`,
      data: { tier: (i % 3) + 1 }, // Tier 1, 2, or 3
    }));

    const cullEdges = (edges: Edge[], zoom: number) => {
      if (zoom < 0.5) return edges.filter(e => e.data?.tier === 1);
      if (zoom < 0.75) return edges.filter(e => (e.data?.tier ?? 1) <= 2);
      return edges;
    };
...
describe('Phase 2: Legend Filter O(1) Performance', () => {
  it('should use Set for visibleTypes instead of Array', () => {
    ...
    const filterNodesWithSet = (nodes: Node[], visibleTypes: Set<string>) => {
      return nodes.filter(node => visibleTypes.has(node.data?.itemType));
    };

    // Simulate node filtering with Array (O(n))
    const filterNodesWithArray = (nodes: Node[], visibleTypes: string[]) => {
      return nodes.filter(node => visibleTypes.includes(node.data?.itemType));
    };

    const mockNodes: Node[] = [
      { id: '1', position: { x: 0, y: 0 }, data: { itemType: 'requirement' } },
      { id: '2', position: { x: 0, y: 0 }, data: { itemType: 'test_case' } },
      { id: '3', position: { x: 0, y: 0 }, data: { itemType: 'epic' } },
      { id: '4', position: { x: 0, y: 0 }, data: { itemType: 'defect' } },
    ];

    const resultSet = filterNodesWithSet(mockNodes, visibleTypesSet);
    const resultArray = filterNodesWithArray(mockNodes, Array.from(visibleTypesSet));

    // Results should be the same
    expect(resultSet.length).toBe(resultArray.length);
    expect(resultSet.map(n => n.id).toSorted((a, b) => String(a).localeCompare(String(b)))).toEqual(resultArray.map(n => n.id).toSorted((a, b) => String(a).localeCompare(String(b))));
  });

  it('should have O(1) lookup performance with Set', () => {
    const largeTypeSet = new Set(Array.from({ length: 1000 }, (_, i) => `type-${i}`));
    const largeTypeArray = Array.from(largeTypeSet);
    ...
    for (let i = 0; i < 10000; i++) {
      largeTypeArray.includes('type-500'); // Middle element
    }
  ...
  it('should maintain Set type for visibleTypes', () => {
    ...
    expect(visibleTypes.has('requirement')).toBe(true);
    expect(visibleTypes.has('epic')).toBe(false);
  });
});
...
describe('Phase 4: Selected Node O(1) Lookup', () => {
  it('should use Set for selectedNodes', () => {
    ...
    expect(selectedNodes.has('node-4')).toBe(false);
  });

  it('should have O(1) selection lookup performance', () => {
    const largeSelection = new Set(Array.from({ length: 10000 }, (_, i) => `node-${i}`));
    const largeSelectionArray = Array.from(largeSelection);
    ...
    const arrayStartTime = performance.now();
    for (let i = 0; i < 100000; i++) {
      largeSelectionArray.includes('node-5000');
    }
    const arrayDuration = performance.now() - arrayStartTime;
...
describe('Phase 6: LOD Node Rendering', () => {
  it('should switch to simple rendering at low zoom', () => {
    ...
    expect(getNodeLOD(1.0)).toBe('detailed');
  });
...
describe('Phase 7: Edge LOD Tiers', () => {
  ...
  it('should show correct tiers at different zoom levels', () => {
    ...
    const edges: Edge[] = [
      { id: '1', source: 'a', target: 'b', data: { tier: 1 } },
      { id: '2', source: 'a', target: 'c', data: { tier: 1 } },
      { id: '3', source: 'b', target: 'c', data: { tier: 2 } },
      { id: '4', source: 'b', target: 'd', data: { tier: 2 } },
      { id: '5', source: 'c', target: 'd', data: { tier: 3 } },
      { id: '6', source: 'd', target: 'e', data: { tier: 3 } },
    ];

    // At zoom 0.4: only tier 1 (2 edges)
    const low = getVisibleEdges(edges, 0.4);
    expect(low.length).toBe(2);
    expect(low.every(e => e.data?.tier === 1)).toBe(true);

    // At zoom 0.6: tier 1 and 2 (4 edges)
    const medium = getVisibleEdges(edges, 0.6);
    expect(medium.length).toBe(4);
    expect(medium.every(e => (e.data?.tier ?? 1) <= 2)).toBe(true);
  ...
  it('should apply all optimizations together', () => {
    ...
    const processGraph = (
      ...
      const culledEdges = edges.filter(e => {
        ...
        if (zoom < 0.5) return tier === 1;
        if (zoom < 0.75) return tier <= 2;
        return true;
      });
      ...
      const styledEdges = culledEdges.map(e => {
        ...
        return { ...e, style: styleCache.get(cacheKey) };
      });
    ...
    const nodes: Node[] = [
      { id: 'node-1', position: { x: 0, y: 0 }, data: { itemType: 'requirement' } },
      { id: 'node-2', position: { x: 0, y: 0 }, data: { itemType: 'test_case' } },
      { id: 'node-3', position: { x: 0, y: 0 }, data: { itemType: 'epic' } },
    ];

    const edges: Edge[] = [
      { id: 'edge-1', source: 'node-1', target: 'node-2', data: { tier: 1 } },
      { id: 'edge-2', source: 'node-2', target: 'node-3', data: { tier: 2 } },
    ];
    ...
    expect(result.nodes.find(n => n.id === 'node-1')?.selected).toBe(true);
    expect(result.nodes.find(n => n.id === 'node-2')?.selected).toBe(false);
...
describe('Performance Benchmarks', () => {
  ...
  it('should cull 10k edges in < 50ms', () => {
    const edges: Edge[] = Array.from({ length: 10000 }, (_, i) => ({
      id: `edge-${i}`,
      source: `node-${i}`,
      target: `node-${i + 1}`,
      data: { tier: (i % 3) + 1 },
