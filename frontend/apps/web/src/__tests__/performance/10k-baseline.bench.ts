...
test.describe("10k Node Baseline Performance", () => {
	...
	test("Scenario 1: Load 10k nodes with 50k edges", async ({ page }) => {
		...
		// Assertions
		logger.info(
			`  ✓ Initial render time: ${metrics.initialRenderTime}ms (target: <3000ms)`,
		);
		logger.info(
			`  ✓ Layout computation: ${metrics.layoutComputationTime.toFixed(2)}ms`,
		);
		logger.info(
			`  ✓ Memory usage: ${metrics.finalMemoryMB.toFixed(2)}MB (target: <500MB)`,
		);
		logger.info(
	...
	test("Scenario 4: Node selection performance", async ({ page }) => {
		...
		logger.info(
			`  ✓ Avg selection time: ${avgSelectionTime.toFixed(2)}ms (target: <100ms)`,
		);
	...
	test("Scenario 5: LOD transition performance", async ({ page }) => {
		...
		logger.info(
			`  ✓ Avg LOD transition: ${avgTransitionTime.toFixed(2)}ms (target: <100ms)`,
		);
	...
	test.afterAll(async () => {
		...
		const scenarios = benchmarkResults.scenarios;
		...
		logger.info(
			`  Zoom FPS: ${scenarios.zoomOperations.zoomFPS.toFixed(2)} (target: >30fps)`,
		);
		logger.info(
		...
		logger.info(
			`  Memory: ${scenarios.load10kGraph.finalMemoryMB.toFixed(2)}MB (target: <500MB)`,
		);
