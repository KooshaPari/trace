...
describe('Cache Integration Scenarios', () => {
	...
	it('should handle project workflow', async () => {
		...
		// Verify items cached
		for (let i = 0; i < 5; i++) {
			expect(await cache.has(CacheKeys.item.byId(`item-${i}`))).toBe(true);
		}
		...
		expect(await cache.has(CacheKeys.project.byId(projectId))).toBe(false);
		for (let i = 0; i < 5; i++) {
			expect(await cache.has(CacheKeys.item.byId(`item-${i}`))).toBe(false);
		}
	});
