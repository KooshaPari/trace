...
export function QADashboardView({ projectId }: QADashboardViewProps) {
	...
	const { data: summary, isLoading: summaryLoading } =
		useQAMetricsSummary(projectId);
	const { data: passRateTrend, isLoading: trendLoading } = usePassRateTrend(
		projectId,
		trendDays,
	);
	const { data: coverageMetrics, isLoading: coverageLoading } =
		useCoverageMetrics(projectId);
	const { data: defectDensity, isLoading: defectLoading } =
		useDefectDensity(projectId);
	const { data: flakyTests, isLoading: flakyLoading } =
		useFlakyTests(projectId);
	const { data: executionHistory, isLoading: historyLoading } =
		useExecutionHistory(projectId);
			...
			<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
				...
				<Card>
					<CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
						...
						{summary?.testRuns.averagePassRate &&
						summary.testRuns.averagePassRate >= 80 ? (
									...
									<div className="grid grid-cols-3 gap-4 text-center text-sm">
										<div>
											...
											<p className="font-bold">
												{passRateTrend?.trend.reduce(
													(acc, p) => acc + p.totalRuns,
													0,
												) || 0}
											</p>
										</div>
							...
							<CardContent>
								{summaryLoading ? (
									...
									<div className="space-y-3">
										{Object.entries(summary?.testCases.byStatus || {}).map(
											([status, count]) => (
												<div key={status} className="flex items-center gap-3">
													...
													<Progress
														value={
															(count / (summary?.testCases.total || 1)) * 100
														}
														className="flex-1"
													/>
						...
						<Card>
							...
							<CardContent>
								{summaryLoading ? (
									...
									<div className="space-y-3">
										{Object.entries(summary?.testRuns.byType || {}).map(
											([type, count]) => (
												<div key={type} className="flex items-center gap-3">
													...
													<Progress
														value={
															(count / (summary?.testRuns.total || 1)) * 100
														}
														className="flex-1"
													/>
													<div className="w-12 text-right text-sm font-medium">
							...
							{coverageLoading ? (
								...
								<div className="space-y-6">
									...
									<div className="space-y-2">
										<div className="flex justify-between text-sm">
											...
											<span className="font-bold">
												{coverageMetrics?.overall.coveragePercentage?.toFixed(
													1,
												)}
												%
											</span>
										...
										<Progress
											value={coverageMetrics?.overall.coveragePercentage || 0}
										/>
										<div className="flex justify-between text-xs text-muted-foreground">
											<span>
												{coverageMetrics?.overall.coveredRequirements || 0}{" "}
												covered
											</span>
											<span>
												{coverageMetrics?.overall.totalRequirements || 0} total
											</span>
										</div>
									</div>
									...
									<div>
										...
										<div className="space-y-2">
											{Object.entries(coverageMetrics?.byView || {}).map(
												([view, data]) => (
													<div key={view} className="flex items-center gap-3">
														<div className="w-32 text-sm truncate">{view}</div>
														<Progress
									...
									{coverageMetrics && coverageMetrics.gapsCount > 0 && (
										<div className="p-4 rounded-lg bg-yellow-500/10 border border-yellow-500/30">
											...
											<p className="text-sm mt-1 text-muted-foreground">
												{coverageMetrics.gapsCount} requirements without test
												coverage
								...
								{flakyLoading ? (
									...
									<div className="space-y-4">
										...
										<div className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
											...
											<Badge variant="secondary">
												{flakyTests?.potentiallyFlakyCount || 0}
											</Badge>
										</div>
											...
											(flakyTests.markedFlakyCount > 0 ||
												...
												<div className="pt-2">
													...
													<div className="space-y-1">
														{[
															...flakyTests.markedFlaky,
															...flakyTests.potentiallyFlaky,
														]
															.slice(0, 5)
															.map((test, i) => (
																<div
