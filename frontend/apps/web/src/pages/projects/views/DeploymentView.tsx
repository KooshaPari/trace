...
const healthColors = { healthy: 'bg-green-500', degraded: 'bg-yellow-500', down: 'bg-red-500' }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button className="flex items-center gap-2 rounded-lg border px-3 py-1.5 text-sm hover:bg-accent">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {environments.map((env) => (
          <div key={env.name} className="rounded-lg border p-4">
            ...
            <button
              onClick={() => triggerDeploy(env.name)}
              disabled={deploying === env.name}
              className="mt-4 flex w-full items-center justify-center gap-2 rounded-lg bg-primary py-2 text-sm text-primary-foreground disabled:opacity-50"
            >
              {deploying === env.name ? (
                <>
          ...
          {recentDeploys.map((d) => {
            ...
            return (
              <div key={d.id} className="flex items-center gap-4 border-b p-3 last:border-b-0">
                ...
                <span className="ml-auto text-xs text-muted-foreground">{d.deployedAt}</span>
