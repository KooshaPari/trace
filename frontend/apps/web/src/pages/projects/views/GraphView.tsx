...
const nodes = [
  // Features (purple)
  { data: { id: 'f1', label: 'User Authentication', type: 'feature', status: 'done' } },
  { data: { id: 'f2', label: 'Project Management', type: 'feature', status: 'in_progress' } },
  { data: { id: 'f3', label: 'Traceability Links', type: 'feature', status: 'todo' } },
  { data: { id: 'f4', label: 'Multi-View System', type: 'feature', status: 'in_progress' } },
  // Code modules (blue)
  { data: { id: 'c1', label: 'auth.py', type: 'code', status: 'done' } },
  { data: { id: 'c2', label: 'projects.py', type: 'code', status: 'in_progress' } },
  ...
  { data: { id: 't2', label: 'test_projects.py', type: 'test', status: 'in_progress' } },
...
export function GraphView() {
  ...
  const initCytoscape = useCallback(() => {
    ...
    const filteredNodes = filterType ? nodes.filter((n) => n.data.type === filterType) : nodes
      ...
      style: [
        {
          ...
          style: {
            ...
            'background-opacity': (ele: any) => statusOpacity[ele.data('status')] || 1,
            label: 'data(label)',
            color: '#fff',
            'text-outline-color': (ele: any) => typeColors[ele.data('type')] || '#666',
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <select
            value={layout}
            onChange={(e) => setLayout((e.target as HTMLSelectElement).value as typeof layout)}
          ...
          <select
            value={filterType || ''}
            onChange={(e) => setFilterType((e.target as HTMLSelectElement).value || null)}
            className="rounded-md border px-2 py-1 text-sm"
          >
          ...
          <div className="flex gap-1 rounded-md border p-1">
            <button onClick={handleZoomIn} className="rounded p-1 hover:bg-accent">
              <ZoomIn className="h-4 w-4" />
            </button>
            <button onClick={handleZoomOut} className="rounded p-1 hover:bg-accent">
              <ZoomOut className="h-4 w-4" />
            </button>
            <button onClick={handleFit} className="rounded p-1 hover:bg-accent">
              <Maximize2 className="h-4 w-4" />
            </button>
            <button onClick={handleReset} className="rounded p-1 hover:bg-accent">
      ...
      <div className="flex gap-4">
        <div ref={containerRef} className="h-[600px] flex-1 rounded-lg border bg-slate-950" />
      ...
      <div className="flex items-center gap-6 text-sm">
        <div className="flex items-center gap-2">
          <span className="h-3 w-3 rounded-full" style={{ backgroundColor: typeColors.feature }} />
          <span>Feature</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="h-3 w-3 rounded-full" style={{ backgroundColor: typeColors.code }} />
          <span>Code</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="h-3 w-3 rounded-full" style={{ backgroundColor: typeColors.test }} />
          <span>Test</span>
        </div>
