...
test.describe("Mobile Optimization - Phase 12 & 13", () => {
	...
	test("card items are responsive with proper touch targets", async ({
		page,
	}) => {
		await page.goto("/items");
		await page.waitForLoadState("networkidle");
	...
	test("menu items have proper touch targets (52px minimum)", async ({
		page,
	}) => {
		await page.goto("/");
	...
	test("responsive grid adjusts from 1 to 2 columns on larger mobile", async ({
		page,
	}) => {
		await page.goto("/items");
		await page.waitForLoadState("networkidle");
