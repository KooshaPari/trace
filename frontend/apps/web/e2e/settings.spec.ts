...
test.describe("Settings Page Navigation", () => {
	...
	test("should display settings header and description", async ({ page }) => {
		...
		const description = page.getByText(
			/Manage your preferences and configuration/i,
		);
...
test.describe("Notifications Settings Tab", () => {
	...
	test("should save multiple notification preferences at once", async ({
		page,
	}) => {
...
test.describe("API Keys Tab", () => {
	...
	test("should display API key section", async ({ page }) => {
		...
		// Description text
		const description = page.getByText(
			/Used for external integrations and webhooks/i,
		);
		await expect(description).toBeVisible();
	...
	test("should have password-type API key input", async ({ page }) => {
		...
		const inputType = await apiKeyInput.evaluate(
	...
	test("should allow entering API key", async ({ page }) => {
		...
		// Verify value is set
		const inputValue = await apiKeyInput.evaluate(
...
test.describe("Settings Form Validation", () => {
	...
	test("should validate email field with HTML5 validation", async ({
		page,
	}) => {
		// Go to general tab
		const generalTab = page.getByRole("tab", { name: /general/i });
...
test.describe("Settings Tab Navigation", () => {
	...
	test("should switch between tabs", async ({ page }) => {
		...
		let isSelected = await generalTab.evaluate((el) =>
		...
		isSelected = await appearanceTab.evaluate((el) =>
			el.getAttribute("aria-selected"),
		);
		expect(isSelected).toBe("true");
		...
		isSelected = await notificationsTab.evaluate((el) =>
			el.getAttribute("aria-selected"),
		);
		expect(isSelected).toBe("true");
		...
		isSelected = await generalTab.evaluate((el) =>
			el.getAttribute("aria-selected"),
		);
		expect(isSelected).toBe("true");
	});
	...
	test("should be accessible via keyboard", async ({ page }) => {
		...
		const appearanceTab = page.getByRole("tab", { name: /appearance/i });
		const isSelected = await appearanceTab.evaluate((el) =>
			el.getAttribute("aria-selected"),
		);
		expect(isSelected).toBe("true");
...
test.describe("Settings Integration", () => {
	...
	test("should handle settings page load errors gracefully", async ({
		page,
	}) => {
		// Go to settings
		await page.goto("/settings");
