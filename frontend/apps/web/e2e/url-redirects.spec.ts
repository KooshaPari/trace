...
test.describe("URL Redirects - Backward Compatibility", () => {
	...
	test("should redirect /items/:itemId to new URL format", async ({ page }) => {
		...
		await page.waitForURL(
			"**/projects/project-456/views/feature/valid-item-123",
			{
				timeout: 5000,
			},
		);
	...
	test("should show loading state during redirect", async ({ page }) => {
		...
		const navigationPromise = page.goto(
			"http://localhost:5173/items/valid-item-123",
		);
	...
	test("should convert uppercase view type to lowercase in URL", async ({
		page,
	}) => {
	...
	test("should redirect /graph to /projects with hint", async ({ page }) => {
		...
		await expect(
			page.locator("text=Please select a project to view its graph"),
		).toBeVisible();
	...
	test("should redirect /search to /projects with search hint", async ({
		page,
	}) => {
		...
		await expect(page.locator("text=Redirecting to projects")).toBeVisible();
		await expect(
			page.locator("text=Use Cmd+K or Ctrl+K to open global search"),
		).toBeVisible();
	...
	test("should not allow back navigation to old URL after redirect", async ({
		page,
	}) => {
		// Start from home
