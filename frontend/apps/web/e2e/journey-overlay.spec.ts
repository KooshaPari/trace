...
test.describe('Journey Overlay', () => {
  ...
  test.describe('Journey Selection', () => {
    test('should display journey selector dropdown', async ({ page }) => {
      ...
      if (await journeySelector.isVisible({ timeout: 2000 }).catch(() => false)) {
    ...
    test('should select journey from dropdown', async ({ page }) => {
      ...
      if (await journeySelector.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        if (await journeyOption.isVisible({ timeout: 1000 }).catch(() => false)) {
    ...
    test('should show selected journey name', async ({ page }) => {
      ...
      if (await journeySelector.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        if (await journeyOption.isVisible({ timeout: 1000 }).catch(() => false)) {
          ...
          const selectedText = await journeySelector.textContent().catch(() => null);
    ...
    test('should highlight journey nodes in graph', async ({ page }) => {
      ...
      if (await journeySelector.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        if (await journeyOption.isVisible({ timeout: 1000 }).catch(() => false)) {
          ...
          const highlightedCount = await highlightedNodes.count().catch(() => 0);
    ...
    test('should distinguish journey start and end nodes', async ({ page }) => {
      ...
      if (await journeySelector.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        if (await journeyOption.isVisible({ timeout: 1000 }).catch(() => false)) {
          ...
          const startVisible = await startNode.isVisible({ timeout: 2000 }).catch(() => false);
          const endVisible = await endNode.isVisible({ timeout: 2000 }).catch(() => false);
        ...
        if (await journeyOption.isVisible({ timeout: 1000 }).catch(() => false)) {
          ...
          if (await clearBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
            ...
            if (nodesAfterClear === allNodes) {
              console.log('Graph restored to normal state after clearing journey');
            }
          }
        }
      }
