...
test.describe('Sigma.js WebGL Performance', () => {
  ...
  test('should render 10k nodes at 60 FPS', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      (window as Window & { __testGraphData?: { nodes: NodeItem[]; edges: EdgeItem[] } }).__testGraphData = { nodes, edges };
    });
    ...
    await page.waitForSelector('[data-testid="sigma-container"]', { timeout: 10000 });
  ...
  test('should highlight selected node and neighbors', async ({ page }) => {
    ...
    const highlightTime = await page.evaluate(() => {
      ...
      window.dispatchEvent(new CustomEvent('updateHighlights'));
  ...
  test('should handle 50k nodes with performance mode', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      (window as Window & { __testGraphData?: { nodes: NodeItem[]; edges: unknown[] } }).__testGraphData = { nodes, edges: [] };
    });

    // Wait for graph to load
    await page.waitForSelector('[data-testid="sigma-container"]', { timeout: 15000 });
  ...
  test('should handle 100k nodes (stress test)', async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="sigma-container"]', { timeout: 20000 });
