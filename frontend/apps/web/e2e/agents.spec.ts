	...
	test.describe("Agent Status", () => {
		...
		test("should show agent capabilities", async ({ page }) => {
			...
			if (await firstAgent.isVisible({ timeout: 2000 })) {
				...
				const capabilitiesSection = page.getByRole("heading", {
					name: /capabilities/i,
				});
				await expect(capabilitiesSection)
					.toBeVisible({ timeout: 5000 })
					.catch(() => {
				...
				const capabilities = page.getByText(
					/code.*analysis|test.*execution|doc.*generation/i,
				);
				await expect(capabilities.first())
					.toBeVisible({ timeout: 5000 })
					.catch(() => {
	...
	test.describe("Agent Actions", () => {
		test("should start agent task", async ({ page }) => {
			...
			if (await idleAgent.isVisible({ timeout: 2000 })) {
				...
				const startTaskBtn = page.getByRole("button", {
					name: /start.*task|run.*task|execute/i,
				});
		...
		test("should display task history", async ({ page }) => {
			...
			if (await firstAgent.isVisible({ timeout: 2000 })) {
				...
				const historySection = page.getByRole("heading", {
					name: /task.*history|recent.*tasks/i,
				});
			...
			if (await firstAgent.isVisible({ timeout: 2000 })) {
				...
				if (await settingsBtn.isVisible({ timeout: 2000 })) {
					...
					const settingsDialog = page.getByRole("dialog", {
						name: /settings|configuration/i,
					});
					await expect(settingsDialog)
						.toBeVisible({ timeout: 5000 })
						.catch(() => {
				...
				if (await settingsBtn.isVisible({ timeout: 2000 })) {
					...
					if (await settingsDialog.isVisible({ timeout: 2000 })) {
						...
						if (await nameInput.isVisible({ timeout: 2000 })) {
							...
							// Save settings
							const saveBtn = settingsDialog.getByRole("button", {
								name: /save|update/i,
							});
							await saveBtn.click();
							await page.waitForLoadState("networkidle");
							...
							// Verify update
							await expect(page.getByText(/updated agent name/i)).toBeVisible({
								timeout: 5000,
							});
						} else {
	...
	test.describe("Agent Monitoring", () => {
		test("should display agent metrics", async ({ page }) => {
			...
			if (await firstAgent.isVisible({ timeout: 2000 })) {
				...
				const metricsSection = page.getByRole("heading", {
					name: /metrics|statistics/i,
				});
				if (await metricsSection.isVisible({ timeout: 2000 })) {
					// Check for common metrics
					const tasksCompleted = page.getByText(
						/tasks.*completed|completed.*tasks/i,
					);
					await expect(tasksCompleted)
						.toBeVisible({ timeout: 5000 })
						.catch(() => {
	...
	test.describe("Agent Dashboard Widget", () => {
		...
		test("should navigate to agents page from widget", async ({ page }) => {
			...
			if (await agentsWidget.isVisible({ timeout: 2000 })) {
				...
				const viewAllLink = agentsWidget.getByRole("link", {
					name: /view.*all|see.*all|more/i,
				});
				if (await viewAllLink.isVisible({ timeout: 2000 })) {
					...
					const widgetHeader = agentsWidget.getByRole("heading", {
						name: /agents/i,
					});
