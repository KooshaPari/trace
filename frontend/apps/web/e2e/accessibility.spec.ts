	...
	test("should maintain focus order", async ({ page }) => {
		...
		for (let i = 1; i < focusOrder.length - 1; i++) {
			...
			const isLogicalOrder =
				curr.y > prev.y || (curr.y === prev.y && curr.x >= prev.x);
...
test.describe("Accessibility - Automated Testing", () => {
	...
	test("items page should have no accessibility violations", async ({
		page,
	}) => {
		await page.goto("/items");
		await page.waitForLoadState("networkidle");
	...
	test("agents page should have no accessibility violations", async ({
		page,
	}) => {
	...
	test("graph page should have no accessibility violations", async ({
		page,
	}) => {
	...
	test("item detail page should have no accessibility violations", async ({
		page,
	}) => {
		await page.goto("/items");
		await page.waitForLoadState("networkidle");
	...
	test("modal dialogs should have no accessibility violations", async ({
		page,
	}) => {
		await page.goto("/items");
		await page.click('button:has-text("New Item")');
