...
test.describe("Multi-Perspective Display Modes", () => {
	...
	test.describe("Display Mode Switching", () => {
		...
		test("should switch to split view mode", async ({ page }) => {
			...
			if (await modeBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
				...
				if (await splitOption.isVisible({ timeout: 1000 }).catch(() => false)) {
					...
					const splitContainers = page.locator(
						".react-flow, [class*='split'], [class*='panel']",
					);
		...
		test("should switch to layered view mode", async ({ page }) => {
			...
			if (await modeBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
				...
				if (
					await layeredOption.isVisible({ timeout: 1000 }).catch(() => false)
				) {
		...
		test("should switch to unified view mode", async ({ page }) => {
			...
			if (await modeBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
				...
				if (
					await unifiedOption.isVisible({ timeout: 1000 }).catch(() => false)
				) {
		...
		test("should toggle between view modes without data loss", async ({
			page,
		}) => {
			...
			if (await modeBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
				...
				if (initialNodeCount > 0) {
					...
					if (
						await splitOption.isVisible({ timeout: 1000 }).catch(() => false)
					) {
						await splitOption.click();
						await page.waitForTimeout(1000);
						...
						if (
							await singleOption.isVisible({ timeout: 1000 }).catch(() => false)
						) {
	...
	test.describe("Split View Perspectives", () => {
		test("should display two side-by-side graph containers", async ({
			page,
		}) => {
			const modeBtn = page
				.locator("button")
			...
			if (await modeBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
				...
				if (await splitOption.isVisible({ timeout: 1000 }).catch(() => false)) {
					...
					const leftPanel = page
						.locator("[class*='left'], [class*='primary']")
						.first();
					const rightPanel = page
						.locator("[class*='right'], [class*='secondary']")
						.first();

					const leftVisible = await leftPanel
						.isVisible({ timeout: 2000 })
						.catch(() => false);
					const rightVisible = await rightPanel
						.isVisible({ timeout: 2000 })
						.catch(() => false);
		...
		test("should select different perspectives for split panels", async ({
			page,
		}) => {
			const modeBtn = page
				.locator("button")
			...
			if (await modeBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
				...
				if (await splitOption.isVisible({ timeout: 1000 }).catch(() => false)) {
					...
					const selectorCount = await perspectiveSelectors
						.count()
						.catch(() => 0);
		...
		test("should synchronize pan/zoom between split panels", async ({
			page,
		}) => {
			const modeBtn = page
				.locator("button")
	...
	test.describe("Pivot Navigation", () => {
		test("should navigate between equivalents in pivot view", async ({
			page,
		}) => {
			const modeBtn = page
				.locator("button")
			...
			if (await modeBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
				...
				if (await pivotOption.isVisible({ timeout: 1000 }).catch(() => false)) {
					...
					if (buttonCount > 0) {
						...
						const equivalentItems = page.locator(
							"[class*='equivalent'], [class*='pivot']",
						);
		...
		test("should show equivalence relationships in pivot mode", async ({
			page,
		}) => {
			const modeBtn = page
				.locator("button")
			...
			if (await modeBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
				...
				if (await pivotOption.isVisible({ timeout: 1000 }).catch(() => false)) {
					...
					const equivalenceLabels = page.getByText(
						/equivalent|related|maps to|corresponds/i,
					);
	...
	test.describe("Dimension Perspective Selection", () => {
		test("should show available dimensions for perspective selection", async ({
			page,
		}) => {
			...
			if (
				await dimensionSelector.isVisible({ timeout: 2000 }).catch(() => false)
			) {
				// Check dimensions available
				const dimensions = page.locator(
					"button[class*='dimension'], option, [role='option']",
				);
		...
		test("should switch dimension perspective and update display", async ({
			page,
		}) => {
			...
			if (await dimensionBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
				...
				if (
					await dimensionOption.isVisible({ timeout: 1000 }).catch(() => false)
				) {
...
test.describe("View Persistence and State Management", () => {
	test("should remember selected display mode after page refresh", async ({
		page,
	}) => {
		await page.goto("/graph");
		await page.waitForLoadState("networkidle");
	...
	test("should maintain perspective selections across view changes", async ({
		page,
	}) => {
		await page.goto("/graph");
		await page.waitForLoadState("networkidle");
