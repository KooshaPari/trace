...
test.describe('Hybrid Graph View Transitions', () => {
  ...
  test('should show ReactFlow mode for < 10k nodes', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      window.__testGraphData = { nodes, edges: [] };
    });

    // Should show ReactFlow mode indicator
    await expect(page.getByText('ReactFlow Mode')).toBeVisible({ timeout: 5000 });
  ...
  test('should switch to WebGL mode at 10k nodes', async ({ page }) => {
    ...
    await page.evaluate(() => {
      const existingNodes = window.__testGraphData!.nodes;
      for (let i = 5000; i < 10500; i++) {
        existingNodes.push({
          id: `node-${i}`,
      ...
      window.__testGraphData = { nodes: existingNodes, edges: [] };
    ...
    await expect(page.getByText(/Switching to WebGL/i)).toBeVisible({ timeout: 1000 });
    await expect(page.getByText(/Switching to WebGL/i)).not.toBeVisible({ timeout: 2000 });
  });
  ...
  test('should show transition notification', async ({ page }) => {
    ...
    await page.evaluate(() => {
      const nodes = window.__testGraphData!.nodes;
  ...
  test('should preserve node selection across transition', async ({ page }) => {
    ...
    await page.evaluate(() => {
      window.dispatchEvent(
        new CustomEvent('selectNode', { detail: { nodeId: 'node-0' } })
      );
    });
  ...
  test('should handle rapid threshold crossings', async ({ page }) => {
    ...
    await page.evaluate(() => {
      const nodes = window.__testGraphData!.nodes.slice(0, 9000);
      window.__testGraphData!.nodes = nodes;
      window.dispatchEvent(new CustomEvent('updateGraph'));
    });

    await expect(page.getByText('ReactFlow Mode')).toBeVisible({ timeout: 3000 });
  ...
  test('should show performance mode for large graphs', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      for (let i = 0; i < 60000; i++) {
        nodes.push({
          id: `node-${i}`,
          position: { x: Math.random() * 30000, y: Math.random() * 30000 },
          data: { label: `Node ${i}` },
        });
      }
      window.__testGraphData = { nodes, edges: [] };
    ...
    await expect(page.getByText('WebGL Mode')).toBeVisible({ timeout: 10000 });
  ...
  test('should maintain graph state during transition', async ({ page }) => {
    ...
    await page.evaluate(() => {
      window.dispatchEvent(
        new CustomEvent('applyFilter', { detail: { type: 'requirement' } })
      );
    });
  ...
  test('should open detail panel in WebGL mode on node click', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      for (let i = 0; i < 11000; i++) {
        nodes.push({
          id: `node-${i}`,
          position: { x: Math.random() * 11000, y: Math.random() * 11000 },
          data: { label: `Node ${i}`, description: `Description for node ${i}` },
        });
      }
      window.__testGraphData = { nodes, edges: [] };
    });
    ...
    await page.evaluate(() => {
      window.dispatchEvent(
        new CustomEvent('clickNode', { detail: { node: 'node-0' } })
      );
    });
