...
export function assertWebVitals(
	...
	if (metrics.fcp && metrics.fcp > thresholds["first-contentful-paint"]) {
		failures.push(
			`FCP: ${metrics.fcp.toFixed(0)}ms > ${thresholds["first-contentful-paint"]}ms`,
		);
	}

	if (metrics.lcp && metrics.lcp > thresholds["largest-contentful-paint"]) {
		failures.push(
			`LCP: ${metrics.lcp.toFixed(0)}ms > ${thresholds["largest-contentful-paint"]}ms`,
		);
	}

	if (metrics.cls && metrics.cls > thresholds["cumulative-layout-shift"]) {
		failures.push(
			`CLS: ${metrics.cls.toFixed(3)} > ${thresholds["cumulative-layout-shift"]}`,
		);
	}
...
export async function getResourceMetrics(page: Page) {
	return await page.evaluate(() => {
		...
		for (const resource of resources) {
			...
			else if (resource.name.includes("/api/")) type = "api";
