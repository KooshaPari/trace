 ...
 * Form Helpers
 */
export async function fillProjectForm(page: Page, data: { name: string; description?: string }): Promise<void> {
 ...
 * Filter Helpers
 */
export async function applyFilter(page: Page, filterLabel: string | RegExp, value: string | RegExp): Promise<void> {
...
export async function clearFilters(page: Page): Promise<void> {
  const clearBtn = page.getByRole('button', { name: /clear.*filter|reset.*filter/i })
 ...
 */
export async function createProject(page: Page, data: { name: string; description?: string }): Promise<void> {
...
export async function updateItemStatus(page: Page, itemName: string, newStatus: string): Promise<void> {
  await navigateToItems(page)
  const item = page.getByText(itemName)
  await item.click()
  await page.waitForLoadState('networkidle')
...
export async function waitForText(page: Page, text: string | RegExp, timeout = 5000): Promise<void> {
...
export async function takeElementScreenshot(page: Page, selector: string, name: string): Promise<void> {
  const element = page.locator(selector)
...
export async function getTableCellValue(page: Page, row: number, column: number, tableSelector = 'table'): Promise<string> {
