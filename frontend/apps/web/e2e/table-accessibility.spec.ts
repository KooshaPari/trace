...
test.describe("Table Accessibility - Keyboard Navigation", () => {
	...
	test("should have accessible column headers with sort indicators", async ({
		page,
	}) => {
...
test.describe("Table Accessibility - Screen Reader Support", () => {
	...
	test("should have descriptive labels on action buttons", async ({ page }) => {
		...
		const openButtons = await page
			.locator('button[aria-label*="Open item"]')
			.all();
	...
	test("should label priority information with icons and text", async ({
		page,
	}) => {
	...
	test("should label form inputs in modal", async ({ page }) => {
		...
		const priorityLabel = await page
...
test.describe("Table Accessibility - Focus Management", () => {
	...
	test("should indicate focused element with visible indicator", async ({
		page,
	}) => {
		// Focus an element
