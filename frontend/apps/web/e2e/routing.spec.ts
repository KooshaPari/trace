...
test.describe("Route Navigation - Dynamic Routes", () => {
	...
	test("should navigate to project views with viewType parameter", async ({
		page,
	}) => {
		const projectId = "test-project-123";
		...
		expect(response?.status()).toBeLessThan(500);
		await expect(page).toHaveURL(
			new RegExp(`/projects/${projectId}/views/${viewType}`),
		);
	});

	test("should navigate to project views with viewType and itemId", async ({
		page,
	}) => {
		const projectId = "test-project-123";
		const viewType = "feature";
		...
		expect(response?.status()).toBeLessThan(500);
		await expect(page).toHaveURL(
			new RegExp(`/projects/${projectId}/views/${viewType}/${itemId}`),
		);
	});
	...
	test("should navigate to project views - architecture", async ({ page }) => {
		...
		expect(response?.status()).toBeLessThan(500);
		await expect(page).toHaveURL(
			new RegExp(`/projects/${projectId}/views/architecture`),
		);
	});
	...
	test("should navigate to project views - api", async ({ page }) => {
		...
		expect(response?.status()).toBeLessThan(500);
		await expect(page).toHaveURL(
			new RegExp(`/projects/${projectId}/views/api`),
		);
	});
	...
	test("should navigate to project views - database", async ({ page }) => {
		...
		expect(response?.status()).toBeLessThan(500);
		await expect(page).toHaveURL(
			new RegExp(`/projects/${projectId}/views/database`),
		);
	});
	...
	test("should navigate to feature detail", async ({ page }) => {
		...
		expect(response?.status()).toBeLessThan(500);
		await expect(page).toHaveURL(
			new RegExp(`/projects/${projectId}/features/${featureId}`),
		);
	});
	...
	test("should navigate to ADR detail", async ({ page }) => {
		...
		expect(response?.status()).toBeLessThan(500);
		await expect(page).toHaveURL(
			new RegExp(`/projects/${projectId}/adrs/${adrId}`),
		);
	});
	...
	test("should navigate to project contracts", async ({ page }) => {
		...
		expect(response?.status()).toBeLessThan(500);
		await expect(page).toHaveURL(
			new RegExp(`/projects/${projectId}/contracts`),
		);
	});
	...
	test("should navigate to contract detail", async ({ page }) => {
		...
		expect(response?.status()).toBeLessThan(500);
		await expect(page).toHaveURL(
			new RegExp(`/projects/${projectId}/contracts/${contractId}`),
		);
	});
});
...
test.describe("Route Navigation - Sidebar Navigation", () => {
	...
	test("should navigate via sidebar to items", async ({ page }) => {
		...
		const isVisible = await itemsLink
			.isVisible({ timeout: 2000 })
			.catch(() => false);
	...
	test("should navigate via sidebar to graph view", async ({ page }) => {
		...
		const isVisible = await graphLink
			.isVisible({ timeout: 2000 })
			.catch(() => false);
...
test.describe("Route Navigation - Route Transitions", () => {
	...
	test("should maintain scroll position during navigation", async ({
		page,
	}) => {
		await page.goto("/projects");
		await page.waitForLoadState("networkidle");
