-- Comprehensive hierarchical project data population script V2 FIXED
-- Creates 5 projects with exactly 100+ items of each type
-- Structure: 10 requirements -> 100 features -> 200 tests -> 100 APIs -> 100 databases -> 100 components -> 100 docs -> 100 security -> 100 performance -> 100 deployment = 1000 items per project
...
-- Function to create a project with hierarchical items (ensuring exactly 100+ of each type)
CREATE OR REPLACE FUNCTION create_hierarchical_project_v2_fixed(
    p_name TEXT,
    p_description TEXT,
    p_domain TEXT
) RETURNS uuid AS $$
...
BEGIN
    ...
    FOR i IN 1..10 LOOP
        ...
        -- Store requirement ID for later use (we'll query it)
    END LOOP;
    ...
    FOR i IN 1..10 LOOP
        FOR j IN 1..10 LOOP
            ...
            VALUES (
                ...
                (SELECT id FROM items WHERE project_id = v_project_id AND type = 'requirement' AND title LIKE 'REQ-' || LPAD(i::text, 2, '0') || ':%' ORDER BY created_at LIMIT 1),
                NOW(),
                NOW(),
    ...
    FOR i IN 1..10 LOOP
        FOR j IN 1..10 LOOP
            ...
            VALUES (
                generate_uuid(),
                v_project_id,
                'DB-' || LPAD((i - 1) * 10 + j::text, 3, '0') || ': Database schema ' || j || ' for REQ-' || LPAD(i::text, 2, '0'),
    ...
    WHERE feat.project_id = v_project_id
        ...
        AND EXISTS (
            SELECT 1 FROM items req WHERE req.id = feat.parent_id AND req.id = comp.parent_id
        );
    ...
    WHERE f1.project_id = v_project_id
        ...
        AND f1.id < f2.id
        AND EXISTS (
            SELECT 1 FROM items req1, items req2 
            WHERE req1.id = f1.parent_id AND req2.id = f2.parent_id 
            AND (req1.id = req2.id OR ABS(CAST(SUBSTRING(req1.title FROM 'REQ-(\d+)') AS INTEGER) - CAST(SUBSTRING(req2.title FROM 'REQ-(\d+)') AS INTEGER)) <= 2)
        )
        AND random() < 0.1;
...
-- Delete existing projects if they exist (CASCADE will delete items and links)
DO $$
BEGIN
    ...
    RAISE NOTICE 'Deleted existing projects';
...
BEGIN
    ...
    SELECT create_hierarchical_project_v2_fixed(
        'E-Commerce Platform',
