...
try:
    ...
    raise SystemExit(
        "Missing dependency: markdown. Install via `pip install pheno-sdk[docs]` "
...
<body>
  ...
  <footer>
    Generated by scripts/build_docsets.py on {dt.datetime.now(dt.timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}
  </footer>
</body>
"""
...
def write_index(docset: Docset, manifest: list[dict], destination: Path) -> None:
    ...
    for group_name, entries in sorted(grouped.items()):
        ...
        links = "\n".join(
            f'<li><a href="{entry["relative_html"]}">{entry["title"]}</a></li>' for entry in sorted(entries, key=lambda e: e["relative_html"])
        )
