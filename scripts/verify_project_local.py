    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            result = session.execute(
                storage.engine.dialect.statement_compiler(
                    storage.engine.dialect, None
                ).preparer.quote(
            ...
            checks_total += 1
            if check(
                result.fetchone() is not None, "project_registry table exists"
            ):
                checks_passed += 1
        except Exception:
            checks_total += 1
        ...
        try:
            trace_dir, project_id = storage.init_project(
                project_path, project_name="Test Project"
            )
            ...
            checks_total += 1
            if check(
                (trace_dir / "project.yaml").exists(), "project.yaml created"
            ):
                checks_passed += 1
            ...
            for subdir in ["epics", "stories", "tests", "tasks", "docs", "changes"]:
                checks_total += 1
                if check(
                    (trace_dir / subdir).is_dir(), f"{subdir}/ directory created"
                ):
                    checks_passed += 1
        ...
        checks_total += 1
        if check(
            project_config.get("name") == "Test Project", "Project name correct"
        ):
            checks_passed += 1
        ...
        checks_total += 1
        if check(
            project_config.get("counters", {}).get("epic") == 0, "Epic counter is 0"
        ):
            checks_passed += 1
        ...
        checks_total += 1
        if check(
            storage.is_trace_project(project_path), "is_trace_project() works"
        ):
            checks_passed += 1
        ...
        checks_total += 1
        if check(
            project_storage is not None, "get_project_storage_for_path() works"
        ):
            checks_passed += 1
        ...
        checks_total += 1
        if check(
            "EPIC-002" in epic_content, "Epic markdown has external ID"
        ):
            checks_passed += 1
        ...
        print("\n8. Search Functionality")
        try:
            search_results = storage.search_items("Test", project_id=project.id)
            checks_total += 1
