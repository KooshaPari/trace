...
ITEM_TITLES = {
    "requirement": ["User authentication", "Payment processing", "Data encryption", "API rate limiting", "Real-time notifications"],
    ...
    "api": ["GET /api/v1/users", "POST /api/v1/orders", "PUT /api/v1/products", "DELETE /api/v1/cart", "PATCH /api/v1/profile"],
...
def create_items(session: Session, project_id: str, count: int):
    ...
    for i in range(1, count + 1):
        ...
        if item_type == "code":
            title = ITEM_TITLES[item_type](i)
        else:
            title_base = choice(ITEM_TITLES[item_type])
...
def create_links(session: Session, project_id: str, item_ids: list, count: int):
...
def main():
    ...
    try:
        for config in PROJECTS:
            ...
            # Create items
            item_ids = create_items(session, project_id, config["item_count"])
            
            # Create links
            create_links(session, project_id, item_ids, config["link_count"])
