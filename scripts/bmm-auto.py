...
class BMMAutomation:
    ...
    def _load_workflow_status(self) -> Optional[Dict[str, Any]]:
        """Load workflow status file"""
        if not self.workflow_status_file.exists():
            return None
        with open(self.workflow_status_file) as f:
            return yaml.safe_load(f)
    ...
    def check_initialization(self) -> bool:
        ...
        return (self.bmad_folder.exists() and
                self.workflow_status_file.exists() and
    ...
    async def run_init(self) -> bool:
        ...
        success = await self._run_workflow(
            agent="bmad-master",
            workflow_command="/bmad:bmm:workflows:workflow-status:init",
            interactive=True
        )
    ...
    def _parse_workflow_tasks(self) -> List[WorkflowTask]:
        ...
        for phase_key, phase_data in workflow_data.items():
...
"""
    ...
    async def _run_auggie_workflow(
        ...
        if interactive:
            ...
            cmd.extend([
                "--workspace-root", str(self.project_root),
            ...
            cmd.extend([
                "--print",
                "--output-format", "json" if structured else "text",
                "--workspace-root", str(self.project_root),
                "--instruction", prompt
            ])
    ...
    async def _run_claude_workflow(
        ...
        if interactive:
            ...
            cmd.extend([
                "--print",
                "--output-format", "stream-json" if structured else "text",
                prompt
            ])
    ...
    async def _run_parallel_workflows(self, tasks: List[WorkflowTask], interactive: bool):
        ...
        agent_coroutines = [
            run_agent_tasks(agent, agent_tasks)
...
Examples:
  ...
  %(prog)s run --auto
    ...
    parser.add_argument(
        'workflows',
        nargs='*',
        help='Specific workflow IDs to run (for run command)'
    )
    ...
    parser.add_argument(
        '--cli',
        choices=['auto', 'auggie', 'claude'],
        default='auto',
        help='CLI tool to use (default: auto-detect)'
    )
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=True,
        help='Run workflows interactively (default)'
    )

    parser.add_argument(
        '--auto',
        action='store_true',
        help='Run workflows automatically without user interaction'
    )

    parser.add_argument(
        '--parallel',
        action='store_true',
        help='Run compatible workflows in parallel'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        choices=[0, 1, 2, 3],
        help='Run all workflows in a specific phase'
    )
    ...
    if args.command == 'init':
        ...
        asyncio.run(automation.run_workflow_sequence(
            workflow_ids=args.workflows if args.workflows else None,
