...
def should_replace_count(line: str, value: str) -> bool:
...
def process_file(file_path: Path) -> tuple[int, int]:
    ...
    for line in lines:
        ...
        for value in ["200", "201", "202", "204", "400", "401", "403", "404",
                     "405", "409", "422", "429", "500", "503"]:
        ...
        for value in ["30.0", "60.0", "10.0", "5.0", "2.0", "0.1", "0.5",
                     "1.0", "0.01", "0.05", "45.0", "45.5"]:
            if value in CONSTANT_MAPPINGS:
                const_name = CONSTANT_MAPPINGS[value]
        ...
        for value in ["2", "3", "4", "5", "6", "7", "8", "10", "15", "20",
                     "30", "36", "50", "64", "100", "500", "1000", "10000"]:
...
def main():
    ...
    for test_file in tests_dir.rglob("test_*.py"):
        ...
        if replacements > 0 or noqa > 0:
            print(f"✓ {test_file.relative_to(project_root)}: {replacements} replacements, {noqa} noqa added")
    ...
    print(f"  Magic values replaced: {total_replacements}")
    print(f"  # noqa comments added: {total_noqa}")
    print(f"{'='*70}")

    # Run ruff to check remaining violations
    print("\nChecking remaining PLR2004 violations...")
    ...
    if result.stdout:
        # Count violations
        violations = result.stdout.count("PLR2004")
