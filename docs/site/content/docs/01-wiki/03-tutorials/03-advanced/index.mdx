...
Comprehensive guide for Advanced Tutorials.

## Introduction

This guide covers everything you need to know about Advanced Tutorials.
...
```bash
tracertm init
tracertm config set advanced enabled
```
...
```bash
# Step 1: Initialize
tracertm advanced init

# Step 2: Execute
tracertm advanced run

# Step 3: Verify
tracertm advanced status
```
...
async function example() {
  const tracertm = new TraceRTM()
  const result = await tracertm.advanced()
  console.log(result)
}
```
