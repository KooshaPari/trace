...
Comprehensive guide for Settings.

## Introduction

This guide covers everything you need to know about Settings.
...
```bash
tracertm init
tracertm config set settings enabled
```
...
```bash
# Step 1: Initialize
tracertm settings init

# Step 2: Execute
tracertm settings run

# Step 3: Verify
tracertm settings status
```
...
async function example() {
  const tracertm = new TraceRTM()
  const result = await tracertm.settings()
  console.log(result)
}
```
