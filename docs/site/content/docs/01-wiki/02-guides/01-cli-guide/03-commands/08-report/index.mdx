...
description: "Documentation for report Command"
---
...
Comprehensive guide for report Command.

## Introduction

This guide covers everything you need to know about report Command.
...
```bash
tracertm init
tracertm config set report enabled
```
...
```bash
# Step 1: Initialize
tracertm report init

# Step 2: Execute
tracertm report run

# Step 3: Verify
tracertm report status
```
...
async function example() {
  const tracertm = new TraceRTM()
  const result = await tracertm.report()
  console.log(result)
}
```
