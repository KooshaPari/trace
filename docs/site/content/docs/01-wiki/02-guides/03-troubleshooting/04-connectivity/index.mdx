...
description: "Documentation for Connectivity Problems"
---
...
Comprehensive guide for Connectivity Problems.

## Introduction

This guide covers everything you need to know about Connectivity Problems.
...
```bash
tracertm init
tracertm config set connectivity enabled
```
...
```bash
# Step 1: Initialize
tracertm connectivity init

# Step 2: Execute
tracertm connectivity run

# Step 3: Verify
tracertm connectivity status
```
...
**Solution:**
```bash
tracertm optimize connectivity
```
...
async function example() {
  const tracertm = new TraceRTM()
  const result = await tracertm.connectivity()
  console.log(result)
}
```
