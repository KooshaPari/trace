...
- No changes needed - no hardcoded route references
...
1. `/frontend/apps/web/src/config/constants.ts` - Added AUTH_ROUTES
2. `/frontend/apps/web/src/components/auth/AuthKitSync.tsx` - Use AUTH_ROUTES.CALLBACK
3. `/frontend/apps/web/src/lib/auth-utils.ts` - Use AUTH_ROUTES in isPublicRoute()
4. `/frontend/apps/web/src/routes/auth.callback.tsx` - Use AUTH_ROUTES.LOGIN
5. `/frontend/apps/web/src/routes/auth.logout.tsx` - Use AUTH_ROUTES.LOGIN
6. `/frontend/apps/web/src/routes/auth.register.tsx` - Use AUTH_ROUTES.LOGIN
...
- ✅ TypeScript types preserved (using `as const`)
...
6. **DRY Principle**: No duplicate route strings scattered across codebase
...
3. **Logout Flow**:
   - When authenticated, navigate to `/auth/logout`
...
if (
  ...
  // getReturnTo already filters out auth routes and returns "/" as default
  window.location.href = returnTo;
}
```
...
- Removed redundant auth route checks (getReturnTo handles this)
- Used centralized constants instead of hardcoded strings
- Clearer intent with isPublicRoute() utility function
...
- The `createFileRoute()` calls still use hardcoded strings (required by TanStack Router)
- The `isAuthRoute()` helper uses `startsWith("/auth/")` for general auth route detection
