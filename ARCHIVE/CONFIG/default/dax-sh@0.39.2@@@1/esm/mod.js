  ...
  try {
    for await (const entry of Deno2.readDir(root)) {
      ...
      if (isSymlink) {
        ...
        ({ isSymlink, isDirectory: isDirectory2 } = await Deno2.lstat(realPath));
      }
...
function instantiateWithInstance() {
  if (instanceWithExports == null) {
    ...
    instanceWithExports = {
      instance,
      exports: { parse: parse4, static_text_render_text, static_text_clear_text, static_text_render_once, strip_ansi_codes }
    };
  }
...
function render(state) {
  return [
    colors_exports.bold(colors_exports.blue(state.title)) + " " + (state.hasCompleted ? "" : state.default == null ? "(Y/N) " : state.default ? "(Y/n) " : "(y/N) ") + state.inputText + (state.hasCompleted ? "" : "\u2588")
    // (block character)
  ];
}
...
function renderProgressBar(state, size) {
  if (state.hasCompleted) {
    ...
    let text = colors_exports.green(tickStrings[Math.abs(state.tickCount) % tickStrings.length]);
    if (state.prefix != null) {
      text += ` ${colors_exports.green(state.prefix)}`;
    }
    if (state.message != null) {
...
function render3(state) {
  ...
  return [
    colors_exports.bold(colors_exports.blue(state.title)) + " " + inputText + (state.hasCompleted ? "" : "\u2588")
    // (block character)
  ];
}
...
function resolvePath(cwd, arg) {
  return mod_exports4.resolve(mod_exports4.isAbsolute(arg) ? arg : mod_exports4.join(cwd, arg));
}
var Box = class {
  value;
  constructor(value) {
...
async function getCopyAndMoveOperations(cwd, paths) {
  ...
  if (fromArgs.length > 1) {
    ...
    for (const from of fromArgs) {
      ...
      const toPath = mod_exports4.join(destination, mod_exports4.basename(fromPath));
      operations.push({
        from: {
          specified: from,
          path: fromPath
...
async function resolveRedirectPipe(redirect, context) {
  ...
  if (ioFile.kind === "fd") {
    ...
    switch (redirect.op.kind) {
      case "input": {
        const outputPath = mod_exports4.isAbsolute(words[0]) ? words[0] : mod_exports4.join(context.getCwd(), words[0]);
        try {
...
var CommandBuilder = class {
  #state = {
    ...
    printCommandLogger: new LoggerTreeBox(
      ...
      (cmd) => console.error(colors_exports.white(">"), colors_exports.blue(cmd))
    ),
    timeout: void 0,
    signal: void 0
  };
  ...
  cwd(dirPath) {
    return this.#newWithState((state) => {
      state.cwd = dirPath instanceof URL ? mod_exports4.fromFileUrl(dirPath) : dirPath instanceof Path ? dirPath.resolve().toString() : mod_exports4.resolve(dirPath);
    });
  }
  /**
...
function build$FromState(state) {
  ...
  const result = Object.assign((strings, ...exprs) => {
    ...
    logStep(firstArg, ...data) {
      logStep(firstArg, data, (t) => colors_exports.bold(colors_exports.green(t)), state.infoLogger.getValue());
    },
    logError(firstArg, ...data) {
      logStep(firstArg, data, (t) => colors_exports.bold(colors_exports.red(t)), state.errorLogger.getValue());
    },
    logWarn(firstArg, ...data) {
      logStep(firstArg, data, (t) => colors_exports.bold(colors_exports.yellow(t)), state.warnLogger.getValue());
    },
    ...
    setErrorLogger(logger2) {
      ...
      commandBuilder.setPrintCommandLogger((cmd) => logger2(colors_exports.white(">"), colors_exports.blue(cmd)));
