...
description: "Documentation for Error Messages"
---
...
Comprehensive guide for Error Messages.

## Introduction

This guide covers everything you need to know about Error Messages.
...
```bash
tracertm init
tracertm config set error.messages enabled
```
...
```bash
# Step 1: Initialize
tracertm error-messages init

# Step 2: Execute
tracertm error-messages run

# Step 3: Verify
tracertm error-messages status
```
...
**Solution:**
```bash
tracertm optimize error-messages
```
...
async function example() {
  const tracertm = new TraceRTM()
  const result = await tracertm.errormessages()
  console.log(result)
}
```
