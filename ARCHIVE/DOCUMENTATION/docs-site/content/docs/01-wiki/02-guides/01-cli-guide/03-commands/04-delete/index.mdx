...
description: "Documentation for delete Command"
---
...
Comprehensive guide for delete Command.

## Introduction

This guide covers everything you need to know about delete Command.
...
```bash
tracertm init
tracertm config set delete enabled
```
...
```bash
# Step 1: Initialize
tracertm delete init

# Step 2: Execute
tracertm delete run

# Step 3: Verify
tracertm delete status
```
...
async function example() {
  const tracertm = new TraceRTM()
  const result = await tracertm.delete()
  console.log(result)
}
```
