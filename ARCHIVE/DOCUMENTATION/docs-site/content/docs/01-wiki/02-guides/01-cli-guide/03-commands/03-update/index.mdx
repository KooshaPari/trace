...
description: "Documentation for update Command"
---
...
Comprehensive guide for update Command.

## Introduction

This guide covers everything you need to know about update Command.
...
```bash
tracertm init
tracertm config set update enabled
```
...
```bash
# Step 1: Initialize
tracertm update init

# Step 2: Execute
tracertm update run

# Step 3: Verify
tracertm update status
```
...
async function example() {
  const tracertm = new TraceRTM()
  const result = await tracertm.update()
  console.log(result)
}
```
