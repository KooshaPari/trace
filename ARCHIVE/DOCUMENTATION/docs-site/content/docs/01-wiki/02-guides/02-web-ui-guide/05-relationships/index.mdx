...
description: "Documentation for Relationship Views"
---
...
Comprehensive guide for Relationship Views.

## Introduction

This guide covers everything you need to know about Relationship Views.
...
```bash
tracertm init
tracertm config set relationships enabled
```
...
```bash
# Step 1: Initialize
tracertm relationships init

# Step 2: Execute
tracertm relationships run

# Step 3: Verify
tracertm relationships status
```
...
**Solution:**
```bash
tracertm optimize relationships
```
...
async function example() {
  const tracertm = new TraceRTM()
  const result = await tracertm.relationships()
  console.log(result)
}
```
