...
description: "Documentation for Managing Artifacts"
---
...
Comprehensive guide for Managing Artifacts.

## Introduction

This guide covers everything you need to know about Managing Artifacts.
...
```bash
tracertm init
tracertm config set artifacts enabled
```
...
```bash
# Step 1: Initialize
tracertm artifacts init

# Step 2: Execute
tracertm artifacts run

# Step 3: Verify
tracertm artifacts status
```
...
async function example() {
  const tracertm = new TraceRTM()
  const result = await tracertm.artifacts()
  console.log(result)
}
```
