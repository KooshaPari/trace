...
This page provides comprehensive information about Data Model.
...
```bash
tracertm init
tracertm config set data.model enabled
```
...
const result = await tracertm.datamodel()
```
...
```typescript
const example = await tracertm.datamodel({
  option1: 'value1',
  option2: 'value2'
})
```
...
```typescript
const advanced = await tracertm.datamodel({
  advanced: {
    feature1: true,
    feature2: false
  }
