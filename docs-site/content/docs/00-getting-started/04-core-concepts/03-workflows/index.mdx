...
description: "Documentation for Workflow Concepts"
---
...
Welcome to the Workflow Concepts documentation.

## Introduction

This page provides comprehensive information about Workflow Concepts.
...
```bash
tracertm init
tracertm config set workflows enabled
```
...
const result = await tracertm.workflows()
```
...
```typescript
const example = await tracertm.workflows({
  option1: 'value1',
  option2: 'value2'
})
```
...
```typescript
const advanced = await tracertm.workflows({
  advanced: {
    feature1: true,
    feature2: false
  }
