...
description: "Documentation for Permissions Model"
---
...
Welcome to the Permissions Model documentation.

## Introduction

This page provides comprehensive information about Permissions Model.
...
```bash
tracertm init
tracertm config set permissions enabled
```
...
const result = await tracertm.permissions()
```
...
```typescript
const example = await tracertm.permissions({
  option1: 'value1',
  option2: 'value2'
})
```
...
```typescript
const advanced = await tracertm.permissions({
  advanced: {
    feature1: true,
    feature2: false
  }
