...
description: "Documentation for Creating Requirements"
---
...
Welcome to the Creating Requirements documentation.

## Introduction

This page provides comprehensive information about Creating Requirements.
...
```bash
tracertm init
tracertm config set requirements enabled
```
...
const result = await tracertm.requirements()
```
...
```typescript
const example = await tracertm.requirements({
  option1: 'value1',
  option2: 'value2'
})
```
...
```typescript
const advanced = await tracertm.requirements({
  advanced: {
    feature1: true,
    feature2: false
  }
