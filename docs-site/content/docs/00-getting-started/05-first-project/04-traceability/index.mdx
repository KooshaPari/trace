...
description: "Documentation for Establishing Traceability"
---
...
Welcome to the Establishing Traceability documentation.

## Introduction

This page provides comprehensive information about Establishing Traceability.
...
```bash
tracertm init
tracertm config set traceability enabled
```
...
const result = await tracertm.traceability()
```
...
```typescript
const example = await tracertm.traceability({
  option1: 'value1',
  option2: 'value2'
})
```
...
```typescript
const advanced = await tracertm.traceability({
  advanced: {
    feature1: true,
    feature2: false
  }
