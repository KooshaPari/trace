...
description: "Documentation for Test Management"
---
...
Welcome to the Test Management documentation.

## Introduction

This page provides comprehensive information about Test Management.
...
```bash
tracertm init
tracertm config set test.management enabled
```
...
const result = await tracertm.testmanagement()
```
...
```typescript
const example = await tracertm.testmanagement({
  option1: 'value1',
  option2: 'value2'
})
```
...
```typescript
const advanced = await tracertm.testmanagement({
  advanced: {
    feature1: true,
    feature2: false
  }
