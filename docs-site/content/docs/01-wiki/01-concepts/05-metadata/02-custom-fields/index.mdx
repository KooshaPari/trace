...
Welcome to the Custom Fields documentation.

## Introduction

This page provides comprehensive information about Custom Fields.
...
```bash
tracertm init
tracertm config set custom.fields enabled
```
...
const result = await tracertm.customfields()
```
...
```typescript
const example = await tracertm.customfields({
  option1: 'value1',
  option2: 'value2'
})
```
...
```typescript
const advanced = await tracertm.customfields({
  advanced: {
    feature1: true,
    feature2: false
  }
