...
description: "Documentation for Microsoft Teams"
---
...
Welcome to the Microsoft Teams documentation.

## Introduction

This page provides comprehensive information about Microsoft Teams.
...
```bash
tracertm init
tracertm config set teams enabled
```
...
const result = await tracertm.teams()
```
...
```typescript
const example = await tracertm.teams({
  option1: 'value1',
  option2: 'value2'
})
```
...
```typescript
const advanced = await tracertm.teams({
  advanced: {
    feature1: true,
    feature2: false
  }
