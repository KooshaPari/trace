...
description: "Documentation for Change History"
---
...
Welcome to the Change History documentation.

## Introduction

This page provides comprehensive information about Change History.
...
```bash
tracertm init
tracertm config set history enabled
```
...
const result = await tracertm.history()
```
...
```typescript
const example = await tracertm.history({
  option1: 'value1',
  option2: 'value2'
})
```
...
```typescript
const advanced = await tracertm.history({
  advanced: {
    feature1: true,
    feature2: false
  }
