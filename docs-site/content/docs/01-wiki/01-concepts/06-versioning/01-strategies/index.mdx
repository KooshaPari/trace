...
description: "Documentation for Versioning Strategies"
---
...
Welcome to the Versioning Strategies documentation.

## Introduction

This page provides comprehensive information about Versioning Strategies.
...
```bash
tracertm init
tracertm config set strategies enabled
```
...
const result = await tracertm.strategies()
```
...
```typescript
const example = await tracertm.strategies({
  option1: 'value1',
  option2: 'value2'
})
```
...
```typescript
const advanced = await tracertm.strategies({
  advanced: {
    feature1: true,
    feature2: false
  }
