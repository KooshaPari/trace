...
description: "Documentation for Rollback Procedures"
---
...
Welcome to the Rollback Procedures documentation.

## Introduction

This page provides comprehensive information about Rollback Procedures.
...
```bash
tracertm init
tracertm config set rollback enabled
```
...
const result = await tracertm.rollback()
```
...
```typescript
const example = await tracertm.rollback({
  option1: 'value1',
  option2: 'value2'
})
```
...
```typescript
const advanced = await tracertm.rollback({
  advanced: {
    feature1: true,
    feature2: false
  }
