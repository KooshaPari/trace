...
description: "Documentation for Relationship Mapping"
---
...
Welcome to the Relationship Mapping documentation.

## Introduction

This page provides comprehensive information about Relationship Mapping.
...
```bash
tracertm init
tracertm config set mapping enabled
```
...
const result = await tracertm.mapping()
```
...
```typescript
const example = await tracertm.mapping({
  option1: 'value1',
  option2: 'value2'
})
```
...
```typescript
const advanced = await tracertm.mapping({
  advanced: {
    feature1: true,
    feature2: false
  }
