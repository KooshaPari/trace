...
description: "Documentation for Design Documents"
---
...
Welcome to the Design Documents documentation.

## Introduction

This page provides comprehensive information about Design Documents.
...
```bash
tracertm init
tracertm config set design enabled
```
...
const result = await tracertm.design()
```
...
```typescript
const example = await tracertm.design({
  option1: 'value1',
  option2: 'value2'
})
```
...
```typescript
const advanced = await tracertm.design({
  advanced: {
    feature1: true,
    feature2: false
  }
