...
description: "Documentation for Code Artifacts"
---
...
Welcome to the Code Artifacts documentation.

## Introduction

This page provides comprehensive information about Code Artifacts.
...
```bash
tracertm init
tracertm config set code enabled
```
...
const result = await tracertm.code()
```
...
```typescript
const example = await tracertm.code({
  option1: 'value1',
  option2: 'value2'
})
```
...
```typescript
const advanced = await tracertm.code({
  advanced: {
    feature1: true,
    feature2: false
  }
