...
description: "Documentation for Building Custom Plugins"
---
...
Comprehensive guide for Building Custom Plugins.

## Introduction

This guide covers everything you need to know about Building Custom Plugins.
...
```bash
tracertm init
tracertm config set custom.plugins enabled
```
...
```bash
# Step 1: Initialize
tracertm custom-plugins init

# Step 2: Execute
tracertm custom-plugins run

# Step 3: Verify
tracertm custom-plugins status
```
...
**Solution:**
```bash
tracertm optimize custom-plugins
```
...
async function example() {
  const tracertm = new TraceRTM()
  const result = await tracertm.customplugins()
  console.log(result)
}
```
