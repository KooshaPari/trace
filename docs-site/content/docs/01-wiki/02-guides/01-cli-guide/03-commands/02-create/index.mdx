...
description: "Documentation for create Command"
---
...
Comprehensive guide for create Command.

## Introduction

This guide covers everything you need to know about create Command.
...
```bash
tracertm init
tracertm config set create enabled
```
...
```bash
# Step 1: Initialize
tracertm create init

# Step 2: Execute
tracertm create run

# Step 3: Verify
tracertm create status
```
...
async function example() {
  const tracertm = new TraceRTM()
  const result = await tracertm.create()
  console.log(result)
}
```
