...
description: "Documentation for import Command"
---
...
Comprehensive guide for import Command.

## Introduction

This guide covers everything you need to know about import Command.
...
```bash
tracertm init
tracertm config set import enabled
```
...
```bash
# Step 1: Initialize
tracertm import init

# Step 2: Execute
tracertm import run

# Step 3: Verify
tracertm import status
```
...
async function example() {
  const tracertm = new TraceRTM()
  const result = await tracertm.import()
  console.log(result)
}
```
