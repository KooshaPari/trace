---
title: "sync Command"
description: "Documentation for sync Command"
---
...
Comprehensive guide for sync Command.

## Introduction

This guide covers everything you need to know about sync Command.
...
```bash
tracertm init
tracertm config set sync enabled
```
...
async function example() {
  const tracertm = new TraceRTM()
  const result = await tracertm.sync()
  console.log(result)
}
```
