---
title: "CLI Plugins"
description: "Documentation for CLI Plugins"
---
...
Comprehensive guide for CLI Plugins.

## Introduction

This guide covers everything you need to know about CLI Plugins.
...
```bash
tracertm init
tracertm config set plugins enabled
```
...
```bash
# Step 1: Initialize
tracertm plugins init

# Step 2: Execute
tracertm plugins run

# Step 3: Verify
tracertm plugins status
```
...
async function example() {
  const tracertm = new TraceRTM()
  const result = await tracertm.plugins()
  console.log(result)
}
```
