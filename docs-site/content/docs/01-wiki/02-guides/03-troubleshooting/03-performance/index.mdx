...
This guide covers everything you need to know about Performance Issues.
...
```bash
tracertm init
tracertm config set performance enabled
```
...
```bash
# Step 1: Initialize
tracertm performance init

# Step 2: Execute
tracertm performance run

# Step 3: Verify
tracertm performance status
```
...
async function example() {
  const tracertm = new TraceRTM()
  const result = await tracertm.performance()
  console.log(result)
}
```
