...
This page provides comprehensive information about Data Flow.
...
```bash
tracertm init
tracertm config set data.flow enabled
```
...
const result = await tracertm.dataflow()
```
...
```typescript
const example = await tracertm.dataflow({
  option1: 'value1',
  option2: 'value2'
})
```
...
```typescript
const advanced = await tracertm.dataflow({
  advanced: {
    feature1: true,
    feature2: false
  }
