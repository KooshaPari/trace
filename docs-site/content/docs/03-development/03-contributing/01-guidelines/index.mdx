...
description: "Documentation for Contribution Guidelines"
---
...
Comprehensive guide for Contribution Guidelines.

## Introduction

This guide covers everything you need to know about Contribution Guidelines.
...
```bash
tracertm init
tracertm config set guidelines enabled
```
...
```bash
# Step 1: Initialize
tracertm guidelines init

# Step 2: Execute
tracertm guidelines run

# Step 3: Verify
tracertm guidelines status
```
...
async function example() {
  const tracertm = new TraceRTM()
  const result = await tracertm.guidelines()
  console.log(result)
}
```
