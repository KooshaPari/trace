...
**Planner agents** (PM, Analyst, Architect, SM, TEA, UX Designer, Tech Writer, BMad Master) must **never write code**, especially in documentation and plans. Their job is to **equip others**—engineer agents (e.g. DEV) or humans—to create code or act on docs. They write specs, acceptance criteria, architecture decisions, and clear handoffs; they do not write implementation. Avoid stuffing docs or plans with code where it is not directly relevant; prefer references, file paths, or brief pseudocode when necessary.
