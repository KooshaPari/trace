...
def extract_learnings(session_data):
    ...
    blockers_resolved = [m for m in messages if 'blocker' in m.lower() and ('resolved' in m.lower() or 'fixed' in m.lower())]
...
def format_retrospective(learnings, topic):
...
def update_memory_files(retro, timestamp, topic):
    ...
    if failures_file.exists():
        ...
        content = "# Failures & Anti-Patterns\n\n"
    ...
    print(f"📝 Session learning saved: {retro_file}", file=sys.stderr)
    print(f"📝 Failures updated: {failures_file}", file=sys.stderr)
...
def main():
    ...
    try:
        ...
        print(f"\n✅ Session learnings captured for: {topic}", file=sys.stderr)

        sys.exit(0)

    except Exception as e:
        print(f"❌ Session learning hook failed: {e}", file=sys.stderr)
