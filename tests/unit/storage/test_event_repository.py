...
class TestEventRepository:
    ...
    def storage_instance(self, mock_db_session):
        ...
        try:
            import importlib
            module_name = f"tracertm.storage.event_repository" if "repository" not in storage_name else f"tracertm.repositories.event_repository"
            try:
                module = importlib.import_module(module_name)
            except ImportError:
                # Try alternative location
                module_name = f"tracertm.database.event_repository"
                module = importlib.import_module(module_name)
...
class TestEventRepositoryErrorHandling:
    """Test error handling."""
