...
async def test_get_by_project(db_session: AsyncSession):
    ...
    events = await event_repo.get_by_project(project1.id, limit=10)
    assert len(events) == 2
...
async def test_get_by_project_with_limit(db_session: AsyncSession):
    ...
    events = await event_repo.get_by_project(project.id, limit=5)
    assert len(events) <= 5
...
async def test_get_entity_at_time(db_session: AsyncSession):
    ...
    state = await event_repo.get_entity_at_time("item-1", datetime.utcnow())
...
async def test_get_entity_at_time_nonexistent(db_session: AsyncSession):
    ...
    state = await event_repo.get_entity_at_time("nonexistent", datetime.utcnow())
