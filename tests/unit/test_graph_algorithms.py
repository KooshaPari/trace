...
class TestImpactAnalysisAlgorithms:
    ...
    def test_propagate_changes_with_cycle(self):
        ...
        def propagate(start, graph, max_depth=10):
            visited = set()
            queue = [(start, 0)]
            while queue:
    ...
    def test_calculate_impact_depth(self):
        ...
        def calculate_depth(start, graph):
            depths = {start: 0}
            queue = [start]
            while queue:
                node = queue.pop(0)
    ...
    def test_find_critical_paths(self):
        ...
        def find_paths(start, end, graph, path=None):
            if path is None:
                path = []
            path = [*path, start]
            if start == end:
...
class TestAPIEndpointCoverage:
    ...
    def test_item_list_pagination(self):
        ...
        def paginate(items, page, limit=10):
            start = (page - 1) * limit
            end = start + limit
