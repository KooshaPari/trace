...
@pytest.fixture
def concurrent_session(concurrent_test_db):
...
@pytest.fixture
def concurrent_initialized_db(concurrent_test_db):
...
@pytest.fixture
def concurrent_session_from_initialized(concurrent_initialized_db):
...
class TestConcurrencyReport:
    ...
    def test_throughput_measurement(self, concurrent_initialized_db, num_threads, operations):
        ...
        elapsed = time.time() - start_time
        throughput = (operations * num_threads) / elapsed if elapsed > 0 else 0
