...
@pytest.fixture(scope="function")
def sample_items_10(db_session, sample_project):
...
@pytest.fixture(scope="function")
def sample_items_50(db_session, sample_project):
...
class TestExtendedLinkRetrieval:
    ...
    def test_get_links_with_pagination(self, db_session, sample_project, sample_items_10):
        ...
        # Test pagination
        page_size = 3
        total_links = db_session.query(Link).filter_by(project_id=sample_project.id).count()
...
class TestGraphOperations:
    ...
    def test_get_transitive_closure_linear(self, db_session, sample_project, sample_items_10):
        ...
        def get_reachable(start_id, graph):
            visited = set()
            queue = [start_id]
            while queue:
                node = queue.pop(0)
...
class TestLinkIntegrity:
    ...
    def test_link_source_target_not_null(self, db_session, sample_project, sample_items_10):
        ...
        try:
            ...
            assert False, "Should not allow null source_item_id"
        except:
            db_session.rollback()
    ...
    def test_link_project_id_required(self, db_session, sample_items_10):
        ...
        try:
            ...
            db_session.add(link)
            db_session.commit()
            assert False, "Should not allow null project_id"
        except:
            db_session.rollback()
