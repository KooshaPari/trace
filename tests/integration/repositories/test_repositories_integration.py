...
async def test_item_repository_get_by_id_with_project_scope(db_session: AsyncSession):
    ...
    found = await item_repo.get_by_id(item1.id, project_id=project1.id)
    assert found is not None

    # Query with wrong project
    not_found = await item_repo.get_by_id(item1.id, project_id=project2.id)
    assert not_found is None
...
async def test_item_repository_list_by_view(db_session: AsyncSession):
    ...
    stories = await item_repo.list_by_view(project.id, "STORY")
...
async def test_item_repository_list_by_view_include_deleted(db_session: AsyncSession):
    ...
    # Without deleted
    active_only = await item_repo.list_by_view(project.id, "FEATURE", include_deleted=False)
    assert len(active_only) == 1
    assert active_only[0].id == item1.id

    # With deleted
    all_items = await item_repo.list_by_view(project.id, "FEATURE", include_deleted=True)
...
async def test_item_repository_update_concurrency_error(db_session: AsyncSession):
    ...
    # First update
    await item_repo.update(item.id, expected_version=1, status="in_progress")
    await db_session.commit()
...
async def test_item_repository_hard_delete(db_session: AsyncSession):
    ...
    result = await item_repo.delete(item_id, soft=False)
    assert result is True
    await db_session.commit()

    # Should not exist at all
    found = await item_repo.get_by_id(item_id)
    assert found is None
...
async def test_item_repository_get_by_project(db_session: AsyncSession):
    ...
    p1_items = await item_repo.get_by_project(project1.id)
    assert len(p1_items) == 3

    p2_items = await item_repo.get_by_project(project2.id)
...
async def test_item_repository_get_by_project_pagination(db_session: AsyncSession):
    ...
    # First page
    page1 = await item_repo.get_by_project(project.id, limit=5, offset=0)
    assert len(page1) == 5

    # Second page
    page2 = await item_repo.get_by_project(project.id, limit=5, offset=5)
    assert len(page2) == 5

    # Third page
    page3 = await item_repo.get_by_project(project.id, limit=5, offset=10)
    assert len(page3) == 5
...
async def test_item_repository_get_by_view_pagination(db_session: AsyncSession):
    ...
    page1 = await item_repo.get_by_view(project.id, "FEATURE", limit=3, offset=0)
    assert len(page1) == 3

    page2 = await item_repo.get_by_view(project.id, "FEATURE", limit=3, offset=3)
    assert len(page2) == 3
...
async def test_item_repository_query_dynamic_filters(db_session: AsyncSession):
    ...
    # Filter by owner
    alice_items = await item_repo.query(
        project.id,
        filters={"owner": "alice"}
    )
    ...
    # Multiple filters
    specific = await item_repo.query(
        project.id,
        filters={"priority": "high", "owner": "alice"}
    )
...
async def test_link_repository_get_by_project(db_session: AsyncSession):
    ...
    p1_item2 = await item_repo.create(
        project_id=project1.id,
        title="P1 Item 2",
        view="API",
        item_type="api"
    ...
    p2_item2 = await item_repo.create(
        project_id=project2.id,
        title="P2 Item 2",
        view="API",
        item_type="api"
    ...
    await link_repo.create(
        project_id=project1.id,
        source_item_id=p1_item1.id,
        target_item_id=p1_item2.id,
        link_type="implements"
    )
    await link_repo.create(
        project_id=project2.id,
        source_item_id=p2_item1.id,
        target_item_id=p2_item2.id,
        link_type="tests"
    )
    await db_session.commit()

    p1_links = await link_repo.get_by_project(project1.id)
    assert len(p1_links) == 1
    assert p1_links[0].project_id == project1.id

    p2_links = await link_repo.get_by_project(project2.id)
...
async def test_link_repository_get_by_item(db_session: AsyncSession):
    ...
    links = await link_repo.get_by_item(item2.id)
    assert len(links) == 2
...
async def test_link_repository_delete_by_item(db_session: AsyncSession):
    ...
    deleted_count = await link_repo.delete_by_item(item2.id)
    assert deleted_count == 2
    await db_session.commit()

    # Verify all links for item2 are gone
    remaining = await link_repo.get_by_item(item2.id)
...
async def test_agent_repository_get_by_project(db_session: AsyncSession):
    ...
    p1_agents = await agent_repo.get_by_project(project1.id)
    assert len(p1_agents) == 2

    p2_agents = await agent_repo.get_by_project(project2.id)
...
async def test_transaction_rollback_link_creation(db_session: AsyncSession):
    ...
    found = await link_repo.get_by_id(link_id)
    assert found is None
...
async def test_complex_query_items_with_links(db_session: AsyncSession):
    ...
    # Traverse graph
    api_links = await link_repo.get_by_item(api.id)
    assert len(api_links) == 2

    # Get implementers
    implementers = await link_repo.get_by_target(api.id)
...
async def test_complex_hierarchy_operations(db_session: AsyncSession):
    ...
    epic_children = await item_repo.get_children(epic.id)
    assert len(epic_children) == 2

    # Get descendants of epic (should include stories and tasks)
    epic_descendants = await item_repo.get_descendants(epic.id)
    assert len(epic_descendants) == 4  # 2 stories + 2 tasks

    # Get ancestors of task1
    task1_ancestors = await item_repo.get_ancestors(task1.id)
