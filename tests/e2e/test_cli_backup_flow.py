...
def test_backup_restore_success(tmp_path):
    ...
    with patch("tracertm.cli.commands.backup.ConfigManager") as cfg, patch(
        "tracertm.cli.commands.backup.DatabaseConnection"
    ) as db:
        cfg_inst = MagicMock()
        ...
        res_restore = runner.invoke(
            app, ["backup", "restore", str(backup_path), "--force"], catch_exceptions=False
        )
...
def test_backup_restore_corrupt_file(tmp_path):
    ...
    with patch("tracertm.cli.commands.backup.ConfigManager") as cfg, patch(
        ...
        res_restore = runner.invoke(
            app, ["backup", "restore", str(corrupt), "--force"], catch_exceptions=False
        )
