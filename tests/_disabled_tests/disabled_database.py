...
class TestDatabaseIntegration:
    ...
    async def test_postgres_connection(self, db_service):
        """Test PostgreSQL connection"""
        assert db_service.postgres is not None

    async def test_postgres_query(self, db_service):
        """Test PostgreSQL query"""
        async with db_service.postgres.acquire() as conn:
            result = await conn.fetchval("SELECT 1")
            assert result == 1

    async def test_valkey_connection(self, db_service):
        """Test Valkey connection"""
        assert db_service.valkey is not None

    async def test_valkey_operations(self, db_service):
