...
async def test_create_and_get_item(async_session):
    ...
    fetched = await repo.get_by_id(item.id, project_id="proj-1")
...
async def test_update_optimistic_lock(async_session):
    ...
    updated = await repo.update(item.id, expected_version=starting_version, status="done")
    assert updated.status == "done"
    assert updated.version == starting_version + 1

    with pytest.raises(ConcurrencyError):
        await repo.update(item.id, expected_version=1, status="todo")
...
async def test_soft_delete_and_restore(async_session):
    ...
    ok = await repo.delete(item.id, soft=True)
    assert ok is True
    # direct fetch without deleted filter
    result = await async_session.execute(
        __import__("sqlalchemy").select(Item).where(Item.id == item.id)
    )
...
async def test_hard_delete_cascades_links(async_session):
    ...
    link = Link(id="link-1", project_id="proj-1", source_item_id=a.id, target_item_id=b.id, link_type="implements", metadata={})
    async_session.add(link)
    await async_session.commit()

    removed = await item_repo.delete(a.id, soft=False)
