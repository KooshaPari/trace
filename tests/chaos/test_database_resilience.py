...
class TestDatabaseConnectionResilience:
    ...
    def test_connect_raises_connection_error_on_engine_failure(self):
        ...
        with patch("tracertm.database.connection.create_engine") as mock_create:
            ...
            mock_engine.connect.side_effect = OperationalError(
                "connection refused", params=None, orig=Exception("conn refused")
            )
    ...
    def test_health_check_returns_error_on_query_failure(self):
        ...
        mock_conn.execute.side_effect = OperationalError(
            "disk I/O error", params=None, orig=Exception("disk I/O error")
        )
    ...
    def test_create_tables_raises_when_not_connected(self):
        """create_tables must raise RuntimeError when engine is None."""
        db = DatabaseConnection("sqlite:///fake.db")
        with pytest.raises(RuntimeError, match="Not connected"):
            db.create_tables()

    def test_close_disposes_engine_and_clears_state(self):
...
class TestTransactionRollbackOnFailure:
    ...
    @pytest.mark.asyncio
    async def test_session_rollback_on_execute_failure(self):
    ...
    @pytest.mark.asyncio
    async def test_flush_failure_triggers_rollback(self):
...
class TestOptimisticLockingUnderFailure:
    ...
    @pytest.mark.asyncio
    async def test_retry_exhaustion_raises_concurrency_error(self):
        ...
        async def always_conflict():
            nonlocal call_count
            call_count += 1
            raise ConcurrencyError(f"version mismatch attempt {call_count}")
    ...
    @pytest.mark.asyncio
    async def test_retry_succeeds_after_transient_conflict(self):
        """update_with_retry must succeed when the conflict resolves before exhaustion."""
        call_count = 0

        async def transient_conflict():
            nonlocal call_count
            call_count += 1
            if call_count < 3:
    ...
    @pytest.mark.asyncio
    async def test_non_concurrency_error_not_retried(self):
