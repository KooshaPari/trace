...
func TestConcurrentEventStorage(t *testing.T) {
	...
	require.Greater(t, successCount, int64(totalEvents*0.95), "95%+ success rate expected")
}
...
func TestConcurrentSearch(t *testing.T) {
	...
	require.Greater(t, successCount, int64(concurrentSearches*0.95))
...
func TestDatabaseConnectionPool(t *testing.T) {
	...
	require.Greater(t, successCount, int64(totalQueries*0.95))
}
...
func BenchmarkGraphTraversal(b *testing.B) {
	...
	for i := 0; i < 50; i++ {
		...
		if i > 0 {
			sourceUUID, _ := pgtype.UUIDFromString(items[i-1])
			targetUUID, _ := pgtype.UUIDFromString(items[i])
