...
async def db_session(postgres_sessionmaker) -> AsyncGenerator[AsyncSession, None]:
...
@pytest_asyncio.fixture
async def nats_jetstream(nats_client) -> JetStreamContext:
...
@pytest_asyncio.fixture
async def event_publisher(nats_client) -> AgentEventPublisher:
