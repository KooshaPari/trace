...
func TestBatchRequest(t *testing.T) {
	...
	assert.Equal(t, testBatchCount, len(batches))
	assert.Equal(t, testBatchSize, len(batches[0]))
	assert.Equal(t, testBatchSize, len(batches[1]))
	assert.Equal(t, testBatchLastSize, len(batches[2]))

	// Test with default batch size
	batches = BatchRequest(texts, 0)
	assert.Equal(t, testDefaultBatchCount, len(batches))
	assert.Equal(t, testDefaultBatchSize, len(batches[0]))
	assert.Equal(t, testDefaultBatchRemainder, len(batches[1]))
}
...
func TestMergeResponses(t *testing.T) {
	...
	assert.Equal(t, 3, len(merged.Embeddings))
...
func TestTokenize(t *testing.T) {
	...
	assert.Equal(t, 0, len(emptyTokens))
}
...
func TestVoyageIntegration(t *testing.T) {
	...
	assert.NotNil(t, resp)
	assert.Equal(t, testEmbeddingsCount, len(resp.Embeddings))
	assert.Equal(t, testDimensionsVoyage, len(resp.Embeddings[0]))
...
func TestOpenRouterIntegration(t *testing.T) {
	...
	require.NoError(t, err)
	assert.NotNil(t, resp)
	assert.Equal(t, testEmbeddingCountSingle, len(resp.Embeddings))
	assert.Equal(t, testDimensionsOpenRouter, len(resp.Embeddings[0]))
}
