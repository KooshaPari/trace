...
func TestLoadConfig_Defaults(t *testing.T) {
	...
	assert.Equal(t, "", cfg.NATSCreds)
	assert.Equal(t, "redis://localhost:6379", cfg.RedisURL)
	assert.Equal(t, "", cfg.UpstashRedisRestURL)
	assert.Equal(t, "", cfg.UpstashRedisRestToken)
	...
	assert.Equal(t, "", cfg.WorkOSClientID)
	assert.Equal(t, "", cfg.WorkOSAPIKey)
}
...
func TestConfig_StructureComplete(t *testing.T) {
	...
	assert.Greater(t, cfg.Embeddings.VoyageDimensions, 0)
	assert.Greater(t, cfg.Embeddings.RateLimitPerMinute, 0)
	assert.Greater(t, cfg.Embeddings.TimeoutSeconds, 0)
	assert.Greater(t, cfg.Embeddings.MaxRetries, 0)
	assert.Greater(t, cfg.Embeddings.MaxBatchSize, 0)
	assert.Greater(t, cfg.Embeddings.IndexerWorkers, 0)
	assert.Greater(t, cfg.Embeddings.IndexerBatchSize, 0)
	assert.Greater(t, cfg.Embeddings.IndexerPollInterval, 0)
}
