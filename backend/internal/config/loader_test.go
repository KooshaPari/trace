...
func setRequiredEnvVars() {
	_ = os.Setenv("DATABASE_URL", requiredDatabaseURL)
	_ = os.Setenv("REDIS_URL", requiredRedisURL)
	_ = os.Setenv("NATS_URL", requiredNATSURL)
}
...
func runAppSettingsCase(t *testing.T, tt appSettingsCase) {
	...
	if tt.appEnvValue != "" {
		_ = os.Setenv("APP_ENV", tt.appEnvValue)
	}
	if tt.portVar != "" && tt.portValue != "" {
		_ = os.Setenv(tt.portVar, tt.portValue)
	}
	if tt.appPortValue != "" {
		_ = os.Setenv("APP_PORT", tt.appPortValue)
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.serviceRoleKey != "" {
				_ = os.Setenv("SUPABASE_SERVICE_ROLE_KEY", tt.serviceRoleKey)
			}
			if tt.serviceKey != "" {
				_ = os.Setenv("SUPABASE_SERVICE_KEY", tt.serviceKey)
			}
...
func TestLoadWithEnvManager_BooleanEdgeCases(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_ = os.Setenv("REDIS_URL", "redis://localhost:6379")
			_ = os.Setenv("NATS_URL", "nats://localhost:4222")
			_ = os.Setenv("RERANK_ENABLED", tt.value)
...
func TestLoadWithEnvManager_EmptyStringHandling(t *testing.T) {
	...
	_ = os.Setenv("VOYAGE_API_KEY", "")
	_ = os.Setenv("WORKOS_CLIENT_ID", "")
	defer clearConfigEnvVars(t)
...
func TestLoadWithEnvManager_NATSCredentialsOptional(t *testing.T) {
	...
	assert.Equal(t, "", cfg.NATSCreds) // Should be empty when not set
}
