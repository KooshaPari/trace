...
func TestAuditLogger_GetEventsByType(t *testing.T) {
	...
	assert.Equal(t, 3, len(successEvents))
...
func TestAuditLogger_Clear(t *testing.T) {
	...
	assert.Equal(t, 10, len(logger.GetEvents()))

	// Clear
	logger.Clear()
	assert.Equal(t, 0, len(logger.GetEvents()))
}
...
func TestAuditLogger_FilterOperations(t *testing.T) {
	...
	// Filter by user
	user1Events := logger.GetEventsByUserID("user-1")
	assert.Equal(t, 5, len(user1Events)) // All events are for user-1

	// Filter by client
	client1Events := logger.GetEventsByClientID("client-1")
	assert.Equal(t, 3, len(client1Events))
}
