...
func (pt *PresenceTracker) Join(clientID, userID, projectID string) *PresenceActivity {
	...
	log.Printf("User %s joined project %s (client: %s)", userID, projectID, clientID)
	return presence
}
...
func (pt *PresenceTracker) Leave(clientID string) {
	...
	log.Printf("User %s left project %s (client: %s)", presence.UserID, presence.ProjectID, clientID)
}
...
func (pt *PresenceTracker) StartViewing(clientID, entityID, entityType string) {
	...
	log.Printf("User %s started viewing %s %s", presence.UserID, entityType, entityID)
}
...
func (pt *PresenceTracker) StartEditing(clientID, entityID, entityType string) {
	...
	log.Printf("User %s started editing %s %s", presence.UserID, entityType, entityID)
}
...
func (pt *PresenceTracker) StopViewing(clientID string) {
	...
	if presence.EntityID != "" {
		...
		log.Printf("User %s stopped viewing %s %s", presence.UserID, presence.EntityType, presence.EntityID)
...
func (pt *PresenceTracker) CleanupStalePresence(timeout time.Duration) int {
	...
	if removed > 0 {
		log.Printf("Cleaned up %d stale presence entries", removed)
	}
