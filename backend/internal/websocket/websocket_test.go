...
func TestPingPong(t *testing.T) {
	...
	assert.True(t, time.Since(lastActive) < websocketTestShortTimeout)
}
...
func TestNoTokenInURL(t *testing.T) {
	...
	assert.True(t, true, "Tokens should not be passed as URL parameters")
}
...
func TestAuthenticationTimeout(t *testing.T) {
	server := httptest.NewServer(websocket.Handler(func(ws *websocket.Conn) {
		_ = ws.SetReadDeadline(time.Now().Add(websocketTestTickDelay))
