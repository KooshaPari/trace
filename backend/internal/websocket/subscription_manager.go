...
func (sm *SubscriptionManager) Subscribe(client *Client, subType SubscriptionType, resourceID string, filters map[string]interface{}) (*Subscription, error) {
	sm.mu.Lock()
	defer sm.mu.Unlock()
	...
	log.Printf("Created subscription: %s (client: %s, type: %s, resource: %s)",
		subID, client.ID, subType, resourceID)

	return sub, nil
}

// SubscribeMultiple creates a subscription for multiple resources
func (sm *SubscriptionManager) SubscribeMultiple(client *Client, resourceIDs []string, filters map[string]interface{}) (*Subscription, error) {
	sm.mu.Lock()
	defer sm.mu.Unlock()
	...
	log.Printf("Created multi-subscription: %s (client: %s, resources: %d)",
		subID, client.ID, len(resourceIDs))
...
func (sm *SubscriptionManager) UnsubscribeAll(clientID string) error {
	...
	log.Printf("Removed all subscriptions for client: %s (%d subscriptions)", clientID, len(subIDsCopy))
	return nil
}
...
func (sm *SubscriptionManager) CleanupStaleSubscriptions(timeout time.Duration) int {
	...
	if removed > 0 {
		log.Printf("Cleaned up %d stale subscriptions", removed)
	}
...
func (sm *SubscriptionManager) MatchesFilter(sub *Subscription, message *Message) bool {
	...
	if eventTypes, ok := sub.Filters["event_types"].([]string); ok {
		if message.Event != nil {
			matched := false
			for _, et := range eventTypes {
				if string(message.Event.EventType) == et {
					matched = true
					break
				}
			}
	...
	if entityTypes, ok := sub.Filters["entity_types"].([]string); ok {
		if message.Event != nil {
			matched := false
			for _, et := range entityTypes {
				if string(message.Event.EntityType) == et {
					matched = true
					break
				}
			}
