...
func TestDefaultBackpressureConfig(t *testing.T) {
	...
	assert.Equal(t, backpressureDropThreshold, config.DropThreshold)
}
...
func TestBackpressureHandler_Reset(t *testing.T) {
	...
	assert.Greater(t, stats["messages_sent"].(int64), int64(0))
	...
	assert.Equal(t, int64(0), stats["messages_dropped"].(int64))
}
...
func TestNewMessageBatcher(t *testing.T) {
	...
	assert.Equal(t, 0, len(batcher.messages))
}
...
func TestNewRateLimiter(t *testing.T) {
	...
	assert.Equal(t, rate, stats["rate"].(float64))
	assert.Equal(t, burst, stats["burst"].(int))
}
...
func TestRateLimiter_GetStats(t *testing.T) {
	...
	stats := limiter.GetStats()
	assert.Equal(t, 10.0, stats["rate"].(float64))
	assert.Equal(t, 5, stats["burst"].(int))
...
func TestPriorityQueue_Enqueue(t *testing.T) {
	...
	assert.Equal(t, 1, stats["high_priority"].(int))
	assert.Equal(t, 1, stats["normal_priority"].(int))
	assert.Equal(t, 1, stats["low_priority"].(int))
}
...
func TestPriorityQueue_GetStats(t *testing.T) {
	...
	stats := queue.GetStats()
	assert.Equal(t, 2, stats["high_priority"].(int))
	assert.Equal(t, 1, stats["normal_priority"].(int))
	assert.Equal(t, 1, stats["low_priority"].(int))
	assert.Equal(t, 4, stats["total"].(int))
}
