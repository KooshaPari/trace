...
func subscribeIntegrationClients(sm *SubscriptionManager, client1 *Client, client2 *Client) {
...
func TestFullIntegration(t *testing.T) {
	...
	subscribeIntegrationClients(sm, client1, client2)
	...
	assert.NoError(t, waitForHubClients(hub, 0))
}
...
func TestConcurrentConnections(t *testing.T) {
	...
	assert.NoError(t, waitForHubClients(hub, numClients))
...
func TestBackpressureHandling(t *testing.T) {
	...
	// Check stats
	stats := bp.GetStats()
	assert.Greater(t, stats["messages_dropped"].(int64), int64(0))
}
...
func TestSubscriptionFiltering(t *testing.T) {
	...
	_, _ = sm.Subscribe(client, SubscribeToProject, "project-1", filters)
...
func TestPresenceCollaboration(t *testing.T) {
	...
	assert.Equal(t, 3, len(viewers)) // All 3 are viewing
	assert.Equal(t, 1, len(editors)) // Only 1 is editing
