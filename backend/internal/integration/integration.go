...
func (es *EventSystem) Start(ctx context.Context) {
	...
	err := es.NATSBus.Subscribe(func(event *events.Event) {
		if err := es.Broadcaster.PublishToProject(event.ProjectID, event); err != nil {
			log.Printf("Failed to publish event to project: %v", err)
		}
	})
	if err != nil {
	...
	log.Println("Event system started successfully")
}
...
func (es *EventSystem) PublishEvent(event *events.Event) error {
	...
	if err := es.Broadcaster.PublishToProject(event.ProjectID, event); err != nil {
		log.Printf("Failed to broadcast to WebSocket clients: %v", err)
...
func (es *EventSystem) CreateSnapshot(entityID string) error {
	...
	store := es.Store.(*events.PostgresEventStore)
	...
	log.Printf("Created snapshot for entity %s at version %d", entityID, count)
	return nil
}
