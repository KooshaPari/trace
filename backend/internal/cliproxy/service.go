...
type ProviderConfig struct {
	Name         string
	Type         string // "anthropic", "openai", "codex"
	ClientID     string
	ClientSecret string
	RedirectURI  string
	BaseURL      string
...
func (service *Service) Run(ctx context.Context) error {
	...
	log.Printf("🔐 CLIProxy service starting on %s", addr)
	...
	select {
	case <-ctx.Done():
		log.Println("🔐 CLIProxy service shutting down...")
		shutdownCtx, cancel := context.WithTimeout(context.Background(), cliProxyShutdownTimeout)
		defer cancel()
		return service.server.Shutdown(shutdownCtx)
	case err := <-errChan:
...
func (service *Service) exchangeCodeForToken(provider *ProviderConfig, _ string) (string, error) {
	...
	log.Printf("Exchanging code for token with provider: %s", provider.Name)
...
func (service *Service) refreshAccessToken(provider *ProviderConfig, _ string) (string, error) {
	...
	log.Printf("Refreshing token with provider: %s", provider.Name)
...
func (service *Service) forwardRequest(
	...
	log.Printf("Forwarding request to provider: %s", provider)
