...
func (s *milestoneService) UpdateMilestoneHealth(ctx context.Context, milestoneID uuid.UUID) error {
	...
	// Calculate health based on progress and risks
	now := time.Now()
	isOverdue := milestone.TargetDate.Before(now)
	...
	case riskScore > milestoneHealthRiskRed || (isOverdue && progressRatio < milestoneProgressRatioRed):
		health = HealthRed
	case riskScore > milestoneHealthRiskYellow || (isOverdue && progressRatio < milestoneProgressRatioYellow) || progress.BlockedItems > 0:
		health = HealthYellow
	default:
		health = HealthGreen
	}
	...
	if progress.TotalItems > 0 {
		...
		if actualProgress < expectedProgress*milestoneProgressBehindFactor {
			factor := RiskFactor{
				Type:        "low_velocity",
				Severity:    "medium",
				Description: fmt.Sprintf("Progress is behind schedule (%.0f%% actual vs %.0f%% expected)", actualProgress*milestonePercentScaleFloat, expectedProgress*milestonePercentScaleFloat),
			}
			riskFactors = append(riskFactors, factor)
