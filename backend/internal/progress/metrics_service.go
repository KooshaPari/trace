...
func (m *metricsService) populateCompletionCounts(
	...
	completedCondition := "status = 'done' AND completed_at IS NOT NULL AND completed_at >= $2"
	var err error
	metrics.CompletedThisWeek, err = m.countWithCondition(ctx, projectID, completedCondition, weekAgo)
	if err != nil {
		return err
	}

	metrics.CompletedLastWeek, err = m.countWithCondition(ctx, projectID, completedCondition, twoWeeksAgo)
	if err != nil {
		return err
	}
...
func (m *metricsService) populateRiskCounts(
	...
	var err error
	metrics.BlockedCount, err = m.countWithCondition(ctx, projectID, "status = 'blocked'")
	if err != nil {
		return err
	}

	metrics.AtRiskCount, err = m.countWithCondition(ctx, projectID, "status = 'at_risk'")
	if err != nil {
		return err
	}

	metrics.OverdueCount, err = m.countWithCondition(
		ctx,
		projectID,
		"target_date < now() AND status != 'done'",
	)
	if err != nil {
		return err
	}
...
func (m *metricsService) countWithCondition(
	...
	//nolint:gosec //G201 - condition is hardcoded string literal, not user input
	query := fmt.Sprintf(`
		SELECT COUNT(*) as count
		FROM items
		WHERE project_id = $1 AND %s AND deleted_at IS NULL
	`, condition)
