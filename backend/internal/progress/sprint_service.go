...
func (s *sprintService) UpdateSprint(ctx context.Context, id uuid.UUID, req *UpdateSprintRequest) (*Sprint, error) {
	...
	if req.Health != nil {
		sprint.Health = *req.Health
	}
	if req.PlannedCapacity != nil {
		sprint.PlannedCapacity = req.PlannedCapacity
	}
	if req.ActualCapacity != nil {
		sprint.ActualCapacity = req.ActualCapacity
	}
	if req.PlannedPoints != nil {
		sprint.PlannedPoints = *req.PlannedPoints
	}
	if req.CompletedPoints != nil {
		sprint.CompletedPoints = *req.CompletedPoints
		sprint.RemainingPoints = sprint.PlannedPoints - sprint.CompletedPoints
	}
