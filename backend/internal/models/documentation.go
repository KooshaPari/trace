...
type Documentation struct {
	...
	Format          string         `gorm:"size:50;not null;index:idx_documentation_format" json:"format"` // markdown, rst, html, plaintext
	Content         string         `gorm:"type:text;not null" json:"content"`
	Summary         *string        `gorm:"type:text" json:"summary,omitempty"`
	ParsedStructure datatypes.JSON `gorm:"type:jsonb;default:'{}'" json:"parsed_structure"` // stores headings, sections, code blocks
...
type DocumentationLink struct {
	...
	TargetDocID  *string   `gorm:"type:uuid;index:idx_documentation_links_target_doc" json:"target_doc_id,omitempty"`   // NULL if external link
	TargetItemID *string   `gorm:"type:uuid;index:idx_documentation_links_target_item" json:"target_item_id,omitempty"` // link to items table
	LinkType     *string   `gorm:"size:50;index:idx_documentation_links_type" json:"link_type,omitempty"`               // references, seeAlso, relatedTo, implements, etc.
	URL          *string   `gorm:"type:text" json:"url,omitempty"`                                                      // external URL if target_doc_id is NULL
...
type DocumentationVersion struct {
	ID            string    `gorm:"primaryKey;type:uuid;default:gen_random_uuid()" json:"id"`
	DocID         string    `gorm:"type:uuid;not null;index:idx_documentation_versions_doc_id;uniqueIndex:idx_documentation_versions_unique,composite:doc_version" json:"doc_id"`
	VersionNumber int       `gorm:"not null;uniqueIndex:idx_documentation_versions_unique,composite:doc_version;index:idx_documentation_versions_version_number,composite:doc_version_sort" json:"version_number"`
	Content       string    `gorm:"type:text;not null" json:"content"`
