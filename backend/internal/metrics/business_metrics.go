...
// BusinessMetricsInstance holds the global business metrics registry.
var BusinessMetricsInstance *BusinessMetrics //nolint:gochecknoglobals // Singleton pattern for global access
...
func InitBusinessMetrics(namespace string) {
	BusinessMetricsInstance = NewBusinessMetrics(namespace)
}
...
func RecordUserSession(userID string, duration time.Duration) {
	if BusinessMetricsInstance != nil {
		BusinessMetricsInstance.UserSessions.WithLabelValues(userID).Inc()
		BusinessMetricsInstance.SessionDuration.WithLabelValues(userID).Observe(duration.Seconds())
	}
}
...
func RecordFeatureUsage(feature, action, userID string) {
	if BusinessMetricsInstance != nil {
		BusinessMetricsInstance.FeatureUsage.WithLabelValues(feature, action, userID).Inc()
	}
}
...
func RecordJourneyStep(journeyType, step, userID string) {
	if BusinessMetricsInstance != nil {
		BusinessMetricsInstance.JourneySteps.WithLabelValues(journeyType, step, userID).Inc()
	}
}
...
func RecordSearchQuery(queryType string, resultCount int) {
	...
	BusinessMetricsInstance.SearchQueries.WithLabelValues(queryType, bucket).Inc()

	if resultCount == 0 {
		BusinessMetricsInstance.EmptySearchResults.WithLabelValues(queryType).Inc()
	}
}
...
func RecordWebVitals(page, metric, rating string, value float64) {
	...
	switch metric {
	case "LCP":
		BusinessMetricsInstance.WebVitalsLCP.WithLabelValues(page, rating).Observe(value)
	case "FID":
		BusinessMetricsInstance.WebVitalsFID.WithLabelValues(page, rating).Observe(value)
	case "CLS":
		BusinessMetricsInstance.WebVitalsCLS.WithLabelValues(page, rating).Observe(value)
	case "TTFB":
		BusinessMetricsInstance.WebVitalsTTFB.WithLabelValues(page, rating).Observe(value)
	case "INP":
		BusinessMetricsInstance.WebVitalsINP.WithLabelValues(page, rating).Observe(value)
	}
}
