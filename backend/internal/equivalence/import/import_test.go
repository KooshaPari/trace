...
func TestJSONImporterParse(t *testing.T) {
	...
	assert.Equal(t, 1, len(parsed.CanonicalConcepts))
}
...
func TestRoundTripExportImport(t *testing.T) {
	...
	assert.Equal(t, 1, len(result.Data.CanonicalConcepts))
	...
	assert.Equal(t, originalConcepts[0].Confidence, imported.Confidence)
}
