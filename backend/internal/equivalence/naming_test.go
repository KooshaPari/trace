...
func TestNamingDetectExactMatch(t *testing.T) {
	...
	assert.Equal(t, 0.7, result.Links[0].Confidence) // Default * similarity
}
...
func TestNamingDetectSnakeCaseMatch(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Len(t, result.Links, 1)
	assert.Equal(t, 0.7, result.Links[0].Confidence)
}
...
func TestNamingDetectExcludesSelfMatch(t *testing.T) {
	...
	// Assert
	require.NoError(t, err)
	assert.Len(t, result.Links, 0) // Should not match self
}
...
func TestNamingDetectPluralization(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			if tt.expectMatch {
				assert.Greater(t, len(result.Links), 0, "expected match for %s -> %s", tt.source, tt.target)
			}
		})
	}
}
...
func TestNamingDetectLowSimilarityNoMatch(t *testing.T) {
	...
	// Assert
	require.NoError(t, err)
	assert.Len(t, result.Links, 0) // No match due to low similarity
}
