...
func (s *searchService) SearchItemsAdvanced(ctx context.Context, query string, searchType search.Type, filters SearchFilters) ([]search.Result, error) {
	var statusSlice []string
	if filters.Status != "" {
		statusSlice = []string{filters.Status}
	}
