...
func (s *AgentServiceImpl) GetAgent(ctx context.Context, id string) (*models.Agent, error) {
	...
	if cached, err := s.getFromCache(ctx, cacheKey); cached != nil && err == nil {
		log.Printf("[CACHE HIT] Retrieved agent %s from cache", id)
		return cached, nil
	}
	...
	go func() {
		...
		defer cancel()
		if err := s.setInCache(cacheCtx, cacheKey, agent); err != nil {
			log.Printf("Warning: Failed to cache agent %s: %v", id, err)
		}
	}()
...
func (s *AgentServiceImpl) ListAgents(ctx context.Context) ([]*models.Agent, error) {
	...
	if cached, err := s.getAgentsFromCache(ctx, cacheKey); cached != nil && err == nil {
		log.Printf("[CACHE HIT] Retrieved agents list from cache")
		return cached, nil
	}
	...
	go func() {
		cacheCtx, cancel := context.WithTimeout(context.Background(), cacheWriteTimeout)
		defer cancel()
		if err := s.setAgentsInCache(cacheCtx, cacheKey, agents); err != nil {
			log.Printf("Warning: Failed to cache agents list: %v", err)
		}
	}()
...
func (s *AgentServiceImpl) UpdateAgent(ctx context.Context, agent *models.Agent) error {
	...
	if s.cache != nil {
		_ = s.cache.Delete(ctx, agentCacheKey)
	}
...
func (s *AgentServiceImpl) GetAgentsByProject(ctx context.Context, projectID string) ([]*models.Agent, error) {
	...
	if cached, err := s.getAgentsFromCache(ctx, cacheKey); cached != nil && err == nil {
		log.Printf("[CACHE HIT] Retrieved project agents from cache")
		return cached, nil
	}
	...
	go func() {
		...
		defer cancel()
		if err := s.setAgentsInCache(cacheCtx, cacheKey, agents); err != nil {
			log.Printf("Warning: Failed to cache project agents: %v", err)
		}
	}()
...
func (s *AgentServiceImpl) GetAgentHealth(ctx context.Context, agentID string) (*AgentHealth, error) {
	...
	if agent.Metadata != nil {
		if err := json.Unmarshal(agent.Metadata, &metadata); err != nil {
			log.Printf("Warning: Failed to parse agent metadata: %v", err)
...
func (s *AgentServiceImpl) invalidateProjectCaches(ctx context.Context, projectID string) {
	...
	_ = s.cache.Delete(ctx, projectAgentsKey)

	// Invalidate full agents list cache
	allAgentsKey := "agents:all"
	_ = s.cache.Delete(ctx, allAgentsKey)
}
...
func (s *AgentServiceImpl) publishAgentEvent(_ context.Context, event *AgentEvent) {
	...
	if err != nil {
		log.Printf("Warning: Failed to marshal agent event: %v", err)
		return
	}

	subject := "agents." + event.EventType
	if err := s.natsConn.Publish(subject, payload); err != nil {
		log.Printf("Warning: Failed to publish agent event %s: %v", event.EventType, err)
	} else {
		log.Printf("[EVENT PUBLISHED] %s for agent %s", event.EventType, event.AgentID)
	}
}
