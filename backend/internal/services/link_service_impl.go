...
func (s *LinkServiceImpl) CreateLink(ctx context.Context, link *models.Link) error {
	...
	go func() {
		eventCtx, cancel := context.WithTimeout(context.Background(), asyncOpTimeout)
		defer cancel()
		if err := s.publishLinkEvent(eventCtx, "link.created", link); err != nil {
	...
	go func() {
		cacheCtx, cancel := context.WithTimeout(context.Background(), asyncOpTimeout)
		defer cancel()
		s.invalidateLinkCache(cacheCtx, link)
	}()
...
func (s *LinkServiceImpl) GetLink(ctx context.Context, id string) (*models.Link, error) {
	...
	if s.cache != nil {
		...
		if err := s.cache.Get(ctx, cacheKey, &cached); err == nil {
			log.Printf("[CACHE HIT] Retrieved link %s from cache", id)
			return &cached, nil
		}
	}
	...
	go func() {
		...
		defer cancel()
		if s.cache != nil {
			if err := s.cache.Set(cacheCtx, cacheKey, link); err != nil {
				log.Printf("Warning: Failed to cache link %s: %v", id, err)
			}
		}
	}()
...
func (s *LinkServiceImpl) UpdateLink(ctx context.Context, link *models.Link) error {
	...
	go func() {
		eventCtx, cancel := context.WithTimeout(context.Background(), asyncOpTimeout)
		defer cancel()
		if err := s.publishLinkEvent(eventCtx, "link.updated", link); err != nil {
...
func (s *LinkServiceImpl) DeleteLink(ctx context.Context, id string) error {
	...
	go func() {
		eventCtx, cancel := context.WithTimeout(context.Background(), asyncOpTimeout)
		defer cancel()
		if err := s.publishLinkDeletedEvent(eventCtx, id); err != nil {
...
func (s *LinkServiceImpl) GetItemDependencies(ctx context.Context, itemID string) (*DependencyGraph, error) {
	...
	graph := s.buildDependencyGraph(itemID, item, dependencies, dependents)
...
func (s *LinkServiceImpl) getDependencyGraphFromCache(ctx context.Context, cacheKey, itemID string) *DependencyGraph {
	...
	if err := s.cache.Get(ctx, cacheKey, &cached); err == nil {
		log.Printf("[CACHE HIT] Retrieved dependencies for item %s from cache", itemID)
		return &cached
	}
...
func (s *LinkServiceImpl) buildDependencyGraph(
	...
	s.populateDependencyItems(graph, dependencies, func(link *models.Link) string { return link.TargetID })
	s.populateDependencyItems(graph, dependents, func(link *models.Link) string { return link.SourceID })
...
func (s *LinkServiceImpl) populateDependencyItems(
	...
	for _, link := range links {
		...
		linkedItem, err := s.itemService.GetItem(context.Background(), itemID)
		if err != nil {
			log.Printf("Warning: Failed to load item %s: %v", itemID, err)
			continue
		}
...
func (s *LinkServiceImpl) cacheDependencyGraph(cacheKey, itemID string, graph *DependencyGraph) {
	go func() {
		...
		if err := s.cache.Set(cacheCtx, cacheKey, graph); err != nil {
			log.Printf("Warning: Failed to cache dependencies for item %s: %v", itemID, err)
		}
	}()
}
...
func (s *LinkServiceImpl) invalidateLinkCache(ctx context.Context, link *models.Link) {
	...
	if err := s.cache.Delete(ctx, linkCacheKey); err != nil {
		log.Printf("Warning: Failed to invalidate link cache %s: %v", linkCacheKey, err)
	}
	...
	if err := s.cache.Delete(ctx, sourceDepsKey); err != nil {
		log.Printf("Warning: Failed to invalidate source dependencies cache %s: %v", sourceDepsKey, err)
	}

	targetDepsKey := "dependencies:" + link.TargetID
	if err := s.cache.Delete(ctx, targetDepsKey); err != nil {
		log.Printf("Warning: Failed to invalidate target dependencies cache %s: %v", targetDepsKey, err)
	}
}
...
func (s *LinkServiceImpl) publishLinkEvent(_ context.Context, eventType string, link *models.Link) error {
	...
	log.Printf("[EVENT PUBLISHED] %s for link %s", eventType, link.ID)
	return nil
}
...
func (s *LinkServiceImpl) publishLinkDeletedEvent(_ context.Context, linkID string) error {
	...
	log.Printf("[EVENT PUBLISHED] link.deleted for link %s", linkID)
	return nil
}
