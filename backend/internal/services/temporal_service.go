...
type VersionDiff struct {
	...
	VersionANum  int        `json:"version_a_number"`
	VersionBNum  int        `json:"version_b_number"`
	Added        []DiffItem `json:"added"`
	Removed      []DiffItem `json:"removed"`
	Modified     []DiffItem `json:"modified"`
	UnchangedCnt int        `json:"unchanged_count"`
...
func (ts *temporalService) MergeBranches(ctx context.Context, mrID string, mergedBy string) error {
	...
	if err == nil {
		...
		if ok {
			...
			_ = ts.branchRepo.Update(ctx, branchTyped)
		}
	}
...
func (ts *temporalService) ComparVersions(ctx context.Context, versionAID, versionBID string) (*VersionDiff, error) {
	...
	diff := &VersionDiff{
		...
		VersionANum: versionATyped.VersionNumber,
		VersionBNum: versionBTyped.VersionNumber,
		Added:       []DiffItem{},
		Removed:     []DiffItem{},
		Modified:    []DiffItem{},
		ComputedAt:  time.Now(),
