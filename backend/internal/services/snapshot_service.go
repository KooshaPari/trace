...
func (s *snapshotServiceImpl) CreateSnapshot(
	...
	defer func() { _ = os.Remove(tarballPath) }() // Cleanup temp file
...
func openTarReader(tarballPath string) (*tar.Reader, func(), error) {
	//nolint:gosec //G304 - tarballPath is from snapshot storage, not arbitrary user input
...
func extractTarFile(tarReader *tar.Reader, header *tar.Header, targetPath string, maxTotalSize int64, totalExtracted *int64) error {
	...
	*totalExtracted += header.Size
	...
	//nolint:gosec //G304 - targetPath is derived from tar archive, validated by safePath
	outFile, err := os.OpenFile(targetPath, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, mode)
	if err != nil {
		return fmt.Errorf("failed to create file: %w", err)
	}

	n, err := io.CopyN(outFile, tarReader, header.Size)
	_ = outFile.Close()
	if err != nil && err != io.EOF {
		return fmt.Errorf("failed to write file content: %w", err)
	}
	if n != header.Size {
		return fmt.Errorf("incomplete file write: expected %d bytes, wrote %d bytes", header.Size, n)
	}
