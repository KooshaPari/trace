...
func Initialize(cfg Config) error {
	...
	if cfg.DSN == "" {
		log.Println("[Sentry] Skipping initialization (no DSN provided)")
		return nil
	}
	...
	if cfg.TracesSampleRate == 0 {
		// Default to 10% in production, 100% in development
	...
	err := sentry.Init(sentry.ClientOptions{
		...
		BeforeSend: func(event *sentry.Event, hint *sentry.EventHint) *sentry.Event {
			...
			if hint.OriginalException != nil {
				...
				// Ignore context canceled errors (expected when requests are cancelled)
				if errMsg == "context canceled" || errMsg == "context deadline exceeded" {
					return nil
				}

				// Ignore EOF errors (usually network issues)
				if errMsg == "EOF" {
					return nil
				}
			}
	...
	log.Printf("[Sentry] Initialized for %s environment (release: %s)", cfg.Environment, cfg.Release)
	return nil
}
