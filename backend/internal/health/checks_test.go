...
func TestNewChecker(t *testing.T) {
	...
	assert.Equal(t, defaultDiskThreshold, hc.diskThreshold)
	assert.Equal(t, defaultMemoryThreshold, hc.memoryThreshold)
}
...
func TestComponentHealth_Helper(t *testing.T) {
	...
	assert.Equal(t, int64(42), ch.Latency)
...
// driveDB returns a nil *sql.DB for test scenarios
func driveDB() *sql.DB {
	return nil
}
