...
func (hc *Checker) checkHTTPService(
	...
	defer func() {
		if err := resp.Body.Close(); err != nil {
			log.Printf("failed to close %s response body: %v", name, err)
		}
	}()
...
func LogReport(report Report) {
	log.Printf("🏥 Health Check: %s (version: %s)", report.Status, report.Version)
	for name, component := range report.Components {
		...
		log.Printf("  %s %s: %s - %s", statusIcon, name, component.Status, component.Message)
	}
}
