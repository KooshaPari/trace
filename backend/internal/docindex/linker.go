...
func (l *Linker) appendCodeRefLinks(ctx context.Context, projectID uuid.UUID, docEntityID uuid.UUID, codeRefs []CodeRef, links []TraceLink) []TraceLink {
...
func (l *Linker) appendFilePathLinks(ctx context.Context, projectID uuid.UUID, docEntityID uuid.UUID, codeRef CodeRef, links []TraceLink) []TraceLink {
	...
	for _, ce := range codeEntities {
		links = append(links, newTraceLink(projectID, docEntityID, ce.ID, "documents", codeRef.Confidence, "File path reference: "+codeRef.FilePath))
	}
	return links
}

func (l *Linker) appendSymbolLinks(ctx context.Context, projectID uuid.UUID, docEntityID uuid.UUID, codeRef CodeRef, links []TraceLink) []TraceLink {
	...
	for _, ce := range codeEntities {
		links = append(links, newTraceLink(projectID, docEntityID, ce.ID, "mentions", codeRef.Confidence*docLinkSymbolConfidenceScale, "Symbol reference: "+codeRef.Name))
	}
	return links
}

func (l *Linker) appendAPIRefLinks(ctx context.Context, projectID uuid.UUID, docEntityID uuid.UUID, apiRefs []APIRef, links []TraceLink) []TraceLink {
...
func newTraceLink(projectID uuid.UUID, docEntityID uuid.UUID, targetID uuid.UUID, linkType string, confidence float64, evidence string) TraceLink {
	return TraceLink{
		ID:         uuid.New(),
		ProjectID:  projectID,
		SourceType: "doc",
