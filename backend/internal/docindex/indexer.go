...
func buildFileWalkFn(req *IndexRequest, patterns []string, files *[]string) fs.WalkDirFunc {
	return func(path string, d fs.DirEntry, err error) error {
		...
		if isExcludedPath(path, d.Name(), req.ExcludePatterns) {
			return nil
		}
		if matchesIncludePattern(d.Name(), patterns) {
...
func (idx *Indexer) indexFile(ctx context.Context, projectID uuid.UUID, filePath string, generateEmbeddings bool, result *IndexResult) error {
	// Validate file path is safe
	if err := isSafeFilePath(filePath); err != nil {
		return fmt.Errorf("unsafe file path: %w", err)
	}
	// Read file content
	//nolint:gosec //G304 - filePath is validated by isSafeFilePath before use
	...
	doc.Links = ExtractInternalLinks(doc.RawContent)
