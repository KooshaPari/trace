...
`
	...
	err := os.WriteFile(docPath, []byte(content), 0o644)
	...
	assert.Greater(t, result.SectionsCreated, 0)
}
...
`
	...
	assert.Greater(t, len(apiRefs), 0)
...
`
	...
	assert.Greater(t, len(codeRefs), 0)

	// Extract API references
	apiRefs := ExtractAPIRefs(content)
	assert.Greater(t, len(apiRefs), 0)
	assert.Equal(t, 3, len(apiRefs))
...
func TestCodeBlockPreservationInRefs(t *testing.T) {
	...
	assert.Greater(t, len(fileRefs), 0)
}
