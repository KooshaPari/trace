...
func (r *codeEntityRepository) Search(ctx context.Context, projectID string, query string, limit, offset int) ([]*models.CodeEntity, error) {
	var entities []*models.CodeEntity
	...
	err := q.Order("created_at DESC").Find(&entities).Error
	return entities, err
}
...
func (r *codeEntityRepository) GetStats(ctx context.Context, projectID string) (*models.CodeIndexStats, error) {
	stats := &models.CodeIndexStats{
		...
		EntitiesByLang: make(map[string]int64),
	}
	...
	if err := r.fillEntitiesByLanguage(ctx, projectID, stats.EntitiesByLang); err != nil {
		return nil, err
	}
...
func (r *codeEntityRelationshipRepository) List(ctx context.Context, filter CodeEntityRelationshipFilter) ([]*models.CodeEntityRelationship, error) {
	var rels []*models.CodeEntityRelationship
...
func (r *codeEntityRelationshipRepository) GetRelationships(ctx context.Context, entityID string) ([]*models.CodeEntityRelationship, error) {
	var rels []*models.CodeEntityRelationship
	err := r.db.WithContext(ctx).
