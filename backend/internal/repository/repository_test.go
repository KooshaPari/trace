...
func TestItemRepositoryList(t *testing.T) {
	...
	runListCase(ctx, t, "combined filters", repo, filterByProjectAndType(testProjectID1, testItemType), func(t *testing.T, result []*models.Item) {
...
func TestItemRepositoryUpdate(t *testing.T) {
	...
	t.Run("update item successfully", func(t *testing.T) {
		...
		updated, _ := repo.GetByID(ctx, "item-update-1")
...
func TestItemRepositoryDelete(t *testing.T) {
	...
	t.Run("delete non-existent item", func(t *testing.T) {
		...
		assert.NoError(t, err) // Soft delete succeeds even if not found
	})
}
