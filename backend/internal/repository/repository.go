...
func (r *itemRepository) List(ctx context.Context, filter ItemFilter) ([]*models.Item, error) {
	...
	// Handle cursor-based pagination if enabled
	if filter.UseCursor && filter.Cursor != nil && *filter.Cursor != "" {
		// Import cursor utilities
		cursor, err := pagination.DecodeCursor(*filter.Cursor)
		if err == nil && cursor != nil {
			// Query items where created_at < cursor_timestamp OR (created_at = cursor_timestamp AND id > cursor_id)
			// This ensures consistent ordering for items created at the same time
			query = query.Where(
		...
		// If cursor decode fails, fall back to normal query (return from beginning)
	} else if filter.Offset > 0 {
		// Traditional offset-based pagination (deprecated, only for backward compatibility)
