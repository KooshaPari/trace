...
func (r *ProgressRepository) UpdateMilestoneRisk(ctx context.Context, milestoneID uuid.UUID, riskScore int, riskFactors []progress.RiskFactor) error {
...
func (r *ProgressRepository) GetSnapshotsByDateRange(ctx context.Context, projectID uuid.UUID, startDate, endDate time.Time) ([]progress.Snapshot, error) {
	query := `
		SELECT id, project_id, snapshot_date, metrics, created_at
		FROM progress_snapshots
