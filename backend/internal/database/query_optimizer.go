...
func (qp *QueryPlan) parseExecutionMetrics() {
	...
	if strings.Contains(qp.Plan, "Execution Time") {
		re := regexp.MustCompile(`"Execution Time": ([0-9.]+)`)
		if matches := re.FindStringSubmatch(qp.Plan); len(matches) > 1 {
			if _, err := fmt.Sscanf(matches[1], "%f", &qp.ExecutionTime); err != nil {
				qp.ExecutionTime = 0
			}
		}
	}

	if strings.Contains(qp.Plan, "Planning Time") {
		re := regexp.MustCompile(`"Planning Time": ([0-9.]+)`)
		if matches := re.FindStringSubmatch(qp.Plan); len(matches) > 1 {
			if _, err := fmt.Sscanf(matches[1], "%f", &qp.PlanningTime); err != nil {
				qp.PlanningTime = 0
			}
		}
	}
