...
func runPerformanceCase(ctx context.Context, t *testing.T, cfg poolPerformanceConfig, databaseURL string) {
	...
	pool, err := InitPoolWithConfig(databaseURL, poolCfg)
...
func TestConnectionPoolStress(t *testing.T) {
	...
	pool, err := InitPoolWithConfig(databaseURL, buildStressPoolConfig())
	require.NoError(t, err)
	defer pool.Close()
...
func BenchmarkQueryExecution(b *testing.B) {
	...
	b.Run("Simple SELECT", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			...
			var result int
			_ = conn.QueryRow(ctx, "SELECT 1").Scan(&result)
			conn.Release()
		}
	})
	...
	b.Run("Parameterized Query", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			...
			var result int
			_ = conn.QueryRow(ctx, "SELECT $1", i).Scan(&result)
			conn.Release()
		}
	})
}
...
func TestHealthCheck(t *testing.T) {
	...
	metrics := monitor.GetCurrentMetrics()
	assert.Greater(t, metrics.TotalConnections, int32(0))
}
