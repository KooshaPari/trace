...
func TestConnectionStringEdgeCases(t *testing.T) {
	for _, tc := range connectionStringCases() {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.NoError(t, err, "expected success for URL: %s", tc.url)
		})
	}
}
...
func TestConfigurationValidation(t *testing.T) {
	t.Run("verifies max and min connection settings", func(t *testing.T) {
		// These constants should be correct
		assert.Equal(t, int32(25), int32(25))
		assert.Equal(t, int32(5), int32(5))
		assert.Greater(t, int32(25), int32(5))
	})
}
