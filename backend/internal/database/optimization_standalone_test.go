...
func TestDefaultOptimizationConfigStandalone(t *testing.T) {
	...
	t.Run("config values are positive", func(t *testing.T) {
		config := DefaultOptimizationConfig()
		assert.Greater(t, config.MaxConnections, int32(0))
		assert.Greater(t, config.MinConnections, int32(0))
...
func TestSlowQueryType(t *testing.T) {
	t.Run("SlowQuery can be instantiated", func(t *testing.T) {
		...
		assert.Equal(t, 5000.0, sq.TotalExecTime)
		assert.Equal(t, 50.0, sq.MeanExecTime)
		assert.Equal(t, 200.0, sq.MaxExecTime)
		assert.Equal(t, 25.0, sq.StddevExecTime)
	})
