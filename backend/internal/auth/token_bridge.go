...
func NewTokenBridge(hsSecret []byte, jwksURL, audience, issuer string) (*TokenBridge, error) {
	...
	if err := tb.RefreshJWKS(); err != nil {
		log.Printf("Warning: Failed to fetch JWKS during initialization: %v", err)
...
func (tb *TokenBridge) ValidateToken(tokenString string) (*WorkOSClaims, error) {
	...
	claims, err := tb.validateRS256Token(tokenString)
	if err == nil {
		return claims, nil
	}

	log.Printf("RS256 validation failed: %v, trying HS256...", err)
...
func (tb *TokenBridge) validateRS256Token(tokenString string) (*WorkOSClaims, error) {
	tb.refreshJWKSIfStale()
	...
	publicKey, err := tb.getPublicKey(kid)
	if err != nil {
		return nil, err
	}
...
func (tb *TokenBridge) refreshJWKSIfStale() {
	tb.jwksMutex.RLock()
	...
	if cacheExpired {
		if err := tb.RefreshJWKS(); err != nil {
			log.Printf("Warning: Failed to refresh JWKS: %v", err)
		}
	}
}

func (tb *TokenBridge) getPublicKey(kid string) (interface{}, error) {
	tb.jwksMutex.RLock()
...
func (tb *TokenBridge) validateHS256Token(tokenString string) (*WorkOSClaims, error) {
	...
	if claims.TokenType != "service" {
		log.Printf("Warning: HS256 token missing type=service claim")
	}
...
func (tb *TokenBridge) CreateBridgeToken(userID, orgID string) (string, error) {
	...
	log.Printf("Created bridge token for user %s (org %s), expires in %d minutes", userID, orgID, bridgeTokenTTLMinutes)
	return tokenString, nil
}

// RefreshJWKS fetches latest public keys from WorkOS
func (tb *TokenBridge) RefreshJWKS() error {
	jwksResp, err := tb.fetchJWKSResponse(context.Background())
	if err != nil {
		return err
	}
...
func (tb *TokenBridge) fetchJWKSResponse(ctx context.Context) (JWKSResponse, error) {
	...
	defer func() {
		if err := resp.Body.Close(); err != nil {
			log.Printf("failed to close JWKS response body: %v", err)
		}
	}()
