...
func (adapter *BridgeAuthAdapter) ValidateToken(ctx context.Context, token string) (*User, error) {
	claims, err := adapter.bridge.ValidateToken(token)
	if err != nil {
	...
	if err != nil {
		log.Printf("Failed to sync user from token: %v", err)
...
func (adapter *BridgeAuthAdapter) syncUserFromClaims(ctx context.Context, claims *WorkOSClaims) (*User, error) {
	...
	if err != nil {
		// User doesn't exist in DB, create basic user from claims
		log.Printf("User %s not found in DB, using claims data", claims.Sub)
		return &User{
			ID:        claims.Sub,
			Email:     claims.Email,
			ProjectID: claims.OrgID,
