...
func NewKitAdapter(clientID, jwtSecret string, db *gorm.DB) *KitAdapter {
	...
	if err != nil {
		log.Printf("Warning: Failed to initialize TokenBridge: %v", err)
	}
...
func (adapter *KitAdapter) ValidateToken(ctx context.Context, token string) (*User, error) {
	...
	claims, err := adapter.tokenBridge.ValidateToken(token)
	if err != nil {
	...
	if err != nil {
		log.Printf("Failed to sync WorkOS profile: %v", err)
	...
	log.Printf("Validated WorkOS token for user: %s (%s)", display, user.ID)
	return user, nil
}
...
func (a *KitAdapter) Close() error {
	if a.tokenBridge != nil {
		if err := a.tokenBridge.Close(); err != nil {
			log.Printf("Error closing token bridge: %v", err)
		}
	}
	log.Println("Closing AuthKit adapter")
	return nil
}
