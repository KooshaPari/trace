...
func (m *StateTokenManager) GenerateStateToken(ctx context.Context) (string, error) {
	...
	if err := m.redis.Set(ctx, redisKey, "1", stateTokenTTL).Err(); err != nil {
...
func (m *StateTokenManager) ValidateStateToken(ctx context.Context, stateToken string) error {
	if stateToken == "" {
	...
	redisKey := redisStateKeyPrefix + stateToken
	storedToken, err := m.redis.Get(ctx, redisKey).Result()
	if err == redis.Nil {
	...
	if subtle.ConstantTimeCompare([]byte(storedToken), []byte("1")) != 1 {
		return fmt.Errorf("state token validation failed")
	}
	...
	if err := m.redis.Del(ctx, redisKey).Err(); err != nil {
		...
		fmt.Printf("warning: failed to delete state token from redis: %v\n", err)
	}
...
func (m *StateTokenManager) RevokeStateToken(ctx context.Context, stateToken string) error {
	if stateToken == "" {
		return fmt.Errorf("state token is required")
	}
...
func (m *StateTokenManager) GenerateStateTokenRequest(ctx context.Context) (*StateTokenRequest, error) {
	token, err := m.GenerateStateToken(ctx)
	if err != nil {
		return nil, err
	}
