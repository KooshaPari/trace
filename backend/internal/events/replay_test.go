...
func TestEventReplayService_ProjectStatsEmpty(t *testing.T) {
	...
	assert.Equal(t, 0.0, stats.AvgEventsPerEntity)
}
...
func BenchmarkReplayToVersion(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _ = service.ReplayToVersion(context.Background(), entityID, 50)
	}
}
...
func BenchmarkGetAuditTrail(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _ = service.GetAuditTrail(context.Background(), entityID)
	}
}
