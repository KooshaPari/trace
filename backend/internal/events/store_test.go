...
func TestStoreMany(t *testing.T) {
	...
	// Verify order
	assert.Equal(t, float64(1), retrieved[0].Data["step"])
	assert.Equal(t, float64(2), retrieved[1].Data["step"])
	assert.Equal(t, float64(3), retrieved[2].Data["step"])
}
...
func TestSnapshot(t *testing.T) {
	...
	assert.Equal(t, float64(5), retrieved.State["count"])
}
...
func TestReplayFromSnapshot(t *testing.T) {
	...
	assert.Equal(t, float64(4), state["count"])
}
