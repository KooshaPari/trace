...
func TestAllRoutes(t *testing.T) {
	...
	for _, tc := range routesToTest {
		t.Run(tc.Name, func(t *testing.T) {
			...
			// Validate response
			assert.Greater(t, status, 0, "Route should respond with status > 0")
	...
	for _, result := range results {
		if result["status"].(int) < http.StatusInternalServerError {
			successCount++
		}
	}
...
func TestRouteHeaderForwarding(t *testing.T) {
	...
	t.Run("Custom headers forwarded", func(t *testing.T) {
		...
		assert.Greater(t, rec.Code, 0, "Route should respond")
