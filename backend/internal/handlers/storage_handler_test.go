...
func (m *MockS3Storage) GeneratePresignedUploadURL(_ context.Context, key string, _ string, expiry time.Duration) (*storage.PresignedURL, error) {
	if key == "" {
...
func TestStorageHandlerDelete(t *testing.T) {
	...
	t.Run("successful delete", func(t *testing.T) {
		...
		// Verify deletion
		exists, _ := mockStorage.Exists(context.Background(), key)
