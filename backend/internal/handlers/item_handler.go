...
func formatPriorityForService(priority *int) models.Priority {
	...
	//nolint:gosec //G115 - priority is a small int value, safe to convert
	return models.Priority(int32(*priority))
}
...
func (h *ItemHandler) buildUpdatedItemFromRequest(c echo.Context) (*models.Item, error) {
	...
	if err != nil {
		return nil, c.JSON(http.StatusNotFound, map[string]string{"error": "Item not found"})
	}
...
func (h *ItemHandler) parseUpdateItemRequest(c echo.Context) (updateItemRequest, error) {
	...
	if err := h.binder.Bind(c, &req); err != nil {
		return req, c.JSON(http.StatusBadRequest, map[string]string{"error": err.Error()})
	}
