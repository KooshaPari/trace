...
func TestHeartbeatTimeout(t *testing.T) {
	...
	assert.False(t, now.Sub(agentRecent.LastHeartbeat) > timeout)
	...
	assert.True(t, now.Sub(agentOld.LastHeartbeat) > timeout)
}
...
func TestVersioningLogic(t *testing.T) {
	...
	assert.Equal(t, nextVersion, expectedVersion)
