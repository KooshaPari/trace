...
func TestLockManager_ReleaseLock_Success(t *testing.T) {
	...
	lock, _ := lm.AcquireLock(ctx, "item-1", "task", "agent-1", LockTypePessimistic)
...
func TestLockManager_GetActiveLocks(t *testing.T) {
	...
	_, _ = lm.AcquireLock(ctx, "item-1", "task", "agent-1", LockTypePessimistic)
...
func TestDistributedCoordinator_StartParticipation(t *testing.T) {
	...
	op := &DistributedOperation{ProjectID: "p1", Type: "batch", TargetItems: []string{}}
	_ = dc.CreateDistributedOperation(ctx, op)
	_ = dc.AssignOperationToAgents(ctx, op.ID, map[string][]string{"agent-1": {}})
