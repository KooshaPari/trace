...
func TestNewCoordinator_NilDB_DoesNotPanic(t *testing.T) {
	c := NewCoordinator(nil, testHeartbeatTimeout)
	require.NotNil(t, c)
	defer c.Shutdown()
	assert.Nil(t, c.TaskQueue.db)
}
...
func TestCoordinator_UnregisterAgent_NilDB(t *testing.T) {
	...
	err := c.UnregisterAgent("ag-1")
	require.NoError(t, err)

	_, err = c.GetAgent("ag-1")
	assert.Error(t, err)
...
func TestCoordinator_Heartbeat_NilDB(t *testing.T) {
	...
	err := c.Heartbeat("ag-1", StatusActive)
	require.NoError(t, err)
...
func TestCoordinator_Heartbeat_AgentNotFound(t *testing.T) {
	...
	err := c.Heartbeat("nonexistent", StatusActive)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "not found")
}
...
func TestCoordinator_ListAgents_EmptyFilter(t *testing.T) {
	...
	_ = c.RegisterAgent(&RegisteredAgent{ID: "1", Name: "a1", ProjectID: "p1"})
	_ = c.RegisterAgent(&RegisteredAgent{ID: "2", Name: "a2", ProjectID: "p2"})
...
func TestCoordinator_ListAgents_ByProject(t *testing.T) {
	...
	_ = c.RegisterAgent(&RegisteredAgent{ID: "1", Name: "a1", ProjectID: "p1"})
	_ = c.RegisterAgent(&RegisteredAgent{ID: "2", Name: "a2", ProjectID: "p1"})
	_ = c.RegisterAgent(&RegisteredAgent{ID: "3", Name: "a3", ProjectID: "p2"})

	list := c.ListAgents("p1")
	assert.Len(t, list, 2)
	for _, a := range list {
...
func TestCoordinator_AssignTask_EnqueuesWhenNoAgent(t *testing.T) {
	...
	assert.Equal(t, 1, c.TaskQueue.QueueLength())
}
...
func TestCoordinator_CompleteTask_Success(t *testing.T) {
	...
	err := c.CompleteTask("ag-1", "t1", result)
	require.NoError(t, err)
...
func TestCoordinator_CompleteTask_AgentNotFound(t *testing.T) {
	...
	err := c.CompleteTask("nonexistent", "t1", &TaskResult{})
	require.Error(t, err)
...
func TestCoordinator_CompleteTask_TaskNotAssignedToAgent(t *testing.T) {
	...
	err := c.CompleteTask("ag-1", "other-task", &TaskResult{})
	require.Error(t, err)
...
func TestCoordinator_FailTask_AgentNotFound(t *testing.T) {
	...
	err := c.FailTask("nonexistent", "t1", "err")
	require.Error(t, err)
	assert.Contains(t, err.Error(), "not found")
}
...
func TestCoordinator_GetNextTask_AgentNotFound(t *testing.T) {
	...
	next, err := c.GetNextTask("nonexistent")
	require.Error(t, err)
	assert.Nil(t, next)
	assert.Contains(t, err.Error(), "not found")
}
...
func TestCoordinator_GetNextTask_DequeuesAndAssigns(t *testing.T) {
	...
	_ = c.TaskQueue.EnqueueTask(task)
...
func TestCoordinator_HasRequiredCapabilities_EmptyRequired(t *testing.T) {
	...
	assert.True(t, c.hasRequiredCapabilities(agent, task))
}
...
func TestCoordinator_HasRequiredCapabilities_Missing(t *testing.T) {
	...
	task := &Task{RequiredCapabilities: []string{"a", "b"}}
	assert.False(t, c.hasRequiredCapabilities(agent, task))
}
...
func TestTaskQueue_DequeueTask_ProjectMismatch(t *testing.T) {
	tq := NewTaskQueue(nil)
	_ = tq.EnqueueTask(&Task{ID: "t1", ProjectID: "p1"})
...
func TestTaskQueue_ListTasks_FilterByProject(t *testing.T) {
	tq := NewTaskQueue(nil)
	_ = tq.EnqueueTask(&Task{ID: "t1", ProjectID: "p1"})
	_ = tq.EnqueueTask(&Task{ID: "t2", ProjectID: "p2"})
...
func TestTaskQueue_QueueStats(t *testing.T) {
	tq := NewTaskQueue(nil)
	_ = tq.EnqueueTask(&Task{ID: "t1", ProjectID: "p1", Priority: PriorityHigh})
	stats := tq.QueueStats()
