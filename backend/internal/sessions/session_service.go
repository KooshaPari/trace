...
const (
	...
	// OAuth access tokens typically expire in 1-3 hours
...
func (s *SessionService) CreateSession(ctx context.Context, req *CreateSessionRequest) (*CreateSessionResponse, error) {
	...
	err = tx.QueryRow(ctx,
		`INSERT INTO sessions (id, user_id, provider, access_token_encrypted, refresh_token_encrypted, token_type, expires_at, created_at, updated_at)
	...
	err = tx.QueryRow(ctx,
		`INSERT INTO oauth_tokens (session_id, provider, access_token_encrypted, refresh_token_encrypted, token_expires_at, created_at, updated_at)
	...
	go s.createOrUpdateUserNode(context.Background(), req.UserID, req.Provider)
...
func (s *SessionService) createOrUpdateUserNode(ctx context.Context, userID uuid.UUID, provider string) {
	...
	fmt.Printf("info: skipping neo4j user node creation for provider %s (requires ProjectContext)\n", provider)
}
