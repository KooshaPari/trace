//nolint:gosec //G301,G306 - test files use standard file permissions for test data
package plugin
...
func TestNewPluginManager(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, 0, len(pm.plugins))
		})
	}
}
	...
	tests := []struct {
		...
		},
		{
			...
			setupFn: func() string {
				tmpDir, _ := os.MkdirTemp("", "plugin-test-*")
				// Create non-.so files
				_ = os.WriteFile(filepath.Join(tmpDir, "readme.txt"), []byte("test"), 0o644)
				_ = os.WriteFile(filepath.Join(tmpDir, "config.json"), []byte("{}"), 0o644)
				return tmpDir
			},
			expectError: false,
		},
...
func TestPluginManagerListPlugins(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.expectedCount, len(plugins))
	...
	tests := []struct {
		...
		},
		{
			...
			setupFn: func() (string, string) {
				...
				_ = os.WriteFile(sourceFile, []byte("package main\n"), 0o644)
...
func TestPluginManagerLoadPluginPathConstruction(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.expectPath, expectedPath)
		})
	}
}
...
func TestPluginManagerMultipleLoadUnload(t *testing.T) {
	...
	// Verify all loaded
	assert.Equal(t, 3, len(pm.ListPlugins()))
	...
	// Verify all unloaded
	assert.Equal(t, 0, len(pm.ListPlugins()))
}
...
func TestPluginManagerListPluginsAfterOperations(t *testing.T) {
	...
	assert.Equal(t, 2, len(pm.ListPlugins()))

	// Unload one
	_ = pm.UnloadPlugin("plugin1")
	assert.Equal(t, 1, len(pm.ListPlugins()))
	...
	// Unload all
	_ = pm.UnloadPlugin("plugin2")
	_ = pm.UnloadPlugin("plugin3")
	assert.Equal(t, 0, len(pm.ListPlugins()))
}
