...
// compressionMetrics tracks compression statistics globally
//
...
func handleETagResponse(c echo.Context, originalWriter http.ResponseWriter, wrappedWriter *responseWriter, status int, ifNoneMatch string) error {
	...
	compressionMetrics.recordCompression(int64(len(wrappedWriter.body)), int64(len(wrappedWriter.body)))

	// Check If-None-Match header
	if ifNoneMatch != "" && matchesETag(ifNoneMatch, etag) {
		compressionMetrics.recordETagHit()
