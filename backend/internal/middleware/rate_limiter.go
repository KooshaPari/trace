...
func (rl *EnhancedRateLimiter) Middleware() echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			// Get the endpoint limit configuration
			...
			if !allowed {
				...
				if retryAfter < 0 {
					retryAfter = retryAfterDefaultSec // Default to 60 seconds
				}
...
func (rl *EnhancedRateLimiter) checkRateLimit(ctx context.Context, key string, limit EndpointLimit) (allowed bool, remaining int, resetTime time.Time, err error) {
...
func (rl *EnhancedRateLimiter) checkRedisRateLimit(ctx context.Context, key string, limit EndpointLimit) (bool, int, time.Time, error) {
	...
	resetTimestamp := time.Unix(values[2].(int64), 0)
