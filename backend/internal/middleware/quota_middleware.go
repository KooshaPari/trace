...
func (m *QuotaMiddleware) applyQuota(c echo.Context) (bool, error) {
	...
	// Get user/API key identifiers
	userID := getUserIdentifier(c)
	apiKeyID := getAPIKeyIdentifier(c)

	if userID == "" && apiKeyID == "" {
		// Anonymous users don't have quotas (rate limits still apply)
		return true, nil
	}

	// Get quotas for this user/key
	...
	// Check daily quota
	dailyUsage, dailyRemaining, dailyResetTime, err := m.checkQuota(ctx, userID, apiKeyID, QuotaPeriodDaily, dailyQuota)
	if err != nil {
		c.Logger().Warnf("Failed to check daily quota: %v", err)
	} else if m.config.AddHeaders {
		m.addQuotaHeaders(c, "Daily", dailyQuota, dailyRemaining, dailyResetTime)
	}

	// Check monthly quota
	monthlyUsage, monthlyRemaining, monthlyResetTime, err := m.checkQuota(ctx, userID, apiKeyID, QuotaPeriodMonthly, monthlyQuota)
	if err != nil {
		c.Logger().Warnf("Failed to check monthly quota: %v", err)
	} else if m.config.AddHeaders {
		m.addQuotaHeaders(c, "Monthly", monthlyQuota, monthlyRemaining, monthlyResetTime)
	}

	// Apply grace percentage
	...
	if dailyUsage >= dailyLimit {
		return false, m.handleQuotaExceeded(c, "daily", dailyResetTime)
	}
	if monthlyUsage >= monthlyLimit {
		return false, m.handleQuotaExceeded(c, "monthly", monthlyResetTime)
	}
...
func (m *QuotaMiddleware) checkQuota(ctx context.Context, userID, apiKeyID string, period QuotaPeriod, limit int64) (usage, remaining int64, resetTime time.Time, err error) {
...
func (m *QuotaMiddleware) incrementUsage(ctx context.Context, userID, apiKeyID string) {
	...
	m.config.Redis.Incr(ctx, dailyKey)
	m.config.Redis.Expire(ctx, dailyKey, dailyTTL)
	...
	m.config.Redis.Incr(ctx, monthlyKey)
	m.config.Redis.Expire(ctx, monthlyKey, monthlyTTL)
}
