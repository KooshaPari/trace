...
func (idx *Indexer) processJob(job IndexJob) {
	...
	default:
		log.Printf("Unknown index operation: %s", job.Operation)
		idx.incrementFailed()
		return
	}
	...
	if err != nil {
		log.Printf("Index job failed (operation=%s, item_id=%s, duration=%v): %v",
			job.Operation, job.ItemID, duration, err)
		idx.incrementFailed()
...
func (idx *Indexer) ReindexAll(ctx context.Context) error {
	...
	for rows.Next() {
		...
		if err := idx.QueueIndex(itemID, priorityReindex); err != nil {
			log.Printf("Error queueing item %s for reindex: %v", itemID, err)
			continue
		}
		count++
	}
	...
	log.Printf("Queued %d items for reindexing", count)
	return nil
}
...
func (idx *Indexer) reportStats() {
	...
	for {
		...
		case <-ticker.C:
			...
			log.Printf("Indexer stats: total=%d, completed=%d, failed=%d, queue=%d, rate=%.2f jobs/sec",
				stats.TotalJobs, stats.CompletedJobs, stats.FailedJobs, stats.QueueSize, stats.ProcessingRate)
		}
	}
}
