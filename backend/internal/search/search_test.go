...
func TestFullTextSearch_Wildcards(t *testing.T) {
	...
	assert.True(t, len(query) < len("authentication"))
}
...
func TestVectorSearch_CosineSimilarity(t *testing.T) {
	...
	assert.Equal(t, 0.92, similarity)
}
...
func TestVectorSearch_L2Distance(t *testing.T) {
	...
	assert.Equal(t, 0.95, similarities[0])
	assert.Equal(t, 0.65, similarities[1])
}
...
func TestVectorSearch_TopK_Results(t *testing.T) {
	...
	topK := allResults[:k]
	assert.Equal(t, 5, len(topK))
	assert.Equal(t, 0.95, topK[0])
	assert.Equal(t, 0.75, topK[4])
}
...
func TestHybridSearch_Reranking(t *testing.T) {
	...
	assert.Equal(t, 2, len(mockRerank.results))
...
func TestFuzzySearch_TypoTolerance(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.typo, func(t *testing.T) {
			...
			assert.NotEqual(t, tc.typo, tc.expected)
		})
	}
}
...
func TestFuzzySearch_EditDistance(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			assert.NotEqual(t, tc.query, tc.expected)
...
func TestSearch_HealthCheck(t *testing.T) {
	...
	err = mock.QueryRow(context.Background(),
		"SELECT EXISTS(SELECT 1 FROM pg_extension WHERE extname = 'pg_trgm'), EXISTS(SELECT 1 FROM pg_extension WHERE extname = 'fuzzystrmatch'), EXISTS(SELECT 1 FROM pg_extension WHERE extname = 'unaccent'), EXISTS(SELECT 1 FROM pg_extension WHERE extname = 'vector')").
