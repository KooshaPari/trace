...
func testConcurrentBatchProcessing(t *testing.T) {
	...
	wg.Wait()
	//nolint:gosec //G115 - test values are small, overflow won't occur
	assert.Equal(t, int32(batchSize*numBatches), processedCount)
}
...
func TestStatisticsCollection(t *testing.T) {
	t.Run("stats map structure", func(t *testing.T) {
		...
		assert.Equal(t, 5, stats["subscriptions"].(int))
		assert.Equal(t, int64(100), stats["in_msgs"].(int64))
	})
...
func TestMessageHandling(t *testing.T) {
	t.Run("event unmarshaling in handler", func(t *testing.T) {
		...
		eventData, _ := json.Marshal(expectedEvent)
