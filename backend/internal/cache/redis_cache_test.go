...
func TestRedisCache_Metrics(t *testing.T) {
	...
	_ = cache.Set(ctx, "metric-test", "value")
	_ = cache.Get(ctx, "metric-test", new(string))
	_ = cache.Get(ctx, "nonexistent", new(string))
...
func TestRedisCache_Performance(t *testing.T) {
	...
	t.Run("Lookup latency <10ms", func(t *testing.T) {
		...
		for i := 0; i < iterations; i++ {
			var result string
			_ = cache.Get(ctx, "perf-test", &result)
		}
	...
	t.Run("Handle 1000+ operations", func(t *testing.T) {
		...
		for i := 0; i < 1000; i++ {
			...
			_ = cache.Set(ctx, key, map[string]interface{}{
				"id":   i,
...
func BenchmarkRedisCache_Get(b *testing.B) {
	...
	ctx := context.Background()
	_ = cache.Set(ctx, "bench", "value")

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		var result string
		_ = cache.Get(ctx, "bench", &result)
	}
}
...
func BenchmarkRedisCache_GetMany(b *testing.B) {
	...
	// Prewarm
	for i := 0; i < 100; i++ {
		_ = cache.Set(ctx, string(rune(i)), "value")
	}
