...
func cacheResponseIfEligible(
	...
	// TODO: SetWithTags and TTL not implemented in base Cache interface - using basic Set for now
...
func invalidateRelatedCaches(_ context.Context, cache Cache, path string) {
	...
	// TODO: InvalidateTags not implemented in base Cache interface
...
// parseCacheControlMaxAge extracts max-age from Cache-Control header
func parseCacheControlMaxAge(cacheControl string) time.Duration {
	...
	for _, part := range parts {
		...
		if strings.HasPrefix(part, "max-age=") {
			...
			var seconds int
			if _, err := fmt.Sscanf(ageStr, "%d", &seconds); err == nil {
...
func ReadThroughCache[T any](
	...
	go func() {
		ctx, cancel := context.WithTimeout(context.Background(), cacheReadThroughTimeout)
		defer cancel()
		// TODO: SetWithTTL not implemented - using basic Set
		if err := cache.Set(ctx, key, result); err != nil {
			log.Printf("cache warm failed for %s: %v", key, err)
		}
	}()
...
func BatchGetWithCache[T any](
	...
	addLoadedBatch[T](cache, ttl, loaded, result)
	return result, nil
}
...
func addLoadedBatch[T any](cache Cache, ttl time.Duration, loaded map[string]T, result map[string]T) {
	for key, item := range loaded {
		result[key] = item

		go func(k string, v T) {
			ctx, cancel := context.WithTimeout(context.Background(), cacheReadThroughTimeout)
			defer cancel()
			// TODO: SetWithTTL not implemented - using basic Set
			if err := cache.Set(ctx, k, v); err != nil {
				log.Printf("cache warm failed for %s: %v", k, err)
			}
		}(key, item)
	}
}
