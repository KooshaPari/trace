...
func (uc *UpstashCache) ping(ctx context.Context) error {
	req, err := http.NewRequestWithContext(ctx, "GET", uc.restURL+"/ping", nil)
	if err != nil {
		return err
	}
	...
	defer func() {
		if err := resp.Body.Close(); err != nil {
			log.Printf("error closing upstash response: %v", err)
		}
	}()
...
func (uc *UpstashCache) Get(ctx context.Context, key string, dest interface{}) error {
	req, err := http.NewRequestWithContext(ctx, "GET", uc.restURL+"/get/"+key, nil)
	if err != nil {
		return err
	}
	req.Header.Set("Authorization", "Bearer "+uc.token)
...
func (uc *UpstashCache) Delete(ctx context.Context, keys ...string) error {
	...
	for _, key := range keys {
		req, err := http.NewRequestWithContext(ctx, "DELETE", uc.restURL+"/del/"+key, nil)
		if err != nil {
			return err
		}
		req.Header.Set("Authorization", "Bearer "+uc.token)
