...
func TestRedisPatternEdgeCases(t *testing.T) {
	...
	t.Run("invalidate pattern with only asterisk", func(t *testing.T) {
		...
		_ = cache.Set(ctx, "temp:1", "val1")
		_ = cache.Set(ctx, "temp:2", "val2")
	...
	t.Run("invalidate complex patterns", func(t *testing.T) {
		...
		_ = cache.Set(ctx, "api:user:123", "val1")
		_ = cache.Set(ctx, "api:user:456", "val2")
		_ = cache.Set(ctx, "api:post:123", "val3")
		_ = cache.Set(ctx, "cache:item:789", "val4")
		...
		var result string
		_ = cache.Get(ctx, "api:user:123", &result)
		assert.Empty(t, result)
	})
	...
	t.Run("invalidate pattern with special chars", func(t *testing.T) {
		_ = t
		_ = cache.Set(ctx, "special:key_with-dash.dot", "value")
...
func TestRedisConcurrentErrorScenarios(t *testing.T) {
	...
	t.Run("concurrent invalidate and set", func(t *testing.T) {
		withErrorTestCache(t, func(ctx context.Context, cache *RedisCache) {
			runRedisConcurrentInvalidateAndSet(ctx, cache)
		})
	})
}
...
func runRedisConcurrentInvalidateAndSet(ctx context.Context, cache *RedisCache) {
	var wg sync.WaitGroup
	numGoroutines := 20
	...
	for i := 0; i < numGoroutines; i++ {
		...
		go func(idx int) {
			...
			_ = cache.Set(ctx, key, idx)
		}(i)

		wg.Add(1)
		go func() {
			defer wg.Done()
			_ = cache.InvalidatePattern(ctx, "test:pattern:*")
		}()
	}
