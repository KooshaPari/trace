...
func cleanupGraphTestDB(_ *testing.T, pool *pgxpool.Pool, projectID string) {
	ctx := context.Background()
	queries := db.New(pool)

	projectUUID, err := uuidutil.StringToUUID(projectID)
	if err == nil {
		_ = queries.DeleteProject(ctx, projectUUID)
	}
	pool.Close()
}
...
func BenchmarkBFSAlgorithms(b *testing.B) {
	...
	// Create test graph
	queries := db.New(pool)
	project, _ := queries.CreateProject(context.Background(), db.CreateProjectParams{
	...
	var startID string
	for i := 0; i < 100; i++ {
		item, _ := queries.CreateItem(context.Background(), db.CreateItemParams{
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _ = graph.BFS(context.Background(), startID, "forward", 0)
	}
}
