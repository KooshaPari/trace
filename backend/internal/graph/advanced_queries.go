	...
	for _, k := range nodeIDs {
		for _, i := range nodeIDs {
			for _, j := range nodeIDs {
				if result.ReachabilityMatrix[i][k] && result.ReachabilityMatrix[k][j] {
					if !result.ReachabilityMatrix[i][j] {
						result.ReachabilityMatrix[i][j] = true
						updateDistance(result.Distance, i, k, j)
					}
				}
			}
		}
