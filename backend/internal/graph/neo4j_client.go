...
func NewNeo4jClient(uri, user, password string) (*Neo4jClient, error) {
	...
	if err != nil {
		closeNeo4jDriver(context.Background(), driver)
...
func (c *Neo4jClient) ExecuteQuery(ctx context.Context, projectCtx *ProjectContext, query string, params map[string]interface{}) (neo4j.ResultWithContext, error) {
...
func (c *Neo4jClient) CreateIndexes(ctx context.Context) error {
	...
	for _, queryset := range queries {
		...
		for _, query := range queryset {
			...
			if err != nil {
				...
				fmt.Printf("Index query failed (will try alternative syntax): %v\n", err)
				success = false
				break
			}
		}
	...
	fmt.Println("Warning: Could not create Neo4j indexes with any syntax variant. Indexes will still work with existing data.")
	return nil
}
