...
func TestBFS_BackwardDirection(t *testing.T) {
	...
	assert.Len(t, result.Nodes, 3)
	assert.Equal(t, 0, len(result.Nodes[testUUIDString(3)].Children))
	assert.Equal(t, 1, len(result.Nodes[testUUIDString(3)].Parents))
}
...
func TestBFS_BothDirection(t *testing.T) {
	...
	assert.Equal(t, 1, len(result.Nodes[testUUIDString(2)].Children))
	assert.Equal(t, 1, len(result.Nodes[testUUIDString(2)].Parents))
}
...
func TestDFS_ForwardTraversal(t *testing.T) {
	...
	assert.Equal(t, 2, len(result.Nodes[startID].Children))
}
...
func TestGenerateKey_Consistency(t *testing.T) {
	...
	assert.True(t, len(key1) > 0, "key should not be empty")
}
...
func TestCachedGraph_CacheStats(t *testing.T) {
	...
	assert.Equal(t, 2, stats["total_entries"])
	assert.Equal(t, 60.0, stats["ttl_seconds"])
}
...
func TestResult_SingleNode(t *testing.T) {
	...
	assert.Empty(t, result.Edges)
	assert.Equal(t, 0, len(result.Nodes[testUUIDString(1)].Children))
}
...
func TestCache_ConcurrentGetSet(t *testing.T) {
	...
	stats := cache.Stats()
	assert.Greater(t, stats["total_entries"], 0)
}
