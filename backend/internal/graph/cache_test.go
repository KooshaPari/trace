...
func TestCacheClear(t *testing.T) {
	...
	stats := cache.Stats()
	totalBefore := stats["total_entries"].(int)
	...
	// Verify empty
	stats = cache.Stats()
	totalAfter := stats["total_entries"].(int)
	if totalAfter != 0 {
...
func TestCacheStatsWithExpiredEntries(t *testing.T) {
	...
	validEntries, _ := stats["valid_entries"].(int)
	expiredEntries, _ := stats["expired_entries"].(int)
...
func TestCachedGraphInvalidateItem(t *testing.T) {
	...
	if stats["total_entries"].(int) != 1 {
		t.Fatalf("expected 1 entry, got %d", stats["total_entries"].(int))
	}
	...
	if stats["total_entries"].(int) != 0 {
		t.Fatalf("expected 0 entries after invalidation, got %d", stats["total_entries"].(int))
	}
}
...
func TestCachedGraphInvalidateProject(t *testing.T) {
	...
	if stats["total_entries"].(int) != 2 {
		t.Fatalf("expected 2 entries, got %d", stats["total_entries"].(int))
	}
...
func TestCachedCacheStats(t *testing.T) {
	...
	if stats["total_entries"].(int) != 1 {
		t.Fatalf("expected 1 entry in stats, got %d", stats["total_entries"].(int))
	}
}
