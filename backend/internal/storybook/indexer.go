...
func (idx *Indexer) attachComponentData(component *LibraryComponent, story *StoryEntry, variants []ComponentVariant, includeScreenshots bool, storyID string) error {
...
func (idx *Indexer) extractVariants(
	...
	// Create a default variant from the story args
	...
	// If parameters contain variations, extract them
	if story.Parameters != nil {
		if variations, ok := story.Parameters["variations"]; ok {
			if variationsMap, ok := variations.(map[string]interface{}); ok {
				for varName, varConfig := range variationsMap {
					if varConfigMap, ok := varConfig.(map[string]interface{}); ok {
						...
						// Extract args from variant config
