...
func TestGenerateMatrix(t *testing.T) {
	...
	// Cleanup
	_, _ = pool.Exec(ctx, `DELETE FROM links WHERE source_id LIKE 'req-%' OR target_id LIKE 'test-%'`)
	_, _ = pool.Exec(ctx, `DELETE FROM items WHERE project_id = $1`, projectID)
}
...
func TestGetCoverageReport(t *testing.T) {
	...
	assert.Greater(t, report.Overall.TotalRequirements, 0)
	...
	// Cleanup
	_, _ = pool.Exec(ctx, `DELETE FROM links WHERE source_id LIKE 'req-c%' OR source_id LIKE 'feat-c%'`)
	_, _ = pool.Exec(ctx, `DELETE FROM items WHERE project_id = $1`, projectID)
}
...
func TestGetItemTraceability(t *testing.T) {
	withMatrixService(t, func(ctx context.Context, pool *pgxpool.Pool, service *MatrixService) {
		...
		_, _ = pool.Exec(ctx, `DELETE FROM links WHERE source_id LIKE 'req-it%'`)
		_, _ = pool.Exec(ctx, `DELETE FROM items WHERE project_id = $1`, projectID)
	})
}
...
func TestValidateCompleteness(t *testing.T) {
	...
	// Cleanup
	_, _ = pool.Exec(ctx, `DELETE FROM links WHERE source_id LIKE 'req-v%'`)
	_, _ = pool.Exec(ctx, `DELETE FROM items WHERE project_id = $1`, projectID)
}
...
func TestGetChangeImpact(t *testing.T) {
	withMatrixService(t, func(ctx context.Context, pool *pgxpool.Pool, service *MatrixService) {
		...
		_, _ = pool.Exec(ctx, `DELETE FROM links WHERE source_id LIKE 'req-imp%' OR source_id LIKE 'feat-imp%'`)
		_, _ = pool.Exec(ctx, `DELETE FROM items WHERE project_id = $1`, projectID)
	})
}
