...
type Infrastructure struct {
	...
	// TODO: Migrate gRPC server to use ServiceContainer
	GraphAnalysis *graph.AnalysisService

	// Repository interfaces - kept for gRPC service
	// TODO: Consider moving to ServiceContainer
...
func InitializeInfrastructure(ctx context.Context, cfg *config.Config) (*Infrastructure, error) {
	...
	// Temporal service is initialized lazily in server.go to avoid circular imports
	infra.TemporalService = nil

	// Python backend clients (AI, SpecAnalytics, Execution, Workflow, Chaos) are created in server and held in ServiceContainer

	// Initialize Graph Analysis Service (requires Neo4j and Cache)
	// Legacy service - kept for gRPC server compatibility
	if infra.Cache == nil {
		return nil, errors.New("cache is required for Graph Analysis Service but is not initialized")
	}
	graphAnalysis := graph.NewAnalysisService(infra.Neo4j.GetDriver(), infra.Cache)
	infra.GraphAnalysis = graphAnalysis

	// Initialize CLI Proxy
	infra.CLIProxy = initCLIProxy()

	log.Println("Infrastructure ready (Postgres, Redis, NATS, Neo4j)")
	return infra, nil
}
...
func initCLIProxy() *cliproxy.Service {
	...
	if err != nil {
		log.Printf("CLIProxy config file not found, trying environment variables: %v", err)
		cliproxyCfg = cliproxy.LoadConfigFromEnv()
	}

	if len(cliproxyCfg.Providers) > 0 {
		if err := cliproxyCfg.Validate(); err != nil {
			log.Printf("⚠️  CLIProxy configuration invalid (skipping): %v", err)
		} else {
			cliproxyService, err := cliproxy.NewService(cliproxyCfg)
			if err != nil {
				log.Printf("⚠️  Failed to initialize CLIProxy service (skipping): %v", err)
			} else {
				log.Printf("🔐 CLIProxy service configured with %d provider(s)", len(cliproxyCfg.Providers))
				return cliproxyService
			}
		}
	} else {
		log.Println("ℹ️  CLIProxy not configured (no OAuth providers found)")
	}
	return nil
}
...
func migrateAuthProfiles(gormDB *gorm.DB) error {
	...
	_ = gormDB.Exec(`DO $$ BEGIN
		IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'profiles' AND column_name = 'workos_id') THEN
			BEGIN
	...
	// GORM may try to DROP constraint uni_profiles_email (without IF EXISTS). Ensure it exists in both schemas so DROP succeeds regardless of search_path.
...
func initTracing(ctx context.Context, cfg *config.Config) (*tracing.TracerProvider, error) {
	if !cfg.TracingEnabled {
		log.Println("ℹ️  Distributed tracing is disabled")
		return nil, nil
	}
...
func (i *Infrastructure) HealthCheck(ctx context.Context) error {
	...
	if err := agents.VerifyAgentTaskBackend(ctx, i.GormDB); err != nil {
	...
	if err := graph.VerifyNeo4jSetup(ctx, i.Neo4j); err != nil {
