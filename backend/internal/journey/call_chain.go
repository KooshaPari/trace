...
func (d *callChainDetector) buildCallGraph(ctx context.Context, items []*models.Item) map[string][]*models.Link {
	...
	for _, item := range items {
		...
		links, _ := d.linkRepo.GetBySourceID(ctx, item.ID)
		if len(links) > 0 {
...
func (d *callChainDetector) findCallChains(
	...
	for entryID := range entryPoints {
		d.dfsCallChain(entryID, graph, visited, &paths, []string{}, []string{})
	}

	// If no entry points found, start from high out-degree nodes
	if len(entryPoints) == 0 {
		outDegrees := make(map[string]int)
		...
		// Find top nodes by out-degree
		topNodes := make([]string, 0, len(outDegrees))
		for nodeID := range outDegrees {
			topNodes = append(topNodes, nodeID)
		}

		// Simple sort for top nodes
		for i := 0; i < len(topNodes) && i < callChainTopNodesLimit; i++ {
		...
		for i := 0; i < len(topNodes) && i < callChainTopNodesLimit; i++ {
			d.dfsCallChain(topNodes[i], graph, visited, &paths, []string{}, []string{})
		}
	}
...
func (d *callChainDetector) dfsCallChain(
	...
	// Prevent infinite recursion and explosion
