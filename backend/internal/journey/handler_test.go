...
func (m *MockJourneyRepository) List(_ context.Context, filter Filter) ([]*DerivedJourney, error) {
	...
	for _, j := range m.journeys {
		...
		if filter.ProjectID != nil && j.ProjectID != *filter.ProjectID {
			continue
		}
		...
		if filter.MinScore > 0 && j.Score < filter.MinScore {
			continue
		}
		result = append(result, j)
	}
...
func TestHandlerInitialization(t *testing.T) {
	...
	err := handler.journeyRepo.Create(ctx, j)
	require.NoError(t, err)
	require.NotEmpty(t, j.ID)

	// Verify we can retrieve it
	retrieved, err := handler.journeyRepo.GetByID(ctx, j.ID)
	require.NoError(t, err)
	require.Equal(t, j.ID, retrieved.ID)
...
func TestListJourneys(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, 2, len(journeys))
...
func TestDeleteJourney(t *testing.T) {
	...
	_ = mockRepo.Delete(ctx, "nonexistent-id")
...
func TestGetJourneyVisualization(t *testing.T) {
	...
	require.Equal(t, 3, len(retrieved.NodeIDs))
	require.Equal(t, 2, len(retrieved.Links))
...
func TestJourneyUpdate(t *testing.T) {
	...
	require.Equal(t, 0.9, updated.Score)
}
