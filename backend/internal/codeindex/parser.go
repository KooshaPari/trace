...
func (parser *Parser) extractTSImports(lines []string) []ImportRef {
	...
	for lineNum, line := range lines {
		if matches := namespaceImport.FindStringSubmatch(line); len(matches) >= matchMinCountImport {
		...
		} else if matches := defaultImport.FindStringSubmatch(line); len(matches) >= matchMinCountImport {
			imports = append(imports, ImportRef{
				ModulePath: matches[2],
		...
		} else if matches := namedImport.FindStringSubmatch(line); len(matches) >= matchMinCountImport {
...
func (parser *Parser) extractGoImports(lines []string) []ImportRef {
	...
	for lineNum, line := range lines {
		...
		if inImportBlock {
			if matches := blockImport.FindStringSubmatch(line); len(matches) >= matchMinCountImport {
				imports = append(imports, ImportRef{
					ModulePath: matches[2],
					Alias:      matches[1],
					Line:       lineNum + 1,
		...
		} else if matches := singleImport.FindStringSubmatch(line); len(matches) >= matchMinCountImport {
			imports = append(imports, ImportRef{
				ModulePath: matches[2],
				Alias:      matches[1],
				Line:       lineNum + 1,
