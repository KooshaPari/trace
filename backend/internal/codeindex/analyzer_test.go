...
export type UserId = string;`
	...
	assert.Greater(t, len(analysis.Symbols), 0, "Should extract symbols")
...
func TestAnalyzer_AnalyzeFile_TypeScriptCalls(t *testing.T) {
  ...
  return transform(user, profile);
	...
	assert.Greater(t, len(analysis.Calls), 0, "Should extract calls")
...
export function loginUser(email: string) {
  ...
  return authenticate(email);
	...
	assert.Greater(t, len(analysis.Annotations), 0)
