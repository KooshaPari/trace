...
func TestMockRepository_DeleteCodeEntity(t *testing.T) {
	...
	// Verify it exists
	retrieved, _ := repo.GetCodeEntity(ctx, entity.ID)
	assert.NotNil(t, retrieved)
...
func TestMockRepository_DeleteByFilePath(t *testing.T) {
	...
	result, _ := repo.FindByFilePath(ctx, projectID, filePath)
	assert.Equal(t, 0, len(result))

	// Verify other file is untouched
	result, _ = repo.FindByFilePath(ctx, projectID, "src/other.ts")
	assert.Equal(t, 1, len(result))
}
