...
export function login() {
  return authenticate();
	...
	assert.Greater(t, len(testAnnotations), 0)
...
export function test() {
  ...
  return service.call();
	...
	annotations := parser.ExtractAnnotations(code)
	require.Greater(t, len(annotations), 0)
...
func TestParser_ExtractTSImports_Named(t *testing.T) {
	...
	require.Greater(t, len(imports), 0)
...
func TestParser_ExtractTSImports_Mixed(t *testing.T) {
	...
	assert.Greater(t, len(imports), 0, "Should find imports")
	assert.Greater(t, modulePaths["react"], 0, "Should have react imports")
	assert.Greater(t, modulePaths["axios"], 0, "Should have axios imports")
}
...
func TestParser_ExtractImports_LanguageDispatch(t *testing.T) {
	...
	assert.Greater(t, len(tsImports), 0, "Should extract TypeScript imports")
	assert.Greater(t, len(goImports), 0, "Should extract Go imports")
	assert.Greater(t, len(pyImports), 0, "Should extract Python imports")
}
