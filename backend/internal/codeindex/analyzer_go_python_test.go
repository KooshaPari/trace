...
interface UserRepository {
	...
	assert.Greater(t, len(analysis.Symbols), 0, "Should extract Go symbols")
}
...
func validateToken(token string) {
	...
	assert.Greater(t, len(analysis.Symbols), 0, "Should extract Go functions")
...
func (c *Calculator) Multiply(n int) int {
	...
	assert.Greater(t, len(analysis.Symbols), 0)
}
...
def validate_password(user: User, password: str) -> bool:
	...
	assert.Greater(t, len(analysis.Symbols), 0, "Should extract Python symbols")
}
...
func TestAnalyzer_GoCalls(t *testing.T) {
	...
	assert.Greater(t, len(analysis.Calls), 0, "Should detect function calls")
}
...
from . import helpers`
	...
	assert.Greater(t, len(analysis.Imports), 0)
}
