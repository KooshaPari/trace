...
class TelemetryMiddleware(Middleware):
    ...
    def _get_payload_size(self, obj: Any) -> int:
    ...
    async def on_tool_call(
        ...
        with self.tracer.start_as_current_span(
            ...
            try:
                ...
                logger.debug(f"[TELEMETRY] {tool_name} completed in {elapsed:.3f}s")
...
class PerformanceMonitoringMiddleware(Middleware):
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        durations = [call["duration"] for call in self._call_history]
        successes = [call for call in self._call_history if call["success"]]
        failures = [call for call in self._call_history if not call["success"]]
