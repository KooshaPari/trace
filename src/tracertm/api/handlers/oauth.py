...
async def _find_existing_credential(
    ...
    Args:
        ...
        credential_scope: Scope (user or project)
...
async def _store_or_update_credential(
    ...
    Args:
        ...
        project_id: Project ID if project-scoped
        provider: OAuth provider
        token_data: Token data from exchange
...
async def _process_oauth_exchange(
    ...
    # Get user info
    user_info = await _get_provider_user_info(params.provider, access_token)
...
async def oauth_callback(
    ...
    credential_scope, project_id, provider = _parse_oauth_state(state)
    if project_id:
