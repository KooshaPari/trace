...
async def check_redis(cache: CacheService) -> dict[str, Any]:
...
async def check_go_backend() -> dict[str, Any]:
...
def get_failed_components(components: dict[str, dict[str, Any]]) -> list[str]:
    ...
    """
    return [name for name, c in components.items() if c.get("status") == "unhealthy"]
