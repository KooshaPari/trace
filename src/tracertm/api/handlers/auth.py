...
async def _get_user_info(user_id: str | None) -> dict[str, Any] | None:
...
def _enrich_admin_user(user: dict[str, Any] | None, user_id: str | None) -> dict[str, Any] | None:
...
async def _get_user_accounts(user_id: str | None, db: AsyncSession) -> list[dict[str, Any]]:
...
async def auth_callback_handler(payload: AuthCallbackPayload) -> dict[str, Any]:
...
async def auth_logout_handler(claims: dict[str, Any]) -> dict[str, Any]:
...
async def device_complete_handler(data: dict[str, Any]) -> dict[str, Any]:
...
def _require_user_code(data: dict[str, Any]) -> str:
...
def _require_auth_code(data: dict[str, Any]) -> str:
...
def _extract_user_info(result: dict[str, Any]) -> dict[str, Any] | None:
    if result.get("user"):
        return result["user"]
