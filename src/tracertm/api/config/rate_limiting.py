...
def enforce_rate_limit(request: Request | None, claims: dict[str, Any] | None) -> None:
    ...
    if should_bypass_for_user(request, claims):  # type: ignore[arg-type]
        return

    # Get rate limit key and limit
    key = get_rate_limit_key(request, claims)  # type: ignore[arg-type]
    resolved_limit = get_resolved_limit(request)  # type: ignore[arg-type]

    # Check if request is within limits
    allowed = check_rate_limit(key, request, resolved_limit)  # type: ignore[arg-type]

    # Raise error if limit exceeded
    if not allowed:
        raise_rate_limit_error(key, request)  # type: ignore[arg-type]
