...
class APIKeyManager:
    ...
    def generate(self, *_: object, **__: object) -> dict[str, str]:
        """Generate an API key."""
        return {"api_key": "sk_test_placeholder"}

    def validate(self, *_: object, **__: object) -> dict[str, bool]:
        """Validate an API key."""
        return {"valid": True}

    def has_scope(self, *_: object, **__: object) -> bool:
        """Return True if the API key has the required scope."""
        return True

    def is_expired(self, *_: object, **__: object) -> bool:
...
async def api_health_check(
    ...
    try:
        ...
        if not healthy:
            status = "unhealthy"
    except (OSError, RedisUnavailableError, RuntimeError, TypeError, ValueError) as exc:
...
async def get_item(
    ...
    try:
        ...
        logger.exception("GET /api/v1/items/%s failed", item_id)
        raise HTTPException(
            status_code=500,
...
async def summarize_items_endpoint(
    ...
    return {
        "project_id": project_id,
        "view": view_upper,
        "status_counts": {str(k): v for k, v in status_counts},
...
async def bootstrap_workflow_schedules(
    ...
    for schedule in schedules:
        ...
        existing = await repo.get_by_schedule_id(cast("str", schedule_dict["schedule_id"]))
        if existing:
            skipped.append({
        ...
        try:
            await service.create_schedule(
                schedule_id=cast("str", schedule_dict["schedule_id"]),
                workflow_name=cast("str", schedule_dict["workflow_name"]),
                args=cast("list[Any]", schedule_dict["args"]),
                cron_expressions=cast("list[str] | None", schedule_dict.get("cron_expressions")),
                interval_seconds=cast("int | None", schedule_dict.get("interval_seconds")),
                timezone=timezone,
            )
            await repo.create_schedule(
                schedule_id=cast("str", schedule_dict["schedule_id"]),
                workflow_name=cast("str", schedule_dict["workflow_name"]),
                schedule_type=cast("str", schedule_dict["schedule_type"]),
                schedule_spec={
                    "cron_expressions": schedule_dict.get("cron_expressions"),
                ...
                description=cast("str | None", schedule_dict.get("description")),
            )
            created.append({
...
async def create_execution(
    ...
    execution_obj = await service.create(
        project_id=project_id,
    ...
    execution = cast("Any", execution_obj)
    await db.commit()
...
async def list_executions(
    ...
    service = ExecutionService(db)
    executions_result = await service.list_by_project(
        project_id,
        status=status,
    ...
    executions = cast("list[Any]", executions_result)
...
async def get_execution(
    ...
    service = ExecutionService(db)
    execution_obj = await service.get(execution_id)
    execution = cast("Any", execution_obj)
...
async def update_webhook(
    ...
    return {
        ...
        "provider": updated_webhook.provider.value,
        "status": updated_webhook.status.value,
        "webhook_secret": updated_webhook.webhook_secret,
        "enabled_events": updated_webhook.enabled_events,
        "callback_url": updated_webhook.callback_url,
        "default_suite_id": updated_webhook.default_suite_id,
        "rate_limit_per_minute": updated_webhook.rate_limit_per_minute,
        "auto_create_run": updated_webhook.auto_create_run,
        "auto_complete_run": updated_webhook.auto_complete_run,
        "verify_signatures": updated_webhook.verify_signatures,
...
async def get_coverage_metrics(
    ...
    for stats in coverage_by_view.values():
        total = stats["total"]
        covered = stats["covered"]
        stats["percentage"] = round(covered / total * 100, 1) if total > 0 else 0.0
