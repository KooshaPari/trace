...
class AgentExecutionWorkflow:
        ...
        for turn in range(self._turn_count, max_turns):
            ...
            try:
                ...
                if turn_result.get("done", False):
                    workflow.logger.info(f"Conversation completed at turn {self._turn_count}")
                ...
                workflow.logger.error(f"Turn {self._turn_count} failed: {e}")
    ...
    async def _create_checkpoint(
        ...
        try:
            ...
            workflow.logger.error(f"Failed to create checkpoint: {e}")
