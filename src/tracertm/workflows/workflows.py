...
class IndexingWorkflow:
    ...
    async def run(self, repository_url: str, branch: str = "main") -> dict[str, Any]:
        ...
        """
        workflow.logger.info(f"Starting indexing workflow for {repository_url} (branch: {branch})")
        ...
        workflow.logger.info(f"Indexing workflow completed: {result.get('status')}")
        return result
...
class AnalysisWorkflow:
    ...
    async def run(self, project_id: str, analysis_type: str = "full") -> dict[str, Any]:
        ...
        """
        workflow.logger.info(f"Starting analysis workflow for project {project_id} (type: {analysis_type})")
        ...
        workflow.logger.info(f"Analysis workflow completed: {result.get('status')}")
        return result
...
class GraphSnapshotWorkflow:
    ...
    async def run(
        ...
        """
        workflow.logger.info(f"Creating snapshot for graph {project_id}/{graph_id}")
        ...
        workflow.logger.info(f"Snapshot created: version {result.get('version')}")
        return result
...
class GraphValidationWorkflow:
    ...
    async def run(self, project_id: str, graph_id: str) -> dict[str, Any]:
        ...
        """
        workflow.logger.info(f"Validating graph {project_id}/{graph_id}")
        ...
        workflow.logger.info(f"Validation completed: {result}")
        return result
...
class GraphExportWorkflow:
    ...
    async def run(self, project_id: str) -> dict[str, Any]:
        ...
        Returns:
            dict: Export data
        """
        workflow.logger.info(f"Exporting graph for project {project_id}")
        ...
        workflow.logger.info(f"Export completed: {result.get('format')}")
        return result
...
class IntegrationSyncWorkflow:
    ...
    async def run(self, limit: int = 50) -> dict[str, Any]:
        ...
        Returns:
            dict: Sync results
        """
        workflow.logger.info(f"Processing {limit} pending integration syncs")
        ...
        workflow.logger.info(f"Sync completed: {result}")
        return result
...
class IntegrationRetryWorkflow:
    ...
    async def run(self, limit: int = 50) -> dict[str, Any]:
        ...
        Returns:
            dict: Retry results
        """
        workflow.logger.info(f"Retrying {limit} failed integration syncs")
        ...
        workflow.logger.info(f"Retry completed: {result}")
        return result
...
class AgentRunWorkflow:
    ...
    async def run(
        ...
        """
        workflow.logger.info(f"Starting agent run for session {session_id} (max_turns={max_turns})")
        ...
        for turn in range(max_turns):
            ...
            if result.get("done", False):
                workflow.logger.info(f"Agent run completed at turn {turn + 1}")
                return result
