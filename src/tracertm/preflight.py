...
def _parse_host_port(url: str, default_port: int | None) -> tuple[str, int | None]:  # noqa: C901
    ...
    port = parsed.port
    if not host and parsed.scheme and ":" in parsed.scheme and parsed.netloc == "":
...
def build_api_checks() -> list[PreflightCheck]:
    return [
        ...
        PreflightCheck("go-backend", os.getenv("GO_BACKEND_URL"), required=True, kind="http"),
